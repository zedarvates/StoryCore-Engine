...
async def run_cinematic_coverage_demo():
    ...
    scene_master_desc = "Un samouraï solitaire en armure de jais sur une falaise écossaise au lever du soleil."
    ...
    if shots:
        ...
        logger.info(
            f"- Structure de scène : {coverage_result.get('structured_shot_list', {}).get('status', 'OK')}"
        )
    else:
