...
def test_identity_stability():
    ...
    if create_res.status_code != 201:
        print(
            f"Failed to create identity: {create_res.status_code} - {create_res.text}"
        )
        return
    ...
    extract_res = requests.post(
        f"{API_URL}/identity/{identity_id}/extract", json=extract_data, headers=headers
    )
    if extract_res.status_code != 200:
        if "No such file or directory" in extract_res.text:
            print(
                "   Note: Extraction failed because image doesn't exist, but we proceed for demonstration."
            )
        else:
            print(
                f"Failed to extract/lock: {extract_res.status_code} - {extract_res.text}"
            )
            # Proceed anyway if we want to test apply (though it might fail if locked bit is false)
    else:
        locked_identity = extract_res.json()
        print("   SUCCESS: Identity is now LOCKED.")
        print(
            f"   Detected Face: {locked_identity['visual_attributes'].get('face_shape', 'N/A')}"
        )
    ...
    grid_res = requests.post(
        f"{API_URL}/character/grid/generate", json=grid_data, headers=headers
    )

    if grid_res.status_code != 200:
        print(
            f"   INFO: Consistency grid call failed (expected if ComfyUI not running): {grid_res.status_code}"
        )
    else:
        grid_bundle = grid_res.json()
        print("   SUCCESS: Consistency Grid generated.")
    ...
    requests.put(
        f"{API_URL}/identity/{identity_id}",
        json={
            "visual_attributes": {"face_shape": "oval", "extraction_confidence": 0.9},
            "is_locked": True,
        },
        headers=headers,
    )
    ...
    apply_res = requests.post(
        f"{API_URL}/identity/{identity_id}/apply", json=scene_data, headers=headers
    )
    ...
    if apply_res.status_code != 200:
        ...
        print("   SUCCESS: Prompt specialized for consistent character.")
