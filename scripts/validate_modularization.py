...
class ModularizationValidator:
    ...
    def validate_directory_structure(self):
        ...
        if missing_dirs:
            ...
            self.results.append(
                ValidationResult(
                    "Directory Structure", True, "All required directories exist"
                )
            )
            ...
            try:
                ...
                if line_count > max_lines:
                    oversized_modules.append(
                        (py_file.relative_to(self.project_root), line_count)
                    )
    ...
    def validate_imports(self):
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                if (
                    "from storycore_cli import" in content
                    and "src.storycore_cli" not in content
                ):
        ...
        if problematic_imports:
            ...
            self.results.append(
                ValidationResult(
                    "Import Structure", True, "All imports follow modular structure"
                )
            )
    ...
    def validate_documentation(self):
        ...
        if undocumented_handlers:
            ...
            self.results.append(
                ValidationResult(
                    "Documentation", True, "All handlers have documentation"
                )
            )
    ...
    def validate_test_structure(self):
        ...
        if missing_test_dirs:
            ...
            handler_files = [
                f.stem
                for f in self.handlers_dir.glob("*.py")
                if f.name != "__init__.py"
            ]
            test_files = (
                [
                    f.stem.replace("test_", "").replace("_handler", "")
                    for f in (unit_tests / "handlers").glob("test_*.py")
                ]
                if (unit_tests / "handlers").exists()
                else []
            )
            ...
            if missing_tests:
                ...
                self.results.append(
                    ValidationResult(
                        "Test Structure", True, "Test structure is complete"
                    )
                )
...
def main():
    ...
    parser.add_argument(
        "--export", type=Path, help="Export validation report to JSON file"
    )
