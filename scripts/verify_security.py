...
from api_server import verify_token, create_access_token
...
def check_environment_configuration() -> Dict[str, Any]:
        ...
        results["checks"].append(
            {
                "name": "JWT Secret Key",
                "status": "fail",
                "message": "STORYCORE_JWT_SECRET_KEY not set",
            }
        )
        ...
        results["checks"].append(
            {
                "name": "JWT Secret Key",
                "status": "fail",
                "message": "JWT secret key uses default placeholder",
            }
        )
        ...
        results["checks"].append(
            {
                "name": "JWT Secret Key",
                "status": "warn",
                "message": "JWT secret key is shorter than recommended (32+ chars)",
            }
        )
    ...
    if redis_url:
        results["checks"].append(
            {"name": "Redis URL", "status": "pass", "message": "Redis URL configured"}
        )
        ...
        try:
            ...
            results["issues"].append(
                "Redis must be available for production rate limiting"
            )
            results["status"] = "fail"
    else:
        results["checks"].append(
            {
                "name": "Redis URL",
                "status": "warn",
                "message": "Redis URL not configured - using memory storage",
            }
        )
        results["issues"].append(
            "Redis recommended for production rate limiting and sessions"
        )
    ...
    if (
        trusted_hosts
        and "localhost" not in trusted_hosts
        and "127.0.0.1" not in trusted_hosts
    ):
        results["checks"].append(
            {
                "name": "Trusted Hosts",
                "status": "pass",
                "message": "Trusted hosts configured (no localhost in production)",
            }
        )
    else:
        results["checks"].append(
            {
                "name": "Trusted Hosts",
                "status": "warn",
                "message": "Trusted hosts configuration may allow localhost",
            }
        )
        ...
        results["checks"].append(
            {
                "name": "Token Creation",
                "status": "pass",
                "message": "JWT token creation successful",
            }
        )
            ...
            results["checks"].append(
                {
                    ...
                    "status": "pass",
                    "message": "JWT token verification successful",
                }
            )
        else:
            results["checks"].append(
            ...
            results["checks"].append(
                {
                    "name": "Invalid Token Handling",
                    "status": "fail",
                    "message": "Invalid tokens not properly rejected",
                }
            )
            results["status"] = "fail"
        ...
        results["checks"].append(
            {
                ...
                "status": "fail",
                "message": f"JWT operations failed: {e}",
            }
        )
        results["status"] = "fail"
        ...
        results["checks"].append(
            {
                ...
                "status": "pass",
                "message": "Session cookies marked secure",
            }
        )
    else:
        results["checks"].append(
            {
                "name": "Session Secure Flag",
                "status": "fail",
                "message": "Session cookies not marked secure",
            }
        )
        results["status"] = "fail"
        ...
        results["checks"].append(
            {
                ...
                "status": "pass",
                "message": "Session cookies marked HttpOnly",
            }
        )
    else:
        results["checks"].append(
            {
                "name": "Session HttpOnly Flag",
                "status": "warn",
                "message": "Session cookies not marked HttpOnly",
            }
        )
        ...
        results["checks"].append(
            {
                ...
                "status": "pass",
                "message": "Session cookies use strict SameSite policy",
            }
        )
    else:
        results["checks"].append(
            {
                "name": "Session SameSite",
                "status": "warn",
                "message": f"Session cookies use {session_samesite} SameSite policy",
            }
        )
...
def check_rate_limiting() -> Dict[str, Any]:
    ...
    requests_per_minute = int(
        os.getenv("STORYCORE_RATE_LIMIT_REQUESTS_PER_MINUTE", "60")
    )
        ...
        results["checks"].append(
            {
                ...
                "status": "pass",
                "message": f"Rate limit set to {requests_per_minute} requests/minute",
            }
        )
    else:
        results["checks"].append(
    ...
    if burst_limit > 0:
        results["checks"].append(
            {
                "name": "Rate Limit Burst Limit",
                "status": "pass",
                "message": f"Burst limit set to {burst_limit}",
            }
        )
    else:
        results["checks"].append(
            ...
            results["checks"].append(
                {
                    ...
                    "status": "pass",
                    "message": "Redis available for rate limiting",
                }
            )
        except Exception as e:
            results["checks"].append(
        ...
        results["checks"].append(
            {
                "name": "Rate Limit Storage",
                "status": "warn",
                "message": "Using memory storage for rate limiting",
            }
        )
...
def main():
    ...
    parser.add_argument(
        "--comprehensive", action="store_true", help="Run comprehensive security checks"
    )
    parser.add_argument(
    ...
    if args.fix and not success:
        ...
        print(
            "   Automatic fixes not yet implemented. Please manually configure the issues above."
        )
