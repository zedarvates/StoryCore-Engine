    ...
    def check_system_resources(self) -> Dict[str, float]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Erreur lors de la récupération de l'utilisation du GPU : {e}"
                )

            logger.debug(
                f"System resources - CPU: {cpu_usage}%, Memory: {memory_usage}%, GPU: {gpu_usage}%"
            )

            return {"cpu": cpu_usage, "memory": memory_usage, "gpu": gpu_usage}
        except Exception as e:
    ...
    def check_model_performance(self) -> Dict[str, bool]:
        ...
        for model_type in self.config["models"]["paths"].keys():
            ...
            try:
                ...
                logger.error(
                    f"Erreur lors de la vérification du modèle {model_type} : {e}"
                )
                model_status[model_type] = False
    ...
    def monitor(self):
        ...
        try:
            while True:
                ...
                logger.info(
                    f"Utilisation des ressources - CPU: {resources['cpu']}%, Memory: {resources['memory']}%, GPU: {resources['gpu']}%"
                )
                ...
                for alert_type in triggered_alerts:
                    if not self.alert_states[alert_type]:
                        message = (
                            f"Seuil dépassé pour {alert_type}: {resources[alert_type]}%"
                        )
                ...
                for alert_type in self.alert_states:
                    if (
                        ...
                        and resources[alert_type]
                        < self.monitoring_config["alerts"][f"threshold_{alert_type}"]
                    ):
