from backend.story_generation_service import StoryGenerationService, ProductionMode, StoryGenre, StoryStructure
import json
import os
...
def generate_markdown_report(story, output_path):
    ...
    for prop in story.props:
        ...
        owner = "Environnement" if not prop.owner_id else next((c['nom'] for c in story.characters if c['id'] == prop.owner_id), "Inconnu")
    ...
    for i, scene in enumerate(story.scenes):
        ...
        md += f"**INT/EXT - {scene.location.upper()} - {scene.time_of_day.upper()}**\n\n"
...
if __name__ == "__main__":
    ...
    story = asyncio.run(service.generate_story(
