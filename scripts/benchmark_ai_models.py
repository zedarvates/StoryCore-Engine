...
class ModelBenchmark:
    ...
    def benchmark_super_resolution(self, low_res_img: Image.Image, num_runs: int = 3):
        ...
        reference = low_res_img.resize(
            (low_res_img.width * 4, low_res_img.height * 4), Image.BICUBIC
        )
        result_np = np.array(result)
        ...
        real_esrgan = RealESRGAN(
            scale=4, device=self.device, half_precision=(self.device == "cuda")
        )
    ...
    def benchmark_interpolation(
        self, frame0: Image.Image, frame1: Image.Image, num_runs: int = 3
    ):
    ...
    def benchmark_quantization(self, test_img: Image.Image):
        ...
        results = quantizer.benchmark_quantization(
            model, quantized_model, test_input, num_iterations=10
        )
        ...
        if self.device == "cuda":
            ...
            results_fp16 = quantizer_fp16.benchmark_quantization(
                model.cuda(), fp16_model, test_input_cuda, num_iterations=10
            )
...
def main():
    ...
    parser.add_argument(
        "--skip-style", action="store_true", help="Skip style transfer benchmarks"
    )
    parser.add_argument(
        "--skip-sr", action="store_true", help="Skip super resolution benchmarks"
    )
    parser.add_argument(
        "--skip-interp", action="store_true", help="Skip interpolation benchmarks"
    )
    parser.add_argument(
        "--skip-quant", action="store_true", help="Skip quantization benchmarks"
    )
