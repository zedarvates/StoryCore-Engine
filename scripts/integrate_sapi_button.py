...
def generate_dialogues():
    ...
    try:
        ...
        subprocess.run(
            ["python", os.path.join("scripts", "generate_dialogue.py")], check=True
        )

        messagebox.showinfo("Succès", "Génération des dialogues terminée avec succès !")
    except subprocess.CalledProcessError as e:
        messagebox.showerror(
            "Erreur", f"Erreur lors de la génération des dialogues : {e}"
        )
...
def main():
    ...
    generate_button = tk.Button(
        root, text="Générer les dialogues", command=generate_dialogues
    )
