...
class ModelDownloader:
    ...
    def download_model(self, model: ModelInfo, progress: bool = True) -> bool:
        ...
        try:
                ...
                with open(temp_path, "wb") as f:
                    while True:
                        ...
                        if progress and total_size > 0:
                            ...
                            print(
                                f"\r    Progress: {percent:.1f}% ({downloaded // (1024 * 1024)}/{total_size // (1024 * 1024)} MB)",
                                end="",
                            )
            ...
            logger.info(
                f"  ✓ Download complete: {output_path.stat().st_size / (1024 * 1024):.1f} MB"
            )
    ...
    def download_all(self, max_workers: int = 1) -> dict:
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                list(executor.map(self.download_model, missing))
        ...
        logger.info(
            f"  Already existed: {len(MODELS) - len(self.downloaded) - len(self.failed)}"
        )
        ...
        for model in MODELS:
            ...
            if model_path.exists():
                ...
                if abs(size_mb - expected_mb) / expected_mb < 0.1:
                    ...
                    logger.warning(
                        f"  ⚠️  {model.name}: {size_mb:.1f} MB (expected ~{expected_mb} MB)"
                    )
                    all_valid = False
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download ComfyUI models for StoryCore"
    )
    parser.add_argument(
        "--check", action="store_true", help="Check existing models only"
    )
