...
class ModelDeployer:
    ...
    def __init__(self, config_path: str = "config/production_config.yaml"):
        ...
        self.update_interval = self._parse_interval(
            self.config["models"]["update"]["interval"]
        )
    ...
    def backup_models(self) -> bool:
        ...
        try:
            ...
            if disk_usage.free < total_size * 2:  # Besoin d'au moins 2x l'espace
                logger.error(
                    f"Espace disque insuffisant pour la sauvegarde. Nécessaire: {total_size_mb:.2f} Mo, Disponible: {disk_usage.free / (1024 * 1024):.2f} Mo"
                )
                return False

            logger.info(
                f"Début de la sauvegarde des modèles. Taille totale: {total_size_mb:.2f} Mo"
            )
            ...
            logger.info(
                f"Sauvegarde des modèles terminée dans {backup_dir} (Taille: {total_size_mb:.2f} Mo)"
            )
            return True
        except Exception as e:
    ...
    def deploy_model(self, model_type: str, source_path: str) -> bool:
        ...
        try:
            ...
            if os.path.exists(dest_path):
                logger.warning(
                    f"Le modèle {model_type} existe déjà. Sauvegarde en cours..."
                )
        ...
        try:
            ...
            for model_type, source_path in model_sources.items():
                if not self.deploy_model(model_type, source_path):
                    logger.error(
                        f"Échec du déploiement du modèle {model_type}. Rollback en cours..."
                    )
    ...
    def check_model_integrity(self, model_type: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de la vérification de l'intégrité du modèle {model_type} : {e}"
            )
            return False
