...
suggestions = lib.suggest_for_scene(
    ["montagne", "rocher", "alpin"], SceneContext.EXTERIOR
)
...
print(
    f"  rock_medium code: {len(code_rock)} chars, has place_rock_medium: {'place_rock_medium' in code_rock}"
)

code_spr = placer.generate_placement_code("sprite_grass_ground", count=5, seed=99)
print(
    f"  sprite_grass_ground code: {len(code_spr)} chars, has TRACK_TO: {'TRACK_TO' in code_spr}"
)
...
auto_path = placer.suggest_and_place(
    ["foret", "brume", "nature"], output_path="./exports/blender/auto_foret.py"
)
print(
    f"  Auto-placement foret: {Path(auto_path).name} ({Path(auto_path).stat().st_size} bytes)"
)
...
print(
    f"  Auto-placement desert: {Path(auto_path2).name} ({Path(auto_path2).stat().st_size} bytes)"
)

print()
print("=== TEST 3: ReferenceRenderer ===")
from blender_projection.reference_renderer import ReferenceRenderer
