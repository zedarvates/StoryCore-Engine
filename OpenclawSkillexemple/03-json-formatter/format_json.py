...
def to_yaml(data: Any, indent: int = 2) -> str:
    ...
    try:
        ...
        return yaml.dump(
            data, default_flow_style=False, allow_unicode=True, indent=indent
        )
    except ImportError:
...
Exemples:
  ...
  echo '{"name":"test"}' | %(prog)s - --format pretty
    ...
    parser.add_argument("file", help="Fichier JSON à traiter (ou '-' pour stdin)")
    ...
    parser.add_argument(
        "--validate", "-v", action="store_true", help="Valider le JSON uniquement"
    )

    parser.add_argument(
        "--to-yaml", "-y", action="store_true", help="Convertir en format YAML"
    )
    ...
    parser.add_argument(
        "--sort-keys", "-s", action="store_true", help="Trier les clés alphabétiquement"
    )
