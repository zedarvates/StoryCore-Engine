...
def generate_qr_api(
    ...
    """
    if verbose:
        print("[INFO] Génération via API QRServer...")
    ...
    try:
        ...
        request = urllib.request.Request(
            url, headers={"User-Agent": "OpenClaw-QRGenerator/1.0"}
        )
    ...
    try:
        ...
        if format.lower() == "svg":
            ...
            img = qr_img.make_image(
                fill_color=f"#{color}", back_color=f"#{bgcolor}", image_factory=factory
            )
...
Exemples:
  ...
  %(prog)s "https://openclaw.ai" --format svg --output qr.svg
    ...
    parser.add_argument("data", help="Le contenu à encoder dans le QR code")
    ...
    if not validate_hex_color(args.color):
        print(
            f"Erreur: Couleur invalide '{args.color}'. Utiliser le format RRGGBB (ex: FF0000)",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not validate_hex_color(args.bgcolor):
        print(
            f"Erreur: Couleur de fond invalide '{args.bgcolor}'. Utiliser le format RRGGBB (ex: FFFFFF)",
            file=sys.stderr,
        )
        sys.exit(1)
