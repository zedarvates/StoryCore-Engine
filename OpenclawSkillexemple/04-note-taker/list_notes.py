...
def filter_by_category(
    notes: List[Dict[str, Any]], category: str
) -> List[Dict[str, Any]]:
    """
...
def filter_by_tag(notes: List[Dict[str, Any]], tag: str) -> List[Dict[str, Any]]:
    ...
    tag_lower = tag.lower()
    return [
        note for note in notes if tag_lower in [t.lower() for t in note.get("tags", [])]
    ]


def sort_notes(
    notes: List[Dict[str, Any]], reverse: bool = True
) -> List[Dict[str, Any]]:
    """
...
def format_table(notes: List[Dict[str, Any]]) -> str:
    ...
    for note in notes:
        ...
        line = (
            ...
            + "|".join(
                f" {v:<{widths[i]}} "
                for i, v in enumerate([title, category, date, tags])
            )
            + "|"
        )
        lines.append(line)
...
def get_statistics(notes: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Returns:
        Statistiques
    """
    stats = {"total": len(notes), "categories": {}, "tags": {}}
...
Exemples:
  ...
  %(prog)s --stats
    ...
    parser.add_argument("--tag", help="Filtrer par tag")
