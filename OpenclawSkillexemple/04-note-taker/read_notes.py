...
def find_notes_by_category(
    index: Dict[str, Any], category: str
) -> List[Dict[str, Any]]:
    """
...
def display_note(
    note: Dict[str, Any], content: str, show_metadata: bool = True
) -> None:
    """
...
Exemples:
  ...
  %(prog)s -c ideas -l
    ...
    parser.add_argument("-t", "--title", help="Rechercher par titre")

    parser.add_argument("-f", "--file", help="Nom du fichier spécifique")
    ...
    parser.add_argument(
        "--no-metadata", action="store_true", help="Ne pas afficher les métadonnées"
    )
