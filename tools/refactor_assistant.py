...
def suggest_refactoring_strategy(structure: Dict[str, Any], file_path: str) -> None:
    ...
    if structure["classes"]:
        ...
        for cls in structure["classes"]:
            print(
                f"   {cls['name']}: {cls['method_count']} methods (line {cls['line']})"
            )
        print()
    ...
    if len(structure["classes"]) == 1 and structure["classes"][0]["method_count"] > 15:
        ...
        if len(public_methods) > 8:
            print(
                f"      - Create 'core.py' for main public methods ({len(public_methods)} methods)"
            )
        if len(private_methods) > 5:
            print(
                f"      - Create 'utils.py' for private utilities ({len(private_methods)} methods)"
            )
        ...
        processors = [
            ...
            if any(
                word in m["name"].lower() for word in ["process", "handle", "execute"]
            )
        ]
        generators = [
            m
            for m in cls["methods"]
            if any(
                word in m["name"].lower() for word in ["generate", "create", "build"]
            )
        ]

        if validators:
            print(
                f"      - Create 'validators.py' for validation methods ({len(validators)} methods)"
            )
        if processors:
            print(
                f"      - Create 'processors.py' for processing methods ({len(processors)} methods)"
            )
        if generators:
            print(
                f"      - Create 'generators.py' for generation methods ({len(generators)} methods)"
            )

    elif len(structure["classes"]) > 3:
        print(
            f"   1. Group {len(structure['classes'])} classes into separate modules by responsibility"
        )
        for cls in structure["classes"]:
    ...
    if len(structure["functions"]) > 15:
        print(
            f"   2. Group {len(structure['functions'])} standalone functions by purpose:"
        )
        ...
        utility_funcs = [
            ...
            if any(word in f["name"].lower() for word in ["helper", "util", "format"])
        ]
        validation_funcs = [
            f for f in structure["functions"] if "valid" in f["name"].lower()
        ]

        if utility_funcs:
            print(
                f"      - Move utility functions to 'utils.py' ({len(utility_funcs)} functions)"
            )
        if validation_funcs:
            print(
                f"      - Move validation functions to 'validators.py' ({len(validation_funcs)} functions)"
            )

    if len(structure["constants"]) > 10:
        print(
            f"   3. Extract {len(structure['constants'])} constants to 'constants.py'"
        )
    ...
    print("   ├── __init__.py          # Public interface")
    print("   ├── core.py              # Main logic")

    if (
        any("valid" in cls["name"].lower() for cls in structure["classes"])
        or len([f for f in structure["functions"] if "valid" in f["name"].lower()]) > 3
    ):
        print("   ├── validators.py        # Input/output validation")

    if len(structure["functions"]) > 10:
        print("   ├── utils.py             # Utility functions")

    if len(structure["constants"]) > 5:
        print("   ├── constants.py         # Module constants")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python3 tools/refactor_assistant.py <file_path> [--create-template]"
        )
        sys.exit(1)
