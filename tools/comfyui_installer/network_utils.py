...
class NetworkUtils:
        ...
        try:
            ...
            for interface in all_interfaces:
                try:
                    ...
                    logger.warning(
                        f"Failed to get address for interface {interface}: {e}"
                    )
                    continue
    ...
    def get_best_interface() -> Optional[Dict[str, str]]:
        ...
        for interface in interfaces:
            if interface["ip"].startswith("192.168.") or interface["ip"].startswith(
                "10."
            ):
                return interface
    ...
    def get_host_config(deployment_type: str = "desktop") -> Dict[str, str]:
        ...
        config = {
            "desktop": {"host": "127.0.0.1", "description": "Local connections only"},
            "portable": {
                "host": "0.0.0.0",
