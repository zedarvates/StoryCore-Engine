...
import { ProjectIntegrityChecker } from './ProjectIntegrityChecker';
import { ProjectDiagnosticService } from './ProjectDiagnosticService';
...
`;
      ...
      // Create discussion file if historical chat is provided
      if (data.discussion) {
        console.log(`Creating DISCUSSION.md with ${data.discussion.length} bytes of discussion history...`);
        try {
          fs.writeFileSync(path.join(projectPath, 'DISCUSSION.md'), data.discussion, 'utf-8');
          console.log('Created DISCUSSION.md');
        } catch (error) {
          console.warn('Failed to create DISCUSSION.md:', error);
        }
      }
   ...
   * @param guid Optional GUID if opened via ID
   * @returns Project information
   */
  async openProject(projectPath: string, guid?: string): Promise<Project> {
    try {
      ...
      // Run the new integrity system to ensure project is healthy and missing fields are restored
      const integrityChecker = new ProjectIntegrityChecker(projectPath);
      const integrityResult = await integrityChecker.runIntegrityCheck();
      
      if (!integrityResult.isValid) {
        console.warn(`[ProjectService] Partial integrity check warning: ${integrityResult.issues.length} issues found`);
      } else {
        console.log(`[ProjectService] Integrity check passed: ${integrityResult.corrections.length} items repaired`);
      }

      // DIAGNOSTIC REPORT
      // Run diagnostics to identify potential issues for the user
      const diagnosticService = new ProjectDiagnosticService(projectPath);
      const diagnosticReport = await diagnosticService.runFullDiagnostic();
      (validation.config as any).diagnosticReport = diagnosticReport;
      ...
      // Return project information
      return {
        id: (config.metadata as any)?.id || guid || uuidv4(),
  ...
  /**
   * Save LLM configuration to the project
   * @param projectPath Path to the project directory
   * @param llmConfig LLM Configuration object
   */
  async saveLLMConfig(projectPath: string, llmConfig: any): Promise<void> {
    try {
      ...
      // Update LLM config and modified date
      projectData.llm = llmConfig;
      projectData.modified_at = new Date().toISOString();

      fs.writeFileSync(projectJsonPath, JSON.stringify(projectData, null, 2), 'utf-8');
      console.log(`[ProjectService] Saved LLM configuration for project at ${projectPath}`);
    } catch (error) {
      console.error('[ProjectService] Failed to save LLM config:', error);
      throw error;
    }
  }
  ...
  async updateMetadata(projectPath: string, metadata: Record<string, any>): Promise<Project> {
    try {
      ...
      // Update top-level fields if they are in metadata
      if (metadata.summary !== undefined) {
        projectConfig.summary = metadata.summary;
      }
      if (metadata.header !== undefined) {
        projectConfig.header = metadata.header;
      }
  ...
  async deleteProject(projectPath: string): Promise<boolean> {
    try {
      // Validate path is within expected projects directory or allowed locations
      // For security, we should check if the path actually looks like a StoryCore project
      ...
      const projectJsonPath = path.join(projectPath, 'project.json');
      if (!fs.existsSync(projectJsonPath)) {
        throw new Error('Path is not a valid StoryCore project (missing project.json)');
      }

      // Perform deletion
      // We use fs.rmSync with recursive and force for directory deletion
      fs.rmSync(projectPath, { recursive: true, force: true });
      
      console.log(`[ProjectService] Deleted project at: ${projectPath}`);
      return true;
    } catch (error) {
