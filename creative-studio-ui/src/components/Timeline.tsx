...
import {
  Clock, Play, Pause, SkipBack, SkipForward, FolderOpen, Layers,
   Plus, ZoomIn, ZoomOut, RefreshCw,
  CheckCircle, Eye, EyeOff, Link2,
  Copy, Clipboard, Wand2, Edit3, Settings, MessageSquare,
...
const TimelineShot = memo<TimelineShotProps>(({
  ...
  const handleResizeStart = useCallback((e: React.MouseEvent, direction: 'left' | 'right') => {
    ...
    setResizeStartInPoint(shot.timing?.inPoint || 0);
    setResizeStartOutPoint(shot.timing?.outPoint || shot.duration);
  }, [width, shot.timing?.inPoint, shot.timing?.outPoint, shot.duration]);
  ...
  return (
    <div ref={(node) => {
      ...
      <div
        className={`shot-container group ${isDragging ? 'opacity-50' : ''} ${isOver ? 'ring-2 ring-blue-400' : ''
          } ${ringStyle}`}
        ref={el => {
          if (el) {
            el.style.setProperty('--shot-left', `${startX}px`);
            el.style.setProperty('--shot-width', `${width}px`);
            el.style.setProperty('--track-height', `${SHOT_TRACK_HEIGHT}px`);
          }
        }}
        onClick={handleClick}
      ...
      >
        <div className={`h-full bg-blue-600 hover:bg-blue-500 border rounded overflow-hidden transition-colors relative ${borderStyle}`}>
          ...
          <div className="relative h-full flex flex-col justify-center px-2">
            ...
            {(shot.audioTracks?.length ?? 0) > 0 && (
              <div className="text-xs text-blue-200">
                🔊 {shot.audioTracks?.length} track{shot.audioTracks && shot.audioTracks.length > 1 ? 's' : ''}
              </div>
            )}
          </div>
      ...
      {shot.transitionOut && transitionWidth > 0 && (
        <div
          ...
          ref={el => {
            if (el) {
              el.style.setProperty('--trans-left', `${startX + width}px`);
              el.style.setProperty('--trans-width', `${transitionWidth}px`);
              el.style.setProperty('--track-height', `${SHOT_TRACK_HEIGHT}px`);
            }
          }}
        >
...
export const Timeline: React.FC<TimelineProps> = ({
  ...
  const handlePasteReferences = useCallback(async () => {
    ...
    try {
      ...
      if (referenceUrls.length > 0) {
        ...
        const shot = shots.find(s => s.id === contextMenu.shotId);
        if (shot) {
          const existingRefs = Array.isArray(shot.metadata?.referenceUrls) 
            ? (shot.metadata.referenceUrls as string[]) 
            : [];
          await updateShotInPlan(contextMenu.shotId, {
            metadata: { 
              ...shot.metadata, 
              referenceUrls: [...existingRefs, ...referenceUrls] 
            }
          });
  ...
  const handleAddComment = useCallback(async () => {
    ...
    if (comment) {
      try {
        ...
        const shot = shots.find(s => s.id === contextMenu.shotId);
        if (shot) {
          const existingComments = Array.isArray(shot.metadata?.comments)
            ? (shot.metadata.comments as Array<{ id: string; content: string; createdAt: string; resolved: boolean }>)
            : [];
          const newComment = {
  ...
  }, [isPlaying, currentTime, totalDuration, shots, selectedShotId, selectedShotIds, multiSelectMode, currentPlanData, handlePlayPause, handleSkipBack, handleSkipForward, handleShotSelect, externalOnShotSelect, externalOnMultiShotSelect, handleSplitShot, removeShotFromPlan]);
        ...
        {!currentPlanData || shots.length === 0 ? (
          ...
          <div
            ref={el => {
              ...
              if (el) {
                el.style.setProperty('--timeline-height', `${TIMELINE_HEIGHT}px`);
              }
            }}
          ...
          >
            <div
              ...
              ref={el => {
                if (el) {
                  el.style.setProperty('--timeline-width', `${totalWidth}px`);
                  el.style.setProperty('--timeline-height', `${TIMELINE_HEIGHT}px`);
                }
              }}
            >
              ...
              <div
                ...
                ref={el => {
                  if (el) {
                    el.style.setProperty('--marker-height', `${TIME_MARKER_HEIGHT}px`);
                    el.style.setProperty('--track-height', `${SHOT_TRACK_HEIGHT}px`);
                  }
                }}
              >
              ...
              <div
                ...
                ref={el => {
                  if (el) {
                    el.style.setProperty('--marker-height', `${TIME_MARKER_HEIGHT}px`);
                    el.style.setProperty('--track-height', `${SHOT_TRACK_HEIGHT}px`);
                    el.style.setProperty('--audio-track-height', `${AUDIO_TRACK_HEIGHT}px`);
                  }
                }}
              >
                      ...
                      return (
                        ...
                        >
                          <div className="h-full bg-green-600 border border-green-400 rounded px-1 flex items-center relative overflow-hidden">
                              ...
                              <div className="absolute inset-0 flex items-center justify-around px-0.5">
                                {track.waveformData.slice(0, Math.floor(trackWidth / 2)).map((amplitude, i) => (
                                  <div
                                    ...
                                    ref={el => { if (el) el.style.height = `${Math.max(2, amplitude * 100)}%`; }}
                                  />
                                ))}
                              </div>
                            ) : (
                              ...
                              <div className="absolute inset-0 flex items-center justify-around px-0.5">
                                {Array.from({ length: Math.min(20, Math.floor(trackWidth / 3)) }).map((_, i) => (
                                  <div
                                    ...
                                    ref={el => { if (el) el.style.height = `${20 + Math.random() * 60}%`; }}
                                  />
                                ))}
                              </div>
                            )}
                            ...
                            {/* Volume Indicator */}
                            {!track.muted && (track as { volume?: number }).volume !== undefined && (track as { volume?: number }).volume! < 100 && (
                              <span className="text-xs text-green-100 ml-auto relative z-10 bg-green-600 bg-opacity-80 px-1 rounded">
                                {(track as { volume?: number }).volume}%
                              </span>
                            )}
              ...
              <div
                ...
                ref={el => {
                  if (el) {
                    el.style.left = `${currentTime * pixelsPerSecond}px`;
                    el.style.width = '2px';
                    el.style.backgroundColor = PLAYHEAD_COLOR;
                    el.style.zIndex = '100';
                  }
                }}
              >
