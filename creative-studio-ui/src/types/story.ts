 ...
 */
export type StoryFileFormat = 'md' | 'txt';
...
/**
 * Production modes corresponding to backend StoryGenerationService
 */
export type ProductionMode = 
  | 'fiction' | 'documentary' | 'interview' | 'music_video' 
  | 'social_media' | 'cinematic' | 'recap' | 'influencer' 
  | 'maker' | 'scientific_review' | 'historical_review' 
  | 'top_tier_list' | 'faith_spirituality' | 'game_review' 
  | 'tech_review' | 'finance_review' | 'masterclass' 
  | 'real_estate' | 'product_hype' | 'legal_recon' | 'asmr' 
  | 'meditation' | 'experimental' | 'true_crime' 
  | 'sports_highlight' | 'gardening' | 'renovation';
...
/**
 * Interface returned by the advanced backend API
 */
export interface AdvancedBackendResponse {
  id: string;
  title: string;
  synopsis: string;
  genre: string;
  mode: string;
  length: string;
  characters: AdvancedBackendCharacter[];
...
export interface AdvancedBackendLocation {
  id?: string;
  name: string;
  type?: string;
  description?: string;
...
export interface AdvancedBackendScene {
  title?: string;
  description?: string;
}
 ...
 */
export function createEmptyStory(result?: Partial<AdvancedBackendResponse>): Partial<Story> {
  return {
    ...
    tone: [],
    length: (result?.length as Story['length']) || 'medium',
