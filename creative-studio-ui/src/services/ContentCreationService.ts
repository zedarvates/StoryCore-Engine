        ...
        switch (type) {
            ...
            case 'object': {
                return {
                    ...
                    power: objData.powerLevel || (objData as LegacyAny).power || 1,
                    abilities: objData.abilities || [],
                    lore: (objData as LegacyAny).lore || '',
            ...
            case 'story': {
                return {
                    ...
                    charactersUsed: (Array.isArray(data.characters) ? data.characters : []).map((c: LegacyAny) => {
                        const char = typeof c === 'object' ? c : { name: String(c) };
                        return {
                            id: (char as LegacyAny).id || crypto.randomUUID(),
                            name: (char as LegacyAny).name || 'Unnamed',
                            role: (char as LegacyAny).role || '',
                        };
                    }),
                    locationsUsed: (Array.isArray(data.locations) ? data.locations : []).map((l: LegacyAny) => ({
                        id: (l as LegacyAny).id || generateIdWithPrefix('loc'),
                        name: (l as LegacyAny).name || String(l),
                        description: (l as LegacyAny).description || '',
                    })),
            ...
            case 'world': {
                return {
                    ...
                    culturalElements: {
                        ...
                        ...(typeof data.cultures === 'object' ? data.cultures as LegacyAny : {})
                    },
                    locations: [],
                    createdAt: now,
                    updatedAt: now,
