// Woke gauge module for content sensitivity detection
...
class WokeGauge {
  constructor() {
    ...
    this.antiWokePatterns = [
      ...
      { regex: /\b(anti-critical race theory|anti-gender)\b/gi, weight: -18 },
      { regex: /\b(law and order|individual responsibility)\b/gi, weight: -12 }
    ];
  }
  ...
  analyzeContent(text) {
    ...
    const level = this.mapToSensitivityLevel(score);
  ...
  getSensitivityLevelDescription(level) {
    switch (level) {
      case 'low': return 'Content has minimal ideological markers';
      case 'medium': return 'Content contains moderate ideological themes';
      case 'high': return 'Content contains strong ideological themes';
...
// Export the gauge instance
module.exports = new WokeGauge();