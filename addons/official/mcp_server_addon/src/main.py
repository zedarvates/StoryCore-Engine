...
class MCPServerAddon:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.mcp_client = MCPClient(
                server_config=self.config.get("server", {}), logger=self.logger
            )

            # Initialiser le handler MCP
            self.mcp_handler = MCPHandler(
                mcp_client=self.mcp_client, validator=self.validator, logger=self.logger
            )
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stats": self.stats,
            "config": self.config,
            "active_connections": self.mcp_client.get_connections_count()
            if self.mcp_client
            else 0,
        }
    ...
    async def _register_hooks(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Enregistrement de {len(self.hook_manager.hooks)} hooks terminé"
            )
        ...
        try:
            ...
            for permission, level in required_permissions:
                if not await self.permission_manager.check_permission(
                    self.addon_name, permission, level, PermissionScope.SESSION
                ):
        ...
        if response:
            try:
                ...
                if "error" in resp_data:
                    self.logger.warning(
                        f"Erreur renvoyée dans la réponse MCP: {resp_data['error']}"
                    )
            except Exception:
                pass
    ...
    async def _security_check_hook(self, *args, **kwargs) -> bool:
        ...
        if message and isinstance(message, str):
            ...
            if re.search(r"(__class__|__mro__|__subclasses__)", message):
                self.logger.error(
                    "Tentative d'injection Python (Template/Sandbox Escape) bloquée."
                )
                return False
