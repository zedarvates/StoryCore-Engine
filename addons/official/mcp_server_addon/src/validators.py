...
class MCPValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_response_size = config.get(
    ...
    def validate_resource_access(self, uri: str, method: str) -> SecurityCheck:
        ...
        if not self.uri_pattern.match(uri):
            ...
            recommendations.append(
                "Utilisez un URI valide (ex: file://chemin/fichier.json)"
            )

        # Vérifier les schémas autorisés
        allowed_schemes = self.security_config.get(
        ...
        return SecurityCheck(
            passed=passed, issues=issues, recommendations=recommendations
        )
    ...
    def validate_tool_call(
        ...
        if not self.method_pattern.match(tool_name):
            ...
            recommendations.append(
                "Utilisez uniquement des caractères alphanumériques, tirets et underscores"
            )
    ...
    def validate_configuration(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if "host" not in server_config:
            warnings.append(
                "Configuration 'host' manquante, utilisation de 'localhost' par défaut"
            )

        if "port" not in server_config:
            warnings.append(
                "Configuration 'port' manquante, utilisation de 8080 par défaut"
            )
        ...
        if "max_connections" in limits:
            if (
                not isinstance(limits["max_connections"], int)
                or limits["max_connections"] <= 0
            ):
                errors.append("max_connections doit être un entier positif")

        if "timeout" in limits:
            if (
                not isinstance(limits["timeout"], (int, float))
                or limits["timeout"] <= 0
            ):
        ...
        return ValidationResult(
            is_valid=is_valid, errors=errors, warnings=warnings, info=info, score=score
        )
        ...
        try:
            ...
            for forbidden in forbidden_paths:
                if forbidden in path:
                    ...
                    recommendations.append(
                        "Ce chemin est interdit pour des raisons de sécurité"
                    )
                    break
    ...
    def _check_rate_limit(self) -> SecurityCheck:
        ...
        self._request_history = [
            t for t in self._request_history if current_time - t < 60
        ]
        ...
        return SecurityCheck(passed=True, issues=[], recommendations=[])
