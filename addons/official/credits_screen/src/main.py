...
class CreditsService:
    ...
    async def generate_credits_video(self, req: CreditsRequest) -> Dict[str, Any]:
        ...
        if req.include_pegi:
            ...
            vf_filters.append("drawtext=text='PEGI 18':fontcolor=white@0.5:fontsize=20:x=20:y=h-40")
            vf_filters.append("drawtext=text='CENSURE':fontcolor=white@0.5:fontsize=20:x=w-120:y=h-40")

        vf_str = ",".join(vf_filters)

        cmd = [
            self.ffmpeg_path, "-y",
            "-f", "lavfi", "-i", f"color=c=black:s=1920x1080:d={req.duration}",
            "-vf", vf_str,
