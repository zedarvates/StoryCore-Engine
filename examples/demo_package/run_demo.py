...
def demo_text_analysis(fact_checker: FactCheckerCommand):
    ...
    print(content[:500] + "...\n")
    print_info(
        f"Total length: {len(content)} characters, ~{len(content.split())} words"
    )
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            print(
                f"  Total Claims Extracted: {Colors.BOLD}{summary.get('total_claims', 0)}{Colors.ENDC}"
            )
            print(
                f"  High Risk Claims: {Colors.BOLD}{summary.get('high_risk_count', 0)}{Colors.ENDC}"
            )
            print(
                f"  Average Confidence: {Colors.BOLD}{summary.get('average_confidence', 0):.1f}%{Colors.ENDC}"
            )
            print(
                f"  Domains Analyzed: {Colors.BOLD}{', '.join(summary.get('domains_analyzed', []))}{Colors.ENDC}"
            )
...
def demo_video_analysis(fact_checker: FactCheckerCommand):
    ...
    print_info(
        "This demo analyzes a news interview transcript for manipulation signals."
    )
    print_info("The system will:")
    ...
    transcript_path = (
        Path(__file__).parent / "sample_transcripts" / "news_interview.txt"
    )
        ...
        if result.get("status") == "success":
            ...
            if signals:
                ...
                for i, signal in enumerate(signals[:5], 1):  # Show first 5
                    ...
                    print(
                        f"    Timestamp: {signal.get('timestamp_start', 'N/A')} - {signal.get('timestamp_end', 'N/A')}"
                    )
                    print(
                        f"    Description: {signal.get('description', 'N/A')[:100]}..."
                    )
            ...
            if segments:
                ...
                for i, segment in enumerate(segments[:5], 1):  # Show first 5
                    print(
                        f"  {i}. [{segment.get('timestamp', 'N/A')}] {segment.get('issue', 'N/A')}"
                    )
...
def demo_batch_processing(fact_checker: FactCheckerCommand):
    ...
    for script_file in sample_dir.glob("*.txt"):
        with open(script_file, "r", encoding="utf-8") as f:
            items.append({"id": script_file.stem, "content": f.read()})
    ...
    try:
        ...
        def process_item(content: str) -> Dict[str, Any]:
            return fact_checker.execute(
                input_data=content, mode="text", confidence_threshold=70
            )
        ...
        print(
            f"  Documents Processed: {Colors.BOLD}{success_count}/{len(items)}{Colors.ENDC}"
        )
        print(f"  Total Claims: {Colors.BOLD}{total_claims}{Colors.ENDC}")
        print(f"  Total High Risk: {Colors.BOLD}{total_high_risk}{Colors.ENDC}")
        if success_count > 0:
            print(
                f"  Overall Avg Confidence: {Colors.BOLD}{avg_confidence_sum / success_count:.1f}%{Colors.ENDC}"
            )
        print(
            f"  Processing Speed: {Colors.BOLD}{elapsed / len(items):.2f}s per document{Colors.ENDC}"
        )
...
def demo_caching(fact_checker: FactCheckerCommand):
    ...
    try:
        result1 = fact_checker.execute(
            input_data=sample_text, mode="text", confidence_threshold=70
        )
        ...
        if cached2:
            ...
            print(
                f"  {Colors.GREEN}{Colors.BOLD}Speedup: {speedup:.1f}x faster{Colors.ENDC}"
            )
        else:
...
def demo_pipeline_integration():
    ...
    print_info(
        "This demo explains how the fact-checker integrates with StoryCore-Engine."
    )
