...
def example_error_categorization():
    ...
    error_logger = ErrorLogger(
        log_dir=Path(".storycore/logs"), max_log_size_mb=10, max_log_files=5
    )
    ...
    errors = [
        ...
        (
            ValueError("Node not found in workflow"),
            {"workflow_name": "flux_basic", "missing_node": "FluxSampler"},
        ),
...
def example_retry_manager():
    ...
    try:
        result = retry_mgr.with_retry(unreliable_operation, "connect_operation")
    ...
    def connect(self, url: str):
        ...
        try:
            result = self.retry_mgr.with_retry(
                self._attempt_connection, f"connect_{url}", url
            )
            return result
        except Exception as e:
            # Categorize and log error
            categorized = ErrorCategorizer.categorize_error(e, context={"url": url})
...
def example_error_recovery_workflow():
    ...
    def generate_image(shot_id: str):
        ...
        error_type = random.choice(["connection", "model", "workflow", "success"])
    ...
    for shot_id in shots:
        ...
        try:
            result = retry_mgr.with_retry(
                generate_image, f"generate_{shot_id}", shot_id
            )
            print(f"✓ {result}")
