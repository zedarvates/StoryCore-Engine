...
def demo_custom_parameters():
    ...
    result1 = command.execute(content, mode="text", confidence_threshold=80.0)
    print(f"   Status: {result1['status']}")

    # Test 2: Summary detail level
    print("\n2. Summary Detail Level:")
    result2 = command.execute(content, mode="text", detail_level="summary")
    print(f"   Status: {result2['status']}")
    print(f"   Claims in report: {len(result2['report']['claims'])}")

    # Test 3: Markdown output format
    print("\n3. Markdown Output Format:")
    result3 = command.execute(content, mode="text", output_format="markdown")
...
def demo_error_handling():
    ...
    print("\nTest 2 - Invalid Confidence Threshold:")
    result2 = command.execute("Test content", mode="text", confidence_threshold=150.0)
    print(f"Status: {result2['status']}")
