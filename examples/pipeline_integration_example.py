...
async def example_complete_pipeline_workflow():
    ...
    # Configure all hooks
    integration.configure_hook(
        "before_generate", HookConfig(enabled=True, blocking=False, store_results=False)
    )

    integration.configure_hook(
        "after_generate", HookConfig(enabled=True, blocking=False, store_results=True)
    )

    integration.configure_hook(
        "on_publish",
        HookConfig(
            enabled=True, blocking=True, on_high_risk="block", store_results=True
        ),
    )
