...
def demo_basic_analysis():
    ...
    print(
        f"  Average confidence: {report.summary_statistics['average_confidence']:.1f}%"
    )
    print(f"  High-risk claims: {report.summary_statistics['high_risk_count']}")
    print(
        f"  Domains: {', '.join(report.summary_statistics.get('domains_analyzed', []))}"
    )
...
def demo_batch_processing():
    ...
    for i, (text, report) in enumerate(zip(texts, reports), 1):
        ...
        print(
            f"  Avg Confidence: {report.summary_statistics['average_confidence']:.1f}%"
        )
