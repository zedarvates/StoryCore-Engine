...
class StoryAnalyzer:
    ...
    def extract_pitch(self) -> str:
        ...
        [w for w in ["conflit", "combat", "danger", "quete"] if w in story]
        if len(chars) >= 2:
    ...
    def extract_theme(self) -> tuple:
        ...
        if any(w in story for w in ["amour", "passion"]):
            themes.append("amour")
        elif any(w in story for w in ["combat", "danger"]):
            themes.append("survie")
        elif any(w in story for w in ["secret", "mystere"]):
            themes.append("mystere")
        else:
            themes.append("aventure")
        return (themes[0], themes)

    def extract_ton(self) -> tuple:
        story = self.raw_story.lower()
        if any(w in story for w in ["sombre", "mort", "danger"]):
            return ("sombre", ["sombre"])
        if any(w in story for w in ["mystere", "secret"]):
            return ("mysterieux", ["mysterieux"])
    ...
    def extract_enjeux(self) -> List[str]:
        ...
        if any(w in story for w in ["verite", "secret"]):
            enjeux.append("La verite")
        if any(w in story for w in ["monde", "humanite"]):
            enjeux.append("Sort du monde")
            ...
            for i, word in enumerate(words):
                ...
                if word and word[0].isupper():
                    ...
                    if (
                        ...
                        and name not in chars
                        and not any(
                            name.lower() in w.lower()
                            for w in [
                                "laboratory",
                                "facility",
                                "world",
...
class StoryTransformer:
    ...
    def transform(self) -> Dict[str, Any]:
            ...
            lieux.append(
                {
                    ...
                    "ambiance_sonore": [],
                    "elements_visuels": [],
                    "eclairage": MomentType.JOUR.value,
                    "historique": "",
            ...
            objets.append(
                {
                    ...
                    "pouvoir_capacite": "",
                    "proprietaire_actuel": "",
                    "significance": f"{name} est crucial",
                    "apparitions_scenes": [],
        ...
        structure = {
            ...
            "acte_3": {
                "titre": "La Resolution",
                "description": "Climax et fin",
                "duree": "25%",
            },
            "themes_principaux": [meta["theme"]],
            "themes_secondaires": [],
        }
                ...
                scenes.append(
                    {
                        "id": len(scenes) + 1,
                        "sequence_id": seq["id"],
                        "type": SceneType.EXT.value
                        if j % 2 == 0
                        else SceneType.INT.value,
                        ...
                        "progression": {
                            "etat": MomentNarratif.DEVELOPPEMENT.value,
                            "tension": TensionLevel.MOYENNE.value,
                        },
                    }
                )
        ...
        logger.info(
            f"Done: {len(personnages)} chars, {len(lieux)} locs, {len(scenes)} scenes"
        )
