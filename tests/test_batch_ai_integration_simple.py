...
async def test_job_submission():
    ...
    config = BatchConfig(max_queue_size=10, max_concurrent_jobs=2)
        ...
        for i in range(15):  # More than queue size
            job = AIBatchJob(
                ...
                resource_requirements=ResourceRequirements(
                    gpu_count=1, gpu_memory_mb=1024
                ),
    ...
    try:
        ...
        jobs = [
            AIBatchJob(
                ...
                resource_requirements=ResourceRequirements(
                    gpu_count=1, gpu_memory_mb=2048, estimated_duration_seconds=0.5
                ),
        ...
        print(
            f"   ✅ GPU utilization: {stats['resource_status']['gpu_available']}/{stats['resource_status']['gpu_total']}"
        )
...
async def test_priority_scheduling():
    ...
    config = BatchConfig(max_concurrent_jobs=1, scheduling_interval_seconds=0.5)
    ...
    try:
        ...
        if running_jobs:
            ...
            assert "critical" in running_job_id, (
                "Critical priority job should run first"
            )
...
async def test_graceful_stop():
    ...
    config = BatchConfig(max_concurrent_jobs=2, scheduling_interval_seconds=0.5)
...
async def test_statistics():
    ...
    try:
        ...
        print(
            f"      - Queue utilization: {stats['queue_status']['queue_utilization']:.1%}"
        )
        print(
            f"      - Resource utilization: {stats['resource_status']['utilization']:.1%}"
        )

        assert stats["stats"]["total_jobs_submitted"] == 5, (
            "Should have 5 submitted jobs"
        )
...
async def test_real_time_coordination():
    ...
    config = BatchConfig(enable_real_time_coordination=True, real_time_priority_boost=2)
    ...
    try:
        ...
        await integration.coordinate_with_real_time(
            "realtime_job_1", priority_boost=True
        )
        ...
        print("   ✅ Real-time coordination active")
