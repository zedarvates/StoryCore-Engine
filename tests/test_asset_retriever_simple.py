...
from src.asset_retriever import (
    AssetRetriever,
    CleanupPolicy,
    RetrievalMetrics,
    AssetDownload,
...
class TestAssetRetrieverSimple:
    ...
    def test_cleanup_policy_defaults(self):
        ...
        assert policy.keep_successful_downloads
        assert not policy.keep_failed_downloads
    ...
    async def test_verify_asset_integrity_no_checksum(self):
        ...
        asset_info = AssetInfo(filename="test.jpg", local_path=test_file, checksum=None)

        is_valid = await retriever.verify_asset_integrity(asset_info)
        assert is_valid  # Should assume valid without checksum
    ...
    async def test_verify_asset_integrity_valid(self):
        ...
        asset_info = AssetInfo(
            filename="test.jpg", local_path=test_file, checksum=expected_checksum
        )
    ...
    async def test_verify_asset_integrity_invalid(self):
        ...
        asset_info = AssetInfo(
            filename="test.jpg", local_path=test_file, checksum="invalid_checksum"
        )
