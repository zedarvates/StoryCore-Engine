...
def valid_audio_clip(draw):
    ...
    )  # Very short clips for property testing
    ...
    return {"data": np.array(audio_data, dtype=np.float32), "rate": sample_rate}
...
def metallic_audio_clip(draw):
    ...
    for f in formant_freqs:
        audio_data += (
            0.5 * np.sin(2 * np.pi * f * t) * np.exp(-t / duration)
        )  # Decaying

    # Normalize
    audio_data = audio_data / np.max(np.abs(audio_data))

    return {"data": audio_data.astype(np.float32), "rate": sample_rate}
...
def audio_clip_with_gaps(draw):
    ...
    for _ in range(num_gaps):
        ...
        gap_duration = draw(
            st.floats(min_value=0.005, max_value=min(0.02, total_duration - gap_start))
        )
...
class TestQualityValidatorAudioProperties:
    ...
    def test_property_8_metallic_voice_detection(self, audio_clip):
        ...
        for issue in issues:
            ...
            assert issue["metric_value"] >= 0.0, (
                f"Invalid metric value: {issue['metric_value']}"
            )
    ...
    def test_property_9_voice_quality_issue_reporting(self, audio_clip):
        ...
        assert 0.0 <= result["quality_score"] <= 100.0, (
            f"Quality score out of bounds: {result['quality_score']}"
        )
        assert 0.0 <= result["clarity_score"] <= 100.0, (
            f"Clarity score out of bounds: {result['clarity_score']}"
        )
        ...
        for suggestion in result["suggestions"]:
            ...
            assert "expected_improvement" in suggestion, (
                "Suggestion missing 'expected_improvement'"
            )
    ...
    def test_property_10_voice_clarity_measurement(self, audio_clip):
        ...
        if result["clarity_score"] < 30.0:
            ...
            assert len(result["recommendations"]) > 0, (
                "Low clarity should have recommendations"
            )
    ...
    def test_property_15_gap_detection_with_context(self, audio_clip):
        ...
        for gap in gaps:
            ...
            assert gap["end_timestamp"] > gap["timestamp"], (
                "End timestamp should be after start"
            )
            assert isinstance(gap["is_problematic"], bool), (
                "is_problematic should be boolean"
            )
    ...
    def test_property_17_gap_report_calculations(self, audio_clip):
        ...
        assert abs(report["total_gap_duration"] - expected_total_gap) < 0.01, (
            "Total gap duration calculation incorrect"
        )

        expected_percentage = (
            (expected_total_gap / total_duration) * 100 if total_duration > 0 else 0.0
        )
        assert abs(report["gap_percentage"] - expected_percentage) < 0.01, (
            "Gap percentage calculation incorrect"
        )

        assert report["gap_count"] == len(gaps), "Gap count incorrect"

        expected_problematic = sum(1 for gap in gaps if gap["is_problematic"])
        assert report["problematic_gaps"] == expected_problematic, (
            "Problematic gaps count incorrect"
        )

        expected_intentional = len(gaps) - expected_problematic
        assert report["intentional_gaps"] == expected_intentional, (
            "Intentional gaps count incorrect"
        )
