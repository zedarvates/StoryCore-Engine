...
async def test_non_blocking_record():
    ...
    try:
        ...
        print(
            f"   ✅ Recorded {stats['counters']['total_events']} events in {elapsed:.3f}s"
        )
        print(
            f"   ✅ Dropped {stats['counters']['dropped_events']} events (queue full)"
        )
...
async def test_batch_processing_timeout():
    ...
    try:
        ...
        print("   ✅ Batch timeout respected (didn't wait for full batch)")
...
async def test_bottleneck_detection():
    ...
    config = AnalyticsConfig(max_queue_size=10, enable_bottleneck_detection=True)
...
async def test_performance_snapshot():
    ...
    try:
        ...
        print(
            f"      - Avg processing time: {snapshot.average_processing_time_ms:.1f}ms"
        )
