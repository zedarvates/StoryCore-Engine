...
from src.end_to_end.data_models import ShotConfig, StyleConfig, ComfyUIStatus
...
def generation_engine(
    mock_connection_manager, mock_model_manager, mock_workflow_manager
):
...
def sample_shot_config():
    ...
    return ShotConfig(
        ...
        cfg_scale=7.0,
        seed=42,
        style_config=StyleConfig(style_type="cinematic", style_strength=0.8),
    )
    ...
    async def test_submit_valid_workflow(self, generation_engine):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            mock_response.json = AsyncMock(
                return_value={"prompt_id": "test_prompt_123"}
            )
    ...
    async def test_submit_workflow_with_retry(self, generation_engine):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            mock_response_success.json = AsyncMock(
                return_value={"prompt_id": "retry_prompt_456"}
            )
...
class TestProgressPolling:
    ...
    async def test_poll_for_completed_generation(self, generation_engine):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            mock_history_response.json = AsyncMock(
                return_value={
                    prompt_id: {
                        ...
                        "outputs": {
                            "9": {
                                "images": [
                                    {"filename": "test_image.png", "subfolder": ""}
                                ]
                            }
                        },
                    }
            ...
            mock_session.get = AsyncMock(
                side_effect=[mock_history_response, mock_image_response]
            )
            mock_session.__aenter__ = AsyncMock(return_value=mock_session)
            mock_session.__aexit__ = AsyncMock()
    ...
    async def test_poll_handles_queue_position(self, generation_engine):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            mock_history_response_2.json = AsyncMock(
                return_value={
                    prompt_id: {
                        "status": {"completed": True},
                        "outputs": {
            ...
            mock_queue_response.json = AsyncMock(
                return_value={"queue_running": [], "queue_pending": [[0, prompt_id]]}
            )
            ...
            image_data = await generation_engine._poll_for_generation_result(
                prompt_id, poll_interval=0.01, max_wait=5
            )
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            with pytest.raises(RuntimeError, match="Generation timeout"):
                await generation_engine._poll_for_generation_result(
                    prompt_id, poll_interval=0.01, max_wait=0.1
                )
    ...
    async def test_download_video_streaming(self, generation_engine, tmp_path):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            result = await generation_engine._download_video_from_comfyui(
                filename, output_path=output_path
            )
    ...
    async def test_cancel_active_generation(self, generation_engine):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            generation_engine._track_active_prompt(
                "test_prompt", session.session_id, "test_workflow"
            )
...
class TestErrorHandling:
    ...
    async def test_connection_error_fallback(
        self, generation_engine, sample_shot_config, tmp_path
    ):
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            result = await generation_engine._generate_with_comfyui(
                sample_shot_config, output_path
            )
    ...
    def test_should_fallback_to_mock(self, generation_engine, mock_connection_manager):
        ...
        mock_connection_manager.get_status.return_value = ComfyUIStatus(
            available=False, url="http://localhost:8000"
        )
        assert generation_engine._should_fallback_to_mock() is True

        # Backend available, fallback mode = abort
        mock_connection_manager.get_status.return_value = ComfyUIStatus(
            available=True, url="http://localhost:8000"
        )
...
class TestQueueManagement:
    ...
    def test_track_active_prompts(self, generation_engine):
        ...
        generation_engine._track_active_prompt(
            "prompt_1", session.session_id, "z_image_turbo"
        )
    ...
    def test_concurrent_generation_limit(self, generation_engine):
        ...
        for i in range(limit):
            generation_engine._track_active_prompt(
                f"prompt_{i}", session.session_id, "test_workflow"
            )
