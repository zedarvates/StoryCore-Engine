...
class TestGridFormatOptimizationE2E:
    ...
    def test_complete_action_sequence_optimization(self):
        ...
        optimization_result = optimize_project_format(
            str(self.project_path), quality_vs_speed=0.8, min_quality=80.0
        )
        ...
        assert content_analysis.temporal_requirements
        ...
        assert recommendation.recommended_format in [
            GridFormat.LINEAR_1X3,
            GridFormat.LINEAR_1X4,
        ]
        ...
        assert (
            updated_project["asset_manifest"]["grid"]["dimensions"]
            == recommendation.recommended_format.value
        )
        ...
        print(
            f"✓ Action sequence optimized to {recommendation.recommended_format.value}"
        )
        print(f"  Confidence: {recommendation.confidence_score:.1%}")
        print(
            f"  Predicted improvement: +{recommendation.predicted_quality_improvement:.1f}%"
        )
    ...
    def test_complete_dialogue_sequence_optimization(self):
        ...
        print(
            f"✓ Dialogue sequence optimized to {recommendation.recommended_format.value}"
        )
        print(f"  Confidence: {recommendation.confidence_score:.1%}")
    ...
    def test_format_comparison_analysis(self):
        ...
        for format_type in GridFormat:
            ...
            assert validation.is_valid, (
                f"Format {format_type.value} devrait être compatible"
            )
            ...
            prediction = predictor.predict_quality_metrics(
                format_type, content_analysis
            )
        ...
        for format_type, prediction in format_results.items():
            if format_type != GridFormat.SQUARE_3X3:
                improvement = (
                    (prediction.predicted_quality_score - baseline_score)
                    / baseline_score
                ) * 100
        ...
        assert best_linear_improvement > 0, (
            "Les formats linéaires devraient améliorer la qualité pour l'action"
        )
    ...
    def test_quality_threshold_enforcement(self):
        ...
        assert high_threshold_result["success"]
        assert standard_threshold_result["success"]
    ...
    def test_performance_constraint_handling(self):
        ...
        assert (
            fast_rec.estimated_processing_time <= quality_rec.estimated_processing_time
        )

        print(
            f"Fast optimization: {fast_rec.recommended_format.value} ({fast_rec.estimated_processing_time:.1f}s)"
        )
        print(
            f"Quality optimization: {quality_rec.recommended_format.value} ({quality_rec.estimated_processing_time:.1f}s)"
        )
