...
def mock_llm_server():
    ...
    with patch("requests.Session") as mock_session:
        ...
        mock_completion.json.return_value = {"choices": [{"text": "Generated text"}]}
...
def test_comfyui_pool_creation(mock_comfyui_server):
    ...
    pool = create_comfyui_pool(
        name="test-comfyui", comfyui_config=config, min_connections=1, max_connections=3
    )
...
def test_llm_pool_creation(mock_llm_server):
    ...
    pool = create_llm_pool(
        name="test-llm", llm_config=config, min_connections=1, max_connections=5
    )
...
def test_llm_pool_usage(mock_llm_server):
    ...
    pool = create_llm_pool(
        name="test-llm-usage", llm_config=config, min_connections=1, max_connections=5
    )
