...
try:
    ...
    from src.end_to_end.data_models import GeneratedVideo, FallbackMode
except ImportError as e:
    pytest.skip(
        f"LTX-2 integration modules not available: {e}", allow_module_level=True
    )
        ...
        try:
            ...
            for width, height in sizes:
                test_image = create_test_image(
                    temp_output_dir / f"test_{width}x{height}.png", size=(width, height)
                )
