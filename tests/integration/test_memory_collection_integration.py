...
class TestMemoryCollectionIntegration:
    ...
    def test_collect_memory_state_real_psutil(self):
        ...
        assert process_state["status"] in ["running", "sleeping", "disk-sleep"], (
        ...
        assert "error" not in process_state, (
            "Should not have error with psutil installed"
        )
    ...
    def test_create_report_payload_with_real_memory_state(self):
        ...
        assert payload["diagnostics"]["memory_usage_mb"] > 0, (
            "Should have real memory usage"
        )
    ...
    def test_memory_values_are_properly_formatted(self):
        ...
        assert memory_mb == round(memory_mb, 2), (
            "Memory usage should be rounded to 2 decimals"
        )

        memory_pct = memory_state["memory_percent"]
        assert isinstance(memory_pct, float), "Memory percent should be float"
        assert memory_pct == round(memory_pct, 2), (
            "Memory percent should be rounded to 2 decimals"
        )

        sys_mem_pct = memory_state["system_memory_percent"]
        assert isinstance(sys_mem_pct, float), "System memory percent should be float"
        assert sys_mem_pct == round(sys_mem_pct, 2), (
            "System memory percent should be rounded to 2 decimals"
        )
    ...
    def test_memory_state_consistency(self):
        ...
        assert (
            ...
            <= memory_state["system_memory_total_mb"]
