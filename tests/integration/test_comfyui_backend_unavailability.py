...
class TestBackendUnavailabilityHandling:
    ...
    async def test_fallback_to_mock_mode_on_connection_failure(
        ...
        async with ComfyUIIntegration(
            ...
            assert not status.available, "Backend should be unavailable"
            ...
            for i, panel in enumerate(coherence_sheet.panels):
                assert panel.path.exists(), f"Placeholder panel {i} should exist"
    ...
    async def test_fallback_during_generation(
        ...
        async with ComfyUIIntegration(
            ...
            assert generated_shot is not None, "Should generate shot in mock mode"
            assert generated_shot.path.exists(), "Placeholder shot should exist"
    ...
    async def test_automatic_reconnection_attempts(self):
        ...
        async with ComfyUIIntegration(
            ...
            assert not status1.available, "Backend should initially be unavailable"
    ...
    async def test_error_message_on_unavailability(self):
        ...
        async with ComfyUIIntegration(
            ...
            assert len(status.error_message) > 0, "Error message should not be empty"
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_recovery_after_reconnection(
        self, temp_output_dir, world_config, style_config
    ):
        """
        ...
        async with ComfyUIIntegration(
            ...
            assert coherence_sheet is not None, "Should generate in mock mode"
            assert len(coherence_sheet.panels) == 9, "Should have 9 placeholder panels"

            # All panels should be valid
            for panel in coherence_sheet.panels:
                assert panel.path.exists(), "Placeholder panel should exist"
    ...
    async def test_timeout_handling(self, temp_output_dir, world_config, style_config):
        ...
        async with ComfyUIIntegration(
            ...
            assert elapsed_time < 10, f"Should timeout quickly, took {elapsed_time}s"

            assert not status.available, "Backend should be unavailable after timeout"
    ...
    async def test_partial_generation_recovery(
        ...
        async with ComfyUIIntegration(
            ...
            for i, panel in enumerate(coherence_sheet.panels):
                assert panel.path.exists(), f"Panel {i} should exist"
