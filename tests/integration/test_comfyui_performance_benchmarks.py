...
class TestPerformanceBenchmarks:
    ...
    async def test_single_image_generation_time(
        ...
        async with ComfyUIIntegration(
            ...
            assert generated_shot is not None, "Shot should be generated"
            assert generated_shot.path.exists(), "Generated shot should exist"

            # Report performance
            print(f"\n{'=' * 60}")
            print("Single Image Generation Performance")
            print(f"{'=' * 60}")
            print(f"Generation Time: {generation_time:.2f} seconds")
            print(
                f"Resolution: {shot_config.resolution[0]}x{shot_config.resolution[1]}"
            )
            print(f"{'=' * 60}\n")
    ...
    async def test_master_coherence_sheet_generation_time(
        ...
        async with ComfyUIIntegration(
            ...
            # Validate generation
            assert coherence_sheet is not None, "Coherence sheet should be generated"
            assert len(coherence_sheet.panels) == 9, "Should have 9 panels"
            ...
            # Report performance
            print(f"\n{'=' * 60}")
            print("Master Coherence Sheet Generation Performance")
            print(f"{'=' * 60}")
            print(f"Total Time: {total_time:.2f} seconds")
            print(f"Average Time per Panel: {avg_time_per_panel:.2f} seconds")
            print("Panels Generated: 9")
            print(f"{'=' * 60}\n")
    ...
    async def test_queue_depth_tracking(
        ...
        async with ComfyUIIntegration(
            ...
            assert all(
                isinstance(depth, int) and depth >= 0 for depth in queue_depths
    ...
    async def test_batch_generation_performance(
        ...
        async with ComfyUIIntegration(
            ...
            assert avg_time < 300, f"Average time {avg_time:.2f}s, expected < 300s"
    ...
    async def test_resolution_impact_on_performance(
        ...
        async with ComfyUIIntegration(
                ...
                performance_data.append(
                    {
                        ...
                        "time_per_megapixel": generation_time
            ...
            # Report performance
            print(f"\n{'=' * 60}")
            print("Resolution Impact on Performance")
            print(f"{'=' * 60}")
    ...
    async def test_generation_metrics_collection(
        ...
        async with ComfyUIIntegration(
            ...
            if metrics["generation_times"]:
                metrics["avg_time_per_image"] = statistics.mean(
            ...
            # Validate metrics
            assert metrics["total_images"] > 0, "Should have generated images"
            assert metrics["successful"] > 0, "Should have successful generations"
            assert metrics["total_time"] > 0, "Should have measured time"
