...
try:
    ...
    pytest.skip(
        f"ComfyUI integration modules not available: {e}", allow_module_level=True
    )
...
class FinalCheckpointTest:
    ...
    def setup(self):
        ...
        print(f"\n{'=' * 80}")
        print("FINAL CHECKPOINT TEST - ComfyUI Desktop Default Integration")
        print(f"{'=' * 80}")
    ...
    def add_result(self, result: PropertyTestResult):
        ...
        print(
            f"{status_symbol} Property {result.property_number}: {result.property_name} - {result.status}"
        )
        if result.message:
    ...
    def print_report(self):
        ...
        print(f"\n{'=' * 80}")
        print("FINAL CHECKPOINT TEST REPORT")
        print(f"{'=' * 80}")
...
async def test_connection_and_configuration_properties(checkpoint_test):
    ...
    async def test_property_1():
        async with ComfyUIIntegration(
            backend_url=checkpoint_test.backend_url, timeout=10, max_retries=1
        ) as comfyui:
            status = await comfyui.check_availability()
            assert status is not None, "Should attempt connection"

    await checkpoint_test.test_property(
        1, "Default Connection Attempt", ["1.1"], test_property_1
    )
    ...
    await checkpoint_test.test_property(
        2, "Connection Success Display", ["1.2", "6.1"], test_property_2
    )
    ...
    await checkpoint_test.test_property(
        3, "Connection Failure Fallback", ["1.3", "11.1", "11.2"], test_property_3
    )
    ...
    await checkpoint_test.test_property(
        4, "Configuration Override", ["1.4", "9.5"], test_property_4
    )
    ...
    await checkpoint_test.test_property(
        5, "Configuration Validation", ["9.3", "9.4"], test_property_5
    )
    ...
    await checkpoint_test.test_property(6, "Status Polling", ["6.5"], test_property_6)

    # Property 7: Status Values
    async def test_property_7():
        async with ComfyUIIntegration(
            backend_url=checkpoint_test.backend_url, timeout=5, max_retries=1
        ) as comfyui:
            status = await comfyui.check_availability()
            # Status should be one of the valid values
            assert status.available in [True, False], "Status should be boolean"

    await checkpoint_test.test_property(7, "Status Values", ["6.2"], test_property_7)
...
async def test_cors_properties(checkpoint_test):
    ...
    await checkpoint_test.test_property(
        8, "CORS Validation", ["2.1", "2.3"], test_property_8
    )
    ...
    await checkpoint_test.test_property(
        9, "CORS Failure Guidance", ["2.2", "2.4"], test_property_9
    )
...
async def test_generation_properties(checkpoint_test):
    ...
    async def test_property_25():
        ...
        async with ComfyUIIntegration(
            backend_url=checkpoint_test.backend_url, timeout=60, max_retries=1
        ) as comfyui:
    ...
    await checkpoint_test.test_property(
        25, "Generation Progress Display", ["8.1", "8.2", "8.3"], test_property_25
    )
    ...
    await checkpoint_test.test_property(
        26, "Master Coherence Sheet Progress", ["8.4", "8.5"], test_property_26
    )
    ...
    await checkpoint_test.test_property(
        30, "Asset Dimension Validation", ["5.7"], test_property_30
    )
...
async def test_mock_mode_properties(checkpoint_test):
    ...
    await checkpoint_test.test_property(
        31, "Mock Mode Activation", ["11.1"], test_property_31
    )
    ...
    await checkpoint_test.test_property(
        32, "Mock Mode Indicator", ["11.2"], test_property_32
    )
    ...
    await checkpoint_test.test_property(
        33, "Mock Mode Placeholder Labels", ["11.3"], test_property_33
    )
    ...
    await checkpoint_test.test_property(
        36, "Mock Mode Feature Preservation", ["11.6"], test_property_36
    )
...
async def test_error_handling_properties(checkpoint_test):
    ...
    async def test_property_40():
        async with ComfyUIIntegration(
            backend_url="http://localhost:9999", timeout=2, max_retries=1
        ) as comfyui:
            status = await comfyui.check_availability()
    ...
    await checkpoint_test.test_property(
        40, "Connection Error Guidance", ["12.3"], test_property_40
    )
    ...
    await checkpoint_test.test_property(44, "Error Logging", ["12.7"], test_property_44)
...
async def test_final_checkpoint_complete(checkpoint_test):
    ...
    print(f"\n{'=' * 80}")
    print("Running Final Checkpoint Test Suite")
    print(f"{'=' * 80}\n")
    ...
    spec_report_path = Path(
        ".kiro/specs/comfyui-desktop-default-integration/TASK_20_COMPLETION.md"
    )
