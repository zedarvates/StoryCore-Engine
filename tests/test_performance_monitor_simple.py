...
class TestPerformanceMonitor:
    ...
    def setup_method(self):
        ...
        self.config = ComfyUIConfig(
            installation_path=Path("C:/storycore-engine/comfyui_portable")
        )
    ...
    def test_workflow_complexity_analysis(self):
        ...
        workflow = ComfyUIWorkflow(
            metadata=WorkflowMetadata(
                workflow_id="test_workflow_123", created_at=datetime.utcnow()
            ),
            nodes={
                "1": WorkflowNode(
                ...
                "3": WorkflowNode(class_type="VAEDecode", inputs={"samples": ["2", 0]}),
            },
        )
    ...
    def test_workflow_complexity_error_handling(self):
        ...
        analysis = self.performance_monitor.analyze_workflow_complexity(
            invalid_workflow
        )
        ...
        assert (
            "Error analyzing workflow complexity" in analysis.optimization_suggestions
        )
    ...
    def test_processing_time_estimation(self):
        ...
        time_estimate = self.performance_monitor._estimate_workflow_processing_time(
            nodes
        )
    ...
    def test_optimization_suggestions(self):
        ...
        assert (
            "high node count" in suggestions_text
            or "simplifying workflow" in suggestions_text
        )
        assert (
            "memory usage" in suggestions_text or "smaller models" in suggestions_text
        )
        assert (
            "processing time" in suggestions_text
            or "sampling parameters" in suggestions_text
        )
    ...
    def test_resource_snapshot_collection(self, mock_disk, mock_memory, mock_cpu):
        ...
        mock_memory.return_value = Mock(
            percent=60.2, used=8000000000, available=4000000000
        )
    ...
    def test_resource_threshold_checking(self):
        ...
        critical_alerts = [
            a for a in new_alerts if a.severity == AlertSeverity.CRITICAL
        ]
    ...
    def test_operation_threshold_checking(self):
        ...
        self.performance_monitor._check_operation_thresholds(
            "service_start", slow_duration, True
        )
    ...
    def test_workflow_recommendations(self):
        ...
        workflow = ComfyUIWorkflow(
            metadata=WorkflowMetadata(
                workflow_id="test_recommendations", created_at=datetime.utcnow()
            ),
            nodes={"1": WorkflowNode(class_type="KSampler", inputs={"steps": 50})},
        )

        self.performance_monitor.analyze_workflow_complexity(workflow)

        # Get recommendations
        recommendations = self.performance_monitor.get_workflow_recommendations(
