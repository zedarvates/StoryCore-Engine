...
class TestCameraMovementProperties:
    ...
    def test_property_ve_5_camera_movement_accuracy(self, frames, movement_spec):
        ...
        **Validates: Requirements VE-2.1, VE-2.2, VE-7.7**
        ...
        for i, (original, transformed) in enumerate(
            zip(frames, result.transformed_frames)
        ):
        ...
        assert (
            result.movement_metadata["movement_type"]
            == movement_spec.movement_type.value
        )
    ...
    def test_property_ve_6_motion_curve_smoothness(
        self, start_angle, end_angle, duration
    ):
        """
        ...
        **Validates: Requirements VE-2.3, VE-2.6, VE-5.6**
        ...
        frames = [
            np.random.randint(0, 255, (240, 320, 3), dtype=np.uint8) for _ in range(6)
        ]
        ...
        if len(velocities) > 2:
            ...
            velocity_changes = [
                abs(velocities[i + 1] - velocities[i])
        ...
        if len(accelerations) > 2:
            ...
            if finite_accelerations:
                ...
                assert max_acceleration < 1000.0, (
                    f"Excessive acceleration: {max_acceleration}"
                )
    ...
    def test_property_ve_7_compound_movement_coherence(
        self, start_zoom, end_zoom, duration
    ):
        """
        ...
        **Validates: Requirements VE-2.4, VE-2.7**
        ...
        zoom_result = camera_system.apply_movement(
            frames, zoom_movement, frame_rate=24.0
        )

        # Apply compound movement
        compound_result = camera_system.apply_compound_movement(
            frames, compound, frame_rate=24.0
        )
        ...
        assert compound_result.success, (
            f"Compound movement failed: {compound_result.error_message}"
        )
        ...
        assert compound_result.movement_metadata.get("compound_movement")
    ...
    def test_property_ve_5_movement_type_consistency(self, frames):
        ...
        **Validates: Requirements VE-2.1, VE-2.2**
        ...
        for movement_type in movement_types:
            ...
            result = camera_system.apply_movement(
                frames, movement_spec, frame_rate=24.0
            )

            # Movement should succeed
            assert result.success, (
                f"{movement_type.value} movement failed: {result.error_message}"
            )
    ...
    @given(
        st.sampled_from(list(EasingFunction)), st.floats(min_value=0.5, max_value=2.0)
    )
    @settings(max_examples=10, deadline=1000)
    def test_property_ve_6_easing_function_behavior(self, easing_function, duration):
        ...
        **Validates: Requirements VE-2.3, VE-2.6**
        ...
        # Create test frames (smaller for performance)
        frames = [
            np.random.randint(0, 255, (240, 320, 3), dtype=np.uint8) for _ in range(8)
        ]
        ...
        assert result.success, (
            f"Movement with {easing_function.value} failed: {result.error_message}"
        )
        ...
        if len(easing_values) > 2:
            ...
            assert easing_values[0] <= 0.1, (
                f"Easing should start near 0: {easing_values[0]}"
            )
            ...
            assert easing_values[-1] >= 0.5, (
                f"Easing should progress toward 1: {easing_values[-1]}"
            )
            ...
            if easing_function == EasingFunction.LINEAR:
                ...
                if len(easing_values) > 3:
                    steps = [
                        easing_values[i + 1] - easing_values[i]
                        for i in range(len(easing_values) - 1)
                    ]
                    step_variance = np.var(steps)
                    assert step_variance < 0.1, (
                        f"Linear easing not uniform: variance {step_variance}"
                    )
...
def test_camera_movement_basic_functionality():
    ...
    assert zoom_result.success, (
        f"Basic zoom movement failed: {zoom_result.error_message}"
    )
