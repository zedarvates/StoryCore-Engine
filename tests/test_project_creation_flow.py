...
from src.wizard.story_documentation import StoryDocumentationGenerator
...
def create_mock_config() -> ProjectConfiguration:
    ...
    return ProjectConfiguration(
        ...
        genre={
            "key": "drama",
            "name": "Drama",
            "style_defaults": {"visual_style": "cinematic", "color_palette": "warm"},
        },
        story="A test story about a hero's journey through challenges and triumphs.",
        style_config={"visual_style": "cinematic", "color_palette": "warm"},
        technical_specs={"resolution": "1920x1080", "frame_rate": 24},
    )
    ...
    try:
        ...
        with patch.object(Path, "mkdir", tracked_mkdir):
            with patch("builtins.open", tracked_open):
                ...
                writer.create_project_files(config, wizard_state)
        ...
        if project_dir_created_idx is not None and project_dir_created_idx == 0:
            ...
            print(
                f"[FAIL] Project directory NOT created first (index: {project_dir_created_idx})"
            )
            all_passed = False
        ...
        if story_dir_created_idx is not None and story_doc_written_idx is not None:
            if story_dir_created_idx < story_doc_written_idx:
                print(
                    f"[PASS] Story directory created BEFORE documentation (dir: {story_dir_created_idx}, doc: {story_doc_written_idx})"
                )
            else:
                print(
                    f"[FAIL] Story directory NOT created before documentation (dir: {story_dir_created_idx}, doc: {story_doc_written_idx})"
                )
                all_passed = False
            ...
            print(
                f"[PASS] Story directory created at index {story_dir_created_idx} (no docs written yet)"
            )
        else:
            print(
                f"[FAIL] Could not verify story directory order (dir: {story_dir_created_idx}, doc: {story_doc_written_idx})"
            )
            all_passed = False
...
def test_story_directory_exists_before_documentation():
    ...
    try:
        Path(temp_dir) / "test_project_creation"
        ...
        def patched_generate_all_files(self, output_dir):
            ...
            print(
                f"  Story dir exists when generate_all_files called: {story_dir_exists_when_doc_called}"
            )
        ...
        if story_dir_exists_when_doc_called:
            ...
            print(
                "  This confirms the story/ folder is created BEFORE documentation generation"
            )
            return True
        else:
            print(
...
def test_project_creation_without_wizard_state():
    ...
    try:
        ...
        writer = FileWriter(temp_dir)
        writer.create_project_files(config, wizard_state=None)
        ...
        if story_path.exists():
            ...
            if len(story_contents) == 0:
                ...
                print(
                    f"[FAIL] Story directory has unexpected contents: {[p.name for p in story_contents]}"
                )
                all_passed = False
...
def main():
    ...
    results["All Files and Directories Created"] = (
        test_all_files_and_directories_created()
    )
    results["Story Directory Exists Before Documentation"] = (
        test_story_directory_exists_before_documentation()
    )
    results["Project Creation Without Wizard State"] = (
        test_project_creation_without_wizard_state()
    )
    ...
    if all_passed:
        ...
        print(
            "  2. Creates the directory structure (including story/) BEFORE documentation"
        )
