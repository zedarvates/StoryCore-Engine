...
def model_info_data(draw):
    ...
    return ModelInfo(
        ...
        supported_formats=draw(
            st.lists(st.text(min_size=1, max_size=10), min_size=1, max_size=5)
        ),
        dependencies=draw(
            st.one_of(st.none(), st.lists(st.text(min_size=1, max_size=20), max_size=5))
        ),
...
class TestPlatformManagerProperties:
    ...
    def test_property_27_model_management_consistency(self, platform_type):
        ...
        """
        with patch.object(
            PlatformManager, "_detect_platform", return_value=platform_type
        ):
            ...
            for model_key, model_info in list(all_models.items())[
                ...
                for result in results[1:]:
                    assert result == first_result, (
                        f"Inconsistent validation for {model_info.name}"
                    )
    ...
    def test_property_28_cross_platform_compatibility(self, capabilities):
        ...
        with (
            patch.object(
                PlatformManager, "_get_platform_capabilities", return_value=capabilities
            ),
            patch.object(
                PlatformManager,
                "_detect_platform",
                return_value=capabilities.platform_type,
            ),
        ):
            manager = PlatformManager(self.config)
            ...
            for i, gpu_data in enumerate(gpu_info):
                ...
                assert (
                    gpu_data["memory_available_mb"] == original_gpu.memory_available_mb
                )
            ...
            for complexity in complexities:
                ...
                assert (
                    settings["cpu_threads"] <= capabilities.cpu_cores * 2
                )  # Allow hyperthreading

                # Memory management should be appropriate
                if capabilities.gpu_info:
                    max_gpu_memory = max(
                        gpu.memory_total_mb for gpu in capabilities.gpu_info
                    )
