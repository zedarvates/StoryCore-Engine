...
def test_property_directory_creation_idempotent(project_name, project_type):
    ...
    try:
        ...
        missing_items = director_manager.validate_structure(project_path)
        assert len(missing_items) == 0, (
            "Structure should remain valid after multiple creations"
        )
...
def test_property_validate_structure_detects_complete(project_name, project_type):
    ...
    try:
        ...
        assert isinstance(missing_items, list), (
            "validate_structure should return a list"
        )
        assert len(missing_items) == 0, (
            f"Complete structure should have no missing items, found: {missing_items}"
        )
...
def test_property_json_file_validity(project_name, project_type, objectives):
    ...
    try:
        ...
        with open(config_path, "r", encoding="utf-8") as f:
            ...
            assert "creation_timestamp" in config_data, (
                "Config should have creation_timestamp"
            )
        ...
        with open(variables_path, "r", encoding="utf-8") as f:
            ...
            assert "schema_version" in variables_data, (
                "Variables should have schema_version"
            )
            assert "last_updated" in variables_data, (
                "Variables should have last_updated"
            )
...
def test_property_json_schema_conformance(project_name, project_type):
    ...
    try:
        ...
        with open(project_path / "project_config.json", "r", encoding="utf-8") as f:
            ...
            assert is_valid, (
                f"project_config.json should conform to schema, errors: {errors}"
            )

        # Verify: memory.json conforms to schema
        with open(
            project_path / "assistant" / "memory.json", "r", encoding="utf-8"
        ) as f:
        ...
        with open(
            project_path / "assistant" / "variables.json", "r", encoding="utf-8"
        ) as f:
            variables_data = json.load(f)
            is_valid, errors = validate_schema(variables_data, VARIABLES_SCHEMA)
            assert is_valid, (
                f"variables.json should conform to schema, errors: {errors}"
            )

        # Verify: errors_detected.json conforms to schema
        with open(
            project_path / "build_logs" / "errors_detected.json", "r", encoding="utf-8"
        ) as f:
            errors_data = json.load(f)
            is_valid, errors = validate_schema(errors_data, ERRORS_SCHEMA)
            assert is_valid, (
                f"errors_detected.json should conform to schema, errors: {errors}"
            )
