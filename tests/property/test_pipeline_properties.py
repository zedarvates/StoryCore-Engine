...
def project_paths(draw):
    ...
    project_path = (
        Path(temp_dir)
        / f"test_project_{draw(st.integers(min_value=1, max_value=1000))}"
    )
    ...
    project_data = {"project_name": project_path.name, "schema_version": "1.0"}
...
class TestPipelineExecutionProperty:
    ...
    @pytest.mark.asyncio
    @given(
        project_path=project_paths(), seed=st.integers(min_value=0, max_value=2**31 - 1)
    )
    ...
    async def test_complete_pipeline_execution(self, project_path, seed):
        ...
        with patch(
            "src.end_to_end.pipeline_executor.PipelineExecutor._run_command"
        ) as mock_run:
            ...
            assert (
                result.total_duration_seconds >= total_step_duration * 0.9
            )  # Allow 10% variance
            ...
            if PipelineStep.GRID in step_order and PipelineStep.PROMOTE in step_order:
                assert step_order.index(PipelineStep.GRID) < step_order.index(
                    PipelineStep.PROMOTE
                )
            if PipelineStep.PROMOTE in step_order and PipelineStep.REFINE in step_order:
                assert step_order.index(PipelineStep.PROMOTE) < step_order.index(
                    PipelineStep.REFINE
                )
    ...
    @settings(max_examples=100, deadline=None)
    async def test_autofix_loop_execution(
        self, project_path, qa_threshold, max_iterations, seed
    ):
        """
        ...
        with patch(
            ...
            result = await executor.execute_with_autofix(
                project_path, max_iterations=max_iterations, qa_threshold=qa_threshold
            )
    ...
    async def test_pipeline_handles_step_failures(self, project_path, seed):
        ...
        with patch(
            ...
            promote_result = next(
                (r for r in result.step_results if r.step == PipelineStep.PROMOTE), None
            )
