...
from character_wizard.integration_manager import CharacterIntegrationManager
...
def character_profile_strategy(draw):
    ...
    profile.visual_identity.hair_color = draw(
        st.sampled_from(["black", "brown", "blonde", "red", "white", "gray"])
    )
    profile.visual_identity.hair_style = draw(
        st.sampled_from(["short", "long", "curly", "straight", "wavy"])
    )
    profile.visual_identity.hair_length = draw(
        st.sampled_from(["short", "medium", "long"])
    )
    profile.visual_identity.eye_color = draw(
        st.sampled_from(["brown", "blue", "green", "hazel", "gray"])
    )
    profile.visual_identity.eye_shape = draw(
        st.sampled_from(["round", "almond", "narrow"])
    )
    profile.visual_identity.skin_tone = draw(
        st.sampled_from(["fair", "medium", "tan", "dark"])
    )
    profile.visual_identity.facial_structure = draw(
        st.sampled_from(["oval", "round", "square", "heart"])
    )
    profile.visual_identity.age_range = draw(
        st.sampled_from(["child", "teen", "young_adult", "adult", "elderly"])
    )
    profile.visual_identity.height = draw(st.sampled_from(["short", "average", "tall"]))
    profile.visual_identity.build = draw(
        st.sampled_from(["slim", "average", "athletic", "heavy"])
    )
    profile.visual_identity.clothing_style = draw(
        st.sampled_from(["casual", "formal", "fantasy", "modern"])
    )
    profile.visual_identity.art_style = draw(
        st.sampled_from(["realistic", "anime", "cartoon", "artistic"])
    )
    profile.visual_identity.rendering_style = draw(
        st.sampled_from(["photorealistic", "stylized", "painterly"])
    )
    profile.visual_identity.quality_level = "high"

    # Color palette
    profile.visual_identity.color_palette = ColorPalette()
    profile.visual_identity.color_palette.primary_colors = [
        "#" + draw(st.text(min_size=6, max_size=6, alphabet="0123456789ABCDEF"))
    ]
    ...
    profile.personality_profile.conscientiousness = draw(
        st.floats(min_value=0.0, max_value=1.0)
    )
    profile.personality_profile.extraversion = draw(
        st.floats(min_value=0.0, max_value=1.0)
    )
    profile.personality_profile.agreeableness = draw(
        st.floats(min_value=0.0, max_value=1.0)
    )
    profile.personality_profile.neuroticism = draw(
        st.floats(min_value=0.0, max_value=1.0)
    )
    profile.personality_profile.primary_traits = draw(
        st.lists(st.text(min_size=3, max_size=20), min_size=1, max_size=5)
    )
    profile.personality_profile.strengths = draw(
        st.lists(st.text(min_size=3, max_size=20), min_size=1, max_size=3)
    )
    profile.personality_profile.flaws = draw(
        st.lists(st.text(min_size=3, max_size=20), min_size=1, max_size=3)
    )
    ...
    profile.voice_identity.vocabulary_level = draw(
        st.sampled_from(["simple", "moderate", "advanced"])
    )
    ...
    profile.backstory_profile.education_level = draw(
        st.sampled_from(["none", "basic", "high school", "college", "advanced"])
    )

    # Coherence anchors
    profile.coherence_anchors = CoherenceAnchors()
    profile.coherence_anchors.character_descriptor = (
        f"{profile.name}, {profile.visual_identity.age_range}"
    )
    profile.coherence_anchors.seed_base = draw(
        st.integers(min_value=0, max_value=999999)
    )
    profile.coherence_anchors.cfg_scale = draw(st.floats(min_value=1.0, max_value=20.0))
    profile.coherence_anchors.denoising_strength = draw(
        st.floats(min_value=0.1, max_value=1.0)
    )

    # Integration data
    profile.puppet_category = draw(st.sampled_from(list(PuppetCategory)))
    profile.genre_tags = draw(
        st.lists(st.text(min_size=3, max_size=20), min_size=0, max_size=5)
    )
    profile.style_tags = draw(
        st.lists(st.text(min_size=3, max_size=20), min_size=0, max_size=5)
    )
        ...
        **Validates: Requirements 9.1, 9.2**
        ...
        try:
            ...
            try:
                ...
                pytest.fail(
                    f"Invalid UUID format for character_id: {saved_data['character_id']}"
                )
