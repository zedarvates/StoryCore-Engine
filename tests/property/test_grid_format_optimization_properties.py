...
from src.grid_format_optimization.exceptions import UnsupportedFormatError
...
def content_analysis_strategy(draw):
    ...
    return ContentAnalysis(
        ...
        dominant_colors=draw(
            st.lists(st.text(min_size=3, max_size=10), min_size=1, max_size=5)
        ),
...
def format_preferences_strategy(draw):
    ...
    return FormatPreferences(
        preferred_formats=draw(
            st.lists(st.sampled_from(formats), min_size=1, unique=True)
        ),
        ...
        maximum_processing_time=draw(
            st.one_of(st.none(), st.floats(min_value=1.0, max_value=600.0))
        ),
...
def project_data_strategy(draw):
    ...
    return {
        ...
        "characters": [
            f"character_{i}" for i in range(draw(st.integers(min_value=0, max_value=5)))
        ],
    }
    ...
    def test_property_6_content_based_selection(self, format_type):
        ...
        **Valide: Exigences 2.1, 2.2, 2.3**
        ...
        if format_type == GridFormat.LINEAR_1X2:
            assert any(
                content in spec.optimal_for for content in ["dialogue", "portrait"]
            )
