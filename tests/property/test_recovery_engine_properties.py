    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if error.recovery_attempts < RecoveryEngine.MAX_RECOVERY_ATTEMPTS:
            assert len(result.actions_taken) > 0, (
                "Should have attempted some repair actions"
            )


# Property 44: Repair Attempt Logging
# Feature: storycore-llm-memory-system, Property 44: Repair Attempt Logging
@given(project_name=project_name_strategy(), error=missing_file_error_strategy())
@settings(max_examples=100, deadline=None)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if result.success:
            updated = recovery_engine.update_error_status(
                error.id, "resolved", error.recovery_attempts + 1
            )
            ...
            if found_error:
                ...
                assert (
                    found_error["recovery_attempts"] == error.recovery_attempts + 1
...
@given(project_name=project_name_strategy(), error_type=error_type_strategy())
@settings(max_examples=100, deadline=None)
def test_property_46_failed_repair_marking(project_name, error_type):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not result.success:
            ...
            updated = recovery_engine.update_error_status(
                error.id, "requires_manual_intervention", error.recovery_attempts + 1
            )
...
def test_property_47_recovery_attempt_limiting(project_name, recovery_attempts):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if recovery_attempts >= RecoveryEngine.MAX_RECOVERY_ATTEMPTS:
            ...
            assert (
                "maximum" in result.reason.lower()
                or "exceeded" in result.reason.lower()
...
def test_property_50_memory_reconstitution(project_name):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        build_logger.log_memory_update(
            "objectives", {"added": "Complete recovery system"}
        )
...
def test_property_52_recovery_report_generation(project_name):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert isinstance(report.restored_files, list), (
            "Report should list restored files"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not report.success:
            ...
            for lost_file in report.lost_files:
                assert isinstance(lost_file, Path), "Lost files should be Path objects"
