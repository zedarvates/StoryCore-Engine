...
from src.memory_system.data_models import AssetType
...
class TestAssetManagerProperties:
    ...
    def test_property_20_asset_type_based_routing(
        self, temp_project, filename_and_type, content
    ):
        """
    ...
    def test_property_21_asset_index_entry_completeness(
        self, temp_project, filename_and_type, content
    ):
        """
    ...
    def test_property_22_asset_metadata_generation(
        self, temp_project, filename_and_type, content
    ):
        """
        ...
        **Validates: Requirements 6.3**
        ...
        # Store asset
        asset_manager = AssetManager(temp_project)
        asset_info = asset_manager.store_asset(
            source_path, asset_type, description="Test asset"
        )
    ...
    @given(
        assets=st.lists(
            st.tuples(valid_filename(), file_content()), min_size=1, max_size=5
        )
    )
    @settings(
        max_examples=50,
    ...
    def test_property_24_asset_modification_triggers_summarization(
        self, temp_project, filename_and_type, content
    ):
        """
    ...
    @given(
        assets=st.lists(
            st.tuples(valid_filename(), file_content()), min_size=2, max_size=10
        )
    )
    @settings(
        max_examples=50,
