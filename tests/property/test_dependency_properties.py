...
def test_property_11_dependency_verification(projects_dir, comfyui_url):
    ...
    manager = DependencyManager(projects_dir=projects_dir, comfyui_url=comfyui_url)
    ...
    for required_module in manager.REQUIRED_MODULES:
        assert required_module in checked_modules, (
            f"Required module {required_module} not checked"
        )
    ...
    for check in result.checks:
        if check.status in [DependencyStatus.MISSING, DependencyStatus.INSUFFICIENT]:
            assert check.instructions is not None, (
                f"Check {check.name} missing instructions"
            )
            assert len(check.instructions) > 0, (
                f"Check {check.name} has empty instructions"
            )
    ...
    if critical_failures:
        ...
        assert not result.all_satisfied, (
            "Cannot be all satisfied with critical failures"
        )
    ...
    if backend_checks:
        ...
        if backend_check.status == DependencyStatus.UNAVAILABLE:
            ...
            other_failures = [
                ...
                if check.dependency_type != DependencyType.BACKEND_SERVICE
                and check.status
                in [DependencyStatus.MISSING, DependencyStatus.INSUFFICIENT]
            ]
...
def test_property_backend_checking(comfyui_url):
    ...
    if backend_check.status == DependencyStatus.UNAVAILABLE:
        ...
        assert (
            "fallback" in backend_check.instructions.lower()
            or "placeholder" in backend_check.instructions.lower()
        )
...
def test_property_module_checking(projects_dir):
    ...
    for check in module_checks:
        # Must have a status
        assert check.status in [DependencyStatus.AVAILABLE, DependencyStatus.MISSING]

        # If missing, must have installation instructions
        if check.status == DependencyStatus.MISSING:
            assert check.instructions is not None
            assert (
                "pip install" in check.instructions.lower()
                or "install" in check.instructions.lower()
            )
