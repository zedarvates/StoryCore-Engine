...
def test_property_29_comprehensive_action_logging(
    action_type, affected_files, parameters, triggered_by
):
    """
    Property 29: Comprehensive Action Logging
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert action_type in log_content, (
            f"Action type '{action_type}' should be in log"
        )
        assert f"[{datetime.now().date().isoformat()}" in log_content, (
...
@settings(max_examples=100, deadline=None)
def test_property_30_log_entry_structure_completeness(
    action_type, affected_files, parameters, triggered_by
):
    """
    Property 30: Log Entry Structure Completeness
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "[" in log_content and "]" in log_content, (
            "Log should contain timestamp in brackets"
        )
        ...
        if affected_files:
            assert "Files:" in log_content or any(
                f in log_content for f in affected_files
            ), "Log should contain affected files"

        # Check parameters section
        if parameters:
            assert "Parameters:" in log_content or any(
                k in log_content for k in parameters.keys()
            ), "Log should contain parameters"

        # Check triggered_by
        assert f"Triggered_By: {triggered_by}" in log_content, (
            "Log should contain triggered_by"
        )


# Property 31: Log Format Consistency
# Feature: storycore-llm-memory-system, Property 31: Log Format Consistency
@given(num_actions=st.integers(min_value=2, max_value=10))
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for line in action_lines:
            assert line.startswith("["), (
                "Action line should start with timestamp bracket"
            )
        ...
        assert triggered_by_count == num_actions, (
            "Each action should have Triggered_By field"
        )
...
def test_property_32_log_immutability(initial_actions, additional_actions):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(final_content) > initial_length, "Log should grow with new entries"
...
def test_log_decision():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = logger.log_decision(
            ...
            details={
                "rationale": "Better component model",
