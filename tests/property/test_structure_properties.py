...
def project_components_data(draw):
    ...
    world_config = WorldConfig(
        ...
        visual_style=draw(
            st.lists(
                st.sampled_from(["cinematic", "noir", "vibrant"]),
                min_size=1,
                max_size=3,
            )
        ),
    ...
    if has_dialogue:
        ...
        dialogue_script = DialogueScript(
            script_id=str(uuid.uuid4()), scenes=[], total_lines=0, estimated_duration=0
        )
    ...
    music_description = MusicDescription(
        ...
        mood=draw(
            st.lists(
                st.sampled_from(["epic", "mysterious", "tense"]), min_size=1, max_size=3
            )
        ),
        tempo=draw(st.sampled_from(["slow", "medium", "fast"])),
        instruments=draw(
            st.lists(
                st.sampled_from(["strings", "synth", "drums"]), min_size=1, max_size=3
            )
        ),
        sound_effects=[],
        timeline=[
    ...
    def test_property_4_creates_all_subdirectories(self, components):
        ...
        **Validates: Requirement 4.2**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert structure.exports_path.exists(), "exports directory must exist"
            ...
            assert structure.exports_path.is_dir(), "exports must be a directory"
    ...
    def test_property_4_creates_project_json(self, components):
        ...
        **Validates: Requirement 4.3**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert structure.project_json_path.exists(), "project.json must exist"
            ...
            assert "generation_status" in data, (
                "project.json must have generation_status"
            )
    ...
    def test_property_4_creates_world_config(self, components):
        ...
        **Validates: Requirement 4.5**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert structure.world_config_path.exists(), "world_config.json must exist"
    ...
    def test_property_4_creates_characters(self, components):
        ...
        **Validates: Requirement 4.6**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert structure.characters_path.exists(), "characters.json must exist"
    ...
    def test_property_4_project_path_structure(self, components):
        ...
        **Validates: Requirements 4.1, 4.2**
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                structure.assets_images_path.parent.parent == structure.project_path
            ), "assets/images must be under project directory"
            assert (
                structure.assets_audio_path.parent.parent == structure.project_path
