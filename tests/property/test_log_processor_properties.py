...
def test_property_33_automatic_log_cleaning(num_entries):
    ...
    try:
        ...
        assert clean_log_path.name == "build_steps_clean.txt", (
            "Clean log should have correct filename"
        )
    ...
    try:
        ...
        for line in action_lines:
            assert line.startswith("["), (
                "Action lines should start with timestamp bracket"
            )
...
def test_property_35_automatic_log_translation(target_lang):
    ...
    try:
        ...
        assert translated_log_path.name == "build_steps_translated.txt", (
            "Translated log should have correct filename"
        )
...
def test_language_support(temp_project_dir):
    ...
    # Create a log entry
    logger.log_action(
        action_type="FILE_CREATED", affected_files=["test.txt"], parameters={}
    )
...
def test_get_log_summary(temp_project_dir):
    ...
    for i in range(3):
        logger.log_action(
            action_type="FILE_CREATED", affected_files=[f"file_{i}.txt"], parameters={}
        )
    ...
    assert summary["action_types"]["FILE_CREATED"] == 3, (
        "Should count action types correctly"
    )
