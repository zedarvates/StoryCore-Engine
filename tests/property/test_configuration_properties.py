...
def parsed_prompt_strategy(draw):
    ...
    return ParsedPrompt(
        ...
        characters=[],
        key_elements=draw(
            st.lists(st.text(min_size=3, max_size=30), min_size=0, max_size=10)
        ),
        visual_style=draw(
            st.lists(st.text(min_size=3, max_size=30), min_size=1, max_size=5)
        ),
        aspect_ratio=draw(
            st.sampled_from(["16:9", "9:16", "1:1", "21:9", "4:3", "auto"])
        ),
...
@given(
    ...
    video_type=st.sampled_from(
        ["trailer", "teaser", "short_film", "social_media", "cinematic"]
    ),
)
...
@given(system_capabilities=system_capabilities_strategy())
