...
async def test_end_to_end_integration_with_async_queue():
    ...
    q.start()
    try:
        task_id = await q.submit_task("integration-task-1", sample_operation, priority=TaskPriority.NORMAL)
...
async def test_task_priority_ordering():
    ...
    try:
        ...
        await q.submit_task("critical", critical_priority, priority=TaskPriority.CRITICAL)
...
async def test_circuit_breaker_integration():
    ...
    try:
        ...
        for i in range(5):
            await q.submit_task(f"fail-task-{i}", failing_operation, priority=TaskPriority.NORMAL)
        ...
        assert cb['state'] in ['closed', 'open', 'half_open'], "Invalid circuit breaker state"
    finally:
        q.stop()
...
async def test_rate_limiting_integration():
    ...
    try:
        ...
        for i in range(10):
            await q.submit_task(f"rate-task-{i}", sample_operation, priority=TaskPriority.NORMAL)
...
async def test_task_cancellation():
    ...
    try:
        # Submit a slow task
        task_id = await q.submit_task("cancel-task", slow_operation, priority=TaskPriority.NORMAL)
...
async def test_queue_statistics():
    ...
    try:
        # Submit some tasks
        await q.submit_task("stats-task-1", sample_operation, priority=TaskPriority.NORMAL)
        await q.submit_task("stats-task-2", sample_operation, priority=TaskPriority.HIGH)
