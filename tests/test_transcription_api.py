...
class TestTranscriptionAPI:
    ...
    def test_transcribe_missing_url(self):
        ...
        response = client.post(
            "/api/v1/transcription/transcribe", json={"language": "fr"}
        )
    ...
    def test_transcribe_valid_request(self):
        ...
        response = client.post(
            ...
            json={"audio_url": "/assets/audio/test.wav", "language": "fr"},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_export_srt(self):
        ...
        if response.status_code == 200:
            ...
            export_response = client.get(
                f"/api/v1/transcription/{transcript_id}/export/srt"
            )
    ...
    def test_export_vtt(self):
        ...
        if response.status_code == 200:
            ...
            # Export to VTT
            export_response = client.get(
                f"/api/v1/transcription/{transcript_id}/export/vtt"
            )
            assert export_response.status_code == 200
            content = export_response.text
    ...
    def test_generate_montage_valid(self):
        ...
        if response.status_code == 200:
            ...
            # Generate montage
            montage_response = client.post(
                "/api/v1/transcription/generate-montage",
                json={"transcript_id": transcript_id, "style": "chronologique"},
            )
