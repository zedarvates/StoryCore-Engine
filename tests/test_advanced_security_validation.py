...
class TestModelIntegrityChecker:
    ...
    def test_model_integrity_with_checksums(self, checker, temp_model_file):
        ...
        checker.calculate_file_hash(temp_model_file)
...
class TestRateLimiter:
    ...
    def config(self):
        ...
        return SecurityConfig(
            enable_rate_limiting=True, requests_per_minute=5, burst_limit=2
        )
...
class TestAuditLogger:
    ...
    def config(self):
        """Create test configuration"""
        return SecurityConfig(enable_audit_logging=True, anonymize_user_data=False)
    ...
    def test_anonymization(self):
        ...
        config = SecurityConfig(enable_audit_logging=True, anonymize_user_data=True)
        logger = AuditLogger(config)
