...
def health_check_scenario(draw):
    ...
    if scenario_type == "success":
        ...
        return {
            ...
            "error": draw(
                st.sampled_from(
                    ["ClientConnectorError", "ClientTimeout", "ConnectionRefusedError"]
                )
            ),
        }
        ...
        return {
            "type": "timeout",
            "delay_ms": draw(
                st.floats(min_value=6000, max_value=10000)
            ),  # Longer than 5s timeout
        }
...
class TestHealthMonitorProperties:
    ...
    def test_property_4_health_check_reliability(self, scenario):
        ...
        **Feature: comfyui-integration, Property 4: Health Check Reliability**
        ...
        async def run_test():
            ...
            with patch(
                "src.health_monitor.aiohttp.ClientSession"
                ...
                if scenario["type"] == "success":
                    ...
                    mock_response.json = AsyncMock(
                        return_value=scenario["response_data"]
                    )
                ...
                assert (end_time - start_time) < 10.0, (
                    "Health check should not hang indefinitely"
                )
                ...
                if scenario["type"] == "success":
                    ...
                    ):  # Timeout might not have error message
    ...
    def test_property_6_reconnection_backoff(self, failure_count):
        ...
        **Feature: comfyui-integration, Property 6: Reconnection Backoff**
        ...
        expected_delay = monitor._base_backoff_seconds * (
            monitor._backoff_multiplier ** (failure_count - 1)
        )
        ...
        async def run_test():
            with patch("aiohttp.ClientSession") as mock_session:
                ...
                assert elapsed <= timeout_seconds + 0.5, (
                    f"Should timeout within {timeout_seconds}s, took {elapsed}s"
                )
