...
class TestHardwareAdaptationProperties:
    ...
    def test_property_ve27_gpu_acceleration_adaptation(
        self, cpu_cores, memory_gb, gpu_available, gpu_memory_gb
    ):
        """
        ...
        with patch.object(CrossPlatformManager, "_assess_capabilities") as mock_assess:
            capabilities = PlatformCapabilities(
                ...
                supported_processing_modes=[
                    ProcessingMode.GPU_CUDA
                    if gpu_available
                    else ProcessingMode.CPU_ONLY,
                    ProcessingMode.AUTO,
            ...
            if gpu_available:
                ...
                assert (
                    ProcessingMode.GPU_CUDA in capabilities.supported_processing_modes
                )

                # Optimal config should prefer GPU when available
                optimal_config = manager.get_optimal_config()
                assert optimal_config["processing"]["mode"] in [
                    ProcessingMode.GPU_CUDA.value,
                    ProcessingMode.AUTO.value,
                ]

                # GPU memory should be considered in configuration
                if (
                    gpu_memory_gb
                    and gpu_memory_gb >= 4.0
                    and memory_gb >= 4.0
                    and cpu_cores > 2
                ):
                ...
                assert (
                    ProcessingMode.GPU_CUDA
                    not in capabilities.supported_processing_modes
                )
                assert (
                    ProcessingMode.CPU_ONLY in capabilities.supported_processing_modes
                )

                # Optimal config should use CPU processing
                optimal_config = manager.get_optimal_config()
                assert optimal_config["processing"]["mode"] in [
                    ProcessingMode.CPU_ONLY.value,
                    ProcessingMode.AUTO.value,
                ]
    ...
    def test_property_ve27_cpu_performance_scaling(
        self, cpu_cores, memory_gb, workload_complexity
    ):
        """
        ...
        with patch.object(CrossPlatformManager, "_assess_capabilities") as mock_assess:
            ...
            if cpu_cores == 1:
                assert not adapted_config["parallel_processing"]
                assert adapted_config["max_workers"] == 1

            # For multi-core systems, enable parallel processing
            elif cpu_cores > 2:
                assert adapted_config["parallel_processing"]
                assert adapted_config["max_workers"] > 1
    ...
    @given(
        platform_type=st.sampled_from(
            [PlatformType.WINDOWS, PlatformType.LINUX, PlatformType.MACOS]
        ),
        gpu_type=st.one_of(
            st.none(), st.sampled_from(["nvidia", "amd", "intel", "metal"])
        ),
        processing_requirements=st.sampled_from(["realtime", "batch", "quality"]),
    )
    @settings(max_examples=10, deadline=5000)
    def test_property_ve27_platform_specific_optimization(
        self, platform_type, gpu_type, processing_requirements
    ):
        """
        ...
        with patch.object(CrossPlatformManager, "_assess_capabilities") as mock_assess:
            ...
            if platform_type == PlatformType.MACOS:
                ...
                if gpu_available and gpu_type == "metal":
                    ...
                    assert (
                        "metal" in str(processing_config).lower()
                        or processing_config["mode"] != ProcessingMode.CPU_ONLY.value
                    )
                ...
                if gpu_available and gpu_type == "nvidia":
                    ...
                    assert processing_config["mode"] in [
                        ProcessingMode.GPU_CUDA.value,
                        ProcessingMode.AUTO.value,
                    ]
            ...
            if processing_requirements == "quality":
                assert adapted_config.get("quality_level", "medium") in [
                    "high",
                    "maximum",
                ]
            elif processing_requirements == "realtime":
                assert (
                    adapted_config.get("batch_size", 4) <= 2
    ...
    def test_property_ve27_resource_constrained_adaptation(
        self, memory_constraint, cpu_constraint, gpu_constraint
    ):
        """
        ...
        with patch.object(CrossPlatformManager, "_assess_capabilities") as mock_assess:
            capabilities = PlatformCapabilities(
                ...
                gpu_memory_gb=1.0
                if gpu_constraint
                else None,  # Very limited GPU memory
                supported_processing_modes=[
                    ProcessingMode.GPU_CUDA
                    if gpu_constraint
                    else ProcessingMode.CPU_ONLY,
            ...
            if (
                gpu_constraint
                and capabilities.gpu_memory_gb
                and capabilities.gpu_memory_gb < 2.0
            ):
...
class TestHardwareAdaptationIntegration:
    ...
    def test_property_ve27_video_engine_hardware_integration(
        self, frame_count, resolution_scale
    ):
        """
        ...
        with tempfile.TemporaryDirectory():
            ...
            with patch.object(
                CrossPlatformManager, "_assess_capabilities"
            ) as mock_assess:
                capabilities = PlatformCapabilities(
                    platform_type=PlatformType.LINUX,
                    cpu_cores=4,
                ...
                config = VideoConfig(
                    frame_rate=24,
                    resolution=(
                        int(1920 * resolution_scale),
                        int(1080 * resolution_scale),
                    ),
                    output_format="png",
                )
                ...
                assert (
                    is_valid or len(issues) == 0
                )  # Should be valid or have no critical issues
...
def test_hardware_adaptation_property_integration():
    ...
    with patch.object(CrossPlatformManager, "_assess_capabilities") as mock_assess:
        ...
        assert report["validation"]["is_compatible"]
        assert report["hardware_capabilities"]["gpu_available"]
