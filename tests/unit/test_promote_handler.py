...
class TestPromoteHandler:
    ...
    def test_setup_parser(self):
        ...
        args = parser.parse_args(
            ["--project", "test-proj", "--scale", "4", "--method", "bicubic"]
        )
    ...
    def test_execute_success(self, tmp_path):
        ...
        args = argparse.Namespace(project=str(project_path), scale=2, method="lanczos")
        ...
        with (
            patch("promotion_engine.promote_panels", return_value=mock_result),
    ...
    def test_execute_project_not_found(self):
        ...
        args = argparse.Namespace(
            project="/nonexistent/path", scale=2, method="lanczos"
        )

        with (
            patch("promotion_engine.promote_panels"),
            patch("promotion_engine.update_project_manifest"),
        ):
            exit_code = handler.execute(args)
            assert exit_code != 0
    ...
    def test_execute_large_scale_warning(self, tmp_path, capsys):
        ...
        with (
            ...
            assert (
                "Large scale factor" in captured.out
                or "may result in very large files" in captured.out
            )
    ...
    def test_execute_import_error(self, tmp_path):
        ...
        with patch(
            "promotion_engine.promote_panels",
    ...
    def test_execute_different_methods(self, tmp_path):
        ...
        for method in methods:
            args = argparse.Namespace(project=str(project_path), scale=2, method=method)
            ...
            with (
                patch(
                    "promotion_engine.promote_panels", return_value=mock_result
                ) as mock_promote,
                patch("promotion_engine.update_project_manifest"),
            ):
                exit_code = handler.execute(args)
