...
class TestErrorHandling:
    ...
    def test_handle_error_tracks_attempts(self):
        ...
        error_key = (
            f"{WorkflowStep.IMAGE_GENERATION.value}:{ErrorType.NETWORK_ERROR.value}"
        )
    ...
    def test_load_checkpoint_restores_state(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(loaded_state.completed_steps) == len(
                original_state.completed_steps
            )
...
class TestRecoveryExecution:
    ...
    async def test_execute_retry_recovery(self):
        ...
        action = RecoveryAction(
            strategy=RecoveryStrategy.RETRY, parameters={}, max_attempts=3
        )
    ...
    async def test_execute_skip_recovery(self):
        ...
        action = RecoveryAction(
            strategy=RecoveryStrategy.SKIP, parameters={}, max_attempts=3
        )
    ...
    async def test_execute_fallback_recovery(self):
        ...
        action = RecoveryAction(
            strategy=RecoveryStrategy.FALLBACK, parameters={}, max_attempts=3
        )
    ...
    async def test_execute_abort_recovery(self):
        ...
        action = RecoveryAction(
            strategy=RecoveryStrategy.ABORT, parameters={}, max_attempts=3
        )
...
class TestUserNotification:
    ...
    def test_get_corrective_actions_dependency_error(self):
        ...
        assert len(actions) > 0
        assert any(
            "install" in action.lower() or "dependencies" in action.lower()
