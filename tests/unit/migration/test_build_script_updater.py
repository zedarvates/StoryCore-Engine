...
def path_mapping(temp_project):
    ...
    return {
        ...
        temp_project / "old_frontend" / "app.tsx": temp_project
        / "frontend"
        / "app.tsx",
        temp_project / "old_electron" / "main.ts": temp_project
        / "electron"
        / "main.ts",
    }
...
class TestBuildScriptUpdater:
    ...
    def test_relative_mapping_creation(self, temp_project, path_mapping):
        ...
        assert (
            "old_src/main.py" in updater.relative_mapping
            or "old_src\\main.py" in updater.relative_mapping
        )
    ...
    def test_update_package_json_files_array(self, temp_project, path_mapping):
        ...
        package_data = {
            "name": "test-project",
            "files": ["old_electron/**/*", "old_frontend/**/*", "old_src/**/*"],
        }
        package_json.write_text(json.dumps(package_data, indent=2))
...
"""
        ...
        assert (
            "frontend" in updated_content
            or "electron" in updated_content
            or "src" in updated_content
        )
    ...
    def test_update_electron_builder_config(self, temp_project, path_mapping):
        ...
        config_data = {
            "appId": "com.test.app",
            "directories": {"output": "old_dist", "buildResources": "old_assets"},
            "files": ["old_electron/**/*", "old_frontend/**/*"],
        }
...
"""
    ...
    def test_update_all_build_scripts(self, temp_project, path_mapping):
        ...
        package_json.write_text(
            json.dumps({"scripts": {"build": "tsc -p old_electron/tsconfig.json"}})
        )
        ...
        assert (
            stats["package_json_updated"]
            or stats["shell_scripts_updated"] > 0
            or stats["batch_scripts_updated"] > 0
        )
    ...
    def test_skip_excluded_directories(self, temp_project, path_mapping):
        ...
        updater = BuildScriptUpdater(temp_project, path_mapping)
        updater.update_all_build_scripts()
    ...
    def test_cross_platform_path_handling(self, temp_project, path_mapping):
        ...
        assert (
            "old_frontend" not in updated_content
            or "old_electron" not in updated_content
        )
