...
class TestSingleFileMovement:
    ...
    def test_move_file_source_not_exists(self, file_mover, temp_git_repo):
        ...
        result = file_mover.move_file(Path("nonexistent.txt"), Path("dest.txt"))
    ...
    def test_move_file_without_git_history(self, file_mover, temp_git_repo):
        ...
        result = file_mover.move_file(
            Path("test_file.txt"), dest, preserve_history=False
        )
...
class TestDryRunMode:
    ...
    def test_dry_run_records_movements(self, dry_run_mover, temp_git_repo):
        ...
        assert all(
            m.status == MovementStatus.SUCCESS for m in dry_run_mover.movement_history
        )
...
class TestBatchMovement:
    ...
    def test_move_batch_success(self, file_mover, temp_git_repo):
        ...
        movements = [
            FileMovement(
                source=Path(f"file{i}.txt"), destination=Path("moved") / f"file{i}.txt"
            )
            for i in range(3)
        ]
...
class TestMovementVerification:
    ...
    def test_verify_movements_all_success(self, file_mover, temp_git_repo):
        ...
        for i in range(3):
            file_mover.move_file(Path(f"file{i}.txt"), Path("moved") / f"file{i}.txt")
    ...
    def test_verify_movements_detects_missing_destination(
        self, file_mover, temp_git_repo
    ):
...
class TestMovementHistory:
    ...
    def test_get_path_mapping(self, file_mover, temp_git_repo):
        ...
        file_mover.move_file(
            Path("nonexistent.txt"), Path("moved/file1.txt")
        )  # This will fail
...
class TestBatchResultProperties:
    ...
    def test_success_rate_calculation(self):
        ...
        result = BatchResult(
            total_files=10, successful=8, failed=2, skipped=0, movements=[]
        )
    ...
    def test_success_rate_zero_files(self):
        ...
        result = BatchResult(
            total_files=0, successful=0, failed=0, skipped=0, movements=[]
        )
    ...
    def test_success_rate_all_failed(self):
        ...
        result = BatchResult(
            total_files=5, successful=0, failed=5, skipped=0, movements=[]
        )
    ...
    def test_success_rate_all_success(self):
        ...
        result = BatchResult(
            total_files=5, successful=5, failed=0, skipped=0, movements=[]
        )
