...
class TestWorldConfigGenerator:
    ...
    def basic_prompt(self):
        ...
        return ParsedPrompt(
            ...
            characters=[
                CharacterInfo(
                    name="Protagonist", role="main", description="Main character"
                )
            ],
    ...
    def test_genre_template_cyberpunk(self, generator, basic_prompt):
        ...
        assert (
            "neon" in world_config.lighting_style.lower()
            or "neon" in world_config.visual_style
        )
        assert any(
            loc.name.lower()
            in [
    ...
    def test_genre_template_fantasy(self, generator, basic_prompt):
        ...
        assert (
            "magical" in world_config.lighting_style.lower()
            or "mystical" in world_config.atmosphere.lower()
        )
    ...
    def test_genre_template_horror(self, generator, basic_prompt):
        ...
        assert (
            "shadow" in world_config.lighting_style.lower()
            or "eerie" in world_config.atmosphere.lower()
        )
    ...
    def test_lighting_style_with_dark_mood(self, generator, basic_prompt):
        ...
        assert (
            "low-key" in world_config.lighting_style.lower()
            or "dark" in world_config.lighting_style.lower()
        )
    ...
    def test_location_generation_forest_setting(self, generator, basic_prompt):
        ...
        location_names = [loc.name.lower() for loc in world_config.key_locations]
        assert any(
            "forest" in name or "wood" in name or "tree" in name or "clearing" in name
            for name in location_names
        )
    ...
    def test_location_generation_space_setting(self, generator, basic_prompt):
        ...
        location_names = [loc.name.lower() for loc in world_config.key_locations]
        assert any(
            "space" in name or "station" in name or "ship" in name or "planet" in name
            for name in location_names
        )
    ...
    def test_location_visual_description_includes_genre(self, generator, basic_prompt):
        ...
        visual_descriptions = [
            loc.visual_description.lower() for loc in world_config.key_locations
        ]
    ...
    def test_integration_with_real_prompt(self, generator):
        ...
        prompt = ParsedPrompt(
            ...
            characters=[
                CharacterInfo(
                    name="Snow White", role="main", description="Cyberpunk Snow White"
                ),
                CharacterInfo(
                    name="Queen", role="antagonist", description="Evil AI Queen"
                ),
            ],
        ...
        assert any(
            style in ["neon", "gritty", "industrial"]
            for style in world_config.visual_style
        )
