...
class TestGridHandler:
    ...
    def test_setup_parser(self):
        ...
        args = parser.parse_args(
            ["--project", "test-proj", "--grid", "1x2", "--cell-size", "1024"]
        )
    ...
    def test_execute_success(self, tmp_path):
        ...
        args = argparse.Namespace(
            project=str(project_path), grid="3x3", out=None, cell_size=512
        )
    ...
    def test_execute_project_not_found(self):
        ...
        handler = GridHandler()
        args = argparse.Namespace(
            project="/nonexistent/path", grid="3x3", out=None, cell_size=512
        )
    ...
    def test_execute_import_error(self, tmp_path):
        ...
        with patch(
            "grid_generator.GridGenerator", side_effect=ImportError("Module not found")
        ):
            exit_code = handler.execute(args)
            assert exit_code != 0
    ...
    def test_execute_with_custom_output(self, tmp_path):
        ...
        args = argparse.Namespace(
            project=str(project_path), grid="1x4", out="custom_grid.png", cell_size=1024
        )
