...
def temp_project():
    ...
    project_data = {"project_name": "test_project", "schema_version": "1.0"}
...
class TestPipelineExecutor:
    ...
    def test_build_command_grid(self, mock_cli_path, temp_project):
        ...
        cmd = executor._build_command(
            PipelineStep.GRID, temp_project, grid="3x3", cell_size=512
        )
    ...
    def test_build_command_promote(self, mock_cli_path, temp_project):
        ...
        executor = PipelineExecutor(str(mock_cli_path))
        cmd = executor._build_command(
            PipelineStep.PROMOTE, temp_project, scale=2, method="lanczos"
        )
    ...
    def test_build_command_qa(self, mock_cli_path, temp_project):
        ...
        executor = PipelineExecutor(str(mock_cli_path))
        cmd = executor._build_command(
            PipelineStep.QA, temp_project, threshold=3.5, detailed=True
        )
    ...
    async def test_execute_step_success(self, mock_cli_path, temp_project):
        ...
        with patch.object(executor, "_run_command") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout="Grid generated successfully", stderr=""
            )

            result = await executor.execute_step(PipelineStep.GRID, temp_project)
        ...
        with patch.object(executor, "_run_command") as mock_run:
            ...
            async def mock_command(cmd):
                ...
                if step_name == "export":
                    ...
                    return MagicMock(
                        returncode=0, stdout=f"Export location: {export_dir}", stderr=""
                    )
                elif step_name == "qa":
                    return MagicMock(
                        returncode=0,
    ...
    @pytest.mark.asyncio
    async def test_execute_full_pipeline_with_failure(
        self, mock_cli_path, temp_project
    ):
        ...
        with patch.object(executor, "_run_command") as mock_run:
            ...
            async def mock_command(cmd):
                ...
                if step_name == "promote":
                    return MagicMock(returncode=1, stdout="", stderr="Promotion failed")
                else:
                    return MagicMock(
                        returncode=0, stdout=f"{step_name} completed", stderr=""
                    )
    ...
    @pytest.mark.asyncio
    async def test_execute_with_autofix_qa_passes_first_time(
        self, mock_cli_path, temp_project
    ):
        ...
        with patch.object(executor, "_run_command") as mock_run:
            ...
            result = await executor.execute_with_autofix(
                temp_project, max_iterations=3, qa_threshold=3.0
            )

            assert result.success
            # No autofix should have been attempted
            autofix_results = [
                r for r in result.step_results if r.step == PipelineStep.AUTOFIX
            ]
            assert len(autofix_results) == 0

    @pytest.mark.asyncio
    async def test_execute_with_autofix_qa_fails_then_passes(
        self, mock_cli_path, temp_project
    ):
        ...
        with patch.object(executor, "_run_command") as mock_run:
            ...
            async def mock_command(cmd):
                ...
                if step_name == "qa":
                    ...
                    return MagicMock(returncode=0, stdout="Applied 3 fixes", stderr="")
                elif step_name == "export":
                    export_dir = temp_project / "exports" / "export_001"
                    export_dir.mkdir(parents=True, exist_ok=True)
                    video_file = export_dir / "video.mp4"
    ...
    @pytest.mark.asyncio
    async def test_execute_with_autofix_max_iterations_reached(
        self, mock_cli_path, temp_project
    ):
        ...
        with patch.object(executor, "_run_command") as mock_run:
            ...
            async def mock_command(cmd):
                ...
                if step_name == "qa":
                    ...
                    return MagicMock(returncode=0, stdout="Applied fixes", stderr="")
                else:
                    return MagicMock(
                        returncode=0, stdout=f"{step_name} completed", stderr=""
                    )
            ...
            result = await executor.execute_with_autofix(
                temp_project, max_iterations=2, qa_threshold=3.0
            )
            ...
            assert any(
                "autofix" in err.lower() or "qa" in err.lower() for err in result.errors
            )
