...
class TestValidateProjectName:
    ...
    def test_windows_reserved_names(self):
        ...
        for name in reserved_names:
            ...
            assert is_valid is False, (
                f"Reserved name '{name.lower()}' should be invalid"
            )
...
class TestIntegrationCrossPlatform:
    ...
    def test_validation_prevents_invalid_projects(self, tmp_path):
        ...
        for invalid_name in invalid_names:
            ...
            assert result["success"] is False, (
                f"Name '{invalid_name}' should be invalid"
            )
            assert len(result["errors"]) > 0
            ...
            if invalid_name.strip() and not any(
                c in invalid_name for c in ["/", "\\", ":"]
            ):
                assert (
                    not project_path.exists() or len(list(project_path.iterdir())) == 0
                )
