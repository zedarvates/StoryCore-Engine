...
def create_test_audio_with_voice(
    duration: float = 5.0, sample_rate: int = 44100, voice_segments: list = None
) -> dict:
    """
...
def test_voice_segment_detection_basic():
    ...
    audio = create_test_audio_with_voice(
        duration=5.0, voice_segments=[(1.0, 2.0), (3.0, 4.0)]
    )
    ...
    for i, seg in enumerate(segments):
        print(
            f"  Segment {i + 1}: {seg.start_time:.2f}s - {seg.end_time:.2f}s (confidence: {seg.confidence:.2f})"
        )
...
def test_voice_segment_detection_stereo():
    ...
    mono_audio = create_test_audio_with_voice(duration=3.0, voice_segments=[(0.5, 2.0)])

    # Convert to stereo
    stereo_samples = np.stack([mono_audio["samples"], mono_audio["samples"]], axis=1)

    audio = {"samples": stereo_samples, "sample_rate": 44100, "duration": 3.0}
...
def test_voice_segment_properties():
    ...
    audio = create_test_audio_with_voice(duration=3.0, voice_segments=[(1.0, 2.0)])
...
def test_empty_samples_handling():
    ...
    audio = {"samples": np.array([]), "sample_rate": 44100, "duration": 0.0}
...
def test_missing_samples_handling():
    ...
    audio = {"sample_rate": 44100, "duration": 2.0}
...
def test_voice_music_mixing_basic():
    ...
    voice_track = create_test_audio_with_voice(
        duration=5.0, voice_segments=[(1.0, 3.0)]
    )
    ...
    print(
        f"✓ Voice/music mixing works - {len(result['voice_segments'])} voice segments, {len(result['keyframes'])} keyframes"
    )
...
def test_keyframe_creation():
    ...
    voice_track = create_test_audio_with_voice(
        duration=5.0, voice_segments=[(2.0, 3.0)]
    )
    ...
    assert len(keyframes) >= 4, (
        f"Should have at least 4 keyframes, got {len(keyframes)}"
    )
    ...
    for kf in keyframes[:5]:  # Show first 5
        print(
            f"  {kf['timestamp']:.2f}s: {kf['volume_db']:.1f} dB ({kf['curve_type']})"
        )
...
def test_music_reduction_during_voice():
    ...
    # Create voice track
    voice_track = create_test_audio_with_voice(
        duration=4.0, voice_segments=[(1.5, 2.5)]
    )
...
def test_missing_audio_samples_handling():
    ...
    voice_track = {"sample_rate": 44100, "duration": 2.0}
...
def test_custom_music_reduction():
    ...
    voice_track = create_test_audio_with_voice(
        duration=3.0, voice_segments=[(1.0, 2.0)]
    )
    ...
    assert any(v == -18.0 for v in volumes), (
        "Should have keyframes with custom reduction"
    )
...
def test_interpolation_continuity_check():
    ...
    print(
        f"✓ Interpolation continuity check works - continuous: {result['is_continuous']}, max discontinuity: {result['max_discontinuity']:.6f}"
    )
...
def test_edge_case_single_keyframe():
    ...
    keyframes = [AudioKeyframeModel(timestamp=0.0, volume_db=0.0, curve_type="linear")]
...
def test_edge_case_zero_duration():
    ...
    keyframes = [
        ...
        AudioKeyframeModel(
            timestamp=1.0, volume_db=-6.0, curve_type="linear"
        ),  # Same timestamp
    ]
...
def test_crossfade_basic():
    ...
    print(
        f"✓ Basic crossfade works - duration: {result['duration']:.2f}s, fade: {result['fade_start']:.2f}s to {result['fade_end']:.2f}s"
    )
...
def test_equal_power_crossfade():
    ...
    assert np.allclose(power, 1.0, atol=0.01), (
        f"Power should be constant, got range {power.min():.3f} to {power.max():.3f}"
    )
...
def test_crossfade_gain_compensation():
    ...
    assert max_amplitude < 0.8, (
        f"Amplitude should not increase significantly, got {max_amplitude}"
    )
...
def test_crossfade_sample_rate_mismatch():
    ...
    clip_a = {"samples": np.random.randn(44100), "sample_rate": 44100, "duration": 1.0}
...
def test_crossfade_missing_samples():
    ...
    clip_a = {"sample_rate": 44100, "duration": 1.0}

    clip_b = {"samples": np.random.randn(44100), "sample_rate": 44100, "duration": 1.0}
...
def test_crossfade_sequence():
    ...
    print(
        f"✓ Crossfade sequence works - {result['num_crossfades']} crossfades, duration: {result['duration']:.2f}s"
    )
...
def test_crossfade_stereo_handling():
    ...
    clip_a = {"samples": stereo_a, "sample_rate": sample_rate, "duration": 1.0}

    clip_b = {"samples": stereo_b, "sample_rate": sample_rate, "duration": 1.0}
...
def test_crossfade_custom_position():
    ...
    print(
        f"✓ Custom overlap position works - fade starts at {result['fade_start']:.2f}s"
    )
...
def test_gap_detection_basic():
    ...
    result = engine.detect_and_fill_gaps(
        timeline, threshold_ms=100, fill_method="silence"
    )
    ...
    for i, gap in enumerate(gaps[:3]):  # Show first 3
        print(
            f"  Gap {i + 1}: {gap['start_time']:.2f}s - {gap['end_time']:.2f}s ({gap['duration']:.2f}s)"
        )
...
def test_gap_detection_threshold():
    ...
    timeline = {"samples": samples, "sample_rate": sample_rate, "duration": 3.0}
    ...
    for gap in gaps:
        assert gap["duration"] >= 0.1, (
            f"Gap duration {gap['duration']} should be >= 0.1s"
        )
...
def test_gap_filling_ambient():
    ...
    result = engine.detect_and_fill_gaps(
        timeline, threshold_ms=100, fill_method="ambient", ambient_level_db=-40.0
    )
...
def test_gap_filling_crossfade():
    ...
    result = engine.detect_and_fill_gaps(
        timeline, threshold_ms=100, fill_method="crossfade"
    )
...
def test_gap_statistics():
    ...
    assert result["total_gap_duration"] > 0.8, (
        f"Total gap duration: {result['total_gap_duration']}"
    )
    assert result["gap_percentage"] > 5.0, f"Gap percentage: {result['gap_percentage']}"

    print(
        f"✓ Gap statistics work - {result['total_gap_duration']:.2f}s total ({result['gap_percentage']:.1f}%)"
    )
...
def test_gap_detection_empty_audio():
    ...
    timeline = {"samples": np.array([]), "sample_rate": 44100, "duration": 0.0}
...
def test_gap_detection_missing_samples():
    ...
    timeline = {"sample_rate": 44100, "duration": 2.0}
...
def test_gap_detection_stereo():
    ...
    result = engine.detect_and_fill_gaps(
        timeline, threshold_ms=100, fill_method="ambient"
    )
