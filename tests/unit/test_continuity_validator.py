...
def test_spatial_consistency():
    ...
    shot_a = {
        ...
        "timestamp": 1.0,
        "camera_angle": 45.0,
        "characters": [{"name": "Alice", "position": {"x": 0.3, "y": 0.5}}],
    }
    ...
    spatial_violations = [
        v for v in result.violations if v.violation_type == "spatial_inconsistency"
    ]
...
def test_temporal_object_persistence():
    ...
    object_violations = [
        v for v in result.violations if "book" in v.description.lower()
    ]
...
def test_temporal_lighting_consistency():
    ...
    lighting_violations = [
        v for v in result.violations if "lighting" in v.description.lower()
    ]
