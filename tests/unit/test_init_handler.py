...
class TestInitHandler:
    ...
    def test_execute_legacy_mode_success(self):
        ...
        args = argparse.Namespace(
            project_name="test-project", path=".", interactive=False
        )
    ...
    def test_execute_legacy_mode_missing_project_name(self):
        ...
        args = argparse.Namespace(project_name=None, path=".", interactive=False)

        # Mock wizard to return None (cancelled)
        with patch(
            "wizard.wizard_orchestrator.run_interactive_wizard", return_value=None
        ):
            exit_code = handler.execute(args)
    ...
    def test_execute_wizard_mode_success(self):
        ...
        handler = InitHandler()
        args = argparse.Namespace(project_name=None, path=".", interactive=True)
        ...
        with (
            patch(
                "wizard.wizard_orchestrator.run_interactive_wizard",
                return_value=mock_wizard_state,
            ),
            patch(
                "wizard.config_builder.build_project_configuration",
