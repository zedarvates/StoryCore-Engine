...
def sample_parsed_prompt():
    ...
    return ParsedPrompt(
        ...
        time_period="2048",
        characters=[
            CharacterInfo(name="Alex", role="protagonist", description="A hacker")
        ],
...
def sample_world_config():
    ...
    return WorldConfig(
        ...
        color_palette=ColorPalette(
            primary="blue", secondary="pink", accent="purple", background="black"
        ),
        lighting_style="neon",
...
def test_generate_story_structure_basic(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
    """Test basic story structure generation"""
    story = story_generator.generate_story_structure(
        sample_parsed_prompt, sample_world_config, sample_characters
    )
...
def test_story_has_three_acts_for_trailer(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_act_durations_sum_to_total(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
    ...
    assert abs(total_duration - sample_parsed_prompt.duration_seconds) <= len(
        story.acts
    )


def test_acts_have_scenes(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_logline_includes_protagonist(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_logline_includes_setting(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_themes_are_genre_appropriate(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_emotional_arc_has_progression(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_emotional_arc_reaches_climax(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_teaser_has_two_acts(story_generator, sample_world_config, sample_characters):
    ...
    teaser_prompt = ParsedPrompt(
        ...
        time_period="present",
        characters=[
            CharacterInfo(name="John", role="protagonist", description="Detective")
        ],
    ...
    story = story_generator.generate_story_structure(
        teaser_prompt, sample_world_config, sample_characters
    )
...
def test_short_film_structure(story_generator, sample_world_config, sample_characters):
    ...
    short_film_prompt = ParsedPrompt(
        ...
        time_period="present",
        characters=[
            CharacterInfo(name="Sarah", role="protagonist", description="Teacher")
        ],
    ...
    story = story_generator.generate_story_structure(
        short_film_prompt, sample_world_config, sample_characters
    )
...
def test_act_names_match_video_type(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_act_descriptions_are_meaningful(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
    ...
    assert (
        "Alex" in all_descriptions
        or "Neo Tokyo" in all_descriptions
        or "protagonist" in all_descriptions
    )
...
def test_empty_characters_list(story_generator, sample_world_config):
    ...
    story = story_generator.generate_story_structure(prompt, sample_world_config, [])

    assert story.logline
    assert len(story.acts) > 0


def test_themes_include_key_elements(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_emotional_beats_have_unique_ids(
    story_generator, sample_parsed_prompt, sample_world_config, sample_characters
):
...
def test_commercial_structure(story_generator, sample_world_config, sample_characters):
    ...
    story = story_generator.generate_story_structure(
        commercial_prompt, sample_world_config, sample_characters
    )
