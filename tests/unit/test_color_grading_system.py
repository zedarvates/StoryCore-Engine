...
from backend.color.color_service import ColorService, ColorGrade, ColorSpace
from backend.color.lut_service import LUTService, LUT, LUTMetadata, LUTFormat
from backend.color.scopes_service import ScopesService, ScopesConfig
...
def test_color_service_primary(color_service, sample_frame):
    ...
    assert np.mean(np.abs(result[:, :, 0] - result[:, :, 1])) > np.mean(np.abs(sample_frame[:, :, 0] - sample_frame[:, :, 1]))
