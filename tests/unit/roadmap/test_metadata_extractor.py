...
"""
        ...
        assert (
            description
            == "This is a description that spans multiple lines but is one paragraph."
        )
...
class TestInferCategory:
    ...
    def test_infrastructure_category_from_name(self, extractor):
        ...
        assert (
            extractor.infer_category("docker-deployment")
            == FeatureCategory.INFRASTRUCTURE
        )
        assert (
            extractor.infer_category("aws-integration")
            == FeatureCategory.INFRASTRUCTURE
        )
        assert (
            extractor.infer_category("ci-cd-pipeline") == FeatureCategory.INFRASTRUCTURE
        )

    def test_documentation_category_from_name(self, extractor):
        """Test Documentation category inference from spec name."""
        assert (
            extractor.infer_category("documentation-update")
            == FeatureCategory.DOCUMENTATION
        )
        assert (
            extractor.infer_category("readme-revision") == FeatureCategory.DOCUMENTATION
        )
    ...
    def test_testing_category_from_name(self, extractor):
        ...
        assert (
            extractor.infer_category("test-suite-debugging") == FeatureCategory.TESTING
        )
    ...
    def test_migration_category_from_name(self, extractor):
        ...
        assert (
            extractor.infer_category("refactor-architecture")
            == FeatureCategory.MIGRATION
        )
        assert (
            extractor.infer_category("project-reorganization")
            == FeatureCategory.MIGRATION
        )
...
class TestInferPriority:
    ...
    def test_priority_with_dependencies(self, extractor):
        ...
        assert (
            extractor.infer_priority(content, ["dep1", "dep2", "dep3"]) == Priority.HIGH
        )
