...
class TestSynchronizationEngine:
    ...
    def test_get_latest_mtime_no_files(self, engine, temp_dir):
        ...
        spec_files = SpecFiles(
            directory=spec_dir, requirements=None, design=None, tasks=None, metadata={}
        )
...
"""
        ...
        intro_idx = next(
            i for i, line in enumerate(lines) if "manually written" in line
        )

        # Find markers around the introduction
        start_idx = next(
            i
            for i in range(intro_idx, -1, -1)
            if engine.PROTECTED_SECTION_START in lines[i]
        )
        end_idx = next(
            i
            for i in range(intro_idx, len(lines))
...
"""
        ...
        restored = engine._restore_protected_sections(new_content, protected_sections)
...
"""
    ...
    def test_update_roadmap_no_existing_file(self, engine, config):
        ...
        assert (
            "# StoryCore-Engine Development Roadmap" in content
            or "# Roadmap" in content
        )
