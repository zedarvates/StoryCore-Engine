...
def sample_parsed_prompt():
    ...
    return ParsedPrompt(
        ...
        characters=[
            CharacterInfo(name="Hero", role="protagonist", description="Knight")
        ],
        key_elements=["quest"],
...
def test_generate_music_description_basic(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_mood_includes_prompt_moods(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_instruments_appropriate_for_genre(
    music_generator, sample_parsed_prompt, sample_story_structure
):
    ...
    assert (
        "orchestra" in instruments_str
        or "strings" in instruments_str
        or "brass" in instruments_str
    )


def test_sound_effects_generated(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_timeline_cues_generated(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_timeline_cues_in_order(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_intensity_progression(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_tempo_appropriate_for_mood(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_cyberpunk_genre(music_generator, sample_story_structure):
    ...
    music = music_generator.generate_music_description(
        cyberpunk_prompt, sample_story_structure
    )
...
def test_horror_genre(music_generator, sample_story_structure):
    ...
    music = music_generator.generate_music_description(
        horror_prompt, sample_story_structure
    )
...
def test_sound_effect_timestamps_valid(
    music_generator, sample_parsed_prompt, sample_story_structure
):
...
def test_cue_descriptions_meaningful(
    music_generator, sample_parsed_prompt, sample_story_structure
):
    ...
    for cue in music.timeline:
        ...
        assert any(
            word in cue.description.lower()
            for word in ["music", "theme", "intensity", "mood", "resolution"]
        )
