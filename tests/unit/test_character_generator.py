...
def test_generate_characters_basic(
    character_generator, sample_parsed_prompt, sample_world_config
):
    """Test basic character generation"""
    characters = character_generator.generate_characters(
        sample_parsed_prompt, sample_world_config
    )
...
def test_character_has_personality_traits(
    character_generator, sample_parsed_prompt, sample_world_config
):
...
def test_character_has_visual_description(
    character_generator, sample_parsed_prompt, sample_world_config
):
    ...
    for char in characters:
        ...
        assert (
            "cyberpunk" in char.visual_description.lower()
            or "neon" in char.visual_description.lower()
        )


def test_character_relationships_mapped(
    character_generator, sample_parsed_prompt, sample_world_config
):
...
def test_protagonist_antagonist_relationship(
    character_generator, sample_parsed_prompt, sample_world_config
):
...
def test_genre_specific_personality_traits(character_generator):
    ...
    traits = character_generator._generate_personality_traits(
        "cyberpunk", "protagonist"
    )
    assert any(
        trait in ["tech-savvy", "rebellious", "street-smart"] for trait in traits
    )
...
def test_visual_description_includes_world_elements(
    character_generator, sample_parsed_prompt, sample_world_config
):
    ...
    visual_desc = character_generator._generate_visual_description(
        char_info, sample_parsed_prompt, sample_world_config
    )
...
def test_single_character_no_relationships(character_generator, sample_world_config):
    ...
    single_char_prompt = ParsedPrompt(
        ...
        characters=[
            CharacterInfo(
                name="John", role="protagonist", description="A lone detective"
            )
        ],
    ...
    characters = character_generator.generate_characters(
        single_char_prompt, sample_world_config
    )
...
def test_empty_characters_list(character_generator, sample_world_config):
    ...
    characters = character_generator.generate_characters(
        empty_prompt, sample_world_config
    )
