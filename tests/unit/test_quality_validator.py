...
from end_to_end.quality_validator import (
    QualityValidator,
    CoherenceScore,
    AudioQualityScore,
    SyncScore,
...
class TestQualityValidator:
    ...
    def test_validate_final_video_with_existing_file(
        self, validator, temp_video_file, project_components
    ):
        """Test validation with an existing video file"""
        report = asyncio.run(
            validator.validate_final_video(temp_video_file, project_components)
        )
    ...
    def test_validate_final_video_with_missing_file(
        self, validator, project_components
    ):
        """Test validation with a missing video file"""
        missing_path = Path("/nonexistent/video.mp4")
        report = asyncio.run(
            validator.validate_final_video(missing_path, project_components)
        )
        ...
        critical_issues = [
            i for i in report.detected_issues if i.severity == "critical"
        ]
    ...
    def test_passed_status_with_high_scores(
        self, validator, temp_video_file, project_components
    ):
        ...
        if (
            ...
            and report.visual_coherence_score >= 0.5
            and report.audio_quality_score >= 0.5
            and report.sync_score >= 0.5
        ):
            assert report.passed

    def test_passed_status_with_low_overall_score(
        self, validator, temp_video_file, project_components
    ):
    ...
    def test_issue_detection_for_low_visual_coherence(self, validator):
        ...
        sync_score = SyncScore(
            score=0.9, offset_ms=0.0, drift_ms=0.0, is_synchronized=True, details={}
        )
    ...
    def test_recommendations_generation(self, validator):
        ...
        sync_score = SyncScore(
            score=0.7, offset_ms=80.0, drift_ms=30.0, is_synchronized=True, details={}
        )

        issues = validator._detect_issues(visual_score, audio_score, sync_score)
        recommendations = validator._generate_recommendations(
            visual_score, audio_score, sync_score, issues
        )
        ...
        visual_recs = [
            r for r in recommendations if "visual" in r.lower() or "style" in r.lower()
        ]
        assert len(visual_recs) > 0

        # Should have recommendations for audio issues
        audio_recs = [
            r for r in recommendations if "audio" in r.lower() or "gap" in r.lower()
        ]
    ...
    def test_recommendations_for_good_quality(self, validator):
        ...
        sync_score = SyncScore(
            score=0.95, offset_ms=0.0, drift_ms=0.0, is_synchronized=True, details={}
        )
        ...
        positive_recs = [
            r for r in recommendations if "good" in r.lower() or "no" in r.lower()
        ]
    ...
    def test_custom_thresholds(self, validator, temp_video_file, project_components):
        ...
        if (
            report.visual_coherence_score < 0.9
            or report.audio_quality_score < 0.9
            or report.sync_score < 0.95
            or report.overall_score < 0.9
        ):
            assert not report.passed
