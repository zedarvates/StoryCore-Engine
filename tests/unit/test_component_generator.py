...
def sample_parsed_prompt():
    ...
    return ParsedPrompt(
        ...
        characters=[
            CharacterInfo(
                name="Captain", role="protagonist", description="Ship captain"
            ),
    ...
    components = ProjectComponents(
        world_config=WorldConfig(
            ...
            color_palette=ColorPalette(
                primary="blue", secondary="white", accent="silver", background="black"
            ),
        ...
        dialogue_script=DialogueScript(
            script_id="d1", scenes=[], total_lines=0, estimated_duration=0
        ),
...
@pytest.mark.asyncio
async def test_music_description_has_timeline(
    component_generator, sample_parsed_prompt
):
...
@pytest.mark.asyncio
async def test_components_duration_consistency(
    component_generator, sample_parsed_prompt
):
    ...
    assert abs(story_duration - sample_parsed_prompt.duration_seconds) <= len(
        components.story_structure.acts
    )

    # Sequence plan total duration should match
    assert (
        components.sequence_plan.total_duration == sample_parsed_prompt.duration_seconds
    )
