...
def create_test_args(handler_name, **overrides):
    ...
    if handler_name == "puppet_layer":
        ...
        base_args.update(
            {"generate": False, "update": False, "validate": False, "shots": None}
        )
    elif handler_name == "video_plan":
        base_args.update({"duration": None, "fps": 24, "style": "cinematic"})
    elif handler_name == "shot_planning":
        base_args.update(
            {"style": "cinematic", "analyze_grammar": False, "camera_specs": False}
        )
