...
class TestModelManager:
    ...
    @pytest.mark.asyncio
    async def test_corrupted_download_detection(
        self, model_manager, sample_model_info, temp_models_dir
    ):
        ...
        hashlib.sha256(corrupted_data).hexdigest()
...
class TestModelDownload:
    ...
    async def test_download_model_success(
        ...
        with patch("aiohttp.ClientSession", return_value=MockClientSession()):
            ...
            result = await model_manager.download_model(
                sample_model_info, progress_callback
            )
    ...
    @pytest.mark.asyncio
    async def test_download_model_progress_tracking(
        self, model_manager, sample_model_info
    ):
...
class TestSequentialDownload:
    ...
    async def test_download_all_missing_priority_order(self, model_manager):
        ...
        assert priorities == sorted(priorities), (
            "Models should be downloaded in priority order"
        )
...
class TestRetryLogic:
    ...
    @pytest.mark.asyncio
    async def test_download_succeeds_first_attempt(
        self, model_manager, sample_model_info
    ):
        ...
        with patch.object(
            model_manager, "_download_model_attempt", side_effect=mock_attempt
        ):
            result = await model_manager.download_model(sample_model_info)
    ...
    async def test_download_retries_on_failure(self, model_manager, sample_model_info):
        ...
        with patch.object(
            ...
            result = await model_manager.download_model(
                sample_model_info, max_retries=3
            )
    ...
    async def test_exponential_backoff_timing(self, model_manager, sample_model_info):
        ...
        assert total_time >= 3.0, (
            f"Expected at least 3 seconds of backoff, got {total_time}"
        )
    ...
    async def test_max_retries_parameter(self, model_manager, sample_model_info):
        ...
        with patch.object(
            ...
            result = await model_manager.download_model(
                sample_model_info, max_retries=5
            )
