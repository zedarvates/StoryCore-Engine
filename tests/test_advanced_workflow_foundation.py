...
class TestAdvancedWorkflowBase:
    ...
    def test_capability_scoring(self):
        ...
        score = workflow.get_capability_score(
            WorkflowCapability.ANIME_GENERATION, request
        )
        assert score.score == 0.0
...
class TestAdvancedWorkflowRegistry:
    ...
    def test_workflows_by_capability(self):
        ...
        video_workflows = registry.get_workflows_by_capability(
            WorkflowCapability.TEXT_TO_VIDEO
        )
        assert "video/test_video" in video_workflows

        image_workflows = registry.get_workflows_by_capability(
            WorkflowCapability.TEXT_TO_IMAGE
        )
