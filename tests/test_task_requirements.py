...
def test_requirement_1_4_async_operation_support():
    ...
    try:
        ...
        task_id = task_manager.create_task(operation=sample_operation, params={})
...
def test_requirement_16_5_timeout_handling():
    ...
    try:
        ...
        task_id = task_manager.create_task(
            operation=slow_operation_with_partial, params={}, timeout=1.0
        )
...
def test_requirement_18_2_immediate_return():
    ...
    try:
        ...
        start_time = time.time()
        task_id = task_manager.create_task(operation=very_long_operation, params={})
        creation_time = time.time() - start_time
...
def test_requirement_18_4_task_status_polling():
    ...
    try:
        ...
        # Create task
        task_id = task_manager.create_task(operation=progressive_operation, params={})
...
def test_requirement_18_5_task_cancellation():
    ...
    try:
        ...
        # Create task
        task_id = task_manager.create_task(operation=cancellable_operation, params={})
...
def test_all_task_statuses():
    ...
    try:
        ...
        task_id = task_manager.create_task(operation=quick_operation, params={})
        time.sleep(2)
        status = task_manager.get_task_status(task_id)
        assert status["status"] == TaskStatus.COMPLETED.value

        # Test FAILED
        task_id = task_manager.create_task(operation=failing_operation, params={})
        time.sleep(2)
        status = task_manager.get_task_status(task_id)
        assert status["status"] == TaskStatus.FAILED.value
