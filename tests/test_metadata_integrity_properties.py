...
class TestMetadataIntegrityProperties:
    ...
    def test_metadata_round_trip_property(
        self, shot_id, frame_count, duration, metadata_fields
    ):
        """
        ...
        **Validates: Requirements VE-10.3, VE-10.5**
        ...
        shot_data = self._create_shot_data(
            shot_id, frame_count, duration, metadata_fields
        )
        ...
        export_result = self.export_manager.export_shot_metadata(
            shot_data, export_config
        )
        assert export_result.success, (
            f"Export should succeed: {export_result.error_message}"
        )
        ...
        assert (
            abs(exported_metadata["duration"] - duration) < 0.001
        )  # Float precision tolerance

        # Verify custom metadata fields
        if "custom_metadata" in exported_metadata:
            for key, value in metadata_fields.items():
                assert key in exported_metadata["custom_metadata"], (
                    f"Custom field {key} should be preserved"
                )
                ...
                if isinstance(value, float) and isinstance(
                    exported_value, (int, float)
                ):
                    assert abs(float(exported_value) - value) < 0.001
                else:
                    assert exported_value == value, (
                        f"Value for {key} should be preserved: {value} vs {exported_value}"
                    )
    ...
    @given(
        ...
        export_format=st.sampled_from(
            [MetadataFormat.JSON, MetadataFormat.XML, MetadataFormat.YAML]
        ),
    )
    ...
    def test_multi_shot_metadata_consistency_property(self, shots_data, export_format):
        ...
        **Validates: Requirements VE-10.6, VE-10.7**
        ...
        for i, (shot_id, frame_count, duration) in enumerate(shots_data):
            shot_data = self._create_shot_data(
                f"{shot_id}_{i}", frame_count, duration, {}
            )
        ...
        export_result = self.export_manager.export_multi_shot_metadata(
            shot_list, export_config
        )
        assert export_result.success, (
            f"Multi-shot export should succeed: {export_result.error_message}"
        )
    ...
    def test_large_metadata_integrity_property(
        self, metadata_size, field_name_length, field_value_length
    ):
        """
        ...
        **Validates: Requirements VE-10.3, VE-10.7**
        ...
        assert export_result.success, (
            f"Large metadata export should succeed: {export_result.error_message}"
        )
        ...
        if "custom_metadata" in exported_metadata:
            ...
            for key, value in large_metadata.items():
                assert key in exported_custom, (
                    f"Large metadata field {key} should be preserved"
                )
                assert exported_custom[key] == value, (
                    f"Large metadata value for {key} should match"
                )
    ...
    def test_timestamp_metadata_integrity_property(
        self, timestamp_offset, timezone_offset
    ):
        """
        ...
        **Validates: Requirements VE-10.5, VE-10.6**
        ...
        assert export_result.success, (
            f"Timestamp metadata export should succeed: {export_result.error_message}"
        )
        ...
        if "custom_metadata" in exported_metadata:
            ...
            if "creation_time" in exported_custom:
                ...
                assert abs(exported_time - test_timestamp) < 1.0, (
                    "Creation timestamp should be preserved"
                )

            if "timezone_offset" in exported_custom:
                assert exported_custom["timezone_offset"] == timezone_offset, (
                    "Timezone offset should be preserved"
                )

        # Verify export includes its own timestamp
        assert "export_timestamp" in exported_metadata, (
            "Export should include its own timestamp"
        )
        export_time = exported_metadata["export_timestamp"]
        assert isinstance(export_time, (int, float, str)), (
            "Export timestamp should be valid"
        )

    @given(
        unicode_text=st.text(min_size=1, max_size=100),
        special_chars=st.text(
            min_size=0, max_size=20, alphabet="!@#$%^&*()[]{}|;:,.<>?"
        ),
    )
    @settings(max_examples=30, deadline=5000)
    def test_unicode_metadata_integrity_property(self, unicode_text, special_chars):
        ...
        **Validates: Requirements VE-10.3, VE-10.5**
        ...
        assert export_result.success, (
            f"Unicode metadata export should succeed: {export_result.error_message}"
        )
        ...
        if "custom_metadata" in exported_metadata:
            ...
            for key, value in unicode_metadata.items():
                if key in exported_custom:
                    ...
                    assert exported_value == value, (
                        f"Unicode content for {key} should be preserved: {value} vs {exported_value}"
                    )

    def _create_shot_data(
        self, shot_id: str, frame_count: int, duration: float, metadata: Dict[str, Any]
    ) -> ShotData:
...
def test_metadata_integrity_integration():
    ...
    try:
        ...
        for format_type in [MetadataFormat.JSON, MetadataFormat.YAML]:
            ...
            export_result = export_manager.export_shot_metadata(
                shot_data, export_config
            )
            assert export_result.success, (
                f"Integration test export should succeed for {format_type.value}"
            )

            # Verify file exists and is readable
            metadata_file = Path(export_result.metadata_path)
            assert metadata_file.exists(), (
                f"Metadata file should exist for {format_type.value}"
            )
            assert metadata_file.stat().st_size > 0, (
                f"Metadata file should not be empty for {format_type.value}"
            )
