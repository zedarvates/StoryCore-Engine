    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for preset_name in test_presets:
            if preset_name in preset_names:
                ...
                assert isinstance(config, VideoConfiguration), (
                    f"Should load {preset_name} preset"
                )

                is_valid, issues = config.validate()
                assert is_valid, (
                    f"Preset {preset_name} should be valid, issues: {issues}"
                )
...
def test_custom_configuration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert custom_config.motion_blur_intensity == 0.8, (
            "Custom motion blur intensity should be applied"
        )
        assert custom_config.interpolation_method == "ai_based", (
            "Custom interpolation method should be applied"
        )
        assert custom_config.cinematic_preset == "custom", (
            "Should be marked as custom preset"
        )
...
def test_hardware_optimization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert not optimized.quality.gpu_acceleration, (
            "GPU acceleration should be disabled"
        )
        assert not optimized.quality.parallel_processing, (
            "Parallel processing should be disabled"
        )
        assert optimized.quality.memory_limit_gb <= 2.0, (
            "Memory limit should be within available memory"
        )
...
def test_save_and_load():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert loaded_config.interpolation_method == "depth_aware", (
            "Interpolation method should be preserved"
        )
        assert loaded_config.motion_blur_intensity == 0.7, (
            "Motion blur intensity should be preserved"
        )
