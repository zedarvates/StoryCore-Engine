...
async def test_transparent_video_generation():
    ...
    config = WanVideoConfig(width=720, height=480, num_frames=10, enable_alpha=True)
...
async def test_video_inpainting():
    ...
    config = WanVideoConfig(
        width=720, height=480, num_frames=10, enable_inpainting=True
    )
    integration = WanVideoIntegration(config, timeout_seconds=30.0)
    ...
    try:
        ...
        # Create mask
        mask = InpaintingMask(mask_image=Image.new("L", (720, 480), 255), blur_radius=4)
...
async def test_dual_guidance():
    ...
    config = WanVideoConfig(width=720, height=480, num_frames=10)
    integration = WanVideoIntegration(config, timeout_seconds=30.0)
    ...
    try:
        ...
        result_frames = await integration.generate_with_dual_guidance(
            prompt="Generate video with guidance", guidance=guidance, timeout=15.0
        )
...
async def test_video_compositing():
    ...
    try:
        # Create layers
        layer1_frames = [Image.new("RGB", (720, 480), (255, 0, 0)) for _ in range(5)]

        layer2_frames = [Image.new("RGB", (720, 480), (0, 255, 0)) for _ in range(5)]

        layers = [
            CompositeLayer(video_frames=layer1_frames, opacity=1.0),
            CompositeLayer(
                video_frames=layer2_frames, opacity=0.5, offset_x=100, offset_y=100
            ),
        ]

        # Composite
        result_frames = await integration.composite_videos(layers=layers, timeout=15.0)
...
async def test_convenience_functions():
    ...
    try:
        ...
        rgba_frames = await generate_transparent_video(
            prompt="A ghost character", width=720, height=480, num_frames=5
        )
