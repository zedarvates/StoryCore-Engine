...
def test_basic_functionality():
    ...
    config = QualityConfig(
        thresholds=QualityThresholds(
            temporal_consistency=0.80, visual_quality=0.85, motion_smoothness=0.75
        ),
    ...
    print(
        f"✅ Custom config created: {custom_monitor.config.thresholds.temporal_consistency == 0.80}"
    )
    ...
    print(
        f"✅ Quality metrics calculated: {metrics_count} metrics, expected present: {has_expected}"
    )
    ...
    has_dashboard_keys = all(
        key in dashboard_data
        for key in [
            "overall_score",
            "grade",
            "metrics",
    ...
    alpha_monitor = AdvancedVideoQualityMonitor(
        QualityConfig(enable_alpha_analysis=True)
    )
    ...
    print(
        f"✅ Performance metrics: {analysis_time:.2f}s for {frame_count} frames (good: {performance_good})"
    )
...
def test_quality_thresholds():
    ...
    test_configs = [
        (
            "Strict",
            QualityThresholds(
                temporal_consistency=0.90, visual_quality=0.90, motion_smoothness=0.85
            ),
        ),
        (
            "Moderate",
            QualityThresholds(
                temporal_consistency=0.75, visual_quality=0.80, motion_smoothness=0.70
            ),
        ),
        (
            "Lenient",
            QualityThresholds(
                temporal_consistency=0.60, visual_quality=0.65, motion_smoothness=0.55
            ),
        ),
    ]
    ...
    for name, thresholds in test_configs:
        ...
        print(
            f"✅ {name} thresholds: {report.overall_score:.3f} score, {issues_count} issues"
        )
        results[name.lower()] = {"score": report.overall_score, "issues": issues_count}
    ...
    print(
        f"✅ Threshold logic validation: {threshold_logic} (strict: {strict_issues}, lenient: {lenient_issues})"
    )
...
def test_workflow_specific_analysis():
    ...
    for workflow_id, workflow_name in workflows:
        ...
        print(
            f"✅ {workflow_name}: {report.overall_score:.3f} score, {len(report.metric_scores)} metrics"
        )
...
def test_real_time_monitoring():
    ...
    print(
        f"✅ Real-time analysis: {analysis_time:.2f}s (under limit: {real_time_fast})"
    )
    ...
    print(
        f"✅ Frame sampling: {report.frame_count} frames analyzed (sampling effective: {sampling_effective})"
    )
...
def main():
    ...
    try:
        ...
        print(
            f"Real-time Monitoring: {'✅ PASS' if realtime_results['real_time_fast'] else '❌ FAIL'}"
        )

        # Overall assessment
        overall_success = (
            basic_success >= basic_total * 0.8  # 80% basic tests pass
            and len(threshold_results) >= 3  # All threshold configs tested
            and len(workflow_results) >= 5  # All workflows tested
            and realtime_results["real_time_fast"]  # Real-time performance good
        )
