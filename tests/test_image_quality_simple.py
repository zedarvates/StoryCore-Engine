...
async def test_image_quality_monitor_basic():
    ...
    try:
        ...
        print(
            f"   ✓ Config created with excellent threshold: {config.excellent_threshold}"
        )
        ...
        print(
            f"   ✓ Quality metrics ({len(quality_metrics)}): {quality_metrics[:5]}..."
        )
        print(f"   ✓ Quality grades: {quality_grades}")
        print(
            f"   ✓ Enhancement types ({len(enhancement_types)}): {enhancement_types[:4]}..."
        )
        ...
        report = await monitor.analyze_image_quality(
            test_image, image_id="test_basic_001"
        )
        ...
        print(
            f"   ✓ Sharpness: {sharpness_result.score:.3f} ({sharpness_result.grade.value})"
        )
        print(
            f"   ✓ Color Accuracy: {color_result.score:.3f} ({color_result.grade.value})"
        )
        print(
            f"   ✓ Contrast: {contrast_result.score:.3f} ({contrast_result.grade.value})"
        )
        print(
            f"   ✓ Brightness: {brightness_result.score:.3f} ({brightness_result.grade.value})"
        )
        ...
        anime_report = await monitor.analyze_image_quality(
            test_image, image_id="test_anime_001", style_context="anime"
        )

        realistic_report = await monitor.analyze_image_quality(
            test_image, image_id="test_realistic_001", style_context="realistic"
        )
        ...
        anime_style_metric = anime_report.metrics.get(
        ...
        reference_report = await monitor.analyze_image_quality(
            test_image, image_id="test_reference_001", reference_image=reference_image
        )

        print(
            f"   ✓ Reference analysis completed: {reference_report.overall_score:.3f}"
        )

        detail_metric = reference_report.metrics.get(
            ImageQualityMetric.DETAIL_PRESERVATION
        )
        if detail_metric:
            print(f"   ✓ Detail preservation: {detail_metric.score:.3f}")
            print(
                f"   ✓ Has reference: {detail_metric.details.get('has_reference', False)}"
            )
        ...
        if report.enhancement_suggestions:
            ...
            for i, suggestion in enumerate(report.enhancement_suggestions[:3]):
                print(
                    f"     {i + 1}. {suggestion['type']} (Priority: {suggestion['priority']})"
                )
                print(f"        Confidence: {suggestion['confidence']:.3f}")
                print(
                    f"        Expected improvement: {suggestion.get('expected_improvement', 0):.3f}"
                )
        else:
        ...
        print(
            f"   ✓ Cached result matches: {cached_report.image_id == report.image_id}"
        )
        ...
        for i, image in enumerate(batch_images):
            report = await monitor.analyze_image_quality(
                image, image_id=f"batch_test_{i:03d}"
            )
            batch_reports.append(report)

        successful_analyses = [r for r in batch_reports if r.overall_score > 0]
        print(
            f"   ✓ Batch analysis: {len(successful_analyses)}/{len(batch_images)} successful"
        )

        avg_score = sum(r.overall_score for r in successful_analyses) / len(
            successful_analyses
        )
        ...
        if export_path.exists():
            ...
            print(
                f"   ✓ Export info: {export_data['export_info']['total_reports']} total reports"
            )
            export_path.unlink()  # Clean up

        print("\n" + "=" * 70)
        print(
            "🎉 ALL TESTS PASSED! Advanced Image Quality Monitor is working correctly!"
        )
        print("=" * 70)
...
async def test_quality_workflow_scenarios():
    ...
    try:
        ...
        anime_report = await monitor.analyze_image_quality(
            anime_image, image_id="anime_quality_001", style_context="anime"
        )
        print(
            f"   ✓ Anime analysis: {anime_report.overall_grade.value} ({anime_report.overall_score:.3f})"
        )
        ...
        if anime_report.enhancement_suggestions:
            enhanced_anime, improvement = await monitor.enhance_image_quality(
                anime_image, anime_report.enhancement_suggestions
            )
            print(f"   ✓ Enhancement applied: {improvement:.3f} expected improvement")

        scenario1_success = anime_report.overall_score > 0
        print(
            f"   🎯 Anime Quality Assessment: {'SUCCESS' if scenario1_success else 'PARTIAL'}"
        )
        ...
        print(
            f"   ✓ Original photo: {original_report.overall_grade.value} ({original_report.overall_score:.3f})"
        )
        ...
        print(
            f"   ✓ Edited photo: {edited_report.overall_grade.value} ({edited_report.overall_score:.3f})"
        )

        # Step 3: Compare detail preservation
        detail_metric = edited_report.metrics.get(
            ImageQualityMetric.DETAIL_PRESERVATION
        )
        if detail_metric:
            print(f"   ✓ Detail preservation: {detail_metric.score:.3f}")
        ...
        print(
            f"   🎯 Professional Photo Quality: {'SUCCESS' if scenario2_success else 'PARTIAL'}"
        )
        ...
        for iteration in range(3):
            ...
            iter_report = await monitor.analyze_image_quality(
                current_image, image_id=f"improvement_iter_{iteration}"
            )
            iteration_scores.append(iter_report.overall_score)
            print(
                f"   ✓ Iteration {iteration + 1}: {iter_report.overall_grade.value} ({iter_report.overall_score:.3f})"
            )

            # Apply enhancements if available
            if iter_report.enhancement_suggestions:
                enhanced_image, improvement = await monitor.enhance_image_quality(
                    current_image, iter_report.enhancement_suggestions[:2]
                )
                current_image = enhanced_image
                print(
                    f"     - Applied enhancements: {improvement:.3f} expected improvement"
                )
            else:
                break
        ...
        print(
            f"   🎯 Quality Improvement Iteration: {'SUCCESS' if scenario3_success else 'PARTIAL'}"
        )
        ...
        for style in style_contexts:
            style_report = await monitor.analyze_image_quality(
                test_image, image_id=f"style_test_{style}", style_context=style
            )
        ...
        print(
            f"   🎯 Multi-Style Comparison: {'SUCCESS' if scenario4_success else 'PARTIAL'}"
        )

        # Overall workflow assessment
        all_scenarios_success = all(
            [scenario1_success, scenario2_success, scenario3_success, scenario4_success]
        )
        print(
