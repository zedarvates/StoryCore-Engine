...
class TestPerformanceOptimizer:
    ...
    def test_optimizer_initialization(self):
        ...
        config = OptimizationConfig(
            strategy=OptimizationStrategy.SPEED, target_fps=60.0, max_batch_size=32
        )
    ...
    def test_quality_level_adjustment(self):
        ...
        quality = optimizer.adjust_quality_level(
            current_performance=10.0, target_performance=30.0
        )
        assert 0.0 <= quality <= 1.0

        # Good performance should increase quality
        quality = optimizer.adjust_quality_level(
            current_performance=40.0, target_performance=30.0
        )
        assert 0.0 <= quality <= 1.0
    ...
    def test_update_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            updated_config = manager.update_config(
                {"max_concurrent_jobs": 16, "job_timeout_seconds": 600.0}
            )
...
class TestProductionMonitor:
    ...
    def test_alert_generation(self):
        ...
        config = MonitoringConfig(
            enable_alerts=True, alert_thresholds={"error_rate": 0.05}
        )
        monitor = ProductionMonitor(config)
