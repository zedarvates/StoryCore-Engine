...
class TestMemoryMonitor:
    ...
    def test_can_load_model_cuda(
        self, mock_memory, mock_props, mock_count, mock_available
    ):
...
def run_simple_test():
    ...
    stats = monitor.get_memory_stats()
    print(
        f"  Memory Stats: {stats.total_ram_gb:.1f}GB RAM, {stats.total_vram_gb:.1f}GB VRAM"
    )
