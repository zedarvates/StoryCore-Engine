...
from color.color_service import ColorService, ColorGrade, ColorSpace
from color.scopes_service import ScopesService, ScopesConfig
from color.lut_service import LUTService, LUTMetadata, LUTFormat, LUTType
...
class TestColorService:
    ...
    def test_create_color_service(self, color_service):
        ...
        assert color_service.gpu == False
    ...
    def test_apply_saturation(self, color_service):
        ...
        g_diff = np.abs(result[:, :, 1] - result[:, :, 2])
    ...
    def test_delete_grade(self, color_service):
        ...
        grade = color_service.create_grade("To Delete")
        grade_id = color_service.list_grades()[0]
        
        assert color_service.delete_grade(grade_id) == True
        assert len(color_service.list_grades()) == 0
        assert color_service.delete_grade("nonexistent") == False
...
class TestColorIntegration:
    ...
    def test_full_color_grading_workflow(self):
        ...
        grade = ColorGrade(
            contrast=1.2 if exposure['dynamic_range'] < 0.8 else 1.0,
            saturation=1.1
        )
