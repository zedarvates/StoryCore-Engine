...
class TestCLIServiceManagement:
    ...
    def test_handle_comfyui_restart_success(
        self, mock_sleep, mock_stdout, mock_engine_class
    ):
    ...
    @patch("sys.stdout", new_callable=StringIO)
    @patch("time.sleep")
    def test_handle_comfyui_restart_start_failure(
        self, mock_sleep, mock_stdout, mock_engine_class
    ):
...
class TestCLIServiceManagementErrorHandling:
    ...
    def test_engine_initialization_failure(self, mock_stdout, mock_engine_class):
        ...
        output = mock_stdout.getvalue()
        assert (
            "[ERROR] ComfyUI service management error: ComfyUI engine not available"
            in output
        )
