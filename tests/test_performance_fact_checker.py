...
from src.fact_checker.caching import FactCheckerCache, reset_cache
...
class TestTextProcessingPerformance:
    ...
    def test_text_processing_5000_words_under_30_seconds(self):
        ...
        print(
            f"[Performance Test] Claims extracted: {report.summary_statistics['total_claims']}"
        )
    ...
    def test_text_processing_1000_words_performance(self):
        ...
        print(
            f"[Performance Test] Baseline processing completed in {elapsed_time:.2f}s"
        )
...
class TestTranscriptProcessingPerformance:
    ...
    def test_transcript_processing_10000_words_under_60_seconds(self):
        ...
        print(
            f"[Performance Test] Transcript processing completed in {elapsed_time:.2f}s"
        )
        print(
            f"[Performance Test] Manipulation signals: {len(report.manipulation_signals)}"
        )
        print(
            f"[Performance Test] Coherence score: {report.summary_statistics.get('coherence_score', 0):.1f}"
        )
    ...
    def test_transcript_processing_2000_words_performance(self):
        ...
        agent.analyze(transcript)

        elapsed_time = time.time() - start_time

        print(
            f"[Performance Test] Baseline transcript processing completed in {elapsed_time:.2f}s"
        )
...
class TestBatchProcessingPerformance:
    ...
    def test_batch_processing_parallel_speedup(self):
        ...
        print(
            f"[Performance Test] Parallel processing (5 workers): {parallel_time:.2f}s"
        )
    ...
    def test_batch_processing_large_batch(self):
        ...
        num_items = 50
        items = [
            {"id": f"item_{i}", "content": f"Content {i}"} for i in range(num_items)
        ]
        ...
        print(
            f"[Performance Test] Average time per item: {elapsed_time / num_items:.4f}s"
        )
    ...
    def test_batch_processing_with_failures(self):
        ...
        """
        items = [{"id": f"item_{i}", "content": f"Content {i}"} for i in range(20)]
    ...
    def test_batch_processing_progress_tracking(self):
        ...
        """
        items = [{"id": f"item_{i}", "content": f"Content {i}"} for i in range(10)]
...
class TestEndToEndPerformance:
    ...
    def test_complete_workflow_with_caching(self):
        ...
        try:
            ...
            # Cache the result
            cache.set(
                text, {"metadata": report1.metadata, "summary": report1.human_summary}
            )
            ...
            if cache_time > 0:
                ...
                print(
                    "[Performance Test] Speedup: >1000x (cache retrieval too fast to measure)"
                )
