...
class TestImageGenerationResult:
    ...
    def test_failed_result(self):
        ...
        result = ImageGenerationResult(
            success=False, processing_time=0.5, error_message="Generation failed"
        )
...
class TestEnhancedImageEngine:
    ...
    def test_style_detection(self, engine):
        ...
        realistic_style = engine._detect_style(
            "professional photo portrait of a person"
        )
        assert realistic_style == ImageStyle.REALISTIC

        # Test artistic detection
        artistic_style = engine._detect_style(
            "abstract art painting with geometric patterns"
        )
    ...
    def test_generation_mode_determination(self, engine):
        ...
        request = ImageGenerationRequest(
            prompt="quick generation", strategy=WorkflowStrategy.SPEED_FIRST
        )
        mode = engine._determine_generation_mode(request)
    ...
    async def test_lightning_fast_generation(self, engine):
        ...
        anime_request = ImageGenerationRequest(
            prompt="anime character quick generation"
        )
        ...
        ref_request = ImageGenerationRequest(
            prompt="quick enhancement", reference_images=[MockImage()]
        )
    ...
    async def test_standard_generation(self, engine):
        ...
        request = ImageGenerationRequest(
            prompt="realistic portrait", width=1280, height=720
        )
        ...
        assert (
            fast_result.quality_score < result.quality_score
    ...
    async def test_auto_generation(self, engine):
        ...
        edit_request = ImageGenerationRequest(
            prompt="enhance this photo", reference_images=[MockImage(), MockImage()]
        )
    ...
    def test_performance_stats_update(self, engine):
        ...
        engine._update_performance_stats(ImageGenerationMode.ANIME, 1.5, 0.9, True)

        assert (
            engine.performance_stats["total_generations"]
            == initial_stats["total_generations"] + 1
        )
        assert (
            engine.performance_stats["successful_generations"]
            == initial_stats["successful_generations"] + 1
        )
        assert (
            engine.performance_stats["mode_usage"]["anime"]
            == initial_stats["mode_usage"]["anime"] + 1
        )
        assert 0.9 in engine.performance_stats["quality_scores"]

        # Update with failed operation
        engine._update_performance_stats(ImageGenerationMode.STANDARD, 0.5, 0.0, False)

        assert (
            engine.performance_stats["total_generations"]
            == initial_stats["total_generations"] + 2
        )
        assert (
            engine.performance_stats["successful_generations"]
            == initial_stats["successful_generations"] + 1
    ...
    def test_performance_report(self, engine):
        ...
        engine._update_performance_stats(
            ImageGenerationMode.PROFESSIONAL_EDIT, 1.5, 0.85, True
        )
        engine._update_performance_stats(ImageGenerationMode.STANDARD, 0.8, 0.75, True)
        engine._update_performance_stats(
            ImageGenerationMode.LIGHTNING_FAST, 0.3, 0.65, False
        )
