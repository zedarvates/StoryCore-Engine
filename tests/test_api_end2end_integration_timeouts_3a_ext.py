...
def test_api_end2end_3a_timeout_status(monkeypatch):
    ...
    monkeypatch.setattr('backend.task_queue_api.get_async_task_queue', lambda: FakeQueueTimeout())
    payload = {"job_id": "e2e-3a-timeout", "payload": {"input": "timeout"}, "priority": 5, "timeout_seconds": 300}
...
def test_api_end2end_3a_unauthorized(monkeypatch):
    monkeypatch.setattr('backend.task_queue_api.get_async_task_queue', lambda: FakeQueueUnauthorized())
    payload = {"job_id": "e2e-3a-auth", "payload": {"input": "auth"}, "priority": 5, "timeout_seconds": 300}
    resp = client.post("/api/tasks/api", json=payload)
    assert resp.status_code == 200
    resp2 = client.get(f"/api/tasks/api/{payload['job_id']}")
...
def test_api_end2end_3a_status_response_structure(monkeypatch):
    ...
    monkeypatch.setattr('backend.task_queue_api.get_async_task_queue', lambda: FakeQueueNormal())
    payload = {"job_id": "e2e-3a-status-struct", "payload": {"input": "ok"}, "priority": 5, "timeout_seconds": 300}
    resp = client.post("/api/tasks/api", json=payload)
    assert resp.status_code == 200
    resp2 = client.get(f"/api/tasks/api/{payload['job_id']}")
    assert resp2.status_code == 200
    data = resp2.json()
    for k in ("job_id", "status", "progress", "created_at", "started_at", "completed_at", "error"):
        assert k in data
