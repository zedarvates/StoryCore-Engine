...
async def test_text_to_video():
    ...
    try:
        config = HunyuanVideoConfig(
            width=720, height=480, num_frames=10, steps=20, enable_upscaling=False
        )
...
async def test_image_to_video():
    ...
    try:
        config = HunyuanVideoConfig(width=720, height=480, num_frames=10, steps=20)
        ...
        print("Generating video from image and prompt...")
        result = await integration.generate_video(request)
...
async def test_super_resolution():
    ...
    try:
        ...
        print("Generating video with upscaling (factor: 1.5x)...")
        result = await integration.generate_video(request)

        if result.success:
            print("✓ SUCCESS")
            print("  - Original resolution: 720x480")
            print(
                f"  - Upscaled resolution: {result.resolution[0]}x{result.resolution[1]}"
            )
            print("  - Expected: 1080x720")
...
async def test_convenience_functions():
    ...
    try:
        ...
        result1 = await generate_text_to_video(
            prompt="Test convenience T2V", num_frames=5, steps=10
        )

        if not result1.success:
            print("✗ T2V convenience function failed")
            return False
        ...
        result2 = await generate_image_to_video(
            prompt="Test convenience I2V", image=test_image, num_frames=5, steps=10
        )

        if not result2.success:
            print("✗ I2V convenience function failed")
            return False
