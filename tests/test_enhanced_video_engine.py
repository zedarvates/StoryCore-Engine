...
from src.advanced_workflow_base import WorkflowCapability
...
class TestEnhancedVideoEngine:
    ...
    def mock_workflow_manager(self):
        ...
        manager.get_available_workflows = Mock(
            return_value={
                "video/hunyuan_video": {
                    "capabilities": [
                ...
                "video/wan_video": {
                    "capabilities": [
                        WorkflowCapability.TEXT_TO_VIDEO,
                        WorkflowCapability.ALPHA_VIDEO,
                    ],
        ...
        manager.get_workflows_by_capability = Mock(
            return_value=["video/hunyuan_video", "video/wan_video"]
        )
    ...
    async def test_capability_based_selection(
        self, enhanced_engine, mock_workflow_manager
    ):
        """Test capability-based workflow selection"""
        await enhanced_engine.initialize()

        # Mock capability-based selection
        mock_workflow_manager.get_workflows_by_capability.return_value = [
            "video/wan_video"
        ]
    ...
    @pytest.mark.asyncio
    async def test_performance_based_selection(
        self, enhanced_engine, mock_workflow_manager
    ):
    ...
    async def test_workflow_config_creation_hunyuan(self, enhanced_engine):
        ...
        workflow_config = await enhanced_engine._create_workflow_config(
            config, "video/hunyuan_video"
        )
    ...
    @pytest.mark.asyncio
    async def test_advanced_workflow_execution_success(
        self, enhanced_engine, mock_workflow_manager
    ):
        ...
        mock_result.frames = [
            np.random.randint(0, 255, (480, 832, 3), dtype=np.uint8) for _ in range(33)
        ]
        ...
        config = AdvancedVideoConfig(
            prompt="Test prompt", width=832, height=480, num_frames=33
        )
    ...
    async def test_legacy_workflow_execution(self, enhanced_engine, mock_legacy_engine):
        ...
        config = AdvancedVideoConfig(
            prompt="Test prompt", width=832, height=480, num_frames=33, fps=16
        )
    ...
    async def test_quality_validation(self, enhanced_engine):
        ...
        config = AdvancedVideoConfig(prompt="Test prompt", enable_alpha_channel=True)
