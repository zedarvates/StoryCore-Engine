...
def test_core_architecture():
    ...
    thresholds = QualityThresholds(temporal_consistency=0.80, visual_quality=0.85)
...
def test_configuration_system():
    ...
    custom_thresholds = QualityThresholds(
        temporal_consistency=0.90, visual_quality=0.95, motion_smoothness=0.85
    )
    custom_config = QualityConfig(
        thresholds=custom_thresholds, enable_alpha_analysis=True, sample_frame_rate=0.5
    )

    AdvancedVideoQualityMonitor(custom_config)
...
def test_export_functionality():
    ...
    with patch("builtins.open", create=True) as mock_open:
        with patch("json.dump") as mock_json_dump:
            monitor.export_quality_report("export_test.mp4", "test_output.json")
    ...
    return {"export_attempted": export_attempted, "missing_handled": missing_handled}
...
def main():
    ...
    try:
        ...
        print(
            f"Architecture Tests: {sum(arch_results.values())}/{len(arch_results)} passed"
        )
        print(
            f"Calculation Tests: {sum(calc_results.values())}/{len(calc_results)} passed"
        )
        print(
            f"Configuration Tests: {sum(config_results.values())}/{len(config_results)} passed"
        )
        print(
            f"Error Handling Tests: {sum(error_results.values())}/{len(error_results)} passed"
        )
        print(
            f"Dashboard Tests: {sum(dashboard_results.values())}/{len(dashboard_results)} passed"
        )
        print(
            f"Export Tests: {sum(export_results.values())}/{len(export_results)} passed"
        )
