...
from src.comfyui_config import (
    ComfyUIConfig,
    ConfigManager,
    ControlNetConfig,
    IPAdapterConfig,
...
def valid_comfyui_config(draw):
    ...
    return ComfyUIConfig(
        ...
        memory_limit_gb=draw(
            st.one_of(st.none(), st.floats(min_value=1.0, max_value=64.0))
        ),
...
def invalid_comfyui_config(draw):
    ...
    if config_type == "invalid_port":
        base_config.server_port = draw(
            st.one_of(st.integers(max_value=0), st.integers(min_value=65536))
        )
...
class TestComfyUIConfigurationProperties:
    ...
    @given(
        st.sampled_from(["127.0.0.1", "localhost", "0.0.0.0"]),
    ...
    def test_property_controlnet_config_validation(self, strength, preprocessing):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if 0.0 <= strength <= 2.0:
                ...
                strength_errors = [
                    err for key, err in errors.items() if "strength" in key.lower()
                ]
                assert len(strength_errors) == 0, (
                    f"Valid strength {strength} should not produce errors"
                )
            else:
                # Invalid strength should produce errors
                assert len(errors) > 0, (
                    f"Invalid strength {strength} should produce validation errors"
                )

    @given(
        st.floats(min_value=0.0, max_value=2.0), st.floats(min_value=0.0, max_value=1.0)
    )
    @settings(max_examples=25)
    def test_property_ipadapter_config_validation(self, weight, noise):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if 0.0 <= weight <= 2.0:
                weight_errors = [
                    err for key, err in errors.items() if "weight" in key.lower()
                ]
                assert len(weight_errors) == 0, (
                    f"Valid weight {weight} should not produce errors"
                )
            else:
                assert len(errors) > 0, (
                    f"Invalid weight {weight} should produce validation errors"
                )

            # Check noise validation
            if 0.0 <= noise <= 1.0:
                noise_errors = [
                    err for key, err in errors.items() if "noise" in key.lower()
                ]
                assert len(noise_errors) == 0, (
                    f"Valid noise {noise} should not produce errors"
                )
            else:
                assert len(errors) > 0, (
                    f"Invalid noise {noise} should produce validation errors"
                )
    ...
    def test_property_config_serialization_roundtrip(self, extra_data):
        ...
        try:
            ...
            assert restored_config.server_port == config.server_port
            assert restored_config.server_host == config.server_host
            assert str(restored_config.installation_path) == str(
    ...
    def test_property_default_config_validity(self):
        ...
        [err for key, err in errors.items() if "installation_path" in key]
        other_errors = [
            err for key, err in errors.items() if "installation_path" not in key
        ]

        assert len(other_errors) == 0, (
            f"Default config should be valid except for path: {other_errors}"
        )
