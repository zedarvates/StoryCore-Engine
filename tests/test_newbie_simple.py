...
def test_basic_functionality():
    ...
    print(
        f"✅ Custom config created: {custom_integration.config.model_path == 'test_newbie_model'}"
    )
    ...
    print(
        f"✅ XML character parsed: {xml_character.name} with {len(xml_character.personality_traits)} traits"
    )

    # Test 5: Prompt template system
    templates_count = len(integration.prompt_templates)
    has_default_templates = all(
        template in integration.prompt_templates
        for template in ["classic_portrait", "modern_scene", "fantasy_adventure"]
    )
    print(
        f"✅ Prompt templates loaded: {templates_count} templates, defaults present: {has_default_templates}"
    )
    ...
    has_key_elements = all(
        element in description.lower()
        for element in ["teenager", "female", "pink", "green", "school uniform"]
    )
    print(
        f"✅ Character description: {len(description)} chars, elements present: {has_key_elements}"
    )

    # Test 8: Image generation (mock)
    result = integration.generate_image(prompt, character, ImageQuality.HIGH)
    generation_successful = result.quality_score > 0 and result.resolution == (
        1024,
        1536,
    )
    print(
        f"✅ Image generation: Quality {result.quality_score:.3f}, Resolution {result.resolution}, Success: {generation_successful}"
    )

    # Test 9: Quality validation
    quality_results = integration.validate_anime_quality(
        result.image_path, AnimeStyle.CLASSIC
    )
    quality_valid = (
        "overall_anime_quality" in quality_results
        and quality_results["overall_anime_quality"] > 0
    )
    print(
        f"✅ Anime quality validation: {quality_results.get('overall_anime_quality', 0):.3f}, Valid: {quality_valid}"
    )

    # Test 10: Character consistency checking
    image_paths = ["image1.png", "image2.png", "image3.png"]
    consistency_results = integration.check_character_consistency(
        image_paths, character
    )
    consistency_valid = (
        "overall_consistency" in consistency_results
        and consistency_results["overall_consistency"] > 0
    )
    print(
        f"✅ Character consistency: {consistency_results.get('overall_consistency', 0):.3f}, Valid: {consistency_valid}"
    )
...
def test_anime_styles_and_quality():
    ...
    print(
        f"✅ Anime styles available: {len(styles)} ({', '.join([s.value for s in styles[:5]])}...)"
    )
    ...
    for quality in qualities:
        ...
        print(
            f"   {quality.value}: {result.resolution} (expected: {expected_resolution}) ✅"
            if resolution_correct
            else f"   {quality.value}: {result.resolution} (expected: {expected_resolution}) ❌"
        )
...
def test_error_handling():
    ...
    consistency_result = integration.check_character_consistency(
        ["single_image.png"], character
    )
    ...
    char_with_invalid_gender = integration.create_character_from_dict(
        invalid_gender_data
    )
...
def main():
    ...
    try:
        ...
        # Overall assessment
        total_success = (
            basic_success + quality_success + library_success + error_success
        )
