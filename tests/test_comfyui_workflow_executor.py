...
try:
    ...
    pytestmark = pytest.mark.skip(reason="aiohttp/websockets not available")
...
class TestComfyUIConfig:
    ...
    def test_custom_config(self):
        ...
        config = ComfyUIConfig(
            host="192.168.1.100", port=8080, timeout=600, check_interval=0.5
        )
...
class TestComfyUIWorkflowExecutor:
    ...
    async def test_submit_workflow_success(self):
        ...
        with patch("aiohttp.ClientSession") as mock_session:
            ...
            mock_response.json = AsyncMock(
                return_value={"prompt_id": expected_prompt_id}
            )
            mock_response.__aenter__.return_value = mock_response
            mock_response.__aexit__.return_value = None
    ...
    async def test_get_history_success(self):
        ...
        expected_history = {"outputs": {}, "status": {"completed": True}}
    ...
    async def test_retrieve_outputs_with_videos(self):
        ...
        history = {
            "outputs": {
                "node_1": {
                    "videos": [
                        {"filename": "video1.mp4", "subfolder": "", "type": "output"}
                    ]
                }
            }
        }
