...
async def test_optimization_strategies():
    ...
    base_params = {"quality_level": 3, "steps": 20, "resolution": (1024, 1024)}
    ...
    assert len(set(execution_times)) > 1, (
        "Different strategies should produce different execution times"
    )
...
async def test_model_management():
    ...
    config = PerformanceConfig(
        max_models_in_memory=2, model_cache_size_mb=4096, enable_model_sharing=True
    )
    optimizer = create_advanced_performance_optimizer(config)

    # Load multiple models
    model1 = await optimizer.model_manager.load_model("test_model_1", "diffusion", 1024)
    await optimizer.model_manager.load_model("test_model_2", "diffusion", 1024)

    # Test model sharing (load same model again)
    model1_shared = await optimizer.model_manager.load_model(
        "test_model_1", "diffusion", 1024
    )

    assert model1 == model1_shared, "Model sharing should return same instance"
    assert optimizer.model_manager.models["test_model_1"].usage_count == 2, (
...
async def test_performance_profiling():
    ...
    assert workflow_id in optimizer.workflow_profiles, (
        "Workflow profile should be created"
    )
...
async def test_high_load_scenario():
    ...
    assert len(result["optimizations_applied"]) > 0, (
        "Should apply optimizations under high load"
    )
