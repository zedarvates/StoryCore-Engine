...
class TestProductionConfiguration:
    ...
    def test_environment_template_complete(self):
        ...
        required_vars = ["JWT_SECRET", "DATABASE_URL", "REDIS_URL", "GRAFANA_PASSWORD"]
            ...
            if "ports" in service:
                for port_mapping in service["ports"]:
                    assert ":" in str(port_mapping), (
                        f"Port invalide pour {service_name}"
                    )
...
class TestMonitoringConfiguration:
    ...
    def test_grafana_datasource_has_prometheus(self):
        ...
        with open(
            "monitoring/grafana/provisioning/datasources/datasource.yml", "r"
        ) as f:
            config = yaml.safe_load(f)
