...
def test_stack_grids():
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project stack-demo --grid 1x2 --cell-size 288"
    )
    if not success:
    ...
    with open(grid_file, "r") as f:
        ...
        if width != expected_width or height != expected_height:
            print(
                f"✗ Wrong grid dimensions: {width}x{height}, expected {expected_width}x{expected_height}"
            )
            return False
    ...
    # Run QA
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py qa --project stack-demo"
    )
    if not success:
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project stack-demo --grid 1x4 --cell-size 200"
    )
    if not success:
    ...
    with open(grid_file_1x4, "r") as f:
        ...
        if width != expected_width or height != expected_height:
            print(
                f"✗ Wrong 1x4 grid dimensions: {width}x{height}, expected {expected_width}x{expected_height}"
            )
            return False
...
def test_invalid_grids():
    ...
    # Test invalid grid
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project invalid-test --grid 2x2"
    )
    if success:
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project invalid-test --grid 3x2"
    )
    if success:
...
def main():
    ...
    tests = [test_stack_grids, test_invalid_grids]
