    ...
    def register_models_from_config(self, config: Dict[str, Any]):
        ...
        for name, model_data in config.items():
            model_info = ModelInfo(
                ...
                quantization=QuantizationType(model_data["quantization"])
...
def test_memory_requirements():
    ...
    print(
        f"  Text encoder models: {len(text_encoder_models)} models, {text_encoder_size}GB"
    )
    ...
    for model in large_models:
        print(
            f"    - {model.name}: {model.size_gb}GB ({model.quantization.value if model.quantization else 'no quantization'})"
        )
    ...
    fp8_models = [
        ...
        if model.quantization == QuantizationType.FP8
    ]
    fp16_models = [
        model
        for model in registry.models.values()
        if model.quantization == QuantizationType.FP16
    ]
...
def test_workflow_scenarios():
    ...
    video_models = ["hunyuan_video_i2v", "qwen_text_encoder"]
    ...
    large_workflow_models = ["wan_video_inpaint_high", "qwen_text_encoder"]
