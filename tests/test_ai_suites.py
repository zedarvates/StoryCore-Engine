...
async def _test_image_suite():
    ...
    print(
        f"Skin Enhance Result: {'Success' if result.success else 'Failed'} - Path: {output_path}"
    )
    assert result.success, (
        f"Skin Enhance failed: {getattr(result, 'error_message', 'Unknown error')}"
    )
    ...
    assert result.success, (
        f"AI Stylist failed: {getattr(result, 'error_message', 'Unknown error')}"
    )
...
async def _test_audio_suite():
    ...
    print(
        f"SFX Result: {'Success' if result.success else 'Failed'} - Path: {result.audio_path}"
    )
    ...
    print(
        f"V2A Result: {'Success' if result.success else 'Failed'} - Synced Path: {result.synchronized_audio_path}"
    )
