...
class TestGPUOptimization:
    ...
    async def test_cpu_fallback_mechanism(self):
        ...
        with patch.object(
            self.model_manager.gpu_memory_manager, "can_allocate", return_value=False
        ):
            with patch.object(
                self.model_manager.gpu_memory_manager,
                "allocate_memory",
                return_value=False,
            ):
                # Load model with GPU request - should fallback to CPU
                result = await self.model_manager.load_model(
                    "style_transfer_v1", device="cuda"
                )
    ...
    def test_gpu_memory_allocation_tracking(self):
        ...
        if success:
            ...
            assert (
                updated_report["allocated_memory_mb"] == initial_allocated + memory_mb
            )
            assert (
                updated_report["active_allocations"]
                == initial_report["active_allocations"] + 1
            )
    ...
    async def test_device_placement_optimization(self):
        ...
        result_gpu = await self.model_manager.load_model(
            "style_transfer_v1", device="cuda"
        )
        result_cpu = await self.model_manager.load_model(
            "super_resolution_v2", device="cpu"
        )
