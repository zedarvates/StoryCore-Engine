...
class TestRealTimePreviewSystem(unittest.TestCase):
    ...
    async def test_async_frame_generation(self):
        ...
        cached_frame = await self.system.generate_preview_frame_async(
    ...
    def test_system_statistics(self):
        ...
        self.system.create_session("user1", "project1")
        self.system.create_session("user2", "project2")
        ...
        update_message = json.dumps(
            {
                ...
                "parameters": {"interpolation_strength": 0.9, "quality_level": "high"},
            }
        )
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    async def test_complete_preview_workflow(self):
        ...
        frames = []
        for i in range(5):
            frame = await self.system.generate_preview_frame_async(
        ...
        self.assertEqual(stats["sessions"]["active_sessions"], 1)
        self.assertEqual(
            stats["performance"]["frames_generated"], 5
        )  # Only non-cached frames
    ...
    async def test_multi_session_performance(self):
        ...
        for session in sessions:
            for frame_idx in range(3):
                task = self.system.generate_preview_frame_async(
                    session.session_id, frame_idx
                )
                tasks.append(task)
    ...
    def test_quality_level_performance_scaling(self):
        ...
        for quality in quality_levels:
            ...
            self.system.update_session_parameters(
                session.session_id, {"quality_level": quality.value}
            )
        ...
        self.assertLess(
            processing_times[0], processing_times[1] + 0.01
        )  # Wireframe < Preview
        self.assertLess(
            processing_times[1], processing_times[2] + 0.01
        )  # Preview < High
                ...
                if asyncio.iscoroutinefunction(test_method):
                    ...
                    if result.wasSuccessful():
                        ...
                        for failure in result.failures + result.errors:
                            print(
                                f"      Error: {failure[1].split('AssertionError:')[-1].strip()}"
                            )
            except Exception as e:
