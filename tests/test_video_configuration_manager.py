...
class TestResolutionConfig(unittest.TestCase):
    ...
    def test_odd_resolution_dimensions(self):
        ...
        self.assertFalse(is_valid)
        self.assertIn(
            "Width and height should be even numbers for video compatibility", issues
        )
...
class TestQualityConfig(unittest.TestCase):
    ...
    def test_valid_quality_config(self):
        ...
        config = QualityConfig(
            preset=QualityPreset.PRODUCTION, quality_level=0.8, interpolation_samples=16
        )
        is_valid, issues = config.validate()
        self.assertTrue(is_valid)
        self.assertEqual(len(issues), 0)
...
class TestMotionCurveConfig(unittest.TestCase):
    ...
    def test_valid_motion_curve_config(self):
        ...
        config = MotionCurveConfig(
            curve_type=MotionCurveType.EASE_IN_OUT, ease_strength=2.0
        )
        is_valid, issues = config.validate()
        self.assertTrue(is_valid)
        self.assertEqual(len(issues), 0)
    ...
    def test_invalid_control_points(self):
        ...
        self.assertFalse(is_valid)
        self.assertTrue(
            any("x-coordinate must be between 0 and 1" in issue for issue in issues)
        )
...
class TestCameraOverrideConfig(unittest.TestCase):
    ...
    def test_invalid_position_keyframe_time(self):
        ...
        self.assertFalse(is_valid)
        self.assertTrue(
            any("time must be between 0 and 1" in issue for issue in issues)
        )
    ...
    def test_invalid_rotation_keyframe_angles(self):
        ...
        self.assertFalse(is_valid)
        self.assertTrue(
            any("pitch must be between -180 and 180" in issue for issue in issues)
        )
    ...
    def test_invalid_zoom_keyframe(self):
        ...
        self.assertFalse(is_valid)
        self.assertTrue(
            any("zoom factor must be positive" in issue for issue in issues)
        )
...
class TestVideoConfigurationManager(unittest.TestCase):
    ...
    def test_create_custom_configuration(self):
        ...
        custom_config = self.manager.create_custom_configuration(
            "documentary", motion_blur_intensity=0.8, interpolation_method="depth_aware"
        )
    ...
    def test_hardware_optimization(self):
        ...
        optimized = self.manager.optimize_for_hardware(
            base_config, gpu_available=False, memory_gb=2.0, cpu_cores=2
        )
    ...
    def test_preset_validation(self):
        ...
        for preset_name in preset_names:
            with self.subTest(preset=preset_name):
                ...
                is_valid, issues = config.validate()
                self.assertTrue(
                    is_valid, f"Preset '{preset_name}' validation failed: {issues}"
                )
    ...
    def test_configuration_format_detection(self):
        ...
        self.manager.save_configuration(
            yaml_config, yaml_file, ConfigurationFormat.YAML
        )
...
class TestConfigurationIntegration(unittest.TestCase):
    ...
    def test_hardware_optimization_consistency(self):
        ...
        for hw_config in hardware_configs:
            with self.subTest(hardware=hw_config):
                ...
                self.assertEqual(
                    optimized.quality.gpu_acceleration, hw_config["gpu_available"]
                )

                # Memory limit should not exceed available memory
                self.assertLessEqual(
                    optimized.quality.memory_limit_gb, hw_config["memory_gb"]
                )
    ...
    def test_configuration_serialization_roundtrip(self):
        ...
        self.assertEqual(
            original_config.interpolation_method, loaded_json.interpolation_method
        )
        self.assertEqual(
            original_config.motion_blur_intensity, loaded_json.motion_blur_intensity
        )
        ...
        # Compare key attributes
        self.assertEqual(
            original_config.interpolation_method, loaded_yaml.interpolation_method
        )
        self.assertEqual(
            original_config.motion_blur_intensity, loaded_yaml.motion_blur_intensity
        )
