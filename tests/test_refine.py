...
def test_refine_command():
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project refine-demo --grid 1x2 --cell-size 288"
    )
    if not success:
    ...
    print("\n3. Promoting panels...")
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py promote --project refine-demo --scale 2"
    )
    if not success:
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py refine --project refine-demo --mode unsharp --strength 1.0"
    )
    if not success:
    ...
    print(
        f"✓ Refinement metadata: input={metadata['input']}, mode={metadata['mode']}, strength={metadata['strength']}"
    )
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py qa --project refine-demo"
    )
    if not success:
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py refine --project refine-demo --input panels --mode sharpen --strength 1.5"
    )
    if not success:
    ...
    metadata = project_data["asset_manifest"]["refinement_metadata"]
    print(
        f"  Final settings: input={metadata['input']}, mode={metadata['mode']}, strength={metadata['strength']}"
    )

    refined_panels = project_data["asset_manifest"]["refined_panels"]
    for panel in refined_panels:
        print(
            f"  {panel['asset_id']}: {panel['resolution']} from {panel['original_source']}"
        )
