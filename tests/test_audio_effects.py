...
class TestAudioEffects:
    ...
    def test_equalization(self):
        ...
        result = self.engine.apply_equalization(
            self.test_audio, low_gain=6.0, mid_gain=-3.0, high_gain=3.0
        )
        assert len(result) == len(self.test_audio)
    ...
    def test_compression(self):
        ...
        result = self.engine.apply_compression(
            combined, threshold_db=-20.0, ratio=4.0, attack_time=0.01, release_time=0.1
        )
    ...
    def test_reverb(self):
        ...
        result = self.engine.apply_reverb(
            self.test_audio, room_size=0.5, damping=0.5, wet_level=0.3, dry_level=0.7
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_vibrato(self):
        ...
        result = self.engine.apply_vibrato(
            self.test_audio, rate=5.0, depth=0.5, wet_level=0.5, dry_level=0.5
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_distortion(self):
        ...
        result = self.engine.apply_distortion(
            self.test_audio, drive=3.0, tone=0.5, wet_level=0.5, dry_level=0.5
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_delay(self):
        ...
        result = self.engine.apply_delay(
            self.test_audio, delay_time=0.3, feedback=0.4, wet_level=0.3, dry_level=0.7
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_doppler_effect(self):
        ...
        result = self.engine.apply_doppler_effect(
            self.test_audio, speed=10.0, direction="approaching"
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_voice_modification(self):
        ...
        result = self.engine.modify_voice(
            self.test_audio, pitch_shift=2.0, formant_shift=0.5, gender_change=False
        )
        assert len(result) == len(self.test_audio)
        assert not np.array_equal(result, self.test_audio)
    ...
    def test_performance(self):
        ...
        self.engine.apply_gain(long_audio, 3.0)
        gain_time = time.time() - start_time

        start_time = time.time()
        self.engine.apply_compression(long_audio)
        compression_time = time.time() - start_time

        start_time = time.time()
        self.engine.apply_reverb(long_audio)
