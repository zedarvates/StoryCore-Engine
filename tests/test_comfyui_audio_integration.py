...
def test_workflow_loading():
    ...
    for workflow_type in loaded_workflows:
        ...
        print(
            f"   ✓ {workflow_type.value}: {len(workflow['nodes'])} nodes, {len(workflow['links'])} links"
        )
...
def test_workflow_customization():
    ...
    if ComfyUIWorkflowType.ACE_STEP_T2A in engine.audio_workflows:
        engine._customize_workflow(
            engine.audio_workflows[ComfyUIWorkflowType.ACE_STEP_T2A], request_t2a
        )
    ...
    if ComfyUIWorkflowType.STABLE_AUDIO in engine.audio_workflows:
        engine._customize_workflow(
            engine.audio_workflows[ComfyUIWorkflowType.STABLE_AUDIO], request_stable
        )
...
def test_audio_generation_integration():
        ...
        "scenes": [
            {
                ...
                "dialogue": [
                    {
                        "character_id": "hero",
                        "text": "What a beautiful forest!",
                        "emotion": "happy",
                    }
                ],
                "actions": [{"type": "walk", "description": "Walking through forest"}],
            },
            {
                "scene_id": "scene_2",
                "environment": {
                ...
                "dialogue": [
                    {
                        "character_id": "hero",
                        "text": "This cave is spooky.",
                        "emotion": "nervous",
                    }
                ],
                "actions": [{"type": "magic", "description": "Casting light spell"}],
            },
        ]
    }
    ...
    for track in project.tracks:
        ...
        if comfyui_clips > 0:
            print(
                f"   ✓ {track.track_id}: {comfyui_clips}/{len(track.clips)} clips use ComfyUI"
            )
        ...
        if audio_path:
            ...
            if Path(audio_path).exists():
                with open(audio_path, "r") as f:
                    ...
                    print("   ✓ Mock file contains expected metadata")
...
def test_export_with_comfyui_metadata():
    ...
    character_data = {
        "characters": [{"character_id": "test", "age": "adult", "gender": "neutral"}]
    }
