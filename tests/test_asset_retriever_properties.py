...
from src.asset_retriever import AssetRetriever, CleanupPolicy, AssetDownload
...
class TestAssetRetrieverProperties:
    ...
    def test_property_15_asset_retrieval_completeness(self, execution_result):
        ...
        retriever.metrics.successful_downloads = (
            len(execution_result.output_images) - 1
        ...
        assert (
            retriever.metrics.successful_downloads + retriever.metrics.failed_downloads
            == retriever.metrics.total_assets
        )
        ...
        if retriever.metrics.total_assets > 0:
            expected_rate = (
                retriever.metrics.successful_downloads / retriever.metrics.total_assets
            ) * 100.0
    ...
    def test_property_16_file_organization_consistency(
        self, file_data, organization_scheme
    ):
        """
    ...
    def test_property_17_storage_management(self, cleanup_policy):
        ...
        for dir_name, dir_stats in stats["directories"].items():
            ...
            assert dir_stats["size_mb"] == round(
                dir_stats["size_bytes"] / (1024 * 1024), 2
            )
        ...
        assert (
            retriever.cleanup_policy.max_total_size_mb
            == cleanup_policy.max_total_size_mb
        )
        assert (
            retriever.cleanup_policy.keep_successful_downloads
            == cleanup_policy.keep_successful_downloads
        )
        assert (
            retriever.cleanup_policy.keep_failed_downloads
            == cleanup_policy.keep_failed_downloads
        )
        ...
        for asset_info in asset_list:
            ...
            if asset_info.checksum:
                ...
                if len(asset_info.checksum) == 64:
                    assert all(
                        c in "0123456789abcdef" for c in asset_info.checksum.lower()
                    )
    ...
    def test_property_metrics_consistency(self, total_operations):
        ...
        if total_operations > 0:
            ...
            assert (
                retriever.metrics.successful_downloads
                + retriever.metrics.failed_downloads
                == retriever.metrics.total_assets
            )
