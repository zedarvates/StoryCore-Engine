...
async def test_hunyuan_circuit_breaker():
    ...
    try:
        ...
        # Check can execute
        assert integration.t2v_circuit.can_execute()
...
async def test_wan_resilient_initialization():
    ...
    try:
        ...
        # Check fallback chains configured
        transparent_chain = integration.error_system.get_fallback_chain(
            "wan_transparent"
        )
...
async def test_wan_resilient_transparent_video():
    ...
    try:
        ...
        rgba_frames = await integration.create_transparent_video(
            prompt="A floating ghost", alpha_mode=AlphaChannelMode.THRESHOLD
        )
...
async def test_wan_resilient_inpainting():
    ...
    try:
        config = WanVideoConfig(
            width=720, height=480, num_frames=41, enable_inpainting=True
        )
        ...
        inpainted_frames = await integration.inpaint_video(
            prompt="Fill with landscape", video_frames=input_frames, mask=mask
        )
