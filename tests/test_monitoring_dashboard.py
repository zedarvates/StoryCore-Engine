...
class TestMetricPoint:
    ...
    def test_metric_point_creation(self):
        ...
        point = MetricPoint(
            timestamp=timestamp, value=0.95, metadata={"source": "test"}
        )
    ...
    def test_metric_point_defaults(self):
        ...
        point = MetricPoint(timestamp=datetime.now(), value=1.0)
...
class TestMetricsCollector:
    ...
    def test_get_metric_history_with_time_window(self, collector):
        ...
        assert all(
            (datetime.now() - p.timestamp) < timedelta(seconds=2) for p in history
        )
...
class TestAlertManager:
    ...
    def test_create_alert_with_details(self, manager):
        ...
        manager.create_alert(
            "critical", "error_rate", "Error rate too high", {"value": 0.15}
        )
...
class TestIntegrationScenarios:
    ...
    async def test_monitoring_with_failures(self):
        ...
        for i in range(10):
            request = WorkflowRequest(
                workflow_type="flaky_workflow", user_id="user123", prompt=f"Test {i}"
            )
            try:
