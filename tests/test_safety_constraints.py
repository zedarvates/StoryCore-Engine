...
def test_intention_attribution_filtering():
    ...
    claim = Claim(
        id="test_1", text="The sky is blue", position=(0, 15), domain="physics"
    )
...
def test_apply_uncertainty_handling():
    ...
    assert (
        "uncertain" in processed_report.claims[0].reasoning.lower()
        or "confidence" in processed_report.claims[0].reasoning.lower()
    )
...
def test_manipulation_signal_filtering():
    ...
    assert (
        ...
        not in filtered_report.manipulation_signals[0].description.lower()
    )
