...
async def test_comfyui_integration():
    ...
    integration = WanVideoIntegration(config=config, timeout_seconds=10.0)
        ...
        with patch.object(integration.comfyui_manager, "initialize", return_value=True):
            with patch.object(integration.comfyui_manager, "client") as mock_client:
                ...
                if "PIL" in str(type(__import__("PIL", fromlist=["Image"]))):
                    ...
                    # Create mask
                    mask = InpaintingMask(mask_image=Image.new("L", (832, 480), 255))

                    # Mock workflow execution
                    with patch.object(
                        integration, "_execute_comfyui_workflow"
                    ) as mock_execute:
                        ...
                        assert len(result) == len(input_frames), (
                ...
                with patch.object(
                    ...
                    mock_execute.return_value = {
                        "status": "completed",
                        "video_frames": [
                            Image.new("RGBA", (832, 480), (255, 0, 0, 255))
                            for _ in range(16)
                        ]
                        if "PIL" in str(type(__import__("PIL", fromlist=["Image"])))
                        else [],
                    ...
                    (
                        rgb_frames,
                        alpha_masks,
                    ) = await integration.generate_video_with_alpha(
