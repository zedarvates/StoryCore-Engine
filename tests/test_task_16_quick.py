...
def test_integration():
    ...
    try:
        ...
        deblur_result = deblurrer.deblur_frame(
            denoise_result.denoised_frame, method=DeblurMethod.WIENER
        )
        print(f"   - Deblurred: improvement={deblur_result.sharpness_improvement:.2%}")

        # Tone map
        tone_result = tone_mapper.tone_map(
            deblur_result.deblurred_frame, method=ToneMappingMethod.REINHARD
        )
    ...
    try:
        ...
        graded = color_grader.grade_sequence(
            denoised, style=ColorGradingStyle.WARM, smooth_transitions=True
        )
    ...
    try:
        ...
        print(
            f"   - Denoiser: {denoise_stats.get('frames_processed', 0)} frames processed"
        )
        print(
            f"   - Deblurrer: {deblur_stats.get('frames_processed', 0)} frames processed"
        )
        print(
            f"   - Tone Mapper: {tone_stats.get('frames_processed', 0)} frames processed"
        )
