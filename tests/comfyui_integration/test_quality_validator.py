...
class TestQualityValidator:
    ...
    def create_test_image(
        self, path: Path, width: int = 512, height: int = 512, format: str = "PNG"
    ):
    ...
    def test_check_file_size_valid(self, validator, temp_dir):
        ...
        result = validator.check_file_size(
            file_path, validator.IMAGE_MIN_SIZE, validator.IMAGE_MAX_SIZE
        )
    ...
    def test_validation_result_error_messages(self, validator, temp_dir):
        ...
        for error in result.errors:
            ...
            assert any(
                keyword in error.lower() for keyword in ["format", "size", "dimension"]
            )
