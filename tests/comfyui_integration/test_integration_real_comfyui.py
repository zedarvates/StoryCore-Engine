...
class TestFluxTurboImageGeneration:
    ...
    async def test_flux_turbo_image_generation_basic(
        self, workflow_executor, quality_validator, output_manager
    ):
        """
        ...
        parameters = {"prompt": test_prompt, "seed": 42}
        ...
        result = await workflow_executor.wait_for_completion(
            prompt_id, timeout=TEST_TIMEOUT
        )
        assert result is not None, "Workflow execution failed"

        # Download output
        output_path = (
            output_manager.create_timestamped_directory() / "flux_turbo_test.png"
        )
        ...
        assert validation_result.metadata["file_size"] > 10240, (
            "File too small (< 10KB)"
        )
    ...
    async def test_flux_turbo_multiple_prompts(
        ...
        for idx, prompt in enumerate(TEST_IMAGE_PROMPTS):
            ...
            # Download
            output_path = (
                output_manager.create_timestamped_directory()
                / f"flux_turbo_prompt_{idx}.png"
            )
            downloaded_path = await workflow_executor.download_output(
                result, output_path
            )
...
class TestLTX2VideoGeneration:
    ...
    async def test_ltx2_video_generation_basic(
        self, workflow_executor, quality_validator, output_manager, test_image_path
    ):
        """
        ...
        assert validation_result.passed, (
            f"Video validation failed: {validation_result.errors}"
        )
        ...
        assert validation_result.metadata["file_size"] > 102400, (
    ...
    async def test_ltx2_with_different_prompts(
        ...
        for idx, prompt in enumerate(camera_prompts):
            ...
            # Download
            output_path = (
                output_manager.create_timestamped_directory() / f"ltx2_prompt_{idx}.mp4"
            )
            downloaded_path = await workflow_executor.download_output(
                result, output_path
            )
    ...
    async def test_ltx2_with_test_runner(self, test_runner, test_image_path):
        ...
        # Run test
        result = await test_runner.run_video_generation_test(
            test_image_path, test_prompt
        )
    ...
    async def test_ltx2_video_format_validation(
        ...
        output_path = (
            output_manager.create_timestamped_directory() / "ltx2_format_test.mp4"
        )
        downloaded_path = await workflow_executor.download_output(result, output_path)
...
class TestFullPipeline:
    ...
    async def test_full_pipeline_text_to_video(
        ...
        image_workflow = workflow_executor.load_workflow(
            "z_image_turbo_generation.json"
        )
        image_prompt = "A beautiful mountain landscape at sunset"
        image_params = {"prompt": image_prompt, "seed": 42}
        image_workflow = workflow_executor.inject_parameters(
            image_workflow, image_params
        )
        ...
        image_result = await workflow_executor.wait_for_completion(
            image_prompt_id, timeout=TEST_TIMEOUT
        )
        ...
        assert image_validation.passed, (
            f"Image validation failed: {image_validation.errors}"
        )
        ...
        video_workflow = workflow_executor.inject_parameters(
        ...
        video_result = await workflow_executor.wait_for_completion(
            video_prompt_id, timeout=TEST_TIMEOUT
        )
        ...
        assert video_validation.passed, (
            f"Video validation failed: {video_validation.errors}"
        )
        ...
        assert str(image_path) in str(video_params["image_path"]), (
            "Image not used as video input"
        )
    ...
    async def test_pipeline_execution_order(
        ...
        image_path = await workflow_executor.download_output(
            image_result, output_dir / "order_test_image.png"
        )
        ...
        await workflow_executor.download_output(
            video_result, output_dir / "order_test_video.mp4"
        )
        ...
        image_complete = next(
            t for event, t in execution_log if event == "image_complete"
        )
        video_start = next(t for event, t in execution_log if event == "video_start")
        video_complete = next(
            t for event, t in execution_log if event == "video_complete"
        )
    ...
    async def test_pipeline_output_chaining(
        ...
        image_path = await workflow_executor.download_output(
            image_result, output_dir / "chain_test_image.png"
        )
        ...
        video_path = await workflow_executor.download_output(
            video_result, output_dir / "chain_test_video.mp4"
        )
    ...
    async def test_pipeline_all_outputs_saved(
        ...
        image_path = await workflow_executor.download_output(
            image_result, output_dir / "save_test_image.png"
        )
        ...
        video_path = await workflow_executor.download_output(
            video_result, output_dir / "save_test_video.mp4"
        )
...
class TestErrorScenarios:
    ...
    async def test_workflow_file_not_found(self, connection_manager):
        ...
        assert "nonexistent_workflow.json" in error_message, (
            "Workflow name not in error"
        )
    ...
    async def test_invalid_workflow_json(self, connection_manager, tmp_path):
        ...
        assert any(
            keyword in error_message
            for keyword in ["json", "parse", "invalid", "decode"]
        ), f"Error doesn't indicate JSON problem: {exc_info.value}"

        print(f"✓ Clear error for invalid JSON: {exc_info.value}")

    async def test_workflow_execution_timeout(self, workflow_executor, output_manager):
        """
    ...
    async def test_missing_model_error_message(self, connection_manager, workflows_dir):
        """
        ...
        try:
            ...
            await executor.wait_for_completion(prompt_id, timeout=30)
    ...
    async def test_error_logging_completeness(self, workflow_executor, caplog):
        """
    ...
    async def test_validation_error_messages(self, quality_validator, tmp_path):
        """
    ...
    async def test_workflow_submission_error_handling(
        self, connection_manager, workflows_dir
    ):
        """
        ...
        try:
            await executor.execute_workflow(invalid_workflow)
    ...
    async def test_output_download_error_handling(
        self, workflow_executor, output_manager
    ):
        """
