...
def mock_executor():
    ...
    mock.wait_for_completion = AsyncMock(
        return_value={
            "outputs": {
                "node_1": {"images": [{"filename": "test.png", "type": "output"}]}
            }
        }
    )
...
def mock_output_manager():
    ...
    mock = MagicMock()
    mock.create_timestamped_directory = MagicMock(
        return_value=Path("/tmp/20260128_120000")
    )
    mock.save_output = MagicMock(
        return_value=Path("/tmp/20260128_120000/image/test.png")
    )
    mock.log_output_path = MagicMock()
    mock.generate_report = MagicMock(
        return_value=Path("/tmp/20260128_120000/report.json")
    )
    return mock
...
class TestComfyUITestRunner:
    ...
    def test_init(self, test_config):
        ...
        with (
            patch("src.comfyui_test_framework.test_runner.ComfyUIConnectionManager"),
    ...
    async def test_run_image_generation_test_success(
        ...
        with (
            patch(
                "src.comfyui_test_framework.test_runner.ComfyUIConnectionManager",
                return_value=mock_connection,
            ),
            patch(
                "src.comfyui_test_framework.test_runner.WorkflowExecutor",
                return_value=mock_executor,
            ),
            patch(
                "src.comfyui_test_framework.test_runner.QualityValidator",
                return_value=mock_validator,
            ),
            patch(
                "src.comfyui_test_framework.test_runner.OutputManager",
            ...
            with (
                ...
                result = await runner.run_image_generation_test(
                    prompt="A beautiful landscape", test_name="test_image", seed=12345
                )
            ...
            mock_executor.load_workflow.assert_called_once_with(
    ...
    async def test_run_image_generation_test_failure(
        ...
        with (
            ...
            result = await runner.run_image_generation_test(
                prompt="Test prompt", test_name="test_image_fail"
            )
    ...
    async def test_run_video_generation_test_success(
        ...
        mock_executor.download_output = AsyncMock(
            return_value=Path("/tmp/test_output.mp4")
        )
        mock_executor.wait_for_completion = AsyncMock(
            return_value={
                "outputs": {
                    "node_1": {"videos": [{"filename": "test.mp4", "type": "output"}]}
                }
            }
        )
