...
def sample_workflow():
    ...
    return {
        ...
        "57": {
            "inputs": {"seed": 12345, "steps": 20, "width": 512, "height": 512},
            "class_type": "KSampler",
        },
    }
...
def sample_ltx2_workflow():
    ...
    return {
        "98": {"inputs": {"image": "default.png"}, "class_type": "LoadImage"},
        "92": {
            "inputs": {
...
class TestWorkflowExecutorInit:
    ...
    def test_init_with_valid_directory(
        self, mock_connection_manager, temp_workflows_dir
    ):
...
class TestLoadWorkflow:
    ...
    def test_load_workflow_success(
        self, mock_connection_manager, temp_workflows_dir, sample_workflow
    ):
...
class TestInjectParameters:
    ...
    def test_inject_prompt_flux_turbo(
        self, mock_connection_manager, temp_workflows_dir, sample_workflow
    ):
    ...
    def test_inject_image_path_ltx2(
        self, mock_connection_manager, temp_workflows_dir, sample_ltx2_workflow
    ):
    ...
    def test_inject_video_prompt_ltx2(
        self, mock_connection_manager, temp_workflows_dir, sample_ltx2_workflow
    ):
    ...
    def test_inject_multiple_parameters(
        self, mock_connection_manager, temp_workflows_dir, sample_workflow
    ):
...
class TestExecuteWorkflow:
    ...
    async def test_execute_workflow_success(
        ...
        mock_connection_manager.post = AsyncMock(
            return_value={"prompt_id": "test-prompt-123"}
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_workflow_no_prompt_id(
        self, mock_connection_manager, temp_workflows_dir, sample_workflow
    ):
...
class TestWaitForCompletion:
    ...
    async def test_wait_for_completion_success(
        ...
        mock_connection_manager.get = AsyncMock(
            return_value={
                "test-prompt-123": {
        ...
        result = await executor.wait_for_completion(
            "test-prompt-123", timeout=10, poll_interval=1
        )
    ...
    @pytest.mark.asyncio
    async def test_wait_for_completion_with_outputs(
        self, mock_connection_manager, temp_workflows_dir
    ):
    ...
    async def test_wait_for_completion_timeout(
        ...
        with pytest.raises(TimeoutError) as exc_info:
            await executor.wait_for_completion(
                "test-prompt-123", timeout=2, poll_interval=1
            )
    ...
    @pytest.mark.asyncio
    async def test_wait_for_completion_error(
        self, mock_connection_manager, temp_workflows_dir
    ):
        ...
        mock_connection_manager.get = AsyncMock(
            return_value={
                "test-prompt-123": {
                    "status": {"completed": False, "error": "Model not found"}
                }
            }
        )
...
class TestDownloadOutput:
    ...
    @pytest.mark.asyncio
    async def test_download_output_image_success(
        self, mock_connection_manager, temp_workflows_dir, tmp_path
    ):
        ...
        mock_connection_manager.session.get.return_value.__aenter__ = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_download_output_video_success(
        self, mock_connection_manager, temp_workflows_dir, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_download_output_no_outputs(
        self, mock_connection_manager, temp_workflows_dir, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_download_output_no_files(
        self, mock_connection_manager, temp_workflows_dir, tmp_path
    ):
        """Test download failure when no files in outputs."""
        executor = WorkflowExecutor(mock_connection_manager, temp_workflows_dir)

        output_info = {"outputs": {"node_1": {}}}
        save_path = tmp_path / "output.png"
    ...
    @pytest.mark.asyncio
    async def test_download_output_http_error(
        self, mock_connection_manager, temp_workflows_dir, tmp_path
    ):
        ...
        output_info = {
            "outputs": {
                "node_1": {"images": [{"filename": "output.png", "type": "output"}]}
            }
        }
