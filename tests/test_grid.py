...
def test_grid_generation():
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py init test-grid-project"
    )
    if not success:
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project test-grid-project --grid 3x3"
    )
    if not success:
    ...
    # Test 6: Run QA
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py qa --project test-grid-project"
    )
    if not success:
...
def test_grid_validation():
    ...
    success, stdout, stderr = run_command(
        "python3 storycore_cli.py grid --project test-validation --grid 2x2"
    )
    if success:
...
def main():
    ...
    tests = [test_grid_generation, test_grid_validation]
