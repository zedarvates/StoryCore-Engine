...
def test_video_processing_pipeline():
    ...
    denoised = denoiser.denoise_sequence(
        frames, method=DenoiseMethod.GAUSSIAN, strength=0.5
    )
    print(f"   - Denoised {len(denoised)} frames")

    # Step 3: Deblur
    print("   Step 3/7: Deblurring...")
    deblurred = deblurrer.deblur_sequence(
        denoised, method=DeblurMethod.WIENER, strength=0.5
    )
    print(f"   - Deblurred {len(deblurred)} frames")

    # Step 4: Tone mapping
    print("   Step 4/7: Tone mapping...")
    tone_mapped = tone_mapper.tone_map_sequence(
        deblurred, method=ToneMappingMethod.ACES
    )
    ...
    assert all(f.shape == (480, 640, 3) for f in result.converted_frames), (
...
def test_quality_enhancement_integration():
    ...
    deblur_result = deblurrer.deblur_frame(
        denoise_result.denoised_frame, method=DeblurMethod.RICHARDSON_LUCY
    )
    print(
        f"   - Deblurred: sharpness_improvement={deblur_result.sharpness_improvement:.2%}"
    )

    # Tone map
    tone_result = tone_mapper.tone_map(
        deblur_result.deblurred_frame, method=ToneMappingMethod.FILMIC
    )
    print(f"   - Tone mapped: compression={tone_result.compression_ratio:.2f}x")

    # Color grade
    grade_result = color_grader.apply_style(
        tone_result.mapped_frame, ColorGradingStyle.CINEMATIC, intensity=0.8
    )
...
def test_temporal_consistency():
    ...
    return {"flicker_score": metrics.flicker_score, "color_drift": metrics.color_drift}
...
def test_performance_metrics():
    ...
    print(
        f"   Denoiser: {denoise_stats['frames_processed']} frames, "
    ...
    print(
        f"   Deblurrer: {deblur_stats['frames_processed']} frames, "
        f"avg_improvement={deblur_stats['avg_sharpness_improvement']:.2%}"
    )

    # Tone map
    tone_mapper.tone_map_sequence(deblurred)
    tone_stats = tone_mapper.get_statistics()
    print(
        f"   Tone Mapper: {tone_stats['frames_processed']} frames, "
