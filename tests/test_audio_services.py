...
from backend.sfx_profile_builder import SFXService, SFXProfileBuilder, MufflingType
...
class TestAudioEffects:
    ...
    def test_apply_eq(self, sample_audio):
        """Test EQ application"""
        result = AudioEffects.apply_eq(
            sample_audio, low_gain=3, mid_gain=0, high_gain=-3
        )
    ...
    def test_apply_compressor(self, sample_audio):
        ...
        result = AudioEffects.apply_compressor(
            sample_audio, threshold_db=-20.0, ratio=4.0
        )
    ...
    def test_apply_limiter(self, sample_audio):
        ...
        assert (
            np.max(np.abs(result)) <= 10 ** (-1 / 20) * 1.1
...
class TestAudioMixService:
    ...
    def test_set_mix_config(self, service):
        ...
        config = MixConfig(channel_layout=AudioChannel.SURROUND_51, sample_rate=96000)
    ...
    def test_sort_by_priority(self, service):
        ...
        tracks = [
            MixNode(
                ...
                priority=TrackPriority.AMBIENT,
                volume=-12,
            ),
            MixNode(
                id="t2",
                ...
                priority=TrackPriority.DIALOGUE,
                volume=0,
            ),
            MixNode(
                id="t3",
    ...
    def test_auto_mix(self, service):
        ...
        tracks_data = [
            {"id": "t1", "name": "Dialogue", "category": "dialogue", "volume": 0.0},
            {"id": "t2", "name": "Music", "category": "music", "volume": -6.0},
        ]
...
class TestVoiceService:
    ...
    async def test_generate_voice(self, voice_service):
        ...
        success, audio_data, message = await voice_service.generate_voice(
            text="Bonjour, ceci est un test.", voice="Amelie", language="fr-FR"
        )
    ...
    async def test_generate_voice_with_emotion(self, voice_service):
        ...
        success, audio_data, message = await voice_service.generate_voice(
            text="Ceci est un test.", voice="Aria", language="en-US", emotion="happy"
        )
...
class TestSFXService:
    ...
    async def test_play_sfx(self, sfx_service):
        ...
        success, sound_id = await sfx_service.play_sfx(
            category="ui_click", variant=0, volume=1.0
        )
...
class TestConvenienceFunctions:
    ...
    def test_create_mix_config(self):
        ...
        config = create_mix_config(
            channels="5.1", sample_rate=96000, bit_depth=24, output_format="wav"
        )
