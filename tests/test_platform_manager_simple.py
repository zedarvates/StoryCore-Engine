...
class TestPlatformManager:
    ...
    def setup_method(self):
        ...
        (models_dir / "checkpoints" / "test_model.safetensors").write_bytes(
            b"fake model data" * 1000
        )
        (models_dir / "loras" / "test_lora.safetensors").write_bytes(
            b"fake lora data" * 100
        )
    ...
    def test_gpu_detection_fallback_cpu(self):
        ...
        with (
            patch.object(PlatformManager, "_detect_nvidia_gpus", return_value=[]),
    ...
    def test_nvidia_gpu_detection_nvidia_smi(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="GeForce RTX 3080, 10240, 8192\n"
        )
    ...
    def test_amd_gpu_detection(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="GPU,VRAM Total,VRAM Used\n0,8192,2048\n"
        )
    ...
    def test_model_scanning(self):
        ...
        checkpoint_models = [
            m for m in models.values() if m.model_type == "checkpoints"
        ]
    ...
    def test_memory_constraints(self, mock_memory):
        ...
        [issue for issue in issues if "memory" in issue.lower()]
