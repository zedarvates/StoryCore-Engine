...
class TestEndToEndOrchestrator:
        ...
        return ParsedPrompt(
            ...
            characters=[
                CharacterInfo(
                    name="Snow White",
                    role="main",
                    description="Princess in cyberpunk world",
                )
            ],
    ...
    async def test_parsing_step(self, orchestrator, sample_prompt, parsed_prompt):
        ...
        orchestrator.prompt_parser.validate_parsed_data = MagicMock(
            return_value=(True, [])
        )
    ...
    async def test_name_generation_step(self, orchestrator, parsed_prompt):
        ...
        assert (
            orchestrator._current_state.current_step
            == WorkflowStep.COMPONENT_GENERATION
        )
        assert (
            WorkflowStep.NAME_GENERATION in orchestrator._current_state.completed_steps
        )
    ...
    async def test_name_sanitization(self, orchestrator, parsed_prompt):
        ...
        orchestrator.name_generator.validate_project_name = MagicMock(
            return_value=False
        )
...
class TestCreateProjectFunction:
    ...
    async def test_create_project_function(self, temp_dir):
        ...
        with (
            patch("src.end_to_end.orchestrator.PromptParser") as mock_parser,
            ...
            patch(
                "src.end_to_end.orchestrator.ProjectStructureBuilder"
            ) as mock_builder,
            ...
            parsed = ParsedPrompt(
                ...
                time_period="2048",
                characters=[
                    CharacterInfo(name="Hero", role="main", description="Test")
                ],
                key_elements=["test"],
                visual_style=["neon"],
                aspect_ratio="16:9",
            ...
            mock_name_gen_instance.generate_project_name = MagicMock(
                return_value="test_project"
            )
            ...
            mock_comp.validate_coherence = MagicMock(
                return_value={"is_coherent": True, "issues": []}
            )
            ...
            mock_builder_instance.validate_structure = MagicMock(
                return_value={"valid": True}
            )
            ...
            mock_pipeline_instance.execute_with_autofix = AsyncMock(
                return_value=mock_pipeline_result
            )
            mock_pipeline.return_value = mock_pipeline_instance

            mock_qa_instance = MagicMock()
            mock_qa_instance.validate_final_video = AsyncMock(
                return_value=MagicMock(overall_score=0.9, passed=True)
            )
            mock_qa.return_value = mock_qa_instance

            # Call create_project
            result = await create_project(
                prompt="Create a cyberpunk trailer", projects_dir=str(temp_dir)
            )
...
class TestOrchestratorEdgeCases:
    ...
    async def test_workflow_error_handling(self, orchestrator, sample_prompt):
        ...
        orchestrator.prompt_parser.parse = MagicMock(
