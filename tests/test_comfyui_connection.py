...
async def generate_test_image(port):
        ...
        "nodes": [
            {
                ...
                "widgets_values": [
                    "A beautiful test image for StoryCore, cinematic lighting, high quality"
                ],
            },
            {
                "id": 2,
            ...
            },
            {
                ...
                "outputs": [
                    {"name": "CLIP", "type": "CLIP", "links": [1, 3], "slot_index": 0}
                ],
                "widgets_values": [
            ...
            },
            {
                ...
                "size": {"0": 315.0, "1": 60.0},
                "outputs": [
                    {"name": "VAE", "type": "VAE", "links": [5], "slot_index": 0}
                ],
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            payload = {"prompt": test_workflow, "client_id": "storycore_test"}
...
async def main():
    ...
    print("=" * 60)
    print(
        f"   ComfyUI: {'✅ Port ' + str(comfyui_port) if comfyui_port else '❌ Not found'}"
    )
    print(
        f"   StoryCore API: {'✅ Port ' + str(api_port) if api_port else '⚠️ Not running'}"
    )
    print("=" * 60)
