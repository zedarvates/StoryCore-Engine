...
async def test_non_blocking_hook_returns_quickly(pipeline_integration):
    ...
    pipeline_integration.configure_hook(
        "before_generate", HookConfig(enabled=True, blocking=False)
    )
...
async def test_blocking_hook_waits_for_completion(pipeline_integration):
    ...
    pipeline_integration.configure_hook(
        "on_publish", HookConfig(enabled=True, blocking=True, on_high_risk="block")
    )
...
async def test_disabled_hook_skips_execution(pipeline_integration):
    ...
    # Disable hook
    pipeline_integration.configure_hook("before_generate", HookConfig(enabled=False))
...
async def test_high_risk_content_emits_warning(pipeline_integration, temp_project_dir):
    ...
    pipeline_integration.configure_hook(
        "on_publish", HookConfig(enabled=True, blocking=True, on_high_risk="warn")
    )
...
async def test_data_contract_storage(pipeline_integration, temp_project_dir):
    ...
    pipeline_integration.configure_hook(
        "after_generate", HookConfig(enabled=True, blocking=True, store_results=True)
    )

    await pipeline_integration.execute_hook("after_generate", content)
...
async def test_project_json_update(pipeline_integration, temp_project_dir):
    ...
    # Configure to store results
    pipeline_integration.configure_hook(
        "before_generate", HookConfig(enabled=True, blocking=True, store_results=True)
    )

    await pipeline_integration.execute_hook("before_generate", content)
...
def test_event_callback_registration(pipeline_integration):
    ...
    event = WarningEvent(
        risk_level="high", summary="Test warning", hook_stage="before_generate"
    )
...
async def test_error_handling_in_hook(pipeline_integration):
    ...
    pipeline_integration.configure_hook(
        "before_generate", HookConfig(enabled=True, blocking=True)
    )
...
def test_is_high_risk_detection(pipeline_integration):
    ...
    high_risk_result = {
        "status": "success",
        "report": {
            "summary_statistics": {"high_risk_count": 2},
            "claims": [{"risk_level": "high"}],
        },
    }
    ...
    low_risk_result = {
        "status": "success",
        "report": {
            "summary_statistics": {"high_risk_count": 0},
            "claims": [{"risk_level": "low"}],
        },
    }
