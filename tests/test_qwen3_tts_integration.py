...
class TestQwen3TTSIntegration(unittest.TestCase):
    ...
    def test_clone_voice_success(self, mock_qwen_tts):
        ...
        # Clonage de voix
        result = qwen3_tts.clone_voice(
            reference_audio_path, "Test text", self.output_path
        )

        # Vérifications
        self.assertTrue(result)
        self.assertTrue(os.path.exists(self.output_path))
        mock_instance.clone_voice.assert_called_once_with(
            b"fake_reference_audio", "Test text"
        )
...
class TestQwen3TTSPromptStrategy(unittest.TestCase):
    ...
    def test_generate_prompt_few_shot(self):
        ...
        prompt = self.prompt_strategy.generate_prompt(
            "male", "happy", "en", use_few_shot=True
        )
    ...
    def test_save_custom_prompt(self):
        ...
        result = self.prompt_strategy.save_custom_prompt(
        ...
        self.assertTrue(result)
        self.assertEqual(
            self.prompt_strategy.get_custom_prompt("test_prompt"),
