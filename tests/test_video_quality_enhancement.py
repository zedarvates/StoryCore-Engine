...
def test_ai_deblurrer():
    ...
    deblurrer = AIDeblurrer(
        default_method=DeblurMethod.ADAPTIVE, max_iterations=10, preserve_edges=True
    )
...
def test_color_grading_ai():
    ...
    result = grader.apply_custom_grade(
        frame, brightness=0.1, contrast=0.2, saturation=0.15, temperature=0.1
    )
    print(f"   {result}")
    assert result.graded_frame.shape == frame.shape, "Shape mismatch"
    ...
    graded_frames = grader.grade_sequence(
        frames, style=ColorGradingStyle.WARM, intensity=0.9, smooth_transitions=True
    )
...
def test_hdr_tone_mapper():
    ...
    mapper = HDRToneMapper(
        target_standard=HDRStandard.SDR, preserve_colors=True, adaptive_local=True
    )
    ...
    sdr_frames = mapper.tone_map_sequence(
        frames, method=ToneMappingMethod.FILMIC, smooth_transitions=True
    )
    ...
    for method in methods:
        ...
        assert result.mapped_frame.shape == hdr_frame.shape, (
...
def test_integration():
    ...
    grade_result = grader.apply_style(
        tone_result.mapped_frame, ColorGradingStyle.CINEMATIC
    )
