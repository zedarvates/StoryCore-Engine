...
class EndToEndPipelineVerifier:
    ...
    async def setup_test_environment(self) -> bool:
        ...
        try:
            ...
            if status.available:
                self.log_result(
                    "ComfyUI Connection", True, f"Connected to {status.url}"
                )
            else:
                self.log_result(
                    "ComfyUI Connection",
    ...
    async def verify_workflow_deployment(self) -> bool:
        ...
        try:
            ...
            self.log_result(
                "Workflow Check", True, f"Found {installed_count} installed workflows"
            )
            ...
            self.workflow_manager.create_z_image_turbo_workflow(
                "Test prompt for verification", z_config, seed=42
            )
            self.log_result(
                "Z-Image Turbo Workflow", True, "Workflow created successfully"
            )

            # Test LTX-2 workflow creation
            ltx2_config = LTX2ImageToVideoConfig(input_image_path="test_image.png")
            self.workflow_manager.create_ltx2_image_to_video_workflow(
                "test_image.png", "Test motion prompt", ltx2_config
            )
    ...
    async def verify_outputs(self) -> bool:
        ...
        try:
            ...
            if self.test_dir.exists():
                ...
                self.log_result(
                    "Output Files", True, f"Generated {file_count} output files"
                )
                ...
                for file in files:
                    if file.is_file():
                        ...
                        print(
                            f"  - {file.relative_to(self.test_dir)} ({size_kb:.1f} KB)"
                        )
