...
class ComponentVerifier:
    ...
    async def verify_configuration(self) -> bool:
        ...
        try:
            ...
            self.log_result(
                "Configuration Defaults", True, "Default configuration loaded correctly"
            )
            ...
            if errors:
                self.log_result(
                    "Configuration Validation", False, f"Validation errors: {errors}"
                )
                return False
            self.log_result(
                "Configuration Validation", True, "Configuration validated successfully"
            )
    ...
    async def verify_connection_manager(self) -> bool:
        ...
        try:
            ...
            if status.available:
                self.log_result(
                    "ComfyUI Connection", True, f"Connected to ComfyUI at {status.url}"
                )
                self.log_result(
                    "ComfyUI Version", True, f"Version: {status.version or 'Unknown'}"
                )
            else:
                self.log_result(
                    "ComfyUI Connection",
                ...
                self.log_result(
                    "Fallback Mode", True, "System will use mock mode fallback"
                )
    ...
    async def verify_model_manager(self) -> bool:
        ...
        try:
            ...
            self.log_result(
                "ModelManager Creation", True, "ModelManager created successfully"
            )
            ...
            if missing_models:
                ...
                self.log_result(
                    "Required Models Check", True, "All required models present"
                )
    ...
    async def verify_workflow_manager(self) -> bool:
        ...
        try:
            ...
            self.log_result(
                "WorkflowManager Creation", True, "WorkflowManager created successfully"
            )
            ...
            self.log_result(
                "Default Workflow", True, f"Default workflow: {default_workflow}"
            )
            ...
            self.workflow_manager.create_z_image_turbo_workflow(
                "Test prompt", config, seed=42
            )
            self.log_result(
                "Workflow Creation", True, "Z-Image Turbo workflow created successfully"
            )
    ...
    async def verify_generation_engine(self) -> bool:
        ...
        try:
            ...
            self.generation_engine = GenerationEngine(
                self.connection_manager, self.model_manager, self.workflow_manager
            )
            self.log_result(
                "GenerationEngine Creation",
                True,
                "GenerationEngine created successfully",
            )
            ...
            if backend_ready:
                self.log_result(
                    "Backend Readiness", True, "ComfyUI backend is ready for generation"
                )
            else:
                self.log_result(
                    "Backend Readiness", True, "Backend not ready, will use mock mode"
                )
    ...
    async def verify_all_components(self) -> bool:
        ...
        await self.verify_connection_manager()
        await self.verify_model_manager()
        await self.verify_workflow_manager()
        await self.verify_generation_engine()
