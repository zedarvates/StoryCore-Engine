    ...
    def print_summary(self):
        ...
        if self.updates:
            ...
            for i, update in enumerate(self.updates, 1):
                print(
                    f"  {i}. {update.current_step}: {update.percentage:.1f}% - {update.current_message}"
                )
...
def test_progress_structure():
    ...
    try:
        ...
        assert stage1.session_id == stage2.session_id, (
            "Both stages should share session ID"
        )
...
def test_progress_callback():
    ...
    try:
        ...
        print("✅ Stage separation working:")
        ...
        for i in range(len(tracker.updates) - 1):
            ...
            assert next_update.percentage >= current.percentage, (
        ...
        for i in range(len(tracker.updates) - 1):
            current = tracker.updates[i]
            next_update = tracker.updates[i + 1]
            assert next_update.elapsed_time >= current.elapsed_time, (
                "Elapsed time should increase"
            )
        print("✅ Elapsed time increases correctly")

        # Verify estimated remaining decreases
        for i in range(len(tracker.updates) - 1):
            current = tracker.updates[i]
            next_update = tracker.updates[i + 1]
            assert next_update.estimated_remaining <= current.estimated_remaining, (
                "Remaining time should decrease"
            )
        print("✅ Estimated remaining time decreases correctly")
...
def test_ui_display_format():
    ...
    try:
        ...
        print(
            f"  Progress: {progress.current_item}/{progress.total_items} ({progress.percentage:.1f}%)"
        )
        print(f"  Status: {progress.current_message}")
        print(
            f"  Time: {progress.elapsed_time:.1f}s elapsed, {progress.estimated_remaining:.1f}s remaining"
        )
        ...
        assert progress.percentage >= 0 and progress.percentage <= 100, (
        ...
        print(
            f"Stage Indicators: {stage_indicator} Stage 1 (Latent)  {stage2_indicator} Stage 2 (Upscaling)"
        )
...
def test_stage_transitions():
    ...
    try:
        ...
        assert stage1_end.session_id == stage2_start.session_id, (
            "Session should continue"
        )
        assert stage1_end.current_item < stage2_start.current_item, (
            "Item should increment"
        )
        assert stage1_end.current_step != stage2_start.current_step, (
            "Step should change"
        )
        assert stage1_end.percentage <= stage2_start.percentage, (
