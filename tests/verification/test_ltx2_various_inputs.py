...
def create_illustration_style_image(path: Path, size=(1280, 720)):
    ...
    # Geometric shapes
    draw.rectangle(
        [100, 100, 400, 400], fill=(255, 182, 193), outline=(255, 105, 180), width=5
    )
    draw.ellipse(
        [500, 150, 800, 450], fill=(173, 216, 230), outline=(70, 130, 180), width=5
    )
    draw.polygon(
        [(900, 400), (1100, 100), (1300, 400)],
        fill=(144, 238, 144),
        outline=(34, 139, 34),
        width=5,
    )
...
def test_workflow_with_various_inputs():
    ...
    try:
        ...
        illustration_image = create_illustration_style_image(
            test_dir / "illustration_shapes.png"
        )
        ...
        for i, (image_path, prompt, description) in enumerate(test_cases, 1):
            ...
            try:
                ...
                workflow = workflow_manager.create_ltx2_image_to_video_workflow(
                    str(image_path), prompt, config
                )

                # Verify workflow structure
                assert "98" in workflow, "LoadImage node missing"
                assert workflow["98"]["inputs"]["image"] == str(image_path), (
                    "Image path not set"
                )
                ...
                assert workflow["92:62"]["inputs"]["value"] == 121, (
                    "Frame count not set"
                )
                ...
                print(
                    f"     - Prompt length: {len(workflow['92:3']['inputs']['text'])} chars"
                )
                print(f"     - Frame count: {workflow['92:62']['inputs']['value']}")
                print(
                    f"     - Resolution: {workflow['102']['inputs']['resize_type.width']}x{workflow['102']['inputs']['resize_type.height']}"
                )
        ...
        for frame_count, frame_rate, width, height, description in parameter_tests:
            ...
            print(
                f"  Frames: {frame_count}, FPS: {frame_rate}, Resolution: {width}x{height}"
            )
            ...
            try:
                ...
                workflow = workflow_manager.create_ltx2_image_to_video_workflow(
                    str(photo_image), "Test prompt", config
                )
                ...
                assert abs(duration - expected_duration) < 0.01, (
                    f"Duration mismatch: {duration} vs {expected_duration}"
                )

                print("  ✅ Parameters applied correctly")
                print(
        ...
        prompt_tests = [
            ...
            (
                "Wide establishing shot pulling back to reveal the full scene",
                "Zoom out",
            ),
