    ...
    try:
        ...
        for model in required_models:
            assert model in found_models, (
                f"Required model {model} not found in workflow"
            )
...
def verify_property_61():
    ...
    try:
        ...
        assert stage1_node["class_type"] == "SamplerCustomAdvanced", (
            "Wrong stage 1 node type"
        )
        ...
        assert stage2_node["class_type"] == "SamplerCustomAdvanced", (
            "Wrong stage 2 node type"
        )
...
def verify_property_62():
    ...
    try:
        ...
        workflow = workflow_manager.create_ltx2_image_to_video_workflow(
            "test.png", test_prompt, config
        )

        # Verify prompt was set
        assert workflow["92:3"]["inputs"]["text"] == test_prompt, (
            "Prompt not set correctly"
        )
...
def verify_property_63():
    ...
    try:
        ...
        assert stage1_sigma["class_type"] == "ManualSigmaSchedule", (
            "Wrong sigma schedule type"
        )
        ...
        assert stage2_sigma["class_type"] == "ManualSigmaSchedule", (
            "Wrong sigma schedule type"
        )
...
def verify_property_66():
    ...
    try:
        ...
        print(
            f"   Progress: {stage1.current_item}/{stage1.total_items} ({stage1.percentage}%)"
        )

        print("✅ Stage 2 progress display configured")
        print(f"   Step: {stage2.current_step}")
        print(
            f"   Progress: {stage2.current_item}/{stage2.total_items} ({stage2.percentage}%)"
        )
