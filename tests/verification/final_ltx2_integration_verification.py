...
from end_to_end.workflow_manager import (
    WorkflowManager,
    LTX2ImageToVideoConfig,
    ZImageTurboConfig,
)
...
def verify_complete_pipeline():
    ...
    try:
        ...
        z_workflow = workflow_manager.create_z_image_turbo_workflow(
            prompt="A beautiful mountain landscape at sunset", config=z_config, seed=42
        )
        ...
        assert ltx2_workflow["92:48"]["class_type"] == "LTXVAudioVAELoader", (
            "Wrong audio loader type"
        )
...
def verify_workflow_switching():
    ...
    try:
        ...
        assert default_workflow == "z_image_turbo_generation", (
            "Z-Image Turbo should be default"
        )
        ...
        assert "ltx2_image_to_video" in video_workflows, (
            "LTX-2 should be available for video"
        )
        ...
        z_workflow = workflow_manager.create_z_image_turbo_workflow(
            "test prompt", ZImageTurboConfig(), 0
        )
        ltx2_workflow = workflow_manager.create_ltx2_image_to_video_workflow(
            "test.png",
            "test prompt",
            LTX2ImageToVideoConfig(input_image_path="test.png"),
        )
    ...
    try:
        ...
        try:
            config = LTX2ImageToVideoConfig(
                input_image_path="nonexistent_image.png", frame_count=121, frame_rate=25
            )
            print(
                "✅ Configuration accepts invalid path (validation happens at generation time)"
            )
        except Exception as e:
        ...
        try:
            ...
            print(
                "⚠️  Warning: Negative frame count accepted (validation may be needed)"
            )
        except Exception as e:
        ...
        config = ComfyUIConfig(
            host="localhost", port=8000, fallback_mode=FallbackMode.PLACEHOLDER
        )
        ...
        print("   Backend availability check works")
...
def verify_backend_availability_checks():
    ...
    try:
        ...
        config = ComfyUIConfig(host="localhost", port=8000, timeout=5)
        ...
        assert hasattr(status, "workflows_ready"), (
            "Status should have workflows_ready flag"
        )
...
def main():
    ...
    results.append(
        ("Backend Availability Checks", verify_backend_availability_checks())
    )
