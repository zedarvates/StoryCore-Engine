...
def temp_workflow_file():
    ...
    workflow_data = {"nodes": [], "links": [], "version": "1.0"}
...
class TestModelInstallation:
    ...
    def test_install_vae_model_to_correct_directory(self, temp_comfyui_dir):
        ...
        try:
            # Install VAE model
            models = [str(vae_file)]
            installed, errors = installer._install_models(
                temp_comfyui_dir, models, None
            )
    ...
    def test_install_multiple_models_with_failures(
        self, temp_comfyui_dir, temp_model_file
    ):
    ...
    def test_install_models_with_progress_callback(
        self, temp_comfyui_dir, temp_model_file
    ):
        ...
        models = [str(temp_model_file)]
        installed, errors = installer._install_models(
            temp_comfyui_dir, models, progress_callback
        )

        # Verify progress updates were received
        assert len(progress_updates) > 0
        assert any(
            "Installing model" in update["message"] for update in progress_updates
        )
...
class TestWorkflowInstallation:
    ...
    def test_install_invalid_json_workflow_fails(self, temp_comfyui_dir):
        ...
        try:
            ...
            installed, errors = installer._install_workflows(
                temp_comfyui_dir, workflows
            )
    ...
    def test_install_multiple_workflows_with_failures(
        self, temp_comfyui_dir, temp_workflow_file
    ):
    ...
    def test_verify_installed_workflows_exist(
        self, temp_comfyui_dir, temp_workflow_file
    ):
...
class TestIntegration:
    ...
    def test_complete_installation_with_models_and_workflows(
        self, temp_model_file, temp_workflow_file
    ):
