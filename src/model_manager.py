...
from .ai_enhancement_engine import ModelInfo, ModelType, ModelConfig, GPUResourceStatus
            ...
            if self.registry_file.exists():
                ...
                for model_id, model_data in registry_data.items():
                    self.models[model_id] = ModelInfo(
                        ...
                        performance_characteristics=model_data[
...
class GPUMemoryManager:
    ...
    def get_gpu_status(self) -> GPUResourceStatus:
        ...
        with self._lock:
            ...
            utilization = (
                (allocated_memory / total_memory) * 100 if total_memory > 0 else 0
            )
        ...
        status = GPUResourceStatus(
            ...
            temperature=65.0
            + (utilization * 0.2),  # Temperature increases with utilization
    ...
    def allocate_memory(self, allocation_id: str, memory_mb: float) -> bool:
        ...
        with self._lock:
            if self.can_allocate(memory_mb):
                ...
                self.logger.debug(
                    f"Allocated {memory_mb}MB GPU memory for {allocation_id}"
                )
                return True
            return False
    ...
    def deallocate_memory(self, allocation_id: str):
        ...
        with self._lock:
            if allocation_id in self.memory_allocations:
                ...
                self.logger.debug(
                    f"Deallocated {memory_mb}MB GPU memory for {allocation_id}"
                )
    ...
    def get_memory_usage_report(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            largest_allocation_id = (
                max(allocations.items(), key=lambda x: x[1])[0] if allocations else None
            )
    ...
    def optimize_memory_usage(self) -> Dict[str, Any]:
        ...
        if report["utilization_percent"] > 80:
            recommendations.append(
                "GPU memory usage is high (>80%). Consider unloading unused models."
            )

        # Check for inefficient allocations
        if report["active_allocations"] > 5:
            recommendations.append(
                "Many active allocations detected. Consider model consolidation."
            )

        # Check temperature
        if report["temperature"] > 80:
            recommendations.append(
                "GPU temperature is high. Consider reducing workload or improving cooling."
            )
            ...
            with self._lock:
                if len(self.memory_allocations) > 1:
                    ...
                    actions_taken.append(
                        "Automatic memory cleanup triggered due to critical usage"
                    )
    ...
    def force_memory_cleanup(self) -> Dict[str, Any]:
        ...
        if current_time - self.last_optimization_time < self.optimization_cooldown:
            remaining_cooldown = self.optimization_cooldown - (
        ...
        with self._lock:
            if self.memory_allocations:
                ...
                if memory_freed > 0:
                    actions_taken.append(
                        f"Memory defragmentation freed {memory_freed:.1f}MB"
                    )
        ...
        optimization_record = {
            ...
            "memory_freed_mb": status_before.total_memory
            - status_before.available_memory
            - (status_after.total_memory - status_after.available_memory),
            "actions_taken": actions_taken,
        }
        ...
        return {
            ...
            "utilization_improvement": status_before.utilization_percent
            - status_after.utilization_percent,
        }
    ...
    def predict_memory_exhaustion(self) -> Dict[str, Any]:
        ...
        recent_utilizations = [
            entry["utilization_percent"] for entry in self.memory_history[-10:]
        ]
...
class ModelManager:
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="ModelLoader"
        )
    ...
    async def load_model(self, model_id: str, device: str = "auto") -> ModelLoadResult:
        ...
        try:
            ...
            if device == "auto":
                device = self.gpu_memory_manager.get_optimal_device(
                    model_info.gpu_memory_required
                )
            ...
            if load_result.success:
                ...
                self.logger.info(
                    f"Model {model_id} loaded successfully on {load_result.device}"
                )
            else:
                self.load_stats["failed_loads"] += 1
                self.logger.error(
                    f"Failed to load model {model_id}: {load_result.error_message}"
                )
        ...
        try:
            ...
            if model_info.file_path and Path(model_info.file_path).exists():
                ...
                if device == "cuda":
                    ...
                    if not self.gpu_memory_manager.allocate_memory(
                        allocation_id, model_info.gpu_memory_required
                    ):
                        # GPU allocation failed, check if CPU fallback is enabled
                        if self.config.cpu_fallback_enabled:
                            device = "cpu"
                            self.logger.warning(
                                f"GPU allocation failed for {model_info.model_id}, falling back to CPU"
                            )
                            self.load_stats["cpu_fallback_loads"] += 1
                        else:
                ...
                if device == "cpu":
                    ...
                    memory_used = (
                        model_info.size_mb * 0.7
                    )  # Estimate 30% less memory on CPU
                ...
                self.logger.info(
                    f"Downloading model {model_info.model_id} from {model_info.download_url}"
                )
                ...
                if model_info.file_path:
                    ...
                    self.logger.info(
                        f"Model {model_info.model_id} downloaded and saved to {model_path}"
                    )
            ...
            load_time = (time.time() - start_time) * 1000
            return ModelLoadResult(
                success=False, error_message=str(e), load_time_ms=load_time
            )
    ...
    def get_cache_status(self) -> Dict[str, Any]:
        ...
        with self.cache_lock:
            total_memory = sum(
                entry.memory_used_mb for entry in self.model_cache.values()
            )
            cache_entries = []
    ...
    async def _ensure_cache_space(self, required_mb: float):
        ...
        with self.cache_lock:
            ...
            if len(self.model_cache) >= self.config.model_cache_size:
                ...
                self.logger.info(
                    f"Evicted LRU model {lru_model_id} to make cache space"
                )
    ...
    def optimize_model_loading(self) -> Dict[str, Any]:
        ...
        gpu_ratio = self.load_stats["gpu_loads"] / max(
            1, self.load_stats["successful_loads"]
        )
        ...
        if self.load_stats["successful_loads"] > 0:
            avg_load_time = (
                self.load_stats["total_load_time"] / self.load_stats["successful_loads"]
            )
        ...
        return {
            ...
            "average_load_time_ms": self.load_stats["total_load_time"]
            / max(1, self.load_stats["successful_loads"]),
            "recommendations": recommendations,
    ...
    async def optimize_gpu_memory(self) -> Dict[str, Any]:
        ...
        if active_alerts:
            actions_taken.append(
                f"Responding to {len(active_alerts)} active GPU alerts"
            )

        # If GPU memory is critically low, unload least recently used models
        current_utilization = optimization_report["current_status"][
            "utilization_percent"
        ]
        ...
        if current_utilization > 90:
            with self.cache_lock:
                ...
                sorted_models = sorted(
                    self.model_cache.items(), key=lambda x: x[1].last_accessed
                )
                ...
                for model_id, entry in sorted_models:
                    ...
                    if await self.unload_model(model_id):
                        ...
                        actions_taken.append(
                            f"Unloaded model {model_id} (freed {entry.memory_used_mb:.1f}MB)"
                        )

                if models_unloaded > 0:
                    actions_taken.append(
                        f"Total: {models_unloaded} models unloaded, {memory_freed:.1f}MB freed"
                    )
    ...
    async def _optimize_model_device_placement(self):
        ...
        with self.cache_lock:
            ...
            if gpu_status.available_memory > 500 and cpu_models:  # 500MB threshold
                ...
                for model_id, entry in cpu_models[
                    :1
                ]:  # Move top 1 CPU model to GPU if possible
                    model_info = entry.model_info
                    if self.gpu_memory_manager.can_allocate(
                        model_info.gpu_memory_required
                    ):
                        # In a real implementation, we'd reload the model on GPU
                        # For now, we'll just update the tracking
                        self.logger.info(
                            f"Model {model_id} could benefit from GPU placement"
                        )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = self.load_stats["successful_loads"] / max(
            1, self.load_stats["total_loads"]
        )

        avg_load_time = self.load_stats["total_load_time"] / max(
            1, self.load_stats["successful_loads"]
        )

        # Calculate GPU efficiency score
        gpu_efficiency = self.gpu_memory_manager._calculate_optimization_score(
            gpu_report
        )
        ...
        return {
            ...
            "cache_metrics": {
                ...
                "cache_utilization": cache_status["cached_models"]
            ...
            "gpu_metrics": {
                ...
                "memory_trend": memory_trends.get("utilization", {}).get(
                    "trend", "unknown"
                ),
                "monitoring_enabled": self.gpu_memory_manager.monitoring_enabled,
            },
            "performance_metrics": {
                ...
                "gpu_load_ratio": self.load_stats["gpu_loads"]
                / max(1, self.load_stats["successful_loads"]),
                "cpu_fallback_rate": self.load_stats["cpu_fallback_loads"]
                / max(1, self.load_stats["successful_loads"]),
            },
            "load_statistics": self.load_stats.copy(),
            "alerts": {
