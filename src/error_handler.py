...
from .comfyui_models import PerformanceMetrics
...
class ErrorHandler:
    ...
    def _setup_fallback_strategies(self) -> Dict[ErrorCategory, List[FallbackStrategy]]:
        ...
        strategies = {
            ...
            ErrorCategory.WORKFLOW: [
                ...
                FallbackStrategy(
                    ...
                    conditions=[
                        lambda e: True
                    ],  # Always fallback to mock for workflow errors
                ),
            ],
            ErrorCategory.RESOURCE: [
                FallbackStrategy(
                    ...
                    conditions=[
                        lambda e: (
                            "memory" in e.error_message.lower()
                            or "vram" in e.error_message.lower()
                        )
                    ],
                ),
                FallbackStrategy(
        ...
        strategies[ErrorCategory.UNKNOWN] = [
            FallbackStrategy(mode=FallbackMode.RETRY, max_retries=1, retry_delay=2.0),
            FallbackStrategy(mode=FallbackMode.MOCK, conditions=[lambda e: True]),
        ]
    ...
    def analyze_error(
        ...
        try:
            ...
            self._error_patterns[error_pattern] = (
                self._error_patterns.get(error_pattern, 0) + 1
            )
    ...
    def _classify_error(
        ...
        if any(
            ...
            for keyword in ["connection", "timeout", "http", "socket"]
        ):
        ...
        if any(
            keyword in error_msg
            for keyword in ["service", "startup", "shutdown", "process", "pid", "port"]
        ...
        ) or any(keyword in error_type for keyword in ["validation", "config"]):
        ...
        if any(
            keyword in error_msg
            for keyword in ["system", "os", "platform", "permission", "access"]
        ) or any(keyword in error_type for keyword in ["os", "system", "permission"]):
    ...
    def _assess_severity(
        ...
        if any(
            keyword in error_msg
            for keyword in ["critical", "fatal", "crash", "abort", "corrupt"]
        ) or error_type in ["SystemExit", "KeyboardInterrupt"]:
            return ErrorSeverity.CRITICAL

        # High severity conditions
        if category in [ErrorCategory.SERVICE, ErrorCategory.SYSTEM] or any(
            keyword in error_msg
            for keyword in [
                "failed to start",
                "cannot connect",
        ...
        if category == ErrorCategory.CONFIGURATION or any(
            keyword in error_msg
            for keyword in ["warning", "deprecated", "minor", "temporary"]
        ):
    ...
    def _generate_suggestions(
        ...
        if category == ErrorCategory.NETWORK:
            ...
            if "port" in error_msg:
                suggestions.append(
                    "Try using a different port or stop conflicting services"
                )
    ...
    async def handle_error(
        ...
        for strategy in strategies:
            if strategy.should_apply(error_info):
                ...
                fallback_mode = await self._apply_fallback_strategy(
                    strategy, error_info
                )
                ...
                if fallback_mode:
                    ...
                    for callback in self._recovery_callbacks:
                        try:
                            ...
                            self.logger.error(
                                f"Recovery callback error: {callback_error}"
                            )

                    return fallback_mode

        self.logger.warning(
            f"No applicable fallback strategy found for error {error_info.error_id}"
        )
        return None
        ...
        try:
            if strategy.mode == FallbackMode.RETRY:
                if strategy.can_retry():
                    ...
                    if delay > 0:
                        self.logger.info(
                            f"Retrying in {delay:.1f} seconds (attempt {strategy.retry_count + 1}/{strategy.max_retries})"
                        )
                    ...
                    self.logger.warning(
                        "Maximum retry attempts reached, trying next strategy"
                    )
                    return None
            ...
            self.logger.error(
                f"Failed to apply fallback strategy {strategy.mode.value}: {e}"
            )
            return None

    def add_recovery_callback(
        self, callback: Callable[[ErrorInfo, FallbackMode], None]
    ) -> None:
        """
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_fallback_mode": self._current_fallback_mode.value
            if self._current_fallback_mode
            else None,
            "last_error": self._error_history[-1].to_dict()
    ...
    def get_diagnostic_info(self) -> Dict[str, Any]:
        ...
        return {
            "error_handler_status": {
                "fallback_active": self._fallback_active,
                "current_mode": self._current_fallback_mode.value
                if self._current_fallback_mode
                else None,
                "total_errors": len(self._error_history),
