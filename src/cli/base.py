...
class BaseHandler(ABC):
        ...
        try:
            ...
            if "project_name" not in project_data:
                raise ConfigurationError(
                    "Project file missing required 'project_name' field"
                )
    ...
    def log_command_execution(self, args: argparse.Namespace, success: bool) -> None:
        ...
        if success:
            self.logger.debug(
                f"Command '{self.command_name}' completed in {duration:.2f}s"
            )
        else:
            self.logger.error(
                f"Command '{self.command_name}' failed after {duration:.2f}s"
            )
