...
class CommandRegistry:
        ...
        try:
            ...
            for module_info in pkgutil.iter_modules([str(handlers_path)]):
                ...
                if self.lazy_load:
                    ...
                    self.logger.debug(
                        f"Discovered handler module: {command_name} -> {module_name}"
                    )
                else:
                    ...
                    try:
                        ...
                        for attr_name in dir(module):
                            ...
                            if (
                                ...
                                and hasattr(attr, "command_name")
                                and hasattr(attr, "execute")
                            ):
                                handler_classes.append(attr)
                                self.logger.debug(
                                    f"Discovered handler: {attr.command_name}"
                                )

                    except Exception as e:
                        self.logger.warning(
                            f"Failed to load handler module {module_info.name}: {e}"
                        )
                        continue
    ...
    def _load_handler_class(self, command_name: str) -> Optional[Type[BaseHandler]]:
        ...
        try:
            # Import the module
            self.logger.debug(
                f"Attempting to load module {module_name} for {command_name}"
            )
            module = importlib.import_module(module_name)
            ...
            for attr_name in dir(module):
                ...
                if isinstance(attr, type) and hasattr(attr, "command_name"):
                    if hasattr(attr, "execute") and attr.command_name == command_name:
                        self.logger.debug(f"Lazy loaded handler: {command_name}")
                        return attr

            self.logger.warning(
                f"No handler class found in module {module_name} with command_name='{command_name}'"
            )
            return None

        except Exception as e:
            self.logger.error(
                f"Failed to lazy load handler {command_name} from {module_name}: {e}",
                exc_info=True,
            )
            return None
    ...
    def register_handler(self, handler_class: Type[BaseHandler]) -> None:
        ...
        try:
            ...
            if not hasattr(handler_class, "command_name"):
                raise SystemError(
                    f"Handler {handler_class.__name__} missing command_name attribute"
                )

            if not hasattr(handler_class, "description"):
                raise SystemError(
                    f"Handler {handler_class.__name__} missing description attribute"
                )
            ...
            if command_name in self.handler_classes:
                self.logger.warning(
                    f"Duplicate handler for command '{command_name}', skipping"
                )
                return
    ...
    def register_all_handlers(self) -> None:
        ...
        if self.lazy_load:
            ...
            self.logger.info(
                f"Registered {len(self.handler_modules)} command handlers (lazy mode)"
            )
        else:
            ...
            for handler_class in handler_classes:
                try:
                    ...
                    self.logger.error(
                        f"Failed to register {handler_class.__name__}: {e}"
                    )
                    continue

            self.logger.info(
                f"Registered {len(self.handlers)} command handlers (eager mode)"
            )
        ...
        for command_name, handler in self.handlers.items():
            try:
                ...
                if not hasattr(handler, "setup_parser"):
                    self.logger.error(
                        f"Handler {command_name} missing setup_parser method"
                    )
                    all_valid = False
    ...
    def register_external_handler(self, handler_class: Type[BaseHandler]) -> bool:
        ...
        try:
            self.register_handler(handler_class)
            self.logger.info(
                f"Registered external handler: {handler_class.command_name}"
            )
            return True
        except Exception as e:
    ...
    def unregister_handler(self, command_name: str) -> bool:
        ...
        try:
            ...
            aliases_to_remove = [
                alias for alias, cmd in self.aliases.items() if cmd == command_name
            ]
    ...
    def get_command_info(self, command: str) -> Optional[Dict[str, Any]]:
        ...
        if (
            actual_command not in self.handler_classes
            and actual_command not in self.handler_modules
        ):
            return None
        ...
        info = {
            ...
            "aliases": [
                alias for alias, cmd in self.aliases.items() if cmd == actual_command
            ],
