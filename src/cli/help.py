...
class CommandHelp:
    ...
    def add_example(self, command: str, description: str) -> "CommandHelp":
        ...
        Returns:
            Self for chaining
        """
        self.examples.append({"command": command, "description": description})
        return self
        ...
        if self.notes:
            ...
            for note in self.notes:
                wrapped = textwrap.fill(
                    note, width=70, initial_indent="  • ", subsequent_indent="    "
                )
...
def format_command_list(
    commands: Dict[str, str], title: str = "Available Commands"
) -> str:
    """
...
def generate_shell_completion_fish(commands: List[str]) -> str:
    ...
    for command in commands:
        lines.append(
            f"complete -c storycore -n '__fish_use_subcommand' -a {command} -d 'Execute {command} command'"
        )
