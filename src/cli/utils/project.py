        ...
        for field in required_fields:
            if field not in project_data:
                raise ConfigurationError(
                    f"Project file missing required '{field}' field"
                )
...
def get_project_metadata(project_path: str) -> Dict[str, Any]:
    ...
    for dir_name in key_dirs:
        ...
        if dir_path.exists():
            ...
            metadata["directories"][dir_name] = {"file_count": 0, "size": 0}
...
def save_project_config(project_path: str, config: Dict[str, Any]) -> None:
    ...
    for field in required_fields:
        if field not in config:
            raise ConfigurationError(
                f"Project configuration missing required '{field}' field"
            )
...
def validate_project_config_schema(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    required_fields = {"project_name": str, "schema_version": str}
