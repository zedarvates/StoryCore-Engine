...
def validate_choice(value: str, choices: List[str]) -> str:
    ...
    if value not in choices:
        raise UserError(
            f"Invalid choice: {value}", f"Must be one of: {', '.join(choices)}"
        )
    return value
...
def validate_scale_factor(scale: int) -> int:
    ...
    if scale < 1 or scale > 8:
        raise UserError(
            f"Invalid scale factor: {scale}", "Scale factor must be between 1 and 8"
        )
...
def validate_directory_exists(path: str) -> Path:
    ...
    if not dir_path.exists():
        raise UserError(
            f"Directory not found: {path}", "Check the path or create the directory"
        )

    if not dir_path.is_dir():
        raise UserError(
            f"Path is not a directory: {path}", "Provide a valid directory path"
        )
...
def validate_file_exists(path: str) -> Path:
    ...
    if not file_path.exists():
        raise UserError(f"File not found: {path}", "Check the file path")

    if not file_path.is_file():
        raise UserError(f"Path is not a file: {path}", "Provide a valid file path")
...
def validate_json_string(json_str: str) -> dict:
    ...
    try:
        ...
        raise UserError(f"Invalid JSON: {e}", "Check the JSON syntax")
...
def validate_regex_pattern(pattern: str) -> re.Pattern:
    ...
    try:
        ...
        raise UserError(
            f"Invalid regex pattern: {e}", "Check the regular expression syntax"
        )
...
def validate_non_empty_string(value: str, name: str = "Value") -> str:
    ...
    if not value or not value.strip():
        raise UserError(f"{name} cannot be empty", f"Provide a valid {name.lower()}")
...
def validate_email(email: str) -> str:
    ...
    if not email_pattern.match(email):
        raise UserError(
            f"Invalid email format: {email}", "Provide a valid email address"
        )
...
def validate_percentage(value: float) -> float:
    ...
    if value < 0 or value > 100:
        raise UserError(
            f"Invalid percentage: {value}", "Percentage must be between 0 and 100"
        )
