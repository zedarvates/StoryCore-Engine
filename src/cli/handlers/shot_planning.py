...
class ShotPlanningHandler(BaseHandler):
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(
                f"  Average shot duration: {result['processing_metadata']['average_shot_duration']:.1f} seconds"
            )
            print(
                f"  Shot variety score: {result['processing_metadata']['shot_variety_score']:.1f}/5.0"
            )
            print(
                f"  Camera complexity: {result['processing_metadata']['camera_complexity_score']:.1f}/5.0"
            )
            ...
            if result.get("cinematic_grammar"):
                ...
                print(
                    f"    Coverage complete: {grammar['coverage_completeness']['coverage_complete']}"
                )
                print(
                    f"    Visual rhythm: {grammar['visual_rhythm']['rhythm']} ({grammar['visual_rhythm']['tempo']} tempo)"
                )

            # Show shot type distribution
            if result.get("cinematic_grammar", {}).get("shot_type_distribution"):
                print("\n  Shot type distribution:")
                for shot_type, percentage in result["cinematic_grammar"][
            ...
            if args.camera_specs and result.get("camera_specifications"):
                ...
                print("\n  Camera requirements:")
                print(
                    f"    Movement complexity: {specs['movement_requirements']['complexity_level']}"
                )
                print(f"    Primary lens: {specs['lens_requirements']['primary_lens']}")
                print(
                    f"    Equipment needed: {', '.join(specs['movement_requirements']['equipment_needed'])}"
                )

            print("\n  Shot planning saved: shot_planning.json")
            print("  Updated project.json with shot planning results")
