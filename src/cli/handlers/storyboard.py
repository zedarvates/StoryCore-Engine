...
class StoryboardHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--update", action="store_true", help="Update existing storyboard"
        )

        parser.add_argument(
            "--validate", action="store_true", help="Validate storyboard structure"
        )

        parser.add_argument(
            "--shots", type=int, help="Number of shots to generate (optional)"
        )
    ...
    def _generate_storyboard(self, project_path: Path, args: argparse.Namespace) -> int:
        ...
        print("  Storyboard saved: storyboard.json")
    ...
    def _validate_storyboard(self, project_path: Path, args: argparse.Namespace) -> int:
        ...
        if result["valid"]:
            ...
            self.print_error(
                f"Storyboard validation failed: {len(result['errors'])} errors"
            )
            for error in result["errors"]:
                print(f"  - {error}")
            return 1

    def _show_storyboard_info(
        self, project_path: Path, args: argparse.Namespace
    ) -> int:
