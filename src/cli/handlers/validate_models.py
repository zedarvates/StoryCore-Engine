...
class ValidateModelsHandler(BaseHandler):
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            return asyncio.run(self._validate_models_async(models_dir, args))

        except Exception as e:
            return self.handle_error(e, "Model validation")

    async def _validate_models_async(
        self, models_dir: Path, args: argparse.Namespace
    ) -> int:
        ...
        models_to_validate = [
            m for m in models_to_validate if model_manager._get_model_path(m).exists()
        ]
        ...
        for model_info in models_to_validate:
            ...
            if not size_match:
                print(
                    f"    ⚠️  Size mismatch: expected {expected_size}, got {actual_size}"
                )
            ...
            if model_info.sha256_hash:
                ...
                is_valid = await model_manager.validate_model(
                    model_path, model_info.sha256_hash
                )
                ...
                print("    ⚠️  No hash provided, skipping hash validation")
                if size_match:
