...
from src.wizard.comic_to_sequence_wizard import get_transformation_preview
...
class ComicToSequenceWizardHandler(BaseHandler):
    ...
    description = (
        "Transform comic book panel images into professional cinematic sequences"
    )

    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        """Set up comic-to-sequence-wizard command arguments."""
        parser.add_argument("image_path", help="Path to the comic panel image file")
        ...
        parser.add_argument("--title", help="Comic title (optional)")

        parser.add_argument(
            "--page", type=int, default=1, help="Page number in the comic (default: 1)"
        )
    ...
    def _execute_preview_mode(self, image_path: Path) -> int:
        ...
        print("   • Use high-contrast images for better panel detection")
        print("   • Ensure text is clearly readable in panels")
        print("   • Try different styles if results seem inaccurate")
    ...
    def _display_summary_format(self, result) -> int:
        ...
        print(f"   Layout: {sequence.layout_type.value.replace('_', ' ').title()}")
        print(
            f"   Mood: {sequence.overall_mood.title() if sequence.overall_mood else 'Neutral'}"
        )
        ...
        print(
            f"   Shot types: {', '.join(f'{count}x {shot_type}' for shot_type, count in shot_types.items())}"
        )
        ...
        if result.cinematic_shots:
            ...
            for i, shot in enumerate(result.cinematic_shots[:3], 1):
                print(
                    f"   {i}. {shot.shot_type} ({shot.duration_seconds:.1f}s) - {shot.description[:50]}..."
                )
    ...
    def _display_detailed_format(self, result) -> int:
        ...
        if sequence.panels:
            ...
            for i, panel in enumerate(sequence.panels, 1):
                print(
                    f"   {i}. Panel {panel.panel_number}: {panel.content_description}"
                )
                if panel.characters_present:
                    print(f"      Characters: {', '.join(panel.characters_present)}")
                if panel.camera_angle:
                    print(
                        f"      Camera: {panel.camera_angle.value.replace('_', ' ').title()}"
                    )
        ...
        if result.storyboard_panels:
            ...
            for i, panel in enumerate(result.storyboard_panels, 1):
                print(
                    f"   {i}. Shot {panel['shot_number']}: {panel['description'][:60]}..."
                )
                print(
                    f"      Duration: {panel['duration']:.1f}s, Camera: {panel['camera_angle']}"
                )

        self._display_generated_assets(result)

        print("\n✅ Comic to sequence transformation completed!")
        print("   Results saved to project directory for use in video production.")
