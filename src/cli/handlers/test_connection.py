...
class TestConnectionHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--check-models", action="store_true", help="Check for required models"
        )
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            try:
                from end_to_end.connection_manager import (
                    ConnectionManager,
                    ComfyUIConfig,
                )
            ...
            return asyncio.run(self._test_connection_async(config, args))

        except Exception as e:
            return self.handle_error(e, "Connection test")

    async def _test_connection_async(self, config, args: argparse.Namespace) -> int:
        ...
        print(
            "║                    ComfyUI Desktop Connection Test                            ║"
        )
        print(
            "╚══════════════════════════════════════════════════════════════════════════════╝"
        )
        ...
        if status.available:
            self.print_success("Connected to ComfyUI Desktop")
            print(f"  Version: {status.version or 'Unknown'}")
            print(f"  Queue Size: {status.queue_size}")
            print()
        else:
            self.print_error("Failed to connect to ComfyUI Desktop")
        ...
        if args.check_cors:
            ...
            if cors_result.valid:
                ...
                if args.verbose:
                    print(
                        f"  Headers present: {', '.join(cors_result.headers_present)}"
                    )
                print()
            else:
        ...
        if args.check_models:
            ...
            if not comfyui_models_dir.exists():
                self.print_warning(
                    f"ComfyUI models directory not found: {comfyui_models_dir}"
                )
                ...
                if not missing_models:
                    ...
                    self.print_warning(
                        f"{len(missing_models)} required models are missing"
                    )
                    print()
                    ...
                    print()
                    print(
                        "  Run 'python storycore.py download-models' to download missing models"
                    )
                    print()
        ...
        if args.check_workflows:
            ...
            if not workflows_dir.exists():
                self.print_warning(
                    f"StoryCore workflows directory not found: {workflows_dir}"
                )
                print("  Skipping workflow check")
                print()
            elif not comfyui_workflows_dir.exists():
                self.print_warning(
                    f"ComfyUI workflows directory not found: {comfyui_workflows_dir}"
                )
                print("  Skipping workflow check")
                print()
            else:
                ...
                outdated_workflows = [
                    w for w in installed_workflows if w.installed and not w.up_to_date
                ]

                if not missing_workflows and not outdated_workflows:
                    self.print_success(
                        "All required workflows are installed and up-to-date"
                    )
                    print()
                else:
                    if missing_workflows:
                        self.print_warning(
                            f"{len(missing_workflows)} required workflows are missing"
                        )
                        print()
                    ...
                    if outdated_workflows:
                        self.print_warning(
                            f"{len(outdated_workflows)} workflows need updates"
                        )
                        print()
                    ...
                    print(
                        "  Run 'python storycore.py deploy-workflows' to install/update workflows"
                    )
                    print()
