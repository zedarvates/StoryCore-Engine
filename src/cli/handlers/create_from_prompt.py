...
class CreateFromPromptHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "prompt", nargs="*", help="Natural language prompt describing the project"
        )
    ...
    async def _execute_workflow(self, args: argparse.Namespace, prompt: str) -> int:
        ...
        def progress_callback(report: ProgressReport):
            ...
            if args.verbose:
                ...
                # Simple progress
                status = "#" * int(report.progress_percent / 10) + "-" * (
                    10 - int(report.progress_percent / 10)
                )
                print(
                    f"\r  Progress: [{status}] {report.progress_percent:5.1f}% ",
                    end="",
                    flush=True,
                )
    ...
    def _execute_resume(self, args: argparse.Namespace) -> int:
        ...
        if orchestrator.resume(str(project_path)):
            ...
            print(
                f"  Starting from step: {orchestrator.get_workflow_state().current_step.value}"
            )
