...
class GenerateBoxSceneHandler(BaseHandler):
    ...
    description = (
        "Generate a Blender script for a simple 3D box layout (room, corridor)"
    )
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--name", type=str, default="location", help="Name of the generated object"
        )
        parser.add_argument(
            "--dimensions",
            type=float,
        ...
        parser.add_argument(
            "--width", type=float, default=5.0, help="Width of the space (default: 5.0)"
        )
        parser.add_argument(
            "--depth", type=float, default=5.0, help="Depth of the space (default: 5.0)"
        )
        parser.add_argument(
            "--height",
            type=float,
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if (
                hasattr(args, "dimensions")
                and args.dimensions
                and len(args.dimensions) == 3
            ):
                # [width, height, depth] based on common convention
                width, height, depth = args.dimensions
            else:
                width, height, depth = (
                    getattr(args, "width", 5.0),
                    getattr(args, "height", 2.5),
                    getattr(args, "depth", 5.0),
                )
            ...
            script = BoxSceneGenerator.get_blender_script(
                scene_type=args.scene_type, width=width, depth=depth, height=height
            )
