...
class PromoteHandler(BaseHandler):
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if args.scale > 4:
                self.print_warning(
                    f"Large scale factor ({args.scale}x) may result in very large files"
                )
            ...
            if args.mock:
                ...
                print("Backend mode: Auto (will use ComfyUI if available)")
            ...
            result = promote_panels(
                project_path, args.scale, args.method, **backend_config
            )
            ...
            self.print_success(
                f"Promoted {result['metadata']['total_panels']} panels successfully"
            )

            # Show resolution changes
            for i, (original, promoted) in enumerate(result["resolutions"], 1):
                print(
                    f"  Panel {i:02d}: {original[0]}x{original[1]} -> {promoted[0]}x{promoted[1]}"
                )
            ...
            print("  Updated project.json asset manifest")
            print("  Project status: promoted")
