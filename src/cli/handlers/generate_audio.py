...
class GenerateAudioHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--workflow", help="ComfyUI workflow file to use for audio generation"
        )
        ...
        parser.add_argument("--voice", help="Voice ID or name for dialogue generation")

        parser.add_argument("--music-style", help="Music style for music generation")
        ...
        try:
            ...
            if result.get("generated_audio"):
                ...
                for audio in result["generated_audio"][:5]:
                    print(
                        f"    {audio['shot_id']}: {audio['path']} ({audio['duration']:.1f}s)"
                    )
