...
class ListWorkflowsHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--verbose", action="store_true", help="Show detailed workflow information"
        )
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not workflows_dir.exists():
                self.print_warning(
                    f"Source workflows directory does not exist: {workflows_dir}"
                )
                print()
                return 1
            ...
            if not comfyui_workflows_dir.exists():
                self.print_warning(
                    f"Target workflows directory does not exist: {comfyui_workflows_dir}"
                )
                print()
            ...
            for workflow_info in installed_workflows:
                ...
                if args.verbose:
                    ...
                    print(
                        f"    Required Nodes: {', '.join(workflow_info.required_nodes)}"
                    )
                    print(
                        f"    Required Models: {', '.join(workflow_info.required_models)}"
                    )
            ...
            if missing_count > 0 or outdated_count > 0:
                self.print_warning(
                    f"{missing_count + outdated_count} workflow(s) need deployment"
                )
                print(
                    "  Run 'python storycore.py deploy-workflows' to deploy/update workflows"
                )
            else:
