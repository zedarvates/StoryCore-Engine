...
class GenerateVideoHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--workflow", help="ComfyUI workflow file to use for video generation"
        )

        parser.add_argument(
            "--shots", nargs="+", help="Specific shot IDs to generate (default: all)"
        )
        ...
        parser.add_argument(
            "--duration", type=float, help="Video duration in seconds (optional)"
        )
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if args.fps <= 0:
                raise UserError(
                    f"Invalid FPS value: {args.fps}", "FPS must be a positive integer"
                )
            ...
            engine = ComfyUIVideoEngine(
                comfyui_url=args.comfyui_url, mock_mode=args.mock
            )
            ...
            if result.get("generated_videos"):
                ...
                for video in result["generated_videos"][:5]:
                    print(
                        f"    {video['shot_id']}: {video['path']} ({video['duration']:.1f}s)"
                    )
            ...
            print("  Updated project.json with generation metadata")
