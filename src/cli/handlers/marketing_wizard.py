...
from src.wizard.marketing_wizard import MarketingCampaign, get_campaign_preview
...
class MarketingWizardHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--title", help="Campaign title (auto-generated if not provided)"
        )
        ...
        try:
            ...
            campaign = asyncio.run(
                wizard.create_marketing_campaign(
                    project_path, args.title, target_platforms, args.strategy
                )
            )
    ...
    def _execute_preview_mode(
        self, project_path: Path, args: argparse.ArgumentParser
    ) -> int:
        ...
        print(
            f"🎯 Estimated marketing assets: {preview.get('estimated_assets', 'N/A')}"
        )
        print(
            f"🎨 Recommended viral strategy: {preview.get('recommended_strategy', 'educational').title()}"
        )
        print(
            f"📱 Potential platforms: {', '.join(preview.get('potential_platforms', []))}"
        )

        print("\n💡 Campaign Preview:")
        print("   • Thumbnails: YouTube, TikTok, Instagram optimized")
        print("   • Descriptions: SEO-optimized for each platform")
        print("   • Social Posts: Platform-specific content with engagement hooks")
        print("   • Trailers: Main + short versions for different audiences")
        print("   • Hashtags: Curated strategy for viral reach")
        print("   • Posting Schedule: Optimal timing for maximum engagement")

        print("\n🎯 Viral Potential Analysis:")
        print("   • Content analysis based on project genre and style")
        print("   • Platform algorithm optimization")
        print("   • Audience targeting recommendations")
        print("   • Performance tracking setup")

        return 0

    def _display_campaign_results(
        self,
        ...
        print(
            f"📱 Platforms: {', '.join([p.value.title() for p in campaign.target_platforms])}"
        )
        ...
        print(
            f"   Character Appeal: {analysis.get('character_appeal', 'medium').title()}"
        )
    ...
    def _display_minimal_format(
        self, campaign: MarketingCampaign, args: argparse.ArgumentParser
    ) -> int:
        """Display minimal format output."""
        total_assets = (
            len(campaign.thumbnails)
            + len(campaign.descriptions)
            + len(campaign.social_posts)
            + len(campaign.trailers)
        )
    ...
    def _display_summary_format(
        ...
        if campaign.posting_schedule:
            ...
            for post in campaign.posting_schedule[:3]:
                ...
                content_type = (
                    post.get("content_type", "post").replace("_", " ").title()
                )
    ...
    def _display_detailed_format(
        ...
        if campaign.descriptions:
            ...
            for i, desc in enumerate(campaign.descriptions, 1):
                ...
                print(
                    f"   {i}. {platform} Description - Viral Potential: {viral_score:.1f}/10"
                )
        ...
        if campaign.social_posts:
            ...
            for i, post in enumerate(campaign.social_posts, 1):
                ...
                print(
                    f"   {i}. {platform} Post - Viral Potential: {viral_score:.1f}/10 ({len(post.content)}/{char_limit} chars)"
                )
                print(
                    f"      Content: {post.content[:80]}{'...' if len(post.content) > 80 else ''}"
                )
        ...
        if campaign.trailers:
            ...
            for i, trailer in enumerate(campaign.trailers, 1):
                ...
                print(
                    f"   {i}. {platform} Trailer - {duration}s - Viral Potential: {viral_score:.1f}/10"
                )
                print(f"      Style: {trailer.metadata.get('style', 'N/A')}")
                print(
                    f"      Call to Action: {trailer.metadata.get('call_to_action', 'N/A')}"
                )
        ...
        if campaign.hashtags:
            ...
            categories = {
                "Project": [
                    h
                    for h in campaign.hashtags
                    if "storycore" in h.lower() or "creative" in h.lower()
                ],
                "Genre": [
                    h
                    for h in campaign.hashtags
                ...
                "Viral": [
                    ...
                    if any(
                        v in h.lower()
                        for v in ["viral", "trending", "fyp", "mustwatch"]
                    )
                ],
            }
        ...
        if strategy:
            ...
            print(
                f"   Brand Voice: {strategy.get('brand_voice', {}).get('tone', 'N/A')}"
            )
            content_mix = strategy.get("content_mix", {})
            if content_mix:
                print(
                    f"   Content Mix: Educational {content_mix.get('educational', 0)}%, Entertainment {content_mix.get('entertainment', 0)}%, Promotional {content_mix.get('promotional', 0)}%"
                )
        ...
        print("   Use this campaign to launch your content across social platforms.")
        print("   Track performance and adjust strategy based on audience engagement.")
