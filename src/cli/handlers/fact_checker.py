...
class FactCheckerHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        input_group.add_argument(
            "input", nargs="?", help="Input text or file path to analyze"
        )
        input_group.add_argument(
            "--file", type=str, help="Path to input file (text or transcript)"
        )
        input_group.add_argument(
    ...
    def _load_input(self, args: argparse.Namespace) -> str:
        ...
        if args.stdin:
            ...
            if not content or not content.strip():
                raise UserError(
                    "Input text is empty", "Provide text content to analyze"
                )
            ...
            raise UserError(
                "No input provided", "Provide input via argument, --file, or --stdin"
            )
    ...
    def _format_output(self, result: dict, output_format: str) -> str:
        ...
        if output_format == "json":
            ...
            if isinstance(report, bytes):
                ...
                raise SystemError(
                    "PDF generation failed", "Report is not in PDF format"
                )
