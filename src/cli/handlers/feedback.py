...
class FeedbackHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--steps", type=str, help="Reproduction steps (will prompt if not provided)"
        )
    ...
    def _feedback_recovery_mode(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            if not reproduction_steps:
                self.print_info(
                    "Please provide steps to reproduce (or press Enter to skip):"
                )
            ...
            try:
                ...
                self.print_info(
                    "You can manually copy the template from the output below:"
                )
                print("\n" + "-" * 60)
                print(issue_body)
                print("-" * 60 + "\n")
    ...
    def _retry_pending_reports(self) -> int:
        ...
        try:
            ...
            if failure_count > 0:
                ...
                self.logger.info(
                    f"Returning exit code 1 due to {failure_count} failures"
                )
                return 1
            else:
