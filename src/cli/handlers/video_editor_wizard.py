...
from src.wizard.video_editor_wizard import VideoMontage, get_montage_preview
...
class VideoEditorWizardHandler(BaseHandler):
    ...
    description = (
        "Create automatic video montages from storyboards with intelligent editing"
    )
    ...
    def _execute_preview_mode(self, project_path: Path) -> int:
        ...
        print("\n💡 Tips:")
        print(
            f"   • {'✅' if preview['has_audio_plan'] else '❌'} Audio plan available - {'Higher' if preview['has_audio_plan'] else 'Lower'} quality expected"
        )
        print(f"   • Run with --style {preview['recommended_style']} for best results")
        print("   • Use --preview before full creation to validate")

        return 0

    def _display_montage_results(
        self, montage: VideoMontage, format_type: str, args: argparse.ArgumentParser
    ) -> int:
    ...
    def _display_minimal_format(
        self, montage: VideoMontage, args: argparse.ArgumentParser
    ) -> int:
    ...
    def _display_summary_format(
        ...
        avg_clip_duration = (
            sum(clip_durations) / len(clip_durations) if clip_durations else 0
        )
    ...
    def _display_detailed_format(
        ...
        if montage.video_clips:
            ...
            for i, clip in enumerate(montage.video_clips, 1):
                ...
                print(
                    f"   {i:2d}. {clip.shot_id} | {clip.start_time:.1f}s - {end_time:.1f}s ({clip.duration:.1f}s)"
                )
        ...
        if montage.audio_tracks:
            ...
            for i, track in enumerate(montage.audio_tracks, 1):
                ...
                print(
                    f"   {i:2d}. {track.audio_type.title()} | {track.start_time:.1f}s - {end_time:.1f}s"
                )
                print(
                    f"       Volume: {track.volume_level:.1%} | Fade: {track.fade_in:.1f}s / {track.fade_out:.1f}s"
                )
        ...
        if montage.transitions:
            ...
            for i, transition in enumerate(montage.transitions, 1):
                ...
                print(
                    f"   {i:2d}. {trans_type} ({duration:.1f}s) | {from_shot} → {to_shot}"
                )
        ...
        print("   Import the JSON plan into compatible editing applications.")
