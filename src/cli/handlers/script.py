...
class ScriptHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--text", help="Script text directly from command line")

        parser.add_argument("--input", help="Path to script file to process")
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(
                f"  Total characters: {result['processing_metadata']['total_characters']}"
            )
            print(
                f"  Estimated duration: {result['processing_metadata']['estimated_duration_seconds']} seconds"
            )
            print(
                f"  Complexity score: {result['processing_metadata']['complexity_score']:.1f}/5.0"
            )
            ...
            print("  Updated project.json with script metadata")
    ...
    def _get_script_text(self, args: argparse.Namespace) -> str:
        ...
        if args.input:
            ...
            if not input_path.exists():
                raise UserError(
                    f"Input file not found: {input_path}", "Check the file path"
                )
