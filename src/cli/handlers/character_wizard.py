...
class CharacterWizardHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--resume", help="Resume from saved wizard session")

        parser.add_argument(
            "--batch", type=int, help="Create multiple characters in batch mode"
        )

        parser.add_argument("--genre", help="Character genre for batch creation")

        parser.add_argument("--style", help="Art style for batch creation")
    ...
    def _display_wizard_result(self, result) -> int:
        ...
        if result.success:
            ...
            if result.character_profile:
                ...
                print(
                    f"  Quality Score: {result.character_profile.quality_score:.1f}/5.0"
                )
                ...
                print("\n💡 Next steps:")
                print("   • Character saved to library")
                print("   • Use in storyboard generation")
                print("   • Generate character images with ComfyUI")
