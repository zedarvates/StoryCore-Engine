...
class IntegrationHandler(BaseHandler):
    ...
    def setup_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        subparsers = parser.add_subparsers(
            dest="subcommand", help="Integration subcommands"
        )

        # Integration status command
        subparsers.add_parser(
            "status", help="Show current integration status of all components"
        )

        # Integrate components command
        integrate_parser = subparsers.add_parser(
            "integrate", help="Integrate all StoryCore components"
        )
        integrate_parser.add_argument(
            "--audio-config", help="Audio engine configuration file"
        )
        integrate_parser.add_argument(
            "--3d-config", help="3D rendering engine configuration file"
        )
        integrate_parser.add_argument(
            "--http-config", help="HTTP server configuration file"
        )

        # Start services command
        subparsers.add_parser("start", help="Start all integrated services")
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        if not self.manager:
            return self.handle_error(
                Exception("Integration manager not available"), "integration"
            )
    ...
    def register_commands(self, subparsers) -> None:
        ...
        start_parser = subparsers.add_parser(
            "start-services", help="Start all integrated services"
        )
        start_parser.set_defaults(handler=self.handle_start_services)

        # Stop services command
        stop_parser = subparsers.add_parser(
            "stop-services", help="Stop all running services"
        )
    ...
    def handle_integrate(self, args) -> int:
        ...
        self._load_config(args.audio_config) if args.audio_config else None
        self._load_config(args._3d_config) if args._3d_config else None
        self._load_config(args.http_config) if args.http_config else None
