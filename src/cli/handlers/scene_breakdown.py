...
class SceneBreakdownHandler(BaseHandler):
    ...
    def execute(self, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(
                f"  Total scenes processed: {result['processing_metadata']['total_scenes_processed']}"
            )
            print(
                f"  Average complexity: {result['processing_metadata']['average_scene_complexity']:.1f}/5.0"
            )
            print(
                f"  Lighting consistency: {result['processing_metadata']['lighting_consistency_score']:.1f}/5.0"
            )
            print(
                f"  Color harmony: {result['processing_metadata']['color_harmony_score']:.1f}/5.0"
            )
            ...
            if result.get("detailed_scenes"):
                ...
                for scene in result["detailed_scenes"][:3]:
                    ...
                    print(
                        f"      Purpose: {scene['scene_purpose']['primary']} ({scene['scene_purpose']['cinematic_function']})"
                    )
                    print(
                        f"      Environment: {scene['environment']['type']} at {scene['environment']['time_of_day']}"
                    )
                    print(
                        f"      Lighting: {scene['lighting']['primary_light']['type']} ({scene['lighting']['primary_light']['direction']})"
                    )
            ...
            if result.get("global_cinematic_rules"):
                ...
                print(
                    f"    Visual consistency: {rules['visual_consistency']['style_anchor']}"
                )
                print(
                    f"    Color harmony: {rules['visual_consistency']['color_harmony']}"
                )
                print(
                    f"    Lighting logic: {rules['visual_consistency']['lighting_logic']}"
                )

            print("\n  Scene breakdown saved: scene_breakdown.json")
            print("  Updated project.json with breakdown processing results")
