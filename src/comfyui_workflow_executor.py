...
from src.comfyui_executor import comfyui_executor  # noqa: E402
...
class ComfyUIWorkflowExecutor:
    ...
    async def execute_lip_sync(
        ...
        logger.info(
            f"Triggering Lip Sync on {target}: {character_image} + {dialogue_audio}"
        )
        ...
        try:
            ...
            # Node 5: SaveImage
            if "5" in workflow:
                workflow["5"]["inputs"]["filename_prefix"] = (
                    f"{output_filename}_{int(time.time())}"
                )
    ...
    async def execute_workflow(
        ...
        return WorkflowResult(
            success=False,
            error_message=f"Workflow {workflow_type} not fully implemented",
        )
...
async def main():
    ...
    result = await executor.execute_lip_sync(
        character_image="input/character_face.png", dialogue_audio="input/dialogue.wav"
    )
