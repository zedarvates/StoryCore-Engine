...
try:
    ...
    from backend.music_lyrics_service import (
        MusicLyricsService,
    ...
    print(
        "Assurez-vous que le PYTHONPATH inclut la racine du projet et que toutes les dépendances sont installées."
    )
...
class OnePromptPipeline:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.char_registry = CharacterRegistry(
            storage_path=str(self.config.output_dir / "characters")
        )
        ...
        self.blender_gen = BlenderScriptGenerator(
            scripts_dir=str(self.config.output_dir / "blender_scripts")
        )
        self.blender_runner = BlenderHeadlessRunner(
            output_dir=str(self.config.output_dir / "blender_renders")
        )
    ...
    async def run(self, user_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            await self._generate_music(story_data.get("music_theme", user_prompt))
            ...
            for i, seg_def in enumerate(segments):
                logger.info(
                    f"🎞️ Segment {i + 1}/{len(segments)} : {seg_def.get('description')}"
                )

                # 4a. Blender Bridge (Layout & ControlNet)
                controlnet_passes = {}
                if self.config.use_blender:
                    controlnet_passes = await self._run_blender_stage(
                        i, seg_def, characters
                    )

                # 4b. ComfyUI (Rendu Final via CineProductionService)
                segment_video = await self._run_comfyui_stage(
                    i, seg_def, characters, controlnet_passes
                )
                ...
                if segment_video:
                    ...
                    logger.error(
                        f"❌ Échec du segment {i + 1}. Tentative de continuation..."
                    )
...
{{
    ...
    "segments": [
        ...
        try:
            ...
            logger.info(
                f"📊 Plan de production généré : {len(data.get('segments', []))} segments."
            )
            return data
        except Exception as e:
    ...
    async def _setup_characters(
        self, char_defs: List[Dict[str, Any]]
    ...
    async def _generate_music(self, theme: str) -> Dict[str, Any]:
        ...
        try:
            lyrics = await self.music_service.generate_lyrics(
                LyricsGenerationRequest(theme=theme)
            )
            style = await self.music_service.generate_music_style(
                MusicStyleRequest(prompt=theme)
            )
    ...
    async def _run_blender_stage(
        self, index: int, seg_def: Dict[str, Any], characters: List[CharacterIdentity]
    ) -> Dict[str, str]:
        ...
        try:
            ...
            scene = SceneJSON(
                ...
                camera=CameraConfig(
                    shot_type=ShotType(b_cfg.get("camera_shot", "medium"))
                ),
                atmosphere=AtmosphereConfig(
                    type=AtmosphereType(b_cfg.get("atmosphere", "none"))
                ),
                render=RenderSettings(
                    ...
                    output_path=str(
                        self.blender_runner.output_dir / f"layout_{index:03d}.png"
                    ),
                ),
            )
            ...
            if result["success"]:
                ...
                if self.config.export_controlnet:
                    ...
                    passes["depth"] = str(base / "depth_0001.png")
                    passes["canny"] = str(base / "canny_0001.png")
    ...
    async def _run_comfyui_stage(
        ...
        controlnet_passes: Dict[str, str],
    ) -> Optional[str]:
                    ...
                    if job.results:
                        ...
                        video_res = next(
                            (
                                ...
                                if r.get("step") in ["video", "muxed_video"]
                            ),
                            job.results[-1],
                        )
    ...
    def _get_fallback_story_data(self, prompt: str) -> Dict[str, Any]:
        ...
        return {
            "characters": [
                {"name": "Protagoniste", "description": "Un personnage mystérieux"}
            ],
...
async def main():
    ...
    parser.add_argument(
        "--project",
        type=str,
        default=f"prod_{uuid.uuid4().hex[:6]}",
        help="ID du projet",
    )
    parser.add_argument(
        "--output", type=str, default="./output/one_prompt", help="Dossier de sortie"
    )
    parser.add_argument(
        "--no-blender", action="store_true", help="Désactiver l'étape Blender"
    )
