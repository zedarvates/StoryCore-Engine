...
class ExportManager:
    ...
    def __init__(
        ...
        logger.info(
            f"  Default organization: {self.default_settings.organization.value}"
        )
    ...
    def validate_export_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "organize_by_shot" in config and not isinstance(
                config["organize_by_shot"], bool
            ):
                return False

            if "include_metadata" in config and not isinstance(
                config["include_metadata"], bool
            ):
                return False

            if "generate_timeline" in config and not isinstance(
                config["generate_timeline"], bool
            ):
                return False
    ...
    def export_frame_sequence(
        ...
        shot_metadata: Optional[List[Dict[str, Any]]] = None,
    ) -> ExportManifest:
        """
        ...
        try:
            ...
            # Export frames
            exported_frames = self._export_frames(
                frames, export_path, export_settings, shot_metadata
            )

            # Export metadata
            metadata_files = []
            if export_settings.include_metadata:
                metadata_files = self._export_metadata(
                    exported_frames, export_path, export_settings
                )

            # Export timeline data
            timeline_files = []
            if export_settings.include_timeline and timeline_data:
                timeline_files = self._export_timeline_data(
                    timeline_data, export_path, export_settings
                )
            ...
            if export_settings.include_qa_reports and qa_reports:
                qa_files = self._export_qa_reports(
                    qa_reports, export_path, export_settings
                )
    ...
    def _create_export_structure(
        self, export_id: str, settings: ExportSettings
    ) -> Path:
    ...
    def _export_frames(
        ...
        for i, frame in enumerate(frames):
            ...
            # Generate filename
            filename = (
                settings.frame_naming_pattern.format(
                    index=i,
            ...
            if settings.organization == OrganizationStructure.PROFESSIONAL:
                ...
                settings.organization == OrganizationStructure.BY_SEQUENCE
                and sequence_id
            ):
    ...
    def _export_metadata(
        ...
        self._save_metadata_file(
            frame_manifest, manifest_file, settings.metadata_format
        )
        ...
        if shots:
            ...
            self._save_metadata_file(
                {"shots": list(shots.values())}, shots_file, settings.metadata_format
            )
            metadata_files.append(str(shots_file.relative_to(export_path)))

        return metadata_files

    def _export_timeline_data(
        self, timeline_data: Dict[str, Any], export_path: Path, settings: ExportSettings
    ) -> List[str]:
    ...
    def _export_qa_reports(
        ...
        summary = {
            ...
            "overall_quality": sum(r.get("overall_score", 0) for r in qa_reports)
            / len(qa_reports)
            if qa_reports
            else 0,
            "issues_found": sum(len(r.get("detected_issues", [])) for r in qa_reports),
            "recommendations": sum(
                len(r.get("recommendations", [])) for r in qa_reports
            ),
    ...
    def _save_metadata_file(
        self, data: Dict[str, Any], filepath: Path, format_type: str
    ):
    ...
    def _create_compressed_package(self, export_path: Path, manifest: ExportManifest):
        ...
        logger.info(
            f"Compressed package created: {package_path} ({package_size / (1024 * 1024):.1f} MB)"
        )
    ...
    def load_manifest(self, export_path: Path) -> Optional[ExportManifest]:
        ...
        try:
            ...
            settings = ExportSettings(
                ...
                frame_naming_pattern=settings_data.get(
                    "frame_naming_pattern", "frame_{index:06d}"
                ),
    ...
    def export_shot_metadata(
        self, shot_data, export_config: ExportConfig
    ) -> ExportResult:
        ...
        try:
            ...
            # Determine filename
            filename = (
                f"{shot_data.shot_id}_metadata.{export_config.metadata_format.value}"
            )
            metadata_path = output_path / filename

            # Save metadata
            self._save_metadata_file(
                metadata, metadata_path, export_config.metadata_format.value
            )
