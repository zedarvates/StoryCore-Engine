...
class TypeScriptImport:
    ...
    def __post_init__(self):
        ...
        self.is_relative = self.module_path.startswith(
            "."
        ) or self.module_path.startswith("/")
...
class TypeScriptImportUpdater:
    ...
    def calculate_new_import_path(
        self, file_path: Path, import_stmt: TypeScriptImport
    ) -> Optional[str]:
        """
        ...
        new_import_path = self.path_mapping.get_relative_import(
            new_file_path, new_imported_file
        )
        ...
        try:
            ...
            if outdir_match:
                ...
                content = content.replace(
                    f"outDir: '{old_outdir}'", f"outDir: '{new_outdir}'"
                )
    ...
    def _parse_import_match(
        self, match: re.Match, line: str, line_num: int
    ) -> Optional[TypeScriptImport]:
        """
    ...
    def _build_import_statement(
        self, import_stmt: TypeScriptImport, new_path: str
    ) -> str:
        """
...
def update_typescript_imports(
    project_root: Path, path_mapping: PathMapping, typescript_files: List[Path]
) -> Dict[str, int]:
    """
