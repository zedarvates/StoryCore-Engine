...
class BuildScriptUpdater:
    ...
    def update_package_json(self, file_path: Path) -> UpdateResult:
        ...
        try:
            ...
            return UpdateResult(
                file_path=file_path, success=True, updates_made=updates_made
            )

        except Exception as e:
            return UpdateResult(
                file_path=file_path, success=False, updates_made=0, error_message=str(e)
            )
        ...
        for old_path, new_path in sorted_mappings:
            ...
            if old_path_backslash in updated_text and old_path_backslash != old_path:
                updated_text = updated_text.replace(
                    old_path_backslash, new_path_backslash
                )
...
def update_build_scripts(
    project_root: Path, path_mapping: Dict[Path, Path]
) -> Dict[str, any]:
    """
