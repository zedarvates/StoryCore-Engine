...
class FileAnalyzer:
    ...
    DEMO_PATTERNS = [r".*-demo$", r"demo-.*", r"test-.*-project$", r".*_demo$"]
    ...
    TEMP_PATTERNS = [r".*\.tmp$", r".*\.temp$", r".*\.cache$", r".*\.log$", r".*~$"]
    ...
    def scan_project(self) -> ProjectStructure:
        ...
        print(
            f"Scan complete: {structure.total_files} files ({structure.total_size / (1024 * 1024):.2f} MB)"
        )
    ...
    def analyze_dependencies(self, files: List[FileInfo]) -> DependencyGraph:
        ...
        for file_info in files:
            if (
                file_info.category == FileCategory.PYTHON_SOURCE
                or file_info.category == FileCategory.PYTHON_TEST
            ):
            ...
            elif file_info.category in [
                FileCategory.TYPESCRIPT_SOURCE,
                FileCategory.REACT_COMPONENT,
                FileCategory.ELECTRON_SOURCE,
                FileCategory.TYPESCRIPT_TEST,
            ]:
        ...
        print(
            f"Dependency analysis complete: {len(graph.nodes)} files with dependencies"
        )
    ...
    def categorize_file(self, file_path: Path) -> FileCategory:
        ...
        if "model" in str(relative_path).lower() and extension in {
            ".safetensors",
            ".ckpt",
            ".pth",
            ".bin",
    ...
    def generate_movement_plan(self, structure: ProjectStructure) -> MovementPlan:
        ...
        all_files = (
            ...
            + structure.typescript_files
            + structure.config_files
            + structure.doc_files
            + structure.test_files
            + structure.asset_files
            + structure.build_artifacts
            + structure.model_files
            + structure.workflow_files
            + structure.demo_files
