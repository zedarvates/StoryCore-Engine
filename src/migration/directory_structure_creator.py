...
class DirectoryStructureCreator:
    ...
    REQUIRED_STRUCTURE = {
        "src": ["engines", "cli", "services", "models", "utils"],
        "frontend": ["components", "stores", "services", "utils"],
        "electron": [],
        "tests": ["unit", "integration", "property", "e2e"],
        "docs": ["api", "guides", "architecture", "deployment", "troubleshooting"],
        "config": ["development", "production", "test"],
        "assets": ["images", "audio", "video", "icons", "fonts", "cache"],
        "examples": ["demos"],
        "tools": ["scripts"],
        "build": [],
        "dist": [],
    ...
    OPTIONAL_DIRECTORIES = {".github", ".kiro", "comfyui_portable"}
    ...
    def create_structure(self) -> DirectoryTree:
        ...
        if self.validation_errors:
            ...
            for error in self.validation_errors:
                print(f"  - {error}")
            raise ValueError(
                f"Directory structure validation failed with {len(self.validation_errors)} errors"
            )
    ...
    def _create_directory(self, directory: Path, tree: DirectoryTree):
        ...
        if not directory.exists():
            if not self.dry_run:
                ...
                print(
                    f"  [DRY RUN] Would create: {directory.relative_to(self.project_root)}"
                )
    ...
    def _add_gitkeep_files(self, tree: DirectoryTree):
        ...
        for directory in tree.directories:
            ...
            has_files = (
                any(item.is_file() for item in directory.iterdir())
            ...
            if not has_files or not list(directory.iterdir()):
                ...
                if not gitkeep_path.exists():
                    if not self.dry_run:
                        gitkeep_path.touch()
                        print(
                            f"  Added .gitkeep: {directory.relative_to(self.project_root)}"
                        )
                    else:
                        print(
                            f"  [DRY RUN] Would add .gitkeep: {directory.relative_to(self.project_root)}"
                        )
