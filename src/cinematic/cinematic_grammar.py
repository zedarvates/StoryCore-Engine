...
class CinematicGrammarEngine:
    ...
    def analyze_context(
        self, scene_content: str, scene_heading: str, mood_keywords: List[str] = None
    ) -> SceneContext:
        """
    ...
    def _get_mood_modifier(self, shot_class: ShotClass, mood: str) -> float:
        ...
        if mood in MOOD_VISUAL_MAP:
            ...
            if shot_class == mood_shot:
                return 1.3
            elif mood in ["tense", "mysterious"] and shot_class in [
                ShotClass.CLOSE_UP,
                ShotClass.EXTREME_CLOSE_UP,
            ]:
                return 1.2
    ...
    def _generate_shot_reason(
        self, shot_class: ShotClass, context: SceneContext, mood: str
    ) -> str:
    ...
    def _get_recommended_movement(
        self, context: SceneContext, shot_class: ShotClass
    ...
    def _generate_shot_notes(
        self, shot_class: ShotClass, num_characters: int
    ) -> List[str]:
    ...
    def _generate_pacing_notes(
        self, pacing: PacingType, rhythm: RhythmPattern, mood: str
    ) -> List[str]:
    ...
    def _generate_technical_notes(
        self, context: SceneContext, shot_recs: List[ShotRecommendation]
    ) -> List[str]:
        ...
        if movements:
            notes.append(
                f"Recommended movements: {', '.join(m.value for m in movements[:2])}"
            )

        return notes

    def _generate_suggestions(
        self, context: SceneContext, mood: str, shot_recs: List[ShotRecommendation]
    ) -> List[str]:
    ...
    def analyze_rhythm(
        self, shot_sequence: List[Dict], total_duration: float
    ) -> SceneRhythmAnalysis:
        """
...
def analyze_cinematic_context(
    scene_content: str, scene_heading: str, mood_keywords: List[str] = None
) -> SceneContext:
