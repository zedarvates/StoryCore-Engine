...
class QualityAnalysis:
    ...
    def get_failing_dimensions(self) -> List[QualityDimension]:
        ...
        return [dim for dim, metric in self.metrics.items() if not metric.is_acceptable]

    def get_critical_issues(self) -> List[QualityMetric]:
        """Get metrics with critical severity."""
        return [
            metric for metric in self.metrics.values() if metric.severity == "critical"
        ]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                dim.value: metric.to_dict() for dim, metric in self.metrics.items()
            },
...
class QualityAssessor:
    ...
    async def assess_quality(self, frame: VideoFrame) -> QualityAnalysis:
        ...
        overall_score = sum(
            metrics[dim].score * weights[dim] for dim in QualityDimension
        )
...
class QualityOptimizer:
    ...
    def _suggest_contrast_adjustment(
    ...
    def _suggest_brightness_adjustment(
        self, metric: QualityMetric
    ) -> EnhancementSuggestion:
    ...
    def _suggest_detail_enhancement(
        self, metric: QualityMetric
    ) -> EnhancementSuggestion:
    ...
    async def apply_auto_enhancement(
        self, frame: VideoFrame, suggestions: List[EnhancementSuggestion]
    ) -> EnhancementResult:
        """
        ...
        try:
            ...
            for suggestion in suggestions:
                # Apply enhancement
                enhanced_data = await self._apply_enhancement(enhanced_data, suggestion)
            ...
            self.logger.error(
                f"Auto-enhancement failed for frame {frame.frame_id}: {e}"
            )
    ...
    async def _apply_enhancement(
        self, frame_data: bytes, suggestion: EnhancementSuggestion
    ) -> bytes:
        """
    ...
    async def preview_enhancement(
        self, frame: VideoFrame, suggestion: EnhancementSuggestion
    ) -> Dict[str, Any]:
        """
    ...
    async def apply_selective_enhancements(
        self, frame: VideoFrame, selected_suggestions: List[EnhancementSuggestion]
    ) -> EnhancementResult:
        """
    ...
    async def get_enhancement_feedback(
        self, result: EnhancementResult
    ) -> Dict[str, Any]:
        """
        ...
        if result.success:
            ...
            if result.get_improvement() > 0.1:
                ...
                feedback["recommendations"] = self._get_alternative_recommendations(
                    result
                )
    ...
    def _get_failure_recommendations(self, result: EnhancementResult) -> List[str]:
        ...
        if "memory" in error_msg.lower():
            recommendations.append(
                "Reduce enhancement strength or process in smaller batches"
            )
            recommendations.append("Close other applications to free up system memory")

        elif "timeout" in error_msg.lower():
            recommendations.append(
                "Reduce quality level or use faster enhancement modes"
            )
            recommendations.append("Process fewer enhancements simultaneously")

        elif "model" in error_msg.lower():
            recommendations.append(
                "Ensure AI models are properly installed and accessible"
            )
    ...
    async def suggest_alternative_approaches(
        self, frame: VideoFrame, failed_suggestions: List[EnhancementSuggestion]
    ) -> List[EnhancementSuggestion]:
        """
        ...
        self.logger.info(
            f"Generated {len(alternative_suggestions)} alternative enhancement suggestions"
        )
    ...
    async def batch_suggest_enhancements(
        self, analyses: List[QualityAnalysis]
    ) -> Dict[str, List[EnhancementSuggestion]]:
        """
        ...
        """
        self.logger.info(
            f"Batch generating enhancement suggestions for {len(analyses)} analyses"
        )
    ...
    def get_enhancement_summary(
        ...
        for result in successful:
            for enhancement_type in result.enhancements_applied:
                enhancement_counts[enhancement_type.value] = (
                    enhancement_counts.get(enhancement_type.value, 0) + 1
                )
        ...
        return {
            ...
            "best_improvement": max(
                (r.get_improvement() for r in successful), default=0.0
            ),
