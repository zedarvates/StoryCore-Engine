...
class WorkflowExecutor:
    ...
    def __init__(
        self, connection_manager: ComfyUIConnectionManager, workflows_dir: Path
    ):
        """
        ...
        logger.info(
            f"Initialized WorkflowExecutor with workflows_dir: {self.workflows_dir}"
        )
    ...
    def load_workflow(self, workflow_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = (
                f"Invalid JSON in workflow file: {workflow_path}. Error: {str(e)}"
            )
            logger.error(error_msg)
    ...
    def inject_parameters(
        ...
        if "prompt" in parameters and "58" in modified_workflow:
            ...
            if "inputs" in modified_workflow["58"]:
                ...
                logger.debug(
                    f"Injected prompt into node 58: {parameters['prompt'][:50]}..."
                )
        ...
        if "image_path" in parameters and "98" in modified_workflow:
            ...
            if "inputs" in modified_workflow["98"]:
                ...
                logger.debug(
                    f"Injected image_path into node 98: {parameters['image_path']}"
                )
        ...
        if "video_prompt" in parameters and "92" in modified_workflow:
            ...
            if "inputs" in modified_workflow["92"]:
                ...
                logger.debug(
                    f"Injected video_prompt into node 92: {parameters['video_prompt'][:50]}..."
                )
        ...
        if "video_length" in parameters and "92" in modified_workflow:
            ...
            if "inputs" in modified_workflow["92"]:
                ...
                logger.debug(
                    f"Injected video_length into node 92: {parameters['video_length']}"
                )

        if "seed_stage1" in parameters and "92" in modified_workflow:
            # Node 92:11 is RandomNoise Stage 1
            if "inputs" in modified_workflow["92"]:
                modified_workflow["92"]["inputs"]["noise_seed"] = parameters[
                    "seed_stage1"
                ]
                logger.debug(
                    f"Injected seed_stage1 into node 92: {parameters['seed_stage1']}"
                )
        ...
        if "seed_stage2" in parameters and "92" in modified_workflow:
            ...
            if "inputs" in modified_workflow["92"]:
                ...
                logger.debug(
                    f"Injected seed_stage2 into node 92: {parameters['seed_stage2']}"
                )
    ...
    async def execute_workflow(
        self, workflow: Dict[str, Any], client_id: str = "test_runner"
    ) -> str:
        """
                ...
                if prompt_id in history:
                    ...
                    if "status" in prompt_history:
                        ...
                        if status.get("completed", False):
                            logger.info(
                                f"Workflow completed successfully. Prompt ID: {prompt_id}"
                            )
                    ...
                    if "outputs" in prompt_history and prompt_history["outputs"]:
                        logger.info(
                            f"Workflow completed (outputs detected). Prompt ID: {prompt_id}"
                        )
                        return prompt_history
    ...
    async def download_output(
        self, output_info: Dict[str, Any], save_path: Path
    ) -> Path:
        """
