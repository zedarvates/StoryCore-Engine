...
class ComfyUIConnectionManager:
    ...
    def __init__(
        self, base_url: str, timeout: int = 10, auth: Optional[Dict[str, str]] = None
    ):
        """
        ...
        try:
            ...
            if self.session is None:
                ...
                self.session = aiohttp.ClientSession(
                    timeout=timeout_config, auth=auth_obj
                )
            ...
            try:
                async with self.session.get(
                    ...
                    if response.status == 401:
                        ...
                        logger.info(
                            f"Successfully connected to ComfyUI at {self.base_url}"
                        )
                        return True
                    else:
