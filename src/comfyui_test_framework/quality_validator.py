...
class QualityValidator:
    ...
    def validate_image(self, image_path: Path) -> ValidationResult:
        ...
        # Check file format
        format_valid = self.check_file_format(image_path, list(self.IMAGE_FORMATS))
        ...
        size_valid = self.check_file_size(
            image_path, self.IMAGE_MIN_SIZE, self.IMAGE_MAX_SIZE
        )
    ...
    def validate_video(self, video_path: Path) -> ValidationResult:
        ...
        # Check file format
        format_valid = self.check_file_format(video_path, list(self.VIDEO_FORMATS))
        result.checks["format_check"] = format_valid
        if not format_valid:
            result.passed = False
            result.errors.append(
        ...
        # Check file size
        size_valid = self.check_file_size(
            video_path, self.VIDEO_MIN_SIZE, self.VIDEO_MAX_SIZE
        )
        result.checks["size_check"] = size_valid
        if not size_valid:
    ...
    def check_file_format(self, file_path: Path, expected_formats: List[str]) -> bool:
        ...
        try:
            ...
            if extension in [".png"] and header.startswith(self.MAGIC_BYTES["png"]):
                return True
            elif extension in [".jpg", ".jpeg"] and header.startswith(
                self.MAGIC_BYTES["jpeg"]
            ):
                return True
