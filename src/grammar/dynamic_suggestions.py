...
class DynamicShotEngine:
    ...
    def analyze_context(
        self, scene_content: str, scene_heading: str
    ) -> ContextIndicators:
        """
    ...
    def detect_emotional_tones(
        ...
        normalized_scores = {
            tone: score / max_score for tone, score in tone_scores.items()
        }
        ...
        primary = (
            sorted_tones[0][0] if sorted_tones[0][1] > 0 else EmotionalTone.DRAMATIC
        )
    ...
    def detect_narrative_beat(
        ...
        action_words = sum(
            1 for w in ["fight", "chase", "explosion", "battle"] if w in content
        )
    ...
    def _generate_shot_notes(
        self, tone: EmotionalTone, shot_class: str, position: str
    ) -> List[str]:
        ...
        tone_notes = {
            EmotionalTone.TENSE: [
                "Use tight framing",
                "Minimize headroom",
                "Consider off-center composition",
            ],
            EmotionalTone.ROMANTIC: [
                "Soft focus possible",
                "Consider lens flare",
    ...
    def generate_shot_sequence(
        ...
        action_density = len(indicators.action_words) / max(
            len(scene_content.split()), 1
        )
        ...
        # Opening shot
        opening_shot_class = self._select_shot_class(
            num_characters, "opening", indicators
        )
        ...
        # Closing shot
        closing_shot_class = self._select_shot_class(
            num_characters, "closing", indicators
        )
    ...
    def _select_shot_class(
        self, num_characters: int, position: str, indicators: ContextIndicators
    ) -> str:
    ...
    def _generate_recommendations(
        self, tone: EmotionalTone, sequence: ShotSequence, context: NarrativeContext
    ) -> List[str]:
        ...
        if sequence.variety_score < 0.5:
            recommendations.append(
                "Add more visual variety with different angles and movements"
            )
