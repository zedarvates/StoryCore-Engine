...
class Camera:
    ...
    def _calculate_perspective_matrix(self) -> np.ndarray:
        ...
        proj[2, 3] = (2.0 * self.far_clip * self.near_clip) / (
    ...
    def _calculate_orthographic_matrix(self) -> np.ndarray:
        ...
        proj[2, 3] = -(self.far_clip + self.near_clip) / (
...
def create_default_camera() -> Camera:
    ...
    return Camera(position=(0.0, 1.6, 0.0), look_at=(0.0, 1.6, 10.0), fov=60.0)
...
def create_orbit_camera(
    distance: float = 10.0, height: float = 5.0, angle: float = 0.0
) -> Camera:
    """
    ...
    return Camera(position=(x, height, z), look_at=(0.0, 1.0, 0.0), fov=60.0)
