...
class AtmosphericConditions:
    ...
    # Composition
    composition: GasComposition = field(
        default_factory=lambda: GasComposition(N2=0.78, O2=0.21, Ar=0.01)
    )
...
class SunParameters:
    ...
    position: CelestialPosition = field(
        default_factory=lambda: CelestialPosition(elevation=45.0)
    )
...
class AtmosphereModel:
    ...
    def __init__(
        ...
        if custom_conditions:
            ...
            self.conditions = self.WORLD_CONFIGS.get(
                world_type, self.WORLD_CONFIGS[WorldType.EARTH]
            )
    ...
    def get_sky_color(self, view_elevation: float = 90.0) -> Tuple[float, float, float]:
        ...
        scattering_blue = (
            rayleigh_blue * rayleigh_phase * optical_depth * scale_height_factor
        )
        scattering_green = (
            rayleigh_green * rayleigh_phase * optical_depth * scale_height_factor
        )
        scattering_red = (
            rayleigh_red * rayleigh_phase * optical_depth * scale_height_factor
        )
    ...
    def get_lighting_conditions(
        ...
        return {
            "sun_position": (self.sun.position.azimuth, self.sun.position.elevation),
            "sun_color": sun_color,
            "sun_intensity": self.sun.intensity
            * max(0, math.sin(math.radians(self.sun.position.elevation))),
            "sky_color": sky_color,
            ...
            "visibility_km": 10.0
            * (1 - self.conditions.haze_density)
            * (1 - self.conditions.dust_storm_intensity),
            "atmospheric_perspective": self.conditions.haze_density
            + self.conditions.dust_storm_intensity,
        }
