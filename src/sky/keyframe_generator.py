...
class KeyframeResult:
    ...
    def to_comfyui_payload(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ground_mask": str(self.ground_mask_path)
            if self.ground_mask_path
            else None,
...
class KeyframeGenerator:
    ...
    def __init__(self, config: Optional[KeyframeConfig] = None):
        ...
        logger.info(
            f"KeyframeGenerator initialized: {self.config.width}x{self.config.height}"
        )
    ...
    def setup_atmosphere(
        ...
        logger.info(
            f"Atmosphere setup: {world_type.value} at {time_of_day:.1f}h, lat={latitude}°"
        )
    ...
    def generate_keyframe(self) -> KeyframeResult:
        ...
        try:
            ...
            paths = self._save_outputs(
                image, depth_map, sky_mask, ground_mask, metadata
            )
            ...
            return KeyframeResult(success=False, error_message=str(e))
    ...
    def _add_sun(self, image: np.ndarray, horizon_y: int) -> None:
        ...
        # Get sun position
        sun_az, sun_el = (
            self.atmosphere.sun.position.azimuth,
            self.atmosphere.sun.position.elevation,
        )
    ...
    def generate_for_comfyui(
        self, prompt: str, comfyui_config: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        payload["prompt_with_atmosphere"] = (
            f"{prompt}, {result._generate_prompt_enhancement()}"
        )
...
def quick_generate(
    ...
    Returns:
        KeyframeResult
    """
    config = KeyframeConfig(output_dir=Path(output_dir), quality=quality)
...
if __name__ == "__main__":
    ...
    result = quick_generate(
        world_type=WorldType.EARTH, time_of_day=18.5, quality=RenderQuality.DRAFT
    )
