...
class AIUserErrorHandler:
    ...
    def validate_parameters(
        self, parameters: Dict[str, Any], schema: Dict[str, Dict[str, Any]]
            ...
            if param_type == "int":
                if not isinstance(value, int):
                    try:
                        ...
                        warnings.append(
                            f"Parameter '{param_name}' converted to integer"
                        )
    ...
    def create_invalid_parameter_error(
        ...
        suggestions = [f"Expected type: {expected_type}"]
    ...
    def create_unsupported_content_error(
        self, content_type: str, supported_types: List[str]
    ) -> UserFriendlyError:
    ...
    def create_missing_requirement_error(
        self, requirement_name: str, requirement_type: str
    ) -> UserFriendlyError:
    ...
    def create_offline_mode_error(
        self, operation_name: str, cached_available: bool = False
    ) -> UserFriendlyError:
    ...
    def enable_offline_mode(self, cached_models: List[str]):
        ...
        self.logger.info(
            f"Offline mode enabled with {len(cached_models)} cached models"
        )
    ...
    def format_error_for_display(self, error: UserFriendlyError) -> str:
        ...
        lines = [f"❌ {error.title}", f"   {error.message}", ""]
