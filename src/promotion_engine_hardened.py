...
class PromotionEngine:
    ...
    def _load_and_validate_plan(self, plan_path: str) -> Dict[str, Any]:
        ...
        if plan["grid_specification"] not in ["3x3", "1x2", "1x4"]:
            raise ValueError(
                f"Unsupported grid specification: {plan['grid_specification']}"
            )

        # Validate master grid exists
        if not Path(plan["master_grid_path"]).exists():
            raise FileNotFoundError(
                f"Master grid not found: {plan['master_grid_path']}"
            )
    ...
    def _validate_panels_config(self, panels: List[Dict], grid_spec: str) -> None:
        ...
        for panel in panels:
            if "panel_id" not in panel or "grid_position" not in panel:
                raise ValueError(
                    "Panel missing required fields: panel_id, grid_position"
                )
    ...
    def _load_and_validate_image(self, image_path: Path) -> Image.Image:
        ...
        try:
            ...
            if (
                image.size[0] < self.min_image_size
                or image.size[1] < self.min_image_size
            ):
                raise ValueError(
                    f"Image too small: {image.size}, minimum: {self.min_image_size}px"
                )
            return image
        except Exception as e:
    ...
    def _process_single_panel(
        ...
        self._prepare_refinement_metadata(panel_config, panel_seed, plan)
        ...
        if ratio_deviation > 0.05:  # 5% tolerance
            raise ValueError(
                f"Aspect ratio deviation too large: {actual_ratio:.3f} vs {self.target_aspect_ratio:.3f}"
            )
    ...
    def _prepare_refinement_metadata(
        self, panel_config: Dict, seed: int, plan: Dict
    ) -> Dict[str, Any]:
    ...
    def _determine_validation_status(self, scores: List[float]) -> str:
        ...
        if mean_score < self.SHARPNESS_THRESHOLDS["too_soft"]:
            ...
            score > self.SHARPNESS_THRESHOLDS["oversharpen_risk"] for score in scores
        ):
    ...
    def _validate_final_results(self, qa_report: Dict) -> None:
        ...
        if status == "FAILED":
            raise ValueError(
                f"QA validation failed: mean sharpness {mean_sharpness:.1f} below threshold"
            )
        ...
        for metric in qa_report["panel_metrics"]:
            if metric["aspect_ratio_deviation"] > 0.05:
                raise ValueError(
                    f"Panel {metric['panel_id']} aspect ratio deviation too large"
                )
