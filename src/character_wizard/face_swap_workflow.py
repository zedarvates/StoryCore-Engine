...
class FaceSwapWorkflow:
    ...
    async def load_workflow_template(self, workflow_name: str) -> Optional[Dict]:
        ...
        try:
            response = requests.get(
                f"{self.comfyui_url}/workflow/{workflow_name}", timeout=10
            )
        ...
        try:
            ...
            if not source_face_b64 or not target_image_b64:
                return FaceSwapResult(
                    success=False, error_message="Failed to encode images"
                )

            # Build workflow based on method
            workflow = await self._build_face_swap_workflow(
                source_face_b64, target_image_b64, style_transfer_image
            )

            if workflow is None:
                return FaceSwapResult(
                    success=False, error_message="Failed to build workflow"
                )
            ...
            if workflow_id is None:
                return FaceSwapResult(
                    success=False, error_message="Failed to queue workflow"
                )
            ...
            if variation_prompts:
                variations = await self._generate_variations(result, variation_prompts)
            ...
            return FaceSwapResult(success=False, error_message=str(e))
            ...
            workflow = {
                ...
                "nodes": {
                    "load_source_face": {
                        ...
                        "inputs": {},
                        "inputs_values": {"image": source_face_b64},
                    },
                    "load_target_image": {
                        "class_type": "LoadImage",
                        "inputs": {},
                        "inputs_values": {"image": target_image_b64},
                    },
                    "face_swap": {
                    ...
                    "output": {
                        ...
                        "inputs": {"images": ["face_swap", 0]},
                        "inputs_values": {"filename_prefix": "face_swap_result"},
                    },
                },
            }
    ...
    async def _queue_workflow(self, workflow: Dict) -> Optional[str]:
        ...
        try:
            response = requests.post(
                ...
                json={"prompt": workflow, "client_id": "storycore_face_swap"},
                timeout=30,
            )
    ...
    async def _wait_for_result(
        self, workflow_id: str, timeout: int = 300
    ) -> Optional[str]:
        ...
        try:
            while time.time() - start_time < timeout:
                response = requests.get(
                    f"{self.comfyui_url}/history/{workflow_id}", timeout=10
                )
    ...
    async def _generate_variations(
        self, base_image_b64: str, prompts: List[str]
    ...
    async def _build_variation_workflow(
        self, base_image_b64: str, prompt: str
    ) -> Optional[Dict]:
            ...
            workflow = {
                ...
                "nodes": {
                    "load_image": {
                        "class_type": "LoadImage",
                        "inputs": {},
                        "inputs_values": {"image": base_image_b64},
                    },
                    "generate_variation": {
                        "class_type": "KSampler",
                        "inputs": {"image": ["load_image", 0]},
                        "inputs_values": {"prompt": prompt, "strength": 0.6},
                    },
                    "output": {
                        "class_type": "SaveImage",
                        "inputs": {"images": ["generate_variation", 0]},
                    },
                },
            }
            return workflow
    ...
    async def generate_character_sheet(
        ...
        for expr in expressions:
            ...
            result = await self._generate_single_variation(
                face_b64, prompt, "expression"
            )
            if result:
    ...
    async def _generate_single_variation(
        self, face_b64: str, prompt: str, variation_type: str
    ...
    def _compute_face_embedding(self, face_image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            hog = cv2.HOGDescriptor(
                winSize=(64, 64), blockSize=(16, 16), blockStride=(8, 8), nbins=9
            )
...
def get_face_swap_workflow(
    config: Optional[FaceSwapConfig] = None, comfyui_url: str = "http://127.0.0.1:8188"
) -> FaceSwapWorkflow:
