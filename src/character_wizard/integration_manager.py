...
from .models import CharacterProfile, PuppetCategory
from .error_handler import CharacterWizardError
...
class CharacterIntegrationManager:
    ...
    def integrate_character(
        self, profile: CharacterProfile, project_context: Optional[str] = None
    ) -> IntegrationResult:
        """
        ...
        try:
            ...
            if not self._validate_data_contract(profile):
                result.integration_errors.append(
                    "Character profile does not comply with Data Contract v1"
                )
                return result
            ...
            self._generate_test_prompts(profile)
    ...
    def _update_project_files(
        self, profile: CharacterProfile, project_context: str
    ) -> bool:
...
class ComfyUICharacterIntegration:
    ...
    def _build_character_descriptor(self, profile: CharacterProfile) -> str:
        ...
        if profile.visual_identity.hair_color and profile.visual_identity.hair_style:
            parts.append(
                f"{profile.visual_identity.hair_color} {profile.visual_identity.hair_style} hair"
            )
