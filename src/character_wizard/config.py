...
class CharacterWizardConfig:
    ...
    genre_archetypes: Dict[str, List[str]] = field(
    ...
    # Style Presets
    art_style_presets: Dict[str, Dict[str, Any]] = field(
        default_factory=lambda: {
            "realistic": {
    ...
    def validate(self) -> List[str]:
        ...
        if self.min_character_strengths > self.max_character_strengths:
            issues.append(
                "min_character_strengths cannot exceed max_character_strengths"
            )
        ...
        if self.color_extraction_method not in ["kmeans", "median_cut", "octree"]:
            issues.append(
                "color_extraction_method must be 'kmeans', 'median_cut', or 'octree'"
            )
