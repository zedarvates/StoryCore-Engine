...
class FaceExtractionService:
    ...
    def extract_face(
        ...
        if not self._initialized:
            return ExtractedFace(
                success=False, error_message="Face extraction service not initialized"
            )
        ...
        try:
            ...
            if not faces:
                return ExtractedFace(
                    success=False, error_message="No face detected in image"
                )
            ...
            # Estimate face angle
            angle = (
                self._estimate_face_angle(landmarks) if landmarks else FaceAngle.UNKNOWN
            )

            # Estimate expression
            expression = (
                self._estimate_expression(landmarks)
                if landmarks
                else FaceExpression.UNKNOWN
            )
            ...
            if PIL_AVAILABLE:
                ...
                face_pil = face_pil.resize(
                    self.config.output_size, Image.Resampling.LANCZOS
                )
            ...
            return ExtractedFace(success=False, error_message=str(e))
    ...
    def _detect_landmarks(
        self, image: np.ndarray, face_data: np.ndarray
        ...
        try:
            ...
            if landmarks is not None and len(landmarks) > 0:
                ...
                return FacialLandmarks(
                    ...
                    right_mouth_corner=tuple(points[54])
                    if len(points) > 54
                    else (0, 0),
                    # Extended landmarks
    ...
    def _estimate_expression(
        self, landmarks: Optional[FacialLandmarks]
    ) -> FaceExpression:
        ...
        try:
            ...
            mouth_height = (
                nose_tip[1] - mouth_center[1]
            )  # Negative if mouth is above nose
    ...
    def _align_face(
        self, face_image: np.ndarray, landmarks: FacialLandmarks
    ) -> np.ndarray:
    ...
    def _enhance_face(self, face_image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            cv2.GaussianBlur(face_image, (3, 3), 0)
    ...
    def save_face_image(
        self, extracted_face: ExtractedFace, output_path: Union[str, Path]
    ) -> bool:
    ...
    def face_to_base64(
        self, extracted_face: ExtractedFace, format: str = "PNG"
    ) -> Optional[str]:
        ...
        try:
            if PIL_AVAILABLE:
                ...
                _, buffer = cv2.imencode(
                    f".{format.lower()}", extracted_face.face_image
                )
