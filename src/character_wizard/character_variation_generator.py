...
class StylePromptConfig:
    ...
    quality_modifiers: List[str] = field(default_factory=lambda: [
        "highly detailed",
        "best quality",
        "professional"
    ])
...
class CharacterVariationConfig:
    ...
    styles: List[ArtisticStyle] = field(default_factory=lambda: [
        ArtisticStyle.REALISTIC,
        ArtisticStyle.ANIME,
        ArtisticStyle.CARTOON
    ])
    expressions: List[ExpressionType] = field(default_factory=lambda: [
        ExpressionType.NEUTRAL,
        ExpressionType.HAPPY,
        ExpressionType.SAD
    ])
    poses: List[PoseType] = field(default_factory=lambda: [
        PoseType.FRONT_VIEW,
        PoseType.THREE_QUARTER_LEFT,
        PoseType.THREE_QUARTER_RIGHT
    ])
    lighting_options: List[LightingType] = field(default_factory=lambda: [
...
class StylePromptBuilder:
    ...
    STYLE_TEMPLATES: Dict[ArtisticStyle, Dict[str, Any]] = {
        ArtisticStyle.REALISTIC: {
            ...
            "modifiers": ["photorealistic", "8k uhd", "dslr", "soft focus", "professional photography"],
            "artists": ["greg rutkowski", "artgerm", "alphonse mucha"]
        },
        ArtisticStyle.ANIME: {
            "base": "anime style illustration",
            "modifiers": ["cel shading", "vibrant colors", "clean lines", "anime aesthetics"],
        ...
        ArtisticStyle.SKETCH: {
            ...
            "modifiers": ["detailed linework", "shading", "cross-hatching", "hand drawn"],
        ...
        ArtisticStyle.CYBERPUNK: {
            ...
            "modifiers": ["neon lights", "futuristic", "tech enhancements", "dystopian"],
        ...
        ArtisticStyle.NOIR: {
            ...
            "modifiers": ["high contrast", "shadows", "dramatic lighting", "black and white"],
    ...
    def build_prompt(
        ...
        """
        style_template = cls.STYLE_TEMPLATES.get(config.style, cls.STYLE_TEMPLATES[ArtisticStyle.REALISTIC])
...
class CharacterVariationGenerator:
    ...
    async def generate_variations(
        ...
        try:
            ...
            if config.generate_character_sheet and variations:
                character_sheet_base64 = self._generate_character_sheet(variations, character_id)
    ...
    async def _generate_single_variation(
        ...
        try:
            ...
            # Check cache
            cache_key = self._get_cache_key(character_id, style, expression, pose, lighting)
            cached_result = None
        ...
        try:
            with open(cache_file, 'w') as f:
                json.dump({
                    ...
                    "timestamp": str(os.path.getmtime(cache_file) if cache_file.exists() else 0)
                }, f)
        except Exception as e:
        ...
        return [
            {
                ...
                "description": self.prompt_builder.EXPRESSION_DESCRIPTIONS.get(expr, "")
            }
