...
class CharacterNameGenerator:
    ...
    def generate_name(
        ...
        style_preference: Optional[NameStyle] = None,
    ) -> str:
        """
        ...
        candidates = self._generate_candidates(
            components, archetype_role, personality_traits, style, count=10
        )
    ...
    def generate_full_name(
        ...
        first_name = self.generate_name(
            culture, genre, archetype_role, personality_traits
        )
    ...
    def _get_name_components(
        self, culture: str, genre: str, style: NameStyle
    ) -> NameComponents:
    ...
    def _generate_candidates(
        ...
        if personality_traits:
            for _ in range(count // 3):
                name = self._generate_meaningful_name(
                    personality_traits, archetype_role, components
                )
                if name:
                    candidates.append(name)
    ...
    def _construct_name_from_components(
        self, components: NameComponents, style: NameStyle
    ) -> str:
        ...
        if style == NameStyle.FANTASY:
            ...
            if random.random() < 0.5:
                prefix = (
                    random.choice(components.prefixes) if components.prefixes else ""
                )
                ...
                root = random.choice(components.roots)
                suffix = (
                    random.choice(components.suffixes) if components.suffixes else ""
                )
                name = root + suffix
    ...
    def _generate_phonetic_name(
        self, archetype_role: str, components: NameComponents, style: NameStyle
    ) -> str:
        """Generate name using phonetic patterns"""
        # Get phonetic pattern for archetype
        patterns = self.phonetic_rules.get(
            archetype_role, self.phonetic_rules["default"]
        )
    ...
    def _filter_candidates(
        self, candidates: List[str], culture: str, genre: str
    ) -> List[str]:
    ...
    def _generate_surname(self, culture: str, genre: str, first_name: str) -> str:
        ...
        if genre.lower() == "fantasy":
            ...
            prefix = random.choice(
                ["Iron", "Silver", "Gold", "Dark", "Bright", "Swift", "Strong"]
            )
