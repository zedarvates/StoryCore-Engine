...
class BackstoryGenerator:
    ...
    def generate_backstory(
        self, params: AutoGenerationParams, archetype: CharacterArchetype, personality
    ) -> BackstoryProfile:
        ...
        template = self.backstory_templates.get(
            template_key, self.backstory_templates["default"]
        )

        # Generate origin story
        backstory.origin_story = self._generate_origin_story(
            template, personality, params
        )

        # Generate key life events
        backstory.key_life_events = self._generate_life_events(
            template, personality, params.age_range
        )

        # Generate formative experiences
        backstory.formative_experiences = self._generate_formative_experiences(
            personality, archetype
        )

        # Generate family and relationships
        backstory.family_background = self._generate_family_background(
            template, personality
        )
        backstory.significant_relationships = self._generate_relationships(
            personality, archetype
        )
        backstory.relationship_patterns = self._generate_relationship_patterns(
            personality
        )

        # Generate professional/social background
        backstory.occupation = self._generate_occupation(
            archetype, params.genre, params.age_range
        )
        backstory.education_level = self._generate_education_level(
            personality, backstory.occupation
        )
        backstory.social_status = self._generate_social_status(
            archetype, backstory.occupation
        )
        ...
        backstory.unresolved_conflicts = self._generate_unresolved_conflicts(
            personality, archetype
        )

        # Generate character arc potential
        backstory.growth_opportunities = self._generate_growth_opportunities(
            personality, archetype
        )
        backstory.potential_conflicts = self._generate_potential_conflicts(
            personality, archetype
        )
        backstory.transformation_triggers = self._generate_transformation_triggers(
            personality, archetype
        )

        return backstory

    def _generate_origin_story(
        self, template: dict, personality, params: AutoGenerationParams
    ) -> str:
        """Generate character origin story"""
        patterns = template.get(
            "origin_patterns", ["ordinary upbringing with defining moments"]
        )
        pattern = random.choice(patterns)
        return f"Character with {pattern} in a {params.genre} setting"

    def _generate_life_events(
        self, template: dict, personality, age_range: str
    ) -> list[str]:
        """Generate key life events"""
        events = template.get(
            "life_events",
            ["overcame a personal challenge", "formed important relationships"],
        )
        num_events = {
            "child": 2,
            "teen": 3,
    ...
    def _generate_formative_experiences(
        self, personality, archetype: CharacterArchetype
    ) -> list[str]:
        """Generate formative experiences"""
        experiences = [
    ...
    def _generate_relationships(
        ...
        return (
            random.sample(relationships, min(3, len(relationships)))
            if relationships
            else ["few but deep friendships"]
        )
    ...
    def _generate_occupation(
        self, archetype: CharacterArchetype, genre: str, age_range: str
    ) -> str:
    ...
    def _generate_social_status(
        self, archetype: CharacterArchetype, occupation: str
    ) -> str:
    ...
    def _generate_secrets(
        self, personality, archetype: CharacterArchetype, genre: str
    ) -> list[str]:
    ...
    def _generate_unresolved_conflicts(
        self, personality, archetype: CharacterArchetype
    ) -> list[str]:
    ...
    def _generate_potential_conflicts(
        self, personality, archetype: CharacterArchetype
    ) -> list[str]:
    ...
    def _generate_transformation_triggers(
        self, personality, archetype: CharacterArchetype
    ) -> list[str]:
