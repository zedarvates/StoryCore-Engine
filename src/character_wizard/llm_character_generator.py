...
from .llm_prompts import PromptType, get_prompt, get_system_prompt
...
class CharacterContext:
    ...
    def get_personality_summary(self) -> str:
        ...
        if self.openness > 0.7:
            traits.append("open and curious")
        elif self.openness < 0.3:
            traits.append("traditional and cautious")
        ...
        if self.extraversion > 0.7:
            ...
            traits.append("reserved and reflective")

        if self.agreeableness > 0.7:
            traits.append("cooperative and warm")
...
class LLMCharacterGenerator:
    ...
    async def generate_complete_character(
        ...
        try:
            ...
            client = self.llm.get_client(
                provider or self.default_provider, model or self.default_model
            )
            ...
            result.provider_used = (
                ...
                if hasattr(client.provider, "value")
                else str(client.provider)
            )
    ...
    async def _generate_description(self, context: CharacterContext, client) -> str:
        ...
        prompt = get_prompt(
            ...
            primary_traits=", ".join(context.primary_traits)
            if context.primary_traits
            else "balanced traits",
            genre=context.genre,
            tone=context.tone,
        )
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate description: {result.text}"
            )

    async def _generate_personality_narrative(
        self, context: CharacterContext, client
    ) -> str:
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate narrative: {result.text}"
            )

    async def _generate_backstory(self, context: CharacterContext, client) -> str:
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate backstory: {result.text}"
            )

    async def _generate_dialogue(self, context: CharacterContext, client) -> str:
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate dialogue: {result.text}"
            )

    async def _generate_motivations(self, context: CharacterContext, client) -> str:
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate motivations: {result.text}"
            )
    ...
    async def _generate_appearance_description(
        ...
        prompt = get_prompt(
            ...
            accessories=", ".join(context.accessories)
        ...
        if result.success:
            ...
            raise CharacterGenerationError(
                f"Failed to generate appearance: {result.text}"
            )

    async def _generate_voice_style(self, context: CharacterContext, client) -> str:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": 1.0
            - (self._total_errors / max(1, self._total_generations)),
        }
