...
class CharacterExporter:
    ...
    def export_to_json(
        self, character: Dict[str, Any], output: TextIO, pretty: bool = True
    ) -> int:
        """
        ...
        try:
            if pretty:
                json.dump(character, output, indent=2, ensure_ascii=False, default=str)
            else:
                json.dump(character, output, ensure_ascii=False, default=str)
            output.write("\n")
            return output.tell()
    ...
    def export_to_json_file(
        self, character: Dict[str, Any], filepath: str, pretty: bool = True
    ) -> bool:
        """
    ...
    def export_multiple_to_json(
        self, characters: List[Dict[str, Any]], output: TextIO, pretty: bool = True
    ) -> int:
        """
    ...
    def export_to_yaml(
        self, character: Dict[str, Any], output: TextIO, include_metadata: bool = True
    ) -> int:
        """
    ...
    def export_to_yaml_file(
        self, character: Dict[str, Any], filepath: str, include_metadata: bool = True
    ) -> bool:
        """
    ...
    def export_to_markdown(
        ...
        if "appearance" in include_sections:
            ...
            if appearance:
                lines.append(
                    f"- **Style:** {appearance.get('clothing_style', 'Not specified')}"
                )
                lines.append(
                    f"- **Colors:** {', '.join(appearance.get('clothing_colors', [])) or 'Not specified'}"
                )
        ...
        if "relationships" in include_sections:
            ...
            if relationships:
                for rel in relationships[:10]:
                    ...
                    lines.append(
                        f"- **{char}** ({rel_type}) - {status} [{'★' * int(strength * 5)}]"
                    )
                lines.append("")
            else:
    ...
    def export_to_llm_prompt(
        self, character: Dict[str, Any], output: TextIO, prompt_type: str = "summary"
    ) -> int:
        """
        ...
        if prompt_type == "summary":
            ...
            if traits:
                ...
                for trait, value in traits.items():
                    level = (
                        "high" if value > 0.6 else "low" if value < 0.4 else "moderate"
                    )
    ...
    def export_to_llm_prompt_file(
        self, character: Dict[str, Any], filepath: str, prompt_type: str = "summary"
    ) -> bool:
        """
    ...
    def export_to_comfyui_prompt(
        self, character: Dict[str, Any], output: TextIO, style_preset: str = "realistic"
    ) -> int:
        """
    ...
    def export_to_comfyui_prompt_file(
        self, character: Dict[str, Any], filepath: str, style_preset: str = "realistic"
    ) -> bool:
        """
    ...
    def generate_comfyui_workflow(
        self, character: Dict[str, Any], style_preset: str = "realistic"
    ) -> Dict[str, Any]:
        """
    ...
    def _build_comfyui_prompt(
        self, character: Dict[str, Any], style_preset: str
    ) -> str:
    ...
    def export_3d_deployment(self, character: Any, output_dir: str) -> Dict[str, Any]:
        """
        ...
        char_id = (
            ...
            if is_dict
            else getattr(character, "character_id", "unknown")
        )
        name = (
            character.get("name") if is_dict else getattr(character, "name", "Unnamed")
        )

        # Extract 3D assets from VisualProfile
        visual = (
            character.get("visual") if is_dict else getattr(character, "visual", None)
        )
        mesh_assets = {}
        if visual:
            mesh_assets = (
                visual.get("mesh_assets", {})
                if isinstance(visual, dict)
                else getattr(visual, "mesh_assets", {})
            )
        ...
        logger.info(
            f"3D Deployment Package created at {output_dir} with formats: {list(mesh_assets.keys())}"
        )
        return manifest

    def export_comfy_puppet_config(self, character: Any, output_dir: str) -> Path:
        """
...
def export_character(
    character: Dict[str, Any], filepath: str, format: str = "json"
) -> bool:
    """
...
def export_character_summary(character: Dict[str, Any], output: TextIO) -> int:
    """
