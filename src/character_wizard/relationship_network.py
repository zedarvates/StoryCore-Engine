...
class RelationshipNetwork:
    ...
    def remove_character(self, character_id: str) -> bool:
        ...
        keys_to_remove = [
            key for key in self.relationships.keys() if character_id in key
        ]
    ...
    def add_relationship(
        ...
        # Check if asymmetric
        get_symmetric_relationship(relationship_type)
    ...
    def get_relationship(
        self, character_id_1: str, character_id_2: str
    ) -> Optional[CharacterRelationship]:
    ...
    def add_relationship_event(
        ...
        **kwargs,
        ...
        if not relationship:
            logger.error(
                f"No relationship found between {character_id_1} and {character_id_2}"
            )
            return None
    ...
    def get_close_relationships(
        ...
        if character_id:
            return [
                r for r in self.get_character_relationships(character_id) if r.is_close
            ]

        return [r for r in self.relationships.values() if r.is_close]
    ...
    def get_hostile_relationships(
        ...
        if character_id:
            return [
                r
                for r in self.get_character_relationships(
                    character_id, include_negative=True
                )
                if r.is_hostile
            ]

        return [r for r in self.relationships.values() if r.is_hostile]
    ...
    def get_stats(self) -> RelationshipStats:
        ...
        for rel in self.relationships.values():
            ...
            stats.category_counts[cat_name] = stats.category_counts.get(cat_name, 0) + 1
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RelationshipNetwork":
        ...
        network.created_at = datetime.fromisoformat(
            data.get("created_at", datetime.now().isoformat())
        )
        network.modified_at = datetime.fromisoformat(
    ...
    def _add_relationship_to_indexes(
        self, relationship: CharacterRelationship, pair: tuple
    ) -> None:
