...
class ConsistencyTracker:
    ...
    def record_scene_appearance(
        ...
        snapshot = AppearanceSnapshot(
            scene_id=scene_id, timestamp=datetime.now(), **appearance_data
        )
    ...
    def record_scene_behavior(
        ...
        snapshot = BehaviorSnapshot(
            scene_id=scene_id, timestamp=datetime.now(), **behavior_data
        )
    ...
    def record_scene_dialogue(
        ...
        snapshot = DialogueSnapshot(
            scene_id=scene_id, timestamp=datetime.now(), **dialogue_data
        )
    ...
    def record_scene_knowledge(
        ...
        snapshot = KnowledgeSnapshot(
            scene_id=scene_id, timestamp=datetime.now(), **knowledge_data
        )
    ...
    def _check_appearance_variations(
        self, record: CharacterConsistencyRecord, current: AppearanceSnapshot
    ) -> List[Variation]:
    ...
    def _check_behavior_variations(
        self, record: CharacterConsistencyRecord, current: BehaviorSnapshot
    ) -> List[Variation]:
    ...
    def _check_dialogue_variations(
        self, record: CharacterConsistencyRecord, current: DialogueSnapshot
    ) -> List[Variation]:
    ...
    def _check_knowledge_variations(
        self, record: CharacterConsistencyRecord, current: KnowledgeSnapshot
    ) -> List[Variation]:
    ...
    def _update_consistency_score(self, record: CharacterConsistencyRecord) -> None:
        ...
        if record.appearances:
            score.appearance_score = self._calculate_category_score(
                record.appearances, ["hair_color", "eye_color", "height", "build"]
            )
            score.clothing_score = self._calculate_category_score(
                record.appearances, ["clothing_top", "clothing_bottom", "footwear"]
            )
        ...
        if record.behaviors:
            score.behavior_score = self._calculate_category_score(
                record.behaviors, ["mood", "posture", "movement_style"]
            )
        ...
        if record.dialogues:
            score.dialogue_score = self._calculate_category_score(
                record.dialogues, ["vocabulary_level", "sentence_structure", "tone"]
            )
    ...
    def _calculate_category_score(self, snapshots: List, fields: List[str]) -> float:
    ...
    def _generate_warning(
        self, record: CharacterConsistencyRecord, variation: Variation
    ) -> ConsistencyWarning:
    ...
    def get_active_warnings(
        self, character_id: Optional[str] = None, severity: Optional[str] = None
    ...
    def get_character_report(
        ...
        if include_warnings:
            report["warnings"] = [
                w.to_dict() for w in record.warnings if not w.is_resolved
            ]
    ...
    def _generate_suggestion(
        ...
        suggestions = {
            ...
            ConsistencyCategory.CLOTHING: "Clothing changed. Ensure this is explained by events in the story.",
            ConsistencyCategory.ACCESSORIES: "Accessories changed. Verify this is consistent with character habits.",
            ConsistencyCategory.BEHAVIOR: "Behavior changed. Make sure this fits the character's personality arc.",
            ConsistencyCategory.PERSONALITY: "Personality trait changed. This could affect character consistency.",
            ConsistencyCategory.DIALOGUE: "Dialogue style changed. Check if this matches the character's voice.",
            ConsistencyCategory.KNOWLEDGE: "Knowledge state changed. Verify this doesn't create plot holes.",
        }
