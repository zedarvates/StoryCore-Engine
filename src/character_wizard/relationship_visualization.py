...
from .relationship_network import RelationshipNetwork
from .relationship_types import RelationshipCategory
...
class GraphNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"id": self.id, "label": self.label, "type": self.node_type}
...
class GraphEdge:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        result = {"source": self.source, "target": self.target, "type": self.edge_type}
...
class RelationshipVisualizer:
    ...
    def _generate_nodes(self, network: RelationshipNetwork) -> List[GraphNode]:
        ...
        for char_id, char_node in network.characters.items():
            ...
            color = self.ARCHETYPE_COLORS.get(
                char_node.archetype.lower() if char_node.archetype else "", "#999999"
            )
    ...
    def _generate_edges(
        ...
        for pair, rel in network.relationships.items():
            ...
            if strength_value > 0:
                self.POSITIVE_STRENGTH_COLORS.get(strength_value, "#CCCCCC")
            else:
                self.NEGATIVE_STRENGTH_COLORS.get(strength_value, "#CCCCCC")
    ...
    def _calculate_centrality(
        ...
        for node in nodes:
            ...
            node.centrality = (
                connections / max_connections if max_connections > 0 else 0
            )

    def _assign_clusters(
        self, nodes: List[GraphNode], network: RelationshipNetwork
    ) -> None:
    ...
    def _get_layout_hints(self, network: RelationshipNetwork) -> Dict[str, Any]:
    ...
    def to_d3_json(self, network: RelationshipNetwork, pretty: bool = True) -> str:
        """
    ...
    def to_cytoscape_json(
        self, network: RelationshipNetwork, pretty: bool = True
    ) -> str:
        """
        ...
        elements = {
            "nodes": [{"data": n.to_dict()} for n in graph.nodes],
            "edges": [{"data": e.to_dict()} for e in graph.edges],
        }
        ...
        result = {"elements": elements, "style": style, "layout": layout}
    ...
    def _get_cytoscape_style(
        ...
        return [
            ...
            },
            {
                ...
                "style": {"border-width": 3, "border-color": "#DC143C"},
            },
        ]
    ...
    def export_for_d3_force(
        self, network: RelationshipNetwork, output_path: str
    ) -> bool:
    ...
    def export_for_visjs(self, network: RelationshipNetwork, output_path: str) -> bool:
