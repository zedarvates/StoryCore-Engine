...
from src.models.character_ccd import (
    CharacterCoreData, CreationMethod, ArtStyle, 
    ArtisticLock, LockingStrength
)
...
from src.character_wizard.image_to_character_service import get_image_to_character_service
...
class MethodologySwitcher:
    ...
    async def execute_pipeline(self, ccd: CharacterCoreData) -> CharacterCoreData:
        ...
        self.logger.info(f"Checking for {len(ccd.artistic_locks)} artistic locks before methodology shift.")
        
        if not pipeline_func:
            self.logger.warning(f"No pipeline found for method {method}, falling back to Narrative-First")
    ...
    async def _pipeline_narrative_first(self, ccd: CharacterCoreData) -> CharacterCoreData:
        """
    ...
    async def _pipeline_2d_first(self, ccd: CharacterCoreData) -> CharacterCoreData:
        ...
        if ccd.visual.reference_images:
            ...
            if result.success:
                ...
                self.logger.info("Successfully analyzed 2D input and populated Artistic Locks.")
    ...
    async def _pipeline_stylized_first(self, ccd: CharacterCoreData) -> CharacterCoreData:
        """
