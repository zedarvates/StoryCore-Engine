...
class CharacterWizardOrchestrator:
    ...
    def resume_wizard(self, state_file: str) -> CharacterCreationResult:
        ...
        try:
            ...
            print(
                f"Resuming character creation session: {self.current_state.session_id}"
            )
            ...
            return CharacterCreationResult(
                success=False, error_message=f"Failed to resume wizard: {str(e)}"
            )

    def batch_create_characters(
        self, count: int, common_params: Dict
    ) -> List[CharacterCreationResult]:
    ...
    def _display_welcome(self):
        ...
        print("=" * 50)
        print(
            "Create comprehensive character profiles for your StoryCore-Engine projects."
        )
    ...
    def _collect_auto_generation_params(self) -> AutoGenerationParams:
        ...
        # Get genre
        params.genre = (
            input("Genre (fantasy, sci-fi, modern, horror, etc.): ").strip()
            or "fantasy"
        )
        ...
        gender_map = {"1": "male", "2": "female", "3": "non_binary", "4": "other"}
        ...
        if selected_gender == "other":
            custom_gender = input(
                "Specify custom gender (e.g., alien, robotic, genderless, etc.): "
            ).strip()
        ...
        style = (
            input("Art style (realistic, anime, cartoon, artistic): ").strip()
            or "realistic"
        )
    ...
    def _get_image_path(self) -> str:
        ...
        while True:
            ...
            if not self.config.is_supported_image_format(path.suffix):
                print(
                    f"Unsupported format. Supported: {', '.join(self.config.supported_image_formats)}"
                )
                continue
            ...
            if file_size > self.config.get_max_image_size_bytes():
                print(
                    f"File too large. Maximum size: {self.config.max_image_size_mb}MB"
                )
                continue

            return str(path.absolute())

    def _generate_auto_character(
        self, params: AutoGenerationParams
    ) -> CharacterCreationResult:
        ...
        try:
            ...
            print(f"❌ {error_msg}")
            return CharacterCreationResult(success=False, error_message=error_msg)
    ...
    def _display_character_summary(self, character: CharacterProfile):
        ...
        print(
            f"   Appearance: {character.visual_identity.age_range} with {character.visual_identity.hair_color} hair"
        )
        print(
            f"   Personality: {', '.join(character.personality_profile.primary_traits[:3])}"
        )
    ...
    def _build_character_from_analysis(
        self, analysis: ImageAnalysisResult
    ) -> CharacterCreationResult:
        ...
        try:
            ...
            character.personality_profile.primary_traits = [
                "mysterious",
                "confident",
                "observant",
            ]
    ...
    def _save_character(self, character: CharacterProfile) -> bool:
        ...
        try:
            ...
            character_dict = {
                ...
                "name": character.name,
                "gender": character.visual_identity.gender.value
                if character.visual_identity.gender
                else None,
        ...
        if self.current_state:
            ...
            try:
                state_dict = {
                    ...
                    "creation_method": self.current_state.creation_method.value
                    if self.current_state.creation_method
                    else None,
    ...
    async def _run_voice_calibration(self, profile: VoiceProfile) -> VoiceProfile:
        ...
        while True:
            choice = (
                input(
                    "\nCommands: [P] Pitch, [I] Intonation, [G] Generate Sample, [S] Save: "
                )
                .strip()
                .lower()
            )
            ...
            if choice == "p":
                ...
                text = (
                    input("Enter text to synthesize: ").strip()
                    or "Checking my voice configuration."
                )
    ...
    async def _run_character_sheet_generation(self, ccd: CharacterCoreData):
        ...
        locks_desc = ", ".join(
            [f"{lock.attribute}: {lock.value}" for lock in ccd.artistic_locks]
        )
        ...
        config = CharacterGridConfig(
            ...
            outfits=[
                ...
                CharacterOutfit.COMBAT,
                CharacterOutfit.FORMAL,
            ],
            style=ccd.visual.art_style.value
            if hasattr(ccd.visual.art_style, "value")
            else str(ccd.visual.art_style),
        ...
        print("✓ Layout: 3 rows (Outfits) x 3 columns (Poses)")
