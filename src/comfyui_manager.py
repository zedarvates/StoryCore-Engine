...
from .error_handler import ErrorHandler, FallbackMode
...
class ComfyUIManager:
    ...
    def __init__(self, config: Optional[ComfyUIConfig] = None):
        ...
        self._service_status = ServiceStatus(
            is_running=False, state=ServiceState.STOPPED, port=self.config.server_port
        )
        ...
        self.health_monitor: Optional["HealthMonitor"] = None  # noqa: F821
        ...
        self.logger.info(
            f"ComfyUI Manager initialized with installation path: {self.config.installation_path}"
        )
    ...
    def set_health_monitor(self, health_monitor: "HealthMonitor") -> None:  # noqa: F821
    ...
    def start_service(self) -> ServiceStartResult:
        ...
        try:
            ...
            self.logger.debug(
                f"Starting ComfyUI with command: {' '.join(map(str, cmd))}"
            )
            ...
            try:
                ...
                return ServiceStartResult(
                    success=False, error_message=str(process_error)
                )
            ...
            if self.health_monitor:
                try:
                    service_ready = asyncio.run(
                        self.health_monitor.wait_for_service_ready(
                    ...
                    if service_ready:
                        ...
                        self.logger.info(
                            f"ComfyUI service started successfully in {startup_time:.2f} seconds"
                        )
                    ...
                    if fallback_mode == FallbackMode.DEGRADED:
                        self.logger.warning(
                            "Health monitoring failed, continuing without health checks"
                        )
                        time.sleep(2)
                        self._service_status.state = ServiceState.RUNNING
                        startup_time = time.time() - start_time
                    ...
                    return ServiceStartResult(
                        success=False, error_message=str(health_error)
                    )
            else:
                ...
                self.logger.warning(
                    "No health monitor available, assuming service started successfully"
                )
                metrics.complete(success=True)
            ...
            fallback_mode = asyncio.run(
                self.error_handler.handle_error(
                    e, {"component": "manager", "operation": "start_service"}
                )
            )
            ...
            self.performance_monitor.record_operation_end(
                "service_start", operation_id, False, {"error": error_msg}
            )
    ...
    def stop_service(self) -> ServiceStopResult:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"ComfyUI service stopped gracefully in {shutdown_time:.2f} seconds"
                )
                self._cleanup_after_stop()
                metrics.complete(success=True)

                return ServiceStopResult(
                    success=True, shutdown_time_seconds=shutdown_time
                )

            except subprocess.TimeoutExpired:
                # Force kill if graceful shutdown failed
                self.logger.warning(
                    f"Graceful shutdown timed out, force killing process {self.process.pid}"
                )
                ...
                shutdown_time = time.time() - start_time
                self.logger.info(
                    f"ComfyUI service force stopped in {shutdown_time:.2f} seconds"
                )
    ...
    def restart_service(self) -> ServiceRestartResult:
        ...
        if success:
            self.logger.info(
                f"ComfyUI service restarted successfully in {total_time:.2f} seconds"
            )
        else:
    ...
    async def get_service_status_async(self) -> ServiceStatus:
        ...
        # Update health status if monitor is available
        if self.health_monitor:
            self._service_status.health_status = (
    ...
    def _on_error_recovery(self, error_info, fallback_mode: FallbackMode) -> None:
        ...
        """
        self.logger.info(
            f"Error recovery activated: {fallback_mode.value} for error {error_info.error_id}"
        )
    ...
    def get_diagnostic_info(self) -> Dict[str, Any]:
        ...
        return {
            "service_status": {
                ...
                "startup_time": self._service_status.startup_time.isoformat()
                if self._service_status.startup_time
                else None,
            },
            "configuration": {
    ...
    def __del__(self):
        ...
        if self._service_status.is_running and self.process:
            self.logger.warning(
                "ComfyUI Manager being destroyed with running service, attempting cleanup"
            )
            try:
                self.stop_service()
            except Exception as e:
