...
async def generate_video(
    ...
    if not _seedance_addon:
        ...
        if not _seedance_addon:
            raise HTTPException(status_code=503, detail="Addon Seedance non disponible")

    try:
        logger.info(
            f"Génération vidéo Seedance demandée pour: {scene.get('name', 'Untitled')}"
        )

        result = await _seedance_addon.generate(
            scene=scene, references=references, config_overrides=config_overrides
        )
...
async def get_seedance_config():
    ...
    try:
        status = await _seedance_addon.get_status()
        return {"success": True, "config": status.get("config", {})}
    except Exception as e:
...
@router.put("/config")
async def update_seedance_config(new_config: Dict[str, Any] = Body(...)):
    """
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Échec de la mise à jour de la configuration"
            )
