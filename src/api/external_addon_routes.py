...
async def install_from_url(request: InstallFromUrlRequest, user_payload: dict = Depends(verify_jwt_token)):
    ...
    if not external_loader:
        raise HTTPException(status_code=500, detail="External addon loader not initialized")
...
async def install_from_github(request: InstallFromGithubRequest, user_payload: dict = Depends(verify_jwt_token)):
    user_id = user_payload.get("sub") if isinstance(user_payload, dict) else str(user_payload)
    logger.info(f"User {user_id} requesting addon installation from GitHub: {request.repo}")
    """
...
async def install_from_pypi(request: InstallFromPypiRequest, user_payload: dict = Depends(verify_jwt_token)):
    user_id = user_payload.get("sub") if isinstance(user_payload, dict) else str(user_payload)
    logger.info(f"User {user_id} requesting addon installation from PyPI: {request.package_name}")
    """
...
async def install_dependencies(request: InstallDependenciesRequest, user_payload: dict = Depends(verify_jwt_token)):
    user_id = user_payload.get("sub") if isinstance(user_payload, dict) else str(user_payload)
    ...
    if not dependency_manager:
        raise HTTPException(status_code=500, detail="Dependency manager not initialized")
...
async def check_dependencies(dependencies: dict, user_payload: dict = Depends(verify_jwt_token)):
    user_id = user_payload.get("sub") if isinstance(user_payload, dict) else str(user_payload)
