...
class CodeExamplesGenerator:
    ...
    def _generate_javascript_example(
        ...
        if endpoint.async_capable:
            ...
            lines.append("  while (true) {")
            lines.append(
                f'    const statusResponse = await fetch("{self.base_url}/storycore.task.status", {{'
            )
            ...
            lines.append(
                "      console.log('Task completed:', statusData.data.result);"
            )
            lines.append("      break;")
    ...
    def _generate_example_params(self, endpoint: EndpointDefinition) -> Dict[str, Any]:
        ...
        if "narration.generate" in endpoint.path:
            ...
            return {"project_name": "my-story", "path": "/path/to/projects"}
            ...
            return {"key": "character_name", "value": "Aria the Brave"}
