...
class ImageCategoryHandler(BaseAPIHandler):
    ...
    def __init__(self, config: APIConfig, router: APIRouter):
        ...
        try:
            ...
            logger.warning(
                "GridGenerator not available - grid endpoints will be limited"
            )
        ...
        try:
            ...
            logger.warning(
                "PromotionEngine not available - promotion endpoints will be limited"
            )
        ...
        try:
            ...
            if self.grid_generator:
                ...
                grid_path = generator.generate_grid(
                    str(project_path), grid_format, output_path, cell_size
                )
            else:
            ...
            panel_paths = [
                str(
                    project_path / "assets" / "images" / "panels" / f"panel_{i:02d}.ppm"
                )
    ...
    def promote_panel(
        ...
        try:
            ...
            if scale > 4:
                logger.warning(
                    f"Large scale factor ({scale}x) may result in very large files"
                )
            ...
            if self.promote_panels:
                ...
                response_data = {
                    ...
                    "output_dir": result.get(
                        "output_dir",
                        str(project_path / "assets" / "images" / "promoted"),
                    ),
    ...
    def refine_image(
        ...
        try:
            ...
            if not output_path:
                output_path = str(
                    img_path.parent / f"{img_path.stem}_refined{img_path.suffix}"
                )
    ...
    def _analyze_image_quality(self, image_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            laplacian_img = self.np.abs(
                self.np.convolve(img_array.flatten(), laplacian.flatten(), mode="same")
            )
            ...
            if laplacian_variance < 50:
                ...
                recommendations.append(
                    "Consider using sharpen filter or re-generating with higher quality"
                )
    ...
    def apply_style(
        ...
        error = self.validate_required_params(
            params, ["target_image_path", "style_parameters"], context
        )
        if error:
            return error
        ...
        try:
            ...
            if not output_path:
                output_path = str(
                    img_path.parent / f"{img_path.stem}_styled{img_path.suffix}"
                )
    ...
    def batch_process(
        ...
        """
        error = self.validate_required_params(
            params, ["image_paths", "operation"], context
        )
        if error:
            return error
        ...
        try:
            ...
            params.get("parallel", True)
            params.get("max_workers", 4)
            ...
            logger.info(
                f"Batch processed {len(image_paths)} images: {successful} successful, {failed} failed"
            )
            return self.create_success_response(response_data, context)
    ...
    def _batch_refine(self, image_path: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output_path = str(
            ...
            / f"{Path(image_path).stem}_refined{Path(image_path).suffix}"
        )
    ...
    def _batch_upscale(self, image_path: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output_path = str(
            Path(image_path).parent
            / f"{Path(image_path).stem}_upscaled{Path(image_path).suffix}"
        )
    ...
    def _batch_style_transfer(
        ...
        params.get("style_parameters", {})
        strength = params.get("strength", 0.7)

        output_path = str(
            Path(image_path).parent
            / f"{Path(image_path).stem}_styled{Path(image_path).suffix}"
        )
