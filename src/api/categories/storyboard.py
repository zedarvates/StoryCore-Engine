...
class StoryboardCategoryHandler(BaseAPIHandler):
    ...
    def _save_storyboard_to_disk(
        self, storyboard: Storyboard, base_path: str = "."
    ) -> None:
    ...
    def _load_storyboard_from_disk(
        ...
        try:
            ...
            shots = [
                Shot(
                    ...
                    created_at=datetime.fromisoformat(shot_data["created_at"])
                    if shot_data.get("created_at")
                    else None,
                    updated_at=datetime.fromisoformat(shot_data["updated_at"])
                    if shot_data.get("updated_at")
                    else None,
                )
    ...
    def _recalculate_total_duration(self, storyboard: Storyboard) -> None:
        ...
        storyboard.total_duration_seconds = sum(
        ...
        try:
            ...
            if auto_generate_shots and scene_data:
                ...
                if self.storyboard_engine:
                    try:
                        self.storyboard_engine.generate_storyboard(
                            Path(base_path) / project_name, num_shots=num_shots
                        )
                        # Engine would populate shots
                    except Exception as e:
                        logger.warning(
                            f"Storyboard engine failed: {e}, using mock generation"
                        )
    ...
    def validate(self, params: Dict[str, Any], context: RequestContext) -> APIResponse:
        ...
        try:
            ...
            if not storyboard:
                ...
                if project_name:
                    storyboard = self._load_storyboard_from_disk(
    ...
    def export(self, params: Dict[str, Any], context: RequestContext) -> APIResponse:
        ...
        try:
            ...
            if not output_path:
                ...
                output_path = str(
                    exports_dir / f"storyboard_{timestamp}.{export_format}"
                )
    ...
    def _export_html(
        self, storyboard: Storyboard, output_path: str, include_images: bool
    ) -> int:
    ...
    def timeline_generate(
        ...
        try:
            ...
            for shot in storyboard.shots:
                ...
                if (
                    include_transitions
                    and shot.sequence_number < len(storyboard.shots) - 1
                ):
            ...
            response = self.create_success_response(response_data, context)
            self.log_response(
                "storycore.storyboard.timeline.generate", response, context
            )
            return response
