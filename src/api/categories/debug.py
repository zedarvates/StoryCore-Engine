...
class DebugCategoryHandler(BaseAPIHandler):
    ...
    def _collect_api_metrics(self) -> APIMetrics:
        ...
        if self.latencies:
            ...
            p50_latency = (
                sorted_latencies[p50_idx] if p50_idx < len(sorted_latencies) else 0
            )
            p95_latency = (
    ...
    def _check_component_health(
        self, component_name: str, timeout_seconds: int
    ) -> ComponentHealth:
        ...
        try:
            ...
            if level and not validate_log_level(level):
                return self.create_error_response(
                    ...
                    context=context,
                    details={"level": level, "supported_levels": SUPPORTED_LOG_LEVELS},
            ...
            if not self.log_entries:
                self._add_log_entry(
                    "INFO", "api_router", "API system initialized", context.request_id
                )
                self._add_log_entry(
                    "INFO", "debug_handler", "Debug handler ready", context.request_id
                )
                self._add_log_entry(
                    "DEBUG", "auth_service", "Authentication service started"
                )
                self._add_log_entry(
                    "WARNING", "rate_limit_service", "Rate limit threshold approaching"
                )

            # Filter logs
            filtered_entries = self._filter_logs(
                level, component, start_time, end_time, search_text
            )
    ...
    def trace_enable(
        ...
        try:
            ...
            if components:
                invalid_components = [
                    c for c in components if not validate_component_name(c)
                ]
            ...
            logger.info(
                f"Enabled tracing: {trace_id} for {len(components_traced)} components"
            )
                ...
                try:
                    ...
                    with open(trace_path, "w") as f:
                        ...
                        if "enabled_at" in trace_data_copy:
                            trace_data_copy["enabled_at"] = trace_data_copy[
                                "enabled_at"
                            ].isoformat()
    ...
    def profiler_run(
        ...
        try:
            ...
            try:
                ...
                profile_filename = (
                    f"profile_{operation.replace('.', '_')}_{int(time.time())}.json"
                )
