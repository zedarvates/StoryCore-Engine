...
class PipelineCategoryHandler(BaseAPIHandler):
        ...
        try:
            ...
            if "schema_version" in project_data:
                if (
                    ...
                    != self.project_manager.schema_version
                ):
                    warnings.append(
                        f"Schema version mismatch: expected {self.project_manager.schema_version}, got {project_data['schema_version']}"
                    )
            ...
            result = {
                ...
                "issues": issues,
                "warnings": warnings,
                "data_contract_compliant": len(missing_fields) == 0
    ...
    def get_status(
        ...
        try:
            ...
            current_phase = project_data.get("status", {}).get(
    ...
    def execute_pipeline(
        ...
        error = self.validate_required_params(
            params, ["project_name", "stages"], context
        )
        if error:
            return error
        ...
        try:
            ...
            if async_mode:
                ...
                return self.create_pending_response(
                    execution_id, context, response_data
                )
        ...
        try:
            ...
            if not pipeline_info.get("is_running"):
                return self.create_error_response(
                    ...
                    details={
                        "project_name": project_name,
                        "is_paused": pipeline_info.get("is_paused"),
                    },
            ...
            response_data = {
                ...
                "current_stage": pipeline_info["stages"][
                    pipeline_info["current_stage_index"]
                ],
            }
    ...
    def resume_pipeline(
        ...
        try:
            ...
            response_data = {
                ...
                "remaining_stages": pipeline_info["stages"][
                    pipeline_info["current_stage_index"] :
                ],
            }
    ...
    def cancel_pipeline(
        ...
        try:
            ...
            response_data = {
                ...
                "stages_completed": pipeline_info["stages"][
                    : pipeline_info["current_stage_index"]
                ],
                "stages_cancelled": pipeline_info["stages"][
                    pipeline_info["current_stage_index"] :
                ],
            }
    ...
    def configure_stage(
        ...
        """
        error = self.validate_required_params(
            params, ["project_name", "stage_name"], context
        )
        if error:
            return error
    ...
    def create_checkpoint(
        ...
        try:
            ...
            # Create checkpoint
            checkpoint_id = (
                f"checkpoint_{project_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            checkpoint = {
    ...
    def restore_checkpoint(
        ...
        """
        error = self.validate_required_params(
            params, ["project_name", "checkpoint_id"], context
        )
        if error:
            return error
        ...
        try:
            ...
            if checkpoint["project_name"] != project_name:
                return self.create_error_response(
                    ...
                    context=context,
                    details={
                        "checkpoint_project": checkpoint["project_name"],
