...
class SecurityCategoryHandler(BaseAPIHandler):
    ...
    def _generate_token(
        self, user_id: str, token_type: str = "bearer", expires_in_seconds: int = 3600
    ) -> str:
    ...
    def _validate_token(
        self, token: str, validate_expiry: bool = True
    ...
    def _get_rate_limit_key(
        self, user_id: Optional[str], endpoint: Optional[str]
    ) -> str:
    ...
    def _check_rate_limit(
        self, user_id: Optional[str], endpoint: Optional[str], window_seconds: int = 60
    ) -> RateLimitInfo:
        ...
        limit_data["requests"] = [
            req_time for req_time in limit_data["requests"] if req_time > cutoff_time
        ]
    ...
    def auth_validate(
        ...
        try:
            ...
            error_response = self.validate_required_params(params, ["token"], context)
            if error_response:
            ...
            if required_permissions:
                invalid_perms = [
                    p for p in required_permissions if not validate_permission_type(p)
                ]
                if invalid_perms:
                    return self.create_error_response(
                        error_code=ErrorCodes.VALIDATION_ERROR,
            ...
            if validate_permissions and required_permissions:
                ...
                missing_perms = [
                    ...
                    if p not in user_perms and "admin" not in user_perms
                ]
                ...
                if missing_perms:
                    ...
                    response = self.create_success_response(response_data, context)
                    self.log_response(
                        "storycore.security.auth.validate", response, context
                    )
                    return response
    ...
    def permissions_check(
        ...
        try:
            ...
            if user_id not in self.user_permissions:
                ...
                response = self.create_success_response(response_data, context)
                self.log_response(
                    "storycore.security.permissions.check", response, context
                )
                return response
    ...
    def rate_limit(
        ...
        try:
            ...
            if user_id and endpoint:
                ...
                limit_info = self._check_rate_limit(
                    user_id, endpoint, time_window_seconds
                )
                ...
                for ep in common_endpoints:
                    limit_info = self._check_rate_limit(
                        user_id, ep, time_window_seconds
                    )
                    limits.append(limit_info)
            elif endpoint:
