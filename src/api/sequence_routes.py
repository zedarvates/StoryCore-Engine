...
async def list_sequences(
    project_path: str, current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        for file_path in sequences_dir.glob("sequence_*.json"):
            try:
                ...
                if "shot_ids" not in sequence_data or not isinstance(
                    sequence_data["shot_ids"], list
                ):
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list sequences: {str(e)}"
        )


@sequences_router.get("/{project_path:path}/{sequence_id}", response_model=SequenceData)
async def get_sequence(
    project_path: str, sequence_id: str, current_user: User = Depends(get_current_user)
):
    """
...
async def update_sequence(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update sequence: {str(e)}"
        )
...
async def create_sequence(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create sequence: {str(e)}"
        )
...
async def delete_sequence(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete sequence: {str(e)}"
        )
