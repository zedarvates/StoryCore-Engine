...
class DocumentationHandler(BaseAPIHandler):
    ...
    def __init__(self, config: APIConfig, router: APIRouter):
        ...
        self.examples_generator = CodeExamplesGenerator(
            router, base_url=f"http://{config.host}:{config.port}"
        )
    ...
    def get_code_examples(
        ...
        try:
            ...
            if endpoint_path:
                ...
                examples = self.examples_generator.generate_examples_for_endpoint(
                    endpoint
                )
                ...
                if language_filter:
                    examples = [
                        ex
                        for ex in examples
                        if ex.language.lower() == language_filter.lower()
                    ]
                ...
                examples_by_endpoint = (
                    self.examples_generator.generate_examples_for_category(category)
                )
                ...
                data = {
                    ...
                    "categories": list(
                        set(
                            self._extract_category(path) for path in all_examples.keys()
                        )
                    ),
