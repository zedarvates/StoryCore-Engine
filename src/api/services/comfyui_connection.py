...
class ComfyUIConnection(Connection):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.base_url}/system_stats", timeout=self.config.timeout
            )
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            response = self.session.get(f"{self.base_url}/system_stats", timeout=5.0)
    ...
    def _queue_prompt(
        ...
        response = self.session.post(
            f"{self.base_url}/prompt", json=payload, timeout=self.config.timeout
        )
        response.raise_for_status()
    ...
    def _get_history(self, prompt_id: str) -> Dict[str, Any]:
        ...
        response = self.session.get(
            f"{self.base_url}/history/{prompt_id}", timeout=self.config.timeout
        )
        response.raise_for_status()
        return response.json()
    ...
    def _get_queue(self) -> Dict[str, Any]:
        ...
        response = self.session.get(
            f"{self.base_url}/queue", timeout=self.config.timeout
        )
        response.raise_for_status()
        return response.json()
    ...
    def _interrupt(self) -> Dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.base_url}/interrupt", timeout=self.config.timeout
        )
        response.raise_for_status()
        return response.json()
