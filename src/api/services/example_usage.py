...
def example_authentication():
    ...
    # Check permissions
    has_perm = authz_service.check_permission(user, "storycore.narration", "execute")
    print(f"✓ Permission check (narration.execute): {has_perm}")

    has_perm = authz_service.check_permission(user, "storycore.pipeline", "write")
...
def example_rate_limiting():
    ...
    for i in range(8):
        ...
        if status.allowed:
            ...
            print(
                f"✗ Request {i + 1}: Rate limited (retry after: {status.retry_after_seconds}s)"
            )
...
def example_observability():
    ...
    print("✓ Logged request (params sanitized)")
    ...
    obs_service.record_metric(
        "api.request.duration", 100.5, {"endpoint": "narration.generate"}
    )
