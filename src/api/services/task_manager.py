...
class TaskManager:
    ...
    def _start_cleanup_thread(self) -> None:
        ...
        cleanup_thread = threading.Thread(
            target=cleanup_loop, name="TaskCleanup", daemon=True
        )
    ...
    def _execute_task(self, task: Task) -> None:
        ...
        """
        self.logger.info(
            f"Executing task: {task.task_id} (priority: {task.priority.name})"
        )
    ...
    def get_task_status(
        self, task_id: str, use_cache: bool = True
    ...
    def cancel_task(self, task_id: str) -> bool:
        ...
        with self.lock:
            ...
            if task.status in (
                ...
                self.logger.info(
                    f"Task {task_id} already finished with status: {task.status.value}"
                )
                return False
    ...
    def get_pending_tasks(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with self.lock:
            ...
            pending.sort(
                key=lambda t: (TaskPriority[t["priority"]].value, t["created_at"])
            )
