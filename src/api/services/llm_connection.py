...
class LLMConnection(Connection):
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            if self.config.provider in [LLMProvider.OPENAI, LLMProvider.LOCAL]:
                response = self.session.get(f"{self.base_url}/models", timeout=10.0)
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            ...
            if self.config.provider in [LLMProvider.OPENAI, LLMProvider.LOCAL]:
                response = self.session.get(f"{self.base_url}/models", timeout=5.0)
    ...
    def _complete(
        ...
        if self.config.provider == LLMProvider.OPENAI:
            ...
            response = self.session.post(
                f"{self.base_url}/complete", json=payload, timeout=self.config.timeout
            )
            response.raise_for_status()
            return response.json()
    ...
    def _list_models(self) -> Dict[str, Any]:
        ...
        if self.config.provider in [LLMProvider.OPENAI, LLMProvider.LOCAL]:
            response = self.session.get(
                f"{self.base_url}/models", timeout=self.config.timeout
            )
            response.raise_for_status()
            return response.json()
        else:
