...
class MusicRemixConfig:
    ...
    instrument_bias: Dict[str, float] = field(
        default_factory=lambda: {"drums": 1.0, "bass": 1.0, "vocals": 0.0}
    )
    ...
    async def remix(
        ...
        try:
            ...
            if not settings.USE_MOCK_COMFYUI:
                ...
                upload_res = await comfyui_executor.upload_image(
                    audio_bytes, filename
                )  # Reuse upload_image (works for any byte data in ComfyUI)
                ...
                if workflow_path.exists():
                    ...
                    if res.get("success"):
                        self.logger.info(
                            "Music remix completed via ComfyUI Transformers"
                        )
                        output_path = self.output_dir / f"remix_{int(time.time())}.wav"
                        with open(output_path, "wb") as f:
                            f.write(b"MOCK_REMIX_WAV")
