...
class VideoToAudioResult:
    success: bool
    audio_tracks: Dict[str, str] = field(
        default_factory=dict
    )  # track_type -> file_path
...
class VideoToAudioEngine:
    ...
    async def generate_audio_from_video(
        self, video_path: str, config: Optional[VideoToAudioConfig] = None
    ) -> VideoToAudioResult:
        """
        ...
        if not os.path.exists(video_path):
            return VideoToAudioResult(
                success=False, error_message=f"Video file not found: {video_path}"
            )
    ...
    async def _mix_tracks(
        self, tracks: Dict[str, str], config: VideoToAudioConfig
    ) -> str:
