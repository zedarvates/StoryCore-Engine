...
class StatusTracker:
    ...
    def calculate_completion(self, tasks_file: Path) -> float:
        ...
        try:
            ...
            logger.error(
                "Unexpected error reading %s: %s", tasks_file, e, exc_info=True
            )
        ...
        try:
            ...
            required_tasks = [
                status
                for status, optional in matches
            ...
            logger.debug(
                "Calculated completion for %s: %.1f%% (%d/%d)",
                tasks_file.name,
                completion * 100,
                completed,
                total,
            ...
            logger.error(
                "Error calculating completion for %s: %s", tasks_file, e, exc_info=True
            )
            raise ValueError(f"Failed to calculate completion for {tasks_file}: {e}")

    def determine_status(
        self, completion: float, metadata: Optional[Dict] = None
    ) -> FeatureStatus:
        """
        ...
        try:
                ...
                try:
                    ...
                    for status in FeatureStatus:
                        if status.value == status_str:
                            logger.debug(
                                "Using manual status override: %s", status.value
                            )
                            return status
                    logger.warning(
                        "Invalid status override '%s', using automatic determination",
                        status_str,
                    )
            ...
            logger.debug(
                "Determined status: %s (completion=%.1f%%)",
                status.value,
                completion * 100,
            )
            return status
    ...
    def get_completion_date(self, spec_dir: Path) -> Optional[datetime]:
        ...
        try:
            ...
            for commit_hash, date_str in commits:
                try:
                    ...
                    if content_result.returncode != 0:
                        logger.debug(
                            "Failed to get content for commit %s", commit_hash[:8]
                        )
                        continue
                    ...
                    logger.warning(
                        "Git command timed out for commit %s", commit_hash[:8]
                    )
                    continue
                except Exception as e:
            ...
            logger.error(
                "Unexpected error getting completion date for %s: %s",
                spec_dir,
                e,
                exc_info=True,
            )
