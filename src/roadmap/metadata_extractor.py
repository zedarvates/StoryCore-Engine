...
class MetadataExtractor:
        ...
        try:
            ...
            try:
                ...
                if not isinstance(metadata, dict):
                    logger.warning(
                        "Frontmatter in %s is not a dictionary, ignoring", file_path
                    )
                    return {}
                logger.debug(
                    "Extracted frontmatter from %s: %d fields", file_path, len(metadata)
                )
                return metadata
            except yaml.YAMLError as e:
                # Log warning but don't fail - return empty dict
                logger.warning(
                    "Failed to parse YAML frontmatter in %s: %s", file_path, e
                )
                return {}

        except Exception as e:
            logger.error(
                "Unexpected error extracting frontmatter from %s: %s",
                file_path,
                e,
                exc_info=True,
            )
    ...
    def infer_category(self, spec_name: str, content: str = "") -> FeatureCategory:
        ...
        if not isinstance(content, str):
            logger.warning(
                "content must be a string, got %s, using empty string", type(content)
            )
            content = ""
        ...
        try:
                ...
                for pattern in patterns:
                    try:
                        if re.search(pattern, spec_name_lower):
                            logger.debug(
                                "Inferred category %s from spec name: %s",
                                category.value,
                                spec_name,
                            )
                            return category
            ...
            if content:
                ...
                if category_scores:
                    ...
                    logger.debug(
                        "Inferred category %s from content analysis", inferred.value
                    )
                    return inferred
    ...
    def infer_priority(self, content: str, dependencies: list = None) -> Priority:
        ...
        if dependencies is not None and not isinstance(dependencies, list):
            logger.warning(
                "dependencies must be a list, got %s, ignoring", type(dependencies)
            )
            dependencies = None
        ...
        try:
            ...
            high_count = sum(
                content_lower.count(keyword) for keyword in self.HIGH_PRIORITY_KEYWORDS
            )
            ...
            low_count = sum(
                content_lower.count(keyword) for keyword in self.LOW_PRIORITY_KEYWORDS
            )
            ...
            if high_count > 0 or dependency_factor >= 2:
                logger.debug(
                    "Inferred HIGH priority (high_count=%d, dep_factor=%d)",
                    high_count,
                    dependency_factor,
                )
                ...
                logger.debug(
                    "Inferred LOW priority (low_count=%d, medium_count=%d)",
                    low_count,
                    medium_count,
                )
                return Priority.LOW
