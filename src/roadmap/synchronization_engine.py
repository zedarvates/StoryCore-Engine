...
from .models import Feature, RoadmapConfig, SpecFiles
...
class SynchronizationEngine:
    ...
    def detect_changes(self) -> Dict[str, List[SpecFiles]]:
        ...
        {spec.directory for spec in current_specs}
    ...
    def update_roadmap(
        self, changes: Optional[Dict[str, List[SpecFiles]]] = None
    ) -> None:
        """
        ...
        if not self.config.output_path.exists():
            logger.warning("Roadmap file doesn't exist, performing full generation")
            from .roadmap_generator import RoadmapGenerator
        ...
        final_content = self._restore_protected_sections(
            new_content, protected_sections
        )
    ...
    def _restore_protected_sections(
        self, new_content: str, protected_sections: Dict[str, str]
    ) -> str:
        """
        ...
        if "section_0" in protected_sections:
            ...
            new_content = re.sub(
                intro_pattern, restore_intro, new_content, count=1, flags=re.DOTALL
            )
    ...
    def _build_features(self, spec_files_list: List[SpecFiles]) -> List[Feature]:
        ...
        for spec_files in spec_files_list:
            try:
                ...
                logger.warning(f"Failed to process spec {spec_files.directory}: {e}")
                continue
    ...
    def _process_spec(self, spec_files: SpecFiles) -> Feature:
        ...
        category = self.metadata_extractor.infer_category(
            spec_files.directory.name, content
        )
        ...
        # Assign timeline
        timeline = self.timeline_organizer.assign_quarter(completion_date, status)
    ...
    def _generate_roadmap_content(
        self, timeline_groups: Dict[str, List[Feature]]
    ) -> str:
        """
