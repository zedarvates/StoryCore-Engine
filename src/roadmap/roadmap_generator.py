...
class RoadmapGenerator:
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Step 1: Scanning specs directory: %s", self.config.specs_directory
            )
    ...
    def _build_features(self, spec_files_list: List[SpecFiles]) -> List[Feature]:
        ...
        for spec_files in spec_files_list:
            try:
                ...
                logger.warning("Failed to process spec %s: %s", spec_files.directory, e)
    ...
    def _process_spec(self, spec_files: SpecFiles) -> Feature:
        ...
        if spec_files.tasks:
            try:
                ...
                logger.warning(
                    "Failed to calculate completion for %s: %s", spec_files.directory, e
                )
        ...
        timeline = self.timeline_organizer.assign_quarter(
            target_date or completion_date, status
        )
    ...
    def _generate_changelog(self, features: List[Feature]) -> None:
        ...
        completed_features = [f for f in features if f.status.value == "completed"]
        ...
        for feature in completed_features:
            try:
                self.changelog_writer.append_entry(feature, feature.completion_date)
            except Exception as e:
                logger.warning(
                    "Failed to add changelog entry for %s: %s", feature.name, e
                )
    ...
    def _validate_links(
        self, roadmap_content: str, spec_files_list: List[SpecFiles]
    ) -> None:
        """
    ...
    def _inject_badges(self, features: List[Feature]) -> None:
        ...
        for feature in features:
            ...
            try:
                ...
                logger.warning("Failed to inject badge for %s: %s", feature.name, e)
