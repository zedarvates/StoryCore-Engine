...
class SpecBadgeInjector:
    ...
    def _has_roadmap_badge(self, content: str) -> bool:
        ...
        badge_pattern = re.compile(
            r"\[.*?[Rr]oadmap.*?\]\(.*?ROADMAP\.md#[^\)]+\)", re.IGNORECASE
        )

        return bool(badge_pattern.search(content))

    def _inject_new_badge(
        self, content: str, spec_file: Path, roadmap_anchor: str
    ) -> str:
        """
        ...
        updated_content = content[:insertion_point] + badge + content[insertion_point:]
    ...
    def _update_existing_badge(self, content: str, new_anchor: str) -> str:
        ...
        updated_content = badge_pattern.sub(r"\1" + new_anchor + r"\3", content)
