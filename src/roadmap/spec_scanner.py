...
class SpecScanner:
            ...
            for item in self.base_path.rglob("*"):
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Skipping item not relative to base_path: %s", item
                        )
                        continue
                    ...
                    logger.error(
                        "Unexpected error processing %s: %s", item, e, exc_info=True
                    )
                    continue

        except PermissionError as e:
            logger.error(
                "Permission denied scanning directory %s: %s", self.base_path, e
            )
            return []
        except OSError as e:
        ...
        try:
            ...
            for spec_file in spec_files:
                try:
                    ...
                    logger.warning(
                        "Permission denied checking %s: %s", directory / spec_file, e
                    )
                    continue
                except OSError as e:
            ...
            logger.error(
                "Error validating spec directory %s: %s", directory, e, exc_info=True
            )
            return False
    ...
    def get_spec_files(self, directory: Path) -> SpecFiles:
        ...
        try:
            ...
            logger.error(
                "Error getting spec files from %s: %s", directory, e, exc_info=True
            )
