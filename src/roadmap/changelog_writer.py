...
class ChangelogEntry:
    ...
    def __init__(
        self, feature: Feature, completion_date: datetime, version: Optional[str] = None
    ):
        """
...
class ChangelogWriter:
    ...
    def append_entry(
        ...
        entry = ChangelogEntry(
            feature=feature, completion_date=completion_date, version=version
        )
    ...
    def organize_by_version(
        self, entries: Optional[List[ChangelogEntry]] = None
    ) -> str:
        """
        ...
        sorted_entries = sorted(entries, key=lambda e: e.completion_date, reverse=True)
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    ...
                    self.changelog_path.parent,
                    e,
                )
                raise ValueError(f"Permission denied creating directory: {e}")
            except OSError as e:
                logger.error(
                    "OS error creating directory %s: %s", self.changelog_path.parent, e
                )
