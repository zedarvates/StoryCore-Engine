...
from src.api.external_addon_routes import (
    router as external_addon_router,
...
async def media_search(body: dict):
    ...
    return {"query": query, "results": [], "message": f"Search executed for: {query}"}
...
async def generate_video(project_id: str, body: dict):
    ...
    context = RequestContext(
        endpoint="storycore.integration.comfyui.generate_video", method="POST"
    )
...
async def extend_video(project_id: str, body: dict):
    ...
    context = RequestContext(
        endpoint="storycore.integration.comfyui.extend_video", method="POST"
    )
...
async def list_vault_assets(project_id: str):
    ...
    try:
        ...
        if response is None:
            ...
            raise HTTPException(
                status_code=500, detail="Internal error: response is None"
            )

        logger.info(
            f"[vault_assets] response status={response.status}, has_error={response.error is not None}"
        )
        ...
        if response.data is None:
            logger.warning(
                "[vault_assets] Success but data is None, returning empty list"
            )
            return {"assets": []}

        logger.info("[vault_assets] Success, returning data")
        return response.data
...
async def get_generation_status(project_id: str, task_id: str):
    ...
    context = RequestContext(
        endpoint="storycore.integration.comfyui.get_status", method="GET"
    )
    ...
    try:
        ...
        for addon_name, addon_info in manager.addons.items():
            if addon_info.state == AddonState.ENABLED and addon_info.module:
                if (
                    hasattr(addon_info.module, "ADDON_INFO")
                    and "router" in addon_info.module.ADDON_INFO
                ):
        ...
        logger.info(
            f"Addon system initialized: {len(manager.addons)} addons discovered"
        )
