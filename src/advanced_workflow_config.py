...
class ConfigurationManager:
                ...
                if "." in config_key:
                    ...
                    if len(parts) == 2:
                        ...
                        if workflow_config and hasattr(workflow_config, attr):
                            ...
                            logger.info(
                                f"Environment override: {workflow}_config.{attr} = {converted_value}"
                            )
                else:
                    if hasattr(config, config_key):
                        ...
                        logger.info(
                            f"Environment override: {config_key} = {converted_value}"
                        )
    ...
    def migrate_config(
        self, old_config: Dict[str, Any], from_version: str, to_version: str
    ) -> Dict[str, Any]:
        """
    ...
    def apply_quality_preset(
        self, config: AdvancedWorkflowConfig, quality_level: QualityLevel
...
class AdvancedWorkflowConfigManager:
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        errors = config.validate()
        return {"is_valid": len(errors) == 0, "issues": errors}
