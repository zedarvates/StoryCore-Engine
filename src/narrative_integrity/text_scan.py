...
def hidden_channel_scan(text: str, settings: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if settings:
        for key, value in settings.items():
            if key in channels and value:
                channels[key] = value
    zero_width = set(channels.get("zero_width", []))
    nonstandard = set(channels.get("nonstandard_spaces", []))
    tag_ranges = _ranges_from_settings(channels, "tag_block_ranges")
    vs_ranges = _ranges_from_settings(channels, "variation_selector_ranges")
    interlinear = _ranges_from_settings(channels, "interlinear_ranges")
    before, after = _french_space_roles({})
    ...
    for index, char in enumerate(text):
        if char in zero_width:
            if char == chr(0xFEFF) and index > 0:
                ...
                counts["zero_width"] += 1
            continue
        if char in nonstandard:
            if _is_french_typography(text, index, before, after):
                counts["nonstandard_spaces_french"] += 1
            else:
                counts["nonstandard_spaces_removable"] += 1
            continue
        code = ord(char)
        if any(low <= code <= high for low, high in tag_ranges):
            counts["tag_block"] += 1
        elif any(low <= code <= high for low, high in vs_ranges):
            counts["variation_selector"] += 1
        elif any(low <= code <= high for low, high in interlinear):
            counts["interlinear"] += 1
...
_CYRILLIC = set(chr(c) for c in range(0x0400, 0x04FF))
_GREEK = set(chr(c) for c in range(0x0370, 0x03FF))
_CONFUSABLE = {
    "а": "a", "е": "e", "о": "o", "р": "p", "с": "c",
    "х": "x", "у": "y", "к": "k", "м": "m", "т": "t",
    "в": "b", "н": "h", "і": "i", "α": "a", "ο": "o",
...
def clean_text(
    ...
    vs_ranges = _ranges_from_settings(channels, "variation_selector_ranges")
    interlinear = _ranges_from_settings(channels, "interlinear_ranges")
    before, after = _french_space_roles(settings or {})
    ...
    for index, char in enumerate(text):
        ...
        if any(low <= code <= high for low, high in tag_ranges):
            removed["tag_block"] += 1
            continue
        if any(low <= code <= high for low, high in vs_ranges):
            removed["variation_selector"] += 1
            continue
        if any(low <= code <= high for low, high in interlinear):
            removed["interlinear"] += 1
            continue
        out.append(char)
