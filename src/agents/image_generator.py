...
class AgentImageGenerator:
        ...
        try:
            ...
            if loop.is_running():
                ...
                future = asyncio.run_coroutine_threadsafe(
                    self._check_comfyui_connection(), loop
                )
    ...
    def generate_image(
        ...
        try:
            ...
            if loop.is_running():
                future = asyncio.run_coroutine_threadsafe(
                    self._send_to_comfyui(workflow, agent_id), loop
                )
    ...
    async def _send_to_comfyui(
        self, workflow: Dict[str, Any], agent_id: str
    ) -> Dict[str, Any]:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if prompt_id in history:
                            ...
                            return await self._save_comfyui_output(result, agent_id)
            except Exception:
                pass
    ...
    async def _save_comfyui_output(
        self, result: Dict[str, Any], agent_id: str
    ) -> Dict[str, Any]:
                            ...
                            async with session.get(
                                ...
                                if response.status == 200:
                                    ...
                                    output_path = os.path.join(
                                        self.assets_dir, f"{agent_id}_card.png"
                                    )
    ...
    def _get_default_workflow(
        ...
        workflow = {
            "1": {
                "inputs": {"text": full_prompt, "clip": ["3", 0]},
            ...
            "3": {
                "inputs": {"ckpt_name": "sd15_default.yaml"},
                "class_type": "CheckpointLoaderSimple",
                "id": "3",
            },
            "4": {
                "inputs": {"samples": ["5", 0], "model": ["3", 0]},
                "class_type": "KSampler",
                "id": "4",
            },
            "5": {
            ...
            "8": {
                "inputs": {"filename_prefix": "agent_card", "images": ["7", 0]},
    ...
    def generate_all_images(self, agents: Optional[List[str]] = None) -> Dict[str, Any]:
        """
...
def generate_agent_image(
    agent_id: str, prompt: str, base_path: Optional[str] = None
) -> Dict[str, Any]:
    """
