...
class GridGenerator:
    ...
    def generate_grid(
        ...
        # Slice into panels
        panel_paths = self._slice_grid(
            grid_path, panels_dir, cols, rows, cell_width, cell_height
        )

        # Update project.json
        self._update_project_manifest(
            project_path, grid_path, panel_paths, cols, rows, grid_spec
        )

        return str(grid_path)

    def _create_ppm_grid(
        self, output_path: Path, cols: int, rows: int, cell_width: int, cell_height: int
    ) -> None:
    ...
    def _slice_grid(
        ...
        for i in range(0, len(pixel_data), 3):
            pixels.append(
                (int(pixel_data[i]), int(pixel_data[i + 1]), int(pixel_data[i + 2]))
            )
