...
class AIError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "suggested_fallback": self.suggested_fallback.value
...
class AIErrorHandler:
    ...
    def __init__(self, config: ErrorHandlerConfig):
        ...
        self.error_counts: Dict[AIErrorCategory, int] = {
            cat: 0 for cat in AIErrorCategory
        }
    ...
    async def handle_error(
        self, error: AIError, fallback_context: Optional[Dict[str, Any]] = None
    ) -> ErrorRecoveryResult:
        """
        ...
        try:
            result = await self._execute_fallback(
                strategy, error, fallback_context or {}
            )
            ...
            if self.config.log_recoveries:
                self.logger.info(
                    f"Recovered from {error.category.value} using {strategy.value}"
                )
    ...
    async def handle_with_timeout(
        ...
        while True:
            try:
                ...
                if (
                    allow_extension
                    and self.config.enable_timeout_extension
                    and extensions_used < self.config.max_timeout_extensions
                ):
                    extensions_used += 1
                    timeout *= 1.5  # Extend by 50%
                    self.logger.warning(
                        f"Extending timeout to {timeout}s (extension {extensions_used})"
                    )
                    continue
    ...
    async def _execute_fallback(
        self, strategy: FallbackStrategy, error: AIError, context: Dict[str, Any]
    ) -> Any:
        ...
        if strategy == FallbackStrategy.RETRY:
            ...
            raise NotImplementedError(
                f"Fallback strategy {strategy.value} not implemented"
            )
    ...
    async def _fallback_retry(self, error: AIError, context: Dict[str, Any]) -> Any:
        ...
        return await self.handle_with_retry(
            operation, max_retries=self.config.max_retries
        )
    ...
    async def _fallback_lower_quality(
        self, error: AIError, context: Dict[str, Any]
    ) -> Any:
