...
class PuppetLayerGenerators:
    ...
    def _generate_frame_puppet_rigs(
        self, frame: Dict[str, Any], scene_breakdown: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for placement in character_placements:
            ...
            puppet_rig = self._create_puppet_rig(
                character_id, puppet_type, placement, frame
            )
    ...
    def _generate_frame_layer_files(
        self, frame: Dict[str, Any], scene_breakdown: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for layer_name, layer_info in layer_placeholders.items():
            ...
            layer_file = self._create_layer_file(
                layer_name, layer_info, frame, scene_breakdown
            )
    ...
    def _determine_puppet_type(
        self, placement: Dict[str, Any], frame: Dict[str, Any]
    ) -> str:
    ...
    def _create_puppet_rig(
        ...
        control_points = self._generate_control_points(
            pose_data, puppet_config, placement
        )

        # Generate reference data for character consistency
        reference_data = self._generate_character_reference_data(
            character_id, placement
        )
    ...
    def _create_layer_file(
        ...
        visual_content = self._generate_layer_visual_content(
            layer_name, layer_info, frame, scene_breakdown
        )

        # Generate generation parameters for ComfyUI
        generation_params = self._generate_layer_generation_params(
            layer_name, layer_config, visual_content, frame
        )
        ...
        layer_file = {
            ...
            "comfyui_integration": {
                ...
                "resolution_priority": layer_config.get(
                    "resolution_priority", "medium"
                ),
                "generation_method": layer_config.get(
    ...
    def _generate_pose_data(
        self, pose_requirements: Dict[str, Any], puppet_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        pose_points = self._generate_pose_points(
            base_pose, puppet_config, pose_requirements
        )
        ...
        pose_data = {
            ...
            "facial_landmarks": self._generate_facial_landmarks(
                puppet_config, pose_requirements
            ),
    ...
    def _generate_control_points(
        ...
        control_points = {
            "pose_control": {
                ...
                "strength": 1.0
                if puppet_config["rig_complexity"] != "minimal"
                else 0.5,
                "pose_points": pose_data["pose_points"],
                "preprocessing": "openpose_full"
                if puppet_config["rig_complexity"] == "full"
                else "openpose_basic",
            },
            "face_control": {
    ...
    def _generate_character_reference_data(
        self, character_id: str, placement: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_layer_visual_content(
        self,
        layer_name: str,
        layer_info: Dict[str, Any],
        frame: Dict[str, Any],
        ...
        if layer_type == "environment":
            return self._generate_environment_content(
                layer_name, frame, scene_breakdown
            )
    ...
    def _generate_environment_content(
        self, layer_name: str, frame: Dict[str, Any], scene_breakdown: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_character_layer_content(
        self, layer_name: str, layer_info: Dict[str, Any], frame: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        content = {
            ...
            "rendering_requirements": {
                "pose_accuracy": "high"
                if layer_name == "L6_foreground_characters"
                else "medium",
                "facial_detail": "high"
                if layer_name in ["L5_main_characters", "L6_foreground_characters"]
                else "low",
                "clothing_detail": "high"
                if layer_name != "L3_background_characters"
                else "low",
            },
            "interaction_level": "primary"
            if layer_name == "L5_main_characters"
            else "secondary",
        }

        return content

    def _generate_props_content(
        self, layer_name: str, frame: Dict[str, Any], scene_breakdown: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_effects_content(
        self, layer_name: str, frame: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        content = {
            ...
            "lighting_setup": lighting_guides.get(
                "lighting_motivation", "natural_lighting"
            ),
    ...
    def _generate_layer_generation_params(
        self,
        layer_name: str,
        ...
        if resolution_priority == "maximum":
            generation_params.update(
                {"width": 1536, "height": 864, "steps": 40, "cfg_scale": 8.0}
            )
        elif resolution_priority == "high":
            generation_params.update({"width": 1280, "height": 720, "steps": 35})
        elif resolution_priority == "low":
            generation_params.update(
                {"width": 768, "height": 432, "steps": 25, "cfg_scale": 6.0}
            )
        ...
        generation_params["negative_prompt"] = self._generate_negative_prompt(
            layer_name
        )

        return generation_params

    def _generate_pose_points(
        self,
        base_pose: Dict[str, Any],
        puppet_config: Dict[str, Any],
        pose_requirements: Dict[str, Any],
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_facial_landmarks(
        self, puppet_config: Dict[str, Any], pose_requirements: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if num_landmarks >= 68:  # Full facial landmarks (P1)
            ...
            facial_landmarks = [
                {
                    "landmark": "left_eyebrow_outer",
                    "x": 0.45,
                    "y": 0.25,
                    "confidence": 0.8,
                },
                {
                    "landmark": "left_eyebrow_inner",
                    "x": 0.48,
                    "y": 0.24,
                    "confidence": 0.8,
                },
                {
                    "landmark": "right_eyebrow_inner",
                    "x": 0.52,
                    "y": 0.24,
                    "confidence": 0.8,
                },
                {
                    "landmark": "right_eyebrow_outer",
                    "x": 0.55,
                    "y": 0.25,
                    "confidence": 0.8,
                },
    ...
    def _generate_layer_prompt(
        self, visual_content: Dict[str, Any], frame: Dict[str, Any]
    ) -> str:
        ...
        if content_type == "character_layer":
            prompt_parts.extend(
                ["detailed character", "consistent appearance", "natural pose"]
            )
        elif "environment" in content_type:
            prompt_parts.extend(
                ["atmospheric perspective", "environmental detail", "natural lighting"]
            )
        elif content_type == "lighting_effects":
            prompt_parts.extend(
                ["volumetric lighting", "atmospheric effects", "subtle particles"]
            )
    ...
    def _generate_negative_prompt(self, layer_name: str) -> str:
        ...
        if "character" in layer_name:
            ...
            negative_elements.extend(
                ["cluttered", "distracting elements", "inconsistent perspective"]
            )
    ...
    def _calculate_layer_complexity(self, layer_files: List[Dict[str, Any]]) -> float:
        ...
        for layer_file in layer_files:
            ...
            resolution_priority = layer_file["comfyui_integration"][
                "resolution_priority"
            ]
            conditioning_method = layer_file["comfyui_integration"][
                "conditioning_method"
            ]
    ...
    def _calculate_puppet_consistency(self, puppet_rigs: List[Dict[str, Any]]) -> float:
        ...
        for char_id, rigs in character_rigs.items():
            if len(rigs) > 1:
                ...
                ref_data_list = [
                    rig["reference_data"]["visual_consistency"] for rig in rigs
                ]
