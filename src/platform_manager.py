...
class PlatformManager:
    ...
    def _initialize_platform(self):
        ...
        self.logger.info(
            f"Initializing platform manager for {self._platform_type.value}"
        )
                ...
                for line in lines[1:]:  # Skip header
                    if line.strip():
                        ...
                        if len(parts) >= 3:
                            gpu_id = parts[0]
                            memory_total = (
                                float(parts[1]) if parts[1] != "N/A" else 8192
                            )
    ...
    def _get_platform_capabilities(self) -> PlatformCapabilities:
        ...
        supports_metal = any(
            gpu.gpu_type == GPUType.APPLE_SILICON for gpu in self._gpu_info
        )
        supports_directml = self._platform_type == PlatformType.WINDOWS and any(
            gpu.gpu_type in [GPUType.AMD, GPUType.INTEL, GPUType.NVIDIA]
    ...
    def _scan_available_models(self):
        ...
        if not self.config.installation_path.exists():
            self.logger.warning(
                f"ComfyUI installation path not found: {self.config.installation_path}"
            )
            return
        ...
        model_dirs = {
            ...
            "upscale_models": self.config.installation_path
            / "models"
            / "upscale_models",
    ...
    def get_available_models(
        ...
        return {
            key: model
            for key, model in self._available_models.items()
    ...
    def get_optimal_settings(
        self, workflow_complexity: str = "medium"
    ) -> Dict[str, Any]:
        ...
        if self._platform_type == PlatformType.WSL:
            ...
            settings["cpu_threads"] = min(
                settings["cpu_threads"], 6
    ...
    def validate_environment(self) -> Tuple[bool, List[str]]:
        ...
        if not self.config.installation_path.exists():
            issues.append(
                f"ComfyUI installation not found: {self.config.installation_path}"
            )
        else:
            main_script = self.config.installation_path / "main.py"
        ...
        try:
            ...
            if free_gb < 10:
                issues.append(
                    f"Low disk space: {free_gb:.1f}GB free, recommend at least 10GB"
                )
        except Exception:
            pass
    ...
    def get_recommended_models(self, use_case: str = "general") -> List[str]:
        ...
        if use_case == "general":
            if max_gpu_memory > 12000:
                ...
                recommendations.extend(["checkpoints/v1-5-pruned-emaonly.safetensors"])
            ...
            recommendations.extend(
                ["upscale_models/RealESRGAN_x4plus.pth", "upscale_models/ESRGAN_4x.pth"]
            )
