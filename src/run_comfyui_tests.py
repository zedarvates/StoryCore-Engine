...
from comfyui_test_framework.test_runner import ComfyUITestRunner, TestConfig, TestResult
...
Environment Variables:
  ...
  COMFYUI_TEST_TYPE    Test type to run (image, video, pipeline, or all)
    ...
    parser.add_argument(
        "--no-report", action="store_true", help="Skip generating JSON report"
    )
...
def get_config_from_args(args: argparse.Namespace) -> TestConfig:
    ...
    comfyui_url = args.url or os.environ.get("COMFYUI_URL") or "http://localhost:8000"

    # Requirement 8.2: Output directory
    output_dir = Path(
        args.output_dir
        or os.environ.get("COMFYUI_OUTPUT_DIR")
        or "temp_comfyui_export_test"
    )

    # Requirement 8.3: Timeout
    timeout = args.timeout or int(os.environ.get("COMFYUI_TIMEOUT", "300"))

    # Requirement 8.6: Workflows directory
    workflows_dir = Path(
        args.workflows_dir
        or os.environ.get("COMFYUI_WORKFLOWS_DIR")
        or "assets/workflows"
    )
...
async def run_image_tests(
    runner: ComfyUITestRunner, prompts: List[str]
) -> List[TestResult]:
    """
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            result = await runner.run_image_generation_test(
                prompt=prompt, test_name=f"image_test_{i + 1}"
            )
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            image_result = await runner.run_image_generation_test(
                prompt=prompt, test_name=f"video_input_image_{i + 1}"
            )
    ...
    for i, (prompt, image_path) in enumerate(zip(prompts, image_paths)):
        ...
        try:
            result = await runner.run_video_generation_test(
                image_path=image_path, prompt=prompt, test_name=f"video_test_{i + 1}"
            )
            results.append(result)
        except Exception as e:
    ...
    for i, prompt in enumerate(prompts):
        ...
        try:
            result = await runner.run_pipeline_test(
                prompt=prompt, test_name=f"pipeline_test_{i + 1}"
            )
            results.append(result)
        except Exception as e:
...
def print_summary(results: List[TestResult]):
    ...
    for result in results:
        ...
        print(
            f"{status} | {result.test_name} ({result.test_type}) | {result.duration:.2f}s"
        )
...
async def main():
    ...
    test_type = args.test_type or os.environ.get("COMFYUI_TEST_TYPE") or "all"
    ...
    if not config.workflows_dir.exists():
        ...
        logger.error(
            "Please ensure the workflows directory exists and contains workflow JSON files"
        )
        return 1
