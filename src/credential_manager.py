...
class CredentialManager:
    ...
    def _raise_missing_credentials_error(self, missing: List[CredentialConfig]) -> None:
        ...
        if self.config_path:
            ...
            error_lines.append(
                "No config file specified. Set credentials via environment variables."
            )
    ...
    def generate_template(
        self, output_path: Path, include_env_example: bool = True
    ) -> None:
        """
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  python credential_manager.py generate <output_path>  - Generate template"
        )
        print(
            "  python credential_manager.py validate <config_path>  - Validate config"
        )
        sys.exit(1)
