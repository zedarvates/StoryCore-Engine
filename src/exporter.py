...
class Exporter:
    ...
    def export_project(self, project_dir: str, export_base: str = "exports") -> str:
        ...
        self._generate_summary(
            export_dir, project_path, project_name, timestamp, qa_report
        )
        ...
        # Create ZIP archive
        self._create_zip_archive(export_dir, export_base_path, project_name, timestamp)
    ...
    def _generate_summary(
        ...
        try:
            with open(project_path / "project.json", "r") as f:
                project_data = json.load(f)
            total_panels = (
                project_data.get("asset_manifest", {})
                .get("promotion_metadata", {})
                .get("total_panels", 0)
            )
        except Exception:
            pass
...
"""
        ...
        if sharpness_improvement is not None:
            summary_content += (
                f"**Sharpness Improvement:** {sharpness_improvement:.1f}%\n"
            )
    ...
    def _copy_demo_assets(self, project_path: Path, export_dir: Path) -> None:
        ...
        if promoted_dir.exists():
            promoted_files = sorted([f for f in promoted_dir.iterdir() if f.is_file()])[
                :2
            ]
        ...
        if refined_dir.exists():
            refined_files = sorted([f for f in refined_dir.iterdir() if f.is_file()])[
                :2
            ]
    ...
    def _update_project_status(self, project_path: Path, qa_report: dict) -> None:
        ...
        try:
            ...
            project_data["status"]["current_phase"] = (
                "qa_completed" if qa_report["passed"] else "qa_failed"
            )
    ...
    def _print_qa_summary(self, qa_report: dict) -> None:
        ...
        if qa_report.get("categories"):
            ...
            for category, score in qa_report["categories"].items():
                ...
                print(
                    f"  {status} {category.replace('_', ' ').title()}: {score:.1f}/5.0"
                )
...
def generate_dashboard(project_path: Path) -> Path:
            ...
            panels.append(
                {
                    ...
                    "promoted_path": f"assets/images/promoted/panel_{panel_num}_promoted.png"
    ...
    html_content = _generate_dashboard_html(
        project_name, qa_score, qa_categories, panels, video_ready
    )
