...
class CharacterGridConfig:
    ...
    # Éléments à inclure
    outfits: List[CharacterOutfit] = field(
        default_factory=lambda: [CharacterOutfit.CASUAL]
    )
    poses: List[CharacterPose] = field(
    ...
    camera_angles: List[CameraAngle] = field(
        default_factory=lambda: [CameraAngle.EYE_LEVEL]
    )
    lighting_types: List[LightingType] = field(
        default_factory=lambda: [LightingType.CINEMATIC]
    )
    ...
    character_description: Optional[str] = (
        None  # Description du personnage pour les prompts
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "lighting_types": [lt.value for lt in self.lighting_types],
...
class CharacterGridAutomation:
    ...
    def generate_character_grid(
        self, config: CharacterGridConfig
    ) -> CharacterGridBundle:
        """
        ...
        panels = self._generate_panels(
            config=config, cells=cells, prompts=cell_prompts, output_dir=char_dir
        )

        # Générer l'image de grille combinée
        grid_image_path = self._generate_grid_image(
            config=config, panels=panels, output_dir=char_dir
        )
    ...
    def _calculate_grid_cells(self, config: CharacterGridConfig) -> List[GridCell]:
    ...
    def _generate_cell_prompts(
        self, config: CharacterGridConfig, cells: List[GridCell]
    ...
    def _generate_grid_image(
        self, config: CharacterGridConfig, panels: List[GridPanel], output_dir: Path
    ) -> Path:
        """Génère l'image de grille combinée."""
        grid_filename = (
            f"{config.character_id}_grid_{config.grid_size.value}.{config.image_format}"
        )
    ...
    def _save_bundle_metadata(self, bundle: CharacterGridBundle, output_dir: Path):
    ...
    def get_bundle_by_id(self, bundle_id: str) -> Optional[CharacterGridBundle]:
    ...
    def get_character_bundles(self, character_id: str) -> List[CharacterGridBundle]:
        """Récupère tous les bundles pour un personnage."""
        return [
            b for b in self.generation_history if b.config.character_id == character_id
        ]

    def get_latest_bundle(self, character_id: str) -> Optional[CharacterGridBundle]:
    ...
    def get_grid_layout(self, grid_size: GridSize) -> Dict[str, Any]:
        """Récupère les informations de layout pour une taille de grille."""
        return {"size": grid_size.value, **self.GRID_LAYOUTS[grid_size]}
    ...
    def export_bundle(self, bundle_id: str, format: str = "json") -> Optional[str]:
...
[FORMAT] Return JSON: {{"poses": ["best_{count}"], "expr": ["best_{count}"], "outfits": ["best_{count}"], "style": "one word"}}"""
        ...
        try:
            ...
            def map_enum(vals, enum_class):
                return [
                    enum_class(v) for v in vals if any(v == e.value for e in enum_class)
                ]
            ...
            return CharacterGridConfig(
                character_id="default", character_name=character_name
            )
