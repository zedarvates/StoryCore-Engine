...
class ComfyUIClient:
    ...
    def queue_workflow(
        self, workflow: Dict[str, Any], global_seed: int, prompt: str
    ) -> Optional[str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                modified_workflow = self._inject_parameters(
                    workflow, global_seed, prompt
                )

                # Queue the workflow
                response = self.session.post(
                    urljoin(self.base_url, "/prompt"),
                    json={"prompt": modified_workflow, "client_id": self.client_id},
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"Queue attempt {attempt + 1} failed: {response.status_code}"
                    )
                ...
                if self._is_vram_error(str(e)):
                    raise VRAMOverflowError(
                        "VRAM overflow detected - reduce batch size"
                    )
                delay = base_delay * (2**attempt)  # Exponential backoff
                time.sleep(delay)
                ...
                if data["type"] == "progress":
                    ...
                    if not self._validate_outputs(outputs):
                        raise ValidationError(
                            "ComfyUI outputs failed schema validation"
                        )
