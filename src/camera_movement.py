...
class CameraMovementSystem:
    ...
    def __init__(self, enable_motion_blur: bool = True, motion_blur_samples: int = 16):
        ...
        logger.info("Camera Movement System initialized")
    ...
    def apply_movement(
        ...
        try:
            ...
            motion_curve = self._generate_motion_curve(
                movement_spec, len(frames), frame_rate
            )
            ...
            for i, frame in enumerate(frames):
                ...
                transformed_frame = self._apply_camera_transform(
                    frame, position, velocity
                )
                transformed_frames.append(transformed_frame)

            # Generate metadata
            movement_metadata = self._generate_movement_metadata(
                movement_spec, motion_curve
            )

            processing_time = time.time() - start_time

            logger.info(
                f"Applied {movement_spec.movement_type.value} movement to {len(frames)} frames in {processing_time:.2f}s"
            )
    ...
    def _generate_motion_curve(
        self, movement_spec: MovementSpec, num_frames: int, frame_rate: float
    ) -> MotionCurve:
        ...
        for i in range(num_frames):
            ...
            i / (num_frames - 1) if num_frames > 1 else 0.0
            ...
            position = self._interpolate_camera_position(
                movement_spec.start_position, movement_spec.end_position, eased_t
            )
    ...
    def _apply_camera_transform(
        self, frame: np.ndarray, position: CameraPosition, velocity: float
    ) -> np.ndarray:
            ...
            if new_height > 0 and new_width > 0:
                ...
                if zoom_factor > 1.0:
                    ...
                    if PIL_AVAILABLE:
                        ...
                        frame = np.repeat(
                            np.repeat(cropped, height // new_height, axis=0),
                            width // new_width,
                            axis=1,
                        )[:height, :width]
    ...
    def _create_transform_matrix(
        self, position: CameraPosition, width: int, height: int
    ) -> np.ndarray:
        ...
        if position.zoom != 1.0:
            scale_matrix = np.array(
                [[position.zoom, 0, 0], [0, position.zoom, 0], [0, 0, 1]],
                dtype=np.float32,
            )
    ...
    def _interpolate_camera_position(
        self, start: CameraPosition, end: CameraPosition, t: float
    ) -> CameraPosition:
        ...
        return CameraPosition(
            ...
            focal_length=start.focal_length
            + (end.focal_length - start.focal_length) * t,
        )
    ...
    def _combine_motion_curves(self, curves: List[MotionCurve]) -> MotionCurve:
        ...
        for i, base_pos in enumerate(base_curve.positions):
            ...
            combined_pos = CameraPosition(
                ...
                y=base_pos.y,
                z=base_pos.z,
                pitch=base_pos.pitch,
                yaw=base_pos.yaw,
                roll=base_pos.roll,
                zoom=base_pos.zoom,
                focal_length=base_pos.focal_length,
            )
    ...
    def _generate_movement_metadata(
        self, movement_spec: MovementSpec, motion_curve: MotionCurve
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "motion_statistics": {
                "max_velocity": max(motion_curve.velocities)
                if motion_curve.velocities
                else 0.0,
                "avg_velocity": np.mean(motion_curve.velocities)
                if motion_curve.velocities
                else 0.0,
                "max_acceleration": max(motion_curve.accelerations)
                if motion_curve.accelerations
                else 0.0,
                "total_frames": len(motion_curve.positions),
            },
        }

    def _initialize_easing_functions(
        self,
    ) -> Dict[EasingFunction, Callable[[float], float]]:
        ...
        return {
            ...
            EasingFunction.EASE_IN_OUT: lambda t: (
                2 * t * t if t < 0.5 else 1 - 2 * (1 - t) * (1 - t)
            ),
            EasingFunction.EASE_IN_CUBIC: lambda t: t * t * t,
            EasingFunction.EASE_OUT_CUBIC: lambda t: 1 - (1 - t) * (1 - t) * (1 - t),
            EasingFunction.EASE_IN_OUT_CUBIC: lambda t: (
                4 * t * t * t if t < 0.5 else 1 - 4 * (1 - t) * (1 - t) * (1 - t)
            ),
        }
...
def main():
    ...
    pan_movement = create_pan_movement(
        start_angle=0.0, end_angle=30.0, duration=2.0, easing=EasingFunction.EASE_IN_OUT
    )

    result = camera_system.apply_movement(test_frames, pan_movement, frame_rate=24.0)

    if result.success:
        print("[SUCCESS] Camera movement successful")
        print(f"  Movement type: {pan_movement.movement_type.value}")
        print(f"  Frames processed: {len(result.transformed_frames)}")
        print(f"  Processing time: {result.processing_time:.2f}s")
        print(
            f"  Max velocity: {result.movement_metadata['motion_statistics']['max_velocity']:.2f}"
        )
        print(
            f"  Avg velocity: {result.movement_metadata['motion_statistics']['avg_velocity']:.2f}"
        )
    else:
    ...
    zoom_movement = create_zoom_movement(
        start_zoom=1.0, end_zoom=1.5, duration=2.0, easing=EasingFunction.EASE_IN_OUT
    )

    compound = CompoundMovement(
        movements=[pan_movement, zoom_movement], blend_mode="additive"
    )

    compound_result = camera_system.apply_compound_movement(
        test_frames, compound, frame_rate=24.0
    )

    if compound_result.success:
        print("[SUCCESS] Compound movement successful")
