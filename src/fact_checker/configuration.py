...
class ConfigurationManager:
    ...
    def __init__(
        ...
        """
        self.environment = (
            environment if environment in self.VALID_ENVIRONMENTS else "production"
        )
    ...
    def _build_config_with_defaults(self, raw_config: Dict[str, Any]) -> Configuration:
        ...
        max_concurrent_verifications = self._validate_positive_int(
            raw_config.get(
                ...
                default_config.max_concurrent_verifications,
            ),
            "max_concurrent_verifications",
        )
    ...
    def _validate_confidence_threshold(self, value: Any) -> float:
        ...
        if not isinstance(value, (int, float)):
            logger.warning(
                f"confidence_threshold: Expected number, got {type(value).__name__}. Using default: {default}"
            )
            return default

        if not 0 <= value <= 100:
            logger.warning(
                f"confidence_threshold: Value {value} out of range [0, 100]. Using default: {default}"
            )
            return default

        return float(value)

    def _validate_risk_level_mappings(
        self, value: Any
        ...
        if not isinstance(value, dict):
            logger.warning(
                f"risk_level_mappings: Expected object, got {type(value).__name__}. Using defaults"
            )
            return default
        ...
        for level, range_tuple in value.items():
            if not isinstance(range_tuple, (list, tuple)) or len(range_tuple) != 2:
                logger.warning(
                    f"risk_level_mappings.{level}: Expected [min, max] tuple. Skipping"
                )
                continue
            ...
            if not isinstance(min_val, (int, float)) or not isinstance(
                ...
                logger.warning(
                    f"risk_level_mappings.{level}: Expected numeric values. Skipping"
                )
                continue

            if not (0 <= min_val <= 100 and 0 <= max_val <= 100 and min_val < max_val):
                logger.warning(
                    f"risk_level_mappings.{level}: Invalid range [{min_val}, {max_val}]. Skipping"
                )
                continue
        ...
        if not valid_mappings:
            logger.warning(
                "risk_level_mappings: No valid mappings found. Using defaults"
            )
            return default
    ...
    def _validate_trusted_sources(self, value: Any) -> Dict[str, List[str]]:
        ...
        if not isinstance(value, dict):
            logger.warning(
                f"trusted_sources: Expected object, got {type(value).__name__}. Using defaults"
            )
            return default
        ...
        for domain, sources in value.items():
            if not isinstance(sources, list):
                logger.warning(
                    f"trusted_sources.{domain}: Expected list, got {type(sources).__name__}. Skipping"
                )
                continue
            ...
            if len(string_sources) != len(sources):
                logger.warning(
                    f"trusted_sources.{domain}: Some non-string sources removed"
                )
    ...
    def _validate_custom_domains(self, value: Any) -> List[str]:
        ...
        if not isinstance(value, list):
            logger.warning(
                f"custom_domains: Expected list, got {type(value).__name__}. Using defaults"
            )
            return default
        ...
        if len(string_domains) != len(value):
            logger.warning("custom_domains: Some non-string domains removed")
    ...
    def _validate_boolean(self, value: Any, field_name: str) -> bool:
        ...
        if not isinstance(value, bool):
            ...
            logger.warning(
                f"{field_name}: Expected boolean, got {type(value).__name__}. Using default: {default}"
            )
            return default
        return value
    ...
    def _validate_positive_int(self, value: Any, field_name: str) -> int:
        ...
        if not isinstance(value, int):
            logger.warning(
                f"{field_name}: Expected integer, got {type(value).__name__}. Using default: {default}"
            )
            return default

        if value <= 0:
            logger.warning(
                f"{field_name}: Expected positive value, got {value}. Using default: {default}"
            )
            return default
