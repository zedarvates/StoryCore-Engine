...
class BatchResult:
    ...
    def __post_init__(self):
        ...
        self.successful_items = [
            item for item in self.items if item.status == BatchItemStatus.COMPLETED
        ]
        self.failed_items = [
            item for item in self.items if item.status == BatchItemStatus.FAILED
        ]
...
class BatchProcessor:
    ...
    def process_batch(
        self, items: List[Dict[str, str]], process_func: Callable[[str], Dict[str, Any]]
    ) -> BatchResult:
        """
        ...
        # Create batch items
        batch_items = [
            BatchItem(id=item["id"], content=item["content"]) for item in items
        ]
    ...
    def _process_item(
        self, item: BatchItem, process_func: Callable[[str], Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
    ...
    def process_files(
        self, file_paths: List[Path], process_func: Callable[[str], Dict[str, Any]]
    ) -> BatchResult:
        """
        ...
        for path in file_paths:
            try:
                with open(path, "r", encoding="utf-8") as f:
                    content = f.read()
                items.append({"id": str(path), "content": content})
            except Exception as e:
