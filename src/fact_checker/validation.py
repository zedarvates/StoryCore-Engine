...
def validate_claim(claim_data: Dict[str, Any]) -> ValidationResult:
    ...
    # Validate position
    if (
        not isinstance(claim_data["position"], (list, tuple))
        or len(claim_data["position"]) != 2
    ):
        result.add_error(
            ValidationError(
                field="position",
...
def validate_evidence(evidence_data: Dict[str, Any]) -> ValidationResult:
    ...
    # Validate source
    if (
        not isinstance(evidence_data["source"], str)
        or len(evidence_data["source"]) == 0
    ):
        result.add_error(
            ValidationError(
                field="source",
    ...
    # Validate excerpt
    if (
        not isinstance(evidence_data["excerpt"], str)
        or len(evidence_data["excerpt"]) == 0
    ):
        result.add_error(
            ValidationError(
                field="excerpt",
...
def validate_scientific_audit_input(input_data: Dict[str, Any]) -> ValidationResult:
    ...
    if (
        "confidence_threshold" in input_data
        and input_data["confidence_threshold"] is not None
    ):
...
def validate_fact_checker_command(command_data: Dict[str, Any]) -> ValidationResult:
    ...
    if not isinstance(command_data["input"], str) or len(command_data["input"]) == 0:
        result.add_error(
            ValidationError(
                ...
                expected="Non-empty string",
                received=type(command_data["input"]).__name__
                if "input" in command_data
                else None,
            )
        )
    ...
    # Validate optional confidence_threshold
    if (
        "confidence_threshold" in command_data
        and command_data["confidence_threshold"] is not None
    ):
        ...
        if not isinstance(mappings, dict):
            ...
            for level in required_levels:
                if level not in mappings:
                    ...
                    not isinstance(mappings[level], (list, tuple))
                    or len(mappings[level]) != 2
                ):
                    result.add_error(
                        ValidationError(
                            field=f"risk_level_mappings.{level}",
