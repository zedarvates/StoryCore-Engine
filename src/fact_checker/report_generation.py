...
def generate_report(
    ...
    recommendations = _generate_recommendations(
        verification_results, manipulation_signals
    )
...
def export_report_markdown(report: Report) -> str:
    ...
    lines.append(
        f"- **Processing Time**: {report.metadata.get('processing_time_ms', 0)}ms"
    )
    lines.append("")
    ...
    for i, result in enumerate(report.claims, 1):
        ...
        if result.supporting_evidence:
            ...
            for ev in result.supporting_evidence:
                lines.append(
                    f"- {ev.source} (credibility: {ev.credibility_score:.0f}%)"
                )
            lines.append("")
    ...
    if report.manipulation_signals:
        ...
        for signal in report.manipulation_signals:
            ...
            if signal.timestamp_start:
                lines.append(
                    f"**Timestamp**: {signal.timestamp_start} - {signal.timestamp_end}"
                )
...
def _generate_human_summary(
    ...
    if avg_confidence >= 80:
        parts.append(
            f"Overall confidence is high ({avg_confidence:.1f}%), indicating well-supported claims."
        )
    elif avg_confidence >= 60:
        parts.append(
            f"Overall confidence is moderate ({avg_confidence:.1f}%), suggesting mixed evidence quality."
        )
    else:
        parts.append(
            f"Overall confidence is low ({avg_confidence:.1f}%), indicating insufficient supporting evidence."
        )
    ...
    if manipulation_signals:
        ...
        if high_severity > 0:
            parts.append(
                f"⚠️ Detected {high_severity} high-severity manipulation signal(s)."
            )
