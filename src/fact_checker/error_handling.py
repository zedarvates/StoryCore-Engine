...
class FactCheckerError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        error_dict = {"error": {"code": self.category.value, "message": self.message}}
...
class ValidationError(FactCheckerError):
    ...
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(
            category=ErrorCategory.VALIDATION_ERROR, message=message, details=details
        )
...
class ConfigurationError(FactCheckerError):
    ...
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(
            category=ErrorCategory.CONFIGURATION_ERROR, message=message, details=details
        )
...
def with_retry(
    config: Optional[RetryConfig] = None, retryable_errors: Optional[tuple] = None
) -> Callable:
    """
...
def handle_error(
    ...
    if isinstance(error, FactCheckerError):
        ...
        fact_error = ValidationError(message=str(error), details=context)
    elif isinstance(error, Exception):
        fact_error = ProcessingError(message=str(error), details=context)
    else:
...
def graceful_degradation(
    fallback_value: T, error_message: str = "Operation failed, using fallback"
