...
def apply_safety_constraints(
    report: Report, config: Optional[Dict[str, Any]] = None
) -> Report:
    """
...
def _filter_verification_result(
    result: VerificationResult, config: Optional[Dict[str, Any]] = None
) -> VerificationResult:
    """
...
def _filter_manipulation_signal(
    signal: ManipulationSignal, config: Optional[Dict[str, Any]] = None
) -> ManipulationSignal:
    """
...
def _filter_political_judgments(text: str) -> str:
    ...
    for pattern in POLITICAL_JUDGMENT_PATTERNS:
        # Replace with neutral language
        filtered = re.sub(
            pattern, "content with political themes", filtered, flags=re.IGNORECASE
        )
...
def _generate_enhanced_disclaimer(
    report: Report, config: Optional[Dict[str, Any]] = None
) -> str:
    """
...
def add_uncertainty_language(
    text: str, confidence: float, threshold: float = 70.0
) -> str:
    """
    ...
    if confidence < 30:
        ...
        uncertainty_note = " (Confidence: {:.1f}% - insufficient evidence for reliable conclusion)".format(
            confidence
        )
    elif confidence < 50:
        uncertainty_prefix = "⚠️ UNCERTAIN: "
        uncertainty_note = (
            " (Confidence: {:.1f}% - limited supporting evidence)".format(confidence)
        )
    else:
        uncertainty_prefix = "Note: "
        uncertainty_note = " (Confidence: {:.1f}% - below standard threshold)".format(
            confidence
        )
...
def apply_uncertainty_handling(
    report: Report, confidence_threshold: float = 70.0
) -> Report:
    """
    ...
    for result in report.claims:
        if result.confidence < confidence_threshold:
            ...
            enhanced_reasoning = add_uncertainty_language(
                result.reasoning, result.confidence, confidence_threshold
            )

            # Add uncertainty to recommendation
            enhanced_recommendation = add_uncertainty_language(
                result.recommendation, result.confidence, confidence_threshold
            )
    ...
    for signal in report.manipulation_signals:
        if signal.confidence < confidence_threshold:
            ...
            enhanced_description = add_uncertainty_language(
                signal.description, signal.confidence, confidence_threshold
            )
...
def check_uncertainty_compliance(
    report: Report, threshold: float = 70.0
) -> Dict[str, Any]:
    """
    ...
    for i, result in enumerate(report.claims):
        if result.confidence < threshold:
            ...
            has_uncertainty = any(
                ...
                or keyword in result.recommendation.lower()
    ...
    for i, signal in enumerate(report.manipulation_signals):
        if signal.confidence < threshold:
            ...
            has_uncertainty = any(
                keyword in signal.description.lower()
                for keyword in uncertainty_keywords
            )
    ...
    if avg_confidence < threshold:
        ...
        summary_has_uncertainty = any(
            keyword in report.human_summary.lower() for keyword in uncertainty_keywords
        )

    return {
        "is_compliant": len(missing_uncertainty) == 0
        and (avg_confidence >= threshold or summary_has_uncertainty),
        ...
        "summary_has_uncertainty": summary_has_uncertainty
        if avg_confidence < threshold
        else None,
    }
...
def get_safety_report(report: Report) -> Dict[str, Any]:
    ...
    if not is_safe:
        all_violations.extend([("summary", v) for v in violations])
    ...
    for i, result in enumerate(report.claims):
        ...
        if not is_safe:
            all_violations.extend(
                [(f"claim_{i}_recommendation", v) for v in violations]
            )
