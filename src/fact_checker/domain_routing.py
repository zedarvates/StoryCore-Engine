...
def classify_domain(claim: Claim, config: Optional[Configuration] = None) -> str:
    ...
    for domain_type in DomainType:
        ...
        scores[domain_type.value] = _calculate_domain_score(
            claim.text, domain_type, custom_domains.get(domain_type.value, {})
        )
...
def classify_domains_batch(
    claims: List[Claim], config: Optional[Configuration] = None
) -> List[str]:
    """
...
def _calculate_domain_score(
    text: str, domain_type: DomainType, custom_keywords: Dict[str, List[str]]
) -> float:
    """
...
def get_domain_confidence(
    claim: Claim, assigned_domain: str, config: Optional[Configuration] = None
) -> float:
    """
