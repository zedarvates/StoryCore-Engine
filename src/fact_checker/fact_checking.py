...
def verify_claim(
    claim: Claim, evidence: List[Evidence], config: Optional[Configuration] = None
) -> VerificationResult:
    """
...
def _classify_evidence(
    claim: Claim, evidence: List[Evidence]
...
def _generate_reasoning(
    ...
    if contradicting:
        avg_credibility = sum(e.credibility_score for e in contradicting) / len(
            contradicting
        )
        parts.append(
    ...
    if supporting:
        ...
        if high_quality:
            parts.append(
                f"{len(high_quality)} high-quality academic or government sources support this claim."
            )
...
def filter_by_risk_level(
    results: List[VerificationResult], risk_levels: List[str]
) -> List[VerificationResult]:
    """
