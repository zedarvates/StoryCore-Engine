...
class ScientificAuditAgent:
    ...
    def analyze(self, text: str, domain_hint: Optional[str] = None) -> Report:
        ...
        report = self._generate_report(
            verification_results, preprocessed_text, processing_time_ms
        )
    ...
    def _evaluate_claims(self, claims: List[Claim]) -> List[VerificationResult]:
        ...
        for i, claim in enumerate(claims, 1):
            print(
                f"[Scientific Audit Agent] Evaluating claim {i}/{len(claims)}: {claim.text[:50]}..."
            )
    ...
    def _generate_report(
        ...
        """
        report = generate_report(verification_results, original_text, format="json")
    ...
    def analyze_batch(
        self, texts: List[str], domain_hints: Optional[List[str]] = None
    ) -> List[Report]:
        """
