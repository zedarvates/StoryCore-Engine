...
class AntiFakeVideoAgent:
    ...
    def analyze(
        ...
        manipulation_signals = self._detect_manipulation_signals(
            parsed_data["text"], parsed_data["segments"]
        )
        ...
        integrity_score = self._score_integrity(
            parsed_data["text"], manipulation_signals, coherence_score
        )

        # Step 5: Risk Assessment - Assign risk level
        risk_level = self._assess_risk(
            manipulation_signals, coherence_score, integrity_score
        )
    ...
    def _parse_transcript(
        ...
        print(
            f"[Anti-Fake Video Agent] Parsed transcript: {len(text)} chars, {len(segments)} segments"
        )
    ...
    def _parse_timestamp_segments(
        self, text: str, timestamps: Optional[List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for pattern1, pattern2 in contradiction_patterns:
            if re.search(pattern1, text_lower) and re.search(pattern2, text_lower):
                ...
                if match1 and match2:
                    ...
                    if segments:
                        segment_idx = self._find_segment_for_position(
                            match1.start(), text, segments
                        )
    ...
    def _detect_narrative_bias(
        ...
        alternative_count = sum(
            1 for indicator in alternative_indicators if indicator in text_lower
        )
    ...
    def _find_segment_for_position(
        self, char_position: int, text: str, segments: List[Dict[str, Any]]
    ) -> Optional[int]:
        """
    ...
    def _analyze_coherence(self, text: str, segments: List[Dict[str, Any]]) -> float:
        """
    ...
    def _score_integrity(
        ...
        text_lower = text.lower()
        citation_count = sum(
            1 for pattern in citation_patterns if re.search(pattern, text_lower)
        )
    ...
    def _assess_risk(
        ...
        high_severity_count = sum(
            1 for s in manipulation_signals if s.severity == "high"
        )
    ...
    def _generate_report(
        ...
        problematic_segments = self._identify_problematic_segments(
            manipulation_signals, parsed_data["segments"]
        )

        # Generate human summary
        human_summary = self._generate_human_summary(
            manipulation_signals, coherence_score, integrity_score, risk_level
        )
    ...
    def _get_segment_recommendation(self, signal: ManipulationSignal) -> str:
        ...
        if signal.type == "logical_inconsistency":
            ...
            return (
                "Consider using more neutral language and reducing emotional appeals."
            )
        elif signal.type == "narrative_bias":
            return (
                "Add alternative perspectives and balance the narrative presentation."
            )
        else:
    ...
    def _generate_human_summary(
        ...
        summary_parts.append(
            f"- Manipulation Signals Detected: {len(manipulation_signals)}\n"
        )
        ...
        if manipulation_signals:
            ...
            for signal_type, signals in by_type.items():
                ...
                summary_parts.append(
                    f"- {type_name}: {len(signals)} instance(s) detected"
                )
        else:
            summary_parts.append("**Risk Highlights:**")
