...
class FactCheckerCommand:
    ...
    def _load_input(self, input_data: Union[str, Path]) -> str:
        ...
        if isinstance(input_data, Path) or (
            ...
            and (input_data.endswith(".txt") or input_data.endswith(".json"))
        ):
    ...
    def _validate_confidence_threshold(self, threshold: float) -> None:
        ...
        if not 0 <= threshold <= 100:
            raise ValueError(
                f"Confidence threshold must be between 0 and 100. Provided: {threshold}"
            )
    ...
    def _determine_mode(self, content: str, mode: ModeType) -> Literal["text", "video"]:
        ...
        if mode == "text":
            ...
            raise ValueError(
                f"Invalid mode: {mode}. Must be 'text', 'video', or 'auto'"
            )
    ...
    def _auto_detect_input_type(self, content: str) -> Literal["text", "video"]:
        ...
        keyword_matches = sum(
            1 for keyword in transcript_keywords if keyword in content_lower
        )
        ...
        if timestamp_matches >= 3 or keyword_matches >= 2:
            print(
                f"[Fact Checker Command] Auto-detected input type: video (timestamps: {timestamp_matches}, keywords: {keyword_matches})"
            )
            return "video"

        # Default to text for general content
        print("[Fact Checker Command] Auto-detected input type: text")
        return "text"
    ...
    def _route_to_agent(self, content: str, mode: Literal["text", "video"]) -> Report:
        ...
        """
        if mode == "text":
            print("[Fact Checker Command] Routing to Scientific Audit Agent")
            return self.scientific_agent.analyze(content)
        else:  # mode == "video"
            print("[Fact Checker Command] Routing to Anti-Fake Video Agent")
    ...
    def _apply_detail_level(
        self, report: Report, detail_level: DetailLevelType
    ) -> Report:
        """
    ...
    def _apply_output_format(
        self, report: Report, output_format: OutputFormatType
    ...
    def _claim_to_dict(self, claim) -> Dict[str, Any]:
        ...
        return {
            ...
            "evidence": [
                {"source": e.source, "relevance": e.relevance, "excerpt": e.excerpt}
    ...
    def _format_error_response(
        self, error_message: str, processing_time_ms: int
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "status": "error",
            "error": {"message": error_message, "code": "PROCESSING_ERROR"},
            "processing_time_ms": processing_time_ms,
        }
