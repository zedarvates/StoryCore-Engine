...
class PipelineIntegration:
    ...
    def configure_hook(self, hook_stage: HookStage, config: HookConfig) -> None:
        """
            ...
            # For blocking hooks, execute synchronously
                ...
                if should_block:
                    warning_event = self._create_warning_event(
                        result=result, hook_stage=hook_stage
                    )
    ...
    async def _verify_content_async(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                self.executor, self._run_fact_checking, content, hook_config
            )

            # Store results if configured
            if hook_config.store_results and self.project_path:
                await self._store_results(
                    result=result, hook_stage=hook_stage, metadata=metadata
                )
    ...
    def _run_fact_checking(
        self, content: str, hook_config: HookConfig
    ) -> Dict[str, Any]:
        """
    ...
    async def _update_project_json(
        self, hook_stage: HookStage, result_filepath: Path
    ) -> None:
        """
        Update project.json with fact-checking status.
        ...
        try:
            ...
            if "fact_checking" not in project_data:
                project_data["fact_checking"] = {"enabled": True, "hooks": {}}
    ...
    def _should_block_pipeline(
        self, result: Dict[str, Any], hook_config: HookConfig
    ) -> bool:
        """
    ...
    def _create_warning_event(
        self, result: Dict[str, Any], hook_stage: HookStage
    ) -> WarningEvent:
        """
