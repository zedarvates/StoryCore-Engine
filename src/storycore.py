...
def initialize_comfyui_integration():
    ...
    try:
        ...
        if os.getenv("COMFYUI_AUTO_DOWNLOAD"):
            _comfyui_config.auto_download_models = (
                os.getenv("COMFYUI_AUTO_DOWNLOAD").lower() == "true"
            )
        ...
        logger.debug(
            f"ModelManager initialized with models directory: {comfyui_models_dir}"
        )
...
def shutdown_comfyui_integration():
    ...
    try:
        ...
        if _generation_engine and _generation_engine.current_session:
            ...
            try:
                state_file = (
                    Path.home() / ".storycore" / "state" / "generation_state.json"
                )
