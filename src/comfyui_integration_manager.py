...
from .comfyui_config import ConfigManager
from .platform_manager import PlatformManager
from .integration_utils import (
    load_workflow,
    inject_storycore_parameters,
    create_panel_workflow,
    process_comfyui_outputs,
    validate_workflow_structure,
    optimize_workflow_for_memory,
)
...
class ComfyUIIntegrationManager:
    ...
    def __init__(
        self, base_url: str = "http://127.0.0.1:8188", workflow_path: str = None
    ):
        ...
        try:
            ...
            logger.info(
                f"Dynamic VRAM detection: {self.available_vram:.2f} GB available"
            )
        except Exception as e:
            logger.warning(
                f"Failed to initialize PlatformManager: {e}. Using fallback VRAM estimate."
            )
            ...
            if self.platform_manager is not None:
                ...
                if gpu_info.get("gpu_info"):
                    ...
                    if max_available > 0:
                        ...
                        logger.info(
                            f"Detected {vram_gb:.2f} GB available VRAM from GPU detection"
                        )
                        return vram_gb
    ...
    def process_project(
        self, project_data: Dict[str, Any], progress_callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            optimized_workflow = optimize_workflow_for_memory(
                self.base_workflow, self.available_vram
            )
            ...
            results = {}
            if (
                "coherence_anchors" in project_data
                and "master_grid_path" in project_data["coherence_anchors"]
            ):
                results = self._process_master_grid(
                    project_workflow, project_data, progress_callback
                )
    ...
    def process_single_panel(
        ...
        try:
            ...
            panel_workflow = create_panel_workflow(
                self.base_workflow, panel_id, prompt, global_seed
            )
            ...
            if result["status"] == "completed":
                ...
                processed_outputs = process_comfyui_outputs(
            ...
            return self._process_panel_with_reduced_batch(
                panel_id, prompt, global_seed, progress_callback
            )
        except Exception as e:
    ...
    def _process_master_grid(
        ...
        try:
            project_data["coherence_anchors"]["master_grid_path"]
            ...
            prompt_id = self.client.queue_workflow(
                grid_workflow, global_seed, "master coherence sheet"
            )
            if not prompt_id:
    ...
    def _handle_vram_overflow(
        ...
        try:
            ...
            return self._process_master_grid(
                project_workflow, project_data, progress_callback
            )
    ...
    def _process_panel_with_reduced_batch(
        self,
        panel_id: str,
        prompt: str,
        global_seed: int,
        ...
        try:
            ...
            reduced_workflow = optimize_workflow_for_memory(
                self.base_workflow, 4.0
            )  # Very conservative
            panel_workflow = create_panel_workflow(
                reduced_workflow, panel_id, prompt, global_seed
            )
