...
try:
    ...
    raise ImportError(
        "numpy est requis pour les calculs vectorisés de cohérence temporelle"
    )

from .types import GridFormat, CoherenceMetrics
...
class Panel:
    ...
    dominant_colors: List[
        Tuple[int, int, int]
    ]  # Max 3 couleurs pour optimisation mémoire
    brightness: float
    contrast: float
...
class TemporalCoherenceEngine:
    ...
    def __init__(
        ...
        autofix_rules_path: Optional[str] = None,
    ):
        """
    ...
    def optimize_panel_transitions(
        ...
        for i, panel in enumerate(panels):
            ...
            if coherence_score < self.coherence_threshold:
                ...
                optimized_panel = self._enhance_temporal_coherence(
    ...
    def calculate_coherence_metrics(self, panels: List[Panel]) -> CoherenceMetrics:
        ...
        for i in range(len(panels) - 1):
            ...
            color_smooth = self._calculate_color_transition_smoothness(
            ...
            lighting_coherence = self._calculate_lighting_coherence(
                current_panel, next_panel
            )
    ...
    def ensure_visual_continuity(self, panels: List[Panel]) -> ContinuityReport:
        ...
        if metrics.color_transition_smoothness < 0.7:
            ...
            recommendations.append(
                "Ajuster la palette de couleurs pour plus de fluidité"
            )
    ...
    def trigger_autofix_if_needed(
        self, coherence_score: float, panel_id: str = "temporal_coherence"
    ) -> Optional[AutofixAction]:
        """
        ...
        if self.autofix_engine is not None:
            ...
            adapted_metric = int(
                coherence_score * 255
            )  # Cohérence 0.85 -> 216.75, proche de la netteté acceptable
            ...
            needs_retry, adjustments = self.autofix_engine.should_retry(
                panel_id, adapted_qa_metrics
            )
            ...
            if needs_retry:
                ...
                expected_improvement = min(
                    0.3, (self.coherence_threshold - coherence_score) * 0.5
                )
                ...
                return AutofixAction(
                    ...
                    parameters={
                        "coherence_adjustment": adjustments.get(
    ...
    def get_optimization_statistics(self) -> Dict[str, float]:
        ...
        enhanced_optimizations = [
            opt for opt in self._optimization_history if opt["action"] == "enhanced"
        ]

        if enhanced_optimizations:
            avg_before = sum(
                opt["coherence_before"] for opt in enhanced_optimizations
            ) / len(enhanced_optimizations)
            avg_after = sum(
                opt["coherence_after"] for opt in enhanced_optimizations
            ) / len(enhanced_optimizations)
            improvement_rate = len(enhanced_optimizations) / len(
                self._optimization_history
            )
        else:
        ...
        return {
            ...
            "autofix_trigger_rate": self._autofix_triggers
            / max(1, len(self._optimization_history)),
        }
    ...
    def _calculate_panel_coherence(self, panel1: Panel, panel2: Panel) -> float:
        ...
        color_coherence = self._calculate_color_coherence(
            panel1.dominant_colors, panel2.dominant_colors
        )
        ...
        style_coherence = self._calculate_style_coherence(
            panel1.style_features, panel2.style_features
        )
    ...
    def _enhance_temporal_coherence(
        self, previous_panel: Panel, current_panel: Panel
    ) -> Panel:
    ...
    def _calculate_color_coherence(
        ...
        try:
            colors1_array = np.array(
                colors1[:3], dtype=np.float32
            )  # Top 3 couleurs max
            ...
            for color1 in colors1[:3]:
                distances = [
                    math.sqrt(sum((c1 - c2) ** 2 for c1, c2 in zip(color1, color2)))
        ...
        max_distance = np.sqrt(
            3 * 255**2
        )  # Distance maximale possible dans l'espace RGB
    ...
    def _calculate_style_coherence(
    ...
    def _blend_colors(
        ...
        for i in range(max_colors):
            ...
            blended_color = tuple(
                int(c2 + (c1 - c2) * blend_factor) for c1, c2 in zip(color1, color2)
            )
    ...
    def _record_optimization(
        self, panel_id: str, coherence_score: float, action: str
    ) -> None:
        ...
        self._optimization_history.append(
            {
                ...
                "coherence_after": min(1.0, coherence_score + 0.1)
                if action == "enhanced"
