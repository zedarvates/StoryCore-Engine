...
class QualityPredictor:
    ...
    def predict_quality_metrics(
        self, format_type: GridFormat, content: ContentAnalysis
    ) -> QualityPrediction:
        """
        ...
        try:
            ...
            # Estimation du temps
            estimated_time = self._estimate_processing_time_detailed(
                format_type, content
            )
            ...
            raise QualityPredictionError(
                f"Erreur de prédiction: {str(e)}", format_type.value
            )
    ...
    def estimate_processing_time(
        ...
        # Qualité par seconde
        predicted_quality = self.predict_quality_metrics(
            format_type, content
        ).predicted_quality_score
        quality_per_second = (
            predicted_quality / estimated_time if estimated_time > 0 else 0
        )
    ...
    def calculate_quality_performance_ratio(
        self, prediction: QualityPrediction
    ) -> float:
        """
    ...
    def update_prediction_models(
        ...
        error = abs(
            actual_results.overall_quality_score - prediction.predicted_quality_score
        )
        ...
        if error > 10.0:
            ...
            self._record_performance_data(
                format_type, content, actual_results, prediction
            )
    ...
    def get_prediction_accuracy(self) -> Dict[GridFormat, float]:
        ...
        for format_type in GridFormat:
            format_history = [
                h for h in self._performance_history if h["format"] == format_type
            ]
    ...
    def _calculate_content_adjustment(
        self, format_type: GridFormat, content: ContentAnalysis
    ) -> float:
        ...
        if format_type == GridFormat.SQUARE_3X3 and content.scene_complexity > 0.7:
            ...
            complexity_adjustment = (
                0.1  # Formats linéaires meilleurs pour scènes simples
            )

        return content_bonus + temporal_adjustment + complexity_adjustment

    def _estimate_processing_time_detailed(
        self, format_type: GridFormat, content: ContentAnalysis
    ) -> float:
    ...
    def _adjust_baseline_metrics(
        self, format_type: GridFormat, actual_results: QualityMetrics, error: float
    ) -> None:
