...
try:
    ...
    raise ImportError(
        f"Dependencies manquantes pour l'analyse de qualité spécialisée: {e}"
    )
...
from .exceptions import GridFormatError, QualityAnalysisError, MetricCalculationError
...
class SpecializedQualityAnalyzer:
    ...
    def __init__(
        self,
        baseline_format: GridFormat = GridFormat.SQUARE_3X3,
        ...
        self._downsample_factor = (
            0.5  # Facteur de sous-échantillonnage pour performance
        )

    def analyze_format_specific_quality(
        self, panels: List[Panel], format_type: GridFormat
    ...
    def compare_format_performance(
        self, results: Dict[GridFormat, FormatQualityMetrics]
    ) -> ComparisonReport:
        """
        ...
        if self.baseline_format not in results:
            raise GridFormatError(
                f"Format de référence {self.baseline_format.value} manquant"
            )
        ...
        for format_type, metrics in results.items():
            ...
            improvement = (
                (metrics.format_specific_score - baseline_score) / baseline_score
            ) * 100.0
            ...
            performance_comparisons[format_type.value] = {
                ...
                "processing_time": metrics.performance_metrics.processing_time
                if metrics.performance_metrics
                else 0.0,
                "quality_per_second": metrics.performance_metrics.quality_per_second
                if metrics.performance_metrics
                else 0.0,
        ...
        recommendations = self._generate_comparison_recommendations(
            quality_improvements, performance_comparisons
        )
    ...
    def generate_quality_report(self, metrics: FormatQualityMetrics) -> QualityReport:
        ...
        self._classify_quality_level(overall_score)
        ...
        if hasattr(self, "_baseline_score"):
            baseline_comparison = (
                (overall_score - self._baseline_score) / self._baseline_score
            ) * 100.0
    ...
    def maintain_performance_history(self, results: FormatQualityMetrics) -> None:
        ...
        history_entry = {
            ...
            "temporal_coherence": results.temporal_coherence.temporal_coherence_score
            if results.temporal_coherence
            else None,
        ...
        if len(self._performance_history[format_key]) > 50:
            self._performance_history[format_key] = self._performance_history[
                format_key
            ][-50:]
    ...
    def get_performance_trends(self, format_type: GridFormat) -> Dict[str, Any]:
        ...
        if (
            format_key not in self._performance_history
            or not self._performance_history[format_key]
        ):
    ...
    def export_analysis_report(
        self, results: Dict[GridFormat, FormatQualityMetrics], output_path: str
    ) -> None:
        """
        ...
        individual_reports = {}
        for format_type, metrics in results.items():
            individual_reports[format_type.value] = asdict(
        ...
        for format_type in results.keys():
            performance_trends[format_type.value] = self.get_performance_trends(
                format_type
            )
        ...
        final_report = {
            ...
            "summary": self._generate_executive_summary(
                comparison_report, individual_reports
            ),
        }
        ...
        try:
            ...
            if not panel_metrics:
                raise MetricCalculationError(
                    "base_quality", "Aucune métrique valide calculée"
                )
            ...
            laplacian_variance = float(
                scipy.stats.trim_mean(laplacian_values, 0.1)
            )  # Trim 10% outliers
            ...
            color_coherence = self._calculate_color_coherence_across_panels(
                panel_metrics
            )
            ...
            raise QualityAnalysisError(
                f"Erreur calcul métriques de base: {e}", "base_quality"
            )

    def _calculate_temporal_coherence_metrics(
    ...
    def _calculate_spatial_coherence(self, panels: List[Panel]) -> float:
        ...
        for i, (row, col) in enumerate(grid_positions):
            ...
            for dr, dc in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                ...
                if 0 <= new_row < 3 and 0 <= new_col < 3:
                    ...
                    brightness_coherence = 1.0 - abs(
                        panel.brightness - adj_panel.brightness
                    )
        ...
        return (
            (sum(coherence_scores) / len(coherence_scores)) * 100.0
            if coherence_scores
            else 50.0
        )
    ...
    def _calculate_performance_metrics(
        ...
        # Qualité par seconde
        quality_score = (
            sum(p.brightness * 100 for p in panels) / len(panels) if panels else 0
        )
        quality_per_second = (
            quality_score / processing_time if processing_time > 0 else 0
        )
    ...
    def _analyze_strengths_weaknesses(
        self, metrics: FormatQualityMetrics
        ...
        if metrics.format_type.is_linear:
            if (
                ...
                and metrics.temporal_coherence.temporal_coherence_score > 0.8
            ):
                strengths.append("Excellente cohérence temporelle")
            elif (
                metrics.temporal_coherence
                and metrics.temporal_coherence.temporal_coherence_score < 0.6
            ):
    ...
    def _generate_improvement_suggestions(
        self, metrics: FormatQualityMetrics, weaknesses: List[str]
    ) -> List[str]:
        ...
        for weakness in weaknesses:
            if "netteté" in weakness.lower():
                suggestions.append(
                    "Appliquer un filtre de netteté ou ajuster les paramètres d'upscaling"
                )
                ...
                suggestions.append(
                    "Optimiser les paramètres de traitement pour améliorer l'efficacité"
                )

        # Suggestions générales
        if metrics.format_specific_score < 70:
            suggestions.append(
                "Considérer l'utilisation d'un format alternatif mieux adapté au contenu"
            )
    ...
    def _generate_comparison_recommendations(
        ...
        best_format = max(
            quality_improvements.keys(), key=lambda k: quality_improvements[k]
        )
        best_improvement = quality_improvements[best_format]

        if best_improvement > 15.0:
            recommendations.append(
                f"Recommandation forte: Utiliser le format {best_format} "
                f"(amélioration de {best_improvement:.1f}%)"
            )
        elif best_improvement > 5.0:
            recommendations.append(
                f"Recommandation modérée: Considérer le format {best_format} "
                f"(amélioration de {best_improvement:.1f}%)"
            )

        # Analyse des performances
        fastest_format = min(
            performance_comparisons.keys(),
            key=lambda k: performance_comparisons[k]["processing_time"],
        )

        if (
            performance_comparisons[fastest_format]["processing_time"] < 120.0
        ):  # < 2 minutes
            recommendations.append(
                f"Pour un traitement rapide: Format {fastest_format} recommandé"
            )

        return recommendations

    def _generate_executive_summary(
        self, comparison_report: ComparisonReport, individual_reports: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Génère un résumé exécutif."""
        # Format avec la meilleure amélioration
        best_format = max(
            comparison_report.quality_improvements.keys(),
        ...
        if best_improvement > 15.0:
            ...
            main_recommendation = (
                f"Évaluation approfondie du format {best_format} recommandée"
            )
        else:
