...
class FormatSelector:
    ...
    def evaluate_formats(
        self, content_type: str, content: ContentAnalysis
        ...
        for format_type in GridFormat:
            evaluation = self._evaluate_single_format(
                format_type, content_type, content
            )
    ...
    def select_optimal_format(
        self, content_analysis: ContentAnalysis, preferences: FormatPreferences
    ) -> FormatRecommendation:
        """
        ...
        if preferences.preferred_formats:
            evaluations = [
                e for e in evaluations if e.format_type in preferences.preferred_formats
            ]
        ...
        if best_evaluation.score < preferences.minimum_quality_threshold / 100.0:
            ...
            fallback_eval = next(
                (e for e in evaluations if e.format_type == GridFormat.SQUARE_3X3), None
            )
            if fallback_eval:
        ...
        equivalent_formats = [
            e for e in evaluations if abs(e.score - best_evaluation.score) < 0.05
        ]
        ...
        alternatives = [
            (e.format_type, e.score) for e in evaluations[1:4]
        ]  # Top 3 alternatives

        return FormatRecommendation(
            recommended_format=best_evaluation.format_type,
            confidence_score=best_evaluation.score,
            predicted_quality_improvement=self._calculate_improvement(
                best_evaluation, content_type
            ),
            estimated_processing_time=self._estimate_processing_time(
                best_evaluation.format_type
            ),
            justification=self.get_format_justification(
                best_evaluation.format_type, content_type
            ),
            alternatives=alternatives,
        )

    def get_format_justification(
        self, format_type: GridFormat, content_type: str = None
    ) -> str:
        """
    ...
    def _calculate_complexity_score(
        self, format_type: GridFormat, complexity: float
    ) -> float:
        """Calcule le score basé sur la complexité de la scène."""
        FORMAT_SPECIFICATIONS[format_type]
    ...
    def _calculate_motion_score(
        self, format_type: GridFormat, motion_intensity: float
    ) -> float:
    ...
    def _calculate_temporal_score(
        self, format_type: GridFormat, temporal_requirements: bool
    ) -> float:
    ...
    def _select_most_efficient(
        self, equivalent_formats: List["FormatEvaluation"]
    ) -> "FormatEvaluation":
        """Sélectionne le format le plus efficace parmi des formats équivalents."""
        return min(
            equivalent_formats,
            key=lambda e: FORMAT_SPECIFICATIONS[e.format_type].processing_complexity,
        )

    def _calculate_improvement(
        self, evaluation: "FormatEvaluation", content_type: str
    ) -> float:
        """Calcule l'amélioration prédite par rapport au format 3x3."""
        baseline_score = self._content_type_weights.get(content_type, {}).get(
            GridFormat.SQUARE_3X3, 0.5
        )
