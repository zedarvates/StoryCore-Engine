...
class StoryCoreAssetGenerator:
            ...
            "nodes": [
                ...
                },
                {
                    ...
                    "widgets_values": [
                        ...
                        height,
                        0.5,
                        seed if seed >= 0 else int(time.time()),
                    ],
                },
                {
                    "id": 6,
                ...
                },
                {
                    ...
                    "widgets_values": ["StoryCore_Asset"],
                },
            ],
            "links": [
                [1, 3, 0, 1, 0, "CLIP"],
    ...
    async def download_image(
        self, image_info: Dict[str, Any], output_path: Path
    ) -> bool:
    ...
    async def generate_asset(self, asset_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.mock_mode:
            ...
            image_data = create_mock_image(
                asset_def["width"], asset_def["height"], asset_name
            )
    ...
    def save_generation_report(self, results: List[Dict[str, Any]]) -> None:
        ...
        report = {
            "generation_summary": {
                ...
                "average_time": sum(r.get("generation_time", 0) for r in results)
                / len(results)
                if results
                else 0,
...
async def main():
    ...
    mock_flag = (
        "--mock" in sys.argv or os.environ.get("MOCK_MODE", "").lower() == "true"
    )
