...
try:
    ...
    async def call_llm_real(req, uid):
        return type("obj", (object,), {"text": f"Translated: {req.prompt}"})
...
class TranscriptionEngine:
    ...
    async def _perform_transcription(
        ...
        self.logger.info(
            f"Language: {language or 'auto-detect'}, Diarization: {enable_speaker_diarization}"
        )
        ...
        self.logger.info(
            f"Transcription complete: {len(mock_segments)} segments, {transcript.word_count} words"
        )
        return transcript
    ...
    async def translate_transcript(
        ...
        self.logger.info(
            f"Translating transcript {transcript.transcript_id} to {target_language}"
        )
        ...
        for segment in transcript.segments:
            ...
            try:
                ...
                self.logger.error(
                    f"Translation failed for segment {segment.segment_id}: {e}"
                )
    ...
    def _generate_mock_transcript(self, audio_id: str) -> List[TranscriptSegment]:
        ...
        for i, dialogue in enumerate(dialogues):
            words = self._generate_word_timestamps(
                dialogue["text"], dialogue["start"], dialogue["end"]
            )
    ...
    async def _perform_montage_generation(
        self, request: MontageRequest
    ) -> MontageResult:
        ...
        self.logger.info(
            f"Montage generated: {len(shots)} shots, {total_duration:.1f}s total"
        )
        return result

    def _filter_segments(
        self, transcript: Transcript, request: MontageRequest
    ...
    def _generate_chronological_shots(
        self, segments: List[TranscriptSegment], transcript: Transcript
    ) -> List[MontageShot]:
    ...
    def _generate_compact_shots(
        ...
        for segment in segments:
            if current_group is None:
                ...
                segment.speaker
                and current_group["speaker"]
                and segment.speaker.speaker_id == current_group["speaker"].speaker_id
                and segment.start_time - current_group["end"]
                < 2.0  # Moins de 2s de gap
            ):
                ...
                shot = MontageShot(
                    ...
                    speaker=current_group["speaker"].speaker_label
                    if current_group["speaker"]
                    else None,
    ...
    def _trim_to_duration(
        self, shots: List[MontageShot], max_duration: float
    ) -> List[MontageShot]:
