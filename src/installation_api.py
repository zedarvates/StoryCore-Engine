...
async def initialize_installation():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to initialize installation: {str(e)}"
        )
...
async def verify_installation():
    ...
    try:
        ...
        cors_enabled = False
        if (comfyui_dir / "run_with_cors.bat").exists() or (
            comfyui_dir / "run_with_cors.sh"
        ).exists():
            cors_enabled = True
        else:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to verify installation: {str(e)}"
        )
...
async def start_comfyui():
    ...
    try:
        ...
        if not COMFYUI_INSTALL_DIR.exists():
            raise HTTPException(
                status_code=404, detail="ComfyUI installation not found"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start ComfyUI: {str(e)}"
        )
...
async def test_cors():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to test CORS: {str(e)}")
