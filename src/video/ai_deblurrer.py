...
class BlurAnalysis:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"BlurAnalysis(amount={self.blur_amount:.3f}, "
...
class DeblurResult:
    ...
    def __str__(self) -> str:
        return (
            f"DeblurResult(method={self.method_used.value}, "
...
class AIDeblurrer:
    ...
    def _recommend_method(
        self, blur_amount: float, blur_type: BlurType
    ) -> DeblurMethod:
    ...
    def _wiener_deblur(
        self, frame: np.ndarray, strength: float
    ...
    def _calculate_sharpness_improvement(
        self, original: np.ndarray, deblurred: np.ndarray
    ) -> float:
    ...
    def _unsharp_mask(
        self, frame: np.ndarray, kernel_size: int, amount: float
    ) -> np.ndarray:
        ...
        if len(frame.shape) == 3:
            ...
            for c in range(frame.shape[2]):
                blurred[:, :, c] = self._convolve2d(
                    frame[:, :, c].astype(np.float32), kernel
                )
        else:
        ...
        sharpened = frame.astype(np.float32) + amount * (
            frame.astype(np.float32) - blurred
        )
