...
class FrameRateConverter:
    ...
    def convert(
        self, frames: List[np.ndarray], source_fps: float, target_fps: float
        ...
        logger.info(
            f"Converting {source_fps}fps → {target_fps}fps (ratio={conversion_ratio:.2f})"
        )
        ...
        logger.info(
            f"Conversion complete: {len(frames)} → {len(converted)} frames in {processing_time:.2f}s"
        )
    ...
    def _interpolate_at_position(
        self, frames: List[np.ndarray], position: float, settings: Dict
    ) -> np.ndarray:
        ...
        frame = self._multi_pass_blend(
            context_before, context_after, weight, settings["blend_passes"]
        )

        return frame

    def _get_context(
        self, frames: List[np.ndarray], center_idx: int, direction: int, size: int
    ) -> List[np.ndarray]:
        """Get context frames."""
    ...
    def convert_to_common_rates(
        self, frames: List[np.ndarray], source_fps: float, target_format: str
    ) -> FrameRateConversionResult:
        """
    ...
    def create_slow_motion(
        self, frames: List[np.ndarray], source_fps: float, slowdown_factor: float
    ) -> FrameRateConversionResult:
        """
        ...
        logger.info(
            f"Creating slow-motion: {slowdown_factor}x slower ({len(frames)} → {target_count} frames)"
        )
    ...
    def create_time_lapse(
        self, frames: List[np.ndarray], source_fps: float, speedup_factor: float
    ) -> FrameRateConversionResult:
        """
        ...
        logger.info(
            f"Creating time-lapse: {speedup_factor}x faster ({len(frames)} → {target_count} frames)"
        )
    ...
    def get_conversion_info(
        self, source_fps: float, target_fps: float, frame_count: int
    ) -> Dict[str, any]:
    ...
    def _estimate_processing_time(self, source_count: int, target_count: int) -> float:
        """Estimate processing time for conversion."""
        # Rough estimate: 0.01s per output frame for high quality
        quality_multiplier = {"low": 0.005, "medium": 0.01, "high": 0.02}
