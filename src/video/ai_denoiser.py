...
class NoiseAnalysis:
    ...
    def __str__(self) -> str:
        return (
            f"NoiseAnalysis(level={self.noise_level:.3f}, "
...
class DenoiseResult:
    ...
    def __str__(self) -> str:
        return (
            f"DenoiseResult(method={self.method_used.value}, "
...
class AIDenoiser:
    ...
    def _recommend_method(
        self, noise_level: float, noise_type: NoiseType
    ) -> DenoiseMethod:
    ...
    def _edge_preserving_smooth(
        self, frame: np.ndarray, d: int, sigma_color: float, sigma_space: float
    ) -> np.ndarray:
    ...
    def _non_local_means(
        self, frame: np.ndarray, h: float, template_size: int, search_size: int
    ) -> np.ndarray:
