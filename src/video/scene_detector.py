...
class SceneDetector:
    ...
    def _classify_change_type(
        self, frame1: np.ndarray, frame2: np.ndarray, metrics: Dict[str, float]
    ) -> str:
    ...
    def _build_scenes(
        self, frames: List[np.ndarray], changes: List[SceneChange], fps: float
    ) -> List[Scene]:
    ...
    def _analyze_scene_content(self, frames: List[np.ndarray]) -> Dict[str, Any]:
        ...
        sample = all_pixels[
            np.random.choice(len(all_pixels), sample_size, replace=False)
        ]
