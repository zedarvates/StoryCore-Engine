...
class DynamicRangeAnalysis:
    ...
    def __str__(self) -> str:
        return (
            f"DynamicRangeAnalysis(range={self.dynamic_range:.1f} stops, "
...
class ToneMappingResult:
    ...
    def __str__(self) -> str:
        return (
            f"ToneMappingResult(method={self.method_used.value}, "
...
class HDRToneMapper:
    ...
    def compress_dynamic_range(
        self, frame: np.ndarray, target_stops: float = 8.0
    ) -> np.ndarray:
        """
    ...
    def _compress_luminance(
        self, luminance: np.ndarray, compression: float
    ) -> np.ndarray:
    ...
    def _reconstruct_rgb(
        self, original: np.ndarray, original_lum: np.ndarray, target_lum: np.ndarray
    ) -> np.ndarray:
    ...
    def _calculate_compression_ratio(
        self, original: np.ndarray, mapped: np.ndarray
    ) -> float:
