...
class MultiFrameInterpolator:
    ...
    def interpolate_multi(
        ...
        if target_count <= len(frames):
            logger.warning(
                f"Target count ({target_count}) <= input count ({len(frames)})"
            )
            return InterpolationResult(frames, [1.0] * len(frames), "none", 0.0)
        ...
        positions = self._calculate_positions(
            len(frames), target_count, preserve_endpoints
        )
    ...
    def _calculate_positions(
        self, input_count: int, target_count: int, preserve_endpoints: bool
    ) -> List[float]:
    ...
    def _interpolate_at_position(
        self, frames: List[np.ndarray], position: float
        ...
        if self.blend_mode == "weighted":
            frame = self._weighted_blend(context_before, context_after, weight)
        elif self.blend_mode == "optical_flow":
            frame = self._optical_flow_blend(context_before, context_after, weight)
        else:  # adaptive
            frame = self._adaptive_blend(context_before, context_after, weight)
    ...
    def _get_context_frames(
        self, frames: List[np.ndarray], center_idx: int, direction: int
    ) -> List[np.ndarray]:
    ...
    def interpolate_between_keyframes(
        self, keyframes: List[Tuple[int, np.ndarray]], total_frames: int
    ) -> List[np.ndarray]:
        """
        ...
        for i in range(len(keyframes) - 1):
            ...
            if segment_count > 2:
                result = self.interpolate_multi(
                    segment_frames, segment_count, preserve_endpoints=True
                )
    ...
    def get_interpolation_statistics(
        self, result: InterpolationResult
    ) -> Dict[str, any]:
        ...
        return {
            ...
            "low_quality_frames": sum(
                1 for q in result.quality_scores if q < self.quality_threshold
            ),
