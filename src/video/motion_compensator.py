...
class MotionCompensator:
    ...
    def compensate(
        ...
        residual = self._calculate_residual_motion(reference_frame, compensated)
    ...
    def _estimate_translation(
        ...
        transform = np.array([[1, 0, -dx], [0, 1, -dy], [0, 0, 1]], dtype=float)
    ...
    def _estimate_perspective(
        ...
        # Start with affine
        affine, conf_affine = self._estimate_affine(ref_frame, target_frame, flow_field)
    ...
    def _phase_correlation(
        self, ref_frame: np.ndarray, target_frame: np.ndarray
        ...
        # Ensure same size
        h, w = (
            min(gray_ref.shape[0], gray_target.shape[0]),
        ...
        cross_power = (f_ref * np.conj(f_target)) / (
            np.abs(f_ref * np.conj(f_target)) + 1e-10
        )
    ...
    def _apply_transformation(
        self, frame: np.ndarray, transform: np.ndarray
    ) -> np.ndarray:
        ...
        # Copy shifted region
        compensated[dst_y1:dst_y2, dst_x1:dst_x2] = frame[src_y1:src_y2, src_x1:src_x2]
    ...
    def _calculate_residual_motion(
        self, ref_frame: np.ndarray, compensated_frame: np.ndarray
    ) -> float:
        """Calculate remaining motion after compensation."""
        # Simple pixel difference
        diff = np.mean(
            np.abs(ref_frame.astype(float) - compensated_frame.astype(float))
        )

        return float(diff)

    def stabilize_sequence(
        self, frames: List[np.ndarray], reference_index: Optional[int] = None
    ) -> List[np.ndarray]:
        """
    ...
    def get_stabilization_metrics(
        self, original_frames: List[np.ndarray], stabilized_frames: List[np.ndarray]
    ) -> Dict[str, float]:
        ...
        for i in range(1, len(original_frames)):
            orig_diff = np.mean(
                np.abs(
                    original_frames[i].astype(float)
                    - original_frames[i - 1].astype(float)
                )
            )
            stab_diff = np.mean(
                np.abs(
                    stabilized_frames[i].astype(float)
                    - stabilized_frames[i - 1].astype(float)
                )
            )
