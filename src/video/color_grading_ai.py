...
class ColorAnalysis:
    ...
    def __str__(self) -> str:
        ...
        return (
            f"ColorAnalysis(temperature={temp_str}, "
...
class ColorGradingResult:
    ...
    def __str__(self) -> str:
        return (
            f"ColorGradingResult(style={self.style_applied.value}, "
...
class ColorGradingAI:
    ...
    def __init__(
        self, preserve_skin_tones: bool = True, auto_white_balance: bool = False
    ):
        """
    ...
    def apply_style(
        self, frame: np.ndarray, style: ColorGradingStyle, intensity: float = 1.0
    ) -> ColorGradingResult:
        """
    ...
    def apply_lut(
        self, frame: np.ndarray, lut: np.ndarray, intensity: float = 1.0
    ) -> np.ndarray:
        """
        ...
        for i, frame in enumerate(frames):
            ...
            if smooth_transitions and prev_adjustments is not None:
                ...
                blended_adjustments = self._blend_adjustments(
                    prev_adjustments, result.adjustments, alpha=0.7
                )
    ...
    def _find_dominant_colors(
        self, frame: np.ndarray, n_colors: int = 5
    ...
    def _apply_adjustments(
        self, frame: np.ndarray, adjustments: Dict[str, float]
    ) -> np.ndarray:
    ...
    def _adjust_highlights_shadows(
        self, frame: np.ndarray, highlights: float, shadows: float
    ) -> np.ndarray:
        ...
        for c in range(3):
            result[:, :, c] += (
                highlights * 50 * highlight_mask[:, :, np.newaxis]
            ).squeeze()
    ...
    def _blend_adjustments(
        self, adj1: Dict[str, float], adj2: Dict[str, float], alpha: float
    ) -> Dict[str, float]:
