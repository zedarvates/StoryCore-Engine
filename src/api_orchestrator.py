...
class APIOrchestrator:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            fallback_mode = await self.error_handler.handle_error(
                e, {"component": "api_orchestrator", "operation": "connect"}
            )
            ...
            if fallback_mode == FallbackMode.MOCK:
                ...
                self._connected = True
                self.logger.info(
                    "API Orchestrator connected in mock mode due to connection error"
                )
                return True
    ...
    async def _connect_websocket(self) -> bool:
        ...
        try:
            self._websocket = await self._session.ws_connect(
                self.config.websocket_url, timeout=10.0
            )
    ...
    async def _websocket_listener(self) -> None:
        ...
        try:
            ...
            if self._reconnect_attempts < self._max_reconnect_attempts:
                ...
                self.logger.info(
                    f"Attempting WebSocket reconnection ({self._reconnect_attempts}/{self._max_reconnect_attempts})"
                )
    ...
    async def _handle_websocket_message(self, data: Dict[str, Any]) -> None:
        ...
        if message_type == "status":
                    ...
                    if prompt_id in self._execution_callbacks:
                        try:
                            final_update = ExecutionUpdate(
                                prompt_id=prompt_id, status=ExecutionStatus.COMPLETED
                            )
            ...
            if prompt_id and prompt_id in self._active_executions:
                ...
                self.logger.error(
                    f"Execution {prompt_id} failed: {execution.error_message}"
                )
    ...
    async def submit_workflow(
        ...
        self.performance_monitor.record_operation_start(
            "workflow_submission", operation_id
        )
        ...
        try:
            ...
            try:
                async with self._session.post(
                    f"{self.config.server_url}/prompt", json=workflow_data
                ) as response:
                    if response.status == 200:
                        ...
                        self.logger.info(
                            f"Workflow submitted successfully: {actual_prompt_id}"
                        )
                        self.performance_monitor.record_operation_end(
                            "workflow_submission",
                            operation_id,
                        ...
                        http_error = Exception(
                            f"Failed to submit workflow: HTTP {response.status}"
                        )
                        fallback_mode = await self.error_handler.handle_error(
                            http_error,
                            {
                ...
                error_msg = (
                    f"Network error during workflow submission: {str(client_error)}"
                )
                self.logger.error(error_msg)
                self.performance_monitor.record_operation_end(
                    "workflow_submission",
            ...
            if not isinstance(e, Exception) or "Failed to submit workflow" not in str(
                e
            ):
                fallback_mode = await self.error_handler.handle_error(
                    e, {"component": "api_orchestrator", "operation": "submit_workflow"}
                )
            ...
            self.logger.error(error_msg)
            self.performance_monitor.record_operation_end(
                "workflow_submission", operation_id, False, {"error": error_msg}
            )
            raise
        ...
        try:
            async with self._session.get(
                ...
                if response.status == 200:
                    ...
                    for node_id, node_output in prompt_history.get(
                        "outputs", {}
                    ).items():
                    ...
                    self.logger.error(
                        f"Failed to get execution outputs: HTTP {response.status}"
                    )
                    return []
    ...
    async def cancel_execution(self, prompt_id: str) -> bool:
        ...
        try:
            async with self._session.post(
                f"{self.config.server_url}/interrupt"
            ) as response:
                if response.status == 200:
                    ...
                    self.logger.error(
                        f"Failed to cancel execution: HTTP {response.status}"
                    )
                    return False
        ...
        try:
            async with self._session.get(f"{self.config.server_url}/queue") as response:
                if response.status == 200:
                    ...
                    self.logger.error(
                        f"Failed to get queue status: HTTP {response.status}"
                    )
    ...
    def __del__(self):
        ...
        if self._connected:
            self.logger.warning(
                "API Orchestrator being destroyed with active connection"
            )
    ...
    def _submit_mock_workflow(self, workflow: ComfyUIWorkflow) -> str:
        ...
        execution = ExecutionResult(
            ...
            output_files=[
                GeneratedAsset(
                    filename=f"mock_output_{i}.png", subfolder="", folder_type="output"
                )
    ...
    def _on_error_recovery(self, error_info, fallback_mode: FallbackMode) -> None:
        ...
        """
        self.logger.info(
            f"API Orchestrator error recovery: {fallback_mode.value} for error {error_info.error_id}"
        )
