...
class AdvancedWorkflowRegistry:
    ...
    def register_workflow(
        self, category: str, name: str, workflow_class: Type[BaseAdvancedWorkflow]
    ) -> bool:
        """
        ...
        try:
            ...
            if not issubclass(workflow_class, BaseAdvancedWorkflow):
                self.logger.error(
                    f"Workflow class {workflow_class} must inherit from BaseAdvancedWorkflow"
                )
                return False
            ...
            if name in self.workflows[category]:
                self.logger.warning(
                    f"Workflow {name} already registered in category {category}, overwriting"
                )
            ...
            self.logger.error(
                f"Failed to register workflow {category}/{name}: {str(e)}"
            )
            return False

    def get_workflow_class(
        self, category: str, workflow_type: str
    ) -> Optional[Type[BaseAdvancedWorkflow]]:
        """
    ...
    def get_workflow_instance(
        self, category: str, workflow_type: str, config: Optional[Dict[str, Any]] = None
        ...
        try:
            # Create new instance
            workflow_type_enum = (
                WorkflowType.VIDEO if category == "video" else WorkflowType.IMAGE
            )
            ...
            self.logger.error(
                f"Failed to create workflow instance {workflow_key}: {str(e)}"
            )
            return None

    def _update_capability_matrix(
        self, workflow_key: str, instance: BaseAdvancedWorkflow
    ):
            ...
            for py_file in path.glob("**/*_workflow.py"):
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (
                            issubclass(obj, BaseAdvancedWorkflow)
                            and obj != BaseAdvancedWorkflow
                        ):
                            ...
                            try:
                                ...
                                self.logger.error(
                                    f"Failed to instantiate {name} for discovery: {str(e)}"
                                )

                except Exception as e:
                    self.logger.error(
                        f"Failed to import workflow module {py_file}: {str(e)}"
                    )
    ...
    def validate_registry(self) -> Dict[str, Any]:
        ...
        uncovered_capabilities = [
            capability.value
            for capability, workflows in self.capability_matrix.items()
            if not workflows
        ]

        if uncovered_capabilities:
            warnings.append(
                f"No workflows available for capabilities: {uncovered_capabilities}"
            )
        ...
        for workflow_key, instance in self.workflow_instances.items():
            try:
                ...
                if not status.get("capabilities"):
                    issues.append(
                        f"Workflow {workflow_key} has no capabilities defined"
                    )
            except Exception as e:
                issues.append(
                    f"Failed to get status for workflow {workflow_key}: {str(e)}"
                )
        ...
        return {
            ...
            "warnings": warnings,
            "total_workflows": sum(
                len(workflows) for workflows in self.workflows.values()
            ),
    ...
    def cleanup_instances(self):
        ...
        for workflow_key, instance in list(self.workflow_instances.items()):
            try:
                ...
                self.logger.error(
                    f"Failed to unload models for {workflow_key}: {str(e)}"
                )
...
def register_workflow(
    category: str, name: str, workflow_class: Type[BaseAdvancedWorkflow]
) -> bool:
...
def get_workflow(
    category: str, workflow_type: str, config: Optional[Dict[str, Any]] = None
) -> Optional[BaseAdvancedWorkflow]:
