...
class CrossPlatformManager:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Detected capabilities: {self.capabilities.supported_processing_modes}"
        )
    ...
    def _determine_processing_modes(
        self, gpu_available: bool, gpu_type: Optional[str]
    ) -> List[ProcessingMode]:
        """Determine supported processing modes."""
    ...
    def _get_optimization_flags(self) -> Dict[str, Any]:
        ...
        if self.platform_info == PlatformType.WINDOWS:
            ...
            flags.update(
                {"use_posix_api": True, "file_buffering": "line", "nice_level": 0}
            )
        elif self.platform_info == PlatformType.MACOS:
            flags.update(
                {
    ...
    def validate_dependencies(self) -> Tuple[bool, List[str]]:
        ...
        if self.capabilities.memory_gb < 4.0:
            issues.append(
                f"Low memory detected: {self.capabilities.memory_gb:.1f}GB (4GB+ recommended)"
            )
    ...
    def get_processing_config(
        self, processing_mode: Optional[ProcessingMode] = None
    ) -> Dict[str, Any]:
        ...
        if mode == ProcessingMode.GPU_CUDA:
            config.update(
                {"device": "cuda", "gpu_memory_fraction": 0.8, "allow_growth": True}
            )
        elif mode == ProcessingMode.GPU_OPENCL:
            config.update({"device": "opencl", "platform_id": 0, "device_id": 0})
        elif mode == ProcessingMode.GPU_METAL:
            config.update({"device": "metal", "use_unified_memory": True})
        else:
            config.update({"device": "cpu", "num_threads": self.capabilities.cpu_cores})
    ...
    def adapt_for_hardware(self, base_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.capabilities.gpu_available and self.capabilities.gpu_memory_gb:
            if self.capabilities.gpu_memory_gb < 4.0:
                ...
                adapted_config["batch_size"] = min(
                    adapted_config.get("batch_size", 4), 1
                )
    ...
    def get_compatibility_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_processing_modes": [
                mode.value for mode in self.capabilities.supported_processing_modes
            ],
            "recommended_config": self.get_optimal_config(),
            "validation": {"is_compatible": is_valid, "issues": issues},
            "optimizations": {
                "max_workers": self.optimizations.max_parallel_workers,
                "memory_limit_gb": self.optimizations.memory_limit_gb,
...
def main():
    ...
    # Display results
    print(
        f"Platform: {report['platform_info']['type']} ({report['platform_info']['architecture']})"
    )
    ...
    print(
        f"OpenCV: {'✓' if report['software_dependencies']['opencv_available'] else '✗'}"
    )
    print(
        f"FFmpeg: {'✓' if report['software_dependencies']['ffmpeg_available'] else '✗'}"
    )
