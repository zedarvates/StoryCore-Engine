...
class AICharacterEngine:
    ...
    def normalize_trait_value(
        self, value: float, trait: Optional[PersonalityTrait] = None
    ) -> float:
        """
    ...
    def apply_trait_correlations(
        self, traits: Dict[PersonalityTrait, float], correlation_strength: float = 0.5
    ) -> Dict[PersonalityTrait, float]:
        """
    ...
    def calculate_trait_consistency(
        self, traits: Dict[PersonalityTrait, float]
    ) -> float:
        """
    ...
    def get_trait_summary(
        ...
        dominant = [{"trait": t.value, "score": v} for t, v in sorted_traits[:3]]

        # Find suppressed traits (bottom 3)
        suppressed = [{"trait": t.value, "score": v} for t, v in sorted_traits[-3:]]
    ...
    def inherit_traits_from_archetype(
        self,
        archetype: CharacterArchetype,
            ...
            if trait not in traits and trait not in TRAIT_CATEGORIES["big_five"]:
                ...
                if bias != 0:
                    variation = random.uniform(
                        -variation_range / 2, variation_range / 2
                    )
                    base_value = 0.5 + bias + variation
                    traits[trait] = self.normalize_trait_value(base_value, trait)
                else:
        ...
        if base_traits:
            for trait, value in base_traits.items():
                ...
                traits[trait] = self.normalize_trait_value(
                    value + override_variation, trait
                )
    ...
    def _get_archetype_big_five_biases(
    ...
    def _get_archetype_additional_trait_bias(
        self, archetype: CharacterArchetype, trait: PersonalityTrait
    ) -> float:
        """
    ...
    async def generate_character(
        self, config: CharacterGenerationConfig
        ...
        async def _generate_operation():
            try:
                ...
                self.logger.info(
                    f"Generated character: {name} ({config.archetype.value})"
                )
                return character
    ...
    async def _generate_appearance(
        ...
        if not config.appearance_constraints:
            ...
            appearance.accessories = random.sample(
                ["Necklace", "Ring", "Bracelet", "Hat", "Glasses"], random.randint(0, 3)
            )

        return appearance

    async def _generate_backstory(
        self, config: CharacterGenerationConfig, personality: PersonalityProfile
    ) -> CharacterBackstory:
        ...
        archetype_template = self.archetype_templates.get(config.archetype, {})
        key_events = archetype_template.get("typical_motivations", [])[
            : config.backstory_depth
        ]
        ...
        high_traits = [
            trait for trait, score in personality.traits.items() if score > 0.7
        ]
        skills = []
    ...
    async def enhance_character_visuals(
        self, character: GeneratedCharacter, enhancement_type: str = "style_transfer"
    ...
    def _get_average_consistency_score(self) -> float:
        ...
        total_score = sum(
            char.consistency.consistency_score for char in self.character_cache.values()
        )
