...
class ScriptAnalysisConfig:
    ...
    focus_areas: List[str] = field(
        default_factory=lambda: ["characters", "scenes", "dialogue"]
    )
...
class AIScriptAnalysisEngine:
    ...
    async def analyze_script(
        self, script_content: str, config: ScriptAnalysisConfig
    ) -> Optional[ScriptAnalysisResult]:
        """
        ...
        async def _analyze_operation():
            try:
                ...
                story_structure = await self._analyze_story_structure(
                    script_structure, config
                )

                # Calculate metrics
                metrics = await self._calculate_metrics(
                    script_structure, characters, scenes, dialogues
                )
    ...
    async def _analyze_characters(
        self, script_structure: Dict[str, Any], config: ScriptAnalysisConfig
        ...
        for character_name in script_structure["characters"]:
            ...
            dialogue_count = sum(
                1
                for d in script_structure["dialogues"]
            ...
            screen_time = (
                (dialogue_count / total_dialogue) * 100 if total_dialogue > 0 else 0
            )
    ...
    async def _analyze_scenes(
        ...
        for i, scene_data in enumerate(script_structure["scenes"]):
            ...
            scene = SceneAnalysis(
                ...
                key_events=[
                    f"Event {j + 1}" for j in range(min(3, len(scene_data["content"])))
                ],
                dialogue_count=dialogue_count,
    ...
    async def _analyze_dialogues(
        self, script_structure: Dict[str, Any], config: ScriptAnalysisConfig
    ) -> List[DialogueAnalysis]:
        ...
        plot_points = [
            {
                ...
                "scene": 3,
                "description": "Main conflict introduced",
            },
            {
                "name": "Plot Point 1",
                "scene": 10,
    ...
    async def _calculate_metrics(
        ...
        dialogue_to_action_ratio = total_dialogue_lines / max(
            1, script_structure["total_lines"] - total_dialogue_lines
        )
    ...
    async def _generate_recommendations(
        ...
        if metrics.conflict_density < 0.5:
            recommendations.append(
                "Consider increasing conflict density to maintain audience engagement"
            )

        if metrics.emotional_diversity_score < 0.6:
            recommendations.append(
                "Add more emotional variety to create a richer viewing experience"
            )

        if metrics.character_interaction_complexity < 0.7:
            recommendations.append(
                "Develop more complex character relationships and interactions"
            )

        # Analyze character distribution
        protagonist_count = sum(
            1 for c in characters if c.role == CharacterRole.PROTAGONIST
        )
        if protagonist_count == 0:
            recommendations.append(
                "Consider developing a clearer protagonist for audience connection"
            )
    ...
    async def _calculate_quality_score(
        ...
        character_score = min(
            1.0,
            len([c for c in characters if c.role == CharacterRole.PROTAGONIST]) * 0.3,
        )
        scene_score = min(
            1.0, len([s for s in scenes if s.plot_significance > 7]) * 0.1
        )
        dialogue_score = min(1.0, metrics.dialogue_to_action_ratio * 0.5)
        structure_score = min(1.0, len(story_structure.plot_points) * 0.1)
        metrics_score = (
            metrics.emotional_diversity_score
            + metrics.conflict_density
            + metrics.character_interaction_complexity
        ) / 3
    ...
    def _get_average_quality_score(self) -> float:
        ...
        total_score = sum(
            result.quality_score for result in self.analysis_cache.values()
        )
