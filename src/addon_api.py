...
from src.addon_manager import AddonInfo, AddonManifest, AddonState, AddonType
...
class ExternalAddonLoader:
    ...
    async def load_from_url(self, url: str, category: str = "community",
                           verify_checksum: bool = True) -> InstallationResult:
        """
    ...
    async def load_from_github(self, repo_url: str, category: str = "community",
                              branch: str = "main") -> InstallationResult:
        """
    ...
    async def load_from_pypi(self, package_name: str, category: str = "community") -> InstallationResult:
        """
        ...
        try:
            ...
            if result.success and result.addon_info:
                ...
                result.addon_info.manifest.metadata["pypi_version"] = info.get("version")
    ...
    async def _download_file(self, url: str) -> Optional[Path]:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                retry_count += 1
                self.logger.warning(f"Retry {retry_count}/{self.max_retries} pour {url}")
    ...
    async def _verify_checksum(self, file_path: Path, expected_checksum: str = None) -> bool:
        ...
        try:
            ...
            if expected_checksum and file_hash != expected_checksum:
                self.logger.error(f"Checksum mismatch: attendu {expected_checksum}, obtenu {file_hash}")
                return False
    ...
    async def _install_from_archive(self, archive_path: Path, category: str) -> InstallationResult:
        ...
        try:
            ...
            if "engine_version" in compatibility:
                ...
                if not self._check_version_compatibility(self.engine_version, required_version):
                    warnings.append(
...
class AddonRegistryClient:
    ...
    async def search(self, query: str, filters: Optional[Dict] = None,
                   limit: int = 20) -> List[RegistryAddonInfo]:
        """
    ...
    async def get_featured(self, category: str = None) -> List[RegistryAddonInfo]:
        ...
        try:
            ...
            self.logger.error(f"Erreur lors de la récupération des addons vedettes: {e}")
            return []
...
class DependencyManager:
    ...
    async def initialize(self):
        ...
        self.logger.info(f"DependencyManager initialisé avec {len(self._installed_packages)} packages")
    ...
    def get_package_info(self, package_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erreur lors de la récupération des infos du package: {e}")
            return None
