...
class ClothesSwapperEngine:
    ...
    async def swap_clothes(self, source_person_image: PIL_Image, target_garment_image: PIL_Image) -> ClothesSwapResult:
        """
        ...
        try:
            ...
            if not settings.USE_MOCK_COMFYUI:
                ...
                p_upload = await comfyui_executor.upload_image(p_img_byte_arr.getvalue(), "person_tryon.jpg")
                
                # 2. Upload Garment Image
                g_img_byte_arr = io.BytesIO()
                target_garment_image.convert("RGB").save(g_img_byte_arr, format='JPEG')
                g_upload = await comfyui_executor.upload_image(g_img_byte_arr.getvalue(), "garment_tryon.jpg")
                ...
                if workflow_path.exists():
                    ...
                    if "1" in workflow: # Source Person
                        workflow["1"]["inputs"]["image"] = p_upload.get("name", "person_tryon.jpg")
                    if "2" in workflow: # Garment
                        workflow["2"]["inputs"]["image"] = g_upload.get("name", "garment_tryon.jpg")
                    
                    # 5. Execute
                    res = await comfyui_executor.execute_workflow(workflow)
                    
                    if res.get("success") and res.get("outputs"):
                        self.logger.info("Clothes swap completed via ComfyUI LADI-VTON Pro")
