    ...
    async def replace_background(
        ...
        try:
            ...
            if not settings.USE_MOCK_COMFYUI:
                ...
                # 3. Load Workflow
                workflow_path = Path(
                    "src/workflows/comfyui/background_replacer_pro_v1.json"
                )
                if workflow_path.exists():
                    ...
                    if "6" in workflow:  # Positive Prompt
                        workflow["6"]["inputs"]["text"] = (
                            f"{prompt}, high resolution, photography, cinematic lighting"
                        )
            ...
            return BackgroundReplacerResult(
                success=True, image=image, processing_time=time.time() - start_time
            )
