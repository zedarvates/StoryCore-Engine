...
class FaceIdentityEngine:
    ...
    async def register_character(self, name: str, reference_images: List[str]) -> str:
        ...
        self.logger.info(f"Registering character: {name} with {len(reference_images)} references")
    ...
    async def apply_identity(self, image: PIL_Image, identity_id: str, strength: float = 0.9) -> IdentityPreservationResult:
        """
        ...
        if identity_id not in self.identity_registry:
            return IdentityPreservationResult(success=False, error_message=f"Identity {identity_id} not found")
            
        identity = self.identity_registry[identity_id]
        self.logger.info(f"Applying identity '{identity.name}' with strength {strength}")
