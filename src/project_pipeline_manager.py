...
class ProjectPipelineManager:
    ...
    DEFAULT_PROJECTS_DIR = os.path.join(
        os.path.expanduser("~"), "Documents", "StoryCore Projects"
    )
    ...
    def __init__(self, base_output_dir: str = None):
        ...
        for step in self.steps:
            self.step_results[step] = PipelineStepResult(
                step=step, status=PipelineStatus.PENDING
            )
    ...
    async def run_pipeline(
        self, initial_prompt: str, options: Dict[str, Any] = None
    ) -> Dict[PipelineStep, PipelineStepResult]:
    ...
    async def _execute_step(
        self, step: PipelineStep, prompt: str, options: Dict[str, Any]
    ):
        ...
        self._update_step_result(
            step, PipelineStatus.COMPLETED, data=self.step_results[step].data
        )
    ...
    async def _step_generate_name(self, prompt: str, options: Dict[str, Any]):
        ...
        try:
            ...
            parsed = self.step_results[PipelineStep.PARSE_PROMPT].data.get(
                "parsed_prompt", {}
            )
    ...
    async def _step_confirm_name(self, options: Dict[str, Any]):
        ...
        suggestions = self.step_results[PipelineStep.GENERATE_NAME].data.get(
            "name_suggestions", []
        )
        if suggestions:
            ...
            if PipelineStep.CONFIRM_NAME in self.confirmation_callbacks:
                confirmed = self.confirmation_callbacks[PipelineStep.CONFIRM_NAME](
    ...
    async def _step_generate_world(self, options: Dict[str, Any]):
        if not options.get("generate_world", True):
            self._update_step_result(
                PipelineStep.GENERATE_WORLD, PipelineStatus.SKIPPED
            )
            return
        try:
            ...
            confirmed_name = self.step_results[PipelineStep.CONFIRM_NAME].data.get(
                "confirmed_name"
            )
            generator = WorldConfigGenerator()
            world_config = generator.generateWorldConfig(
                {
                    "projectTitle": confirmed_name,
                    "genre": parsed.get("genre"),
                    "mood": parsed.get("mood"),
                    "setting": parsed.get("setting"),
                    "timePeriod": parsed.get("timePeriod"),
    ...
    async def _step_generate_characters(self, options: Dict[str, Any]):
        if not options.get("generate_characters", True):
            self._update_step_result(
                PipelineStep.GENERATE_CHARACTERS, PipelineStatus.SKIPPED
            )
            return
        try:
    ...
    async def _step_generate_story(self, options: Dict[str, Any]):
        if not options.get("generate_story", True):
            self._update_step_result(
                PipelineStep.GENERATE_STORY, PipelineStatus.SKIPPED
            )
            return
        try:
    ...
    async def _step_generate_dialogue(self, options: Dict[str, Any]):
        if not options.get("generate_dialogues", True):
            self._update_step_result(
                PipelineStep.GENERATE_DIALOGUE, PipelineStatus.SKIPPED
            )
            return
        try:
    ...
    async def _step_generate_sequences(self, options: Dict[str, Any]):
        if not options.get("generate_sequences", True):
            self._update_step_result(
                PipelineStep.GENERATE_SEQUENCES, PipelineStatus.SKIPPED
            )
            return
        try:
            ...
            self.step_results[PipelineStep.CONFIRM_NAME].data.get("confirmed_name")
    ...
    async def _step_generate_music(self, options: Dict[str, Any]):
        if not options.get("generate_music", True):
            self._update_step_result(
                PipelineStep.GENERATE_MUSIC, PipelineStatus.SKIPPED
            )
            return
        try:
    ...
    async def _step_create_project(self, options: Dict[str, Any]):
        ...
        project_path = os.path.join(
            self.base_output_dir, confirmed_name.replace(" ", "_")
        )
    ...
    async def _step_finalize(self, options: Dict[str, Any]):
        ...
        project_data = self.step_results[PipelineStep.CREATE_PROJECT].data.get(
            "project_data", {}
        )
        world_config = self.step_results[PipelineStep.GENERATE_WORLD].data.get(
            "world_config"
        )
        characters = self.step_results[PipelineStep.GENERATE_CHARACTERS].data.get(
            "characters"
        )
        story = self.step_results[PipelineStep.GENERATE_STORY].data.get("story")
        sequences = self.step_results[PipelineStep.GENERATE_SEQUENCES].data.get(
            "sequences"
        )
        ...
        self._update_step_result(
            PipelineStep.FINALIZE, PipelineStatus.COMPLETED, data={"summary": summary}
        )
...
async def create_project_from_prompt(
    ...
    return {
        "success": all(
            ...
            if r.status != PipelineStatus.SKIPPED
        ),
        "project_path": results.get(PipelineStep.CREATE_PROJECT, {}).data.get(
            "project_path"
        ),
        "confirmed_name": results.get(PipelineStep.CONFIRM_NAME, {}).data.get(
            "confirmed_name"
        ),
        "parsed_prompt": results.get(PipelineStep.PARSE_PROMPT, {}).data.get(
            "parsed_prompt"
        ),
