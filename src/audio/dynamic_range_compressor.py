...
class DynamicRangeCompressor:
    ...
    def __init__(
        self, sample_rate=44100, threshold=-20.0, ratio=4.0, attack=0.01, release=0.1
    ):
        """
    ...
    def apply_compression(self, audio_data):
        ...
        for i in range(len(audio_data)):
            ...
            if input_level > threshold_linear:
                gain_reduction = (
                    threshold_linear + (input_level - threshold_linear) / self.ratio
                )
            ...
            if target_gain < self.gain:
                ...
                # Release phase
                self.gain = (
                    release_coeff * self.gain + (1 - release_coeff) * target_gain
                )
