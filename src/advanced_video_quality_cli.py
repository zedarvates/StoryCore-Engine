...
@click.option(
    "--enable-realtime/--disable-realtime",
    default=True,
    help="Enable real-time analysis mode",
)
@click.option(
    "--sample-rate", type=float, default=1.0, help="Frame sampling rate (0.1-1.0)"
)
@click.option(
    "--max-time", type=float, default=300.0, help="Maximum analysis time in seconds"
)
...
@click.option(
    "--workflow-type", "-w", default="auto", help="Workflow type for all videos"
)
@click.option(
    "--output-dir", "-d", type=click.Path(), help="Output directory for reports"
)
@click.option(
    "--parallel", "-p", is_flag=True, help="Process videos in parallel (future feature)"
)
...
def batch(
    ...
    if parallel:
        click.echo(
            "⚠️ Parallel processing not yet implemented - processing sequentially"
        )
    ...
    for i, video_path in enumerate(video_paths, 1):
        ...
        try:
            ...
            click.echo(
                f"   Score: {report.overall_score:.3f} ({monitor._score_to_grade(report.overall_score)})"
            )
            ...
            if output_dir:
                output_path = (
                    Path(output_dir) / f"{Path(video_path).stem}_quality_report.json"
                )
