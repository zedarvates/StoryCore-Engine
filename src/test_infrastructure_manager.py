...
class TestInfrastructureManager:
    ...
    def validate_executability(self, test_case: TestCase) -> ExecutabilityReport:
        ...
        """
        report = ExecutabilityReport(test_case=test_case, is_executable=True)

        # Check if file exists
        if not test_case.file_path.exists():
            report.is_executable = False
            report.configuration_issues.append(
                f"Test file not found: {test_case.file_path}"
            )
            return report
    ...
    def _validate_python_test(
        self, test_case: TestCase, report: ExecutabilityReport
    ) -> None:
    ...
    def generate_discovery_report(self) -> TestDiscoveryResult:
        ...
        executability_percentage = (
            (executable_tests / total_tests * 100) if total_tests > 0 else 0
        )
    ...
    def print_report(self, result: TestDiscoveryResult) -> None:
        ...
        print(
            f"Meets 95% Threshold: {'✓ YES' if result.summary['meets_95_percent_threshold'] else '✗ NO'}"
        )
            ...
            for report in result.executability_reports:
                if not report.is_executable:
                    ...
                    if report.missing_dependencies:
                        print(
                            f"  Missing Dependencies: {', '.join(report.missing_dependencies)}"
                        )
