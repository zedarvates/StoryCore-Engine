    ...
    def install_comfyui_portable(
        ...
        try:
            ...
            if enable_cors:
                ...
                cors_success, cors_error = self._configure_cors(
                    comfyui_dir, cors_origin
                )
                if not cors_success:
            ...
            if models:
                ...
                installed_models, model_errors = self._install_models(
                    comfyui_dir, models, progress_callback
                )
            ...
            if workflows:
                ...
                installed_workflows, workflow_errors = self._install_workflows(
            ...
            verification_success, verification_error = self._verify_installation(
                comfyui_dir
            )
...
"""
        ...
        try:
                ...
                try:
                    ...
                    if progress_callback:
                        ...
                        progress_callback(
                            "models",
                            progress,
                            f"Installing model {i + 1}/{total_models}: {model}",
                        )
                    ...
                    if model.startswith("http://") or model.startswith("https://"):
                        ...
                        success = self._download_file(
                            model, target_path, progress_callback
                        )
                        ...
                        if success:
                            ...
                            if target_path.exists() and target_path.stat().st_size > 0:
                                ...
                                logger.info(
                                    f"Successfully installed model: {model_name}"
                                )
                            else:
                                error_msg = (
                                    f"Model file verification failed: {model_name}"
                                )
                                logger.error(error_msg)
                                errors.append(error_msg)
                        else:
                    ...
                    # Check if model is a local file path
                        ...
                        if target_path.exists() and target_path.stat().st_size > 0:
                            ...
                            logger.info(
                                f"Successfully installed model: {model_path.name}"
                            )
                        else:
                            error_msg = (
                                f"Model file verification failed: {model_path.name}"
                            )
                            logger.error(error_msg)
                            errors.append(error_msg)
                    ...
                    # Assume it's just a model name (for testing/placeholder)
                        ...
                        logger.warning(
                            f"Model '{model}' is not a URL or file path, skipping"
                        )
            ...
            if progress_callback:
                progress_callback(
                    "models",
                    70,
                    f"Model installation complete: {len(installed)}/{total_models} successful",
                )
    ...
    def _install_workflows(
        self, comfyui_dir: Path, workflows: List[str]
    ) -> Tuple[List[str], List[str]]:
        """
        ...
        try:
            ...
            for i, workflow in enumerate(workflows):
                try:
                    logger.info(
                        f"Installing workflow {i + 1}/{total_workflows}: {workflow}"
                    )

                    # Check if workflow is a URL
                    if workflow.startswith("http://") or workflow.startswith(
                        "https://"
                    ):
                        ...
                        if success:
                            ...
                            if target_path.exists() and target_path.stat().st_size > 0:
                                ...
                                try:
                                    ...
                                    logger.info(
                                        f"Successfully installed workflow: {workflow_name}"
                                    )
                    ...
                    # Check if workflow is a local file path
                        ...
                        if target_path.exists() and target_path.stat().st_size > 0:
                            ...
                            logger.info(
                                f"Successfully installed workflow: {workflow_path.name}"
                            )
                        else:
            ...
            logger.info(
                f"Workflow installation complete: {len(installed)}/{total_workflows} successful"
            )
            return installed, errors
        ...
        try:
            ...
            if self.platform_type == "windows":
                ...
                if not python_exe.exists():
                    logger.warning(
                        "Embedded Python not found, system Python will be used"
                    )
    ...
    def start_comfyui(self, comfyui_dir: Optional[Path] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if startup_script.exists():
                ...
                if self.platform_type == "windows":
                    subprocess.Popen(
                        [str(startup_script)], cwd=str(comfyui_dir), shell=True
                    )
                else:
                    subprocess.Popen([str(startup_script)], cwd=str(comfyui_dir))

                return True, "ComfyUI started at http://127.0.0.1:8188"
            else:
...
def main():
    ...
    parser.add_argument(
        "--zip", required=True, help="Path to ComfyUI Portable ZIP file"
    )
    parser.add_argument(
        "--install-dir", default="./comfyui_portable", help="Installation directory"
    )
    parser.add_argument(
        "--cors-origin", default="http://localhost:3000", help="CORS origin"
    )
    parser.add_argument(
        "--no-cors", action="store_true", help="Disable CORS configuration"
    )
    parser.add_argument(
        "--start", action="store_true", help="Start ComfyUI after installation"
    )
