...
Uses the working FLUX 2 workflow format from ComfyUI
"""
...
ASSETS = [
    ('storycore_logo_horizontal', 'Professional logo design for StoryCore text, modern typography, clean minimal style, dark background with gradient accent', 1024, 256),
    ('storycore_logo_square', 'Square app icon design SC monogram logo, modern minimal style, dark background with blue-purple gradient', 512, 512),
    ('storycore_panel_placeholder', 'Storyboard panel placeholder, neutral gray gradient background, subtle grid overlay, professional comic panel template', 768, 432),
    ('storycore_loading_state', 'Animated loading spinner placeholder, circular loading animation mockup, professional UI loading indicator, clean minimal design', 256, 256),
    ('storycore_error_state', 'Error state illustration, warning triangle icon, professional UI error graphic, clean minimal design, red-orange accent', 400, 300),
    ('storycore_character_template', 'Character design template for storyboard, generic figure outline, front view, clean line art style, minimal details', 512, 768),
    ('storycore_banner_featured', 'Featured content banner design, horizontal layout, dark gradient background, modern tech aesthetic, clean minimal style', 1200, 400),
]


def create_flux2_workflow(name, prompt, width, height, seed=None):
    """Create FLUX 2 workflow using the working format"""
    if seed is None:
    ...
    return {
        ...
        "68:22": {
            "inputs": {"model": ["68:12", 0], "conditioning": ["68:43", 0]},
            "class_type": "BasicGuider",
            "_meta": {"title": "BasicGuider"}
        },
        "68:43": {
            "inputs": {"conditioning": ["68:26", 0], "latent": ["68:44", 0]},
            "class_type": "ReferenceLatent",
            "_meta": {"title": "ReferenceLatent"}
        },
        "68:44": {
            "inputs": {"pixels": ["45", 0], "vae": ["68:10", 0]},
            "class_type": "VAEEncode",
            "_meta": {"title": "VAE Encode"}
        },
        "45": {
            "inputs": {"upscale_method": "lanczos", "megapixels": 1, "resolution_steps": 1, "image": ["46", 0]},
            "class_type": "ImageScaleToTotalPixels",
            "_meta": {"title": "ImageScaleToTotalPixels"}
        },
        "46": {
            "inputs": {"image": "image_flux2_input_image.png"},
