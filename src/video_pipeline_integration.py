...
class VideoPipelineIntegrator:
    ...
    def load_comfyui_image_output(self) -> bool:
        ...
        try:
            ...
            if not image_gen_file.exists():
                logger.warning(
                    "No ComfyUI image generation metadata found - creating mock data"
                )
            ...
            for result in image_gen_data.get("generation_results", []):
                ...
                for image_data in result.get("generated_images", []):
                    if (
                        image_data.get("type") == "keyframe"
                        or "final_composite" in result
                    ):
                        comfyui_output = ComfyUIImageOutput(
                            ...
                            quality_metrics=image_data.get("generation_result", {}).get(
                                "quality_metrics", {}
                            ),
            ...
            logger.info(
                f"Loaded {len(self.comfyui_outputs)} keyframe images from ComfyUI Image Engine"
            )
            return True
    ...
    def integrate_with_video_engine(
        ...
        try:
            ...
            logger.info(
                f"Successfully integrated {len(integrated_shots)} shots with Video Engine"
            )
            return True
    ...
    def generate_audio_engine_output(self) -> Dict[str, Any]:
        ...
        try:
            ...
            timeline_data = (
                self.video_engine.get_timeline_metadata()
                if self.video_engine
                else self._create_mock_timeline()
            )
            ...
            try:
                ...
                for field in required_fields:
                    if field not in project_data:
                        issues.append(
                            f"Missing required field in project.json: {field}"
                        )

                # Validate schema version
                if project_data.get("schema_version") != "1.0":
                    issues.append(
                        f"Invalid schema version: {project_data.get('schema_version')}"
                    )
        ...
        report = {
            ...
            "component_status": {
                ...
                "video_engine": {
                    ...
                    "shots_integrated": len(self.video_engine.shots)
                    if self.video_engine
                    else 0,
            ...
            "integration_metrics": {
                ...
                "average_shot_duration": sum(
                    shot.duration for shot in self.shot_metadata
                )
                / len(self.shot_metadata)
                if self.shot_metadata
                else 0,
                "keyframes_per_shot": len(self.comfyui_outputs)
                / len(self.shot_metadata)
                if self.shot_metadata
                else 0,
            },
    ...
    def _resolve_image_path(
        self, image_data: Dict[str, Any], result: Dict[str, Any]
    ) -> str:
        ...
        if "final_composite" in result:
            return str(
                ...
                / "images"
                / "generated"
                / f"{result['frame_id']}_composite.png"
            )
        ...
        return str(
            ...
            / "images"
            / "generated"
            / f"{result['frame_id']}_keyframe.png"
        )
    ...
    def _create_integrated_shots(self) -> List[ShotData]:
        ...
        for shot_meta in self.shot_metadata:
            ...
            for comfyui_output in self.comfyui_outputs:
                ...
                if (
                    shot_meta.shot_id in comfyui_output.frame_id
                    or len(shot_keyframes) < 2
                ):
    ...
    def _create_video_engine_config(self) -> VideoConfig:
        ...
        return VideoConfig(
            ...
            gpu_acceleration=(
                manager_config.performance.processing_mode.value != "cpu_only"
            ),
        )
    ...
    def _calculate_average_quality(self) -> float:
        ...
        total_quality = sum(
            output.quality_metrics.get("overall_quality", 0.0)
            for output in self.comfyui_outputs
        )
