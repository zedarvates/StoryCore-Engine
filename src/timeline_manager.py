...
class TimelineManager:
    ...
    def generate_timeline_metadata(self) -> TimelineMetadata:
        ...
        logger.info(
            f"Generated timeline metadata: {total_frames} frames, {total_duration:.2f}s"
        )
    ...
    def get_frame_at_time(self, timestamp: float) -> Optional[FrameTiming]:
        ...
        for frame_timing in self.timeline_metadata.frame_timings:
            if (
                frame_timing.timestamp
                <= timestamp
                < (frame_timing.timestamp + frame_timing.duration)
            ):
                return frame_timing
        return None
    ...
    def calculate_transition_frames(
        ...
        overlap_frames = min(
            transition_frames // 2, shot1.frame_count, shot2.frame_count
        )
        ...
        return {
            ...
            "blend_curve": self._generate_blend_curve(
    ...
    def export_timeline_metadata(self, output_path: str) -> bool:
        ...
        try:
            ...
            metadata_dict = {
                ...
                "shot_timings": [
                    asdict(shot) for shot in self.timeline_metadata.shot_timings
                ],
                "frame_timings": [
                    asdict(frame) for frame in self.timeline_metadata.frame_timings
                ],
    ...
    def _generate_transition_metadata(self) -> Dict[str, Any]:
        ...
        for i in range(len(self.shots) - 1):
            ...
            if shot1.transition_out or shot2.transition_in:
                transition_type = (
                    shot1.transition_out or shot2.transition_in or TransitionType.CUT
                )
                transition_duration = (
                    shot1.transition_duration or shot2.transition_duration or 0.0
                )
    ...
    def _generate_blend_curve(
        self, frames: int, transition_type: TransitionType
    ) -> List[float]:
...
def main():
    ...
    # Add test shots
    timeline.add_shot("shot_001", 48, transition_out=TransitionType.FADE)  # 2 seconds
    timeline.add_shot(
        "shot_002",
        72,
    ...
    timeline.add_shot(
        "shot_003", 120, transition_in=TransitionType.DISSOLVE
    )  # 5 seconds
    ...
    if success:
        print("  Metadata exported to: timeline_test.json")
