    ...
    def add_job(self, job: JobDefinition):
        ...
        with self.lock:
            ...
            if self.algorithm == SchedulingAlgorithm.FIFO:
                ...
                heapq.heappush(
                    self._priority_heap, (job.priority.value, time.time(), job)
                )
            elif self.algorithm == SchedulingAlgorithm.SHORTEST_JOB_FIRST:
                heapq.heappush(
                    self._priority_heap, (job.estimated_duration, time.time(), job)
                )
            else:  # FAIR_SHARE
...
class BatchProcessingSystem:
    ...
    def __init__(
        ...
        logger.info(
            f"Batch Processing System initialized - Max Workers: {max_workers}, Algorithm: {scheduling_algorithm.value}"
        )
    ...
    def _execute_job(self, worker: BatchWorker, job: JobDefinition) -> JobResult:
        ...
        try:
            ...
            if (
                result.status == JobStatus.FAILED
                and result.retry_count < job.max_retries
            ):
                logger.info(
                    f"Retrying job {job.job_id} (attempt {result.retry_count + 1}/{job.max_retries})"
                )
                ...
                logger.warning(
                    f"Job {job.job_id} failed after {job.max_retries} retries. Marking as failed."
                )
                result.status = JobStatus.FAILED
                self._update_job_result(result)
    ...
    def _record_processing_metrics(self):
        ...
        total_jobs = sum(
            worker.stats.jobs_processed for worker in self.workers.values()
        )
        successful_jobs = sum(
            worker.stats.jobs_successful for worker in self.workers.values()
        )
    ...
    def _calculate_average_latency(self) -> float:
        ...
        total_time = sum(
            worker.stats.total_processing_time for worker in self.workers.values()
        )
        total_jobs = sum(
            worker.stats.jobs_processed for worker in self.workers.values()
        )
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        for worker_id, worker in self.workers.items():
            worker_stats[worker_id] = {
                ...
                "current_job": worker.current_job.job_id
        ...
        return {
            ...
            "priority_distribution": {
                p.name: count for p, count in priority_counts.items()
            },
            "active_workers": len(self.running_jobs),
...
def main():
    ...
    batch_system = BatchProcessingSystem(
        max_workers=3, scheduling_algorithm=SchedulingAlgorithm.PRIORITY
    )
    ...
    while completed_jobs < len(jobs):
        ...
        print(
            f"   Queue: {stats['queue_size']}, Active: {stats['active_workers']}, Completed: {completed_jobs}/{len(jobs)}"
        )
    ...
    for job_id in jobs:
        ...
        if result:
            ...
            print(
                f"   {status_icon} Job {job_id[:8]}: {result.status.value} ({result.execution_time:.2f}s)"
            )
