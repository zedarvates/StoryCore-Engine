...
class ComfyUIConfig:
    ...
    def from_env(cls) -> "ComfyUIConfig":
        ...
        if "COMFYUI_PORT" in os.environ:
            try:
                ...
                logger.warning(
                    f"Invalid COMFYUI_PORT value: {os.environ['COMFYUI_PORT']}, using default"
                )
        ...
        if "COMFYUI_TIMEOUT" in os.environ:
            try:
                ...
                logger.warning(
                    f"Invalid COMFYUI_TIMEOUT value: {os.environ['COMFYUI_TIMEOUT']}, using default"
                )

        if "COMFYUI_CONNECTION_TIMEOUT" in os.environ:
            try:
                config.connection_timeout = int(
                    os.environ["COMFYUI_CONNECTION_TIMEOUT"]
                )
            except ValueError:
                logger.warning(
                    "Invalid COMFYUI_CONNECTION_TIMEOUT value, using default"
                )

        if "COMFYUI_GENERATION_TIMEOUT" in os.environ:
            try:
                config.generation_timeout = int(
                    os.environ["COMFYUI_GENERATION_TIMEOUT"]
                )
            except ValueError:
                logger.warning(
                    "Invalid COMFYUI_GENERATION_TIMEOUT value, using default"
                )
        ...
        if "COMFYUI_MAX_RETRIES" in os.environ:
            try:
                ...
                logger.warning("Invalid COMFYUI_MAX_RETRIES value, using default")

        if "COMFYUI_HEALTH_CHECK_INTERVAL" in os.environ:
            try:
                config.health_check_interval = int(
                    os.environ["COMFYUI_HEALTH_CHECK_INTERVAL"]
                )
            except ValueError:
                logger.warning(
                    "Invalid COMFYUI_HEALTH_CHECK_INTERVAL value, using default"
                )
        ...
        if "COMFYUI_RETRY_BACKOFF" in os.environ:
            try:
                ...
                logger.warning("Invalid COMFYUI_RETRY_BACKOFF value, using default")

        # Boolean values
        if "COMFYUI_AUTO_DOWNLOAD" in os.environ:
            config.auto_download_models = os.environ[
                "COMFYUI_AUTO_DOWNLOAD"
            ].lower() in ("true", "1", "yes", "on")

        if "COMFYUI_AUTO_DEPLOY_WORKFLOWS" in os.environ:
            config.auto_deploy_workflows = os.environ[
                "COMFYUI_AUTO_DEPLOY_WORKFLOWS"
            ].lower() in ("true", "1", "yes", "on")

        if "COMFYUI_ENABLE_CORS_CHECK" in os.environ:
            config.enable_cors_check = os.environ[
                "COMFYUI_ENABLE_CORS_CHECK"
            ].lower() in ("true", "1", "yes", "on")
        ...
        if errors:
            logger.warning(
                f"Configuration validation warnings from environment: {errors}"
            )
    ...
    def validate(self) -> List[str]:
        ...
        if (
            not isinstance(self.connection_timeout, (int, float))
        ...
        if (
            not isinstance(self.generation_timeout, (int, float))
            or self.generation_timeout <= 0
        ):
            errors.append(
                f"generation_timeout must be positive, got {self.generation_timeout}"
            )
        ...
        if self.fallback_mode not in valid_fallback_modes:
            errors.append(
                f"fallback_mode must be one of {valid_fallback_modes}, got '{self.fallback_mode}'"
            )

        # Validate health check interval
        if (
            not isinstance(self.health_check_interval, (int, float))
            or self.health_check_interval <= 0
        ):
            errors.append(
        ...
        if self.models_directory is not None:
            ...
            if not models_path.exists():
                errors.append(
                    f"models_directory does not exist: {self.models_directory}"
                )
        ...
        if self.workflows_directory is not None:
            ...
            if not workflows_path.exists():
                errors.append(
                    f"workflows_directory does not exist: {self.workflows_directory}"
                )
...
class ConfigurationManager:
    ...
    def load_config(self, use_env: bool = True) -> ComfyUIConfig:
        ...
        if self.config_file.exists():
            ...
            logger.info(
                f"No configuration file found at {self.config_file}, using defaults"
            )
    ...
    def _get_env_overrides(self) -> Dict[str, Any]:
        ...
        if "COMFYUI_PORT" in os.environ:
            try:
                ...
                logger.warning(
                    f"Invalid COMFYUI_PORT value: {os.environ['COMFYUI_PORT']}, ignoring"
                )
        ...
        if "COMFYUI_TIMEOUT" in os.environ:
            try:
                ...
                logger.warning(
                    f"Invalid COMFYUI_TIMEOUT value: {os.environ['COMFYUI_TIMEOUT']}, ignoring"
                )

        if "COMFYUI_CONNECTION_TIMEOUT" in os.environ:
            try:
                overrides["connection_timeout"] = int(
                    os.environ["COMFYUI_CONNECTION_TIMEOUT"]
                )
            except ValueError:
                logger.warning("Invalid COMFYUI_CONNECTION_TIMEOUT value, ignoring")

        if "COMFYUI_GENERATION_TIMEOUT" in os.environ:
            try:
                overrides["generation_timeout"] = int(
                    os.environ["COMFYUI_GENERATION_TIMEOUT"]
                )
            except ValueError:
                logger.warning("Invalid COMFYUI_GENERATION_TIMEOUT value, ignoring")

        if "COMFYUI_MAX_RETRIES" in os.environ:
            try:
                overrides["max_retries"] = int(os.environ["COMFYUI_MAX_RETRIES"])
            except ValueError:
                logger.warning("Invalid COMFYUI_MAX_RETRIES value, ignoring")

        if "COMFYUI_HEALTH_CHECK_INTERVAL" in os.environ:
            try:
                overrides["health_check_interval"] = int(
                    os.environ["COMFYUI_HEALTH_CHECK_INTERVAL"]
                )
            except ValueError:
                logger.warning("Invalid COMFYUI_HEALTH_CHECK_INTERVAL value, ignoring")
        ...
        if "COMFYUI_RETRY_BACKOFF" in os.environ:
            try:
                ...
                logger.warning("Invalid COMFYUI_RETRY_BACKOFF value, ignoring")

        # Boolean values
        if "COMFYUI_AUTO_DOWNLOAD" in os.environ:
            overrides["auto_download_models"] = os.environ[
                "COMFYUI_AUTO_DOWNLOAD"
            ].lower() in ("true", "1", "yes", "on")

        if "COMFYUI_AUTO_DEPLOY_WORKFLOWS" in os.environ:
            overrides["auto_deploy_workflows"] = os.environ[
                "COMFYUI_AUTO_DEPLOY_WORKFLOWS"
            ].lower() in ("true", "1", "yes", "on")

        if "COMFYUI_ENABLE_CORS_CHECK" in os.environ:
            overrides["enable_cors_check"] = os.environ[
                "COMFYUI_ENABLE_CORS_CHECK"
            ].lower() in ("true", "1", "yes", "on")
