...
class VideoConfigManager:
    ...
    def _load_interpolation_config(self, data: Dict[str, Any]):
        ...
        if "character_preservation" in data:
            self.config.interpolation.character_preservation = data[
                "character_preservation"
            ]
        if "motion_blur_strength" in data:
            self.config.interpolation.motion_blur_strength = data[
    ...
    def _load_performance_config(self, data: Dict[str, Any]):
        ...
        if "processing_mode" in data:
            self.config.performance.processing_mode = ProcessingMode(
    ...
    def _load_quality_config(self, data: Dict[str, Any]):
        ...
        if "minimum_motion_smoothness" in data:
            self.config.quality.minimum_motion_smoothness = data[
                "minimum_motion_smoothness"
            ]
        if "minimum_temporal_coherence" in data:
            self.config.quality.minimum_temporal_coherence = data[
                "minimum_temporal_coherence"
            ]
        ...
        if "quality_report_generation" in data:
            self.config.quality.quality_report_generation = data[
    ...
    def _validate_interpolation_config(self) -> List[str]:
        ...
        if self.config.interpolation.algorithm not in [
            "linear",
            "optical_flow",
            "depth_aware",
        ]:
            issues.append(
                f"Invalid interpolation algorithm: {self.config.interpolation.algorithm}"
            )

        if not 0.0 <= self.config.interpolation.motion_blur_strength <= 1.0:
            issues.append(
                f"Motion blur strength must be 0.0-1.0: {self.config.interpolation.motion_blur_strength}"
            )

        if not 0.0 <= self.config.interpolation.smoothing_factor <= 1.0:
            issues.append(
                f"Smoothing factor must be 0.0-1.0: {self.config.interpolation.smoothing_factor}"
            )
    ...
    def _validate_camera_config(self) -> List[str]:
        ...
        if self.config.camera.motion_blur_samples < 1:
            issues.append(
                f"Motion blur samples must be >= 1: {self.config.camera.motion_blur_samples}"
            )

        if self.config.camera.easing_type not in [
            "linear",
            "ease_in",
            "ease_out",
            "ease_in_out",
        ...
        if not 0.0 <= self.config.camera.smoothing_factor <= 1.0:
            issues.append(
                f"Camera smoothing factor must be 0.0-1.0: {self.config.camera.smoothing_factor}"
            )

        if self.config.camera.focal_length <= 0:
            issues.append(
                f"Focal length must be positive: {self.config.camera.focal_length}"
            )
    ...
    def _validate_performance_config(self) -> List[str]:
        ...
        if self.config.performance.max_threads < 0:
            issues.append(
                f"Max threads must be >= 0: {self.config.performance.max_threads}"
            )

        if self.config.performance.memory_limit_gb <= 0:
            issues.append(
                f"Memory limit must be positive: {self.config.performance.memory_limit_gb}"
            )

        if not 0.1 <= self.config.performance.gpu_memory_fraction <= 1.0:
            issues.append(
                f"GPU memory fraction must be 0.1-1.0: {self.config.performance.gpu_memory_fraction}"
            )

        if self.config.performance.cache_size_gb < 0:
            issues.append(
                f"Cache size must be >= 0: {self.config.performance.cache_size_gb}"
            )
    ...
    def _validate_quality_config(self) -> List[str]:
        ...
        if not 0.0 <= self.config.quality.minimum_visual_quality <= 1.0:
            issues.append(
                f"Visual quality threshold must be 0.0-1.0: {self.config.quality.minimum_visual_quality}"
            )

        if not 0.0 <= self.config.quality.minimum_motion_smoothness <= 1.0:
            issues.append(
                f"Motion smoothness threshold must be 0.0-1.0: {self.config.quality.minimum_motion_smoothness}"
            )

        if not 0.0 <= self.config.quality.minimum_temporal_coherence <= 1.0:
            issues.append(
                f"Temporal coherence threshold must be 0.0-1.0: {self.config.quality.minimum_temporal_coherence}"
            )
...
def main():
    ...
    for preset in presets:
        ...
        if config:
            print(
                f"{preset.title()} preset: {config.output.resolution}, {config.interpolation.algorithm}"
            )
