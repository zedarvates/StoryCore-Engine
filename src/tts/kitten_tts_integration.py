...
class KittenTTSIntegration:
    ...
    def _load_voice_profiles(self) -> Dict[str, VoiceProfile]:
        ...
        return {
            "natural_1": VoiceProfile(
                "natural_1", "Sophie", "female", "en-US", ["soft", "warm", "narrator"]
            ),
            "heroic_male": VoiceProfile(
                ...
                "male",
                "en-GB",
                ["deep", "authoritative", "thespian"],
            ),
            "child_playful": VoiceProfile(
                "child_playful",
                "Leo",
                "male",
                "en-US",
                ["high-pitched", "energetic", "child"],
            ),
            "villain_dark": VoiceProfile(
                "villain_dark",
                "Malakor",
                "male",
                "en-US",
    ...
    async def synthesize(
        ...
        p_offset = (
            pitch_offset if pitch_offset is not None else self.config.pitch_offset
        )
        intn = intonation or self.config.intonation

        self.logger.info(
            f"Synthesizing text: '{text[:30]}...' with voice {v_id}, emotion={emotion}, pitch={p_offset}, intonation={intn}"
        )
        ...
        try:
            ...
            output_path = (
                self.output_dir / f"kitten_{v_id}_{safe_text}_{int(time.time())}.wav"
            )
            ...
            for i, word in enumerate(words):
                ...
                phonemes.append(
                    {
                        "phoneme": word[0].upper(),
                        "start_time": current_t + (i * dur),
                        "duration": dur / 2,
                    }
                )
                ...
                if len(word) > 3:
                    phonemes.append(
                        {
                            "phoneme": word[2].upper(),
                            "start_time": current_t + (i * dur) + dur / 2,
                            "duration": dur / 2,
                        }
                    )
    ...
    async def clone_voice(
        self, reference_audio_path: str, new_voice_name: str
    ) -> VoiceProfile:
        """
        Clones a new voice from a reference audio sample.
        """
        self.logger.info(
            f"Cloning voice from {reference_audio_path} as {new_voice_name}"
        )
