...
"""
    ...
    async def suggest_shot_composition(
        self, request: ShotCompositionRequest
    ) -> Optional[ShotCompositionResult]:
        """
        ...
        async def _compose_operation():
            try:
                ...
                context_analysis = await self._analyze_scene_context(
                ...
                guidelines = await self._generate_composition_guidelines(
                    request.scene_context
                )
                technical_rec = await self._generate_technical_recommendations(request)
                artistic_notes = await self._generate_artistic_notes(
                    request, suggested_shots
                )
                ...
                self.logger.info(
                    f"Generated shot composition suggestions: {request_id}"
                )
                return result
    ...
    def _get_recommended_shot_types(
        self, scene_context: SceneContext
    ) -> List[ShotType]:
    ...
    def _get_preferred_rules(
        ...
        if scene_context.emotional_tone == "dramatic":
            ...
            return [
                CompositionRule.SYMMETRICAL,
                CompositionRule.GOLDEN_RATIO,
                CompositionRule.BALANCED,
            ]
        elif scene_context.action_intensity >= 7:
            return [
                CompositionRule.DYNAMIC,
                CompositionRule.LEADING_LINES,
                CompositionRule.RULE_OF_THIRDS,
            ]
        else:
            return [
                CompositionRule.RULE_OF_THIRDS,
                CompositionRule.BALANCED,
                CompositionRule.DEPTH_OF_FIELD,
            ]
    ...
    async def _generate_shot_suggestions(
        self, request: ShotCompositionRequest, context_analysis: Dict[str, Any]
    ) -> List[ShotSuggestion]:
        ...
        for i, shot_type in enumerate(context_analysis["recommended_shot_types"][:3]):
            ...
            camera_movement = self._get_recommended_movement(
                shot_type, request.scene_context
            )

            # Determine composition rule
            composition_rule = context_analysis["preferred_composition_rules"][
                i % len(context_analysis["preferred_composition_rules"])
            ]

            # Determine lighting style
            lighting_style = context_analysis["ideal_lighting_styles"][
                i % len(context_analysis["ideal_lighting_styles"])
            ]

            # Generate camera position
            camera_position = self._generate_camera_position(
                shot_type, request.character_positions
            )
            ...
            reasoning = self._generate_reasoning(
                shot_type, composition_rule, lighting_style, request.scene_context
            )
    ...
    def _get_recommended_movement(
        self, shot_type: ShotType, scene_context: SceneContext
    ) -> CameraMovement:
    ...
    def _generate_camera_position(
        self, shot_type: ShotType, character_positions: List[CharacterPosition]
    ) -> CameraPosition:
    ...
    def _generate_reasoning(
        ...
        return (
            ...
            if reasoning_parts
            else "Optimal composition choice"
        )
    ...
    def _generate_alternatives(self, shot_type: ShotType, index: int) -> List[str]:
        ...
        if shot_type == ShotType.CLOSE_UP:
            alternatives.extend(
                ["Medium shot for context", "Over-the-shoulder for perspective"]
            )
            ...
            alternatives.extend(
                ["Medium shot for detail", "Establishing shot for context"]
            )

        return alternatives

    async def _calculate_overall_score(
        self, suggested_shots: List[ShotSuggestion]
    ) -> float:
    ...
    async def _generate_composition_guidelines(
        self, scene_context: SceneContext
    ) -> List[str]:
    ...
    async def _generate_artistic_notes(
        self, request: ShotCompositionRequest, suggested_shots: List[ShotSuggestion]
    ) -> List[str]:
    ...
    def _get_average_score(self) -> float:
        ...
        total_score = sum(
            result.overall_score for result in self.composition_cache.values()
        )
    ...
    def _get_most_used_rules(self) -> List[str]:
        ...
        return sorted(rule_counts.keys(), key=lambda x: rule_counts[x], reverse=True)[
            :3
        ]

    async def export_composition(
        self, composition_id: str, format: str = "json"
