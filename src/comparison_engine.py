...
try:
    ...
    raise ImportError(
        "Pillow is required for the compare command. Install with: pip install Pillow>=10.0.0"
    )
...
def create_comparison_images(
    ...
    out_dir: str = "assets/images/compare",
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        refinement_metrics = project_data.get("asset_manifest", {}).get(
            "refinement_metrics", {}
        )
    ...
    if panel == "all":
        ...
        if not panel_numbers:
            raise FileNotFoundError(
                "No promoted panels found. Run 'storycore promote' first."
            )

        if mode == "grid":
            output_file = output_dir / "compare_all.png"
            _create_grid_comparison(
                promoted_dir, refined_dir, panel_numbers, output_file, metrics_data
            )
        else:  # side-by-side
            output_file = output_dir / "compare_all.png"
            _create_stacked_comparison(
                promoted_dir, refined_dir, panel_numbers, output_file, metrics_data
            )

        comparison_assets.append(
            {
                "asset_id": "compare_all_v1",
        ...
        if not promoted_file.exists():
            raise FileNotFoundError(
                f"Promoted panel not found: {promoted_file}. Run 'storycore promote' first."
            )

        if not refined_file.exists():
            raise FileNotFoundError(
                f"Refined panel not found: {refined_file}. Run 'storycore refine' first."
            )

        output_file = output_dir / f"compare_panel_{panel_num}.png"
        panel_key = f"panel_{panel_num}"

        _create_single_comparison(
            promoted_file, refined_file, output_file, metrics_data.get(panel_key)
        )
...
def _create_single_comparison(
    ...
    with (
        Image.open(promoted_file) as promoted_img,
        Image.open(refined_file) as refined_img,
    ):
        # Ensure same height
        if promoted_img.size[1] != refined_img.size[1]:
            # Resize refined to match promoted height
            new_width = int(
                refined_img.size[0] * promoted_img.size[1] / refined_img.size[1]
            )
            refined_img = refined_img.resize(
                (new_width, promoted_img.size[1]), Image.Resampling.LANCZOS
            )
        ...
        comparison.paste(
            refined_img, (promoted_img.size[0] + 2 * margin, header_height + margin)
        )
        ...
        # Labels
        draw.text(
            (margin + promoted_img.size[0] // 2 - 40, 20),
            "PROMOTED",
            fill="black",
            font=font,
        )
        draw.text(
            (promoted_img.size[0] + 2 * margin + refined_img.size[0] // 2 - 35, 20),
            "REFINED",
            fill="black",
            font=font,
        )
        ...
        if metrics:
            ...
            metrics_text = (
                f"Sharpness: {before:.1f} → {after:.1f} ({improvement:+.1f}%)"
            )
...
def _create_grid_comparison(
    ...
    for promoted_img, refined_img, panel_num in images:
        ...
        if panel_key in metrics_data:
            ...
            draw.text(
                (margin, y_offset + promoted_img.size[1] + 5),
                metrics_text,
                fill="blue",
                font=font,
            )
...
def _create_stacked_comparison(
    promoted_dir: Path,
    refined_dir: Path,
    panel_numbers: List[str],
    output_file: Path,
    ...
    for panel_num in panel_numbers:
        ...
        with (
            ...
            comparison.paste(
                refined_img, (promoted_img.size[0] + 2 * margin, header_height)
            )
            ...
            draw.text(
                (margin, 10), f"Panel {panel_num} - PROMOTED", fill="black", font=font
            )
            draw.text(
                (promoted_img.size[0] + 2 * margin, 10),
                "REFINED",
                fill="black",
                font=font,
            )
...
def update_project_manifest_comparison(
    project_path: Path, comparison_data: Dict[str, Any]
) -> None:
    ...
    project_data["asset_manifest"]["comparison_assets"] = comparison_data[
        "comparison_assets"
    ]
