    ...
    def _load_custom_prompts(self, config_path: Optional[str]) -> Dict[str, str]:
        ...
        if config_path:
            try:
                ...
                logger.error(
                    f"Erreur lors du chargement des prompts personnalisés: {e}"
                )

        return custom_prompts

    def generate_prompt(
        self, voice_type: str, emotion: str, language: str, use_few_shot: bool = False
    ) -> str:
        """
        ...
        if not prompt:
            logger.warning(
                f"Aucun template de prompt trouvé pour voice_type={voice_type} et emotion={emotion}"
            )
        ...
        if use_few_shot:
            prompt = self._apply_few_shot_learning(
                prompt, voice_type, emotion, language
            )

        return prompt

    def _apply_few_shot_learning(
        self, prompt: str, voice_type: str, emotion: str, language: str
    ) -> str:
        """
...
def main():
    ...
    few_shot_prompt = prompt_strategy.generate_prompt(
        voice_type, emotion, language, use_few_shot=True
    )
