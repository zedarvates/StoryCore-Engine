...
class MusicStructure:
    ...
    def verse_duration(self) -> float:
        total = sum(
            ...
            if s.section_type == MusicSection.VERSE
        )
        return total
    ...
    def chorus_duration(self) -> float:
        total = sum(
            s.end_time - s.start_time
            for s in self.sections
            if s.section_type == MusicSection.CHORUS
        )
        return total
    ...
    def bridge_duration(self) -> float:
        total = sum(
            s.end_time - s.start_time
            for s in self.sections
            if s.section_type == MusicSection.BRIDGE
        )
        return total
...
class AudioRemixEngine:
    ...
    async def _perform_analysis(self, audio_url: str) -> MusicStructure:
        ...
        self.logger.info(
            f"Analyzed structure: {len(sections)} sections, {len(beats)} beats"
        )
        return structure
        ...
        *,
        ...
        if request is None:
            if music_url is None or target_duration is None:
                raise ValueError(
                    "music_url and target_duration are required when request is not provided"
                )
            ...
            request = RemixRequest(
                ...
                preserve_outro=False
                if preserve_sections is None
                else ("outro" in preserve_sections),
                preserve_intro=False
                if preserve_sections is None
                else ("intro" in preserve_sections),
            )
    ...
    async def _perform_remix(
        self, request: RemixRequest, structure: MusicStructure
    ) -> RemixResult:
        """Effectuer le remixage effectif."""
        self.logger.info(f"Remixing audio: {request.audio_id}")
        self.logger.info(
            f"Target duration: {request.target_duration}s (original: {structure.duration}s)"
        )
        ...
        output_url = (
            f"/api/audio/remixed/{request.audio_id}_{request.target_duration}.wav"
        )
        ...
        self.logger.info(
            f"Remix complete: {len(cuts)} cuts, {final_duration:.1f}s final duration"
        )
        return result

    def _calculate_cuts(
        self, structure: MusicStructure, target_duration: float, request: RemixRequest
    ) -> List[RemixCut]:
        ...
        for section in structure.sections:
            ...
            if (
                not request.preserve_intro
                and section.section_type == MusicSection.INTRO
            ):
                available_sections.append(
                    (section_priority[section.section_type], section)
                )
            elif (
                not request.preserve_outro
                and section.section_type == MusicSection.OUTRO
            ):
                available_sections.append(
                    (section_priority[section.section_type], section)
                )
        ...
        for cut in cuts:
            for section in structure.sections:
                if (
                    cut.end_time >= section.start_time
                    and cut.end_time < section.end_time
                ):
    ...
    def _find_cut_position(
        self, section: SectionMarker, cut_amount: float, beats: List[BeatMarker]
    ) -> float:
        """Trouver une bonne position de coupe (sur un beat)."""
    ...
    def _calculate_crossfades(
        self, cuts: List[RemixCut], structure: MusicStructure, request: RemixRequest
    ) -> List[Dict[str, float]]:
    ...
    def _calculate_quality_score(
        self, structure: MusicStructure, cuts: List[RemixCut], request: RemixRequest
    ) -> float:
    ...
    def get_recommended_duration(
        self, structure: MusicStructure, target_duration: float
    ) -> Dict[str, Any]:
        ...
        removable_sections = [
            ...
            if s.section_type
            in [MusicSection.OUTRO, MusicSection.INTRO, MusicSection.BRIDGE]
        ]
            ...
            recommendations["recommended_cuts"].append(
                {
                    ...
                    "priority": "high"
                    if section.section_type == MusicSection.OUTRO
                    else "medium",
                }
            )
    ...
    async def preview_remix(self, request: RemixRequest) -> Dict[str, Any]:
        ...
        if structure.duration <= request.target_duration:
            return {
                ...
                "recommendations": self.get_recommended_duration(
                    structure, request.target_duration
                ),
            }
        ...
        preview = {
            ...
            "estimated_quality": self._calculate_quality_score(
                structure, cuts, request
            ),
        }
