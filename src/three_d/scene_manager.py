...
from src.three_d.rendering_engine import (
    Scene3D,
    SceneObject,
    Camera3D,
    LightSource,
...
class SceneManager:
    ...
    def remove_object_from_scene(self, scene_name: str, object_name: str) -> bool:
        ...
        for i, obj in enumerate(scene.objects):
            if obj.name == object_name:
                del scene.objects[i]
                logger.debug(
                    f"Removed object '{object_name}' from scene '{scene_name}'"
                )
                return True
    ...
    def create_scene_sequence(self, sequence_name: str, scene_names: List[str]) -> bool:
        ...
        transitions = [
            SceneTransitionSpec(SceneTransition.CUT)
            for _ in range(len(scene_names) - 1)
        ]
    ...
    def next_scene_in_sequence(self, sequence_name: str) -> bool:
        ...
        logger.info(
            f"Advanced to scene {sequence.current_scene_index}: {self.current_scene.name}"
        )
        return True
...
def main():
    ...
    print(
        f"[SUCCESS] Created scene sequence with {len(manager.list_scene_sequences())} sequences"
    )
