...
class PrimitiveObject:
    ...
    def __init__(self, primitive_id, type, name, position=(0,0,0), rotation=(0,0,0), scale=(1,1,1), 
                 color="#CCCCCC", roughness=0.5, metallic=0.0, material_modifier_id=None):
...
class SetLayout:
    ...
    def __init__(self, scene_id, environment_type="interior", primitives=None, puppets=None, 
                 lighting_setup=None, camera_shot=None):
...
class SetBuilderEngine:
    ...
    def generate_layout_from_breakdown(self, scene_breakdown: Dict[str, Any]) -> SetLayout:
        """
        ...
        if env_type == "interior":
            # Add Ground Plane
            layout.primitives.append(PrimitiveObject(
        ...
        for char in scene_breakdown.get("characters", []):
            layout.puppets.append({
                ...
                "default_pose": "standing_A" if char.get("role_in_scene") == "primary_focus" else "idle",
                "position": char.get("positioning", {}).get("world_position", (0, 0, 0))
            })
            
        return layout

    def export_comfy_manifest(self, layout: SetLayout, export_path: Optional[str] = None) -> Path:
        """
        ...
        path = Path(export_path or self.output_dir / f"set_{layout.scene_id}_comfy.json")
