...
class CompositionEngine:
    ...
    def create_composition(
        ...
        if background_image_path:
            ...
            composition.background_image_size = resolution or self._get_image_size(
    ...
    def remove_object(self, composition: SceneComposition, placement_id: str) -> bool:
        ...
        composition.objects = [
            obj for obj in composition.objects if obj.placement_id != placement_id
        ]
    ...
    def calculate_screen_position(
        ...
        distance = (
            rel_pos[0] * forward[0] + rel_pos[1] * forward[1] + rel_pos[2] * forward[2]
        )
    ...
    def render_composition(
        self, composition: SceneComposition, output_path: Optional[str] = None
    ) -> CompositionResult:
    ...
    def _render_mock(
        self, composition: SceneComposition, output_path: Optional[str]
    ) -> CompositionResult:
        ...
        if output_path is None:
            ...
            output_path = (
                f"output/compositions/{composition.composition_id}_{timestamp}.png"
            )
    ...
    def export_composition_data(
        self, composition: SceneComposition, file_path: str
    ) -> bool:
        ...
        try:
            ...
            for obj_data in data.get("objects", []):
                placement = ObjectPlacement3D(
                    ...
                    depth_value=obj_data.get(
                        "depth", obj_data.get("position", [0, 0, 0])[2]
                    ),
    ...
    def create_video_keyframe(
        self, composition: SceneComposition, frame_number: int, fps: int = 24
    ) -> Dict[str, Any]:
...
def main():
    ...
    # Set camera
    engine.set_camera(composition, position=(0, 2, -8), target=(0, 1, 0), fov=50.0)
    ...
    result = engine.render_composition(
        composition, output_path="output/compositions/test.png"
    )
    ...
    for obj_pos in result.object_positions:
        print(
            f"  - {obj_pos['object_name']}: ({obj_pos['screen_position']['x']:.1f}, {obj_pos['screen_position']['y']:.1f})"
        )
