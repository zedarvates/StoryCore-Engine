...
from src.three_d.rendering_engine import Camera3D
...
class CameraSystem:
    ...
    def _interpolate_camera_position(self, path: CameraPath):
        ...
        pos_x = (
            prev_kf.position[0] + (next_kf.position[0] - prev_kf.position[0]) * eased_t
        )
        pos_y = (
            prev_kf.position[1] + (next_kf.position[1] - prev_kf.position[1]) * eased_t
        )
        pos_z = (
            prev_kf.position[2] + (next_kf.position[2] - prev_kf.position[2]) * eased_t
        )
    ...
    def create_camera_rig(
        ...
        rig = CameraRig(name=name, position=position)
...
def main():
    ...
    test_camera = Camera3D(position=(10, -10, 10), target=(0, 0, 0), fov=45.0)
    camera_system.set_current_camera(test_camera)

    # Create a simple camera path
    keyframes = [
        CameraKeyframe(time=0.0, position=(10, -10, 10), target=(0, 0, 0), fov=45.0),
        CameraKeyframe(
            time=2.0,
            position=(0, -10, 10),
            target=(0, 0, 0),
        ...
        CameraKeyframe(time=4.0, position=(-10, -10, 10), target=(0, 0, 0), fov=45.0),
    ]
    ...
    camera_system.create_orbit_path(
        name="orbit_path", center=(0, 0, 0), radius=15, duration=10.0, samples=20
    )
