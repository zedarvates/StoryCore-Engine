...
try:
    ...
    raise ImportError(
        "Pillow is required for the refine command. Install with: pip install Pillow>=10.0.0"
    )
...
def compute_sharpness_laplacian_variance(image: Image.Image) -> float:
    ...
    if HAS_NUMPY:
        ...
        variance = sum((x - mean_val) ** 2 for x in laplacian_values) / len(
            laplacian_values
        )
        return variance
...
def refine_images(
    ...
    if (
        not input_files
        and input_source == "promoted"
        and fallback_dir
        and fallback_dir.exists()
    ):
    ...
    if mode == "unsharp":
        filter_obj = ImageFilter.UnsharpMask(
            radius=2, percent=int(150 * strength), threshold=3
        )
    elif mode == "sharpen":
    ...
    for input_file in sorted(input_files):
        ...
        with Image.open(input_file) as img:
            ...
            sharpness_before = (
                compute_sharpness_laplacian_variance(img) if compute_metrics else None
            )
            ...
            sharpness_after = (
                compute_sharpness_laplacian_variance(refined_img)
                if compute_metrics
                else None
            )
            ...
            if (
                ...
                improvement_pct = (
                    ((sharpness_after - sharpness_before) / sharpness_before * 100)
...
def update_project_manifest_refined(
    project_path: Path, refinement_metadata: Dict[str, Any]
) -> None:
    ...
    project_data["asset_manifest"]["refined_panels"] = refinement_metadata[
        "refined_panels"
    ]
    ...
    if "refinement_metrics" in refinement_metadata:
        project_data["asset_manifest"]["refinement_metrics"] = refinement_metadata[
            "refinement_metrics"
        ]
