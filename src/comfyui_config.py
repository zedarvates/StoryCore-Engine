...
class ComfyUIConfig:
    ...
    def validate(self) -> Dict[str, str]:
        ...
        if not self.installation_path.exists():
            errors["installation_path"] = (
                f"ComfyUI installation path does not exist: {self.installation_path}"
            )
...
class ControlNetConfig:
    ...
    def validate(self) -> Dict[str, str]:
        ...
        if not (0.0 <= self.strength <= 2.0):
            errors["strength"] = (
                f"ControlNet strength must be between 0.0 and 2.0, got {self.strength}"
            )

        if self.control_image_path and not self.control_image_path.exists():
            errors["control_image_path"] = (
                f"Control image not found: {self.control_image_path}"
            )
...
class IPAdapterConfig:
    ...
    def validate(self) -> Dict[str, str]:
        ...
        if not (0.0 <= self.weight <= 2.0):
            errors["weight"] = (
                f"IP-Adapter weight must be between 0.0 and 2.0, got {self.weight}"
            )

        if not (0.0 <= self.noise <= 1.0):
            errors["noise"] = (
                f"IP-Adapter noise must be between 0.0 and 1.0, got {self.noise}"
            )

        if not self.reference_image_path.exists():
            errors["reference_image_path"] = (
