...
class StoryCoreVideoPromptIntegration:
    ...
    def recommend_template_for_project(
        self, project_type: str, audience: str = "general"
    ) -> str:
    ...
    def _generate_prompt_for_sequence(
        self, sequence: Dict[str, Any], template_name: str
    ) -> str:
        ...
        actions = sequence.get(
            "actions", ["se déplacer", "interagir avec l'environnement"]
        )
        details = {
            "détail spécifique": sequence.get("detail", "la fluidité des mouvements"),
            "description de l'environnement": sequence.get(
                "environment", "un environnement générique"
            ),
        }
    ...
    def export_prompts_to_storycore_format(
        self, project_data: Dict[str, Any], output_dir: str = "storycore_output"
    ) -> None:
            ...
            for i, sequence in enumerate(project_data["sequences"]):
                if "video_prompt" in sequence:
                    prompt_file = os.path.join(
                        output_dir, f"sequence_{i:03d}_video_prompt.json"
                    )
                    prompt_data = {
                        "sequence_id": sequence.get("id", i),
                        "video_prompt": sequence["video_prompt"],
                        "template_used": self._determine_template_for_sequence(
                            sequence
                        ),
                        "metadata": {
    ...
    def integrate_with_timeline(self, timeline_data: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if track["type"] == "video":
                    for clip in track.get("clips", []):
                        ...
                        prompt = self.generator.generate_prompt(
                            ...
                            details={
                                "détail spécifique": clip.get(
                                    "detail", "la fluidité des mouvements"
                                ),
                                "description de l'environnement": clip.get(
                                    "environment", "un environnement générique"
                                ),
                            },
                        )
...
def main():
    ...
    print(
        "Intégration avancée réussie! Les prompts vidéo ont été générés et exportés dans le répertoire 'storycore_output'."
    )
