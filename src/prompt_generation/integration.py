...
class VideoPromptIntegration:
        ...
        if "sequences" in project_data:
            for sequence in project_data["sequences"]:
                ...
                prompt = self.generator.generate_prompt(
                    ...
                    details={
                        "détail spécifique": sequence.get(
    ...
    def generate_prompt_for_scene(
        self, scene_data: Dict[str, Any], template_name: str = "video_dynamique"
    ) -> str:
        ...
        details = {
            ...
            "description de l'environnement": scene_data.get(
                "environment", "un environnement générique"
            ),
        }
    ...
    def save_project_with_prompts(
        self, project_data: Dict[str, Any], output_dir: str = "output"
    ) -> None:
            ...
            for i, sequence in enumerate(project_data["sequences"]):
                if "video_prompt" in sequence:
                    prompt_file = os.path.join(
                        output_dir, f"sequence_{i}_video_prompt.txt"
                    )
...
def main():
    ...
    print(
        "Projet intégré avec succès! Les prompts vidéo ont été générés et sauvegardés dans le répertoire 'output'."
    )
