...
try:
    ...
    raise ImportError(
        "Pillow and numpy are required for the promote command. Install with: pip install Pillow>=10.0.0 numpy"
    )
    ...
    for panel_file in panel_files:
        ...
        with Image.open(panel_file) as img:
            ...
            if quality_validator:
                ...
                if (
                    autofix_engine_instance
                    and sharpness_score < quality_validator.sharpness_threshold
                ):
                    ...
                    if needs_retry:
                        ...
                        corrected_img, new_params = (
                            autofix_engine_instance.apply_corrections(
                                processed_img,
                        ...
                        corrected_sharpness = quality_validator.calculate_sharpness(
                            corrected_frame
                        )
...
def update_project_manifest(
    project_path: Path, promotion_metadata: Dict[str, Any]
) -> None:
    ...
    if "asset_manifest" not in project_data or not isinstance(
        project_data["asset_manifest"], dict
    ):
        project_data["asset_manifest"] = {}

    project_data["asset_manifest"]["promoted_panels"] = promotion_metadata[
        "promoted_panels"
    ]
    ...
    if quality_validation.get("enabled", False):
        if "quality_validation" not in project_data or not isinstance(
            project_data["quality_validation"], dict
        ):
            project_data["quality_validation"] = {}

        project_data["quality_validation"]["last_validation_timestamp"] = (
            datetime.utcnow().isoformat() + "Z"
        )
        project_data["quality_validation"]["quality_scores"] = quality_validation.get(
            "quality_scores", []
        )
        project_data["quality_validation"]["autofix_logs"] = quality_validation.get(
            "autofix_logs", []
        )

        # Calculate overall quality score
        scores = [
            float(s.get("sharpness_score", 0))
            for s in quality_validation.get("quality_scores", [])
        ]
        if scores:
            project_data["quality_validation"]["overall_quality_score"] = float(
                sum(scores) / len(scores)
            )
            project_data["quality_validation"]["validation_pass"] = bool(
                project_data["quality_validation"]["overall_quality_score"] >= 70.0
            )
        else:
