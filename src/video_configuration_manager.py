...
class ResolutionConfig:
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if self.width % 2 != 0 or self.height % 2 != 0:
            issues.append(
                "Width and height should be even numbers for video compatibility"
            )
        ...
        if self.aspect_ratio in common_ratios:
            ...
            if abs(calculated_ratio - expected_ratio) > 0.01:
                issues.append(
                    f"Aspect ratio mismatch: {self.aspect_ratio} expected {expected_ratio:.3f}, got {calculated_ratio:.3f}"
                )
...
class MotionCurveConfig:
    ...
    control_points: List[Tuple[float, float]] = field(
        default_factory=lambda: [(0.0, 0.0), (0.33, 0.0), (0.67, 1.0), (1.0, 1.0)]
    )
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if self.curve_type == MotionCurveType.CUSTOM_BEZIER:
            ...
            for i, (x, y) in enumerate(self.control_points):
                if not 0 <= x <= 1:
                    issues.append(
                        f"Control point {i} x-coordinate must be between 0 and 1"
                    )
                if not 0 <= y <= 1:
                    issues.append(
                        f"Control point {i} y-coordinate must be between 0 and 1"
                    )
...
class CameraOverrideConfig:
    ...
    position_keyframes: List[Tuple[float, float, float]] = field(
        default_factory=list
    )  # (time, x, y)
    rotation_keyframes: List[Tuple[float, float, float, float]] = field(
        default_factory=list
    )  # (time, pitch, yaw, roll)
    zoom_keyframes: List[Tuple[float, float]] = field(
        default_factory=list
    )  # (time, zoom_factor)
    ...
    timing_keyframes: List[Tuple[float, float]] = field(
        default_factory=list
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        for i, (time, pitch, yaw, roll) in enumerate(self.rotation_keyframes):
            ...
            if not -180 <= pitch <= 180:
                issues.append(
                    f"Rotation keyframe {i} pitch must be between -180 and 180 degrees"
                )
            if not -180 <= yaw <= 180:
                issues.append(
                    f"Rotation keyframe {i} yaw must be between -180 and 180 degrees"
                )
            if not -180 <= roll <= 180:
                issues.append(
                    f"Rotation keyframe {i} roll must be between -180 and 180 degrees"
                )
        ...
        if self.custom_timing:
            for i, (orig_time, new_time) in enumerate(self.timing_keyframes):
                if not 0 <= orig_time <= 1:
                    issues.append(
                        f"Timing keyframe {i} original time must be between 0 and 1"
                    )
                if not 0 <= new_time <= 1:
                    issues.append(
                        f"Timing keyframe {i} new time must be between 0 and 1"
                    )
...
class VideoConfiguration:
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if self.quality.quality_level < 0.5 and self.depth_of_field_enabled:
            logger.warning(
                "Depth of field may impact performance at low quality settings"
            )
...
class VideoConfigurationManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None):
        ...
        logger.info(
            f"Video Configuration Manager initialized with config dir: {self.config_dir}"
        )
    ...
    def _create_documentary_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            resolution=ResolutionConfig(
                width=1920, height=1080, preset=ResolutionPreset.HD_1080P
            ),
            frame_rate=FrameRateConfig(fps=25.0, preset=FrameRatePreset.PAL_25),
            ...
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.EASE_IN_OUT, ease_strength=1.5
            ),
    ...
    def _create_cinematic_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            resolution=ResolutionConfig(
                width=3840, height=2160, preset=ResolutionPreset.UHD_4K
            ),
            ...
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.EASE_IN_OUT, ease_strength=2.5
            ),
    ...
    def _create_action_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            ...
            frame_rate=FrameRateConfig(fps=60.0, preset=FrameRatePreset.SMOOTH_60),
            quality=QualityConfig(
                preset=QualityPreset.PRODUCTION,
                quality_level=0.85,
            ...
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.EASE_OUT, ease_strength=3.0
            ),
    ...
    def _create_portrait_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            ...
            lens_type="telephoto",
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.EASE_IN_OUT, ease_strength=1.8
            ),
    ...
    def _create_broadcast_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            ...
            frame_rate=FrameRateConfig(
                fps=29.97, preset=FrameRatePreset.NTSC_30, drop_frame=True
            ),
            quality=QualityConfig(
                preset=QualityPreset.BROADCAST,
                quality_level=1.0,
    ...
    def _create_web_video_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            resolution=ResolutionConfig(
                width=1280, height=720, preset=ResolutionPreset.HD_720P
            ),
            frame_rate=FrameRateConfig(fps=30.0, preset=FrameRatePreset.NTSC_30),
            quality=QualityConfig(
            ...
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.EASE_IN_OUT, ease_strength=1.2
            ),
    ...
    def _create_social_media_preset(self) -> VideoConfiguration:
        ...
        return VideoConfiguration(
            ...
            lens_type="standard",
            motion_curves=MotionCurveConfig(
                curve_type=MotionCurveType.BOUNCE, bounce_amplitude=0.05
            ),
    ...
    def _dict_to_config(self, data: Dict[str, Any]) -> VideoConfiguration:
        ...
        if "resolution" in data and isinstance(data["resolution"], dict):
            ...
            if "preset" in data["resolution"] and isinstance(
                data["resolution"]["preset"], str
            ):
                data["resolution"]["preset"] = ResolutionPreset(
        ...
        if "frame_rate" in data and isinstance(data["frame_rate"], dict):
            # Convert enum strings back to enums
            if "preset" in data["frame_rate"] and isinstance(
                data["frame_rate"]["preset"], str
            ):
                data["frame_rate"]["preset"] = FrameRatePreset(
                    data["frame_rate"]["preset"]
                )
            data["frame_rate"] = FrameRateConfig(**data["frame_rate"])

        if "quality" in data and isinstance(data["quality"], dict):
            # Convert enum strings back to enums
            if "preset" in data["quality"] and isinstance(
                data["quality"]["preset"], str
            ):
        ...
        if "motion_curves" in data and isinstance(data["motion_curves"], dict):
            # Convert enum strings back to enums
            if "curve_type" in data["motion_curves"] and isinstance(
                data["motion_curves"]["curve_type"], str
            ):
                data["motion_curves"]["curve_type"] = MotionCurveType(
                    data["motion_curves"]["curve_type"]
                )
            # Convert lists back to tuples for control points
            if "control_points" in data["motion_curves"] and isinstance(
                data["motion_curves"]["control_points"], list
            ):
                data["motion_curves"]["control_points"] = [
        ...
        if "camera_override" in data and isinstance(data["camera_override"], dict):
            ...
            for keyframe_field in [
                "position_keyframes",
                "rotation_keyframes",
                "zoom_keyframes",
                "timing_keyframes",
            ]:
                if keyframe_field in data["camera_override"] and isinstance(
                    data["camera_override"][keyframe_field], list
                ):
                    data["camera_override"][keyframe_field] = [
                        tuple(keyframe) if isinstance(keyframe, list) else keyframe
    ...
    def get_preset_names(self) -> List[str]:
        ...
        preset_files = list(self.presets_dir.glob("*.json")) + list(
            self.presets_dir.glob("*.yml")
        )
    ...
    def create_custom_configuration(
        self, base_preset: str = "cinematic", **overrides
    ) -> VideoConfiguration:
    ...
    def optimize_for_hardware(
        ...
        if not gpu_available:
            ...
            optimized.quality.interpolation_samples = min(
                optimized.quality.interpolation_samples, 12
            )

        # Adjust memory usage
        optimized.quality.memory_limit_gb = min(
            memory_gb * 0.8, optimized.quality.memory_limit_gb
        )
        ...
        logger.info(
            f"Optimized configuration for hardware: GPU={gpu_available}, RAM={memory_gb}GB, CPU={cpu_cores} cores"
        )

        return optimized

    def validate_configuration(
        self, config: VideoConfiguration
    ) -> Tuple[bool, List[str]]:
    ...
    def serialize_configuration(
        self, config: VideoConfiguration, format_type: str
    ) -> str:
...
if __name__ == "__main__":
    ...
    custom_config = manager.create_custom_configuration(
        "documentary", motion_blur_intensity=0.8, interpolation_method="ai_based"
    )

    # Test hardware optimization
    optimized_config = manager.optimize_for_hardware(
        custom_config, gpu_available=False, memory_gb=4.0, cpu_cores=2
    )
