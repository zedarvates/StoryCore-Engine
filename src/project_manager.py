...
def validate_project_name(project_name: str, base_path: str = ".") -> tuple[bool, str]:
    ...
    if invalid_found:
        ...
        return (
            False,
            f"Project name contains invalid characters: {', '.join(display_chars)}",
        )
    ...
    if (
        name_upper in WINDOWS_RESERVED_NAMES
        or name_without_ext in WINDOWS_RESERVED_NAMES
    ):
        return (
            False,
            f"Project name '{project_name}' is a reserved system name on Windows",
        )
    ...
    try:
        ...
        if (
            ...
            and len(estimated_max_path) > WINDOWS_MAX_PATH_LENGTH
        ):
            return (
                False,
        ...
        if len(estimated_max_path) > 4096:  # Most Unix systems support up to 4096
            return (
                False,
                f"Project path would be too long ({len(estimated_max_path)} characters). Please use a shorter name.",
            )

    except Exception as e:
        # If we can't resolve the path, log it but don't fail validation
        logger.warning(
            f"Could not validate path length for project '{project_name}': {e}"
        )
...
class ProjectManager:
    ...
    def init_project(self, project_name: str, base_path: str = ".") -> dict:
        ...
        try:
            ...
            logger.info("Creating assets directory structure")
            ...
            logger.info("Creating story directory structure")
            ...
            logger.info(
                f"Project initialization completed successfully: {project_name}"
            )
            return {
                "success": True,
                "project_path": str(project_path),
    ...
    def _create_project_json(
        self, project_path: Path, project_name: str, seed: int
    ) -> None:
        """Create project.json with all required fields."""
        logger.debug(
            f"Generating project.json for project: {project_name} with seed: {seed}"
        )
        ...
        project_data = {
            ...
            "asset_manifest": {},
            "status": {"current_phase": "initialization", "qa_passed": False},
            "quality_validation": {
                "enabled": True,
                "last_validation_timestamp": None,
                "overall_quality_score": None,
        ...
        logger.debug("project.json created successfully")
    ...
    def _create_storyboard_json(self, project_path: Path, project_name: str) -> None:
        ...
        storyboard_data = {
            ...
            "shots": [
                ...
                self._create_shot_placeholder(
                    ...
                    2,
                    "Medium shot",
                    "Focus on main subject or action",
                    6,
                ),
                self._create_shot_placeholder(
                    "shot_03",
                    ...
                    3,
                    "Closing shot",
                    "Concluding shot to wrap the scene",
                    6,
                ),
            ],
        }
        ...
        logger.debug("storyboard.json created successfully")

    def _create_shot_placeholder(
        self,
        shot_id: str,
        scene_id: str,
        shot_number: int,
    ...
    def validate_project_structure(self, project_path: Path) -> tuple[bool, list[str]]:
        ...
        if is_valid:
            ...
            logger.warning(
                f"Project structure validation failed: {len(missing_items)} items missing"
            )
    ...
    def cleanup_on_failure(self, project_path: Path) -> None:
        ...
        if not project_path.exists():
            logger.info(
                f"Cleanup: Project path does not exist, nothing to clean: {project_path}"
            )
            return
        ...
        try:
            ...
            logger.error(
                f"Cleanup failed: Permission denied when removing {project_path}: {str(e)}"
            )
        except OSError as e:
            logger.error(
                f"Cleanup failed: OS error when removing {project_path}: {str(e)}"
            )
        except Exception as e:
            logger.error(
                f"Cleanup failed: Unexpected error when removing {project_path}: {str(e)}",
                exc_info=True,
            )
