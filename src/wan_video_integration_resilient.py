...
from wan_video_integration import WanVideoIntegration, AlphaChannelMode, InpaintingStage
...
class ResilientWanVideoIntegration:
    ...
    async def _create_transparent_internal(
        self,
        prompt: str,
        negative_prompt: str,
    ...
    async def _execute_transparent_fallback(
        self,
        prompt: str,
        negative_prompt: str,
        alpha_mode: AlphaChannelMode,
    ...
    async def _inpaint_video_internal(
        self,
        prompt: str,
        video_frames: List[Image.Image],
        mask: Image.Image,
    ...
    async def _execute_inpainting_fallback(
        self,
        prompt: str,
        video_frames: List[Image.Image],
        mask: Image.Image,
    ...
    async def _generate_transparent_simple(
        self,
        prompt: str,
        negative_prompt: str,
        alpha_mode: AlphaChannelMode,
    ...
    async def _generate_transparent_minimal(
        self,
        prompt: str,
        negative_prompt: str,
        alpha_mode: AlphaChannelMode,
        ...
        for i in range(num_frames):
            ...
            frame = Image.new(
                "RGBA", (self.config.width, self.config.height), (128, 128, 128, 200)
            )
            frames.append(frame)
    ...
    async def _inpaint_video_single_stage(
        self,
        prompt: str,
        video_frames: List[Image.Image],
        mask: Image.Image,
    ...
    async def _inpaint_video_simple(
        self,
        prompt: str,
        video_frames: List[Image.Image],
        mask: Image.Image,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["transparent_video_attempts"] > 0:
            stats["transparent_success_rate"] = (
                stats["transparent_video_successes"]
                / stats["transparent_video_attempts"]
            )
        else:
...
async def generate_transparent_video_resilient(
    ...
    """
    if config is None:
        config = WanVideoConfig(width=width, height=height, num_frames=num_frames)

    integration = ResilientWanVideoIntegration(config)

    try:
        result = await integration.create_transparent_video(
            prompt=prompt, alpha_mode=alpha_mode
        )
        return result
    finally:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        rgba_frames = await integration.create_transparent_video(
            prompt="A floating ghost character", alpha_mode=AlphaChannelMode.THRESHOLD
        )
