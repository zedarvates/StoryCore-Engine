...
class AutofixEngine:
    ...
    def _load_autofix_rules(self, rules_path: Optional[str]) -> Dict[str, Any]:
        ...
        default_rules = {
            ...
            "acceptable_range": {"min": 50.0, "max": 180.0},
        }
    ...
    def should_retry(
        self, panel_id: str, qa_metrics: Dict[str, Any]
        ...
        if sharpness < self.rules["under_sharpened"]["threshold"]:
            adjustments = {
                "denoising_strength": self.rules["under_sharpened"][
                    "denoising_adjustment"
                ],
        ...
        if sharpness > self.rules["over_sharpened"]["threshold"]:
            adjustments = {
                "denoising_strength": self.rules["over_sharpened"][
                    "denoising_adjustment"
                ],
    ...
    def apply_corrections(
        ...
        if "denoising_strength" in adjustments:
            new_denoising = (
                current_params.get("denoising_strength", 0.35)
                + adjustments["denoising_strength"]
            )
        ...
        if "sharpen_amount" in adjustments:
            new_sharpen = (
                current_params.get("sharpen_amount", 1.0)
                + adjustments["sharpen_amount"]
            )
    ...
    def generate_autofix_log(
        ...
        improvement_delta = (
            final_metrics["sharpness_score"] - initial_metrics["sharpness_score"]
        )
        ...
        return {
            ...
            "improvement_percent": (
                improvement_delta / initial_metrics["sharpness_score"]
            )
            * 100
            if initial_metrics["sharpness_score"] > 0
            else 0,
    ...
    def _identify_applied_rules(self, adjustments: Dict[str, float]) -> List[str]:
        ...
        if (
            adjustments.get("denoising_strength", 0) < 0
            and adjustments.get("sharpen_amount", 0) > 0
        ):
...
class EnhancedPromotionEngine:
    ...
    def _process_panel_with_autofix(
        self, master_grid: Image.Image, panel_config: Dict, plan: Dict, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        panel_slice = self._slice_panel(
            master_grid, grid_position, plan["grid_specification"]
        )
        ...
        initial_params = {
            "denoising_strength": panel_config.get(
                "denoising_strength", self.default_denoising_strength
            ),
            "sharpen_amount": 1.0,
        }

        # Initial refinement pass
        refined_panel = self._apply_refinement_with_params(
            upscaled_panel, initial_params
        )
        ...
        if needs_retry and iteration_count < self.autofix_engine.max_iterations:
            ...
            if corrected_sharpness > initial_sharpness or self._is_in_acceptable_range(
                corrected_sharpness
            ):
                final_panel = corrected_panel
                final_metrics = corrected_metrics
                print(
                    f"✓ Autofix improved {panel_id}: {initial_sharpness:.1f} → {corrected_sharpness:.1f}"
                )
            else:
        ...
        ratio_deviation = (
            abs(actual_ratio - self.target_aspect_ratio) / self.target_aspect_ratio
        )
    ...
    def _calculate_autofix_success_rate(self, autofix_logs: List[Dict]) -> float:
        ...
        improved_count = sum(
            1 for log in autofix_logs if log["final_status"] == "IMPROVED"
        )
    ...
    def _slice_panel(
        self, master_grid: Image.Image, position: List[int], grid_spec: str
    ) -> Image.Image:
    ...
    def _upscale_image(self, image: Image.Image) -> Image.Image:
        ...
        new_size = (
            image.size[0] * self.upscale_factor,
            image.size[1] * self.upscale_factor,
        )
