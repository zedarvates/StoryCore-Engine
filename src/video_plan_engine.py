...
class VideoPlanEngine:
    ...
    def generate_video_plan(self, project_path: Path) -> Dict[str, Any]:
        ...
        for i, shot in enumerate(shots):
            ...
            if not refined_path.exists():
                source_image = (
                    f"assets/images/promoted/panel_{shot_number:02d}_promoted.png"
                )
    ...
    def _update_project_manifest(
        self, project_file: Path, video_plan: Dict[str, Any]
    ) -> None:
    ...
    def save_video_plan_to_file(
        self, project_path: Path, video_plan: Dict[str, Any]
    ) -> str:
        """Save video plan to JSON file and update project manifest."""
        print(
            f"DEBUG: save_video_plan_to_file called with project_path={project_path}, has {len(video_plan.get('video_entries', []))} entries"
        )
        for i, entry in enumerate(video_plan.get("video_entries", [])):
            print(
                f"DEBUG: Saving entry {i}: description='{entry.get('description', 'N/A')}', title='{entry.get('title', 'N/A')}'"
            )
