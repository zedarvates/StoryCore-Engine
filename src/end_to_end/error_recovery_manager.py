...
class ErrorRecoveryManager:
    ...
    def classify_error(
        ...
        if any(
            keyword in error_str
            for keyword in ["permission", "file not found", "directory", "disk"]
        ):
        ...
        if any(
            keyword in error_str
            for keyword in ["parse", "invalid format", "json", "yaml"]
        ):
            return ErrorType.PARSING_ERROR, ErrorSeverity.MEDIUM

        # Generation errors
        if any(
            keyword in error_str for keyword in ["generation", "llm", "model", "api"]
        ):
        ...
        # Pipeline errors
        if any(
            keyword in error_str for keyword in ["pipeline", "command", "execution"]
        ):
        ...
        if any(
            keyword in error_str
            for keyword in ["import", "module", "dependency", "not found"]
        ):
            return ErrorType.DEPENDENCY_ERROR, ErrorSeverity.HIGH

        # Resource errors
        if any(
            keyword in error_str for keyword in ["memory", "resource", "space", "quota"]
        ):
    ...
    def handle_error(self, error: Exception, context: ErrorContext) -> RecoveryAction:
        ...
        """
        # Classify error
        error_type, error_severity = self.classify_error(
            error,
            {
        ...
        strategy = self.determine_recovery_strategy(
            error_type, error_severity, context.workflow_step, attempt_count
        )
        ...
        if strategy == RecoveryStrategy.RETRY_ADJUSTED:
            ...
            parameters["adjusted_params"] = self._get_adjusted_parameters(
                error_type, context
            )

        return RecoveryAction(strategy=strategy, parameters=parameters, max_attempts=3)
    ...
    def _get_adjusted_parameters(
        ...
        if error_type == ErrorType.NETWORK_ERROR:
            ...
            adjusted["temperature"] = (
                0.7  # Reduce temperature for more deterministic output
            )
    ...
    def save_checkpoint(
        self, workflow_state: WorkflowState, project_path: Optional[Path] = None
    ) -> bool:
        """
        ...
        try:
            ...
            checkpoint_data = {
                ...
                "completed_steps": [
                    step.value for step in workflow_state.completed_steps
                ],
                "failed_steps": [
    ...
    def load_checkpoint(self, project_path: Path) -> Optional[WorkflowState]:
        """
    ...
    def format_error_notification(
        self, error: Exception, context: ErrorContext, recovery_action: RecoveryAction
    ) -> str:
        """
        ...
        if recovery_action.strategy == RecoveryStrategy.RETRY:
            ...
            message_parts.append(
                f"   → Retrying with adjusted parameters (waiting {backoff}s)..."
            )
            ...
            message_parts.append(
                "   → Checkpoint saved. Please review and resume manually."
            )
