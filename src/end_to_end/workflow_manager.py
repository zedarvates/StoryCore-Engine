...
class WorkflowManager:
    ...
    def _initialize_registry(self):
        ...
        required_workflows = [
            ...
            WorkflowInfo(
                ...
                version="1.1.0",
                file_path=self.workflows_dir / "ltx23_image_to_video.json",
                description="LTX-2.3 v1.1 image-to-video conversion with audio generation and GGUF support",
                required_nodes=[
            ...
            # LTX-2.3 prompt relay workflow
            WorkflowInfo(
                name="ltx23_prompt_relay",
                version="1.1.0",
                file_path=self.workflows_dir / "ltx23_prompt_relay.json",
                description="LTX-2.3 v1.1 with temporal prompt segmentation (Prompt Relay)",
                required_nodes=[
                    "DiffusionModelLoaderKJ",
                    "DualCLIPLoaderGGUF",
                    "VAELoaderKJ",
    ...
    def check_installed_workflows(self) -> List[WorkflowInfo]:
        ...
        for workflow_name, workflow_info in self.workflow_registry.items():
            ...
            comfyui_workflow_path = (
                self.comfyui_workflows_dir / workflow_info.file_path.name
            )
    ...
    def get_workflow_version(self, workflow_name: str) -> Optional[str]:
        ...
        try:
            with open(comfyui_workflow_path, "r", encoding="utf-8") as f:
                ...
                if isinstance(workflow_data, dict):
                    ...
                    version = (
                        ...
                        or workflow_data.get("metadata", {}).get("version")
                        or workflow_data.get("extra", {}).get("version")
                    )
            ...
            logger.error(
                f"Error reading workflow version for {workflow_name}: {str(e)}"
            )
            return None
    ...
    def deploy_workflow(self, workflow_name: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully deployed workflow: {workflow_name} to {dest_path}"
            )
            ...
            logger.error(
                f"Error deploying workflow {workflow_name}: {str(e)}", exc_info=True
            )
            return False
    ...
    def deploy_all_workflows(self) -> Dict[str, bool]:
        ...
        for workflow_name in self.workflow_registry.keys():
            ...
            if not success:
                logger.warning(
                    f"Failed to deploy {workflow_name}, continuing with next workflow"
                )

        # Summary
        successful = sum(1 for success in results.values() if success)
        logger.info(
            f"Deployment complete: {successful}/{len(results)} workflows deployed successfully"
        )

        return results

    def validate_workflow(
        self, workflow_path: Path, installed_nodes: Optional[List[str]] = None
        ...
        result = WorkflowValidationResult(valid=True, workflow_name=workflow_name)
        ...
        try:
            ...
            if installed_nodes is not None:
                missing_nodes = [
                    node for node in required_nodes if node not in installed_nodes
                ]
            ...
            logger.info(
                f"Workflow validation for {workflow_name}: {'PASSED' if result.valid else 'FAILED'}"
            )
            ...
            logger.error(
                f"Error validating workflow {workflow_name}: {str(e)}", exc_info=True
            )
    ...
    def create_z_image_turbo_workflow(
        self, prompt: str, config: ZImageTurboConfig, seed: int = 0
    ) -> Dict[str, Any]:
        """
    ...
    def create_z_image_turbo_coherence_grid_workflow(
        self, prompt: str, config: ZImageTurboConfig, base_seed: int = 1000
    ) -> Dict[str, Any]:
        """
    ...
    def create_ltx2_image_to_video_workflow(
        ...
        """
        # Check if prompt is segmented (Prompt Relay format)
        is_segmented = False
        import re
        if re.search(r"\d+:\d+:", prompt) or (hasattr(config, "prompt_relay") and config.prompt_relay):
            is_segmented = True
            logger.info("Segmented prompt detected, using LTX 2.3 Prompt Relay workflow")
            workflow = self._load_workflow_template("ltx23_prompt_relay.json")
        else:
            # Load standard LTX 2.3 workflow
            workflow = self._load_workflow_template("ltx23_image_to_video.json")

        # Use prompt_relay if provided in config, otherwise use the prompt argument
        final_prompt = prompt
        if is_segmented and hasattr(config, "prompt_relay") and config.prompt_relay:
            final_prompt = config.prompt_relay

        # --- Model Loaders ---
        if "1" in workflow:
            workflow["1"]["inputs"]["ckpt_name"] = config.checkpoint_name
        if "3" in workflow:
            workflow["3"]["inputs"]["clip_name1"] = config.text_encoder_name
            workflow["3"]["inputs"]["clip_name2"] = config.text_projection_name
        if "4" in workflow:
            workflow["4"]["inputs"]["vae_name"] = config.video_vae_name
        if "5" in workflow:
            workflow["5"]["inputs"]["vae_name"] = config.audio_vae_name

        # --- Input and Conditioning ---
        if "7" in workflow:
            workflow["7"]["inputs"]["image"] = input_image_path
        
        if "16" in workflow:
            # Node 16 is CLIPTextEncode in standard, but LTXVPromptRelay in relay workflow
            if is_segmented:
                workflow["16"]["inputs"]["prompt_segments"] = final_prompt
            else:
                workflow["16"]["inputs"]["text"] = final_prompt
        if "17" in workflow:
            workflow["17"]["inputs"]["text"] = config.negative_prompt

        # --- Latent and Noise ---
        if "10" in workflow:
            workflow["10"]["inputs"]["width"] = config.resize_width
            workflow["10"]["inputs"]["height"] = config.resize_height
            workflow["10"]["inputs"]["length"] = config.frame_count
        
        if "22" in workflow:
            workflow["22"]["inputs"]["noise_seed"] = config.noise_seed_stage1

        # --- Sampling Parameters ---
        if "19" in workflow:
            workflow["19"]["inputs"]["steps"] = config.stage1_steps
            workflow["19"]["inputs"]["max_shift"] = config.max_shift
            workflow["19"]["inputs"]["base_shift"] = config.base_shift

        if "20" in workflow:
            workflow["20"]["inputs"]["cfg"] = config.cfg_scale

        if "21" in workflow:
            workflow["21"]["inputs"]["sampler_name"] = config.stage1_sampler

        # --- Memory Optimizations ---
        if "2" in workflow:
            workflow["2"]["inputs"]["chunks"] = config.chunk_size
        
        if "28" in workflow:
            workflow["28"]["inputs"]["tile_size"] = config.tile_size
            workflow["28"]["inputs"]["tile_overlap"] = config.tile_overlap

        # --- Output Parameters ---
        if "11" in workflow:
            workflow["11"]["inputs"]["value"] = config.frame_count
        if "14" in workflow:
            workflow["14"]["inputs"]["value"] = config.frame_rate
        if "15" in workflow:
            # SaveVideo node often takes filename_prefix
            workflow["15"]["inputs"]["filename_prefix"] = f"LTX23_{int(time.time())}"
