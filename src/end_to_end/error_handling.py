...
class ErrorCategorizer:
    ...
    def _is_connection_error(error_str: str, error_type: str) -> bool:
        ...
        return (
            any(keyword in error_str for keyword in connection_keywords)
            or error_type in connection_types
        )
    ...
    def _is_configuration_error(error_str: str, error_type: str) -> bool:
        ...
        return (
            any(keyword in error_str for keyword in config_keywords)
            or error_type in config_types
        )
...
class RetryManager:
    ...
    def increment_retry(self, operation_id: str) -> int:
        ...
        logger.info(f"Retry attempt {new_count}/{self.max_retries} for {operation_id}")
    ...
    def get_retry_delay(self, operation_id: str) -> float:
        ...
        logger.debug(
            f"Retry delay for {operation_id}: {delay:.2f}s (attempt {retry_count + 1})"
        )

        return delay

    def with_retry(self, func: Callable, operation_id: str, *args, **kwargs) -> Any:
        """
        ...
        if last_error:
            ...
            raise RuntimeError(
                f"Operation {operation_id} failed with no error captured"
            )
...
class ErrorLogger:
    ...
    def __init__(
        self, log_dir: Path, max_log_size_mb: int = 10, max_log_files: int = 5
    ):
        """
    ...
    def log_error(
        ...
        try:
            ...
            logger.error(
                f"[{error.category.value}] {error.message} | Details: {error.details}"
            )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if retry_manager:
                    return retry_manager.with_retry(func, operation_id, *args, **kwargs)
                else:
