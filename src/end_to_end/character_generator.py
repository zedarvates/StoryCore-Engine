...
class CharacterGenerator:
    ...
    def generate_characters(
        self, parsed_prompt: ParsedPrompt, world_config: WorldConfig
    ) -> List[Character]:
        """
        ...
        for char_info in parsed_prompt.characters:
            character = self._generate_character(char_info, parsed_prompt, world_config)
    ...
    def _generate_character(
        ...
        personality_traits = self._generate_personality_traits(
            parsed_prompt.genre, char_info.role
        )

        # Generate visual description
        visual_description = self._generate_visual_description(
            char_info, parsed_prompt, world_config
        )
        ...
        onomatopoeia = self.quirk_templates["onomatopoeia"].get(
            genre, self.quirk_templates["onomatopoeia"]["default"]
        )
        ...
        gestures = self.quirk_templates["gestures"].get(
            primary_trait, self.quirk_templates["gestures"]["default"]
        )

        # Deterministic but "random" selection using hash of character ID
        char_hash = hash(character_id)
        diction_quirk = self.quirk_templates["diction"][
            char_hash % len(self.quirk_templates["diction"])
        ]
        voice_inflection = self.quirk_templates["inflections"][
            char_hash % len(self.quirk_templates["inflections"])
        ]
    ...
    def _generate_personality_traits(self, genre: str, role: str) -> List[str]:
        """Generate personality traits based on genre and role"""
        # Get genre-specific traits
        genre_lower = genre.lower()
        base_traits = self.genre_personality_templates.get(
            genre_lower, self.genre_personality_templates["default"]
        )
    ...
    def _get_genre_visual_elements(self, genre: str) -> str:
        ...
        return genre_visuals.get(
            genre.lower(), "distinctive styling and unique features"
        )
    ...
    def _map_relationships(self, characters: List[Character]) -> List[Character]:
        ...
        for i, char in enumerate(characters):
            ...
            for j, other_char in enumerate(characters):
                ...
                relationship = self._determine_relationship(char.role, other_char.role)
