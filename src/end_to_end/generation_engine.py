...
class GenerationEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"GenerationEngine initialized with default workflow: {default_workflow}"
        )
    ...
    def check_backend_availability(self) -> bool:
        ...
        if missing_models:
            logger.warning(
                f"Missing required models: {[m.name for m in missing_models]}"
            )
            return False
        ...
        if missing_workflows:
            logger.warning(
                f"Missing required workflows: {[w.name for w in missing_workflows]}"
            )
            return False
    ...
    def _create_progress(
        ...
        percentage = (
            (current_item / session.total_items * 100) if session.total_items > 0 else 0
        )
    ...
    def cancel_generation(self):
        ...
        if self.current_session:
            logger.info(
                f"Cancellation requested for session {self.current_session.session_id}"
            )
    ...
    async def _cancel_comfyui_generation(self, prompt_id: str):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{backend_url}/interrupt", timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    if response.status == 200:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{backend_url}/queue", timeout=aiohttp.ClientTimeout(total=5)
                ) as response:
                    if response.status == 200:
    ...
    async def generate_master_coherence_sheet(
        ...
        session = self._create_session(
            ...
            metadata={"world_id": world_config.world_id, "grid_size": grid_size},
        )
    ...
    async def _download_video_from_comfyui(
        self, filename: str, subfolder: str = "", output_path: Path = None
    ) -> bytes:
        """
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        if output_path:
                            ...
                            logger.info(
                                f"Downloaded video to {output_path}: {total_size} bytes"
                            )
                            return None
                        else:
    ...
    async def _poll_for_generation_result(
        self, prompt_id: str, poll_interval: float = 1.0, max_wait: int = 600
    ) -> bytes:
        """
        ...
        while time.time() - start_time < max_wait:
            try:
                        ...
                        if response.status == 200:
                            ...
                            if prompt_id in history:
                                ...
                                if status_info.get("completed", False):
                                    logger.info(
                                        f"Generation completed: prompt_id={prompt_id}"
                                    )
                                    ...
                                    for node_id, node_output in outputs.items():
                                        if (
                                            ...
                                            # Download the image
                                            return (
                                                await self._download_image_from_comfyui(
                                                    filename=filename,
                                                    subfolder=subfolder,
                                                )
                                            )
                                ...
                                if "error" in status_info:
                                    ...
                                    raise RuntimeError(
                                        f"ComfyUI generation error: {error_msg}"
                                    )

                                # Log progress if available
                                if "status_str" in status_info:
                                    logger.debug(
                                        f"Generation status: {status_info['status_str']}"
                                    )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"{backend_url}/queue", timeout=aiohttp.ClientTimeout(total=10)
                    ) as response:
                        if response.status == 200:
                            queue_data = await response.json()
                            ...
                            for item in queue_running:
                                if item[1] == prompt_id:
                                    logger.debug(
                                        f"Prompt {prompt_id} is currently running"
                                    )
                                    break
                            ...
                            for idx, item in enumerate(queue_pending):
                                if item[1] == prompt_id:
                                    logger.debug(
                                        f"Prompt {prompt_id} is in queue position {idx + 1}"
                                    )
                                    break
    ...
    async def _submit_workflow_to_comfyui(
        self, workflow: Dict[str, Any], retry_count: int = 0
    ) -> str:
        """
        ...
        try:
            ...
            if retry_count < self.connection_manager.config.max_retries:
                ...
                raise RuntimeError(
                    f"Workflow submission failed after {retry_count + 1} attempts"
                ) from e
    ...
    async def _generate_with_comfyui(
        ...
        try:
            ...
            logger.error(
                f"Connection error during generation for {shot_config.shot_id}: {e}"
            )
    ...
    def _log_detailed_error(self, error: Exception, context: Dict[str, Any]):
        ...
        error_info = {
            ...
            "context": context,
            "backend_status": self.connection_manager.get_status().to_dict()
            if hasattr(self.connection_manager.get_status(), "to_dict")
            else str(self.connection_manager.get_status()),
    ...
    def _create_placeholder_image(
        self, shot_config: ShotConfig, output_path: Path
    ) -> GeneratedImage:
        """
    ...
    def get_session_history(self) -> List[Dict[str, Any]]:
        ...
        for session_id, metrics in self._session_metrics.items():
            history.append({"session_id": session_id, "metrics": metrics.to_dict()})
        ...
        try:
            ...
            if not image.file_path.exists():
                ...
                validation_result["errors"].append(
                    f"Image file not found: {image.file_path}"
                )
            ...
            if extrema[0] == extrema[1]:
                validation_result["valid"] = False
                validation_result["errors"].append(
                    "Image is blank (all pixels same value)"
                )
    ...
    async def generate_video_from_image(
        ...
        try:
            ...
            workflow = self.workflow_manager.create_ltx2_image_to_video_workflow(
                str(input_image_path), prompt, config
            )
            ...
            logger.info(
                "Stage 1: Latent generation submitted, polling for completion..."
            )
                    ...
                    async with http_session.get(
                        ...
                        if response.status == 200:
                            ...
                            if prompt_id in history:
                                ...
                                if status_info.get("completed", False):
                                    logger.info(
                                        f"Video generation completed: prompt_id={prompt_id}"
                                    )
                                    ...
                                    for node_id, node_output in outputs.items():
                                        ...
                                        if (
                                            ...
                                            # Download the video
                                            return (
                                                await self._download_video_from_comfyui(
                                                    filename=filename,
                                                    subfolder=subfolder,
                                                )
                                            )
                                    ...
                                    raise RuntimeError(
                                        f"No video output found for prompt_id={prompt_id}"
                                    )
                                ...
                                if "error" in status_info:
                                    ...
                                    raise RuntimeError(
                                        f"ComfyUI video generation error: {error_msg}"
                                    )
    ...
    async def validate_video_audio(self, video: GeneratedVideo) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not video.path.exists():
                validation_result["valid"] = False
                validation_result["errors"].append(
                    f"Video file not found: {video.path}"
                )
                return validation_result
