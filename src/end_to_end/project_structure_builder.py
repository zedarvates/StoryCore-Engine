...
class ProjectStructureBuilder:
    ...
    def __init__(self, base_path: str):
        ...
        logger.info(
            f"ProjectStructureBuilder initialized with base_path: {self.base_path}"
        )

    def create_project_structure(
        self, project_name: str, components: ProjectComponents
    ) -> ProjectStructure:
        """
        ...
        if components.dialogue_script and (
            not hasattr(components.dialogue_script, "scenes")
            or components.dialogue_script.scenes
        ):
    ...
    def save_all_components(
        ...
        try:
            ...
            world_data = self._save_locations_and_update_world_config(
                project_path, components.world_config.key_locations, world_data
            )
            ...
            characters_data = {
                "characters": [
                    self._serialize_component(char) for char in components.characters
                ]
            }
            ...
            if components.dialogue_script and components.dialogue_script.scenes:
                ...
                self._save_json_file(
                    project_path / "dialogue_script.json", dialogue_data
                )
    ...
    def validate_structure(self, project_path: Path) -> StructureValidation:
        ...
        if not project_path.exists():
            ...
            return StructureValidation(
                valid=False, missing_directories=[], missing_files=[], errors=errors
            )
        ...
        if valid:
            ...
            logger.warning(
                f"Project structure validation failed: {len(missing_directories)} missing dirs, {len(missing_files)} missing files"
            )
    ...
    def _create_project_template(self, components: ProjectComponents) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_dialogue": components.dialogue_script is not None
            and len(components.dialogue_script.scenes) > 0,
            "music_genre": components.music_description.genre,
        }

    def _save_locations_and_update_world_config(
        self, project_path: Path, locations: List[Any], world_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_professional_story_documentation(
        ...
        # 00 Master Outline
        self._write_file(
            story_dir / "00_master_outline.md", self._tpl_master_outline(components)
        )
        ...
        # 02 Lore
        self._write_file(
            story_dir / "02_lore_worldbuilding.md", self._tpl_lore(components)
        )
        ...
        self._write_file(
            ...
            self._tpl_project_readme(project_path.name, components),
        )
