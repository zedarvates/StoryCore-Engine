...
class CORSValidator:
    ...
    async def validate_cors(
        self, origin: str = "http://localhost:3000"
        ...
        """
        logger.info(
            f"Validating CORS configuration for {self.connection_manager.config.url}"
        )
        ...
        try:
            ...
            if (
                not self.connection_manager._session
                or self.connection_manager._session.closed
            ):
            ...
            async with self.connection_manager._session.options(
                ...
                for header_key, header_name in self.REQUIRED_HEADERS.items():
                    if header_key in response_headers:
                        ...
                        logger.debug(
                            f"Found CORS header: {header_name} = {response_headers[header_key]}"
                        )
                    else:
                ...
                if not valid:
                    error_message = (
                        f"Missing required CORS headers: {', '.join(headers_missing)}"
                    )
            ...
            async with self.connection_manager._session.get(
                ...
                if allow_origin:
                    ...
                    if allow_origin == "*" or allow_origin == origin:
                        ...
                        logger.warning(
                            f"Origin {origin} not allowed. Allowed: {allow_origin}"
                        )
                        return False
                else:
