...
class StoryStructureGenerator:
    ...
    def _generate_logline(
        self, parsed_prompt: ParsedPrompt, characters: List[Character]
    ) -> str:
        ...
        if characters:
            protagonist = next(
                (
                    c
                    for c in characters
                    if "protagonist" in c.role.lower() or "hero" in c.role.lower()
                ),
                characters[0],
            )
    ...
    def _generate_acts(
        ...
        structure = self.video_type_structures.get(
            video_type, self.video_type_structures["default"]
        )
        ...
        for i in range(num_acts):
            ...
            # Generate scene IDs
            scene_ids = [f"scene-{i + 1}-{j + 1}" for j in range(scene_counts[i])]
        ...
        if characters:
            ...
            antagonist = next(
                (
                    c
                    for c in characters
                    if "antagonist" in c.role.lower() or "villain" in c.role.lower()
                ),
                None,
            )
            if antagonist:
    ...
    def _generate_themes(self, parsed_prompt: ParsedPrompt) -> List[str]:
        ...
        base_themes = self.genre_themes.get(genre_lower, self.genre_themes["default"])
    ...
    def _generate_emotional_arc(
        ...
        arc_template = self.emotional_arc_templates.get(
            video_type, self.emotional_arc_templates["default"]
        )
