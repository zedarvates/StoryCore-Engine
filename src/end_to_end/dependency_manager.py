...
class DependencyManager:
    ...
    def __init__(
        self, projects_dir: Optional[Path] = None, comfyui_url: Optional[str] = None
    ):
        """
        ...
        """
        self.projects_dir = (
            projects_dir or Path.home() / "Documents" / "StoryCore Projects"
        )
    ...
    def _check_comfyui_backend(self) -> DependencyCheckResult:
        ...
        try:
            ...
            return DependencyCheckResult(
                ...
                name="ComfyUI",
                status=DependencyStatus.UNAVAILABLE,
                message="Cannot check ComfyUI backend (aiohttp not installed)",
            ...
            if available_space >= self.MIN_DISK_SPACE:
                return DependencyCheckResult(
                    ...
                    details={
                        ...
                        "required_bytes": self.MIN_DISK_SPACE,
                    },
                )
            else:
                return DependencyCheckResult(
    ...
    def _can_proceed_with_degraded_mode(
    ...
    def format_verification_report(self, result: DependencyVerificationResult) -> str:
        ...
        if result.all_satisfied:
            ...
            lines.append(
                "⚠ Some dependencies missing, but can proceed in degraded mode"
            )
        else:
        ...
        for type_name, checks in by_type.items():
            ...
            for check in checks:
                status_symbol = (
                    "✓" if check.status == DependencyStatus.AVAILABLE else "✗"
                )
