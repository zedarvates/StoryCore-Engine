...
class ComfyUIIntegration:
    ...
    async def check_availability(self) -> ComfyUIStatus:
        ...
        try:
            ...
            return ComfyUIStatus(
                available=False, url=self.backend_url, error_message=str(e)
            )
    ...
    def _create_workflow_config(self, shot_config: ShotConfig) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "size": {"width": shot_config.width, "height": shot_config.height},
            "sampling": {
    ...
    async def _poll_for_result(
        self, prompt_id: str, poll_interval: float = 1.0, max_wait: int = 300
    ) -> Optional[bytes]:
        """
    ...
    async def generate_master_coherence_sheet(
        ...
        for i in range(total_panels):
            ...
            if status.available:
                ...
                # Use fallback
                logger.warning(
                    f"ComfyUI unavailable, using fallback mode: {self.fallback_mode}"
                )
    ...
    def _create_coherence_prompt(
        self, world_config: WorldConfig, style_config: StyleConfig, panel_index: int
    ) -> str:
        """
    ...
    async def generate_shot(
        ...
        if status.available:
            ...
            logger.warning(
                f"ComfyUI unavailable for shot {shot_config.shot_id}, using fallback"
            )
            if self.fallback_mode == FallbackMode.PLACEHOLDER:
    ...
    async def _generate_with_retry(
            ...
            try:
                ...
                if image_data:
                    ...
                    return GeneratedImage(
                        ...
                        height=img.height,
                        generation_time=generation_time,
                        metadata={"attempt": attempt + 1, "seed": shot_config.seed},
                    )
                else:
        ...
        # All retries failed
        logger.error(
            f"Failed to generate shot {shot_config.shot_id} after {self.max_retries} attempts"
        )
        ...
        for sequence in sequence_plan.sequences:
            for shot in sequence.shots:
                ...
                if progress_callback:
                    progress_callback(
                        current_shot, total_shots, f"Generating shot {shot.shot_id}"
                    )
                ...
                try:
                    image = await self.generate_shot(
    ...
    def validate_image_quality(
        self, image: GeneratedImage, min_quality_score: float = 0.5
    ) -> bool:
        """
        ...
        try:
            ...
            if img.width < 512 or img.height < 512:
                logger.warning(
                    f"Image {image.image_id} too small: {img.width}x{img.height}"
                )
                return False
