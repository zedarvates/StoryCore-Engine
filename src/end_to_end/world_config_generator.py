...
from src.end_to_end.data_models import ParsedPrompt, WorldConfig, ColorPalette, Location
...
class WorldConfigGenerator:
    ...
    def generate(self, parsed_prompt: ParsedPrompt) -> WorldConfig:
        ...
        visual_style = self._merge_visual_styles(
            parsed_prompt.visual_style, genre_template.get("visual_style_additions", [])
        )

        # Select color palette
        color_palette = self._select_color_palette(
            parsed_prompt.mood, parsed_prompt.genre
        )

        # Get lighting style
        lighting_style = self._get_lighting_style(
            parsed_prompt.genre, parsed_prompt.mood, genre_template
        )

        # Get atmosphere
        atmosphere = self._get_atmosphere(parsed_prompt.mood, genre_template)

        # Generate locations
        key_locations = self._generate_locations(
            parsed_prompt.setting, parsed_prompt.genre, genre_template
        )
    ...
    def _merge_visual_styles(
        self, prompt_styles: List[str], template_styles: List[str]
    ) -> List[str]:
    ...
    def _select_color_palette(self, moods: List[str], genre: str) -> ColorPalette:
        """
    ...
    def _get_lighting_style(
        self, genre: str, moods: List[str], genre_template: Dict
    ) -> str:
    ...
    def _get_atmosphere(self, moods: List[str], genre_template: Dict) -> str:
    ...
    def _generate_locations(
        self, setting: str, genre: str, genre_template: Dict
    ) -> List[Location]:
        ...
        setting_locs = self.SETTING_LOCATIONS.get(
            setting, self.SETTING_LOCATIONS["city"]
        )
        ...
        for i, loc_data in enumerate(setting_locs):
            location = Location(
                ...
                visual_description=self._generate_location_visual_description(
                    loc_data["name"], loc_data["type"], genre
                ),
            )
    ...
    def _generate_location_visual_description(
        self, name: str, location_type: str, genre: str
    ) -> str:
