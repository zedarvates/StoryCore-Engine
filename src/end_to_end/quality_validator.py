...
from src.end_to_end.data_models import QualityReport, Issue, ProjectComponents
...
class QualityValidator:
    ...
    async def validate_final_video(
        self, video_path: Path, project_data: ProjectComponents
    ) -> QualityReport:
        """
        ...
        if not video_path.exists():
            ...
            return self._create_failed_report("Video file not found", video_path)
        ...
        overall_score = (
            visual_score.score * 0.4 + audio_score.score * 0.3 + sync_score.score * 0.3
        )
        ...
        passed = (
            ...
            and visual_score.score >= self.min_visual_coherence
            and audio_score.score >= self.min_audio_quality
            and sync_score.score >= self.min_sync_score
        )
        ...
        self.logger.info(
            f"Quality validation complete: overall={overall_score:.2f}, passed={passed}"
        )
    ...
    def check_synchronization(self, video_path: Path) -> SyncScore:
        ...
        try:
            ...
            is_synchronized = (
                abs(score_data["offset_ms"]) <= self.max_sync_offset
                and abs(score_data["drift_ms"]) <= self.max_sync_drift
            )
    ...
    def _analyze_video_coherence(self, video_path: Path) -> Dict[str, Any]:
        ...
        sample_indices = np.linspace(
            0, frame_count - 1, min(10, frame_count), dtype=int
        )
        ...
        for i in range(len(color_histograms) - 1):
            similarity = cv2.compareHist(
                color_histograms[i], color_histograms[i + 1], cv2.HISTCMP_CORREL
            )
        ...
        if len(color_histograms) >= 2:
            style_drift = cv2.compareHist(
                color_histograms[0], color_histograms[-1], cv2.HISTCMP_CORREL
            )
        else:
            style_drift = 1.0
    ...
    def _generate_recommendations(
        ...
        if not recommendations:
            recommendations.append("Video quality is good! No major issues detected.")

        return recommendations

    def _create_failed_report(self, reason: str, video_path: Path) -> QualityReport:
