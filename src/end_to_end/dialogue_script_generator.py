...
class DialogueScriptGenerator:
    ...
    def generate_dialogue_script(
        ...
        if not self._needs_dialogue(parsed_prompt):
            ...
            return DialogueScript(
                script_id=script_id, scenes=[], total_lines=0, estimated_duration=0
            )

        # Generate dialogue scenes from story acts
        scenes = self._generate_scenes(story_structure, characters, parsed_prompt)
    ...
    def _needs_dialogue(self, parsed_prompt: ParsedPrompt) -> bool:
        ...
        dialogue_types = ["short_film", "trailer", "teaser", "drama", "comedy"]
        ...
        return (
            any(dt in video_type for dt in dialogue_types)
            or any(dt in genre for dt in dialogue_types)
            or len(parsed_prompt.characters) > 0
        )
        ...
        for act in story_structure.acts:
            for scene_id in act.scenes:
                scene = self._generate_scene(
                    scene_id, act, characters, parsed_prompt, story_structure
                )
                scenes.append(scene)
    ...
    def _select_scene_characters(
        self, characters: List[Character], act_number: int, total_acts: int
    ) -> List[Character]:
    ...
    def _generate_dialogue_lines(
        ...
        for i in range(num_lines):
            ...
            # Generate line
            line = self._generate_line(char, scene_context, i, parsed_prompt)
            lines.append(line)
    ...
    def _get_num_lines(self, scene_context: str) -> int:
        ...
        line_counts = {"setup": 2, "conflict": 3, "climax": 4, "resolution": 2}
    ...
    def _generate_line(
        ...
        deliveries = self.delivery_templates.get(
            role_key, self.delivery_templates["default"]
        )
        ...
        text = self._generate_line_text(
            character, scene_context, line_index, parsed_prompt
        )
    ...
    def _generate_line_text(
        ...
        if (
            line_index % 3 == 0
            and hasattr(character, "onomatopoeia")
            and character.onomatopoeia
        ):
