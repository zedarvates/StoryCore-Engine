...
class ComponentGenerator:
    ...
    async def generate_all_components(
        ...
        if not validation_result["is_coherent"]:
            components = self._auto_correct_components(
                components, validation_result["issues"]
            )

        return components

    def validate_coherence(self, components: ProjectComponents) -> Dict[str, any]:
        """
        ...
        world_character_issues = self._check_world_character_coherence(
            components.world_config, components.characters
        )
        ...
        story_character_issues = self._check_story_character_coherence(
            components.story_structure, components.characters
        )
        ...
        dialogue_story_issues = self._check_dialogue_story_coherence(
            components.dialogue_script, components.story_structure
        )
    ...
    def _check_world_character_coherence(
        self, world_config: WorldConfig, characters: List[Character]
    ) -> List[Dict[str, str]]:
    ...
    def _check_story_character_coherence(
        self, story_structure: StoryStructure, characters: List[Character]
    ) -> List[Dict[str, str]]:
        ...
        if len(characters) > 0:
            protagonist = next(
                (c for c in characters if "protagonist" in c.role.lower()), None
            )
    ...
    def _check_dialogue_story_coherence(
        self, dialogue_script: DialogueScript, story_structure: StoryStructure
    ) -> List[Dict[str, str]]:
    ...
    def _check_sequence_story_coherence(
        self, sequence_plan: SequencePlan, story_structure: StoryStructure
    ) -> List[Dict[str, str]]:
    ...
    def _check_music_story_coherence(
        self, music_description: MusicDescription, story_structure: StoryStructure
    ) -> List[Dict[str, str]]:
    ...
    def _auto_correct_components(
        self, components: ProjectComponents, issues: List[Dict[str, str]]
    ) -> ProjectComponents:
        """
