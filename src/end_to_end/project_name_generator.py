...
class ProjectNameGenerator:
    ...
    def _generate_base_name(self, parsed_prompt: ParsedPrompt) -> str:
        ...
        if (
            parsed_prompt.genre
            and parsed_prompt.genre.lower() not in parsed_prompt.project_title.lower()
        ):
            parts.append(parsed_prompt.genre)

        # Add video type if it's not generic
        if parsed_prompt.video_type and parsed_prompt.video_type.lower() not in [
            "video",
            "film",
        ]:
    ...
    def validate_name(self, name: str) -> tuple:
        ...
        if len(name) > self.MAX_NAME_LENGTH:
            return (
                False,
                f"Project name too long (max {self.MAX_NAME_LENGTH} characters)",
            )
