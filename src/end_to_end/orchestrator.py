...
from src.end_to_end.error_recovery_manager import ErrorRecoveryManager, ErrorContext
...
class EndToEndOrchestrator:
    ...
    async def create_project_from_prompt(
        ...
        self.progress_monitor.start_workflow(total_steps=len(self.WORKFLOW_STEPS))
    ...
    async def _execute_parsing(self, prompt: str) -> Optional[ParsedPrompt]:
        ...
        self.progress_monitor.update_step(step.value, 0.0, "Parsing user prompt...")
        ...
        try:
            ...
            is_valid, validation_errors = self.prompt_parser.validate_parsed_data(
                parsed
            )
            ...
            self.progress_monitor.complete_step(
                step.value, {"parsed_fields": list(parsed.__dict__.keys())}
            )
            ...
            if self.config.checkpoint_enabled:
                self.error_recovery.save_checkpoint(
                    self._current_state, self._current_project_path
                )
    ...
    async def _execute_name_generation(self, parsed_prompt: ParsedPrompt) -> str:
        """Execute project name generation step."""
        step = WorkflowStep.NAME_GENERATION

        self.progress_monitor.update_step(step.value, 0.0, "Generating project name...")
        ...
        try:
            ...
            if not is_valid:
                logger.warning(
                    f"Generated name '{project_name}' validation issues: {error}"
                )

            # Update progress
            self.progress_monitor.complete_step(
                step.value, {"project_name": project_name}
            )
    ...
    async def _execute_image_generation(
        self, project_path: Path, components: ProjectComponents, warnings: List[str]
    ) -> bool:
        """Execute image generation step with fallback."""
        if getattr(self.config, "no_comfyui", False):
            logger.info(
                "Skipping image generation as per configuration (no_comfyui=True)"
            )
            return False
        ...
        self.progress_monitor.update_step(
            step.value, 0.0, "Checking ComfyUI availability..."
        )
        ...
        try:
            ...
            if comfy_available:
                self.progress_monitor.update_step(
                    step.value, 10.0, "Generating master coherence sheet..."
                )
                ...
                coherence_sheet = await self.comfyui.generate_master_coherence_sheet(
                    components.world_config, style_config
                )

                self.progress_monitor.update_step(
                    step.value, 50.0, "Generating sequence shots..."
                )
                ...
                await self.comfyui.save_generated_images(
                    project_path / "assets" / "images", shots_result
                )
            ...
            # Update progress
            self.progress_monitor.complete_step(
                step.value, {"images_generated": images_generated}
            )
    ...
    async def _execute_pipeline(self, project_path: Path, warnings: List[str]) -> Any:
        """Execute the StoryCore pipeline."""
        step = WorkflowStep.PIPELINE_EXECUTION

        self.progress_monitor.update_step(
            step.value, 0.0, "Starting StoryCore pipeline..."
        )
        ...
        try:
            ...
            if not pipeline_result.success:
                ...
                logger.warning(
                    f"Pipeline completed with errors: {pipeline_result.errors}"
                )
    ...
    async def _execute_quality_validation(
        self, project_path: Path, pipeline_result: Any
    ) -> Optional[QualityReport]:
        """Execute final quality validation."""
        step = WorkflowStep.QUALITY_VALIDATION

        self.progress_monitor.update_step(
            step.value, 0.0, "Validating final quality..."
        )
        ...
        try:
            ...
            qa_report = await self.quality_validator.validate_final_video(
                pipeline_result.video_path, self._current_components
            )

            # Update progress
            self.progress_monitor.complete_step(
                step.value,
                {"overall_score": qa_report.overall_score, "passed": qa_report.passed},
            )
    ...
    def resume(self, project_path: str) -> bool:
        ...
        logger.info(
            f"Resuming workflow from checkpoint at step: {checkpoint.current_step}"
        )
    ...
    def add_step_complete_hook(
        self, hook: Callable[[WorkflowStep, Dict], None]
    ) -> None:
        """
    ...
    def add_step_fail_hook(self, hook: Callable[[WorkflowStep, str], None]) -> None:
        """
    ...
    def add_workflow_complete_hook(
        self, hook: Callable[[ProjectCreationResult], None]
    ) -> None:
        """
    ...
    def add_workflow_fail_hook(self, hook: Callable[[List[str]], None]) -> None:
        """
...
async def create_project(
    prompt: str, projects_dir: str = "projects", **kwargs
) -> ProjectCreationResult:
    """
    ...
    Returns:
        ProjectCreationResult
    """
    config = OrchestratorConfig(projects_directory=projects_dir, **kwargs)
