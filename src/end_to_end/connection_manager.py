...
class ComfyUIConfig:
    ...
    def from_env(cls) -> "ComfyUIConfig":
        ...
        if "COMFYUI_TIMEOUT" in os.environ:
            try:
                ...
                logger.warning(
                    f"Invalid COMFYUI_TIMEOUT: {os.environ['COMFYUI_TIMEOUT']}"
                )
    ...
    def validate(self) -> List[str]:
        ...
        if not isinstance(self.max_retries, int) or self.max_retries < 0:
            errors.append(
                f"Invalid max_retries: {self.max_retries}. Must be non-negative integer"
            )

        # Validate retry_backoff
        if not isinstance(self.retry_backoff, (int, float)) or self.retry_backoff < 0:
            errors.append(
                f"Invalid retry_backoff: {self.retry_backoff}. Must be non-negative number"
            )
        ...
        if self.fallback_mode not in valid_modes:
            errors.append(
                f"Invalid fallback_mode: {self.fallback_mode}. Must be one of {valid_modes}"
            )
...
class ConnectionManager:
    ...
    def __init__(self, config: ComfyUIConfig):
        ...
        """
        self.config = config
        self.status: ComfyUIStatus = ComfyUIStatus(available=False, url=config.url)
        ...
        try:
            ...
            async with self._session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(
                        f"Successfully connected to ComfyUI {self.status.version}"
                    )
                    ...
                    self.status = ComfyUIStatus(
                        available=False, url=self.config.url, error_message=error_msg
                    )
    ...
    async def disconnect(self):
        ...
        # Update status
        self.status = ComfyUIStatus(
            available=False, url=self.config.url, error_message="Disconnected"
        )
    ...
    async def check_health(self) -> ComfyUIStatus:
        ...
        try:
            ...
            self.status = ComfyUIStatus(
                available=False, url=self.config.url, error_message=str(e)
            )
    ...
    async def start_background_reconnection(self, interval: int = 10):
        ...
        self._reconnection_task = asyncio.create_task(self._reconnection_loop(interval))
