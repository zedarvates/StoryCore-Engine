...
class StoryStructureAnalyzer:
    ...
    def analyze_text(
        self, text: str, title: str = "Untitled Story"
    ) -> StoryStructureAnalysis:
    ...
    def _detect_chapter_heading(self, line: str) -> Optional[Tuple[int, str]]:
                ...
                if len(groups) >= 2:
                    try:
                        num = (
                            int(groups[1])
                            if groups[1].isdigit()
                            else self._roman_to_int(groups[1])
                        )
                        return (num, groups[0])
                    except Exception:
                        pass
        ...
        if re.match(
            r"^(chapter|part|act)?\s*\d+\s*[:\.]?\s*$", line_stripped, re.IGNORECASE
        ):
            match = re.match(
                r"^(?:chapter|part|act)?\s*(\d+)", line_stripped, re.IGNORECASE
            )
            if match:
    ...
    def _create_acts(
        self, chapters: List[Dict[str, Any]], structure_type: StructureType
    ) -> List[Act]:
    ...
    def _find_plot_points(self, chapters: List[Dict[str, Any]]) -> List[PlotPoint]:
            ...
            for point_type, indicators in self.PLOT_INDICATORS.items():
                for indicator in indicators:
                    if indicator in content:
                        existing = [
                            p for p in plot_points if p.point_type == point_type
                        ]
                        if not existing:
        ...
        if not plot_points:
            ...
            plot_points = [
                ...
                PlotPoint(
                    "Climax", PlotPointType.CLIMAX, total, "Final confrontation", 0.9
                ),
            ]
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 20 and len(sentence) < 200:
                if any(
                    ...
                    for kw in ["decides", "discovers", "reveals", "confronts"]
                ):
                    return sentence
    ...
    def _extract_character_arcs(
        self, text: str, chapters: List[Dict[str, Any]]
    ) -> List[CharacterArc]:
        ...
        antagonist_match = re.search(
            r"antagonist[:\s]+([A-Z][a-z]+)", text, re.IGNORECASE
        )
        if antagonist_match:
            arcs.append(
                CharacterArc(
    ...
    def _calculate_complexity(
        self, chapters: List[Dict[str, Any]], plot_points: List[PlotPoint]
    ) -> float:
        ...
        complexity = min(
            1.0, (chapter_count / 20) * 0.5 + (plot_point_count / 10) * 0.5
        )
        return complexity
    ...
    def _assess_pacing(self, curve: List[TensionPoint]) -> str:
        ...
        rising = sum(
            1 for i in range(1, len(tensions)) if tensions[i] > tensions[i - 1]
        )
        falling = sum(
            1 for i in range(1, len(tensions)) if tensions[i] < tensions[i - 1]
        )
    ...
    def generate_visualization_data(
        self, analysis: StoryStructureAnalysis
    ) -> VisualizationData:
                    ...
                    "datasets": [
                        {
                            "label": "Chapters",
                            "data": [
                                a.end_chapter - a.start_chapter + 1
                                for a in analysis.acts
                            ],
                        }
                    ],
                },
