...
class ScriptQualityAnalyzer:
    ...
    def analyze(
        self, script_text: str, title: str = "Untitled Script"
    ) -> ScriptQualityReport:
    ...
    def _parse_scenes(self, script_text: str) -> List[Dict[str, Any]]:
        ...
        scene_pattern = re.compile(
            r"^(?:INT\.|EXT\.|I\/E\.)\s*(.+?)(?:\s+-\s+(.+))?$", re.IGNORECASE
        )
        ...
        for line in lines:
            ...
            if scene_match:
                if current_scene["content"]:
                    current_scene["word_count"] = len(
                        " ".join(current_scene["content"]).split()
                    )
    ...
    def _analyze_dialogue(self, lines: List[str]) -> DialogueAnalysis:
        ...
        if analysis.total_lines > 0:
            ...
            analysis.action_percentage = (
                action_count + description_count
        ...
        if 35 <= dialogue_pct <= 55:
            ...
            analysis.assessment = (
                "Consider adding more dialogue for character development"
            )
            analysis.actual_ratio = f"{dialogue_pct:.0f}% dialogue - Heavy on action"
        else:
            analysis.assessment = (
                "Consider adding more action to maintain visual interest"
            )
    ...
    def _check_character_arcs(
        self, script_text: str, scenes: List[Dict[str, Any]]
    ) -> CharacterArcCheck:
        ...
        for name in check.characters[:10]:
            has_intro = any(
                kw in text_lower
                for kw in ["introduces", "meets", "discovers", "begins"]
            )
            has_resolution = any(
                kw in text_lower
                for kw in ["realizes", "finally", "concludes", "achieves"]
            )
        ...
        if check.characters:
            check.arc_completeness_score = (
                check.complete_arcs / len(check.characters)
            ) * 0.7 + (check.characters_with_arcs / len(check.characters)) * 0.3
        else:
            check.arc_completeness_score = 0.5

        if check.incomplete_arcs > 0:
            check.recommendations.append(
                f"{check.incomplete_arcs} character(s) have beginnings but incomplete arcs."
            )

        if check.missing_arcs:
            check.recommendations.append(
                f"Consider developing arcs for: {', '.join(check.missing_arcs[:3])}"
            )
    ...
    def _analyze_conflict(self, scenes: List[Dict[str, Any]]) -> ConflictAnalysis:
        ...
        if coverage < 0.5:
            analysis.recommendations.append(
                "Consider adding more conflict to engage the audience."
            )
        elif coverage > 0.9:
            analysis.recommendations.append(
                "High conflict coverage - ensure variety in conflict types."
            )
        else:
    ...
    def _calculate_metric_scores(
        self, report: ScriptQualityReport
    ) -> List[MetricScore]:
        """Calculate individual metric scores."""
        scores = []

        dialogue_pct = (
            report.dialogue_analysis.dialogue_percentage
        ...
        pacing_score = (
            report.pacing_analysis.pacing_score if report.pacing_analysis else 0.5
        )
        scores.append(
            MetricScore(
        ...
        arc_score = (
            report.character_arc_check.arc_completeness_score
            if report.character_arc_check
            else 0.5
        )
        scores.append(
            MetricScore(
        ...
        conflict_score = (
            report.conflict_analysis.conflict_coverage
...
def analyze_script_quality(
    script_text: str, title: str = "Untitled Script"
) -> ScriptQualityReport:
    """Analyze script quality."""
