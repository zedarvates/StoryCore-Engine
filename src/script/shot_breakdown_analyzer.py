...
class ShotBreakdownAnalyzer:
    ...
    def analyze_script(
        ...
        analysis.avg_shots_per_scene = analysis.total_shots / max(
            1, analysis.total_scenes
        )
        analysis.avg_shot_duration = analysis.total_duration / max(
            1, analysis.total_shots
        )

        for sb in analysis.scene_breakdowns:
            for shot in sb.shots:
                shot_type = shot.shot_type.value
                analysis.shot_type_counts[shot_type] = (
                    analysis.shot_type_counts.get(shot_type, 0) + 1
                )
    ...
    def _breakdown_scene(
        self, scene: Dict[str, Any], start_shot_number: int
    ) -> SceneShotBreakdown:
        ...
        main_shots = self._generate_main_shots(
            scene, start_shot_number + len(shots), is_dialogue_heavy, is_action_heavy
        )
    ...
    def _suggest_opening_shot(
        self, scene: Dict[str, Any], shot_number: int
    ) -> Optional[Shot]:
    ...
    def _generate_main_shots(
        ...
        for paragraph in paragraphs:
            ...
            shot_type = self._determine_shot_type(
                paragraph, is_dialogue_heavy, is_action_heavy
            )
    ...
    def _determine_shot_type(
        self, content: str, is_dialogue_heavy: bool, is_action_heavy: bool
    ) -> ShotType:
    ...
    def _estimate_duration(
        self, shot_type: ShotType, content: str, is_action_heavy: bool
    ) -> float:
    ...
    def _generate_shot_description(
        self, shot_type: ShotType, content: ShotContent
    ) -> str:
    ...
    def _generate_reasoning(
        self, shot_type: ShotType, movement: CameraMovement, content: ShotContent
    ) -> List[str]:
    ...
    def _suggest_closing_shot(
        self, scene: Dict[str, Any], prev_shot_number: int
    ) -> Optional[Shot]:
        """Suggest closing shot for a scene."""
        return None

    def export_shot_list(
        self, analysis: ShotBreakdownAnalysis, fmt: str = "csv"
    ) -> ShotListExport:
...
def export_shot_list(analysis: ShotBreakdownAnalysis, fmt: str = "csv") -> str:
