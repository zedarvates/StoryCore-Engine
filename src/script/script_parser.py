...
class ScriptParser:
    ...
    EXTENDED_SCENE_PATTERN = re.compile(
        r"^(INT\.?|EXT\.?|I\/E\.?|INT\/EXT\.?)\s+(.+?)(?:\s+-\s+(.+))?$", re.IGNORECASE
    )
    ...
    SHOT_PATTERN = re.compile(
        r"^(?:ANGLE|CU|MS|WS|LS|OTS|POV|OVER|UNDER|SHOW)\s*", re.IGNORECASE
    )
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if self._is_character_line(stripped):
                if current_character and dialogue_buffer:
                    self._add_dialogue(
                        current_scene, current_character, dialogue_buffer, line_num
                    )
                    dialogue_buffer = []
    ...
    def _add_dialogue(
        self, scene: ParsedScene, character: str, lines: List[str], line_num: int
    ):
    ...
    def _calculate_complexity(self, scene: ParsedScene) -> float:
        ...
        action_density = len([a for a in scene.action_blocks if a.is_movement]) / max(
            1, len(scene.action_blocks)
        )

        score = (
            (dialogue_ratio * 0.3) + (character_count * 0.1) + (action_density * 0.2)
        )
    ...
    def _calculate_statistics(self, script: ParsedScript):
        ...
        if stats.total_dialogue_lines > 0:
            total_words = sum(
                len(d.text.split()) for s in script.scenes for d in s.dialogues
            )
