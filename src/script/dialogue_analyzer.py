...
from .script_types import ParsedScript, DialogueLine
...
class DialogueAnalyzer:
    ...
    def analyze_script(self, script: ParsedScript) -> DialogueAnalysisResult:
        ...
        for character, dialogues in character_dialogue.items():
            voice = self._analyze_character(
                character, dialogues, list(character_scenes[character])
            )
        ...
        if character_dialogue:
            sorted_chars = sorted(
                character_dialogue.items(), key=lambda x: len(x[1]), reverse=True
            )
    ...
    def _analyze_character(
        self, character: str, dialogues: List[DialogueLine], scene_numbers: List[int]
    ) -> VoiceSignature:
        voice = VoiceSignature(character_name=character)
        voice.dialogue_stats = self._calc_dialogue_stats(
            character, dialogues, scene_numbers
        )
    ...
    def _analyze_vocabulary(
        self, character: str, dialogues: List[DialogueLine]
    ) -> VocabularyProfile:
    ...
    def _detect_catchphrases(
        self, dialogues: List[DialogueLine], scene_numbers: List[int]
    ) -> List[Catchphrase]:
        catchphrases = []
        ...
        for phrase, count in phrase_count.items():
            if count >= 2:
                catchphrases.append(
                    Catchphrase(phrase=phrase, count=count, scene_first=1, scene_last=1)
                )
