...
class SceneTimingAnalyzer:
    ...
    def analyze(
        ...
        if scene_timings:
            ...
            report.longest_scene = max(
                scene_timings, key=lambda s: s.estimated_duration_seconds
            )
            report.shortest_scene = min(
                scene_timings, key=lambda s: s.estimated_duration_seconds
            )
        ...
        if report.total_duration_seconds > 0:
            report.dialogue_percentage = (
                total_dialogue / report.total_duration_seconds
            ) * 100
            report.action_percentage = (
                total_action / report.total_duration_seconds
            ) * 100
    ...
    def _analyze_scene_timing(
        self, scene: Dict[str, Any], dialogue_speed: DialogueSpeed
    ) -> SceneTiming:
        ...
        lines = [line for line in scene["content"] if line.strip()]
    ...
    def _analyze_dialogue(
        self, lines: List[str], speed: DialogueSpeed
    ) -> DialogueTiming:
    ...
    def _analyze_action(self, lines: List[str], content: str) -> ActionTiming:
        ...
        for line in lines:
            ...
            has_action_kw = any(
                kw in line.lower() for kw in self.ACTION_KEYWORDS.keys()
            )
        ...
        timing.estimated_seconds = (
            action_lines * self.BASE_ACTION_SECONDS * avg_intensity
        )

        return timing

    def _analyze_complexity(self, content: str, lines: List[str]) -> SceneComplexity:
        ...
        viz.timeline_data = {
            ...
            "data": {
                ...
                "datasets": [
                    {
                        "label": "Dialogue",
                        "data": [s.dialogue_seconds for s in report.scene_timings],
                    },
                    {
                        "label": "Action",
                        "data": [s.action_seconds for s in report.scene_timings],
                    },
                    {
                        "label": "Transition",
                        "data": [s.transition_seconds for s in report.scene_timings],
                    },
                ],
            },
        }
