    ...
    while True:
        try:
            ...
            if 0 <= index < len(backups):
                ...
                print(
                    f"Invalid selection. Please enter a number between 1 and {len(backups)}."
                )
        except ValueError:
...
def restore_specific_backup(
    manager: BackupManager, backup_path: Path, target: Path = None
):
...
Examples:
  ...
  python rollback_migration.py --backup backup.tar.gz --target /path/to/restore
    ...
    parser.add_argument("--backup", type=Path, help="Path to backup archive to restore")
