...
class NSMEngine:
    ...
    async def process(
        self, prompt: str, massive_context: str, project_root: Optional[str] = None
    ) -> str:
        ...
        for i in range(self.max_iterations):
            ...
            response = await self.agent_core.call_llm(
            ...
            if plan_match:
                ...
                new_steps = [
                    NSMStep(id=str(uuid.uuid4())[:4], description=s) for s in step_lines
                ]
                ...
                self.state.active_plan = NSMPlan(
                    title="Current Plan", steps=new_steps, rationale="Ongoing strategy"
                )

            # Detect Memory Updates via tags
            mem_updates = re.finditer(
                r'<memory_update key="(.*?)" value="(.*?)" />', response
            )
...
"""
    ...
    async def _execute_tools(self, response: str) -> str:
        ...
        for match in code_matches:
            ...
            def safe_path(path):
                if not self.state.project_root:
                    raise PermissionError(
                        "Project root not set. File operations disabled."
                    )
            ...
            repl.set_variable(
                "query_database", lambda q: run_sync(self.agent_core.query_database(q))
            )
            repl.set_variable(
                "query_graph",
                lambda e, d=1: run_sync(self.agent_core.query_graph(e, d)),
            )

            if hasattr(self.agent_core, "trigger_n8n"):
                repl.set_variable(
                    "n8n_trigger",
                    lambda i, p: run_sync(self.agent_core.trigger_n8n(i, p)),
                )
            if hasattr(self.agent_core, "list_n8n"):
                repl.set_variable(
                    "n8n_list_workflows", lambda: run_sync(self.agent_core.list_n8n())
                )
            if hasattr(self.agent_core, "send_message"):
                repl.set_variable(
                    "send_message",
                    lambda p, t, tid=None: run_sync(
                        self.agent_core.send_message(p, t, tid)
                    ),
                )
    ...
    async def _finalize(self, result: str, original_prompt: str) -> str:
        ...
        if potential_entities:
            self._notify(
                f"Checking lore consistency for: {', '.join(potential_entities[:5])}",
                "critique",
            )
            lore_context = await self.agent_core.query_graph(
                potential_entities[:8], max_depth=1
            )
