...
class DataContractValidator:
    ...
    def validate_project_data(self, project_data: dict) -> ValidationResult:
        ...
        if schema_version != "1.0":
            errors.append(f"Invalid schema version: {schema_version}, expected '1.0'")
        ...
        if result.valid:
            logger.info(
                f"Project {project_name} validation passed with {len(warnings)} warnings"
            )
        else:
            logger.error(
                f"Project {project_name} validation failed with {len(errors)} errors"
            )
    ...
    def validate_project(self, project: Project) -> ValidationResult:
        ...
        if result.valid:
            logger.info(
                f"Project {project.name} validation passed with {len(warnings)} warnings"
            )
        else:
            logger.error(
                f"Project {project.name} validation failed with {len(errors)} errors"
            )
    ...
    def _validate_sequence_data(self, sequence: dict) -> List[str]:
        ...
        if total_duration <= 0:
            errors.append(
                f"Sequence {seq_id} has invalid total_duration: {total_duration}"
            )
    ...
    def _validate_sequence(self, sequence: Sequence) -> List[str]:
        ...
        if sequence.total_duration <= 0:
            errors.append(
                f"Sequence {sequence.id} has invalid total_duration: {sequence.total_duration}"
            )
