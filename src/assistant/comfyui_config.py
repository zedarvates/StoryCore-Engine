...
from .models import Project, Scene, Character, Sequence
...
class ComfyUIConfigGenerator:
    ...
    def generate_config(self, project: Project) -> ComfyUIConfig:
        ...
        shot_prompts = self._generate_shot_prompts(
            project.scenes, project.sequences, project.characters, master_style
        )

        # Generate layer configurations
        layer_configs = self._generate_layer_configs(
            project.sequences, project.characters
        )
        ...
        validation_errors = self._validate_config(
            project, character_prompts, shot_prompts
        )
        ...
        for sequence in sequences:
            ...
            for shot in sequence.shots:
                ...
                positive_parts.append(
                    f"{shot.type} shot, {shot.camera_movement} camera"
                )
                ...
                visual_prompt = VisualPrompt(
                    ...
                    style_tags=self._extract_style_tags(
                        shot.visual_style, master_style
                    ),
                    parameters=GenerationParameters(),
                )
    ...
    def _generate_layer_configs(
        self, sequences: List[Sequence], characters: List[Character]
        ...
        for sequence in sequences:
            for shot in sequence.shots:
                ...
                if (
                    "effects" in shot.description.lower()
                    or "magic" in shot.description.lower()
                ):
    ...
    def _validate_config(
        ...
        for character in project.characters:
            if character.id not in character_prompts:
                ...
                not character_prompts[character.id]
                or not character_prompts[character.id].strip()
            ):
    ...
    def update_generation_parameters(
        ...
        if quality is not None:
            ...
            quality_steps = {"low": 20, "medium": 30, "high": 40, "ultra": 50}
            config.generation_parameters.steps = quality_steps.get(quality, 30)

        if style_strength is not None:
            config.generation_parameters.style_strength = max(
                0.0, min(1.0, style_strength)
            )
