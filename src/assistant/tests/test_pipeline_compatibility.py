...
class TestPipelineCompatibility:
    ...
    def test_generated_project_data_contract_compliance(self, assistant):
        ...
        assert isinstance(project_data["project_name"], str), (
            "Project name should be string"
        )

        # Verify capabilities
        capabilities = project_data["capabilities"]
        assert capabilities["grid_generation"] is True, (
            "grid_generation should be enabled"
        )
        assert capabilities["promotion_engine"] is True, (
            "promotion_engine should be enabled"
        )
        assert capabilities["qa_engine"] is True, "qa_engine should be enabled"
        assert capabilities["autofix_engine"] is True, (
            "autofix_engine should be enabled"
        )
    ...
    def test_generated_project_validator_acceptance(self, assistant):
        ...
        assert validation_result.valid, (
            f"Project should pass validation. Errors: {validation_result.errors}"
        )
    ...
    def test_generated_project_scene_count_bounds(self, assistant):
        ...
        for prompt in prompts:
            ...
            assert 3 <= scene_count <= 12, (
                f"Scene count {scene_count} should be between 3 and 12"
            )
    ...
    def test_generated_project_sequence_shot_bounds(self, assistant):
        ...
        for sequence in project.sequences:
            ...
            assert 1 <= shot_count <= 5, (
                f"Shot count {shot_count} should be between 1 and 5"
            )
    ...
    def test_generated_project_capabilities_enabled(self, assistant):
        ...
        assert capabilities.get("grid_generation") is True, (
            "grid_generation should be enabled"
        )
        assert capabilities.get("promotion_engine") is True, (
            "promotion_engine should be enabled"
        )
        assert capabilities.get("qa_engine") is True, "qa_engine should be enabled"
        assert capabilities.get("autofix_engine") is True, (
            "autofix_engine should be enabled"
        )
