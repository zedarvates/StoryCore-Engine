...
from src.assistant.comfyui_config import ComfyUIConfigGenerator
...
class TestComfyUIConfigGenerator:
    ...
    def test_layer_configs_generation(self, sample_project):
        ...
        for sequence in sample_project.sequences:
            for shot in sequence.shots:
                ...
                bg_layers = [
                    layer for layer in layers if layer.layer_type == "background"
                ]
                assert len(bg_layers) == 1

                # Check for character layer
                char_layers = [
                    layer for layer in layers if layer.layer_type == "character"
                ]
    ...
    def test_update_generation_parameters_partial(self, sample_project):
        ...
        # Update only quality
        updated_config = generator.update_generation_parameters(config, quality="low")
    ...
    def test_effects_layer_added_for_effects_shots(self):
        ...
        effects_layers = [layer for layer in layers if layer.layer_type == "effects"]
...
def sample_project():
    ...
    metadata = ProjectMetadata(schema_version="1.0", project_name="test_project")
    ...
    sequence = Sequence(
        id="seq_1", scene_id="scene_1", shots=[shot], total_duration=5.0
    )
...
def create_project_with_effects():
    ...
    metadata = ProjectMetadata(schema_version="1.0", project_name="effects_project")
