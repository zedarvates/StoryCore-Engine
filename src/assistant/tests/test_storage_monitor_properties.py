...
@settings(max_examples=100, deadline=None)
@given(
    current_bytes=st.integers(
        min_value=0, max_value=100 * 1024 * 1024 * 1024
    ),  # 0-100GB
...
def test_storage_limit_enforcement_property(
    current_bytes, current_files, new_bytes, new_files
):
    """
    ...
    **Validates: Requirements 3.5, 3.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expected_would_fit = (
            total_bytes_after <= limit_bytes and total_files_after <= limit_files
        )
    ...
    **Validates: Requirements 3.3, 3.4, 3.5, 3.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if files_to_create > 0 and result.stats.file_count > 0:
            ...
            if (
                result.stats.usage_percent >= 100
                or result.stats.file_usage_percent >= 100
            ):
