...
valid_shot_type_strategy = st.sampled_from(
    ["wide", "medium", "close-up", "extreme-close-up"]
)

valid_camera_movement_strategy = st.sampled_from(
...
def generate_valid_sequence(
    ...
    return st.builds(
        ...
        lambda seq: Sequence(
            ...
            shots=seq.shots,
            total_duration=sum(
                shot.duration for shot in seq.shots
            ),  # Ensure consistency
        )
    )
...
def test_valid_project_passes_validation_property(scene_count, char_count):
    ...
    **Validates: Requirements 4.2, 4.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert result.valid, (
            f"Valid project failed validation with errors: {result.errors}"
        )
...
def test_invalid_project_fails_validation_property(schema_version, missing_field):
    ...
    **Validates: Requirements 4.2, 4.6**
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert has_issues, (
            "Invalid project passed validation without errors or warnings"
        )
