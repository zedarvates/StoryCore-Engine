...
def rate_limiter():
    ...
    return RateLimiter(max_requests=100, window_seconds=60, warning_threshold=0.9)


@pytest.fixture
def small_rate_limiter():
    """Create rate limiter with small limits for testing"""
    return RateLimiter(max_requests=5, window_seconds=1, warning_threshold=0.9)
...
class TestRateLimitInfo:
    ...
    def test_rate_limit_info_without_warning(self):
        ...
        info = RateLimitInfo(allowed=True, remaining=50, reset_time=time.time() + 60)
...
class TestRateLimitExceededError:
    ...
    def test_error_creation(self):
        ...
        error = RateLimitExceededError(
            user_id="test_user", limit=100, window_seconds=60, reset_time=reset_time
        )
