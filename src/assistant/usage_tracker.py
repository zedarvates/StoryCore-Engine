...
class UsageTracker:
    ...
    def get_user_stats(
        ...
        total_data_transferred_bytes = sum(
            r.request_size_bytes + r.response_size_bytes for r in records
        )
    ...
    def get_all_users_stats(self) -> List[UsageStats]:
        ...
        """
        return [self.get_user_stats(user_id) for user_id in self.usage_records.keys()]

    def get_recent_requests(self, user_id: str, limit: int = 100) -> List[UsageRecord]:
        """
        ...
        try:
            ...
            for user_id, records_data in data.items():
                self.usage_records[user_id] = [
                    UsageRecord(**record_data) for record_data in records_data
                ]
    ...
    def export_stats_json(self, user_id: Optional[str] = None) -> str:
        ...
        if user_id:
            ...
            return json.dumps([asdict(stats) for stats in all_stats], indent=2)
