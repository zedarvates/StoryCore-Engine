...
class StorageMonitor:
    ...
    def get_current_usage(self) -> StorageStats:
        ...
        usage_percent = (
            (total_size / self.limit_bytes * 100) if self.limit_bytes > 0 else 0
        )
        file_usage_percent = (
            (file_count / self.file_limit * 100) if self.file_limit > 0 else 0
        )
    ...
    def check_limits(self) -> LimitCheckResult:
        ...
        can_create = (
            stats.total_bytes < self.limit_bytes and stats.file_count < self.file_limit
        )
        ...
        return LimitCheckResult(
            within_limits=can_create, warnings=warnings, stats=stats
        )
