...
from .exceptions import PathValidationError, ConfirmationRequiredError, ResourceError
...
class FileOperationsManager:
    ...
    def __init__(self, project_directory: Path):
        ...
        logger.info(
            f"FileOperationsManager initialized with directory: {self.project_directory}"
        )
    ...
    def validate_path(self, path: Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Path validation failed: {path} is outside {self.project_directory}"
                )
            ...
            raise PathValidationError(path=str(path), reason=f"Invalid path: {str(e)}")
