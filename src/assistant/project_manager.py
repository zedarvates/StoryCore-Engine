...
from .models import Project, ProjectMetadata, Scene, Character, Sequence, Shot
...
class ProjectManager:
    ...
    def __init__(
        self, file_ops: FileOperationsManager, validator: DataContractValidator
    ):
        """
    ...
    def open_project(self, project_name: str) -> Project:
        ...
        if not project_json_path.exists():
            ...
            raise ProjectError(
                message="Project metadata file not found: project.json",
        ...
        try:
            ...
            project = Project(
                ...
                modified_at=datetime.fromisoformat(
                    project_data.get("modified_at", datetime.now().isoformat())
                ),
            )
            ...
            logger.info(
                f"  Scenes: {len(scenes)}, Characters: {len(characters)}, Sequences: {len(sequences)}"
            )
    ...
    def _load_characters(
        self, project_path: Path, project_data: dict
    ) -> List[Character]:
        """
    ...
    def save_project(self, project: Project) -> None:
        ...
        try:
            ...
            if not validation_result.valid:
                logger.error(
                    f"Project validation failed before save: {validation_result.errors}"
                )
                raise ValidationError(
                    message="Project fails Data Contract v1 validation",
                    code="PROJECT_VALIDATION_FAILED",
    ...
    def modify_scene(self, scene_id: str, updates: dict) -> Scene:
        ...
        if not validation_result.valid:
            raise ValidationError(
                ...
                details={"scene_id": scene_id, "errors": validation_result.errors},
                suggested_action="Fix validation errors and try again",
            )
    ...
    def modify_character(self, character_id: str, updates: dict) -> Character:
        ...
        logger.info(
            f"Modified character {character_id} in project {self.active_project.name}"
        )
        return character
    ...
    def modify_sequence(self, sequence_id: str, updates: dict) -> Sequence:
        ...
        logger.info(
            f"Modified sequence {sequence_id} in project {self.active_project.name}"
        )
        return sequence
    ...
    def add_scene(self, scene: Scene) -> Scene:
        ...
        if not validation_result.valid:
            ...
            raise ValidationError(
                ...
                details={"scene_id": scene.id, "errors": validation_result.errors},
                suggested_action="Fix validation errors and try again",
            )
