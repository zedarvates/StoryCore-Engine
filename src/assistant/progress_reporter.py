...
class ProgressReporter:
    ...
    def update(
        ...
        if self.cancelled:
            logger.warning(
                f"Progress update ignored - operation {self.operation_id} is cancelled"
            )
            return
        ...
        # Log update
        logger.info(
            f"Progress [{self.operation_id}]: {stage.value} - {progress_percent:.1f}% - {message}"
        )
    ...
    def _calculate_progress(self, stage: OperationStage) -> float:
        ...
        total_weight = sum(
            self.stage_weights.get(s, 0) for s in stages if s in self.stage_weights
        )
