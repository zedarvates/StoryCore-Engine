...
from .models import Project, GeneratedProject
...
from .exceptions import ResourceError, ValidationError, StorageLimitExceededError
from .logging_config import get_logger
from .rlm_engine import RLMAgentCore
...
def create_llm_client(provider: str = "mock", model: Optional[str] = None):
    ...
    if provider == "mock":
        ...
        try:
            ...
            logger.warning(
                "OpenAI package (required for OpenRouter) not installed, using mock client"
            )
            return MockLLMClient()
    else:
...
class StoryCoreAssistant:
    ...
    def __init__(
        ...
        """
        logger.info(
            f"Initializing StoryCoreAssistant with project_directory={project_directory}"
        )

        # Initialize component managers
        self.file_ops = FileOperationsManager(project_directory)
        self.storage_monitor = StorageMonitor(
            project_directory, storage_limit_gb, file_limit
        )
    ...
    def generate_project(
        ...
        if not self.storage_monitor.estimate_operation(
            estimated_bytes, estimated_files
        ):
        ...
        try:
            generated_project = self.project_generator.generate_project(
                prompt=prompt, language=language, preferences=preferences
            )
        finally:
        ...
        if not validation_result.valid:
            logger.error(
                f"Generated project validation failed: {validation_result.errors}"
            )
    ...
    async def generate_complex_scenario_rlm(
        ...
        class StoryCoreRLMAgent(RLMAgentCore):
            def __init__(
                self,
                assistant_ref: "StoryCoreAssistant",
            ...
            async def query_database(self, query: str) -> str:
                ...
                for node in self.assistant.story_graph._nodes.values():
                    if (
                        ...
                        or query_lower in str(node.attributes).lower()
                    ):
                        context.append(
                            f"{node.entity_type.title()} '{node.name}': {node.attributes}"
                        )
            ...
            async def create_n8n(
                self, name: str, nodes: List[Dict], connections: Dict
            ) -> Dict:
                # Create n8n workflow
        ...
        rules_path = str(
            Path(self.file_ops.project_directory).parent / "substrate.rules"
        )
        ...
        project_root = (
            str(active_project.path)
            if active_project
            else str(self.file_ops.project_directory)
        )

        final_answer = await engine.process(
            main_prompt, massive_context, project_root=project_root
        )
    ...
    def _save_project_files(
        self, project: Project, comfyui_config: ComfyUIConfig
    ) -> None:
        """
