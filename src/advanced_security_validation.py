...
class InputValidator:
    ...
    def validate_prompt(self, prompt: str, user_id: str = None) -> ValidationReport:
        ...
        if len(prompt) > self.config.max_prompt_length:
            ...
            recommendations.append(
                f"Prompt exceeds maximum length ({self.config.max_prompt_length} chars)"
            )
        ...
        details.update(
            {"prompt_length": len(prompt), "risk_score": risk_score, "user_id": user_id}
        )
    ...
    def validate_file_upload(
        self, file_path: str, file_size: int, user_id: str = None
    ) -> ValidationReport:
    ...
    def _validate_image_magic_bytes(
        self, magic_bytes: bytes, expected_ext: str
    ) -> bool:
...
class ModelIntegrityChecker:
    ...
    def verify_model_integrity(self, model_path: str) -> ValidationReport:
        ...
        if not os.path.exists(model_path):
            ...
            if (
                self.config.verify_model_checksums
                and model_name in self.known_checksums
            ):
                ...
                if current_hash != expected_hash:
                    risk_score += 0.8
                    recommendations.append(
                        "Model checksum mismatch - possible tampering"
                    )
...
class AccessController:
    ...
    def validate_api_key(
        ...
        if not self.config.require_api_keys:
            ...
            # Check permissions
            if required_permission and required_permission not in key_info.get(
                "permissions", []
            ):
                risk_score = 0.8
                recommendations.append(
    ...
    def validate_session(
        ...
        if session_id not in self.active_sessions:
            ...
            if time_since_activity.total_seconds() > (
                ...
                result = (
                    ...
                    if risk_score < 0.3
                    else ValidationResult.SUSPICIOUS
                )
    ...
    def record_failed_attempt(self, identifier: str, ip_address: str = None):
        ...
        if self.failed_attempts[key]["count"] >= self.config.max_failed_attempts:
            ...
            logger.warning(
                f"Blocked {key} after {self.failed_attempts[key]['count']} failed attempts"
            )
...
class RateLimiter:
    ...
    def check_rate_limit(
        ...
        self.burst_counts[key] = [
            req_time for req_time in self.burst_counts[key] if req_time > burst_cutoff
        ]
...
class AuditLogger:
    ...
    def log_action(
        ...
        entry = AuditLogEntry(
            ...
            else self._anonymize_user_id(user_id),
            ...
            ip_address=ip_address
            if not self.config.anonymize_user_data
            else self._anonymize_ip(ip_address),
    ...
    def _cleanup_old_entries(self):
        ...
        self.audit_entries = [
            entry for entry in self.audit_entries if entry.timestamp > cutoff_date
        ]
    ...
    def get_audit_report(self, hours: int = 24) -> Dict[str, Any]:
                    ...
                    if isinstance(entry_dict, dict):
                        entry = AuditLogEntry(
                            timestamp=datetime.fromisoformat(
                                entry_dict.get("timestamp", datetime.now().isoformat())
                            ),
        ...
        recent_entries = [
            entry for entry in all_entries if entry.timestamp > cutoff_time
        ]
...
class SecurityManager:
    ...
    def validate_request(
        ...
        # Session validation
        if session_id:
            session_result = self.access_controller.validate_session(
    ...
    def perform_security_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.audit_logger.get_audit_report(hours=24)

            # Analyze failed attempts
            failed_attempts = sum(
                1
                for entry in self.audit_logger.audit_entries
            ...
            if failed_attempts > self.config.suspicious_activity_threshold:
                scan_results["findings"].append(
                    f"High number of failed attempts: {failed_attempts}"
                )
            ...
            if len(self.access_controller.blocked_ips) > 0:
                scan_results["findings"].append(
                    f"Blocked IPs: {len(self.access_controller.blocked_ips)}"
                )
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "audit_entries_24h": len(
                [
                    e
                    for e in self.audit_logger.audit_entries
...
def run_security_tests():
    ...
    for prompt in test_prompts:
        ...
        print(
            f"  Prompt: '{prompt[:50]}...' -> {result.result.value} (risk: {result.risk_score:.2f})"
        )
    ...
    print(
        f"  Request validation: {result.result.value} (risk: {result.risk_score:.2f})"
    )
