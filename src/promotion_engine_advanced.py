...
class PromotionEngine:
    ...
    def process_grid(self, promotion_plan_path: str) -> Dict[str, Any]:
        ...
        for panel_config in plan["panels"]:
            ...
            cinematic_panel = self._center_fill_crop(
                panel_slice, self.target_aspect_ratio
            )
    ...
    def _prepare_refinement_metadata(
        self, panel_config: Dict[str, Any], global_seed: int
    ) -> Dict[str, Any]:
        """Prepare metadata for ComfyUI/Automatic1111 API call."""
        return {
            "denoising_strength": self.denoising_strength,
            "prompt_extension": panel_config.get(
                "prompt_extension", "highly detailed, cinematic lighting"
            ),
    ...
    def _generate_qa_report(
        self, metrics: List[Dict[str, Any]], plan: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            if np.mean(sharpness_scores) > 100.0
            else "REVIEW_NEEDED",
        }
...
if __name__ == "__main__":
    ...
    print(
        f"  Mean sharpness: {results['qa_report']['sharpness_metrics']['mean_sharpness']:.1f}"
    )
