...
from .comfyui_models import HealthStatus, HealthState, SystemStats, PerformanceMetrics
...
class HealthMonitor:
                        ...
                        if response.status == 200:
                            ...
                            self.performance_monitor.record_operation_end(
                                ...
                                True,
                                {
                                    ...
                                    if hasattr(system_stats, "to_dict")
                                    else None,
                                },
                            )
                            ...
                            self.logger.warning(
                                f"Health check degraded: HTTP {response.status}"
                            )
                            metrics.complete(
    ...
    async def wait_for_service_ready(self, timeout_seconds: float) -> bool:
        ...
        self.logger.info(
            f"Waiting for ComfyUI service to become ready (timeout: {timeout_seconds}s)"
        )
    ...
    def calculate_backoff_delay(self) -> float:
        ...
        self.logger.debug(
            f"Calculated backoff delay: {delay:.2f}s (failures: {self._consecutive_failures})"
        )
        return delay
    ...
    def _parse_system_stats(self, stats_data: Dict[str, Any]) -> SystemStats:
        ...
        try:
            ...
            return SystemStats(
                ...
                queue_remaining=stats_data.get("exec_info", {}).get(
                    "queue_remaining", 0
                ),
    ...
    def _on_error_recovery(self, error_info, fallback_mode: FallbackMode) -> None:
        ...
        """
        self.logger.info(
            f"Health monitor error recovery: {fallback_mode.value} for error {error_info.error_id}"
        )
