...
class GhostTrackerWizard:
    ...
    async def perform_tracking_analysis(
        self,
        project_path: Path,
        tracking_mode: TrackingMode = TrackingMode.CONTINUITY,
        target_shots: Optional[List[str]] = None,
    ) -> TrackingResult:
        """
        ...
        shots_to_analyze = target_shots or [
            shot.get("shot_id") for shot in all_shots if shot.get("shot_id")
        ]
        ...
        if tracking_mode == TrackingMode.CONTINUITY:
            ...
            await self._perform_visual_consistency_check(
                result, all_shots, project_data
            )
    ...
    async def _perform_continuity_tracking(
        self,
        result: TrackingResult,
    ...
    async def _perform_motion_analysis(
        self,
        result: TrackingResult,
        shots: List[Dict[str, Any]],
        project_data: Dict[str, Any],
    ...
    async def _perform_visual_consistency_check(
        self,
        result: TrackingResult,
        shots: List[Dict[str, Any]],
        project_data: Dict[str, Any],
    ...
    async def _track_props_and_objects(
        self,
        result: TrackingResult,
        shots: List[Dict[str, Any]],
        project_data: Dict[str, Any],
    ...
    def _detect_continuity_pattern_issues(
        self, element: TrackedElement, shot_sequence: List[str]
    ) -> bool:
    ...
    async def _detect_motion_anomalies(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
        ...
        for shot in shots:
            ...
            if any(
                word in shot_description
                for word in ["sudden", "jerk", "unusual", "abnormal"]
            ):
                issue = ContinuityIssue(
    ...
    async def _detect_visual_anomalies(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
    ...
    async def _analyze_movement_patterns(
        ...
        for shot in shots:
            ...
            if any(
                word in shot_description
                for word in ["moving", "walking", "running", "driving"]
            ):
    ...
    async def _track_element_paths(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
        """Track paths of moving elements"""
        # Simplified path tracking - in real implementation would use CV tracking
        moving_elements = [
            elem for elem in result.tracked_elements if "movement" in elem.element_id
        ]
    ...
    async def _check_lighting_consistency(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
        ...
        for shot in shots:
            ...
            if lighting_info:
                lighting_descriptions.append(
                    {"shot_id": shot_id, "lighting": lighting_info}
                )
    ...
    async def _check_prop_consistency(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
        ...
        for prop in prop_elements:
            ...
            if len(positions) > 3:  # Multiple appearances
                prop.tracking_notes.append(
                    "Prop appears consistently across multiple shots"
                )
    ...
    def _check_lighting_consistency_in_list(
        self, lighting_list: List[Dict[str, Any]]
    ) -> bool:
    ...
    def _perform_additional_qa_checks(
        self, result: TrackingResult, shots: List[Dict[str, Any]]
    ):
    ...
    def _calculate_overall_confidence(self, result: TrackingResult) -> float:
        ...
        element_confidence = sum(
            elem.confidence_score for elem in result.tracked_elements
        ) / len(result.tracked_elements)
    ...
    def _generate_analysis_summary(self, result: TrackingResult) -> str:
        ...
        summary_parts.append(
            f"Tracking analysis completed in {result.mode.value} mode."
        )
        summary_parts.append(
            f"Tracked {result.total_elements_tracked} elements across {len(result.target_shots)} shots."
        )

        if result.continuity_issues:
            summary_parts.append(
                f"Found {result.total_issues_found} potential issues requiring attention."
            )
        else:
    ...
    def _generate_recommendations(self, result: TrackingResult) -> List[str]:
        ...
        if result.total_issues_found > 0:
            recommendations.append(
                "Review flagged continuity issues and consider reshoots if critical."
            )

        if result.total_elements_tracked < 5:
            recommendations.append(
                "Consider adding more detailed element descriptions for comprehensive tracking."
            )

        if result.overall_confidence < 7.0:
            recommendations.append(
                "Consider professional tracking services for higher accuracy."
            )

        if result.mode == TrackingMode.CONTINUITY:
            recommendations.append(
                "Run anomaly detection mode for additional quality checks."
            )
...
def create_ghost_tracker_wizard(
    vision_engine=None, tracking_engine=None
) -> GhostTrackerWizard:
