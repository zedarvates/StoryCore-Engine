...
class ValidatorService:
    ...
    def validate_project_name_unique(
        ...
        if project_path.exists():
            return (
                False,
                f"Project '{name}' already exists. Choose a different name or delete the existing project.",
            )
    ...
    def validate_duration(duration_str: str, format_key: str) -> Tuple[bool, str]:
        ...
        if duration < min_duration:
            return (
                False,
                f"Duration must be at least {min_duration} minutes for {format_def.name}",
            )

        if duration > max_duration:
            return (
                False,
                f"Duration must be at most {max_duration} minutes for {format_def.name}",
            )
