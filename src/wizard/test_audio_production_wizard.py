...
class TestAudioProductionWizard:
    ...
    async def test_shot_needs_voice_over(self):
        ...
        needs_vo = wizard._shot_needs_voice_over(
            {"purpose": "narration", "description": "explanatory content"}, {}
        )
        assert needs_vo is True

        # Test opening shot
        needs_vo = wizard._shot_needs_voice_over(
            {"purpose": "opening", "description": "introduction scene"}, {}
        )
        assert needs_vo is True

        # Test regular shot
        needs_vo = wizard._shot_needs_voice_over(
            {"purpose": "action", "description": "fight scene"}, {}
        )
    ...
    async def test_generate_voice_over_script(self):
        ...
        script = wizard._generate_voice_over_script(
            {"description": "A beautiful sunset over the mountains"}, AudioMood.PEACEFUL
        )
    ...
    async def test_create_sound_effects_for_shot(self):
        ...
        sequence = AudioSequence(
            sequence_id="test_seq", shot_id="shot_001", duration=5.0
        )
        ...
        elements = await wizard._create_sound_effects_for_shot(
            shot_door, sequence, AudioMood.NEUTRAL
        )
        ...
        elements = await wizard._create_sound_effects_for_shot(
            shot_walk, sequence, AudioMood.NEUTRAL
        )
        footstep_effects = [
            e
            for e in elements
            if "footstep" in e.name.lower() or "foley" in e.audio_type.value
        ]
    ...
    async def test_create_ambient_audio_for_shot(self):
        ...
        elements = await wizard._create_ambient_audio_for_shot(
            {}, sequence, AudioMood.MYSTERIOUS
        )
    ...
    async def test_create_music_cues_for_shot(self):
        ...
        shot_dramatic = {"purpose": "climactic"}

        elements = await wizard._create_music_cues_for_shot(
            shot_dramatic, sequence, AudioMood.DRAMATIC
        )
    ...
    async def test_calculate_audio_quality_metrics(self):
        ...
        plan = AudioProductionPlan(
            ...
            audio_sequences=[
                ...
                AudioSequence(
                    sequence_id="seq2", shot_id="shot2", duration=5.0, audio_elements=[]
                ),
            ],
        )
