...
class StoryDocumentationGenerator:
    ...
    def generate_from_wizard_state(self, wizard_state) -> StoryDocumentation:
        ...
        if hasattr(wizard_state, "project_name"):
            ...
            self.doc.project_title = wizard_state.get(
                "project_name", "Untitled Project"
            )

        if hasattr(wizard_state, "story_content"):
            self.doc.general_pitch = (
                wizard_state.story_content[:500] if wizard_state.story_content else ""
            )
        else:
    ...
    def _generate_defaults(self):
        ...
        if not self.doc.emotional_objectives:
            self.doc.emotional_objectives = (
                "Create an engaging emotional journey for the reader"
            )

        if not self.doc.general_constraints:
            self.doc.general_constraints = (
                "Genre: General | Style: Professional | No explicit content"
            )
        ...
        if not self.doc.narrative_style:
            self.doc.narrative_style = (
                "Third-person narrative, immersive and descriptive"
            )
    ...
    def generate_all_files(self, output_dir: Path) -> Dict[str, str]:
        ...
        for chapter_num in range(1, 6):
            files[f"story-chapter-0{chapter_num}.md"] = self._generate_story_chapter(
                chapter_num
            )
...
"""
    ...
    def _generate_plot_core(self) -> str:
        ...
        obstacles = (
            "\n".join([f"- {o}" for o in self.doc.major_obstacles])
            if self.doc.major_obstacles
            else "- À définir"
        )
        turning_points = (
            "\n".join([f"- {t}" for t in self.doc.turning_points])
            if self.doc.turning_points
            else "- À définir"
        )
        mandatory = (
            "\n".join([f"- {m}" for m in self.doc.mandatory_elements])
            if self.doc.mandatory_elements
            else "- À définir"
        )
...
"""
    ...
    def _generate_hidden_truth(self) -> str:
        ...
        lies = (
            "\n".join([f"- {lie}" for lie in self.doc.initial_lies])
            if self.doc.initial_lies
            else "- À définir"
        )
        events = (
            "\n".join([f"- {e}" for e in self.doc.hidden_events])
            if self.doc.hidden_events
            else "- À définir"
        )
        revelations = (
        ...
        twists = (
            "\n".join([f"- {t}" for t in self.doc.narrative_twists])
            if self.doc.narrative_twists
            else "- À définir"
        )
    ...
    def _generate_timelines(self) -> str:
        ...
        world_events = (
            "\n".join(
                [
                    ...
                    for e in self.doc.world_timeline
                ]
            )
            if self.doc.world_timeline
            else "- À définir"
        )
        novel_events = (
            "\n".join(
                [
                    f"- **{e.date}**: {e.event} ({e.importance})"
                    for e in self.doc.novel_timeline
                ]
            )
        ...
        chapters = (
            "\n".join(
                [
                    f"- Chapitre {i + 1}: {c}"
                    for i, c in enumerate(self.doc.chapter_order)
                ]
            )
            if self.doc.chapter_order
            else "- À définir"
        )
...
**Transition:** {transition}"""
    ...
    def _get_scene_characters(self, scene_num: int) -> str:
        ...
        if self.doc.characters:
            ...
            if scene_num <= 3:
                ...
                return (
                    ", ".join(all_chars[: min(3, len(all_chars))])
                    if all_chars
                    else "PROTAGONIST, ALLIES"
                )
            else:
                # Later scenes focus on key confrontations
                return (
                    ", ".join(all_chars[: min(2, len(all_chars))])
                    if all_chars
                    else "PROTAGONIST, ANTAGONIST"
                )
    ...
    def _get_scene_action(self, scene_num: int) -> str:
        ...
        return default_actions.get(
            scene_num, "The scene advances the plot and develops character arcs."
        )
    ...
    def _get_scene_dialogue(self, scene_num: int, characters: str) -> str:
        ...
        lines = []
        template = dialogue_templates.get(
            scene_num,
            [("thoughtfully", "We must continue."), ("nodding", "I understand.")],
        )
...
{dialogue_text}""")
        ...
        if self.doc.characters:
            ...
            for char in self.doc.characters:
                desc = (
                    char.biography[:80]
                    if char.biography
                    else "Key character in the story"
                )
    ...
    def _generate_story_index(self) -> str:
        ...
        for i in range(1, 6):
            ...
            chapter_list.append(
                f"{i + 1}. [Chapter {i}](story-chapter-0{i}.md) - {chapter_title}"
            )
    ...
    def _get_chapter_characters(self, chapter_num: int) -> str:
        ...
        if self.doc.characters:
            ...
            for i, char in enumerate(self.doc.characters[:3]):  # First 3 characters
                chars.append(
                    f"- **{char.name}**: {char.biography[:100] if char.biography else 'Character role in this chapter'}"
                )
            return "\n".join(chars)
...
- Reference world rules and established lore"""
    ...
    def _get_characters_list(self) -> str:
        ...
        if self.doc.characters:
            return "\n".join(
                [
                    f"- **{char.name}**: {char.biography[:100] if char.biography else 'Character description'}"
        ...
        if self.doc.characters:
            ...
            for char in self.doc.characters[:3]:
                arc = (
                    ...
                    else "Character arc reaches conclusion"
                )
...
- Thematic closing image"""
    ...
    def _get_all_character_arcs(self) -> str:
        ...
        if self.doc.characters:
            ...
            for char in self.doc.characters:
                arc = (
                    char.detailed_arc
                    if char.detailed_arc
                    else char.emotional_arc
                    or "Character development throughout the story"
                )
    ...
    def _get_timeline_summary(self) -> str:
        ...
        if self.doc.novel_timeline:
            return "\n".join(
                [f"- {e.date}: {e.event}" for e in self.doc.novel_timeline[:5]]
            )
