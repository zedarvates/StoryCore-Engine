...
class WizardOrchestrator:
    ...
    def _check_required_configurations(self) -> bool:
        ...
        try:
            ...
            if not genres:
                display_error(
                    "No genre definitions found. Please check your configuration."
                )
                return False
            ...
            if not formats:
                display_error(
                    "No format definitions found. Please check your configuration."
                )
                return False
    ...
    def _show_welcome(self):
        ...
        display_message(
            "This wizard will guide you through creating a new video project."
        )
    ...
    def _collect_project_name(self) -> bool:
        ...
        try:
            self.state.project_name = self.input_handler.prompt_text(
                "Project name", default=default_name, validator=validator
            )
    ...
    def _collect_duration(self) -> bool:
        ...
        min_dur, max_dur = format_def.duration_range
        display_message(
            f"For {format_def.name}, duration must be between {min_dur} and {max_dur} minutes."
        )
        ...
        try:
            duration_str = self.input_handler.prompt_text(
                "Duration (in minutes)", validator=validator
            )
    ...
    def _collect_story_manually(self) -> bool:
        ...
        try:
            ...
            story_input = self.input_handler.prompt_text(
                ...
                validator=lambda x: (
                    (True, "") if x.upper() == "MULTI" else validator(x)
                ),
            )
    ...
    def _generate_story_automatically(self) -> bool:
        ...
        """
        display_message(
            "Generating story automatically based on your project parameters..."
        )
        display_message(
            "This will create a complete 3-act story with theme, conflict, and character arcs."
        )
        ...
        try:
            ...
            for i, act in enumerate(story.acts, 1):
                ...
                display_message(
                    f"  Act {i}: {act.title} ({act.duration_percent}% - {duration_min:.1f} min)"
                )
            ...
            confirmed = self.input_handler.prompt_confirm(
                "Use this generated story?", default=True
            )
            ...
            if confirmed:
                ...
                self.state.generated_story = (
                    story  # Store full story object for V2 pipeline
                )
                self.state.current_step = 5
            ...
            display_error(
                "Story generation feature not available. Please enter story manually."
            )
            return self._collect_story_manually()
        except Exception as e:
    ...
    def _show_summary_and_confirm(self) -> bool:
        ...
        if (
            hasattr(self.state, "generated_story")
            and self.state.generated_story is not None
        ):
            # Show generated story details
            story = self.state.generated_story
            display_message("Story: Generated automatically")
            display_message(f"  Title: {story.title}")
            display_message(f"  Theme: {story.theme}")
            display_message(f"  Tone: {story.tone}")
