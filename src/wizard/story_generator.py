        ...
        if self.acts:
            ...
            for i, (act, expected) in enumerate(zip(self.acts, expected_durations)):
                if abs(act.duration_percent - expected) > 5.0:  # 5% tolerance
                    errors.append(
                        f"Act {i + 1} duration {act.duration_percent}% should be ~{expected}%"
                    )
...
class StoryGenerator:
    ...
    def _generate_acts(
        self, wizard_state: WizardState, theme: str, conflict: str
    ) -> List[Act]:
    ...
    def _generate_confrontation_act(
        self, wizard_state: WizardState, conflict: str
    ) -> Act:
    ...
    def _get_setup_beats(self, genre_key: str) -> List[StoryBeat]:
        ...
        genre_beats = {
            "action": [StoryBeat("Action Sequence", "High-energy opening", 5.0, 8, 6)],
            "drame": [
                StoryBeat(
                    "Character Moment", "Emotional character introduction", 5.0, 5, 2
                )
            ],
            "science_fiction": [
            ...
            "comedie": [StoryBeat("Comedy Setup", "Establish comedic tone", 4.0, 7, 1)],
        }
    ...
    def _get_confrontation_beats(
        self, genre_key: str, conflict: str
    ) -> List[StoryBeat]:
        ...
        # Add genre-specific beats
        genre_beats = {
            "action": [StoryBeat("Chase Sequence", "High-stakes pursuit", 10.0, 9, 8)],
            "drame": [
                StoryBeat(
                    "Emotional Crisis", "Character's internal struggle", 12.0, 9, 6
                )
            ],
            "science_fiction": [
            ...
            "comedie": [
                StoryBeat(
                    "Comedy Complications", "Humorous misunderstandings", 8.0, 8, 4
                )
            ],
        }
    ...
    def _get_resolution_beats(self, genre_key: str, theme: str) -> List[StoryBeat]:
        ...
        genre_beats = {
            ...
            "science_fiction": [
                StoryBeat(
                    "Future Implications", "Show consequences of events", 6.0, 5, 2
                )
            ],
            "horreur": [StoryBeat("Final Scare", "Last moment of terror", 7.0, 9, 8)],
            "comedie": [
    ...
    def _generate_logline(self, genre_key: str, conflict: str) -> str:
        ...
        template = templates.get(
            genre_key, "A story about {conflict} and its consequences."
        )
    ...
    def _generate_summary(
        self, title: str, logline: str, theme: str, acts: List[Act]
    ) -> str:
        ...
        for i, act in enumerate(acts, 1):
            summary_parts.append(
                f"ACT {i} - {act.title.upper()} ({act.duration_percent}%)"
            )
        ...
        summary_parts.append(
            f"The story reinforces the {theme} theme through character development and plot resolution, "
        )
        summary_parts.append(
            "creating a satisfying narrative arc that resonates with audiences."
        )
    ...
    def _load_themes(self) -> Dict[str, List[str]]:
        ...
        return {
            "action": ["Good vs Evil", "Justice", "Heroism", "Sacrifice", "Redemption"],
            "drame": [
                "Love and Loss",
                "Family",
                "Coming of Age",
            ...
            "universal": ["Good vs Evil", "Love", "Growth", "Truth", "Hope"],
        }
