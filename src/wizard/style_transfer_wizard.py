        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload) as response:
                if response.status == 200:
                    ...
                    raise Exception(
                        f"Failed to queue prompt: {response.status} - {text}"
                    )
...
class StyleTransferWizard:
    ...
    DEFAULT_WORKFLOW = {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "outputs": [{"name": "IMAGE", "type": "IMAGE", "links": [346, 358]}],
                "properties": {
                    "proxyWidgets": [
                        ["-1", "noise_seed"],
            ...
            },
            {
                ...
                "order": 20,
                "mode": 0,
                "inputs": [{"name": "images", "type": "IMAGE", "link": 346}],
                "outputs": [],
                "properties": {
                    "cnr_id": "comfy-core",
                    "ver": "0.8.2",
    ...
    async def transfer_style_workflow(
        self, config: WorkflowConfig, progress_callback: Optional[callable] = None
    ) -> StyleTransferResult:
        """
        ...
        try:
            ...
            if config.source_image_path:
                source_name = await self.comfyui_client.upload_image(
                    config.source_image_path, "source_image.png"
                )
            ...
            if config.style_image_path:
                style_name = await self.comfyui_client.upload_image(
                    config.style_image_path, "style_reference.png"
                )
                # Update workflow with uploaded image
    ...
    async def transfer_style_prompt(
        self, config: PromptConfig, progress_callback: Optional[callable] = None
    ) -> StyleTransferResult:
        """
    ...
    async def transfer_video_style(
        self, config: VideoConfig, progress_callback: Optional[callable] = None
    ) -> StyleTransferResult:
        """
    ...
    def _update_image_node(
        self, workflow: Dict[str, Any], node_id: int, image_name: str
    ):
...
def create_style_transfer_wizard(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--style", help="Style reference image path (for workflow mode)"
    )
