...
class TestVideoEditorWizard:
    ...
    async def test_video_editor_wizard_creation(self):
        ...
        with (
            patch.object(wizard, "_load_project_data") as mock_load,
            patch.object(wizard, "_load_audio_production_plan") as mock_audio,
            patch.object(wizard, "_save_montage_plan"),
        ):
    ...
    async def test_editing_style_transitions(self):
        ...
        cinematic_trans = wizard._choose_transition_type(
            EditingStyle.CINEMATIC, test_shots[0], test_shots[1]
        )
        assert isinstance(cinematic_trans, TransitionType)

        # Test dynamic style
        dynamic_trans = wizard._choose_transition_type(
            EditingStyle.DYNAMIC, test_shots[0], test_shots[1]
        )
    ...
    async def test_transition_durations(self):
        ...
        cinematic_duration = wizard._calculate_transition_duration(
            EditingStyle.CINEMATIC, from_shot, to_shot
        )
        assert 0.4 <= cinematic_duration <= 0.6

        # Dynamic should have shorter transitions
        dynamic_duration = wizard._calculate_transition_duration(
            EditingStyle.DYNAMIC, from_shot, to_shot
        )
        assert dynamic_duration < cinematic_duration

        # Minimalist should have no transitions
        minimalist_duration = wizard._calculate_transition_duration(
            EditingStyle.MINIMALIST, from_shot, to_shot
        )
