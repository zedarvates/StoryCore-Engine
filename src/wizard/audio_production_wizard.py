...
class AudioProductionWizard:
    ...
    async def create_audio_production_plan(
        self, project_path: Path, focus_shots: Optional[List[str]] = None
    ) -> AudioProductionPlan:
        """
        ...
        plan.voice_over_script = await self._generate_voice_over_script(
            plan.audio_sequences, project_data
        )
        plan.music_cues = await self._generate_music_cues(
            plan.audio_sequences, project_data
        )
        plan.sound_effects_inventory = await self._generate_sound_effects_inventory(
            plan.audio_sequences
        )
        ...
        print(
            f"🎤 Voice elements: {sum(1 for seq in plan.audio_sequences for elem in seq.audio_elements if elem.audio_type == AudioType.VOICE_OVER)}"
        )
        print(
            f"🔊 Sound effects: {sum(1 for seq in plan.audio_sequences for elem in seq.audio_elements if elem.audio_type == AudioType.SOUND_EFFECT)}"
        )
    ...
    async def _create_audio_sequence_for_shot(
        self, shot: Dict[str, Any], project_data: Dict[str, Any]
    ) -> AudioSequence:
        ...
        sequence = AudioSequence(
            sequence_id=f"audio_seq_{shot_id}", shot_id=shot_id, duration=duration
        )
    ...
    async def _analyze_shot_for_audio(
        self,
        shot: Dict[str, Any],
        sequence: AudioSequence,
        ...
        shot.get("shot_type", {}).get("code", "MS")
        shot_description = shot.get("description", "").lower()
        shot.get("camera", {}).get("movement", {}).get("type", "static")
        ...
        if self._shot_needs_voice_over(shot, project_data):
            voice_element = await self._create_voice_over_element(
                shot, sequence, primary_mood
            )
            sequence.audio_elements.append(voice_element)

        # Create sound effects based on shot content
        sfx_elements = await self._create_sound_effects_for_shot(
            shot, sequence, primary_mood
        )
        sequence.audio_elements.extend(sfx_elements)

        # Create ambient audio
        ambient_elements = await self._create_ambient_audio_for_shot(
            shot, sequence, primary_mood
        )
        sequence.audio_elements.extend(ambient_elements)

        # Create background music cues
        music_elements = await self._create_music_cues_for_shot(
            shot, sequence, primary_mood
        )
    ...
    def _determine_shot_mood(self, description: str, purpose: str) -> AudioMood:
        ...
        if any(
            ...
            for word in ["dark", "shadow", "night", "mysterious", "suspense"]
        ):
            return AudioMood.MYSTERIOUS
        elif any(
            word in description_lower
            for word in ["fight", "battle", "intense", "action"]
        ):
            return AudioMood.TENSE
        elif any(
            word in description_lower
            for word in ["peaceful", "calm", "serene", "quiet"]
        ):
            return AudioMood.PEACEFUL
        elif any(
            word in description_lower
            for word in ["exciting", "energetic", "fast-paced"]
        ):
            return AudioMood.ENERGETIC
        elif any(word in description_lower for word in ["romantic", "love", "tender"]):
            return AudioMood.ROMANTIC
        elif any(
            word in description_lower for word in ["sad", "melancholy", "emotional"]
        ):
            return AudioMood.MELANCHOLIC
        elif any(
            word in description_lower for word in ["dramatic", "important", "climactic"]
        ):
    ...
    def _shot_needs_voice_over(
        ...
        return any(
            indicator in shot_purpose or indicator in shot_description
            for indicator in [
                "exposition",
                "tutorial",
                "narration",
    ...
    async def _create_voice_over_element(
        self, shot: Dict[str, Any], sequence: AudioSequence, mood: AudioMood
    ) -> AudioElement:
        ...
        element = AudioElement(
            ...
            * 0.7,  # Voice over typically 70% of shot duration
    ...
    async def _create_sound_effects_for_shot(
        self, shot: Dict[str, Any], sequence: AudioSequence, mood: AudioMood
    ) -> List[AudioElement]:
        ...
        if any(
            word in shot_description for word in ["footsteps", "walking", "running"]
        ):
            elements.append(
                AudioElement(
    ...
    async def _create_ambient_audio_for_shot(
        self, shot: Dict[str, Any], sequence: AudioSequence, mood: AudioMood
    ) -> List[AudioElement]:
        ...
        if mood in [AudioMood.MYSTERIOUS, AudioMood.TENSE]:
            ...
            elements.append(
                AudioElement(
                    ...
                    technical_specs={
                        "type": "nature_pad",
                        "elements": ["gentle_wind", "distant_birds"],
                    },
                    confidence_score=0.7,
                )
            )
    ...
    async def _create_music_cues_for_shot(
        self, shot: Dict[str, Any], sequence: AudioSequence, mood: AudioMood
    ) -> List[AudioElement]:
        ...
        shot_purpose = shot.get("purpose", "").lower()
        needs_music = any(
            purpose in shot_purpose
            for purpose in ["dramatic", "emotional", "climactic", "opening", "closing"]
        )

        if needs_music or mood in [
            AudioMood.DRAMATIC,
            AudioMood.EPIC,
            AudioMood.ROMANTIC,
        ]:
    ...
    async def _create_foley_for_shot(
        self, shot: Dict[str, Any], sequence: AudioSequence
    ) -> List[AudioElement]:
        ...
        if any(
            action in shot_description for action in ["walking", "running", "moving"]
        ):
            elements.append(
                AudioElement(
    ...
    async def _generate_voice_over_script(
        self, audio_sequences: List[AudioSequence], project_data: Dict[str, Any]
    ) -> str:
        ...
        for sequence in audio_sequences:
            voice_elements = [
                elem
                for elem in sequence.audio_elements
    ...
    async def _generate_music_cues(
        self, audio_sequences: List[AudioSequence], project_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_sound_effects_inventory(
        self, audio_sequences: List[AudioSequence]
    ) -> List[Dict[str, Any]]:
        ...
        for sequence in audio_sequences:
            ...
            for element in sfx_elements:
                ...
                if key not in inventory:
                    inventory[key] = {"category": key, "effects": [], "shots": []}
    ...
    def _generate_production_notes(self, plan: AudioProductionPlan) -> List[str]:
        ...
        notes.append("Audio Production Plan Overview:")
        ...
        if voice_count > 0:
            ...
            notes.append("- Record voice overs in a quiet environment")
            notes.append("- Use a quality microphone for clear audio")
            notes.append("- Consider professional voice talent for important segments")

        if sfx_count > 0:
            notes.append("\nSound Effects Production Notes:")
            notes.append("- Source high-quality sound libraries")
            notes.append("- Record custom effects if needed")
            notes.append("- Ensure consistent quality across all effects")

        if music_count > 0:
            notes.append("\nMusic Production Notes:")
            notes.append("- License royalty-free music or compose original")
            notes.append("- Ensure music fits the emotional tone of scenes")
            notes.append("- Fade music appropriately between scenes")

        return notes

    def _generate_technical_requirements(
        self, plan: AudioProductionPlan
    ) -> Dict[str, Any]:
    ...
    def _calculate_audio_quality_metrics(
        ...
        shots_with_voice = sum(
            1
            for seq in plan.audio_sequences
            if any(
                elem.audio_type == AudioType.VOICE_OVER for elem in seq.audio_elements
            )
        )
        shots_with_sfx = sum(
            1
            for seq in plan.audio_sequences
            if any(
                elem.audio_type in [AudioType.SOUND_EFFECT, AudioType.FOLEY]
                for elem in seq.audio_elements
            )
        )
        shots_with_music = sum(
            1
            for seq in plan.audio_sequences
            if any(
                elem.audio_type == AudioType.BACKGROUND_MUSIC
                for elem in seq.audio_elements
            )
        )

        metrics["voice_coverage"] = (
            shots_with_voice / total_shots if total_shots > 0 else 0
        )
        metrics["sfx_coverage"] = shots_with_sfx / total_shots if total_shots > 0 else 0
        metrics["music_coverage"] = (
            shots_with_music / total_shots if total_shots > 0 else 0
        )
        ...
        avg_confidence = sum(
            ...
            for seq in plan.audio_sequences
            for elem in seq.audio_elements
        ) / max(1, sum(len(seq.audio_elements) for seq in plan.audio_sequences))

        metrics["average_confidence"] = avg_confidence
        metrics["overall_quality"] = avg_confidence * 0.7 + min(
            metrics.get("voice_coverage", 0) * 0.3, 1.0
        )

        return metrics

    def _save_audio_production_plan(
        self, project_path: Path, plan: AudioProductionPlan
    ) -> None:
        ...
        if project_file.exists():
            try:
                ...
                project_data["audio_production"] = {
                    ...
                    "total_elements": sum(
                        len(seq.audio_elements) for seq in plan.audio_sequences
                    ),
    ...
    def get_audio_preview_for_shot(self, shot_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mood = self._determine_shot_mood(
            shot_description, shot_data.get("purpose", "narrative")
        )
...
async def generate_audio_plan(
    project_path: Path, focus_shots: Optional[List[str]] = None
) -> AudioProductionPlan:
    """
