...
def wf(name, p, w, h, s=None):
    ...
    return {
        ...
        "2": {
            "c": "CLIPTextEncode",
            "i": {"text": "blur low watermark", "clip": ["3", 0]},
        },
        "3": {
            "c": "CLIPLoader",
            "i": {
        ...
        "5": {
            "c": "UNETLoader",
            "i": {"unet_name": "qwen_image_2512_fp8_e4m3fn.safetensors"},
        },
        "6": {
            "c": "EmptySD3LatentImage",
            "i": {"width": w, "height": h, "batch_size": 1},
        },
        ...
        "10": {
            "c": "SaveImage",
            "i": {"images": ["9", 0], "filename_prefix": f"SC_{name}"},
        },
    }
...
async def qq(session, wf):
    async with session.post(
        f"{URL}/prompt", json={"prompt": wf, "client_id": "sc"}
    ) as r:
        if r.status == 200:
