...
class QwenTTSComfyUIClient:
    ...
    def __init__(
        ...
        logger.info(
            f"QwenTTSComfyUIClient initialized with URL: {self.config.comfyui_url}"
        )
    ...
    async def _queue_prompt(self, workflow: Dict[str, Any]) -> str:
        ...
        async with session.post(
            f"{self.config.comfyui_url}/prompt", json=payload
        ) as response:
    ...
    async def _get_output_audio(
        ...
        if audio_info:
            ...
            if filename:
                ...
                async with session.get(
                    f"{self.config.comfyui_url}/view", params=params
                ) as response:
                    if response.status == 200:
        ...
        **kwargs,
        ...
        try:
            ...
            result = await self._wait_for_completion(
                prompt_id, timeout=training_timeout
            )
...
class QwenTTSComfyUIClientSync:
    ...
    def generate_custom_voice(self, *args, **kwargs) -> AudioResult:
        ...
        return loop.run_until_complete(
            self._client.generate_custom_voice(*args, **kwargs)
        )
