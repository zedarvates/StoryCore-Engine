...
class StyleTransferProcessor:
    ...
    async def apply_style(
        self, frame: VideoFrame, style_config: StyleConfig
    ) -> StyledFrame:
        """
        ...
        try:
            ...
            model_result = await self.model_manager.load_model(
                style_info.model_id, device="auto"
            )
            ...
            self.logger.debug(
                f"Successfully applied {style_config.style_type.value} style to frame {frame.frame_id}"
            )
            return styled_frame
    ...
    async def apply_style_sequence(
        self, frames: List[VideoFrame], style_config: StyleConfig
    ) -> List[StyledFrame]:
        """
        ...
        self.logger.info(
            f"Applying {style_config.style_type.value} style to sequence of {len(frames)} frames"
        )
        ...
        self.logger.info(
            f"Completed style transfer for sequence: {len(styled_frames)}/{len(frames)} frames successful"
        )
        return styled_frames

    async def _apply_style_transfer(
        self, frame: VideoFrame, style_config: StyleConfig, model: Any
    ...
    async def _apply_style_with_temporal_consistency(
        self,
        frame: VideoFrame,
        style_config: StyleConfig,
        sequence_id: str,
    ...
    def _calculate_frame_temporal_consistency(
        self, current_frame: StyledFrame, previous_frame: StyledFrame
    ) -> float:
        """
    ...
    async def _apply_temporal_smoothing(
        ...
        self.logger.debug(
            f"Applied temporal smoothing to frame {current_frame.original_frame.frame_id}"
        )
        ...
        for i in range(1, len(styled_frames)):
            if styled_frames[i].temporal_consistency_score is None:
                styled_frames[
                    ...
                    self._calculate_frame_temporal_consistency(
    ...
    async def _fallback_to_original(
        self, frame: VideoFrame, style_config: StyleConfig, error_message: str
    ) -> StyledFrame:
        """
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_frames"] > 0:
            stats["success_rate"] = (
                stats["successful_transfers"] / stats["total_frames"]
            )
