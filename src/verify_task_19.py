...
def test_save_failed_report():
    ...
    try:
        ...
        report_id_2 = storage.save_failed_report(
            create_test_payload(description="Second report")
        )
    ...
    try:
        ...
        for i in range(3):
            storage.save_failed_report(
                create_test_payload(description=f"Report {i + 1}")
            )
        ...
        print(
            f"  ✓ Stats: {stats['total_reports']} reports, {stats['total_size_bytes']} bytes"
        )
