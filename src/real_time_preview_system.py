    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                "memory_usage_mb": sum(
                    len(frame.image_data) for frame in self.cache.values()
                )
                / (1024 * 1024),
            }
...
class PreviewProcessor:
    ...
    def generate_preview_frame(
        ...
        try:
            ...
            if parameters.quality_level == PreviewQuality.WIREFRAME:
                ...
                frame_data = self._generate_preview_frame(
                    frame_index, parameters, source_frames
                )
                quality_score = 0.7
            elif parameters.quality_level == PreviewQuality.HIGH:
                # Higher quality preview
                time.sleep(processing_time * 0.3)
                frame_data = self._generate_high_quality_frame(
                    frame_index, parameters, source_frames
                )
                quality_score = 0.9
            else:  # FULL
                ...
                frame_data = self._generate_full_quality_frame(
                    frame_index, parameters, source_frames
                )
                quality_score = 1.0
    ...
    def _generate_wireframe(
        self, frame_index: int, parameters: PreviewParameters
    ) -> bytes:
    ...
    def _generate_high_quality_frame(
        self,
        frame_index: int,
        parameters: PreviewParameters,
        source_frames: List[bytes],
    ...
    def _generate_full_quality_frame(
        self,
        frame_index: int,
        parameters: PreviewParameters,
        source_frames: List[bytes],
...
class WebSocketHandler:
    ...
    async def _handle_join_session(self, connection_id: str, data: Dict[str, Any]):
        ...
        if session_id in self.preview_system.sessions:
            ...
            # Create new session
            session = self.preview_system.create_session(
                user_id, data.get("project_id", "default")
            )
    ...
    async def _handle_frame_request(self, connection_id: str, data: Dict[str, Any]):
        ...
        frame = await self.preview_system.generate_preview_frame_async(
...
class RealTimePreviewSystem:
    ...
    async def generate_preview_frame_async(
        self, session_id: str, frame_index: int
    ) -> Optional[PreviewFrame]:
    ...
    def _record_preview_metrics(self, frame: PreviewFrame):
        ...
        try:
            metrics = PerformanceMetrics(
                ...
                throughput=self.performance_stats["frames_generated"]
    ...
    async def start_websocket_server(self, host: str = "localhost", port: int = 8765):
        ...
        try:
            self.websocket_server = await websockets.serve(
                self.websocket_handler.handle_connection, host, port
            )
        ...
        return {
            "sessions": {
                ...
                "total_connections": sum(
                    s.active_connections for s in self.sessions.values()
                ),
            },
    ...
    def cleanup_inactive_sessions(self, timeout_minutes: int = 30):
        ...
        inactive_sessions = [
            ...
            and session.active_connections == 0
        ]
...
async def main():
    ...
    preview_system.update_session_parameters(
        session.session_id,
        {
    ...
    for i in range(5):
        ...
        if frame:
            print(
                f"   ✅ Frame {i}: {frame.processing_time:.3f}s, Quality: {frame.quality_score:.2f}"
            )
        else:
