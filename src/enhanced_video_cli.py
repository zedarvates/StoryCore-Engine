...
class EnhancedVideoCLI:
    ...
    def create_config_from_args(self, **kwargs) -> AdvancedVideoConfig:
        ...
        if target_resolution and isinstance(target_resolution, str):
            try:
                ...
                logger.warning(
                    f"Invalid target resolution '{target_resolution}', ignoring"
                )
    ...
    async def generate_video(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error_message": str(e), "execution_time": 0.0}
...
@click.option(
    "--min-quality-score", default=0.8, type=float, help="Minimum quality score"
)
@click.option(
    "--enable-quality-validation/--disable-quality-validation",
    default=True,
    help="Enable quality validation",
)
@click.option(
    "--enable-lightning/--disable-lightning",
    default=False,
    help="Enable lightning mode for faster generation",
)
@click.option(
    "--enable-alpha-channel/--disable-alpha-channel",
    default=False,
    help="Enable alpha channel support",
)
@click.option(
    "--transparency-threshold", default=0.5, type=float, help="Transparency threshold"
)
@click.option(
    "--enable-super-resolution/--disable-super-resolution",
    default=False,
    help="Enable super-resolution",
)
@click.option(
    "--target-resolution",
    help="Target resolution for super-resolution (e.g., 1664x960)",
)
@click.option(
    "--input-image", type=click.Path(exists=True), help="Input image for I2V generation"
)
@click.option(
    "--start-image", type=click.Path(exists=True), help="Start image for inpainting"
)
@click.option(
    "--end-image", type=click.Path(exists=True), help="End image for inpainting"
)
@click.option(
    "--enable-fallback/--disable-fallback",
    default=True,
    ...
    async def _generate():
        try:
            ...
            if result["success"]:
                ...
                click.echo(
                    f"   Workflow: {result['workflow_used']} ({result['workflow_type']})"
                )
                ...
                click.echo(
                    f"   Duration: {result['duration_seconds']:.2f}s ({result['num_frames']} frames)"
                )
                ...
                if kwargs.get("verbose"):
                    ...
                    click.echo(
                        f"   Processing Stages: {', '.join(result['processing_stages'])}"
                    )
                ...
                if workflows["advanced"]:
                    ...
                    for workflow_id in workflows["advanced"]:
                        ...
                        if verbose and workflow_id in workflows["details"]:
                            ...
                            click.echo(
                                f"    Capabilities: {', '.join([cap.value for cap in details['capabilities']])}"
                            )
                            click.echo(
                                f"    Resolutions: {', '.join(details['supported_resolutions'])}"
                            )
                            if "minimum_vram" in details["memory_requirements"]:
                                click.echo(
                                    f"    Min VRAM: {details['memory_requirements']['minimum_vram']} GB"
                                )
                    click.echo()
    ...
    async def _stats():
        try:
            ...
            if result["success"]:
                ...
                click.echo(
                    f"  Average Generation Time: {stats['average_generation_time']:.2f}s"
                )
                click.echo(
                    f"  Average Quality Score: {stats['average_quality_score']:.3f}"
                )
                click.echo()
                ...
                click.echo(
                    f"  Advanced Workflows: {stats['advanced_workflow_usage']} ({stats['advanced_usage_rate']:.1%})"
                )
                ...
                if verbose and stats.get("workflow_stats"):
                    ...
                    for workflow_id, workflow_stats in stats["workflow_stats"].items():
                        ...
                        click.echo(
                            f"    Average Time: {workflow_stats['average_time']:.2f}s"
                        )
                        click.echo(
                            f"    Average Quality: {workflow_stats['average_quality']:.3f}"
                        )
                    click.echo()
            else:
