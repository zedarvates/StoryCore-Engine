...
class FrameInterpolator:
    ...
    def __init__(self, config: InterpolationConfig):
        ...
        self.config = config
        logger.info(
            f"Frame interpolator initialized with algorithm: {config.algorithm}"
        )

    def interpolate_frames(
        self, keyframe_a: Dict[str, Any], keyframe_b: Dict[str, Any]
    ) -> InterpolationResult:
        """
        ...
        try:
            ...
            if self.config.algorithm == "linear":
                ...
                raise ValueError(
                    f"Unknown interpolation algorithm: {self.config.algorithm}"
                )
    ...
    def _validate_keyframes(
        ...
        for keyframe, name in [(keyframe_a, "keyframe_a"), (keyframe_b, "keyframe_b")]:
            ...
            if keyframe["width"] <= 0 or keyframe["height"] <= 0:
                raise ValueError(
                    f"Invalid dimensions in {name}: {keyframe['width']}x{keyframe['height']}"
                )
    ...
    def _linear_interpolation(
        ...
        for i in range(self.config.frame_count):
            ...
            frame = {
                ...
                "timestamp": keyframe_a["timestamp"]
                + alpha * (keyframe_b["timestamp"] - keyframe_a["timestamp"]),
                "interpolation_alpha": alpha,
            }

            # Interpolate quality score if available
            if "quality_score" in keyframe_a and "quality_score" in keyframe_b:
                frame["quality_score"] = (1 - alpha) * keyframe_a[
                    "quality_score"
                ] + alpha * keyframe_b["quality_score"]
            ...
            if (
                self.config.enable_character_preservation
                and "character_features" in keyframe_a
                and "character_features" in keyframe_b
            ):
            ...
            if (
                "lighting_conditions" in keyframe_a
                and "lighting_conditions" in keyframe_b
            ):
                ...
                for key in lighting_a:
                    if key in lighting_b:
                        if isinstance(lighting_a[key], (int, float)):
                            frame["lighting_conditions"][key] = (
                                1 - alpha
                            ) * lighting_a[key] + alpha * lighting_b[key]
                        else:
                            # For non-numeric values, use nearest neighbor
                            frame["lighting_conditions"][key] = (
                                lighting_a[key] if alpha < 0.5 else lighting_b[key]
                            )
    ...
    def _optical_flow_interpolation(
        ...
        for i, frame in enumerate(frames):
            ...
            if "quality_score" in frame:
                ...
                optical_flow_bonus = 0.05 * (
                    1 - abs(alpha - 0.5) * 2
                )  # Peak at middle frames
                frame["quality_score"] = min(
                    1.0, frame["quality_score"] + optical_flow_bonus
                )

            # Add motion blur simulation
            if self.config.motion_blur_strength > 0:
                motion_intensity = (
                    abs(alpha - 0.5) * 2 * self.config.motion_blur_strength
                )
                frame["motion_blur_intensity"] = motion_intensity

            # Enhanced character preservation
            if (
                "character_features" in frame
                and self.config.enable_character_preservation
            ):
                ...
                if i > 0 and i < len(frames) - 1:
                    ...
                    next_features = (
                        np.array(frames[i + 1]["character_features"])
                        if i + 1 < len(frames)
                        else features
                    )
    ...
    def _depth_aware_interpolation(
        ...
        for frame in frames:
            ...
            frame["depth_info"] = {
                ...
                "depth_quality": 0.8
                + 0.2 * (1 - abs(alpha - 0.5) * 2),  # Better quality in middle
            ...
            if "character_features" in frame and self.config.enable_depth_awareness:
                ...
                frame["character_features"] = (
                    features * depth_preservation_factor
                ).tolist()
    ...
    def _apply_post_processing(
        ...
        keyframe_a: Dict[str, Any],
        keyframe_b: Dict[str, Any],
    ) -> List[Dict[str, Any]]:
    ...
    def _apply_temporal_smoothing(
        ...
        next_value = (
            frames[index + 1].get(field, current_value)
        ...
        smoothed_value = (
            1 - smoothing_factor
        ) * current_value + smoothing_factor * 0.5 * (prev_value + next_value)
        ...
        if self.config.enable_character_preservation:
            ...
            for frame in frames:
                if "character_features" in frame:
                    ...
                    expected_error = (
                        abs(alpha - 0.5) * 0.1
                    )  # Error increases away from middle
