...
from .comfyui_models import PerformanceMetrics, ComfyUIWorkflow
...
class PerformanceMonitor:
    ...
    def record_operation_start(self, operation_name: str, operation_id: str) -> None:
        ...
        self.logger.debug(
            f"Started monitoring operation: {operation_name} ({operation_id})"
        )
    ...
    def record_operation_end(
        ...
        self.logger.debug(
            f"Completed monitoring operation: {operation_name} ({duration:.3f}s)"
        )
    ...
    def analyze_workflow_complexity(
        ...
        try:
            ...
            self.logger.info(
                f"Analyzed workflow {workflow.metadata.workflow_id}: "
    ...
    def _identify_bottleneck_nodes(self, nodes: Dict[str, Any]) -> List[str]:
        ...
        for node_id, node in nodes.items():
            ...
            if node_type in bottleneck_types:
                bottlenecks.append(
                    f"{node_id} ({node_type}): {bottleneck_types[node_type]}"
                )
    ...
    def _generate_optimization_suggestions(
        ...
        if node_count > 20:
            suggestions.append(
                "Consider simplifying workflow - high node count may impact performance"
            )

        # Memory suggestions
        if estimated_memory > 8000:  # > 8GB
            suggestions.append(
                "High memory usage detected - consider using smaller models or batch processing"
            )

        # Processing time suggestions
        if estimated_time > 120:  # > 2 minutes
            suggestions.append(
                "Long processing time estimated - consider optimizing sampling parameters"
            )

        # Node-specific suggestions
        sampler_count = sum(
            1 for node in nodes.values() if node.get("class_type") == "KSampler"
        )
        if sampler_count > 2:
            suggestions.append(
                "Multiple samplers detected - consider consolidating sampling operations"
            )

        controlnet_count = sum(
            1 for node in nodes.values() if "ControlNet" in node.get("class_type", "")
        )
        if controlnet_count > 3:
            suggestions.append(
                "Multiple ControlNets may cause memory pressure - consider selective usage"
            )

        # Connection complexity
        if connection_count > node_count * 2:
            suggestions.append(
                "High connection complexity - consider simplifying node relationships"
            )
    ...
    def _check_operation_thresholds(
    ...
    def get_performance_summary(
        ...
        if recent_snapshots:
            resource_stats = {
                ...
                "avg_memory_used_mb": sum(s.memory_used_mb for s in recent_snapshots)
                / len(recent_snapshots),
                "max_memory_used_mb": max(s.memory_used_mb for s in recent_snapshots),
                "avg_gpu_utilization": sum(
                    s.gpu_utilization_percent for s in recent_snapshots
                )
                / len(recent_snapshots),
                "max_gpu_utilization": max(
                    s.gpu_utilization_percent for s in recent_snapshots
                ),
                "avg_gpu_memory_used": sum(
                    s.gpu_memory_used_mb for s in recent_snapshots
                )
                / len(recent_snapshots),
                "max_gpu_memory_used": max(
                    s.gpu_memory_used_mb for s in recent_snapshots
                ),
            }
        ...
        if performance_stats:
            if performance_stats["avg_execution_time"] > 120:  # > 2 minutes
                recommendations.append(
                    "Consider reducing sampling steps or image resolution for faster execution"
                )

            if performance_stats["success_rate"] < 90:
                recommendations.append(
                    "Low success rate detected - check for resource constraints or model compatibility"
                )
    ...
    def _get_priority_actions(
        self, analysis: WorkflowComplexityAnalysis, performance_stats: Dict[str, Any]
    ) -> List[str]:
    ...
    def clear_metrics(self, older_than_hours: Optional[float] = None) -> None:
        ...
        if older_than_hours is None:
            ...
            self.logger.info(
                f"Cleared {cleared_metrics} metrics, {cleared_snapshots} snapshots, "
    ...
    def __del__(self):
        ...
        if self._monitoring_active:
            self.logger.warning(
                "Performance Monitor being destroyed with active monitoring"
            )
            try:
