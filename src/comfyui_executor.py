...
class ComfyUIExecutor:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(submit_url, json=payload, timeout=30) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        raise ComfyUIExecutionError(f"Failed to submit workflow to {server_name}: {error_text}")
                    
                    data = await response.json()
                    prompt_id = data.get("prompt_id")
                    
                    if not prompt_id:
                        raise ComfyUIExecutionError(f"No prompt_id returned from {server_name}")
    ...
    async def free_memory(self, server_name: str = "local", comfyui_url: Optional[str] = None):
        """Calls ComfyUI's /free endpoint to release GPU memory."""
        url = comfyui_url or get_comfyui_url(server_name=server_name)
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{url.rstrip('/')}/free", json={"unload_models": True, "free_memory": True}) as resp:
                    if resp.status == 200:
