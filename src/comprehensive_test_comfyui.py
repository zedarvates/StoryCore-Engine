...
async def run_image_test(
    runner: ComfyUITestRunner, prompt: str, test_num: int, total: int
) -> TestResult:
    ...
    result = await runner.run_image_generation_test(
        prompt=prompt, test_name=f"image_test_{test_num}"
    )
...
async def run_video_test(
    runner: ComfyUITestRunner, image_path: Path, prompt: str, test_num: int, total: int
) -> TestResult:
    ...
    result = await runner.run_video_generation_test(
        image_path=image_path, prompt=prompt, test_name=f"video_test_{test_num}"
    )
...
async def run_pipeline_test(
    ...
    result = await runner.run_pipeline_test(
        prompt=prompt, test_name=f"pipeline_test_{test_num}"
    )
...
async def run_comprehensive_tests(
    ...
    try:
        ...
        # Check health
        try:
            await runner.connection.check_health()
            ...
            for i, prompt in enumerate(prompts):
                ...
                try:
                    result = await run_pipeline_test(
                        runner, prompt, test_num, total_tests
                    )
                    results.append(result)
                except Exception as e:
...
Exit Codes:
  ...
  2 - Setup/configuration error
    ...
    parser.add_argument("--image", type=Path, help="Input image for video tests")
    ...
    prompts = (
        args.prompt
        if args.prompt
        else ["A beautiful landscape with mountains and a lake at sunset"]
    )
