...
class WorkflowExecutor:
    ...
    def convert_storycore_panel(
        self, panel_config: StoryCorePanelConfig
    ) -> ComfyUIWorkflow:
        """
        ...
        self.performance_monitor.record_operation_start(
            "workflow_conversion", operation_id
        )

        try:
            self.logger.info("Converting StoryCore panel to ComfyUI workflow")

            # Create new workflow
            workflow = ComfyUIWorkflow()
            workflow.metadata = WorkflowMetadata(
                source_type="storycore_panel", tags=["storycore", "panel_generation"]
            )
            ...
            self.logger.info(
                f"Workflow created with {len(workflow.nodes)} nodes, complexity: {workflow.metadata.complexity_score}"
            )

            # Analyze workflow complexity for performance insights
            complexity_analysis = self.performance_monitor.analyze_workflow_complexity(
                workflow
            )
            ...
            self.performance_monitor.record_operation_end(
                "workflow_conversion", operation_id, False, {"error": str(e)}
            )
            raise

    def _add_checkpoint_loader(
        self, workflow: ComfyUIWorkflow, config: StoryCorePanelConfig
    ) -> str:
    ...
    def _add_controlnet_nodes(
        ...
        last_conditioning = [
            "2",
            0,
        ]  # Initial positive conditioning from CLIPTextEncode
        ...
        for controlnet_config in config.controlnet_configs:
            ...
            loader_node = WorkflowNode(
                ...
                inputs={"control_net_name": controlnet_config.model_name},
            )
            ...
            image_node = WorkflowNode(
                ...
                inputs={
                    "image": str(controlnet_config.control_image_path.name)
                    if controlnet_config.control_image_path
                    else "control_image.jpg"
                },
            )
    ...
    def _add_ipadapter_nodes(
        ...
        loader_node = WorkflowNode(
            ...
            inputs={"ipadapter_file": ipadapter_config.model_name},
        )
        workflow.add_node(loader_node_id, loader_node)
        ...
        image_node = WorkflowNode(
            class_type="LoadImage",
            inputs={"image": str(ipadapter_config.reference_image_path.name)},
        )
        workflow.add_node(image_node_id, image_node)
    ...
    def _add_sampling_nodes(
        ...
        sampler_node = WorkflowNode(
            ...
            inputs={
                "seed": config.seed
                if config.seed >= 0
                else 42,  # Use fixed seed if negative
                "steps": config.steps,
    ...
    def _validate_node_connections(
        self, workflow: ComfyUIWorkflow, errors: List[str], warnings: List[str]
    ) -> None:
        ...
        for node_id, node in workflow.nodes.items():
            if node.class_type == "CheckpointLoaderSimple":
                ...
                if not ckpt_name.endswith((".safetensors", ".ckpt", ".pt")):
                    warnings.append(
                        f"Checkpoint {ckpt_name} may not be a valid model file"
                    )
    ...
    def get_available_models(self) -> Dict[str, List[str]]:
        ...
        models = {"checkpoints": [], "controlnets": [], "ipadapters": [], "vaes": []}

        # Check if ComfyUI installation exists
        if not self.config.installation_path.exists():
            self.logger.warning(
                f"ComfyUI installation not found at {self.config.installation_path}"
            )
        ...
        self._available_models = models
        self.logger.info(
            f"Found models: {sum(len(files) for files in models.values())} total"
        )
