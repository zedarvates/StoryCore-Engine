...
async def run_comprehensive_tests():
    ...
    for suite in test_suites:
        ...
        print(
            f"   ✅ {suite.name} ({suite.test_type.value}, {suite.priority.value} priority)"
        )
    ...
    try:
        ...
        for test_type, stats in test_types.items():
            ...
            print(
                f"   {status_icon} {test_type.title()}: {stats['passed']}/{stats['total']} ({success_rate:.1f}%)"
            )

        # Overall assessment
        overall_success = (
            summary["success_rate"] >= 90
            and quality["overall_quality_score"] >= config.quality_threshold
        )
        ...
        if overall_success:
            ...
            if summary["success_rate"] < 90:
                print(
                    f"❌ Test success rate below threshold: {summary['success_rate']:.1f}% < 90%"
                )
            if quality["overall_quality_score"] < config.quality_threshold:
                print(
                    f"❌ Quality score below threshold: {quality['overall_quality_score']:.1f}% < {config.quality_threshold}%"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Test Runner for Advanced ComfyUI Workflows"
    )
    parser.add_argument(
