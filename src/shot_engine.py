...
class ShotEngine:
    ...
    def process_shot_planning(self, project_path: Path) -> Dict[str, Any]:
        ...
        if not scene_breakdown:
            raise FileNotFoundError(
                "Scene breakdown not found. Run 'storycore scene-breakdown' first."
            )
        ...
        shot_planning_metadata = {
            ...
            "processing_metadata": {
                ...
                "average_shot_duration": self._calculate_average_shot_duration(
                    shot_lists
                ),
                "shot_variety_score": self._calculate_shot_variety(shot_lists),
                "camera_complexity_score": self._calculate_camera_complexity(
                    shot_lists
                ),
            },
        }
    ...
    def _create_scene_shot_list(
        self, scene: Dict[str, Any], start_shot_id: int
    ) -> List[Dict[str, Any]]:
        """Create detailed shot list for a scene."""
        scene["scene_purpose"]["primary"]
        ...
        if shot_strategy["type"] == "three_shot_standard":
            shots = self._create_three_shot_sequence(
                scene, start_shot_id, shot_strategy
            )
        elif shot_strategy["type"] == "establishing_heavy":
            shots = self._create_establishing_sequence(
                scene, start_shot_id, shot_strategy
            )
    ...
    def _determine_pacing_style(self, scene: Dict[str, Any]) -> str:
        ...
        pacing_map = {"slow": "contemplative", "medium": "standard", "fast": "dynamic"}
    ...
    def _create_three_shot_sequence(
        self, scene: Dict[str, Any], start_id: int, strategy: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _create_establishing_sequence(
        self, scene: Dict[str, Any], start_id: int, strategy: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _create_shot(
        ...
        shot = {
            ...
            "narrative_function": {
                ...
                "emotional_function": self._determine_emotional_function(
                    purpose, scene
                ),
            ...
            "lighting": self._adapt_lighting_for_shot(
                scene["lighting"], shot_type, angle
            ),
    ...
    def _calculate_shot_duration(
        self, base_duration: float, purpose: str, strategy: Dict[str, Any]
    ) -> float:
        ...
        pacing_modifiers = {"contemplative": 1.3, "standard": 1.0, "dynamic": 0.8}
    ...
    def _generate_shot_composition(
        self, shot_type: str, angle: str, scene: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _determine_shot_framing(self, shot_type: str) -> Dict[str, str]:
        ...
        return framing_map.get(
            shot_type, {"tightness": "medium", "breathing_room": "medium"}
        )
    ...
    def _adapt_lighting_for_shot(
        self, scene_lighting: Dict[str, Any], shot_type: str, angle: str
    ) -> Dict[str, Any]:
    ...
    def _determine_shot_characters(
        self, scene: Dict[str, Any], shot_type: str, purpose: str
    ) -> List[Dict[str, Any]]:
        ...
        for char in scene_characters:
            ...
            shot_character = {
                ...
                "positioning": self._determine_character_shot_positioning(
                    char, shot_type
                ),
    ...
    def _determine_character_shot_positioning(
        self, character: Dict[str, Any], shot_type: str
    ) -> Dict[str, str]:
    ...
    def _determine_performance_requirements(
        self, character: Dict[str, Any], purpose: str
    ) -> List[str]:
    ...
    def _apply_shot_timing(
        self, shots: List[Dict[str, Any]], total_duration: float
    ) -> List[Dict[str, Any]]:
        ...
        for shot in shots:
            ...
            shot["timing"]["end_time"] = (
                current_time + shot["timing"]["duration_seconds"]
            )
            current_time = shot["timing"]["end_time"]

        return shots

    def _apply_shot_transitions(
        self, shots: List[Dict[str, Any]], scene: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        for i, shot in enumerate(shots):
            if i == 0:
                ...
                shot["transition_in"] = self._determine_transition_type(
                    shots[i - 1], shot, scene
                )

            if i == len(shots) - 1:
                shot["transition_out"] = {"type": "scene_end", "duration": 0}
            else:
                shot["transition_out"] = self._determine_transition_type(
                    shot, shots[i + 1], scene
                )

        return shots

    def _determine_transition_type(
        self, shot_a: Dict[str, Any], shot_b: Dict[str, Any], scene: Dict[str, Any]
    ) -> Dict[str, str]:
        ...
        if pacing == "fast":
            ...
            abs(
                self._get_shot_size_value(shot_a_type)
                - self._get_shot_size_value(shot_b_type)
            )
            <= 1
        ):
    ...
    def _generate_cinematic_grammar(
        self, shot_lists: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _determine_cinematic_style(self, shot_lists: List[Dict[str, Any]]) -> str:
        ...
        movement_count = sum(
            1 for shot in shot_lists if shot["camera"]["movement"]["type"] != "static"
        )
        close_up_count = sum(
            1 for shot in shot_lists if shot["shot_type"]["code"] in ["CU", "ECU"]
        )
    ...
    def _assess_coverage_completeness(
        ...
        return {
            ...
            "coverage_score": len(shot_types.intersection(essential_types))
            / len(essential_types),
        }
    ...
    def _generate_camera_specifications(
        self, shot_lists: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _analyze_movement_requirements(
        self, shot_lists: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _analyze_lens_requirements(
        self, shot_lists: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _plan_shot_transitions(
        ...
        for i in range(len(shot_lists) - 1):
            ...
            transition = {
                ...
                "transition_type": current_shot.get("transition_out", {}).get(
                    "type", "cut"
                ),
                "duration": current_shot.get("transition_out", {}).get("duration", 0),
                "motivation": self._determine_transition_motivation(
    ...
    def _determine_transition_motivation(
        self, shot_a: Dict[str, Any], shot_b: Dict[str, Any]
    ) -> str:
    ...
    def _determine_overall_transition_style(
    ...
    def _generate_technical_requirements(
        ...
        return {
            ...
            "total_shots": len(shot_lists),
            "average_shot_length": total_duration / len(shot_lists)
            if shot_lists
            else 0,
            "aspect_ratio": "16:9",
            "resolution": "1920x1080",
    ...
    def _calculate_average_shot_duration(
        self, shot_lists: List[Dict[str, Any]]
    ) -> float:
    ...
    def _update_project_with_shots(
        self, project_path: Path, shot_metadata: Dict[str, Any]
    ) -> None:
        ...
        project_data["shot_planning_metadata"] = {
            ...
            "average_shot_duration": shot_metadata["processing_metadata"][
                "average_shot_duration"
            ],
            "shot_variety_score": shot_metadata["processing_metadata"][
                "shot_variety_score"
            ],
            "camera_complexity_score": shot_metadata["processing_metadata"][
                "camera_complexity_score"
            ],
        }
