...
from models.quality_models import (
    VoiceSegment as VoiceSegmentModel,
    AudioKeyframe as AudioKeyframeModel,
)
...
class AudioMixingEngine:
    ...
    def detect_voice_segments(
        ...
        for i in range(0, len(samples) - window_size, hop_size):
            ...
            voice_freq_mask = (freqs >= self.voice_freq_min) & (
        ...
        if current_segment_start is not None:
            ...
            avg_rms = (
                np.mean(current_segment_rms_values)
                if current_segment_rms_values
                else 0.0
            )
            segment_duration = segment_end - current_segment_start
            confidence = min(1.0, (avg_rms / 0.1) * (segment_duration / 1.0))
    ...
    def _merge_close_segments(
        self, segments: List[VoiceSegmentModel], gap_threshold: float = 0.2
    ) -> List[VoiceSegmentModel]:
        """
    ...
    def create_voice_music_mix(
        ...
        music_automated = self._apply_volume_automation(
            music_samples, keyframes, sample_rate
        )
    ...
    def _create_ducking_keyframes(
        ...
        for segment in voice_segments:
            ...
            keyframes.append(
                AudioKeyframeModel(
                    timestamp=fade_down_time, volume_db=0.0, curve_type="exponential"
                )
            )
            ...
            keyframes.append(
                AudioKeyframeModel(
                    timestamp=fade_up_time, volume_db=0.0, curve_type="exponential"
                )
            )
        ...
        if not keyframes or keyframes[-1].timestamp < total_duration:
            keyframes.append(
                AudioKeyframeModel(
                    timestamp=total_duration, volume_db=0.0, curve_type="linear"
                )
            )
    ...
    def _apply_volume_automation(
        self, samples: np.ndarray, keyframes: List[AudioKeyframeModel], sample_rate: int
    ) -> np.ndarray:
        """
    ...
    def _logarithmic_interpolation(
        self, start_value: float, end_value: float, num_points: int
    ) -> np.ndarray:
        """
    ...
    def check_interpolation_continuity(
        ...
        for i in range(len(sorted_keyframes) - 1):
            ...
            self._db_to_linear(kf_current.volume_db)
            self._db_to_linear(kf_next.volume_db)
        ...
        envelope = self._apply_volume_automation(test_audio, keyframes, sample_rate)
    ...
    def apply_crossfade(
        ...
        if samples_a is None or samples_b is None:
            return {"error": "Missing audio samples", "crossfaded_samples": None}
        ...
        # Apply fades and mix
        crossfade_region = (
            fade_region_a * fade_out_curve + fade_region_b * fade_in_curve
        )
        ...
        crossfaded_samples = np.concatenate([before_fade, crossfade_region, after_fade])
    ...
    def _create_crossfade_curves(
        self, num_samples: int, curve_type: str = "exponential"
    ...
    def create_crossfade_sequence(
        ...
        """
        if not clips:
            return {"error": "No clips provided", "sequenced_samples": None}
        ...
        for i in range(1, len(clips)):
            crossfade_result = self.apply_crossfade(
                result, clips[i], duration=crossfade_duration, curve=curve
            )
            ...
            for gap in gaps:
                if fill_method == "ambient":
                    filled_samples = self._fill_gap_with_ambient(
                        filled_samples, gap, sample_rate, ambient_level_db
                    )
                    gaps_filled += 1
                elif fill_method == "crossfade":
                    filled_samples = self._fill_gap_with_crossfade(
                        filled_samples, gap, sample_rate
                    )
                    gaps_filled += 1
    ...
    def _fill_gap_with_ambient(
        ...
        fade_length = min(
            int(0.01 * sample_rate), gap_length // 4
        )  # 10ms or 25% of gap
    ...
    def _fill_gap_with_crossfade(
        self, samples: np.ndarray, gap: Dict[str, Any], sample_rate: int
    ) -> np.ndarray:
        """
        ...
        crossfade = (
            before_audio[-crossfade_length:] * fade_out
