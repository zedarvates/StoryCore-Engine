...
class VideoProcessingEngine:
    ...
    def _initialize_sr_engine(self):
        ...
        try:
            ...
            try:
                ...
                self.sr_engine = SuperResolutionEngine(mm)
                logger.info(
                    "Super Resolution Engine initialized successfully (absolute import)."
                )
            except Exception:
                logger.warning(
                    "Super Resolution Engine not available, falling back to FFmpeg scaling. (%s)",
                    str(e),
                )
                self.sr_engine = None
    ...
    def _validate_ffmpeg(self):
        ...
        try:
            ...
            logger.warning(
                "FFmpeg not found at %s. Render operations will fail.", self.ffmpeg_path
            )
    ...
    def assemble(
        ...
        try:
            cmd = [
                ...
                concat_file,
                "-c",
                "copy",  # Try to copy without re-encoding first
                output_path,
            ]
    ...
    def apply_effects(
        self, input_path: str, output_path: str, effects: Dict[str, Any]
    ) -> bool:
        """
    ...
    def render(
        ...
        if use_ai_upscale and self.sr_engine:
            ...
            cmd = [
                ...
                input_path,
                "-vf",
                f"scale={width}:{height}:flags=lanczos,unsharp=5:5:1.0:5:5:0.0",
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
                ...
                "4.1",
                "-crf",
                "18",  # Higher quality for upscaled content
                "-c:a",
                "aac",
                "-b:a",
                "192k",
