...
class MediaIntelligenceEngine:
    ...
    async def index_project_assets(self, project_id: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Indexed {result['indexed_assets']} assets for project {project_id}"
        )
        return result
    ...
    async def search(
        ...
        try:
            ...
            return await self._keyword_fallback_search(
                query, project_id, asset_types, limit
            )
    ...
    async def _perform_search(
        ...
        if search_mode == SearchMode.SEMANTIC:
            return await self._semantic_search(
                query, project_id, asset_types, limit, threshold
            )
            ...
            return await self._similarity_search(
                query, project_id, asset_types, limit, threshold
            )
        else:  # HYBRID
    ...
    def _calculate_keyword_score(
        self, query_words: List[str], metadata: AssetMetadata
    ) -> float:
    ...
    def get_index_stats(self) -> IndexStats:
        ...
        asset_type_counts = {"image": 0, "video": 0, "audio": 0, "text": 0}
...
# Factory function
def create_media_intelligence_engine(
