...
def inject_storycore_parameters(
    workflow: Dict[str, Any], project_config: Dict[str, Any]
) -> Dict[str, Any]:
...
def create_panel_workflow(
    base_workflow: Dict[str, Any], panel_id: str, prompt: str, global_seed: int
) -> Dict[str, Any]:
...
def process_comfyui_outputs(
    outputs: Dict[str, Any], output_dir: str
) -> List[Dict[str, Any]]:
    ...
    for node_id, output_data in outputs.items():
        ...
        for i, image_info in enumerate(images):
            ...
            processed_output = {
                ...
                "metadata": {"generated_by": "comfyui", "node_id": node_id, "index": i},
            }
...
def create_batch_workflow(
    base_workflow: Dict[str, Any], panel_configs: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Create batch workflow for multiple panels with VRAM optimization"""
    if len(panel_configs) > 4:  # VRAM safety limit
        logger.warning(
            f"Batch size {len(panel_configs)} exceeds VRAM limit, reducing to 4"
        )
...
def optimize_workflow_for_memory(
    workflow: Dict[str, Any], available_vram_gb: float = 8.0
) -> Dict[str, Any]:
