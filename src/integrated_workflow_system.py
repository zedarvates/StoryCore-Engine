...
class IntegratedWorkflowSystem:
    ...
    async def _validate_request(self, request: WorkflowRequest) -> Dict[str, Any]:
        ...
        validation_request = {"workflow_type": request.workflow_type}
        ...
        is_valid, results = self.security.validate_workflow_request(
            validation_request, request.user_id
        )
        ...
        return {"is_valid": True, "message": "Validation passed", "results": results}

    async def _execute_with_resilience(
        self, handler: Callable, request: WorkflowRequest
    ) -> WorkflowResult:
        ...
        if fallback_chain_name in self.resilience.fallback_chains:
            ...
            try:
                result = await self.resilience.fallback_chains[
                    fallback_chain_name
                ].execute(request)
                fallback_used = True
            except Exception:
        ...
        return {
            ...
            "execution_summary": {
                ...
                "resilience_recovery_rate": (
                    ...
                    if self.execution_stats["failed_requests"] > 0
                    else 0
                ),
            },
            "timestamp": datetime.now().isoformat(),
    ...
    def setup_video_workflow(self, video_handler: Callable):
        ...
        self.resilience.get_fallback_chain("fallback_advanced_video")
    ...
    def setup_image_workflow(self, image_handler: Callable):
        ...
        # Create fallback chain
        self.resilience.get_fallback_chain("fallback_advanced_image")
