...
class SceneBreakdownEngine:
    ...
    def process_scene_breakdown(self, project_path: Path) -> Dict[str, Any]:
        ...
        if not script_metadata:
            raise FileNotFoundError(
                "Script metadata not found. Run 'storycore script' first."
            )
        ...
        scene_breakdown_metadata = {
            ...
            "global_cinematic_rules": self._generate_global_cinematic_rules(
                detailed_scenes
            ),
            ...
            "processing_metadata": {
                ...
                "average_scene_complexity": self._calculate_average_complexity(
                    detailed_scenes
                ),
    ...
    def _create_detailed_scene_breakdown(
        self,
        scene: Dict[str, Any],
        characters: List[Dict],
        emotional_arcs: List[Dict],
        style_anchors: Dict[str, Any],
    ) -> Dict[str, Any]:
        ...
        composition_requirements = self._generate_composition_requirements(
        ...
        detailed_scene = {
            ...
            "location_details": self._generate_location_details(
                scene, environment_analysis
            ),
            "atmospheric_requirements": self._generate_atmospheric_requirements(scene),
            # Lighting Analysis
            "lighting": lighting_setup,
            "lighting_motivation": self._determine_lighting_motivation(
                scene, lighting_setup
            ),
            "shadow_requirements": self._generate_shadow_requirements(lighting_setup),
            # Character Analysis
            "characters": scene_characters,
            "character_interactions": self._analyze_character_interactions(
                scene, scene_characters
            ),
            "character_emotional_states": self._determine_character_emotions(
                scene, scene_characters
            ),
            # Key Beats
            "key_beats": scene_beats,
            "beat_timing": self._calculate_beat_timing(
                scene_beats, scene["duration_seconds"]
            ),
            ...
            "visual_style": self._generate_visual_style_requirements(
                scene, style_anchors
            ),
            ...
            "continuity": {
                ...
                "character_continuity": self._generate_character_continuity(
                    scene_characters
                ),
                "lighting_continuity": self._generate_lighting_continuity(
                    lighting_setup
                ),
    ...
    def _determine_scene_purpose(self, scene: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "cinematic_function": self._map_purpose_to_cinematic_function(
                primary_purpose
            ),
    ...
    def _determine_pacing_requirements(self, purpose: str) -> Dict[str, Any]:
        ...
        return pacing_map.get(
            purpose, {"rhythm": "medium", "cuts": "standard", "movement": "natural"}
        )
    ...
    def _analyze_environment(self, scene: Dict[str, Any]) -> Dict[str, Any]:
        ...
        env_type = self.environment_types.get(
            location, self.environment_types["exterior"]
        )
        ...
        environment = {
            ...
            "atmospheric_perspective": self._determine_atmospheric_perspective(
                location
            ),
    ...
    def _generate_environmental_storytelling(self, scene: Dict[str, Any]) -> List[str]:
        ...
        if emotional_tone == "tense" and location == "urban":
            storytelling_elements.extend(
                ["harsh_shadows", "stark_contrasts", "angular_architecture"]
            )
        elif emotional_tone == "peaceful" and location == "nature":
            storytelling_elements.extend(
                ["soft_textures", "organic_shapes", "harmonious_colors"]
            )
        elif emotional_tone == "melancholic":
            storytelling_elements.extend(
                ["muted_colors", "empty_spaces", "distant_elements"]
            )
    ...
    def _determine_lighting_setup(self, scene: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_lighting = self.lighting_setups.get(
            time_of_day, self.lighting_setups["afternoon"]
        )

        # Apply emotional modifications
        self.emotional_visual_mapping.get(
            emotional_tone, self.emotional_visual_mapping["neutral"]
        )
        ...
        lighting_setup = {
            ...
            "shadows": {
                ...
                "color_temperature": self._determine_shadow_temperature(
                    base_lighting["temperature"]
                ),
            },
            "ambient_light": {
            ...
            "lighting_motivation": self._determine_lighting_motivation_detailed(
                scene, base_lighting
            ),
            "cinematic_purpose": self._determine_lighting_cinematic_purpose(
                emotional_tone
            ),
        }
    ...
    def _generate_secondary_lights(
        self, location: str, time_of_day: str
    ) -> List[Dict[str, str]]:
        ...
        if location == "urban":
            if time_of_day == "night":
                ...
                secondary_lights.append(
                    {"type": "reflected_light", "direction": "up", "color": "neutral"}
                )
    ...
    def _determine_lighting_motivation_detailed(
        self, scene: Dict[str, Any], base_lighting: Dict[str, str]
    ) -> str:
    ...
    def _identify_scene_characters(
        self, scene: Dict[str, Any], characters: List[Dict]
    ) -> List[Dict[str, Any]]:
        ...
        for char_name in characters_present:
            ...
            if character_data:
                scene_character = {
                    ...
                    "role_in_scene": self._determine_character_role_in_scene(
                        scene, character_data
                    ),
                    "visual_requirements": character_data["visual_attributes"],
                    "color_palette": character_data["color_palette"],
                    "emotional_state": self._determine_character_emotional_state(
                        scene, character_data
                    ),
                    "positioning": self._determine_character_positioning(
                        scene, character_data
                    ),
                    "interaction_requirements": self._determine_character_interactions(
                        scene, character_data
                    ),
                }
    ...
    def _determine_character_role_in_scene(
        self, scene: Dict[str, Any], character: Dict[str, Any]
    ) -> str:
    ...
    def _determine_character_emotional_state(
        self, scene: Dict[str, Any], character: Dict[str, Any]
    ) -> str:
    ...
    def _determine_character_positioning(
        ...
        return {
            ...
            "movement_type": "purposeful"
            if scene["emotional_tone"] == "exciting"
            else "natural",
        }

    def _determine_character_interactions(
        self, scene: Dict[str, Any], character: Dict[str, Any]
    ) -> List[str]:
        ...
        if len(key_actions) > 0:
            ...
            for i, action in enumerate(key_actions):
                beat = {
                    ...
                    "intensity": self._determine_beat_intensity(
                        action, scene["emotional_tone"]
                    ),
    ...
    def _determine_beat_intensity(self, action: str, emotional_tone: str) -> str:
        ...
        if any(
            intense_action in action.lower()
            for intense_action in high_intensity_actions
        ):
            return "high"
        elif any(
            calm_action in action.lower() for calm_action in low_intensity_actions
        ):
            return "low"
    ...
    def _calculate_beat_timing(
        self, beats: List[Dict], total_duration: int
    ) -> Dict[str, Any]:
    ...
    def _determine_dramatic_arc(self, intensity_scores: List[int]) -> str:
        ...
        avg_first_half = sum(intensity_scores[: len(intensity_scores) // 2]) / (
            len(intensity_scores) // 2
        )
        avg_second_half = sum(intensity_scores[len(intensity_scores) // 2 :]) / (
            len(intensity_scores) - len(intensity_scores) // 2
        )
    ...
    def _generate_composition_requirements(
        self, scene: Dict[str, Any], lighting_setup: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Generate visual composition requirements."""
        emotional_tone = scene["emotional_tone"]
        location = scene.get("location", "unspecified")
    ...
    def _determine_depth_of_field(self, scene: Dict[str, Any]) -> Dict[str, str]:
        ...
        if emotional_tone in ["tense", "exciting"]:
            ...
            return {
                "type": "medium",
                "focus_point": "balanced",
                "background": "moderate_blur",
            }
    ...
    def _generate_scene_color_palette(
        self, scene: Dict[str, Any], style_anchors: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_visual_style_requirements(
        self, scene: Dict[str, Any], style_anchors: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_character_continuity(
        self, scene_characters: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _generate_lighting_continuity(
        self, lighting_setup: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_global_cinematic_rules(
        self, detailed_scenes: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _generate_continuity_requirements(
        self, detailed_scenes: List[Dict]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "environmental_continuity": self._analyze_environmental_continuity(
                detailed_scenes
            ),
        }
    ...
    def _analyze_lighting_continuity(self, scenes: List[Dict]) -> Dict[str, Any]:
        ...
        lighting_directions = [
            scene["lighting"]["primary_light"]["direction"] for scene in scenes
        ]
        lighting_temperatures = [
            scene["lighting"]["primary_light"]["temperature"] for scene in scenes
        ]
    ...
    def _generate_technical_specifications(
        ...
        total_duration = sum(
            scene["technical_specs"]["duration_seconds"] for scene in detailed_scenes
        )
    ...
    def _calculate_average_complexity(self, scenes: List[Dict]) -> float:
        ...
        for scene in scenes:
            ...
            score += (
                0.3
                if scene["scene_purpose"]["primary"] in ["conflict", "action"]
                else 0
            )
    ...
    def _map_scene_to_emotional_arc(
        self, scene: Dict[str, Any], emotional_arcs: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _determine_narrative_function(
        self, scene: Dict[str, Any], scene_purpose: Dict[str, Any]
    ) -> str:
    ...
    def _analyze_character_interactions(
        self, scene: Dict[str, Any], scene_characters: List[Dict]
    ) -> List[Dict[str, Any]]:
            ...
            for i, char1 in enumerate(scene_characters):
                for char2 in scene_characters[i + 1 :]:
                    interaction = {
                        ...
                        "visual_relationship": self._determine_visual_relationship(
                            char1, char2
                        ),
                        "emotional_dynamic": self._determine_emotional_dynamic(
                            scene, char1, char2
                        ),
                    }
    ...
    def _determine_interaction_type(
        self, scene: Dict[str, Any], char1: Dict, char2: Dict
    ) -> str:
    ...
    def _update_project_with_breakdown(
        self, project_path: Path, breakdown_metadata: Dict[str, Any]
    ) -> None:
        ...
        project_data["scene_breakdown_metadata"] = {
            ...
            "total_scenes_processed": breakdown_metadata["processing_metadata"][
                "total_scenes_processed"
            ],
            "average_complexity": breakdown_metadata["processing_metadata"][
                "average_scene_complexity"
            ],
            "lighting_consistency": breakdown_metadata["processing_metadata"][
                "lighting_consistency_score"
            ],
            "color_harmony": breakdown_metadata["processing_metadata"][
                "color_harmony_score"
            ],
        }
    ...
    def _generate_location_details(
        self, scene: Dict[str, Any], environment_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_atmospheric_requirements(
        ...
        if emotional_tone == "tense":
            atmosphere["atmospheric_effects"].extend(
                ["dramatic_shadows", "high_contrast"]
            )
        elif emotional_tone == "peaceful":
    ...
    def _determine_lighting_motivation(
        self, scene: Dict[str, Any], lighting_setup: Dict[str, Any]
    ) -> str:
    ...
    def _generate_shadow_requirements(
        self, lighting_setup: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Generate shadow requirements based on lighting setup."""
        lighting_setup["primary_light"]
