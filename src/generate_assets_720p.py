#!/usr/bin/env python3
"""
StoryCore Base Asset Generator - 720p Test Version
Low resolution for system compatibility
"""
...
ASSETS = [
    ('storycore_logo_horizontal', 'Professional logo for StoryCore text, modern typography, minimal style', 1280, 360),
    ('storycore_logo_square', 'Square app icon with SC monogram, modern minimal design', 512, 512),
    ('storycore_panel_placeholder', 'Storyboard panel placeholder, neutral gray background, grid', 854, 480),
    ...
    ('storycore_banner_featured', 'Featured banner, dark gradient background', 1280, 360),
]


def create_sdxl_workflow(name, prompt, width, height, seed=None):
    """Create SDXL workflow (compatible with your models)"""
    if seed is None:
    ...
    return {
        ...
        "4": {
            "class_type": "CLIPTextEncode", 
            "inputs": {"clip": ["5", 1], "text": "blurry, low quality, distorted, ugly, watermark"}
        },
        "5": {
            "class_type": "CLIPLoader",
            "inputs": {"clip_name": "mistral_3_small_flux2_bf16.safetensors", "type": "sdxl"}
        },
        "6": {
        ...
        "7": {
            ...
            "inputs": {"model_name": "jina_clip_v2_bf16.safetensors", "weight_dtype": "default"}
        },
        "8": {
        ...
        "11": {
            ...
            "inputs": {"images": ["10", 0], "filename_prefix": f"StoryCore_{name}"}
        }
    }
...
async def generate(session, name, prompt, w, h):
    ...
    try:
        wf = create_sdxl_workflow(name, prompt, w, h)
...
async def main():
    print('=' * 60)
    print('StoryCore Asset Generator - 720p Test')
    print('=' * 60)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print('')
        print(f'Generating {len(ASSETS)} assets (720p max)...')
        results = []
