...
class DiscussionManager:
    ...
    def record_conversation(
        self, conversation: Conversation, session_id: Optional[str] = None
    ) -> Path:
        """
    ...
    def _append_conversation_to_file(
        self, file_path: Path, conversation: Conversation
    ) -> None:
        """
    ...
    def should_summarize(
        self, discussion_file: Optional[Path] = None, threshold_kb: int = 50
    ) -> bool:
        """
    ...
    def create_summary(
        self, discussion_file: Optional[Path] = None, session_id: Optional[str] = None
    ) -> Optional[Path]:
        """
    ...
    def _generate_summary_content(self, discussions: List[Dict[str, Any]]) -> str:
        ...
        for i, discussion in enumerate(discussions, 1):
            ...
            summary_lines.append(
                f"Start Time: {discussion.get('start_time', 'Unknown')}"
            )
            summary_lines.append("")
    ...
    def extract_key_information(self, discussion_text: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if any(
                ...
                for keyword in ["decision:", "decided:", "we decided", "agreed to"]
            ):
            ...
            if any(
                keyword in line_lower
                for keyword in ["action:", "will:", "todo:", "to do", "action item"]
            ):
            ...
            if any(
                keyword in line_lower for keyword in ["objective:", "goal:", "aim:"]
            ):
    ...
    def get_discussion_history(self, limit: int = 10) -> List[Dict[str, Any]]:
        """
