...
class AssetManager:
    ...
    def store_asset(
        self, asset_path: Path, asset_type: AssetType, description: str = ""
    ) -> Optional[AssetInfo]:
        """
    ...
    def _create_asset_info(
        self, filename: str, path: Path, asset_type: AssetType, description: str = ""
    ) -> AssetInfo:
    ...
    def _format_index_entry(self, asset_info: AssetInfo) -> str:
        ...
        if asset_info.metadata:
            if asset_info.metadata.dimensions:
                lines.append(
                    f"Dimensions: {asset_info.metadata.dimensions[0]}x{asset_info.metadata.dimensions[1]}"
                )
    ...
    def _generate_summary_content(
        self, assets_by_type: Dict[AssetType, List[AssetInfo]]
    ) -> str:
            ...
            for asset in assets:
                ...
                if asset.metadata:
                    if asset.metadata.dimensions:
                        lines.append(
                            f"  Dimensions: {asset.metadata.dimensions[0]}x{asset.metadata.dimensions[1]}"
                        )
