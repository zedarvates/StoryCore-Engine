...
class AutoQASystem:
    ...
    def check_summary_quality(self, original: str, summary: str) -> Dict[str, Any]:
        """
    ...
    def check_memory_consistency(self) -> Dict[str, Any]:
                ...
                "issues": [
                    {
                        "type": "no_memory_file",
                        "severity": "critical",
                        "description": "memory.json not found",
                    }
                ],
                "auto_fixable": False,
            }
        ...
        [d.description for d in memory.decisions]
        ...
        self.build_logger.get_recent_actions(10)
    ...
    def generate_qa_report(self) -> QAReport:
        ...
        overall_score = (
            (checks_passed / checks_performed * 100) if checks_performed > 0 else 0
        )
        ...
        for issue_type, count in issue_counts.items():
            if issue_type == "duplicate_entities":
                recommendations.append(
                    "Consider consolidating duplicate entities or renaming them for clarity"
                )
            elif issue_type == "indexed_files_missing":
                recommendations.append(
                    "Rebuild asset index to remove references to missing files"
                )
            elif issue_type == "unindexed_files":
                recommendations.append(
                    "Run asset indexing to include all project files"
                )
            elif issue_type == "non_chronological_timestamps":
                recommendations.append(
                    "Review and correct timestamps in memory.json for chronological ordering"
                )
            elif issue_type == "stale_state":
                recommendations.append(
                    "Update current_state to reflect recent project activity"
                )
            elif issue_type == "compression_too_high":
                recommendations.append(
                    "Summaries may be too brief - consider including more context"
                )
            elif issue_type == "compression_too_low":
                recommendations.append(
                    "Summaries may be too long - consider more aggressive compression"
                )
    ...
    def auto_fix_issues(
        self, issues: Optional[List[Dict[str, Any]]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for issue in issues:
            ...
            if issue_type == "duplicate_entities":
                ...
                for asset in self.asset_manager.get_all_assets():
                    ...
                    with open(
                        self.asset_manager.index_path, "a", encoding="utf-8"
                    ) as f:
        ...
        return {"fixed": fixed, "failed": failed, "total": len(issues)}
