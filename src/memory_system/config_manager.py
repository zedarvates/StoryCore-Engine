...
class ConfigManager:
        ...
        try:
            ...
            for key, value in updates.items():
                if key == "project_name":
                    ...
                    self._update_memory_system_config(
                        config.memory_system_config, value
                    )
                else:
    ...
    def _update_memory_system_config(
        self, config: MemorySystemConfig, updates: Dict[str, Any]
    ) -> None:
        """
        ...
        for key, value in updates.items():
            if key == "auto_summarize":
                ...
                if not isinstance(value, int) or value < 1:
                    raise ValueError(
                        "summarization_threshold_kb must be a positive integer"
                    )
