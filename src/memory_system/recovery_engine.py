...
class RecoveryEngine:
    ...
    def repair_missing_file(self, error: Error) -> RepairResult:
        ...
        self.build_logger.log_recovery_attempt(
            error_id=error.id, action_taken="repair_missing_file", success=True
        )

        return RepairResult(
            success=True, reason="All missing files repaired", actions_taken=actions
        )
    ...
    def _recreate_config_from_logs(self, file_path: Path) -> bool:
        ...
        try:
            ...
            self.build_logger.get_recent_actions(100)
        ...
        try:
            ...
            for action in recent_actions:
                if action.action_type == "LLM_DECISION":
                    ...
                    action.parameters.get("update_type", "")
    ...
    def repair_invalid_json(self, error: Error) -> RepairResult:
        ...
        # Log recovery attempt
        self.build_logger.log_recovery_attempt(
            error_id=error.id, action_taken="repair_invalid_json", success=True
        )
    ...
    def repair_inconsistent_state(self, error: Error) -> RepairResult:
        ...
        # Log recovery attempt
        self.build_logger.log_recovery_attempt(
            error_id=error.id, action_taken="repair_inconsistent_state", success=True
        )

        return RepairResult(
            success=True, reason="State inconsistencies resolved", actions_taken=actions
        )
    ...
    def repair_corrupted_data(self, error: Error) -> RepairResult:
        ...
        # Log recovery attempt
        self.build_logger.log_recovery_attempt(
            error_id=error.id, action_taken="repair_corrupted_data", success=True
        )

        return RepairResult(
            success=True, reason="Corrupted data repaired", actions_taken=actions
        )
    ...
    def desperate_recovery(self) -> RecoveryReport:
        ...
        if self.reconstitute_memory(history):
            ...
            recommendations.append(
                "Memory may be incomplete - consider reviewing project objectives"
            )
        ...
        for file_path, verified in verification_results.items():
            if verified:
                confidence_scores[str(file_path)] = (
                    confidence_scores.get(str(file_path), 0.5) + 0.3
                )
            else:
    ...
    def update_error_status(
        self, error_id: str, status: str, recovery_attempts: int = 0
    ) -> bool:
        """
