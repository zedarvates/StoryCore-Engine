...
class MemorySystemCore:
    ...
    def initialize_project(
        ...
        try:
            ...
            # Initialize files
            if not self.directory_manager.initialize_files(
    ...
    def _create_conversation(
        ...
        if session_id is None:
            session_id = (
                f"session_{start_time.strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"
            )

        return Conversation(
            messages=message_objects, session_id=session_id, start_time=start_time
        )
            ...
            if key_info["decisions"]:
                for decision in key_info["decisions"]:
                    self.memory_manager.add_decision(
                        description=decision, rationale="Extracted from discussion"
                    )
    ...
    def add_asset(
        self, asset_path: Path, asset_type: str, description: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            asset_info = self.asset_manager.store_asset(
                Path(asset_path), type_enum, description
            )
            ...
            # Log the action
            self.build_logger.log_asset_addition(
                asset_info, triggered_by="memory_system_core"
            )

            # Record in timeline
            self.timeline_generator.record_asset_addition(
                asset_name=asset_info.filename, asset_type=asset_type
            )
    ...
    def add_memory_entity(
        ...
        return self.memory_manager.add_entity(
            name, entity_type, description, attributes
        )

    def add_memory_decision(
        self, description: str, rationale: str, alternatives: Optional[List[str]] = None
    ) -> bool:
    ...
    def trigger_recovery(
        self, recovery_type: RecoveryType = RecoveryType.AUTOMATIC
    ) -> RecoveryReport:
        """
        ...
        try:
            if recovery_type == RecoveryType.DESPERATE:
                ...
                return RecoveryReport(
                    ...
                    recommendations=[
                        "Manual intervention may be required for failed repairs"
                    ]
                    if failed
                    else [],
                    timestamp=datetime.now().isoformat(),
            ...
            return RecoveryReport(
                success=False, errors=[str(e)], timestamp=datetime.now().isoformat()
            )
        ...
        try:
            ...
            if context:
                context_dict = {
                    ...
                    if hasattr(context.config, "to_dict")
                    else {},
                    "memory": context.memory.to_dict()
                    if hasattr(context.memory, "to_dict")
                    else {},
                }
    ...
    def clean_logs(self) -> bool:
        ...
        self.log_processor.translate_logs("fr")
    ...
    def set_variable(self, name: str, value: Any, description: str = "") -> bool:
