...
class KnowledgeGraphManager:
    ...
    def add_node(
        self, name: str, node_type: str, attributes: Dict[str, Any] = None
    ) -> str:
    ...
    def find_node_by_name(
        self, name: str, node_type: str = None
    ) -> Optional[EntityNode]:
    ...
    def get_context_summary(self, entity_name: str) -> str:
        ...
        for rel in related:
            ...
            if direction == "outgoing":
                summary.append(
                    f"- {rel_type} {other_node['name']} ({other_node['type']})"
                )
            else:
                summary.append(
                    f"- {other_node['name']} ({other_node['type']}) recorded as {rel_type} {entity_name}"
                )
    ...
    def get_emotional_landscape(self, entity_name: str) -> str:
        ...
        for rel in emotional_rels:
            ...
            strength_desc = (
                "mildly" if strength < 0.4 else "strongly" if strength > 0.7 else ""
            )
            ...
            if direction == "outgoing":
                ...
                summary.append(
                    f"- {other_name} {strength_desc} {r_type} {entity_name} (Incoming)"
                )
