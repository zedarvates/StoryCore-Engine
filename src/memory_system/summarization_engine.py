...
class SummarizationEngine:
    ...
    def summarize_discussion(self, discussion_text: str, max_points: int = 10) -> str:
        """
        ...
        for line in lines:
            ...
            if any(
                kw in line_lower
                for kw in ["decision:", "decided:", "we decided", "agreed to"]
            ):
            ...
            if any(
                kw in line_lower
                for kw in ["action:", "will:", "todo:", "to do", "need to"]
            ):
                content = self._extract_content(line)
                if content:
            ...
            if any(
                kw in line_lower
                for kw in ["constraint:", "must", "required to", "cannot"]
            ):
                content = self._extract_content(line)
                if content:
    ...
    def synthesize_information(self, sources: List[str]) -> str:
        ...
        other = [
            ...
            if not any(
                p.startswith(prefix)
                for prefix in ["DECISION:", "ACTION:", "IMPORTANT:"]
            )
        ]
    ...
    def validate_summary_quality(
        ...
        if ratio > max_ratio:
            issues.append(
                f"Summary too long (ratio: {ratio:.2%}, max: {max_ratio:.2%})"
            )
        elif ratio < min_ratio:
            issues.append(
                f"Summary too short (ratio: {ratio:.2%}, min: {min_ratio:.2%})"
            )
        ...
        if preservation_ratio < 0.5:
            issues.append(
                f"Low key information preservation ({preservation_ratio:.0%})"
            )
