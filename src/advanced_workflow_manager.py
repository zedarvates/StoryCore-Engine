...
class AdvancedWorkflowManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_manager = (
            AdvancedWorkflowConfigManager(config_path)
            if config_path
            else get_global_config()
        )
    ...
    async def initialize(
        self, workflow_search_paths: Optional[List[str]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if workflow_search_paths:
                discovered_count = self.registry.discover_workflows(
            ...
            if not config_validation["is_valid"]:
                self.logger.warning(
                    f"Configuration issues found: {config_validation['issues']}"
                )
            ...
            if not registry_validation["is_valid"]:
                self.logger.warning(
                    f"Registry issues found: {registry_validation['issues']}"
                )
            ...
            self.logger.error(
                f"Failed to initialize Advanced Workflow Manager: {str(e)}"
            )
            return False
        ...
        try:
            ...
            if not is_valid:
                raise WorkflowValidationError(
                    routing_decision.selected_workflow, validation_error, request
                )

            # Load models if needed
            if not workflow.is_loaded:
                self.logger.info(
                    f"Loading models for {routing_decision.selected_workflow}"
                )
    ...
    def _calculate_quality_score(self, result: WorkflowResult) -> float:
        ...
        if result.quality_metrics:
            ...
            scores = [
                score
                for score in result.quality_metrics.values()
    ...
    def get_workflow_capabilities(
        self, category: str, workflow_name: str
    ...
    def get_performance_analytics(self) -> Dict[str, Any]:
        ...
        execution_times = [
            record["execution_time"] for record in self.performance_history
        ]
        ...
        for workflow_name, stats in workflow_stats.items():
            ...
            stats["avg_memory_usage"] = (
                stats["total_memory"] / stats["executions"]
        ...
        try:
            ...
            if len(self.active_executions) > 10:  # Arbitrary threshold
                health_status["warnings"].append(
                    f"High number of active executions: {len(self.active_executions)}"
                )

            # Check success rate
            if self.stats["total_requests"] > 10:  # Only check if we have enough data
                success_rate = (
                    self.stats["successful_executions"] / self.stats["total_requests"]
                )
                if success_rate < 0.8:
                    health_status["warnings"].append(
