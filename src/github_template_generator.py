    ...
    def format_issue_body(self, payload: Dict[str, Any]) -> str:
        ...
        try:
            ...
            template_parts.append(
                f"- **StoryCore Version:** {system_info.get('storycore_version', 'unknown')}"
            )
            template_parts.append(
                f"- **Active Module:** {module_context.get('active_module', 'unknown')}"
            )
            template_parts.append(
                f"- **OS Platform:** {system_info.get('os_platform', 'unknown')} {system_info.get('os_version', '')}"
            )
            template_parts.append(
                f"- **Python Version:** {system_info.get('python_version', 'unknown')}"
            )
            template_parts.append(
                f"- **Language:** {system_info.get('language', 'en_US')}\n"
            )
            ...
            if logs:
                template_parts.append("<details>")
                template_parts.append(
                    "<summary>Application Logs (Last 500 lines)</summary>\n"
                )
                template_parts.append("```")
                template_parts.append("\n".join(logs))
                template_parts.append("```")
