...
try:
    ...
    logger.warning(
        "Circuit breaker not available - monitoring operations may be vulnerable to blocking"
    )
...
class ResourceMonitor:
    ...
    def get_current_resources(self) -> SystemResources:
        ...
        if CIRCUIT_BREAKER_AVAILABLE and self.monitoring_breaker:
            try:
                return self.monitoring_breaker.call(
                    self._get_current_resources_protected
                )
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                logger.warning(
                    f"Resource monitoring circuit breaker open (failure {consecutive_failures})"
                )
                ...
                consecutive_failures += 1
                logger.error(
                    f"Resource monitoring error (failure {consecutive_failures}): {e}"
                )

                # If too many consecutive failures, stop monitoring to prevent infinite loops
                if consecutive_failures >= max_consecutive_failures:
                    logger.error(
                        f"Too many consecutive monitoring failures ({consecutive_failures}), stopping monitoring"
                    )
                    self.monitoring = False
                    break
...
class ProgressTracker:
    ...
    def start_operation(
        self, operation_id: str, operation_name: str, total_frames: int
    ) -> ProgressInfo:
        ...
        with self.lock:
            ...
            logger.info(
                f"Started tracking operation: {operation_name} ({total_frames} frames)"
            )
            return progress

    def update_progress(
        self, operation_id: str, completed_frames: int, details: Dict[str, Any] = None
    ):
            ...
            if elapsed_time > 0:
                ...
                if progress.current_fps > 0:
                    ...
                    progress.estimated_time_remaining = (
                        remaining_frames / progress.current_fps
                    )
    ...
    def complete_operation(
        self, operation_id: str, success: bool = True, error_message: str = None
    ):
        ...
        with self.lock:
            ...
            logger.info(
                f"Operation completed: {progress.operation_name} ({progress.status})"
            )
...
class ParallelProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"Parallel processor initialized: {self.max_workers} workers, mode: {processing_mode.value}"
        )
    ...
    def process_frames_parallel(
        ...
        if CIRCUIT_BREAKER_AVAILABLE and self.parallel_processing_breaker:
            try:
                ...
                return self._process_frames_sequential(
                    frame_processor, frame_data, progress_callback
                )
        else:
            return self._process_frames_parallel_protected(
                frame_processor, frame_data, progress_callback
            )

    def _process_frames_parallel_protected(
        self,
        frame_processor: Callable,
        frame_data: List[Any],
        progress_callback: Optional[Callable] = None,
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_frame):
                try:
                    ...
                    frame_index = future_to_frame[future]
                    logger.error(
                        f"Frame processing failed for index {frame_index}: {e}"
                    )
    ...
    def optimize_batch_size(
        self, total_frames: int, memory_limit_mb: float = 1024
    ) -> int:
...
class MemoryManager:
    ...
    def _calculate_max_cache_size_mb(self) -> float:
        ...
        return (
            available_mb * (self.memory_limit_percent / 100) * 0.5
        )  # Use 50% of limit for cache

    def cache_frame(self, frame_id: str, frame_data: Any, size_mb: float):
        """Cache frame data with memory management."""
        # Check if we need to free memory
        while (
            self.cache_size_mb + size_mb > self.max_cache_size_mb and self.memory_cache
        ):
...
class VideoPerformanceMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"Video Performance Monitor initialized with strategy: {optimization_strategy.value}"
        )
    ...
    def monitor_operation(self, operation_name: str, frames_count: int = 0):
        ...
        if frames_count > 0:
            self.progress_tracker.start_operation(
                operation_id, operation_name, frames_count
            )
        ...
        try:
            ...
            metrics = PerformanceMetrics(
                ...
                memory_usage_mb=final_resources.memory_total_gb
                * 1024
                * (final_resources.memory_usage_percent / 100),
            ...
            logger.info(
                f"Operation completed: {operation_name} ({duration:.2f}s, {metrics.throughput_fps:.2f} FPS)"
            )
    ...
    def optimize_processing_settings(
        self, total_frames: int, target_quality: str = "high"
    ) -> Dict[str, Any]:
        ...
        if self.optimization_strategy == OptimizationStrategy.SPEED_FIRST:
            settings.update(
                {
                    ...
                    "batch_size": self.parallel_processor.optimize_batch_size(
                        total_frames, 2048
                    ),
            ...
            settings.update(
                {
                    "batch_size": self.parallel_processor.optimize_batch_size(
                        total_frames, available_memory_mb * 0.6
                    ),
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_fps = sum(
            m.throughput_fps for m in self.metrics_history if m.success
        ) / max(1, successful_operations)
        avg_memory_usage = (
            sum(m.memory_usage_mb for m in self.metrics_history) / total_operations
        )
        avg_cpu_usage = (
            sum(m.cpu_usage_percent for m in self.metrics_history) / total_operations
        )
    ...
    def _on_resource_update(self, resources: SystemResources):
        ...
        if (
            ...
            > self.performance_thresholds["max_memory_percent"]
        ):
    ...
    def _on_progress_update(self, progress: ProgressInfo):
        ...
        # Check for performance issues
        if (
            progress.current_fps < self.performance_thresholds["min_fps"]
            and progress.completed_frames > 10
        ):
