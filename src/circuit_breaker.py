...
class CircuitBreaker:
    ...
    def _execute_with_timeout(self, func: Callable, *args, **kwargs) -> Any:
        ...
        if thread.is_alive():
            ...
            logger.warning(
                f"Opération timeout après {self.config.timeout}s dans circuit '{self.name}'"
            )
    ...
    def _get_current_state(self) -> CircuitState:
        ...
        if self.state == CircuitState.OPEN:
            ...
            if (
                ...
                and now - self.stats.last_failure_time >= self.config.recovery_timeout
            ):
                self._change_state(CircuitState.HALF_OPEN)
                logger.info(
                    f"Circuit breaker '{self.name}' passe en HALF_OPEN pour test de récupération"
                )
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                if self.stats.consecutive_successes >= self.config.success_threshold:
                    ...
                    logger.info(
                        f"Circuit breaker '{self.name}' fermé après {self.stats.consecutive_successes} succès"
                    )
    ...
    def _on_failure(self, exception: Exception):
        ...
        with self._lock:
            ...
            if (
                ...
                logger.error(
                    f"Circuit breaker '{self.name}' OUVERT après {self.stats.consecutive_failures} échecs"
                )
                ...
                self._change_state(CircuitState.OPEN)
                logger.warning(
                    f"Circuit breaker '{self.name}' retour en OPEN après échec du test"
                )
