...
class AssetRetriever:
    ...
    def __init__(
        self, config: ComfyUIConfig, cleanup_policy: Optional[CleanupPolicy] = None
    ):
        """
        ...
        self.logger.info(
            f"Asset Retriever initialized with storage at {self.assets_dir}"
        )
    ...
    async def retrieve_execution_assets(
        self, execution_result: ExecutionResult, target_directory: Optional[Path] = None
    ) -> List[AssetInfo]:
        """
        ...
        """
        operation_id = (
            f"asset_retrieval_{execution_result.prompt_id}_{int(time.time())}"
        )
        ...
        try:
            if not execution_result.output_images:
                self.logger.info(
                    f"No assets to retrieve for execution {execution_result.prompt_id}"
                )
            ...
            target_dir = (
                target_directory or self.assets_dir / execution_result.prompt_id
            )
            target_dir.mkdir(parents=True, exist_ok=True)

            self.logger.info(
                f"Retrieving {len(execution_result.output_images)} assets for execution {execution_result.prompt_id}"
            )
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error(
                        f"Failed to download asset {execution_result.output_images[i].filename}: {result}"
                    )
            ...
            success_rate = (
                len(retrieved_assets) / len(execution_result.output_images) * 100
            )
            self.logger.info(
                f"Retrieved {len(retrieved_assets)}/{len(execution_result.output_images)} assets successfully"
            )
    ...
    async def _download_asset(
        self, asset_info: AssetInfo, target_dir: Path
    ) -> AssetInfo:
        """
        ...
        download = AssetDownload(
            filename=asset_info.filename, url=download_url, local_path=target_path
        )
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    self.logger.debug(
                        f"Downloading {asset_info.filename} (attempt {attempt + 1})"
                    )

                    async with self._session.get(
                        download_url, params=params
                    ) as response:
                        if response.status == 200:
                            ...
                            if total_size > 0:
                                ...
                                self.logger.debug(
                                    f"Successfully downloaded {asset_info.filename} ({total_size} bytes)"
                                )
                            ...
                            raise FileNotFoundError(
                                f"Asset not found: {asset_info.filename}"
                            )
                        else:
                    ...
                    if attempt < max_retries:
                        ...
                        self.logger.warning(
                            f"Download attempt {attempt + 1} failed for {asset_info.filename}: {e}. Retrying in {delay}s"
                        )
                        ...
                        self.logger.error(
                            f"Failed to download {asset_info.filename} after {max_retries + 1} attempts: {e}"
                        )
                        raise
    ...
    def organize_assets(
        self, source_dir: Path, organization_scheme: str = "by_date"
    ) -> Dict[str, Path]:
        """
        ...
        for asset_file in source_dir.glob("*"):
            if asset_file.is_file():
                if organization_scheme == "by_date":
                    ...
                    self.logger.warning(
                        f"Unknown organization scheme: {organization_scheme}"
                    )
                    continue
                ...
                if asset_file != new_path:
                    ...
                    while new_path.exists():
                        ...
                        new_path = (
                            original_new_path.parent / f"{stem}_{counter}{suffix}"
                        )
                        counter += 1
        ...
        self.logger.info(
            f"Organized {len(organized_paths)} assets using scheme: {organization_scheme}"
        )
    ...
    async def verify_asset_integrity(self, asset_info: AssetInfo) -> bool:
        ...
        if not asset_info.checksum:
            self.logger.warning(
                f"No checksum available for asset: {asset_info.filename}"
            )
        ...
        try:
            ...
            if calculated_checksum == asset_info.checksum:
                ...
                self.logger.error(
                    f"Asset integrity check failed for {asset_info.filename}: expected {asset_info.checksum}, got {calculated_checksum}"
                )
                return False

        except Exception as e:
            self.logger.error(
                f"Error verifying asset integrity for {asset_info.filename}: {e}"
            )
            return False
    ...
    async def _cleanup_old_files(self) -> None:
        ...
        cutoff_time = datetime.now() - timedelta(
            hours=self.cleanup_policy.max_age_hours
        )
        total_size = 0
        files_to_delete = []
