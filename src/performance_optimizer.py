...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Optional[OptimizationConfig] = None):
        ...
        logger.info(
            f"Performance optimizer initialized with strategy: {self.config.strategy.value}"
        )

    def optimize_batch_size(self, num_items: int, item_complexity: float = 1.0) -> int:
        """
        ...
        with self.lock:
            ...
            if self.config.strategy == OptimizationStrategy.SPEED:
                ...
                if avg_gpu_util < 0.6:
                    ...
                    base_batch_size = max(
                        self.current_batch_size - 1, self.config.min_batch_size
                    )
                else:
    ...
    def adjust_quality_level(
        ...
        with self.lock:
            ...
            if current_performance <= 0:
                logger.error(
                    f"Invalid current_performance value: {current_performance}"
                )
            ...
            if self.config.strategy == OptimizationStrategy.QUALITY:
                ...
                if performance_ratio < 0.8:
                    quality_level = max(
                        self.config.quality_threshold, self.current_quality_level - 0.1
                    )
                ...
                if performance_ratio < 0.7:
                    quality_level = max(
                        self.config.quality_threshold, self.current_quality_level - 0.15
                    )
                    ...
                    quality_level = max(
                        self.config.quality_threshold, self.current_quality_level - 0.05
                    )
    ...
    def create_batches(
        self, items: List[Any], complexity_fn: Optional[Callable[[Any], float]] = None
    ) -> List[List[Any]]:
        """
        ...
        if (
            hasattr(self, "gpu_memory_limit")
            and batch_size * avg_complexity > self.gpu_memory_limit
        ):
            logger.warning(
                f"Batch size {batch_size} exceeds GPU memory limit. Reducing batch size."
            )
...
class AutomaticPerformanceMonitor:
    ...
    def __init__(self, optimizer: PerformanceOptimizer, check_interval: float = 5.0):
        """
        ...
        logger.info(
            f"Automatic performance monitor initialized (interval: {check_interval}s)"
        )
