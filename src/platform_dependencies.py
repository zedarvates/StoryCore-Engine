...
class PlatformDependencyManager:
    ...
    def _get_opencl_install_command(self) -> str:
        ...
        if self.platform_type == "windows":
            ...
            return (
                "OpenCL included with macOS (use Metal instead for better performance)"
            )
        else:
    ...
    ) -> Tuple[DependencyStatus, Optional[str]]:
        """Check the status of a specific dependency."""
        # Skip if not for this platform
        if (
            "all" not in dependency.platforms
            and self.platform_type not in dependency.platforms
        ):
                ...
                if dependency.min_version and version_output:
                    ...
                    if (
                        ...
                        and self._compare_versions(
                            detected_version, dependency.min_version
                        )
                        < 0
                    ):
                        return (
                            DependencyStatus.OUTDATED,
                            f"Found {detected_version}, need {dependency.min_version}+",
                        )
    ...
    def generate_installation_guide(self) -> str:
        ...
        for dependency in self.dependencies:
            ...
            status, message = all_results.get(
                dependency.name, (DependencyStatus.MISSING, "")
            )
        ...
        if self.platform_type == "windows":
            ...
            guide.append(
                "• For GPU acceleration, install appropriate drivers (nvidia-driver, mesa)"
            )
            guide.append(
                "• Consider using virtual environments for Python dependencies"
            )
        elif self.platform_type == "macos":
    ...
    def validate_environment(self) -> Tuple[bool, List[str]]:
        ...
        if missing_required:
            issues.append(
                f"Missing required dependencies: {', '.join(dep.name for dep in missing_required)}"
            )
...
def main():
    ...
    print("")
    print(
        f"Summary: {required_missing} required missing, {optional_missing} optional missing"
    )
