...
class HunyuanVideoWorkflow(BaseAdvancedWorkflow):
    ...
    async def validate_request(self, request: WorkflowRequest) -> Tuple[bool, str]:
        ...
        if not req_caps.intersection(supported_caps):
            return (
                False,
                f"HunyuanVideo does not support required capabilities: {req_caps}",
            )
    ...
    async def execute(self, request: WorkflowRequest) -> WorkflowResult:
        ...
        try:
            ...
            gen_request = VideoGenerationRequest(
                ...
                width=request.output_resolution[0]
                if request.output_resolution
                else params.get("width", self.hunyuan_config.width),
                height=request.output_resolution[1]
                if request.output_resolution
                else params.get("height", self.hunyuan_config.height),
                ...
                enable_upscaling=WorkflowCapability.SUPER_RESOLUTION
                in request.capabilities_required
                or params.get("enable_upscaling", False),
            ...
            if result.frames:
                ...
                output_paths = [
                    f"output/hunyuan_{int(time.time())}_{i}.png"
