...
async def analyze_contribution(
    req: AnalysisRequest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    if (
        "fix" in req.contribution_text.lower()
        or "resolves" in req.contribution_text.lower()
    ):
        impact_multiplier = 2.0
    if (
        "critical" in req.contribution_text.lower()
        or "security" in req.contribution_text.lower()
    ):
...
class AgentRegisterRequest(BaseModel):
    operator_id: str = Field(
        ..., description="Wallet ID of the human operator who owns the agent"
    )
    agent_name: str = Field(..., description="Human-readable name for this agent")
    platform: str = Field(
        default="custom",
        description="Platform: 'openclaw', 'langchain', 'autogpt', 'custom'",
    )
    model: str = Field(default="unknown", description="LLM model used by the agent")
    webhook_endpoint: Optional[str] = Field(
        None, description="Optional callback URL for async notifications"
    )
...
class AgentContributionRequest(BaseModel):
    ...
    operator_id: str = Field(
        ..., description="Wallet ID of the operator receiving the Gems"
    )
    app_id: str = Field(default="storycore")
    output_text: str = Field(
        ..., min_length=10, description="The agent's output / contribution"
    )
    task_type: str = Field(
        default="research",
    ...
    context_prompt: Optional[str] = Field(
        None, description="The task/prompt the agent was given"
    )
    ...
    return {
        ...
        "message": (
            ...
            + (
                ...
                if secret
                else "Running in open mode — no signature required (dev/demo only)."
            )
        ),
    }
...
async def agent_contribution(
    ...
    if agent_info["operator_id"] != req.operator_id:
        raise HTTPException(
            status_code=403, detail="operator_id does not match agent registration."
        )
    ...
    if secret and x_agent_signature:
        if not _verify_hmac_signature(req.output_text, secret, x_agent_signature):
            raise HTTPException(
                status_code=401, detail="Invalid HMAC signature. Contribution rejected."
            )
    elif secret and not x_agent_signature:
        raise HTTPException(
            status_code=401,
            detail="This agent requires X-Agent-Signature header (HMAC-SHA256).",
        )
    ...
    if any(
        kw in text.lower() for kw in ["result", "finding", "solution", "patch", "fix"]
    ):
    ...
    rep["avg_quality_score"] = (
        rep.get("avg_quality_score", 0.0) * total_done + quality_score
    ) / (total_done + 1)
    rep["reputation_factor"] = min(
        3.0, 0.5 + (math.log1p(rep["total_contributions"]) * 0.4)
    )
    ...
    result = [
        {
            ...
            "total_contributions": _agent_reputation.get(agent_id, {}).get(
                "total_contributions", 0
            ),
            "total_gems_earned": _agent_reputation.get(agent_id, {}).get(
                "total_gems_earned", 0
            ),
            "avg_quality_score": round(
                _agent_reputation.get(agent_id, {}).get("avg_quality_score", 0.0), 3
            ),
            "reputation_factor": round(
                _agent_reputation.get(agent_id, {}).get("reputation_factor", 1.0), 3
            ),
        }
...
async def create_escrow(req: EscrowRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not escrow:
        raise HTTPException(
            status_code=400,
            detail="Escrow creation failed. Check balance or hardware availability.",
        )
...
async def register_worker(
    req: WorkerRegisterRequest, db: AsyncSession = Depends(get_db)
):
    engine = GemEngineStandalone(db)
...
async def get_workers(db: AsyncSession = Depends(get_db)):
    ...
    stmt = select(WorkerNode).order_by(
        WorkerNode.status.desc(), WorkerNode.last_seen.desc()
    )
...
async def get_task_categories(db: AsyncSession = Depends(get_db)):
    ...
    stmt = select(TaskCategory).where(TaskCategory.is_active)
    result = await db.execute(stmt)
...
async def seed_tasks(db: AsyncSession = Depends(get_db)):
    ...
    defaults = [
        TaskCategory(
            id="video_draft",
            display_name="Draft Video (LTX2 Fast)",
            base_cost=2,
            min_vram_gb=12,
        ),
        TaskCategory(
            id="video_cinematic",
            display_name="Cinematic Video (LTX2 Pro)",
            base_cost=5,
            min_vram_gb=16,
        ),
        TaskCategory(
            id="video_ultra",
            display_name="Ultra Video (LTX2 Ultra)",
            base_cost=10,
            min_vram_gb=24,
        ),
        TaskCategory(
            id="audio_sfx",
            display_name="Audio SFX Generation",
            base_cost=1,
            min_vram_gb=4,
        ),
        TaskCategory(
...
async def transfer_gems(req: TransferRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not transaction:
        raise HTTPException(
            status_code=400, detail="Transfer failed. Check balance or user existence."
        )
