...
from models import AppRegistration, AppRewardRule, UserWallet, GemTransaction, GemEscrow, WorkerNode, TaskCategory
...
class GemEngineStandalone:
    ...
    async def process_transaction(
        ...
        """
        if amount < 0:
            logger.error(f"Rejected negative gem transaction: {amount} for user {user_id}")
            return None
    ...
    async def transfer_gems(
        ...
        try:
            ...
            # Lock sender
            stmt_from = select(UserWallet).where(UserWallet.user_id == from_user_id).with_for_update()
            res_from = await self.db.execute(stmt_from)
            sender = res_from.scalar_one_or_none()
            
            if not sender or sender.gem_balance < amount:
                logger.error(f"Transfer failed: Insufficient balance for {from_user_id}")
                return None

            # Lock receiver
            stmt_to = select(UserWallet).where(UserWallet.user_id == to_user_id).with_for_update()
            res_to = await self.db.execute(stmt_to)
            receiver = res_to.scalar_one_or_none()
            
            if not receiver:
                # Create receiver if they don't exist
                receiver = UserWallet(user_id=to_user_id, gem_balance=0, gem_total_earned=0)
            ...
            receiver.gem_total_earned += amount # Transfers count towards tier progression for the receiver
    ...
    async def validate_task_feasibility(self, task_type: str) -> bool:
        ...
        try:
            ...
            stmt_task = select(TaskCategory).where(TaskCategory.id == task_type, TaskCategory.is_active == True)
            ...
            stmt_worker = select(WorkerNode).where(
                ...
                WorkerNode.vram_gb >= task.min_vram_gb
            )
            ...
            suitable_workers = [
                w for w in workers 
                if task_type in (w.capabilities if isinstance(w.capabilities, list) else [])
            ]
            
            if not suitable_workers:
                logger.error(f"Feasibility failed: No online worker for {task_type} (Req: {task.min_vram_gb}GB VRAM)")
                return False
    ...
    async def register_worker(self, user_id: str, name: str, vram: int, capabilities: List[str]) -> WorkerNode:
        """Join the compute mesh. If worker already exists for this user/name, update it."""
        try:
            stmt = select(WorkerNode).where(WorkerNode.user_id == user_id, WorkerNode.name == name)
    ...
    async def cleanup_offline_workers(self, threshold_seconds: int = 300) -> int:
        ...
        try:
            ...
            stmt = update(WorkerNode).where(
                and_(
                    WorkerNode.status == "online",
                    WorkerNode.last_seen < cutoff
                )
    ...
    async def create_escrow(
        ...
        try:
            ...
            stmt = select(UserWallet).where(UserWallet.user_id == sender_id).with_for_update()
            res = await self.db.execute(stmt)
    ...
    async def release_escrow(self, escrow_id: str) -> bool:
        ...
        try:
            ...
            stmt_recv = select(UserWallet).where(UserWallet.user_id == escrow.receiver_id).with_for_update()
            res_recv = await self.db.execute(stmt_recv)
            receiver = res_recv.scalar_one_or_none()

            if not receiver:
                receiver = UserWallet(user_id=escrow.receiver_id, gem_balance=0, gem_total_earned=0)
                self.db.add(receiver)
                await self.db.flush()
    ...
    async def cancel_escrow(self, escrow_id: str) -> bool:
        ...
        try:
            ...
            # 1. Lock sender wallet
            stmt_send = select(UserWallet).where(UserWallet.user_id == escrow.sender_id).with_for_update()
