...
- User control complete locally: the Anna adapter offers an optional validated model hint and otherwise preserves the Anna default. It has 65-test, strict-validation, end-to-end Edge, and deletion/storage-preservation evidence. A complete Gemma corpus is the next paid gate; it must not be reported as the default-model score.
