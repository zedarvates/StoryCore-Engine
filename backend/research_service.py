...
try:
    ...
    async def call_llm_real(req, user_id):
        class MockResp:
            text = f"MOCK_RESEARCH: Researched data for '{req.prompt[:30]}...'"
...
class ResearchService:
    ...
    async def get_grounding_facts(
        self, prompt: str, context: GroundingContext
    ) -> Dict[str, Any]:
        """
    ...
    async def enhance_prompt(self, base_prompt: str, facts: Dict[str, Any]) -> str:
        ...
        fact_str = ", ".join(
            [f"{k}: {v}" for k, v in facts.items() if isinstance(v, (str, int, float))]
        )
    ...
    def _build_research_query(self, prompt: str, context: GroundingContext) -> str:
        query_parts = [
            f"Research the following scene for factualaccuracy and cinematic grounding: '{prompt}'"
        ]
