...
class BeatDetectionService:
    ...
    def _detect_beats_librosa(
        self, audio_data: np.ndarray, sr: int, duration: float
    ) -> BeatDetectionResult:
        ...
        try:
            ...
            librosa.beat.beat_track(pulse, sr=sr)
    ...
    def _detect_beats_energy(
        ...
        energy_smooth = np.convolve(
            energy, np.ones(kernel_size) / kernel_size, mode="same"
        )
    ...
    def _detect_beats_spectral(
        ...
        try:
            ...
            beats = [
                BeatInfo(
                    timestamp=float(t), confidence=0.7, beat_type="beat", tempo=tempo
                )
                for t in times
            ]
    ...
    def _identify_downbeats(
        self, beat_times: np.ndarray, time_signature: str
    ) -> List[float]:
...
class AudioCleaningService:
    ...
    def _isolate_voice_demucs(
        self, input_path: str, output_path: str, config: VoiceIsolationConfig
    ) -> Tuple[bool, str]:
        """Use Demucs ML model for voice separation."""
        try:
            # Check if demucs is installed
            result = subprocess.run(
                ["python", "-c", "import demucs"], capture_output=True
            )
    ...
    def _isolate_voice_spectral(
        ...
        try:
            ...
            Zxx_clean = np.maximum(
                np.abs(Zxx) - reduction_factor * noise_profile, 0
...
class AutoDuckingService:
    ...
    def apply_ducking(
        ...
        try:
            ...
            cmd = [
                ...
                "-y",
                "-i",
                music_path,  # Input 0: Music to duck
                "-i",
                dialogue_path,  # Input 1: Dialogue (trigger)
                "-filter_complex",
                filter_complex,
                "-map",
                "[ducked]",
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                success, _ = cleaning_service.isolate_voice(
                    ...
                    VoiceIsolationConfig(
                        method=VoiceIsolationMethod.SPECTRAL_SUBTRACTION
                    ),
                )
    ...
    def detect_speech_segments(
        self, audio_path: str, threshold_db: float = -30.0, min_duration: float = 0.1
    ) -> List[Tuple[float, float]]:
        """
...
class AudioWorldizationService:
    ...
    def __init__(self, ffmpeg_path: str = "ffmpeg"):
        ...
        self.environment_presets = {
            ...
            AudioEnvironment.SMALL_ROOM: {"reverb": "aecho=0.3:0.4:100:0.1", "eq": ""},
...
class TranscriptionService:
    ...
    def _transcribe_whisper(
        ...
        try:
            ...
            return TranscriptionResult(
                success=False, text="", words=[], language="", duration=0.0
            )
        ...
        try:
            ...
            if self._vosk_model is None:
                ...
                if not os.path.exists(model_path):
                    logger.warning(
                        f"Vosk model not found at {model_path}. Transcription will fail."
                    )
    ...
    def get_timestamp_for_word(
        self, transcription: TranscriptionResult, word: str
    ) -> Optional[float]:
        """
    ...
    def search_in_transcription(
        self, transcription: TranscriptionResult, query: str
...
def create_worldization_service(
    ffmpeg_path: str = "ffmpeg",
) -> AudioWorldizationService:
