...
from backend.ttt_lrm_service import TTTLRMService, TTTLRMConfig, ReconstructionMode, OutputFormat
...
class ReconstructionRequest(BaseModel):
    ...
    mode: ReconstructionMode = Field(ReconstructionMode.FEEDFORWARD, description="FEEDFORWARD (fast) or TTT_ADAPTED (high quality)")
...
async def reconstruct_image(request: ReconstructionRequest):
    ...
    return ReconstructionResponse(
        ...
        message="Reconstruction successful" if result.success else "Reconstruction failed",
...
async def reconstruct_video(request: ReconstructionRequest):
    ...
    """
    if not os.path.exists(request.input_path):
        raise HTTPException(status_code=404, detail=f"Input video not found: {request.input_path}")
    ...
    return ReconstructionResponse(
        ...
        format=result.format.value,
        processing_time=result.processing_time,
        message="Scene reconstruction complete" if result.success else "Scene reconstruction failed",
        metrics=result.metrics
    )
...
async def convert_to_mesh(gs_path: str, output_path: str):
    ...
    return {"success": True, "output_path": output_path, "message": "Mesh converted successfully"}
