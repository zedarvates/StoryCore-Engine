...
from backend.ai_story_generation_service import (
    AIDrivenStoryGenerationService,
...
class StoryGenerateRequest(BaseModel):
    prompt: str = Field(
        ..., min_length=5, description="Prompt pour la génération de la story"
    )
    genre: str = Field(
        default="DRAMA", description="Genre narratif (ex: ADVENTURE, DRAMA, HORROR)"
    )
    structure: str = Field(
        default="THREE_ACT",
        description="Structure narrative (ex: THREE_ACT, HERO_JOURNEY)",
    )
    mode: str = Field(
        default="FICTION",
        description="Mode de production (ex: FICTION, DOCUMENTARY, INFLUENCER, SCIENTIFIC_REVIEW)",
    )
    length: str = Field(default="medium", description="Longueur: short, medium, long")
    with_critique: bool = Field(
        default=False, description="Activer la critique multi-agent"
    )
    with_alignment_score: bool = Field(
        default=True, description="Activer le scoring d'alignement détaillé"
    )
    temperature: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Température de créativité"
    )
    max_attempts: int = Field(
        default=3, ge=1, le=10, description="Nombre maximum de tentatives"
    )
...
async def generate_story_endpoint(
    req: StoryGenerateRequest, user_id: str = Depends(verify_jwt_token)
):
    try:
...
async def generate_story_basic_endpoint(
    req: StoryGenerateRequest, user_id: str = Depends(verify_jwt_token)
):
    try:
        ...
        structure = getattr(
            StoryStructure, req.structure.upper(), StoryStructure.THREE_ACT
        )
        ...
        return StoryGenerateResponse(
            ...
            length=story.length
            if hasattr(story, "length") and story.length
            else req.length,
...
async def refine_story_endpoint(
    story_id: str, feedback: Dict[str, str], user_id: str = Depends(verify_jwt_token)
):
    try:
        ...
        if not refined_story:
            raise HTTPException(
                status_code=404, detail="Story not found or refinement failed"
            )
...
async def refine_story_basic_endpoint(
    story_id: str, feedback: Dict[str, str], user_id: str = Depends(verify_jwt_token)
):
    try:
        if story_id not in story_service.stories:
        ...
        return StoryGenerateResponse(
            ...
            mode=story.mode.name,
            length=(
                story.metadata.get("length")
                if isinstance(story.metadata, dict)
                else "medium"
            ),
            characters=story.characters,
            locations=story.locations,
