...
class TestProjectAPI:
    ...
    def test_list_projects(self, client, mock_user_id):
        ...
        response = client.get(
            "/api/projects", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
        assert response.status_code == 200
        data = response.json()
...
class TestShotAPI:
    ...
    def test_get_shot(self, client, mock_user_id):
        ...
        response = client.get(
            f"/api/shots/{shot_id}", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
        assert response.status_code == 200
        data = response.json()
...
class TestLLMAPI:
    ...
    def test_list_templates(self, client, mock_user_id):
        ...
        response = client.get(
            "/api/llm/templates", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_list_models(self, client, mock_user_id):
        ...
        response = client.get(
            "/api/llm/models", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_cache_operations(self, client, mock_user_id):
        ...
        stats_response = client.get(
            "/api/llm/cache/stats", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
        assert stats_response.status_code == 200

        # Clear cache
        clear_response = client.delete(
            "/api/llm/cache", headers={"Authorization": f"Bearer {mock_user_id}"}
        )
