...
async def trigger_workflow(
    webhook_id: str, payload: Dict[str, Any], n8n: N8nService = Depends(get_n8n_service)
):
...
async def create_workflow(
    payload: Dict[str, Any], n8n: N8nService = Depends(get_n8n_service)
):
    ...
    if not name or not nodes or not connections:
        raise HTTPException(
            status_code=400, detail="Missing name, nodes or connections"
        )
...
async def import_n8n_template(
    filename: str, n8n: N8nService = Depends(get_n8n_service)
):
    """
