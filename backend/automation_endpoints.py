...
character_grid_automation = create_character_grid_automation(
    base_output_dir="assets/characters"
)
...
class PromptEnhanceRequest(BaseModel):
    ...
    base_prompt: str = Field(
        ..., min_length=5, description="Prompt de base à améliorer"
    )
...
class PaperEditRequest(BaseModel):
    ...
    structure_type: str = (
        "Classic 3-Act"  # "Classic 3-Act", "Save the Cat", "Documentary"
    )
...
async def generate_character_grid(request: CharacterGridRequest):
    ...
    try:
        ...
        lighting_types = [LightingType(lt) for lt in request.lighting_types]
...
@router.get(
    "/character/{character_id}/grids", response_model=CharacterGridsListResponse
)
...
async def get_prompt_styles():
    ...
    return {
        ...
        "lighting": [ls.value for ls in LightingStyle],
...
async def apply_invisible_editing(request: InvisibleEditingRequest):
    ...
    return InvisibleEditingResponse(
        shots=updated_shots, applied_pattern=request.pattern, total_overlap=overlap
    )
...
async def adapt_for_social_media(request: SocialMediaAdaptRequest):
    ...
    return SocialMediaAdaptResponse(posts=posts, viral_score=0.85)
