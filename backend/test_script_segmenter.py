...
"""
    ...
    def test_detect_emotional_tone(self, service):
        ...
        assert tone in [
            "happy",
            "neutral",
        ]  # Peut être happy ou neutral selon les patterns
    ...
    def test_segment_script_basic(self, service, sample_script):
        ...
        result = service.segment_script(
            text=sample_script, script_id="test_script", project_id="test_project"
        )
    ...
    def test_adjust_segment_merge(self, service, sample_script):
        ...
        if original_count >= 2:
            ...
            result = service.adjust_segment(result, segment_to_merge.id, "merge")
...
class TestSegmentationResult:
    ...
    def test_to_dict(self):
        ...
        result = SegmentationResult(
            id="result_1", script_id="script_1", segments=[segment], total_duration=10.0
        )
        ...
        if len(loaded.segments) >= 1:
            ...
            if len(segment.text) > 50:
                adjusted = service.adjust_segment(
                    loaded, segment.id, "split", {"split_at": len(segment.text) // 2}
                )
