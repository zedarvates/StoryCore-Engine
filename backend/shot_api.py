...
shot_storage = JSONFileStorage(
    "./data/shots", max_cache_size=1000, index_field="project_id"
)


@router.post("/shots", response_model=ShotResponse, status_code=status.HTTP_201_CREATED)
async def create_shot(
    shot: ShotCreate, user_id: str = Depends(verify_jwt_token)
) -> ShotResponse:
    """
    Create a new shot.
    ...
    shot_data = {
        ...
        if hasattr(shot.shot_type, "value")
        else shot.shot_type,
...
async def get_shot(
    shot_id: str, user_id: str = Depends(verify_jwt_token)
) -> ShotResponse:
    """
    ...
    if PROJECT_API_AVAILABLE:
        ...
        if (
            project
            and project.get("owner_id") != user_id
            and not project.get("is_public")
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def update_shot(
    shot_id: str, update: ShotUpdate, user_id: str = Depends(verify_jwt_token)
) -> ShotResponse:
    """
    Update shot details.
...
async def delete_shot(shot_id: str, user_id: str = Depends(verify_jwt_token)):
    """
    Delete a shot.
...
async def list_project_shots(
    ...
    if PROJECT_API_AVAILABLE:
        ...
        if project_data.get("owner_id") != user_id and not project_data.get(
            "is_public"
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    return ShotListResponse(
        shots=shots_response, total=total, page=page, page_size=page_size
    )
