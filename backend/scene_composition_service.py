...
class SceneCompositionService:
    ...
    def _generate_shot_list(
        ...
        shot_types = self.SHOT_SEQUENCES.get(
            sequence_type, self.SHOT_SEQUENCES["dialogue"]
        )
        ...
        for i, shot_type in enumerate(shot_types):
            ...
            if i == 0 and ShotType.WIDE in shot_types:
                ...
                framing = self._generate_framing(
                    shot_type, characters[i % len(characters)] if characters else None
                )
    ...
    def _infer_sequence_type(self, description: str, mood: str) -> str:
        ...
        if any(
            word in desc_lower for word in ["dialogue", "conversation", "talk", "speak"]
        ):
            return "dialogue"
        elif any(word in desc_lower for word in ["fight", "chase", "run", "action"]):
            return "action"
        elif any(
            word in desc_lower for word in ["tension", "fear", "suspense", "waiting"]
        ):
            return "tension"
        elif any(
            word in desc_lower for word in ["establishing", "location", "setting"]
        ):
            return "establishing"
    ...
    def _infer_camera_movement(self, shot_type: ShotType, mood: str) -> CameraMovement:
        ...
        if shot_type == ShotType.WIDE:
            ...
            return (
                CameraMovement.ZOOM_IN if mood == "intense" else CameraMovement.STATIC
            )
    ...
    def _select_rules(
        self, description: str, characters: List[str] = None
        ...
        if any(
            word in desc_lower
            for word in ["landscape", "wide", "outdoor", "environment"]
        ):
            selected.append(self.COMPOSITION_RULES["leading_lines"])
            selected.append(self.COMPOSITION_RULES["depth"])

        if any(
            word in desc_lower for word in ["portrait", "face", "character", "emotion"]
        ):
    ...
    def add_shot(self, composition_id: str, shot_data: dict) -> CameraSetup:
    ...
    def analyze_composition(self, composition_id: str) -> Dict[str, Any]:
        ...
        pacing_score = sum(s.duration_estimate for s in composition.shots) / len(
            composition.shots
        )
