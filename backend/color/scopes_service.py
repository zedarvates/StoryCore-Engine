...
class ScopesService:
    ...
    def waveform(
        ...
        if mode == "luma":
            ...
            for channel, r, g, b in colors:
                for x in range(width):
                    col_histogram = self._column_waveform(frame[:, x, channel], wave_height)
            ...
            for x in range(width):
                u_wave[:, x] = self._column_waveform((yuv[:, x, 1] + 0.5) / 1.0, wave_height)
            waveform[:, :, 2] = np.maximum(waveform[:, :, 2], u_wave * 0.5)
            
            # V channel (red)
            for x in range(width):
                v_histogram = self._column_waveform((yuv[:, x, 2] + 0.5) / 1.0, wave_height)
    ...
    def _column_waveform(
        ...
        indices = np.clip(column * (output_height - 1), 0, output_height - 1).astype(int)
    ...
    def _add_waveform_graticule(self, waveform: np.ndarray) -> np.ndarray:
        ...
        waveform[height - 1, :, :] = np.maximum(waveform[height - 1, :, :], 0.3)  # 100 IRE
            ...
            for channel, color in colors:
                ...
                for i, val in enumerate(hist):
                    ...
                    histogram_img[height - bar_height:, i, :] = np.maximum(
                        histogram_img[height - bar_height:, i, :],
                        color
                    )
    ...
    def false_color(
        ...
        for i in range(len(color_map) - 1):
            ...
            if mask.any():
                ...
                for c in range(3):
                    false_color[mask, c] = low_color[c] + t * (high_color[c] - low_color[c])
    ...
    def _rgb_to_ycbcr(self, frame: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
