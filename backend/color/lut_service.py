...
class LUT:
    ...
    def __post_init__(self):
        ...
        if self.data is not None:
            expected_shape = (self.metadata.size, self.metadata.size, self.metadata.size, 3)
            if self.data.shape != expected_shape:
                logger.warning(f"LUT data shape {self.data.shape} doesn't match expected {expected_shape}")
...
class LUTService:
    ...
    def _parse_cube_data(self, content: str, size: int) -> np.ndarray:
        ...
        if len(raw_data) == size * size * size:
            ...
            if actual_size ** 3 == len(raw_data):
                ...
                raise ValueError(f"Invalid LUT data size: {len(raw_data)} (expected {size**3})")
    ...
    def _load_3dl(self, path: Path) -> LUT:
        ...
        lut_data = raw_data[:actual_size**3].reshape((actual_size, actual_size, actual_size, 3))
    ...
    def _apply_lut(self, frame: np.ndarray, lut_data: np.ndarray, interpolation: str = "trilinear") -> np.ndarray:
        """
        ...
        c001 = lut_data[coords_int[:, :, 0], coords_int[:, :, 1], np.minimum(coords_int[:, :, 2] + 1, size - 1)]
        c010 = lut_data[coords_int[:, :, 0], np.minimum(coords_int[:, :, 1] + 1, size - 1), coords_int[:, :, 2]]
        c011 = lut_data[coords_int[:, :, 0], np.minimum(coords_int[:, :, 1] + 1, size - 1), np.minimum(coords_int[:, :, 2] + 1, size - 1)]
        c100 = lut_data[np.minimum(coords_int[:, :, 0] + 1, size - 1), coords_int[:, :, 1], coords_int[:, :, 2]]
        c101 = lut_data[np.minimum(coords_int[:, :, 0] + 1, size - 1), coords_int[:, :, 1], np.minimum(coords_int[:, :, 2] + 1, size - 1)]
        c110 = lut_data[np.minimum(coords_int[:, :, 0] + 1, size - 1), np.minimum(coords_int[:, :, 1] + 1, size - 1), coords_int[:, :, 2]]
        c111 = lut_data[np.minimum(coords_int[:, :, 0] + 1, size - 1), np.minimum(coords_int[:, :, 1] + 1, size - 1), np.minimum(coords_int[:, :, 2] + 1, size - 1)]
        
        fx, fy, fz = coords_frac[:, :, 0:1], coords_frac[:, :, 1:2], coords_frac[:, :, 2:3]
    ...
    def save_lut(
        ...
        lut: LUT,
        path: str,
        format: Optional[LUTFormat] = None
    ) -> bool:
        """
        Save a LUT to file.
    ...
    def _save_cube(self, lut: LUT, path: Path) -> bool:
        ...
        with open(path, 'w') as f:
            ...
            f.write(f"# Created by StoryCore-Engine\n")
    ...
    def _hsv_to_rgb_point(self, h: float, s: float, v: float) -> Tuple[float, float, float]:
