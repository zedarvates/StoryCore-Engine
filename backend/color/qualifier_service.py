...
class QualifierService:
    ...
    def qualify(
        ...
        lum_matte = self._qualify_luminance(l, qualifier)
    ...
    def _rgb_to_hsl(self, frame: np.ndarray) -> np.ndarray:
        ...
        # Saturation
        s = np.where(l < 0.5, delta / (max_val + min_val + 1e-6), 
    ...
    def _qualify_saturation(self, sat: np.ndarray, qualifier: HSLQualifier) -> np.ndarray:
    ...
    def _qualify_luminance(self, lum: np.ndarray, qualifier: HSLQualifier) -> np.ndarray:
    ...
    def feather_edges(
        self,
        matte: np.ndarray,
        inner_feather: float = 0.0,
        outer_feather: float = 0.0
    ) -> np.ndarray:
        """
    ...
    def _feather_edge(self, matte: np.ndarray, amount: float, inside: bool) -> np.ndarray:
