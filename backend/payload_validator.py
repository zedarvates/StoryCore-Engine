    ...
    try:
        ...
        if e.validator:
            error_messages.append(
                f"  Failed constraint: {e.validator} = {e.validator_value}"
            )
...
def validate_payload_detailed(
    ...
    try:
        ...
        logger.warning(
            f"Detailed payload validation failed with {len(errors)} error(s)"
        )
        return False, errors
...
def validate_schema_version(payload: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if version != "1.0":
        return (
            False,
            f"Unsupported schema version: {version}. Only version 1.0 is supported.",
        )
...
def migrate_payload_to_current_version(
    payload: Dict[str, Any],
    ...
    # These should already have schema_version "1.0" but might be missing some fields
        ...
        if "module_context" not in migrated_payload:
            ...
            migration_notes.append(
                "Replaced null module_context with default for 1.0 payload"
            )
        ...
        if "diagnostics" not in migrated_payload:
            ...
            migration_notes.append(
                "Replaced null diagnostics with default for 1.0 payload"
            )
