...
class FFmpegService:
    ...
    def _find_ffprobe(self) -> str:
        ...
        return self.ffmpeg_path.replace("ffmpeg", "ffprobe").replace(
            "FFmpeg", "FFprobe"
        )
        ...
        try:
            ...
            while True:
                ...
                if progress_info:
                    ...
                    remaining = (
                        (elapsed / (progress_info / 100)) * (100 - progress_info)
                        if progress_info > 0
                        else None
                    )
                    ...
                    callback(
                        ProgressCallback(
                            ...
                            fps=info.fps,
                            bitrate=f"{info.bitrate / 1000000:.1f}M"
                            if info.bitrate
                            else None,
                            status="processing",
                        )
            ...
            callback(
                ProgressCallback(
                    progress=0, status="error", message=f"File not found: {e.filename}"
                )
            )
        except PermissionError as e:
            ...
            callback(
                ProgressCallback(
                    progress=0, status="error", message=f"System error: {e}"
                )
            )

    def _parse_progress(
        self, line: str, total_duration: float, total_frames: int
    ) -> Optional[float]:
    ...
    def extract_frames(
        ...
        Returns:
            Tuple of (success, error_message)
        """
        cmd = [self.ffmpeg_path, "-y", "-ss", str(start_time), "-i", input_path]
    ...
    def generate_sprite_sheet(
        ...
        cmd = [
            ...
            input_path,
            "-vf",
            f"fps={fps},scale=160:-1,tile={columns}x{rows}",
            "-frames:v",
            "1",
            output_path,
        ]
    ...
    def remove_silence(
        ...
        cmd = [
            ...
            options.input_path,
            "-af",
            f"silenceremove=stop_periods=-1:stop_threshold={options.silence_threshold}dB:stop_duration={options.min_silence_duration}",
            "-c:a",
            "aac",
            "-b:a",
            "192k",
    ...
    def concatenate_videos(
        ...
        try:
            ...
            self.get_video_info(options.input_files[0])
    ...
    def concatenate_with_transitions(
        ...
        filter_complex = (
            ";".join(filter_parts)
            + f";{concat_inputs}concat=n={len(input_files)}:v=1:a=1[outv][outa]"
        )
        ...
        try:
            result = subprocess.run(
                cmd, capture_output=True, text=True, shell=True, timeout=3600
            )
            ...
            logger.error("Video concatenation with transitions timed out")
    ...
    def add_watermark(
        ...
        cmd = [
            ...
            watermark_path,
            "-filter_complex",
            f"[1:v]scale=iw*{scale}:-1,format=rgba,colorchannelmixer=aa={opacity}[wm];[0:v][wm]overlay={pos}",
            "-c:a",
            "copy",
            output_path,
        ]
    ...
    def get_gpu_info(self) -> Dict[str, Any]:
        ...
        """
        gpu_info = {"nvenc": False, "amf": False, "vaapi": False, "cuda": False}
    ...
    async def transcode_sse(
        self, options: TranscodeOptions, callback_url: str
    ) -> Tuple[bool, Optional[str]]:
        """
        ...
        def send_progress(progress: ProgressCallback):
            try:
                requests.post(callback_url, json=progress.__dict__, timeout=5)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "command", choices=["info", "convert", "thumbnails", "concat", "gpu-info"]
    )
    parser.add_argument("input", help="Input file path")
    parser.add_argument("--output", "-o", help="Output file path")
    parser.add_argument(
        "--format", "-f", choices=["mp4", "webm", "gif", "mov"], help="Target format"
    )
    ...
    if args.command == "info":
        ...
        options = ThumbnailOptions(
            output_path=args.output, width=args.width or 320, count=9
        )
