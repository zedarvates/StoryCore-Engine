...
class AIWorkflowOrchestrator:
    ...
    async def execute_workflow(self, workflow_id: str):
        ...
        try:
            for i, step in enumerate(instance.steps):
                ...
                self._progress.update_progress(workflow_id, progress_val, f"Executing step: {step.type.value}")

                logger.info(f"Executing step {i+1}/{len(instance.steps)}: {step.type}")
                ...
                try:
                    ...
                    logger.error(f"Step {step.type} failed in workflow {workflow_id}: {e}")
            ...
            self._progress.complete_job(workflow_id, result={"output": instance.context.get("last_output")})
    ...
    async def _execute_step(self, instance: WorkflowInstance, step: WorkflowStep):
        ...
        if step.type == WorkflowStepType.GENERATE_IMAGE:
            ...
            success, message, output = await self._vfx_service.apply_vfx_preset(video_path, vfx_type=vfx_type)
            if not success:
            ...
            video_path = instance.context.get("video_path")
            style = instance.context.get("subtitle_style", "netflix")
            await asyncio.sleep(2)
            output_path = os.path.join("output/pro", f"subtitled_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
        
        elif step.type == WorkflowStepType.SPEED_RAMP:
            video_path = instance.context.get("video_path")
            speed = instance.context.get("speed_multiplier", 1.0)
            await asyncio.sleep(1)
            output_path = os.path.join("output/pro", f"ramped_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
            ...
            video_path = instance.context.get("video_path")
            await asyncio.sleep(1)
            output_path = os.path.join("output/pro", f"ducked_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
        
        elif step.type == WorkflowStepType.BEAT_SYNC:
            video_path = instance.context.get("video_path")
            music_path = instance.context.get("music_path")
            await asyncio.sleep(2)
            output_path = os.path.join("output/pro", f"beatsync_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
            ...
            output_path = os.path.join("output/pro", f"masked_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            ...
            video_path = instance.context.get("video_path")
            bg_path = instance.context.get("background_path")
            await asyncio.sleep(3)
            output_path = os.path.join("output/pro", f"bgreplaced_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
        
        elif step.type == WorkflowStepType.ANIMATION_PRESET:
            media_path = instance.context.get("video_path") or instance.context.get("image_path")
            preset = instance.context.get("animation_preset", "ken_burns")
            await asyncio.sleep(1)
            output_path = os.path.join("output/pro", f"animated_{os.path.basename(media_path)}")
            instance.context["video_path"] = output_path
            ...
            video_path = instance.context.get("video_path")
            await asyncio.sleep(1)
            output_path = os.path.join("output/pro", f"hook_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
        
        elif step.type == WorkflowStepType.LIP_SYNC:
            video_path = instance.context.get("video_path")
            audio_path = instance.context.get("voiceover_path")
            await asyncio.sleep(4)
            output_path = os.path.join("output/pro", f"lipsynced_{os.path.basename(video_path)}")
            instance.context["video_path"] = output_path
            step.output_data = {"path": output_path}
            ...
            for v in videos:
                await asyncio.sleep(1)
                outputs.append(os.path.join("output/pro", f"rendered_{os.path.basename(v)}"))
            ...
            quality = instance.context.get("export_quality", "high")
            output_path = os.path.join("output/exports", f"final_{os.path.basename(video_path)}")
            await asyncio.sleep(2)
        ...
        # ============ Control Flow Steps ============
            ...
            if step.wait_for_workflow:
                ...
                while waited < max_wait:
                    ...
                    if not other or other.status in [WorkflowStatus.COMPLETED, WorkflowStatus.FAILED]:
                        break
                    await asyncio.sleep(1)
                    waited += 1
