...
class AudioSequenceIntegrator:
    ...
    def extract_context_from_sequence(
        self, sequence: Dict[str, Any], project_context: Optional[Dict[str, Any]] = None
        ...
        intensity = (
            project_context.get("emotional_intensity", "medium")
            if project_context
            else "medium"
        )
    ...
    def create_music_profile_from_context(
        self, context: SequenceAudioContext
    ) -> Dict[str, Any]:
        """
        ...
        if context.action_type:
            builder.set_action(
                context.action_type, context.action_intensity, context.visual_rhythm
            )
    ...
    def create_sfx_profile_from_context(
        ...
        intensity_mapping = {"low": "low", "medium": "medium", "high": "high"}
    ...
    def create_voice_profile_from_context(
        self, context: SequenceAudioContext, text_content: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        voice_mapping = {"dialogue": "raw", "narration": "whisper", "action": "raw"}
    ...
    def generate_audio_for_sequence(
        ...
        if (
            SequenceAudioType.MUSIC in audio_types
            or SequenceAudioType.ALL in audio_types
        ):
        ...
        if (
            SequenceAudioType.VOICE in audio_types
            or SequenceAudioType.ALL in audio_types
        ):
            for context in contexts:
        ...
        result.status = (
            IntegrationStatus.COMPLETED if result.success else IntegrationStatus.FAILED
        )

        logger.info(
            f"Audio-sequence integration: {len(result.music_prompts)} music, {len(result.sfx_prompts)} SFX, {len(result.voice_prompts)} voice"
        )
    ...
    def attach_audio_to_shot(
        ...
        music_tracks = [
            p for p in audio_result.music_prompts if p.get("shot_id") == shot_id
        ]
        sfx_tracks = [
            p for p in audio_result.sfx_prompts if p.get("shot_id") == shot_id
        ]
        voice_tracks = [
            p for p in audio_result.voice_prompts if p.get("shot_id") == shot_id
        ]
    ...
    def generate_timeline_sync_data(
        self, sequence: Dict[str, Any], audio_result: IntegratedAudioResult
    ) -> Dict[str, Any]:
        """
        ...
        for shot in shots:
            ...
            for audio_track in audio_tracks:
                ...
                if track_type == "voice":
                    ...
                    if (
                        "base" in track_name.lower()
                        or "fondation" in track_name.lower()
                    ):
                        timeline_track_id = "timeline_music_base"
                    elif (
                        "melody" in track_name.lower()
                        or "mélodie" in track_name.lower()
                    ):
