...
class EnhancedMusicSyncService:
    ...
    def _create_beat_synced_animation(
        ...
        try:
            ...
            beat_filters.append(f"zoompan=z='1+0.1*{intensity}*sin(2*PI*t/{beat_duration})'")
            ...
            cmd = [
                ...
                "-i", animation_path,
                "-i", audio_path,
                "-filter_complex", f"[0:v] {filter_str} [v];[1:a] [v]",
                "-map", "[v]",
                "-map", "1:a",
                "-c:v", "libx264",
                "-c:a", "aac",
    ...
    def create_thumbnail_hook(
        ...
        try:
            ...
            if config.audio_sync and os.path.exists(config.audio_sync):
                filter_str += ",afade=t=in:st=0:d=1,afade=t=out:st={}:d=1".format(config.duration - 1)
            
            cmd = [
                self.ffmpeg, "-y",
                "-loop", "1",
                "-i", config.image_path,
                "-i", config.audio_sync if config.audio_sync else None,
                "-filter_complex", filter_str,
                "-t", str(config.duration),
                "-c:v", "libx264",
                "-pix_fmt", "yuv420p",
                "-r", "30",
                "-c:a", "aac" if config.audio_sync else "copy",
                config.output_path
            ]
...
def create_enhanced_animation_service(ffmpeg_path: str = "ffmpeg") -> EnhancedAnimationPresetsService:
    return EnhancedAnimationPresetsService(ffmpeg_path)

def create_enhanced_pose_service(ffmpeg_path: str = "ffmpeg") -> EnhancedPoseInterpolationService:
    return EnhancedPoseInterpolationService(ffmpeg_path)

def create_enhanced_music_sync_service(ffmpeg_path: str = "ffmpeg") -> EnhancedMusicSyncService:
    return EnhancedMusicSyncService(ffmpeg_path)

def create_enhanced_thumbnail_service(ffmpeg_path: str = "ffmpeg") -> EnhancedThumbnailHookService:
