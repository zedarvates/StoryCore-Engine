...
DIRECTOR_ROUTER = APIRouter(
    prefix="/api/director", tags=["AI Director Mode (Nano Banana 2)"]
)
...
async def do_research(req: ResearchRequest):
    ...
    return {"facts": facts, "enhanced_prompt": enhanced_prompt}
...
async def lock_scene(req: CoverageRequest):
    ...
    profile = service.create_scene_dna(
        req.master_image, req.character_references, req.object_references
    )
    ...
    if req.character_references:
        for i, ref in enumerate(req.character_references):
            name = f"Character_{i}"
            sheets[name] = consistency_service.generate_character_sheet_prompts(
                req.prompt
            )
...
async def generate_coverage(
    scene_id: str, shots: List[str], scene_description: Optional[str] = None
):
    """
    ...
    return {"scene_id": scene_id, "shots": results, "structured_shot_list": shot_list}
...
async def kiwiedit(
    video_path: str, target: str, action: str, replacement: Optional[str] = None
):
    """
