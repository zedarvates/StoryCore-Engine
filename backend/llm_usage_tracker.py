...
class CostCalculator:
    ...
    def calculate_cost(
        ...
        if model_pricing is None:
            ...
            for model_key in provider_pricing:
                if (
                    model_key.lower() in model.lower()
                    or model.lower() in model_key.lower()
                ):
    ...
    def estimate_request_cost(
        ...
        max_cost = cls.calculate_cost(
            provider, model, estimated_prompt_tokens, max_completion_tokens
        )
...
class UsageStore:
    ...
    def _init_db(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_user_id ON usage_records(user_id)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_project_id ON usage_records(project_id)"
            )
            cursor.execute(
    ...
    def get_summary(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            summary = UsageSummary(
                ...
                cache_hit_rate=(cached_count / total_requests * 100)
                if total_requests > 0
                else 0.0,
    ...
    def cleanup_old_records(self, days_to_keep: int = 90) -> int:
        ...
        with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if removed > 0:
                    logger.info(
                        f"Cleaned up {removed} old usage records (older than {days_to_keep} days)"
                    )
...
class UsageAggregator:
    ...
    def aggregate_by_feature(
        ...
        return self.store.get_summary(
            feature=feature, start_time=start_time, end_time=end_time
        )
...
class UsageCollector:
    ...
    def collect(
        self, response: Dict[str, Any], context: UsageContext, latency_ms: int = 0
    ) -> UsageEntry:
        """
    ...
    def estimate_cost(
        self, provider: str, model: str, prompt: str, max_tokens: int
    ) -> Dict[str, float]:
        """
        Estimate the cost for a request before making it.
