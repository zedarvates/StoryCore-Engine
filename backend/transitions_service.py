...
class TransitionsService:
    ...
    def build_transition_command(
        ...
        if transition_type == TransitionType.CUT:
            ...
            return self._build_dissolve(
                clip_a_path, clip_b_path, output_path, config.duration
            )
        elif transition_type == TransitionType.FADE_BLACK:
            return self._build_fade(
                clip_a_path, clip_b_path, output_path, config.duration, "black"
            )
        elif transition_type == TransitionType.FADE_WHITE:
            return self._build_fade(
                clip_a_path, clip_b_path, output_path, config.duration, "white"
            )
        elif transition_type == TransitionType.WIPE_LEFT:
            return self._build_wipe(
                clip_a_path, clip_b_path, output_path, config.duration, "wipeleft"
            )
        elif transition_type == TransitionType.WIPE_RIGHT:
            return self._build_wipe(
                clip_a_path, clip_b_path, output_path, config.duration, "wiperight"
            )
        elif transition_type == TransitionType.WIPE_UP:
            return self._build_wipe(
                clip_a_path, clip_b_path, output_path, config.duration, "wipeup"
            )
        elif transition_type == TransitionType.WIPE_DOWN:
            return self._build_wipe(
                clip_a_path, clip_b_path, output_path, config.duration, "wipedown"
            )
        elif transition_type == TransitionType.SLIDE_LEFT:
            return self._build_slide(
                clip_a_path, clip_b_path, output_path, config.duration, "slideleft"
            )
        elif transition_type == TransitionType.SLIDE_RIGHT:
            return self._build_slide(
                clip_a_path, clip_b_path, output_path, config.duration, "slideright"
            )
        elif transition_type == TransitionType.SLIDE_UP:
            return self._build_slide(
                clip_a_path, clip_b_path, output_path, config.duration, "slideup"
            )
        elif transition_type == TransitionType.SLIDE_DOWN:
            return self._build_slide(
                clip_a_path, clip_b_path, output_path, config.duration, "slidedown"
            )
        elif transition_type == TransitionType.ZOOM_IN:
            return self._build_zoom(
                clip_a_path, clip_b_path, output_path, config.duration, "zoomin"
            )
        elif transition_type == TransitionType.ZOOM_OUT:
            return self._build_zoom(
                clip_a_path, clip_b_path, output_path, config.duration, "zoomout"
            )
        elif transition_type == TransitionType.CROSSFADE:
            return self._build_crossfade(
                clip_a_path, clip_b_path, output_path, config.duration
            )
        elif transition_type == TransitionType.IRIS:
            return self._build_iris(
                clip_a_path, clip_b_path, output_path, config.duration
            )
        elif transition_type == TransitionType.PIXELATE:
            return self._build_pixelate(
                clip_a_path, clip_b_path, output_path, config.duration
            )
        else:
            return self._build_dissolve(
    ...
    def _build_dissolve(
        self, clip_a: str, clip_b: str, output: str, duration: float
    ) -> List[str]:
        return [
            self.ffmpeg_path,
            "-y",
            ...
            clip_b,
            "-filter_complex",
            f"[0:v][1:v]xfade=transition=dissolve:duration={duration}:offset=0[v];[0:a][1:a]acrossfade=d={duration}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
    ...
    def _build_crossfade(
        ...
        return [
            ...
            clip_b,
            "-filter_complex",
            f"[0:v][1:v]xfade=transition=fade:duration={duration}:offset=0[v];[0:a][1:a]acrossfade=d={duration}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
    ...
    def _build_fade(
        self, clip_a: str, clip_b: str, output: str, duration: float, color: str
    ) -> List[str]:
        return [
            self.ffmpeg_path,
            "-y",
            ...
            clip_b,
            "-filter_complex",
            f"[0:v]fade=t=out:st=0:d={duration}[v0];[1:v]fade=t=in:st=0:d={duration}[v1];[v0][v1]concat=n=2:v=1:a=0[v]",
            "-map",
            "[v]",
            "-c:v",
            "libx264",
    ...
    def _build_wipe(
        self, clip_a: str, clip_b: str, output: str, duration: float, transition: str
    ) -> List[str]:
        return [
            self.ffmpeg_path,
            "-y",
            ...
            clip_b,
            "-filter_complex",
            f"[0:v][1:v]xfade=transition={transition}:duration={duration}:offset=0[v];[0:a][1:a]acrossfade=d={duration}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
    ...
    def _build_iris(
        ...
        return [
            ...
            clip_b,
            "-filter_complex",
            f"[0:v][1:v]xfade=transition=iris:duration={duration}:offset=0[v];[0:a][1:a]acrossfade=d={duration}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
    ...
    def _build_pixelate(
        ...
        return [
            ...
            clip_b,
            "-filter_complex",
            f"[0:v][1:v]xfade=transition=pixelize:duration={duration}:offset=0[v];[0:a][1:a]acrossfade=d={duration}[a]",
            "-map",
            "[v]",
            "-map",
            "[a]",
    ...
    def execute_transition(
        ...
        cmd = self.build_transition_command(
            clip_a_path, clip_b_path, transition_type, output_path, config
        )
    ...
    def validate_transition(
        self, transition_type: TransitionType, duration: float
    ) -> bool:
