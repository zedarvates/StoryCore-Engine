...
class StoryFeedback(BaseModel):
    ...
    feedback_type: str = Field(
        ..., description="Type of feedback: quality, structure, characters, plot, etc."
    )
...
class FeedbackAnalytics(BaseModel):
    ...
    common_issues: List[str] = Field(
        default_factory=list, description="Common issues identified"
    )
    improvement_suggestions: List[str] = Field(
        default_factory=list, description="Suggested improvements"
    )
    feedback_trend: Dict[str, float] = Field(
        default_factory=dict, description="Feedback trends over time"
    )
...
class StoryFeedbackIntegrationService:
    ...
    def __init__(self):
        from backend.feedback_proxy import FeedbackProxy
    ...
    async def submit_feedback(self, feedback: StoryFeedback) -> Tuple[bool, str]:
        ...
        try:
            ...
            if feedback.rating <= 3 or feedback.feedback_type in [
                "quality",
                "structure",
            ]:
                ...
                if issue_result[0]:
                    ...
                    await self._link_issue_to_feedback(
                        feedback.story_id, issue_result[1]
                    )
                    return (
                        True,
                        f"Feedback submitted and issue created: {issue_result[1]}",
                    )
                else:
                    return (
                        True,
                        f"Feedback submitted but issue creation failed: {issue_result[1]}",
                    )
    ...
    async def get_feedback_analytics(
        ...
        try:
            ...
            for feedback in feedbacks:
                issue_counts[feedback.feedback_type] = (
                    issue_counts.get(feedback.feedback_type, 0) + 1
                )
    ...
    async def _validate_feedback(self, feedback: StoryFeedback) -> bool:
        ...
        try:
            ...
            if feedback.rating < 1 or feedback.rating > 5:
                logger.warning(
                    f"Invalid rating {feedback.rating} for story {feedback.story_id}"
                )
                return False
    ...
    async def _save_feedback_to_db(self, feedback: StoryFeedback) -> None:
        ...
        logger.info(
            f"Would save feedback for story {feedback.story_id}: {feedback.feedback_type}"
        )
    ...
    async def _create_github_issue(self, feedback: StoryFeedback) -> Tuple[bool, str]:
        ...
        try:
            ...
            template = self.issue_templates.get(
                feedback.feedback_type, self.issue_templates["quality"]
            )
...
async def get_story_feedback_analytics(
