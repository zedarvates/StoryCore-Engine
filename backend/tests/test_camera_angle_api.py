...
class TestGenerateEndpoint:
    ...
    @patch("backend.camera_angle_api.get_camera_angle_service")
    def test_generate_service_error(
        self, mock_get_service, client, sample_generate_request
    ):
...
class TestValidation:
    ...
    def test_optional_fields_defaults(self, client, sample_generate_request):
        ...
        with patch(
            "backend.camera_angle_api.get_camera_angle_service"
        ) as mock_get_service:
            mock_service = AsyncMock()
            mock_service.generate_multiple_angles = AsyncMock(return_value="job-123")
            mock_get_service.return_value = mock_service
            ...
            response = client.post(
                "/camera-angle/generate", json=sample_generate_request
            )
