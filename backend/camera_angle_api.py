...
class GenerateRequest(BaseModel):
    ...
    image_base64: str = Field(
        ..., min_length=1, description="Base64 encoded source image"
    )
    angle_ids: List[CameraAnglePreset] = Field(
        ..., min_length=1, description="List of camera angle presets to generate"
    )
    preserve_style: bool = Field(
        default=True, description="Maintain original image style"
    )
    quality: str = Field(
    ...
    custom_prompt: Optional[str] = Field(
        default=None, description="Additional custom prompt"
    )
    comfyui_url: Optional[str] = Field(
        default=None, description="Optional ComfyUI server URL to use"
    )
...
def _estimate_generation_time(angle_count: int, quality: str) -> int:
    ...
    """
    base_times = {"draft": 5, "standard": 10, "high": 20}
...
async def generate_camera_angles(
    ...
    try:
        ...
        job_id = await service.generate_multiple_angles(
            request=camera_request, user_id=user_id
        )

        # Estimate processing time
        estimated_time = _estimate_generation_time(
            len(request.angle_ids), request.quality
        )
...
async def list_presets() -> CameraAnglePresetsResponse:
    ...
    return CameraAnglePresetsResponse(presets=presets, total=len(presets))
