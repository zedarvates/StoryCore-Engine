...
class SecurityValidator:
    ...
    def __init__(self):
        ...
        # Base64 data pattern
        self.base64_pattern = (
            r"^data:([a-zA-Z0-9]+\/[a-zA-Z0-9\-\+\.]+);base64,([a-zA-Z0-9\+\/]*={0,2})$"
        )
    ...
    def sanitize_input(self, value: Any, max_length: int = 1000) -> str:
        ...
        value_str = "".join(
            char for char in value_str if ord(char) >= 32 or ord(char) == 9
        )
    ...
    def validate_json_payload(
        self, payload: Dict[str, Any], max_depth: int = 10
    ) -> bool:
        """
    ...
    def validate_base64_data(self, data: str, max_size_mb: int = 10) -> str:
        ...
        if estimated_size > max_size_mb * 1024 * 1024:
            raise SecurityValidationError(
                f"Base64 data too large (max {max_size_mb}MB)"
            )
