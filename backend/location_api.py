...
def load_location(
    location_id: str, project_id: Optional[str] = None
    ...
    if project_id:
        try:
            ...
            logger.error(
                f"Error loading project location {project_id}/{location_id}: {e}"
            )
...
async def update_location(
    location_id: str, update_data: dict, project_id: Optional[str] = None
) -> LocationResponse:
    ...
    if not loc:
        raise HTTPException(
            ...
            + (f" in project {p_id}" if p_id else ""),
        )
