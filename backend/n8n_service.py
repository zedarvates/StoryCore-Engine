...
class N8nService:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/healthz", timeout=5) as response:
                    if response.status == 200:
                        ...
                        return {"status": "offline", "message": f"n8n returned status {response.status}"}
    ...
    async def list_workflows(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/api/v1/workflows", headers=headers) as response:
                    if response.status == 200:
    ...
    async def trigger_workflow(self, webhook_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def create_workflow(self, name: str, nodes: List[Dict[str, Any]], connections: Dict[str, Any]) -> Dict[str, Any]:
