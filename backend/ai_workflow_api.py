...
try:
    from src.async_task_queue import get_async_task_queue, TaskPriority, TaskState
...
async def submit_workflow_to_queue(
    workflow_id: str, coroutine, priority: int = 5, timeout_seconds: int = 3600
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Workflow {workflow_id} submitted to AsyncTaskQueue with priority {priority}"
        )
        return True
...
class WorkflowCreateRequest(BaseModel):
    ...
    steps: List[WorkflowStepType] = Field(
        ..., description="Séquence d'étapes à exécuter"
    )
    initial_context: Dict[str, Any] = Field(
        default={}, description="Contexte initial (prompts, paths, presets)"
    )
    ...
    try:
        ...
        if ASYNC_QUEUE_AVAILABLE:
            ...
            if success:
                ...
                logger.warning(
                    f"AsyncTaskQueue unavailable, using BackgroundTasks for workflow {instance.id}"
                )
        else:
            # Fallback to BackgroundTasks if AsyncTaskQueue is not available
            background_tasks.add_task(orchestrator.execute_workflow, instance.id)
            logger.info(
                f"Workflow {instance.id} submitted to BackgroundTasks (AsyncTaskQueue not available)"
            )
...
async def get_workflow_queue_details_route(
    workflow_id: str, user_id: str = Depends(verify_jwt_token)
):
...
async def cancel_workflow_route(
    ...
    if not success:
        raise HTTPException(
            status_code=500, detail="Failed to cancel workflow in queue"
        )
