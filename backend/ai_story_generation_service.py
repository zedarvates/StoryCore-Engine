...
from backend.database_models import Story, generate_uuid
...
class StoryGenerationRequest(BaseModel):
    ...
    with_critique: bool = Field(
        default=False, description="Enable multi-agent critique"
    )
    with_alignment_score: bool = Field(
        default=True, description="Enable detailed alignment scoring"
    )
    temperature: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Creativity temperature"
    )
    max_attempts: int = Field(
        default=3, ge=1, le=10, description="Maximum refinement attempts"
    )
...
class AIDrivenStoryGenerationService:
    ...
    def __init__(self):
        ...
        self.model_configs = {
            "creative": {"temperature": 0.8, "top_p": 0.9, "max_tokens": 2048},
            "balanced": {"temperature": 0.7, "top_p": 0.8, "max_tokens": 1536},
            "precise": {"temperature": 0.5, "top_p": 0.7, "max_tokens": 1024},
        }

    async def generate_story(
        self, req: StoryGenerationRequest
        ...
        while attempt <= max_attempts:
            try:
                logger.info(
                    f"Story generation attempt {attempt}/{max_attempts} for prompt: {current_req.prompt[:100]}..."
                )
                # Convert strings to Enums
                genre = getattr(StoryGenre, current_req.genre.upper(), StoryGenre.DRAMA)
                structure = getattr(
                    StoryStructure,
                    current_req.structure.upper(),
                    StoryStructure.THREE_ACT,
                )
                mode = getattr(
                    ProductionMode, current_req.mode.upper(), ProductionMode.FICTION
                )
                ...
                logger.info(
                    f"Successfully generated story: {story.title} (ID: {story_id})"
                )
                ...
                response = StoryGenerationResponse(
                    ...
                    alignment_report=story.alignment_report
                    if hasattr(story, "alignment_report")
                    else None,
                )
                ...
                if current_req.with_critique:
                    ...
                    if critique and refined_story:
                        ...
                        StoryRefinementRequest(story_id=story_id, feedback=critique)

                        refined_story = await self.refine_story(
                            story_id=story_id, feedback=critique
                        )
                        ...
                        if refined_story:
                            response = StoryRefinementResponse(
                                ...
                                critique=critique,
                                alignment_report=refined_story.alignment_report
                                if hasattr(refined_story, "alignment_report")
                                else None,
                                status="refined",
                            )
                ...
                try:
                    ...
                    logger.error(
                        f"Database save failed for story {story_id}: {db_err}",
                        exc_info=True,
                    )
        ...
        return StoryGenerationResponse(
            ...
            locations=[],
            scenes=[],
            critique="Story generation failed",
        )
    ...
    async def refine_story(self, story_id: str, feedback: str) -> Optional[Story]:
        ...
        try:
            ...
            refined_story = await self.service.refine_story(story_id, feedback)
    ...
    async def _get_story_critique(
        self, story_id: str, prompt: str, story: Story
    ) -> Tuple[Optional[str], Optional[Story]]:
        """
    ...
    async def _generate_ai_critique(
        self, story_id: str, prompt: str, story: Story
    ) -> str:
        """
        ...
        if (
            story.genre
            and story.genre.lower() != "drama"
            and "DRAMA" in str(story.genre)
        ):
        ...
        try:
            async for db in get_db():
                ...
                if existing_story:
                    ...
                    existing_story.alignment_report = (
