...
class VideoEnhancementService:
    ...
    def __init__(
        self, ffmpeg_path: str = "ffmpeg", models_path: str = None, gpu=None, cache=None
    ):
    ...
    def _ensure_ffmpeg(self):
        ...
        try:
            result = subprocess.run(
                [self.ffmpeg, "-version"], capture_output=True, timeout=10
            )
    ...
    def enhance_video(
        ...
        try:
            ...
            for i, enhancement in enumerate(enhancements):
                intermediate_output = output_path.replace(
                    ".", f"_{enhancement.type.value}_inter."
                )
                results["enhancements_applied"].append(
                    {"type": enhancement.type.value, "success": False}
                )
                ...
                if success:
                    ...
                    if callback:
                        callback(
                            enhancement.type.value, (i + 1) / len(enhancements) * 100
                        )
        ...
        try:
            ...
            if ffprobe:
                cmd = [
                    ...
                    "v:0",
                    "-show_entries",
                    "stream=width,height,avg_frame_rate,duration,codec_name,bit_rate",
                    "-of",
                    "json",
                    input_path,
                ]
    ...
    def _build_ffmpeg_command(
        self, input_path: str, output_path: str, filters: List[str] = None
    ) -> List[str]:
    ...
    def _apply_super_resolution(
        self, input_path: str, output_path: str, config: EnhancementConfig
    ) -> bool:
    ...
    def _apply_color_grading(
        ...
        if preset == "cinematic":
            ...
            filters.extend(["curves=vintage", f"saturation={0.8 + 0.2 * strength}"])
            ...
            filters.extend([f"hue=s=70 - {20 * strength}"])
        elif preset == "hdr":
            filters.extend(
                ["hdr=tonemap=hable", "zscale=transfer=bt2020-10: primaries=bt2020"]
            )
        else:  # natural
            filters.extend(
                [
    ...
    def _apply_stabilization(
        ...
        cmd = [
            ...
            input_path,
            "-vf",
            f"vidstabtransform=smoothing={int(smoothness * 30)}",
            "-c:v",
            "libx264",
            "-preset",
            "fast",
    ...
    def _apply_deinterlacing(self, input_path: str, output_path: str) -> bool:
    ...
    def _apply_deblurring(
        ...
        cmd = [
            ...
            input_path,
            "-vf",
            f"unsharp=5:5:{sharpen}:5:5:{sharpen}",
            "-c:v",
            "libx264",
            "-preset",
            "fast",
    ...
    def estimate_processing_time(
        self, input_path: str, enhancements: List[EnhancementConfig]
    ) -> Dict[str, Any]:
    ...
    def list_supported_enhancements(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "frame_interpolation_models": ["rife-v4", "rife-v3", "cain", "amf"],
    ...
    def apply_preset(self, input_path: str, output_path: str, preset_name: str) -> bool:
        """Appliquer un preset d'amélioration prédéfini"""
        presets = {
            "cinematic": [
                EnhancementConfig(
                    type=EnhancementType.COLOR_GRADING, strength=0.8, preset="cinematic"
                ),
                EnhancementConfig(
            ...
            "hdr_upscale": [
                EnhancementConfig(
                    type=EnhancementType.COLOR_GRADING, strength=0.5, preset="hdr"
                ),
                EnhancementConfig(
                    type=EnhancementType.SUPER_RESOLUTION,
                    strength=0.7,
            ...
            "denoise_pro": [
                EnhancementConfig(type=EnhancementType.NOISE_REDUCTION, strength=0.9),
                EnhancementConfig(
                    type=EnhancementType.COLOR_GRADING, strength=0.3, preset="natural"
                ),
            ],
            "vintage_look": [
                EnhancementConfig(
                    type=EnhancementType.COLOR_GRADING, strength=1.0, preset="vintage"
                ),
                EnhancementConfig(type=EnhancementType.NOISE_REDUCTION, strength=0.3),
            ],
        }
    ...
    def export_8k(
        ...
        try:
            for i, p in enumerate(passes):
                ...
                if p == "8K":
                    ...
                    cmd = self._build_ffmpeg_command(
                        current_input, pass_output, filters
                    )
            ...
            if os.path.exists(output_path):
                results["metadata"]["estimated_weight_mb"] = os.path.getsize(
                    output_path
                ) / (1024 * 1024)
