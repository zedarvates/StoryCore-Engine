...
from backend.llm_api import generate_text, LLMRequest, prompt_templates
...
class LyricsGenerationRequest(BaseModel):
    ...
    length: str = Field(
        "medium", description="Length of the lyrics (short, medium, long)"
    )
    characters: Optional[List[str]] = Field(
        None, description="Characters to include in the lyrics"
    )
...
class MusicStyleRequest(BaseModel):
    ...
    mood: List[str] = Field(default_factory=list, description="List of moods")
    instrumentation: Optional[List[str]] = Field(
        None, description="List of instruments"
    )
...
class MusicLyricsService:
    ...
    async def generate_lyrics(
        self, request: LyricsGenerationRequest, user_id: str = "system"
    ) -> LyricsResponse:
        """
        Generate song lyrics based on theme, style, and mood.
        """
        logger.info(
            f"Generating lyrics for theme: {request.theme}, style: {request.style}"
        )
        ...
        if request.characters:
            variables["theme"] += (
                f" (featuring characters: {', '.join(request.characters)})"
            )

        if request.shots:
            variables["theme"] += (
                f" (integrated with shots: {', '.join(request.shots)})"
            )
  ...
  "structure": {{
    ...
    "key": "C Major"
        ...
        llm_request = LLMRequest(
            prompt=prompt, model=self.default_model, temperature=0.7
        )
    ...
    async def generate_music_style(
        self, request: MusicStyleRequest, user_id: str = "system"
    ) -> MusicStyleResponse:
        """
        Generate precise musical style prompts for Suno/Udio.
        """
        logger.info(
            f"Generating music style for: {request.prompt}, style: {request.style}"
        )
...
{{
  ...
  "technical_specs": {{
    ...
    "energy": "high"
        ...
        llm_request = LLMRequest(
            prompt=prompt, model=self.default_model, temperature=0.5
        )
