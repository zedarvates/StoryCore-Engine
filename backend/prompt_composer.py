...
from backend.voice_profile_builder import VoiceProfile
...
class PromptComposer:
    ...
    def compose_music_track_prompt(
        self, profile: MusicProfile, track_type: MusicTrackType
    ) -> GeneratedPrompt:
        """
        ...
        """
        # Get track info
        track = next((t for t in profile.tracks if t.track_type == track_type), None)
    ...
    def _compose_bass_prompt(self, profile: MusicProfile) -> str:
        ...
        if profile.action:
            ...
            bass_rules.append(
                "phase-locked" if profile.action.bass.phase_locked else ""
            )
    ...
    def compose_sfx_track_prompt(
        self, profile: SFXProfile, track_type: SFXTrackType
    ) -> GeneratedPrompt:
    ...
    def _compose_sfx_bullet_time_prompt(self, profile: SFXProfile) -> str:
        ...
        muffling_ctx = (
            "sous l'eau" if profile.muffling.context == "underwater" else "bullet time"
        )
        return (
    ...
    def compose_voice_prompt(self, profile: VoiceProfile) -> GeneratedPrompt:
        ...
        name, style = voice_type_info.get(profile.voice_type, ("Voix", "voix"))
    ...
    def export_prompts_json(
        self, prompts: Dict[str, List[GeneratedPrompt]]
    ) -> Dict[str, Any]:
