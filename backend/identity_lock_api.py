...
from backend.identity_lock_service import get_identity_lock_service, IdentityProfile
from backend.identity_extraction import get_extraction_service, validate_image_path
...
class VisualAttributesModel(BaseModel):
    ...
    face_shape: str = Field(
        default="", description="Forme du visage: oval, round, square, heart"
    )
    ...
    distinctive_features: List[str] = Field(
        default_factory=list, description="Traits distinctifs"
    )
    scars_marks: List[str] = Field(
        default_factory=list, description="Cicatrices et marques"
    )
    extraction_confidence: float = Field(
        default=0.0, description="Score de confiance de l'extraction"
    )
...
class CreateIdentityRequest(BaseModel):
    ...
    name: str = Field(
        ..., description="Nom du personnage", min_length=1, max_length=100
    )
    description: str = Field(
        default="", description="Description du personnage", max_length=1000
    )
    project_id: str = Field(..., description="ID du projet associé")
    source_image_path: Optional[str] = Field(
        default=None, description="Chemin vers l'image source optionnelle"
    )
...
class UpdateIdentityRequest(BaseModel):
    ...
    visual_attributes: Optional[Dict[str, Any]] = Field(
        default=None, description="Attributs visuels à mettre à jour"
    )
...
class ExtractAttributesRequest(BaseModel):
    ...
    use_llm: bool = Field(
        default=True, description="Utiliser LLM Vision pour l'extraction"
    )
...
class ApplyIdentityRequest(BaseModel):
    ...
    scene_type: str = Field(
        default="default",
        description="Type de scène: default, close_up, medium_shot, full_body, action, emotional, outdoor, indoor, night",
    )
...
async def create_identity(
    request: CreateIdentityRequest, user_id: str = Depends(verify_jwt_token)
):
    """
...
async def list_identities(
    project_id: Optional[str] = Query(
        default=None, description="Filtrer par ID de projet"
    ),
...
async def get_identity(identity_id: str, user_id: str = Depends(verify_jwt_token)):
    ...
    try:
        ...
        if hasattr(identity, "owner_id") and identity.owner_id != user_id:
            raise HTTPException(
                status_code=403, detail="Access denied to this identity"
            )
        return _identity_to_response(identity)
...
async def import_identity(
    request: ImportIdentityRequest, user_id: str = Depends(verify_jwt_token)
):
    """
...
async def update_visual_attributes(
    ...
    attributes: Dict[str, Any] = Body(
        ..., description="Attributs visuels à mettre à jour"
    ),
    ...
    try:
        ...
        updated_identity = await service.update_visual_attributes(
            identity_id=identity_id, attributes=attributes, confidence=confidence
        )
        return _identity_to_response(updated_identity)
    except ValueError as e:
        if "not found" in str(e):
...
async def extract_attributes_from_image(
    request: ExtractAttributesRequest, user_id: str = Depends(verify_jwt_token)
):
    """
...
async def batch_extract_attributes(
    request: BatchExtractRequest, user_id: str = Depends(verify_jwt_token)
):
    """
    ...
    try:
        ...
        for image_path in request.image_paths:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid path '{image_path}': {str(e)}"
                )

        extraction_service = get_extraction_service()
        results = await extraction_service.batch_extract(
            image_paths=validated_paths, merge_results=request.merge_results
        )
...
async def create_and_extract_identity(
    request: CreateAndExtractRequest, user_id: str = Depends(verify_jwt_token)
):
    """
    ...
    try:
        ...
        extraction_service = get_extraction_service()
        extraction_result = await extraction_service.extract_attributes(
        ...
        identity = await identity_service.extract_and_lock_attributes(
            identity_id=identity.id, image_path=validated_image_path, use_llm=True
        )
