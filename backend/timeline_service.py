...
class TimelineService:
    ...
    def create_timeline(self, name: str, **kwargs) -> Timeline:
        ...
        timeline = Timeline(id=str(uuid.uuid4()), name=name, **kwargs)
        self.timelines[timeline.id] = timeline
        return timeline

    def add_track(
        self, timeline_id: str, name: str, track_type: ClipType
    ) -> TimelineTrack:
        """Ajouter une piste au timeline"""
        track = TimelineTrack(id=str(uuid.uuid4()), name=name, type=track_type)
        self.timelines[timeline_id].tracks.append(track)
        return track

    def add_clip(
        self, timeline_id: str, track_id: str, clip_data: dict
    ) -> TimelineClip:
    ...
    def ripple_move_clip(
        self, timeline_id: str, clip_id: str, new_start: float
    ) -> bool:
        """
    ...
    def get_track_gaps(self, timeline_id: str, track_id: str) -> List[Dict[str, float]]:
        ...
        if sorted_clips[0].start_time > 0:
            gaps.append(
                {
                    "start": 0.0,
                    "end": sorted_clips[0].start_time,
                    "duration": sorted_clips[0].start_time,
                }
            )
    ...
    def auto_assemble_sequence(
        self, timeline_id: str, shots: List[Dict[str, Any]]
    ) -> bool:
        """
    ...
    def fill_track_gaps(
        self, timeline_id: str, track_id: str, media_info: Dict[str, Any]
    ) -> List[str]:
        """
    ...
    def render_preview(
        self, timeline_id: str, start_time: float = 0, end_time: Optional[float] = None
    ) -> str:
    ...
    def export_timeline(
        self, timeline_id: str, output_path: str, format: str = "mp4", **kwargs
    ) -> str:
