...
class RateLimiter:
    ...
    def check(self, endpoint: str, client_id: str) -> RateLimitResult:
        ...
        with self._locks[key]:
            ...
            self._requests[key] = [
                req_time for req_time in self._requests[key] if req_time > cutoff_time
            ]
...
def setup_rate_limits():
    ...
    # Default config
    rate_limiter.set_default_config(
        RateLimitConfig(requests_per_window=100, window_seconds=60, burst_size=10)
    )

    # Specific endpoint configurations
    rate_limiter.configure(
        "/api/generate",
        RateLimitConfig(requests_per_window=10, window_seconds=60, burst_size=2),
    )

    rate_limiter.configure(
        "/api/comfyui/workflow",
        RateLimitConfig(requests_per_window=5, window_seconds=60, burst_size=1),
    )

    rate_limiter.configure(
        "/api/tasks",
        RateLimitConfig(requests_per_window=50, window_seconds=60, burst_size=5),
    )
