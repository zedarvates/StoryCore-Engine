...
class StoryAlignmentScorer:
    ...
    def __init__(
        self, pass_threshold: float = 70.0, call_llm_func: Optional[Any] = None
    ):
    ...
    async def score_story(
        ...
        """
        logger.info(
            f"Starting alignment scoring for story: {story_data.get('title', 'Unknown')}"
        )
        ...
        # 1. Prompt Alignment
        categories[
            ScoringCategory.PROMPT_ALIGNMENT
        ] = await self._score_prompt_alignment(story_data, original_prompt)

        # 2. Character Consistency
        categories[
            ScoringCategory.CHARACTER_CONSISTENCY
        ] = await self._score_character_consistency(story_data)

        # 3. Genre Conformity
        categories[
            ScoringCategory.GENRE_CONFORMITY
        ] = await self._score_genre_conformity(story_data, target_genre)

        # 4. Structural Integrity
        categories[
            ScoringCategory.STRUCTURAL_INTEGRITY
        ] = await self._score_structural_integrity(story_data, target_structure)
        ...
        total_score = sum(
            categories[cat].score * weights.get(cat, 0.2) for cat in categories
        )
    ...
    async def _score_prompt_alignment(
        self, story_data: Dict[str, Any], original_prompt: str
    ) -> ScoreResult:
    ...
    async def _score_character_consistency(
    ...
    async def _score_genre_conformity(
        self, story_data: Dict[str, Any], target_genre: str
    ) -> ScoreResult:
    ...
    async def _score_structural_integrity(
        self, story_data: Dict[str, Any], target_structure: str
    ) -> ScoreResult:
