...
def get_payload_size_breakdown(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    screenshot_base64_bytes = (
        len(screenshot_base64.encode("utf-8")) if screenshot_base64 else 0
    )
    screenshot_decoded_bytes = calculate_screenshot_decoded_size(screenshot_base64)

    # Calculate logs size
    logs = (
        payload.get("diagnostics", {}).get("logs", [])
    ...
    stacktrace = (
        payload.get("diagnostics", {}).get("stacktrace")
        if isinstance(payload.get("diagnostics"), dict)
        else None
    )
...
def validate_screenshot_size(
    ...
    if decoded_size > max_screenshot_size:
        ...
        error_message = (
            f"Screenshot size ({size_mb:.2f} MB) exceeds maximum allowed size of 5 MB"
        )
