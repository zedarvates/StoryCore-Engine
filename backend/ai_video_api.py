...
async def list_lens_types():
    ...
    return {
        "lenses": [
            {"id": k, "description": v}
            for k, v in service.lens_effects.items()
        ]
    }
...
async def generate_character_sheet(request: CharacterSheetRequest):
    ...
    try:
        ...
        config = CharacterSheetConfig(
            ...
            views=views,
            expressions=request.expressions or ["neutral", "happy", "angry", "surprised"],
...
async def create_consistency_prompt(request: ConsistencyPromptRequest):
    ...
    try:
        if not os.path.exists(request.reference_image_path):
            raise HTTPException(status_code=404, detail=f"Reference image not found: {request.reference_image_path}")
...
async def create_smooth_cut(request: SmoothCutRequest):
    ...
    try:
        if not os.path.exists(request.video1_path):
            raise HTTPException(status_code=404, detail=f"Video 1 not found: {request.video1_path}")
        if not os.path.exists(request.video2_path):
            raise HTTPException(status_code=404, detail=f"Video 2 not found: {request.video2_path}")
