...
class VoiceService:
    ...
    EMOTIONS: Dict[str, VoiceEmotion] = {
        "neutral": VoiceEmotion(
            name="neutral", pitch_modifier=0, speed_modifier=1.0, volume_modifier=0
        ),
        "happy": VoiceEmotion(
            name="happy", pitch_modifier=2, speed_modifier=1.1, volume_modifier=1
        ),
        "sad": VoiceEmotion(
            name="sad", pitch_modifier=-2, speed_modifier=0.9, volume_modifier=-2
        ),
        "angry": VoiceEmotion(
            name="angry", pitch_modifier=-3, speed_modifier=1.2, volume_modifier=2
        ),
        "fear": VoiceEmotion(
            name="fear", pitch_modifier=4, speed_modifier=1.3, volume_modifier=-1
        ),
        "surprised": VoiceEmotion(
            name="surprised", pitch_modifier=3, speed_modifier=1.15, volume_modifier=0
        ),
        "whisper": VoiceEmotion(
            name="whisper", pitch_modifier=0, speed_modifier=0.8, volume_modifier=-6
        ),
        "deep": VoiceEmotion(
            name="deep", pitch_modifier=-4, speed_modifier=0.95, volume_modifier=1
        ),
        "robotic": VoiceEmotion(
            name="robotic", pitch_modifier=-2, speed_modifier=1.0, volume_modifier=0
        ),
    }
    ...
    async def generate_voice(
        ...
        try:
            ...
            if voice not in available_voices:
                logger.warning(
                    f"Voice {voice} not available for {language}, using default"
                )
...
class VoiceProfileBuilder:
    ...
    def set_voice_type(self, voice_type: str) -> "VoiceProfileBuilder":
        ...
        self._voice_type = (
            voice_type.lower() if voice_type.lower() in valid_types else "raw"
        )
        return self
    ...
    def set_voice(self, voice: str, language: str = "fr-FR") -> "VoiceProfileBuilder":
    ...
    def set_emotion(
        self, emotion: str, speed: float = 1.0, pitch: float = 0.0, volume: float = 0.0
    ) -> "VoiceProfileBuilder":
    ...
    def set_synthesis_params(
        self, speed: float = 1.0, pitch: float = 0.0, volume: float = 0.0
    ) -> "VoiceProfileBuilder":
    ...
    def set_eq(
        ...
        """Set EQ for voice"""
        self.add_filter(
            "eq", {"low_gain": low_gain, "mid_gain": mid_gain, "high_gain": high_gain}
        )
        return self
    ...
    def set_reverb(
        self, room_size: float = 0.3, damping: float = 0.5, wet_level: float = 0.2
    ) -> "VoiceProfileBuilder":
    ...
    def set_distortion(
        self, amount: float = 0, mix: float = 0.5
    ) -> "VoiceProfileBuilder":
        """Set distortion for voice"""
        self.add_filter("distortion", {"amount": amount, "mix": mix})
        return self

    def set_band_pass(
        self, center_freq: float = 1000, q: float = 1.0
    ) -> "VoiceProfileBuilder":
        """Set band pass"""
        self.add_filter("band_pass", {"center_frequency": center_freq, "q": q})
        return self
    ...
    def build(self) -> VoiceProfile:
        ...
        logger.info(
            f"VoiceProfile {profile.id} built successfully with {len(tracks)} tracks"
        )
        return profile
