...
async def generate_mask(request: GenerateMaskRequest):
    ...
    try:
        if not os.path.exists(request.input_path):
            raise HTTPException(
                status_code=404, detail=f"Input not found: {request.input_path}"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Unknown mask type: {request.mask_type}"
            )

        success, message = service.generate_mask_frame(
            request.input_path, request.output_path, mask_type, request.refine_edges
        )
...
async def generate_depth_prompt(
    image_path: str, depth_path: str, style: str = "cinematic"
):
    """
...
async def list_bloom_presets():
    ...
    return {
        "presets": [
            ...
            {
                "id": "anamorphic",
                "description": "Horizontal streaks",
                "threshold": 0.7,
                "radius": 40,
                "strength": 0.6,
                "ratio": 2.35,
...
async def transcribe_video(request: TranscribeRequest):
    ...
    try:
        ...
        success, message = service.transcribe_video(
            request.video_path, request.output_srt, request.language
        )
...
async def translate_subtitles(srt_path: str, output_path: str, target_language: str):
    """
    ...
    try:
        if not os.path.exists(srt_path):
            raise HTTPException(
                status_code=404, detail=f"SRT file not found: {srt_path}"
            )

        service = get_subtitle_service()

        success, message = service.translate_subtitles(
            srt_path, output_path, target_language
        )
...
async def list_subtitle_styles():
    ...
    return {
        "styles": [
            ...
            {
                "id": "netflix",
                "description": "Netflix-style bold subtitles",
                "font_size": 28,
            },
            {
                "id": "youtube",
                "description": "YouTube-style with shadow",
                "font_size": 22,
            },
            {
                "id": "cinematic",
                "description": "Large text with background box",
                "font_size": 32,
            },
            {
                "id": "minimal",
                "description": "Small text with thin outline",
                "font_size": 20,
            },
