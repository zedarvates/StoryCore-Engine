...
class KritaLayerService:
    ...
    @staticmethod
    def extract_layer_image(kra_path: str, internal_path: str, tint_hex: Optional[str] = None) -> Optional[str]:
        """
    ...
    def _apply_tint(image: Image.Image, hex_color: str) -> Image.Image:
        ...
        color_layer = Image.new("RGB", image.size, tint_rgb)
    ...
    def _get_tint_for_layer(name: str, overrides: Dict[str, str]) -> Optional[str]:
        ...
        # Vegetation
        if any(w in name_low for w in ["vegetation", "arbre", "tree", "foret", "forest", "plante", "jungle"]):
            return overrides.get("Vegetation", "#228B22") # Forest Green
            
        # Architecture & Urban
        if any(w in name_low for w in ["architecture", "batiment", "building", "mur", "wall", "ville", "city", "ruine"]):
            return overrides.get("Architecture", "#808080") # Concrete Gray
            
        # Vehicles
        if any(w in name_low for w in ["vehicule", "vehicle", "voiture", "car", "vaisseau", "ship", "avion"]):
            return overrides.get("Vehicule", "#A52A2A") # Metallic / Generic
            
        # Animals & Creatures
        if any(w in name_low for w in ["animal", "creature", "faune", "wildlife", "chien", "chat", "dog", "cat"]):
            return overrides.get("Animal", "#DEB887") # Fur / Skin
            
        # Liquids & Transparency
        if any(w in name_low for w in ["eau", "water", "ocean", "lac", "lake", "riviere", "river"]):
            return overrides.get("Eau", "#1E90FF") # DodgerBlue
        if any(w in name_low for w in ["verre", "glass", "reflexion", "reflection", "miroir"]):
            return overrides.get("Verre", "#F0F8FF") # AliceBlue (Translucent look)
            
        # Debris & Geology
        if any(w in name_low for w in ["detritus", "debris", "dechet", "garbage", "trash", "casse"]):
            return overrides.get("Detritus", "#8B4513") # SaddleBrown (Rust)
        if any(w in name_low for w in ["rocher", "roche", "rock", "pierre", "stone", "montagne"]):
    ...
    @staticmethod
    def get_full_composition(kra_path: str, narrative_overrides: Dict[str, str] = None) -> List[Dict[str, Any]]:
        """
        ...
        for layer in layers:
            ...
            img_b64 = KritaLayerService.extract_layer_image(kra_path, layer['internal_path'], tint)
