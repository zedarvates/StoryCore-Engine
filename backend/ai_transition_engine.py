...
from backend.transitions_service import TransitionsService, TransitionConfig
...
class AITransitionEngine:
    ...
    def __init__(
        ...
        self.connection = ComfyUIConnectionManager(
            base_url=self.comfy_config.url,
    ...
    async def generate_transition(
        ...
        logger.info(f"Génération de transition {transition_type.value} entre {clip_a_path} et {clip_b_path}")
        
        try:
            if transition_type == AITransitionType.SMART_FADE:
                return await self._generate_smart_fade(clip_a_path, clip_b_path, output_path, config)
            elif transition_type == AITransitionType.INTERPOLATE:
                return await self._generate_interpolation(clip_a_path, clip_b_path, output_path, config)
            elif transition_type == AITransitionType.MORPH:
                return await self._generate_morph(clip_a_path, clip_b_path, output_path, config)
            else:
    ...
    async def _execute_comfy_transition(
        ...
        try:
            ...
            result = await self.executor.wait_for_completion(prompt_id, timeout=config.duration * 300)
            ...
            success = self._assemble_clip_from_single_image(str(temp_output), output, config.duration, config.fps)
    ...
    def _assemble_clip_from_single_image(self, image_path: str, output_path: str, duration: float, fps: int) -> bool:
        """Crée un clip vidéo à partir d'une seule image répétée."""
        cmd = [
            self.ffmpeg_path, "-y", "-loop", "1", "-i", image_path,
    ...
    async def _extract_last_frame(self, clip_path: str) -> Optional[str]:
        ...
        cmd = [
            self.ffmpeg_path, "-y", "-sseof", "-0.1", "-i", clip_path,
            "-update", "1", "-q:v", "2", output_frame
        ]
        try:
            subprocess.run(cmd, check=True, capture_output=True)
            return output_frame
    ...
    def _assemble_clip(self, frames_dir: str, output_path: str, fps: int) -> bool:
        ...
        cmd = [
            self.ffmpeg_path, "-y", "-framerate", str(fps),
