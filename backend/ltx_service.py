...
class LTXVideoService:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(
                    f"{self.comfyui_url}/prompt", json=params
                ) as resp:
                    if resp.status != 200:
                        ...
                        return {
                            "job_id": job_id,
                            "status": "failed",
                            "error": f"ComfyUI Error: {error_text}",
                        }
                ...
                # 3. Wait for result
                logger.info(
                    f"LTX Job {job_id} submitted. Prompt ID: {prompt_id}. Waiting for completion..."
                )
                self._jobs[job_id]["status"] = "processing"
                ...
                final_path = await self._process_outputs(
                    session, result_data, output_path
                )
    ...
    def _prepare_workflow(self, config: LTXGenerationConfig) -> Dict[str, Any]:
        ...
        workflow = {
            "1": {
                "inputs": {"ckpt_name": "ltx-video-2.3.safetensors"},
            ...
            "4": {
                "inputs": {"text": config.negative_prompt, "clip": ["1", 1]},
                "class_type": "CLIPTextEncode",
            },
            "5": {
                "inputs": {
            ...
            "7": {
                "inputs": {"filename_prefix": "LTX23_StoryCore", "images": ["6", 0]},
    ...
    async def _poll_result(
        self, session, prompt_id: str, job_id: str
    ) -> Dict[str, Any]:
    ...
    async def _process_outputs(
        self, session, result_data: Dict[str, Any], output_path: Optional[str]
    ) -> str:
        ...
        for node_id, node_output in outputs.items():
            if "gifs" in node_output:
                ...
                async with session.get(
                    f"{self.comfyui_url}/view", params={"filename": filename}
                ) as resp:
                    if resp.status == 200:
                        ...
                        if not output_path:
                            output_path = str(
                                self.output_dir / f"ltx_{uuid.uuid4()[:8]}.mp4"
                            )
    ...
    def _generate_mock_result(
        self, job_id: str, config: LTXGenerationConfig
    ) -> Dict[str, Any]:
