...
class LocationConstraint:
    ...
    impact_on_function: (
        str  # How this constraint affects the location's primary function
    )
...
class StoryAnalyzer:
    def __init__(self, raw_story: str):
        ...
        self.lines = [line.strip() for line in raw_story.split("\n") if line.strip()]
    ...
    def _generate_pitch_with_llm(self) -> str:
        ...
        try:
            ...
            request = LLMRequest(
                prompt=prompt, model="gpt-4", temperature=0.7, max_tokens=200
            )
            response = asyncio.get_event_loop().run_until_complete(
                call_llm_mock(request, "system")
            )
    ...
    def _generate_pitch_heuristic(self) -> str:
        ...
        conflicts = [
            w
            for w in ["conflit", "combat", "mystere", "danger", "quete", "amour"]
            if w in story_lower
        ]
    ...
    def extract_theme(self) -> tuple:
        ...
        themes_map = {
            "amour": ["amour", "passion", "coeur"],
            "courage": ["courage", "heroisme"],
            "survie": ["survie", "danger"],
            "justice": ["justice", "verite"],
            "identite": ["identite", "qui suis-je"],
            "pouvoir": ["pouvoir", "autorite"],
            "liberte": ["liberte", "emancipation"],
            "famille": ["famille", "heritage"],
            "redemption": ["redemption", "pardon"],
            "mystere": ["mystere", "secret"],
        }
        themes = []
    ...
    def extract_ton(self) -> tuple:
        story = self.raw_story.lower()
        tones_map = {
            "sombre": ["sombre", "mort", "tragique"],
            "humoristique": ["drole", "humour"],
            "epique": ["epique", "heroique"],
            "romantique": ["amour", "passion"],
            "mysterieux": ["mystere", "secret"],
            "dramatique": ["drame", "tension"],
            "terrorisant": ["terreur", "horreur"],
            "nostalgique": ["nostalgie", "souvenir"],
    ...
    def extract_enjeux(self) -> List[str]:
        story = self.raw_story.lower()
        enjeux = []
        if any(w in story for w in ["mort", "survie"]):
            enjeux.append("Enjeu de vie ou de mort")
        if any(w in story for w in ["amour", "passion"]):
            enjeux.append("Amour et attachement")
        if any(w in story for w in ["pouvoir", "autorite"]):
            enjeux.append("Pouvoir et autorite")
        if any(w in story for w in ["justice", "verite"]):
            enjeux.append("Justice et verite")
        if any(w in story for w in ["liberte", "prison"]):
            enjeux.append("Liberte et captivate")
        if any(w in story for w in ["famille", "heritage"]):
            enjeux.append("Famille et heritage")
        if any(w in story for w in ["secret", "mystere"]):
            enjeux.append("Secret et mysterieux")
...
class StoryTransformer:
    def __init__(
        self, raw_story: str, title: Optional[str] = None, mode: str = "fiction"
    ):
        self.raw_story = raw_story
    ...
    def transform(self) -> StructuredScenario:
        ...
        logger.info(
            f"Transformation complete: {len(self.characters_data)} chars, {len(self.locations_data)} locs, {len(scenes)} scenes"
        )
    ...
    def _build_detailed_characters(self, names: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, name in enumerate(names, 1):
            role = (
                CharacterRole.PROTAGONISTE.value
                if i == 1
                else (
                    CharacterRole.ANTAGONISTE.value
                    if i == 2
                    else CharacterRole.ALLIE.value
                    if i == 3
                    else CharacterRole.FIGURANT.value
                )
            )
            chars.append(
                {
                    ...
                    "age": "Inconnu",
                    "role": role,
                    "description": "Personnage important de l'histoire",
    ...
    def _build_detailed_locations(self, names: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, name in enumerate(names, 1):
            is_interieur = any(
                w in name.lower()
                for w in ["maison", "bureau", "laboratoire", "appartement", "immeuble"]
            )
            locs.append(
                {
                    "id": i,
                    "nom": name,
                    "type": LieuType.INTERIEUR.value
                    if is_interieur
                    else LieuType.EXTERIEUR.value,
        ...
        return [
            {
                ...
                "duree_approx": "5-10 min",
                "personnages": [c["nom"] for c in self.characters_data],
                "personnages_ids": [c["id"] for c in self.characters_data],
                "resume": f"Sequence {title}",
                "description_detaillee": f"Description de {title}",
                "actions_cles": [],
                "dialogues_cles": [],
                "tension": TensionLevel.MOYENNE.value,
                "progression_narrative": f"Progression de {title}",
                "scenes_ids": [],
            }
        ...
        for seq in sequences:
            for i in range(2):
                scene = {
                    ...
                    "lieu_id": seq["lieu_id"],
                    "moment": MomentType.JOUR.value
                    if i % 2 == 0
                    else MomentType.NUIT.value,
                    "duree_estimee": "2-5 min",
                    "description": f"Scene {i + 1} de {seq['titre']}",
                    "description_visuelle": "Visuel de la scene",
                    "moment_narratif": MomentNarratif.DEVELOPPEMENT.value,
                    "actions": [
                        {
                            "ordre": 1,
                            "description": "Action principale",
                            "personnages": [],
                            "camera": {"mouvement": "", "angle": "", "plan": ""},
                            "dialogues": [],
                        }
                    ],
                    "elements_visuels": [
                        {"type": "decor", "description": "", "importance": "moyenne"}
                    ],
                    "ambiance_sonore": {
                        "musique": "",
                        "bruitages": [],
                    ...
                    "liens": {
                        "scene_precedente": max(0, scene_id - 1),
                        "scene_suivante": scene_id + 1,
                        "type_lien": "continuite",
                    },
...
def transform_story_to_scenario(
    raw_story: str, title: Optional[str] = None, mode: str = "fiction"
) -> StructuredScenario:
...
def generate_logic_loop_location(
    ...
    is_interieur = any(
        w in location_name.lower()
        for w in ["maison", "bureau", "laboratoire", "appartement", "chambre", "salle"]
    )
    ...
    function_description = _build_function_description(
        location_name, function, sub_function
    )
    ...
    significance_narrative = _derive_significance(
        location_name, function, constraints, story_context
    )
...
def _build_function_description(name: str, function: str, sub_function: str) -> str:
    ...
    return func_descriptions.get(
        sub_function,
        f"{name} serves a specific purpose defined by its function as a {function} location.",
    )


def _generate_culture(
    function: str, constraints: List[Dict[str, Any]], tone: str
) -> Dict[str, Any]:
    ...
    for constraint in constraints:
        ...
        if ctype == "environmental":
            ...
            valued_skills.extend(
                ["Conservation", "Resource management", "Trade negotiation"]
            )
    ...
    danger_count = sum(
        1 for c in constraints if c.get("severity") in ["high", "critical"]
    )

    if danger_count > 0:
        worldview = "A people who understand that survival requires constant vigilance. They know the world is harsh and only the prepared endure."
    else:
        worldview = "A people shaped by their purpose. They see their role in the world as essential to something larger than themselves."
    ...
    if tone == "dark":
        ...
        attitude = (
            "They believe every challenge can be overcome with ingenuity and community."
        )
    else:
    ...
    if env_pressure:
        ...
        relationship = (
            "Harmony with their surroundings, shaped by generations of adaptation."
        )
...
def _generate_reputation(
    function: str, constraints: List[Dict[str, Any]], culture: Dict[str, Any], name: str
) -> Dict[str, Any]:
    ...
    func_reputations = {
        ...
        "logistical": "A necessary stop on the road. Travelers know to expect shelter but not luxury.",
    }

    external = func_reputations.get(
        function, f"{name} is known among those who travel here."
    )
    ...
    # Perceived danger
    danger_severities = [
        c.get("severity") for c in constraints if c.get("type") == "external_threat"
    ]
    if "critical" in danger_severities or "high" in danger_severities:
        perceived_danger = (
            "Known as genuinely dangerous. Outsiders approach with real caution."
        )
    elif danger_severities:
        perceived_danger = (
            "Respected as having real risks, but manageable for the prepared."
        )
    else:
    ...
    # Pride or shame
    pride_shame = "Source of quiet pride. They know what they've built and what it costs to maintain."
    ...
    # Who avoids
    who_avoids = [
        "Those who fear genuine challenge",
        "The comfortable who prefer certainty",
    ]
...
def _generate_emergent_details(
    ...
    func_landmarks = {
        "economic": [
            {
                "name": "Market Square",
                "description": "The heart of commerce where deals are struck",
                "significance": "Center of all economic activity",
            },
            {
                "name": "Merchant's Guild Hall",
                "description": "Where traders gather to set prices",
                "significance": "Power center of the economy",
            },
        ],
        "defensive": [
            {
                "name": "The Walls",
                "description": "Imposing barriers that have stood for generations",
                "significance": "Symbol of security and determination",
            },
            {
                "name": "Watchtower",
                "description": "The highest point, watching for threats",
                "significance": "Eyes of the community",
            },
        ],
        "social": [
            {
                "name": "The Sacred Site",
                "description": "Where spiritual practices occur",
                "significance": "Heart of the community's faith",
            },
            {
                "name": "Hall of Gathering",
                "description": "Where communities meet and decide",
                "significance": "Center of social life",
            },
        ],
        "logistical": [
            {
                "name": "The Resting Grounds",
                "description": "Where travelers camp and recover",
                "significance": "Essential to the location's purpose",
            },
            {
                "name": "Supply Depot",
                "description": "Storage for provisions",
                "significance": "What makes continued travel possible",
            },
        ],
    }
    ...
    for lm in landmarks[:2]:
        notable_buildings.append(
            {"name": lm["name"], "type": "Landmark", "description": lm["description"]}
        )
    ...
    if function == "defensive":
        layout_principle = (
            "Built for defense first, with sightlines and barriers in mind."
        )
        geo_features = [
            "Natural barriers that enhance defense",
            "Elevated positions",
            "Water sources that can be controlled",
        ]
        ...
        geo_features = [
            ...
            "Natural harbors or roads",
            "Resource deposits nearby",
        ]
    ...
    architectural_style = style_by_genre.get(
        genre, "Practical construction suited to their needs"
    )

    color_palette = ["Earth tones", "Weathered surfaces", "Functional colors"]
    if function == "economic":
        color_palette = [
            "Rich colors of trade goods",
    ...
    common_materials = {
        ...
        "social": [
            "Sacred materials appropriate to faith",
...
def _generate_story_hooks(
    ...
    for constraint in constraints:
        ...
        if ctype == "external_threat":
            hooks.append(
                f"The external threat escalates: {constraint.get('description', 'The danger grows')}"
            )
        elif ctype == "resource_scarcity":
            hooks.append(
                "Resource scarcity reaches crisis levels. Someone must make a terrible choice."
            )
        elif ctype == "environmental":
            hooks.append(
                f"The environment becomes hostile: {constraint.get('description', 'Nature turns against them')}"
            )

    # Culture-based hooks
    if culture.get("secrets"):
        hooks.append(
            "A secret from the culture's past resurfaces, challenging everything they believed."
        )

    # Reputation-based hooks
    if reputation.get("reality_vs_rumor"):
        hooks.append(
            "An outsider discovers the truth behind the reputation—and decides what to do with it."
        )

    return hooks


def _derive_atmosphere(
    constraints: List[Dict[str, Any]], tone: str, function: str
) -> str:
    """Derive atmosphere from constraints, tone, and function"""

    # Base atmosphere on the most severe constraint
    severe_constraints = [
        c for c in constraints if c.get("severity") in ["high", "critical"]
    ]
...
def _derive_visual_elements(
    emergent: Dict[str, Any], function: str, constraints: List[Dict[str, Any]]
) -> List[str]:
    ...
    if emergent.get("landmarks"):
        elements.append("Prominent landmarks defining the skyline")
    ...
    for constraint in constraints:
        if constraint.get("type") == "environmental":
            elements.append("Evidence of environmental adaptation in every structure")
        elif constraint.get("type") == "external_threat":
            elements.append("Defensive features integrated into daily life")
...
def _derive_lighting(constraints: List[Dict[str, Any]], function: str) -> str:
    ...
    for constraint in constraints:
        if (
            "cave" in constraint.get("description", "").lower()
            or "underground" in constraint.get("description", "").lower()
        ):
...
def _build_history(
    ...
    if constraints:
        ...
        history += (
            "Over time, it grew and adapted to serve its purpose ever more effectively."
        )
...
def _derive_significance(
    name: str, function: str, constraints: List[Dict[str, Any]], story_context: str
) -> str:
    """Determine narrative significance"""
