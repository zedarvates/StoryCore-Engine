...
async def invoke_cli_command(request: CLIRequest):
    ...
    try:
        ...
        if not handler:
            return CLIResponse(success=False, error=f"Unknown command: {request.command}. Available: {cli.registry.list_commands()}")
        ...
        if not subparser:
            ...
            args = Namespace(
                verbose=False, quiet=True, log_level="INFO", 
                command=cmd_name, **request.args
            )
        else:
        ...
        try:
            ...
            if exit_code == 0:
                ...
                data = {"output": getattr(handler, 'last_output', output_text), "args": vars(args)}
                return CLIResponse(success=True, data=data)
            else:
                return CLIResponse(success=False, error=f"Command exited with code {exit_code}", data={"output": output_text})
        except Exception as e:
