...
class SegmentScriptRequest(BaseModel):
    ...
    target_duration: float = Field(8.0, description="Durée cible par segment en secondes", ge=4.0, le=20.0)
    language: str = Field("fr", description="Langue du script")
    words_per_minute: Optional[int] = Field(None, description="Vitesse de parole estimée", ge=80, le=250)
...
class RegenerateRequest(BaseModel):
    ...
    target_duration: Optional[float] = Field(None, description="Nouvelle durée cible", ge=4.0, le=20.0)
...
async def segment_script(request: SegmentScriptRequest):
    ...
    try:
        ...
        logger.info(f"Segmentation complete: {len(result.segments)} segments, {result.total_duration:.1f}s")
...
async def get_segmentation(segmentation_id: str):
    ...
    if result is None:
        raise HTTPException(status_code=404, detail=f"Segmentation not found: {segmentation_id}")
...
@router.put("/{segmentation_id}/segments/{segment_id}", response_model=SegmentationResponse)
async def adjust_segment(
    segmentation_id: str,
    segment_id: str,
    request: AdjustSegmentRequest
):
    """
...
async def get_statistics(segmentation_id: str):
    ...
    stats = {
        ...
        "natural_break_ratio": result.natural_breaks / (result.natural_breaks + result.forced_breaks) if (result.natural_breaks + result.forced_breaks) > 0 else 0,
