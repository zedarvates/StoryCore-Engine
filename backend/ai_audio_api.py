...
async def get_beat_aligned_cuts(request: BeatAlignedCutsRequest):
    ...
    try:
        ...
        cuts = service.get_beat_aligned_cuts(
            request.beat_times,
            request.target_durations,
            request.tolerance
        )
        
        return BeatAlignedCutsResponse(
            cuts=cuts,
            total_cuts=len(cuts)
        )
...
async def apply_auto_ducking(request: AutoDuckingRequest):
    ...
    try:
        ...
        if not os.path.exists(request.dialogue_path):
            raise HTTPException(status_code=404, detail=f"Dialogue file not found: {request.dialogue_path}")
...
async def auto_trim_silence(
    ...
    try:
        ...
        cmd = [
            "ffmpeg", "-y",
            "-i", input_path,
            "-af", f"silenceremove=stop_periods=-1:stop_threshold={silence_threshold_db}dB:stop_duration={min_silence_duration}",
            "-c:v", "copy" if input_path.endswith(('.mp4', '.mov', '.mkv')) else "none",
            "-c:a", "aac",
            "-b:a", "192k",
            output_path
        ]
