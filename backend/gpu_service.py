...
class GPUService:
    ...
    def build_gpu_encode_command(
        self, input_path: str, output_path: str, config: GPUConfig = None
    ) -> List[str]:
        ...
        cmd = [self.ffmpeg_path, "-y", "-hwaccel", config.hwaccel, "-i", input_path]
        ...
        if config.encoder == GPUEncoder.NVIDIA_NVENC:
            ...
            cmd.extend(["-c:v", "h264_qsv", "-preset", config.preset])
    ...
    def build_gpu_decode_command(
        self, input_path: str, output_path: str, decoder: GPUDecode = None
    ) -> List[str]:
    ...
    def build_transcode_with_gpu(
        self, input_path: str, output_path: str, gpu_config: GPUConfig = None
    ) -> List[str]:
        ...
        cmd = [self.ffmpeg_path, "-y", "-hwaccel", gpu_config.hwaccel, "-i", input_path]
        ...
        if gpu_config.encoder == GPUEncoder.NVIDIA_NVENC:
            ...
            cmd.extend(["-c:v", "h264_amf", "-rc", "cbr", "-quality", "balanced"])
        elif gpu_config.encoder == GPUEncoder.INTEL_QSV:
            cmd.extend(["-c:v", "h264_qsv", "-preset", gpu_config.preset, "-rc", "vbr"])
