...
class StreamingRequest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"prompt": self.prompt, "stream": True}
        if self.model:
...
class StreamAdapter(ABC):
    ...
    @abstractmethod
    async def stream(
        self, request: StreamingRequest, api_key: Optional[str] = None
...
class OpenAIStreamAdapter(StreamAdapter):
    ...
    async def stream(
        ...
        yield StreamingEvent(
            ...
            data={"request_id": request_id, "model": model, "provider": "openai"},
        )
                ...
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse OpenAI streaming data: {data_str}"
                            )
                            continue

            # Yield done event
            yield StreamingEvent(
                event_type=StreamingEventType.DONE, data={"request_id": request_id}
            )
...
class AnthropicStreamAdapter(StreamAdapter):
    ...
    async def stream(
        ...
        yield StreamingEvent(
            event_type=StreamingEventType.START,
            data={"request_id": request_id, "model": model, "provider": "anthropic"},
        )
                        ...
                        if line.startswith("data: "):
                            ...
                            try:
                                ...
                                if current_event_type == "content_block_delta":
                                    ...
                                    usage = data.get("usage", {})
                                    if usage:
                                        accumulated_usage["output_tokens"] = usage.get(
                                            "output_tokens", 0
                                        )
                                    ...
                                    if usage:
                                        accumulated_usage["input_tokens"] = usage.get(
                                            "input_tokens", 0
                                        )
                                    ...
                                    yield StreamingEvent(
                                        event_type=StreamingEventType.USAGE,
                                        data={
                                            "prompt_tokens": accumulated_usage[
                                                "input_tokens"
                                            ],
                                            "completion_tokens": accumulated_usage[
                                                "output_tokens"
                                            ],
                                            "total_tokens": accumulated_usage[
                                                "input_tokens"
                                            ]
                                ...
                                logger.warning(
                                    f"Failed to parse Anthropic streaming data: {data_str}"
                                )
                                continue
...
class OllamaStreamAdapter(StreamAdapter):
    ...
    async def stream(
        ...
        yield StreamingEvent(
            event_type=StreamingEventType.START,
            data={"request_id": request_id, "model": model, "provider": "ollama"},
        )
                ...
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse Ollama streaming data: {line}"
                            )
                            continue
...
class StreamManager:
    ...
    async def stream(
        self, request: StreamingRequest, provider: Optional[str] = None
    ) -> AsyncIterator[StreamingEvent]:
        """
    ...
    async def stream_with_stats(
        ...
        stats = StreamingStats(
            request_id=str(uuid.uuid4()), provider=selected_provider, model=model
        )
...
async def stream_llm_response(
    request: StreamingRequest, provider: Optional[str] = None
) -> AsyncIterator[str]:
    """
