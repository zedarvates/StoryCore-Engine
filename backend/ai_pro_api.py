    ...
    try:
        ...
        points = [
            SRP(
                time=p.time,
                speed=p.speed,
                curve_type=SpeedRampType(p.curve_type)
            )
...
async def generate_speed_curve(points: List[SpeedRampPoint], num_samples: int = 100):
    ...
    try:
        ...
        return {
            "curve": [{"time": t, "speed": s} for t, s in curve]
        }
...
async def list_detection_methods():
    ...
    return {
        "methods": [
            ...
            {"id": "content", "description": "Content-aware detection using frame differences"},
...
async def export_keyframes(json_path: str):
    ...
    try:
        ...
        return {
            "success": success,
            "path": json_path if success else None
        }
