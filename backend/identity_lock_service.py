...
class IdentityProfile:
    ...
    def from_dict(cls, data: Dict) -> 'IdentityProfile':
        ...
        return cls(
            ...
            visual_attributes=VisualAttributes.from_dict(data.get("visual_attributes", {})),
...
class IdentityLockService:
            ...
            if filename.endswith('.json'):
                try:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        logger.debug(f"Loaded identity: {identity.name} ({identity.id})")
    ...
    async def extract_and_lock_attributes(
        ...
        logger.info(f"Locked identity: {identity.name} with confidence {identity.visual_attributes.extraction_confidence}")
        return identity
    ...
    def _generate_base_prompt(self, identity: IdentityProfile) -> str:
        ...
        if attrs.distinctive_features:
            parts.append(f"distinctive features: {', '.join(attrs.distinctive_features)}")
    ...
    async def list_identities(self, project_id: Optional[str] = None, owner_id: Optional[str] = None) -> List[IdentityProfile]:
    ...
    async def get_identities_by_name(self, name: str, owner_id: Optional[str] = None) -> List[IdentityProfile]:
