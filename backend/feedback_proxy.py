...
try:
    ...
    if not settings.github_api_token:
        logger.warning(
            "GITHUB_API_TOKEN not set - GitHub features disabled. "
    ...
    settings = Settings()
    logger.warning(
        "Using default settings without GitHub token - GitHub features disabled"
    )
...
logger.info(
    f"Rate limiter initialized: {settings.rate_limit_threshold} requests per hour"
)
...
async def submit_report(
    ...
    if GEM_SYSTEM_AVAILABLE:
        try:
            ...
            logger.info(
                f"💎 Contributor identified: {contributor.contributor_type} (ID: {contributor.user_id})"
            )
    ...
    try:
        ...
        logger.info(
            f"Received report submission from {client_ip}: {payload_dict.get('report_type', 'unknown')}"
        )
        ...
        if content_length:
            try:
                ...
                is_valid_size, size_error = validate_raw_request_size(
                    content_length_int
                )
        ...
        try:
            migrated_payload, migration_notes = migrate_payload_to_current_version(
                payload_dict
            )

            # Log migration details if any changes were made
            if migration_notes and not any(
                "already at current version" in note.lower() for note in migration_notes
            ):
        ...
        if not is_valid_version:
            ...
            logger.warning(
                f"Unsupported schema version after migration: {schema_version}"
            )
            log_validation_error(
                field="schema_version",
                value=schema_version,
        ...
        logger.info(
            f"Payload size validation passed: {size_breakdown['total_mb']:.2f} MB"
        )
        ...
        if not is_valid_version:
            ...
            if "schema_version" not in payload_dict:
                logger.info(
                    "Schema version missing - attempting migration from Phase 1"
                )

                # Attempt to migrate the payload
                try:
                    migrated_payload, migration_notes = (
                        migrate_payload_to_current_version(payload_dict)
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Migrated payload failed Pydantic validation: {e}"
                        )
                        log_validation_error(
        ...
        if payload.screenshot_base64:
            is_valid_screenshot, screenshot_error = validate_screenshot_size(
            ...
            if not is_valid_screenshot:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=screenshot_error
                )
        ...
        module_name = (
            payload.module_context.active_module
            if payload.module_context
            else "unknown"
        )
        ...
        logger.info(
            f"  - Has stacktrace: {payload.diagnostics.stacktrace is not None if payload.diagnostics else False}"
        )
        logger.info(
            f"  - Has logs: {len(payload.diagnostics.logs) if payload.diagnostics and payload.diagnostics.logs else 0} lines"
        )
        ...
        if GEM_SYSTEM_AVAILABLE:
            try:
                ...
                is_agent = (
                    contributor.contributor_type == "automated_agent"
                    if contributor
                    else False
                )
                ...
                if dup_result.is_duplicate:
                    ...
                    if is_agent and dup_result.confidence > 0.95:
                        logger.warning(
                            f"🤖 Agent rejected: Strict duplicate detected ({dup_result.existing_issue_number})"
                        )
                        raise HTTPException(
        ...
        try:
            ...
            logger.info(
                f"Successfully created GitHub issue #{issue_number}: {issue_url}"
            )
            ...
            if GEM_SYSTEM_AVAILABLE and contributor and db:
                try:
                    ...
                    fingerprint = hashlib.sha256(
                        payload.user_input.description.strip().lower().encode()
                    ).hexdigest()
                    ...
                    logger.warning(
                        f"Failed to save contribution report (non-fatal): {e}"
                    )
            # ─────────────────────────────────────────────────────────────────────
...
async def retry_pending_report(report_id: str, request: Request):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Successfully created GitHub issue #{issue_number} for report {report_id}"
            )
            ...
            error_msg = str(e)
            logger.error(
                f"GitHub API error during retry of report {report_id}: {error_msg}"
            )
            ...
            error_msg = str(e)
            logger.error(
                f"Configuration error during retry of report {report_id}: {error_msg}"
            )
        ...
        logger.error(
            f"Unexpected error retrying report {report_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content={"status": "error", "message": exc.detail, "fallback_mode": "manual"},
    )
...
def main():
    ...
    if settings.github_api_token == "PLACEHOLDER_TOKEN":
        ...
        logger.warning(
            "Set the GITHUB_API_TOKEN environment variable to enable GitHub integration"
        )
        logger.warning("=" * 80)
