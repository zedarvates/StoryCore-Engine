...
class ScriptSegmenterService:
    ...
    EMOTIONAL_PATTERNS = {
        "tense": re.compile(
            r"\b(tension,urgent,danger,threat,panic,fear,crisis)\b", re.IGNORECASE
        ),
        "happy": re.compile(
            r"\b(joy,happy,laugh,smile,celebrate,wonderful,amazing)\b", re.IGNORECASE
        ),
        "sad": re.compile(
            r"\b(sad,tears,cry,grief,loss,mourn,depressed)\b", re.IGNORECASE
        ),
        "angry": re.compile(
            r"\b(angry,furious,rage,shout,scream,hate,resent)\b", re.IGNORECASE
        ),
        "romantic": re.compile(
            r"\b(love,romantic,kiss,embrace,passion,heart)\b", re.IGNORECASE
        ),
        "action": re.compile(
            r"\b(run,fight,chase,escape,attack,battle,action)\b", re.IGNORECASE
        ),
    }
    ...
    SCENE_TYPE_PATTERNS = {
        ...
        "transition": re.compile(
            r"\b(CUT TO|FADE TO|DISSOLVE TO|TIME LAPSE)\b", re.IGNORECASE
        ),
    }

    def __init__(
        self, words_per_minute: int = 150, storage_path: str = "./data/segments"
    ):
        """
        ...
        logger.info(
            f"ScriptSegmenterService initialized with {words_per_minute} WPM, storage: {storage_path}"
        )
    ...
    def find_natural_breaks(self, text: str) -> List[NaturalBreakPoint]:
        ...
        for scene_marker, pos in self.detect_scene_changes(text):
            context = text[
                max(0, pos - 20) : min(len(text), pos + len(scene_marker) + 20)
            ]
            breaks.append(
                NaturalBreakPoint(
                    position=pos,
    ...
    def segment_script(
        ...
        """
        logger.info(
            f"Starting segmentation for script {script_id}, target duration: {target_duration}s"
        )

        result = SegmentationResult(
            script_id=script_id, project_id=project_id, original_text=text
        )
        ...
        while current_start < len(text):
            ...
            target_ratio = (
                target_duration / remaining_duration if remaining_duration > 0 else 0.5
            )
            ...
            if best_break is None or best_break.position <= current_start:
                ...
                if sentence_match and sentence_match > current_start:
                    ...
                    if space_pos > current_start:
                        best_break = NaturalBreakPoint(
                            position=space_pos, break_type="forced", confidence=0.3
                        )
                    else:
            ...
            if segment_text:
                ...
                if duration < self.MIN_DURATION and segments:
                    ...
                    last_segment.duration_seconds = self.estimate_duration(
                        last_segment.text
                    )
                ...
                # Générer les prompts
                segment.visual_prompt, segment.audio_prompt = (
                    self.generate_prompts_for_segment(segment)
                )
        ...
        logger.info(
            f"Segmentation complete: {len(segments)} segments, {result.total_duration:.1f}s total"
        )

        return result

    def generate_prompts_for_segment(
        self, segment: ScriptSegment, context: str = ""
    ) -> Tuple[str, str]:
        """
    ...
    def _generate_optimization_suggestions(
        ...
        short_segments = [
            s for s in result.segments if s.duration_seconds < self.MIN_DURATION
        ]
        long_segments = [
            s for s in result.segments if s.duration_seconds > self.MAX_DURATION
        ]
    ...
    def adjust_segment(
        ...
        if action == "split":
            ...
            if first_text and second_text:
                ...
                # Générer les prompts
                first_segment.visual_prompt, first_segment.audio_prompt = (
                    self.generate_prompts_for_segment(first_segment)
                )
                second_segment.visual_prompt, second_segment.audio_prompt = (
                    self.generate_prompts_for_segment(second_segment)
                )
