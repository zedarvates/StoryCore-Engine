...
from backend.database_models import Episode
...
@router.post(
    "/episodes", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED
)
...
async def list_project_episodes(project_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [
        {"id": ep.id, "title": ep.title, "number": ep.number, "status": ep.status}
        for ep in episodes
    ]


@router.get("/episodes/{episode_id}", response_model=Dict[str, Any])
async def get_episode_details(episode_id: str, db: AsyncSession = Depends(get_db)):
