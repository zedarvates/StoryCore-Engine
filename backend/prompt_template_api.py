...
class PromptVariableModel(BaseModel):
    ...
    type: str = Field(default="string", description="Type de variable (string, number, list, json)")
...
class PromptTemplateCreate(BaseModel):
    ...
    category: str = Field(default="video", description="Catégorie (video, voice, character, scene, seo, identity)")
    layer: str = Field(default="master", description="Couche (foundation, execution, master)")
    template: str = Field(..., description="Contenu du template avec variables {variable_name}")
    variables: List[PromptVariableModel] = Field(default_factory=list, description="Variables du template")
...
class RenderRequest(BaseModel):
    ...
    variables: Dict[str, Any] = Field(default_factory=dict, description="Variables pour le rendu")
...
async def list_layers():
    ...
    return {
        "success": True,
        "layers": [
            {"value": l.value, "name": l.name, "description": {
...
async def render_template(template_id: str, request: RenderRequest):
    ...
    if not result.get("success"):
        ...
        raise HTTPException(status_code=400, detail=result.get("error", "Erreur de rendu"))
...
async def compose_templates(request: ComposeRequest):
    ...
    if not result.get("success"):
        if "non trouvé" in result.get("error", ""):
            raise HTTPException(status_code=404, detail=result["error"])
        raise HTTPException(status_code=400, detail=result.get("error", "Erreur de composition"))
    
    return result


@router.post("/duplicate/{template_id}")
async def duplicate_template(template_id: str, new_name: str = Query(..., description="Nouveau nom du template")):
    """
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Template dupliqué avec succès",
