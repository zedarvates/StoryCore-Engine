...
async def submit_api_task(job_id: str, payload: dict, priority: int = 5, timeout_seconds: int = 300) -> bool:
    """
...
async def get_task_queue_status(user_id: str = Depends(verify_jwt_token)) -> QueueStatusResponse:
...
async def submit_api_task_route(req: APITaskRequest, user_id: str = Depends(verify_jwt_token)) -> APIQueueResponse:
    ...
    raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Submission to AsyncTaskQueue failed")

@router.get("/tasks/api/{job_id}", response_model=APITaskStatusResponse)
async def get_api_task_status_route(job_id: str, user_id: str = Depends(verify_jwt_token)) -> APITaskStatusResponse:
    """Retrieve status for an API-driven task submitted to AsyncTaskQueue."""
    status_dict = await get_async_task_status(job_id)
    if not status_dict:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="API task not found")
