...
async def extract_frames(
    request: FrameExtractionRequest, user_id: str = Depends(verify_jwt_token)
):
    """
    ...
    if not os.path.exists(video_path):
        ...
        clean_filename = request.video_filename.replace("/output/", "").replace(
            "\\output\\", ""
        )
    ...
    if not os.path.exists(video_path):
        raise HTTPException(
            status_code=404, detail=f"Video not found: {request.video_filename}"
        )
    ...
    return {"job_id": job_id, "frames": sorted(frames)}
