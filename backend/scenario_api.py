...
async def transform_story(
    request: ScenarioTransformRequest, user_id: str = Depends(verify_jwt_token)
    ...
    try:
        ...
        scenario = transform_story_to_scenario(
            request.story, request.title, request.mode
        )
...
async def get_scenario(
    scenario_id: str, user_id: str = Depends(verify_jwt_token)
) -> ScenarioResponse:
    """
    Get scenario by ID.
...
async def export_scenario(
    ...
    return ScenarioExportResponse(
        format=format.value, content=content, filename=filename
    )
...
async def list_scenarios(
    page: int = 1, page_size: int = 20, user_id: str = Depends(verify_jwt_token)
) -> Dict[str, Any]:
    """
    ...
    scenarios = [
        {"id": k, **v}
        for k, v in scenario_storage.cache.items()
