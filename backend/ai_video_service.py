...
class CharacterSheetConfig:
    ...
    character_id: str
    character_name: str
    views: List[CharacterView] = field(
    ...
    expressions: List[str] = field(
        default_factory=lambda: ["neutral", "happy", "angry", "surprised"]
    )
    outfit: str = "default"
    style: str = "realistic"
    resolution: int = 1024
...
class SmartCropService:
    ...
    def detect_faces(
        self, frame: np.ndarray, min_face_size: int = 50
    ) -> List[Dict[str, Any]]:
        """
            ...
            while cap.isOpened():
                ...
                results.append(
                    FaceTrackingResult(
                        ...
                        confidence=faces[main_face_index]["confidence"]
                        if faces
                        else 0.0,
                    )
                )
    ...
    def calculate_crop_regions(
        ...
        for result in tracking_results:
            ...
            # Apply smoothing
            center_x = int(
                prev_center[0] + smoothing * (target_center_x - prev_center[0])
            )
            center_y = int(
                prev_center[1] + smoothing * (target_center_y - prev_center[1])
            )
    ...
    def apply_smart_crop_ffmpeg(
        ...
        try:
            ...
            cmd = [
                ...
                input_path,
                "-vf",
                f"crop={crop_data[0][2]}:{crop_data[0][3]}:{crop_data[0][0]}:{crop_data[0][1]}",
                "-c:v",
                "libx264",
                "-preset",
                "fast",
...
class MultiAngleService:
    ...
    def generate_multi_angle_prompts(
        ...
        for angle in angles:
            ...
            negative = (
                "blurry, low quality, distorted, deformed, bad composition, amateur"
            )
    ...
    def get_angle_sequence(self, scene_type: str = "dialogue") -> List[CameraAngle]:
        """
    ...
    def generate_shot_list(
        self, scene_description: str, scene_type: str = "dialogue", num_shots: int = 5
    ) -> Dict[str, Any]:
        """
...
class CharacterConsistencyService:
    ...
    def generate_character_sheet_prompts(
        self, character_description: str, config: CharacterSheetConfig = None
        ...
        config = config or CharacterSheetConfig(
            character_id="default", character_name="Character"
        )
        ...
        for view in config.views:
            ...
            full_prompt = (
                f"{character_description}, {view_prompt}, {config.style} style"
            )
            full_prompt += ", character sheet, reference image, consistent design"
            full_prompt += f", {config.outfit} outfit"

            negative = (
                "blurry, low quality, inconsistent, different face, altered features"
            )
    ...
    def create_consistency_prompt(
        self, base_prompt: str, reference_image_path: str, strength: float = 0.8
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "controlnet_config": {
                ...
                if "pose" in base_prompt.lower()
                else "control_v11p_sd15_canny",
    ...
    def generate_turnaround_prompt(
        self, character_description: str, num_views: int = 8, style: str = "realistic"
    ) -> str:
        """
    ...
    def create_smooth_cut(
        ...
        try:
            ...
            if method == "crossfade":
                ...
                # Simple blend
                filter_complex = f"[0:v][1:v]blend=all_expr='A*(1-T/{transition_duration})+B*(T/{transition_duration})'[vout]"
