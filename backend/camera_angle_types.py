...
class CameraAngleRequest(BaseModel):
    ...
    angle_ids: Optional[List[CameraAnglePreset]] = Field(
        default=None, description="List of camera angle presets (Legacy)"
    )
    granular_angles: Optional[List[GranularAngle]] = Field(
        default=None, description="List of specific granular angles to generate"
    )
    ...
    quality: str = Field(
        default="standard", description="Generation quality: draft, standard, or high"
    )
    seed: Optional[int] = Field(
        default=None, description="Random seed for reproducible generation"
    )
    custom_prompt: Optional[str] = Field(
        default=None, description="Additional custom prompt to append to angle prompts"
    )
    lens_type: LensType = Field(
        default=LensType.STANDARD, description="Type of lens/objective to simulate"
    )
    comfyui_url: Optional[str] = Field(
        default=None, description="Optional ComfyUI server URL to use for this request"
    )
...
class CameraAngleJob(BaseModel):
    ...
    angle_ids: List[CameraAnglePreset] = Field(
        default=[], description="Angles to generate (presets)"
    )
    granular_angles: List[GranularAngle] = Field(
        default=[], description="Granular angles to generate"
    )
    ...
    comfyui_url: Optional[str] = Field(
        default=None, description="ComfyUI server URL used for this job"
    )
    status: CameraAngleJobStatus = Field(
        default=CameraAngleJobStatus.PENDING, description="Current job status"
    )
    ...
    completed_angles: List[str] = Field(
        default=[], description="Completed angle generations (IDs or strings)"
    )
    remaining_angles: List[str] = Field(
        default=[], description="Remaining angle generations"
    )
...
class CameraAngleResultResponse(BaseModel):
    ...
    results: List[CameraAngleResult] = Field(
        default=[], description="List of generated results"
    )
    total_generation_time: float = Field(
        default=0.0, description="Total generation time in seconds"
    )
