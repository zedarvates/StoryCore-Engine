...
class StoryboardGenerator:
    ...
    def _ensure_default_workflow(self):
        ...
        if not os.path.exists(self.default_workflow_path):
            workflow = {
                ...
                "9": {
                    "inputs": {"filename_prefix": "storyboard", "images": ["8", 0]},
    ...
    async def _generate_scene_image(
        self, scene: StoryScene, index: int, story: Story, semaphore: asyncio.Semaphore
    ) -> Dict[str, Any]:
        async with semaphore:
            ...
            style_suffix = (
                ", high quality concept art, cinematic lighting, 16:9 aspect ratio"
            )
            if story.mode.name == "RENOVATION":
                ...
                style_suffix = (
                    ", garden design, landscape architecture, lush greenery, realistic"
                )
