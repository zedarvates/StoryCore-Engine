...
from backend.feedback_story_integration import (
    StoryFeedback,
    StoryFeedbackIntegrationService,
    FeedbackAnalytics,
...
RETRAINING_IN_PROGRESS = Gauge(
    'storycore_retraining_in_progress',
    'Whether a retraining is currently in progress'
)

RETRAINING_DURATION = Histogram(
    'storycore_retraining_duration_seconds',
    'Duration of model retraining operations'
)
...
# Quality metrics
QUALITY_SCORE = Gauge(
    'storycore_quality_score',
    'Current quality score based on feedback',
    ['dimension']
)

USER_SATISFACTION = Gauge(
    'storycore_user_satisfaction',
...
class PatternAnalyzer:
    ...
    async def analyze_feedback(self, feedbacks: List[StoryFeedback]) -> List[FailurePattern]:
        """
        ...
        for fb_type, type_feedbacks in by_type.items():
            ...
            if pattern:
                ...
                PATTERNS_DETECTED.labels(
                    ...
                    severity='high' if pattern.severity > 0.7 else 'medium' if pattern.severity > 0.4 else 'low'
                ).inc()
        
        # Update active pattern gauge
        for pattern_type in FailureType:
            count = sum(
                1 for p in self.detected_patterns.values()
    ...
    async def _analyze_type(self, fb_type: str, feedbacks: List[StoryFeedback]) -> Optional[FailurePattern]:
        ...
        pattern = FailurePattern(
            ...
            failure_type=FailureType(fb_type) if fb_type in [t.value for t in FailureType] else FailureType.UNKNOWN,
    ...
    def _generate_fix_suggestion(self, fb_type: str, feedbacks: List[StoryFeedback]) -> str:
    ...
    async def get_patterns_by_severity(self, min_severity: float = 0.0) -> List[FailurePattern]:
...
class ModelRetrainer:
    ...
    async def check_and_trigger_retraining(
        ...
        recent_events = [
            e for e in self.retraining_events.values()
            if e.status == "completed" and
            (datetime.now() - e.end_time).total_seconds() < self._retrain_cooldown_hours * 3600
        ]
    ...
    async def _execute_retraining(
        ...
        try:
            ...
            logger.info(f"Retraining completed: {event.event_id}, duration: {duration}s")
...
class MetricsCollector:
    ...
    async def record_metric(self, metric_type: MetricType, value: float, labels: Optional[Dict[str, str]] = None) -> None:
        """
        ...
        if len(self._metric_history[metric_type.value]) > self._history_size:
            self._metric_history[metric_type.value] = self._metric_history[metric_type.value][-self._history_size:]
...
class FeedbackLoopManager:
    ...
    async def trigger_manual_retraining(self, reason: str = "Manual trigger") -> RetrainingEvent:
        """
...
async def integrate_with_story_feedback():
    ...
    analytics = await story_feedback_service.get_feedback_analytics()
