...
class CUDAAgent:
    ...
    def optimize_environment(self):
        ...
        if not self.is_cuda:
            logger.warning(
                "CUDA Agent: GPU non-NVidia ou absent. Optimisation restreinte."
            )
            return
        ...
        if torch.cuda.get_device_capability()[0] >= 8:
            ...
            logger.info(
                "CUDA Agent: TF32 Precision enabled (Ampere architecture detected)"
            )
        ...
        if hasattr(torch.nn.functional, "scaled_dot_product_attention"):
            logger.info(
                "CUDA Agent: SDPA (Flash Attention) confirmed for high-throughput video."
            )
