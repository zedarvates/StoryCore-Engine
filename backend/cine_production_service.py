...
from src.addon_manager import AddonManager, AddonState
...
class CineChainType(Enum):
    GENERATE_SCENE = (
        "generate_scene"  # None/One character scene (Storyboard -> Wan 2.1 -> Audio)
    )
    SPEAKING_CHARACTER = (
        "speaking_character"  # Character speaking (Audio -> Wan 2.1 LipSync)
    )
...
class CineProductionRequest(BaseModel):
    ...
    character_image_path: Optional[str] = Field(
        default=None, alias="characterImagePath"
    )
...
class CineProductionService:
    ...
    def __init__(self, comfyui_url: str = None):
        ...
        try:
            ...
            self._addon_init_task = asyncio.create_task(
                self.addon_manager.initialize_all_addons()
            )
        except RuntimeError:
        ...
        logger.info(
            f"CineProductionService initialized with ComfyUI at {self.comfyui_url}"
        )
    ...
    async def start_production_job(
        self, request: CineProductionRequest, user_id: str = None
    ) -> str:
    ...
    async def _process_job(self, job_id: str):
        ...
        if job.user_id:
            ...
            if cost > 0:
                ...
                logger.info(
                    f"Creating escrow for job {job_id} (Type: {task_type}): {cost} gems from {job.user_id}"
                )
        ...
        try:
            ...
            if not job.request.scene_description and job.request.scene_id:
                job.request.scene_description = await self._get_scene_description(
                    job.request.project_id, job.request.scene_id
                )
            ...
            use_addon = False
            if (
                job.request.preferred_engine
                and job.request.preferred_engine.lower() != "comfyui"
            ):
                addon_info = self.addon_manager.get_addon_info(
                    job.request.preferred_engine
                )
                # Fallback check
                if not addon_info:
                ...
                if addon_info and addon_info.state == AddonState.ENABLED:
                    use_addon = True
                    logger.info(
                        f"Using addon engine {job.request.preferred_engine} - skipping standard chains"
                    )
            ...
            if job.escrow_id:
                logger.warning(
                    f"Refunding gems for failed job {job_id} (escrow {job.escrow_id})"
                )
                await gem_client.cancel_escrow(job.escrow_id)
        finally:
    ...
    async def _run_visual_director_phase(
        self, job: CineProductionJob, context: Dict[str, Any]
    ):
        ...
        from backend.llm_api import LLMRequest

        # Priority: Request overrides > Project Context > Defaults
        genre = (
            job.request.genre
            or context.get("genre")
            or context.get("metadata", {}).get("genre", "Cinematic")
        )
        style = (
            job.request.style
            or context.get("style")
            or context.get("metadata", {}).get("style", "Photorealistic")
        )
        tone = (
            job.request.tone
            or context.get("tone")
            or context.get("metadata", {}).get("tone", "Dramatic")
        )
        ...
        try:
            ...
            if not job.request.image_prompt:
                job.request.image_prompt = prompts.get("image_prompt")
            if not job.request.video_prompt:
                job.request.video_prompt = prompts.get("video_prompt")
            if not job.request.audio_prompt:
                job.request.audio_prompt = prompts.get("audio_prompt")

            logger.info(f"Visual Director expanded prompts for job {job.id}")

        except Exception as e:
            logger.warning(
                f"Visual Director failed: {e}. Falling back to default prompts."
            )
    ...
    async def _get_project_context(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            scenario = storage_scenario.load(
                project_id
            )  # Sometimes project_id = scenario_id
            if scenario:
    ...
    async def _get_scene_description(
    ...
    async def _run_scene_generation_chain(self, job: CineProductionJob):
        ...
        storyboard_result = await self._execute_workflow(
            ...
            {"image_prompt": job.request.image_prompt, "seed": job.request.seed},
        ...
        )  # Re-fetch or pass from _process_job
    ...
    async def _run_ltx_video_generation_chain(self, job: CineProductionJob):
        ...
        video_result = await self._execute_workflow(
            ...
            {
                "positive": job.request.video_prompt
                or job.request.image_prompt
        ...
        if job.request.audio_prompt:
            ...
            audio_result = await self._execute_workflow(
                "ace_step",
                self.WORKFLOW_PATHS["ace_step"],
                {"audio_input": audio_context, "image": None},
                comfyui_url=job.comfyui_url,
            )
            job.results.append({"step": "audio", "output": audio_result})
            await self._mux_results(job, video_result, audio_result)
    ...
    async def _run_tts_speaking_character_chain(self, job: CineProductionJob):
        ...
        tts_result = await tts_service.text_to_speech(
            text=text, voice=voice_id, instruct=job.request.tone or "neutral"
        )
    ...
    async def _run_speaking_character_chain(self, job: CineProductionJob):
        ...
        audio_input = (
            job.request.audio_prompt
        )  # In S2V, sometimes audio_prompt is a filename
        if not audio_input:
        ...
        if not ref_image and job.results:
            ...
            story_res = next(
                (r for r in job.results if r["step"] == "storyboard"), None
            )
            if story_res:
        ...
        video_result = await self._execute_workflow(
            ...
            {
                "positive": job.request.video_prompt
                or "character speaking directly to camera, cinematic",
                "image": ref_image,
                "audio": audio_input,
            },
            comfyui_url=job.comfyui_url,
    ...
    def _build_audio_context_prompt(
        self, job: CineProductionJob, context: Dict[str, Any]
    ) -> str:
        ...
        if job.request.scene_description:
            ...
            sfx_matches = re.findall(
                r"\[(\d+s\s*-\s*\d+s)\]:.*?\[SFX:\s*(.*?)\]",
                job.request.scene_description,
            )
            if sfx_matches:
    ...
    async def _run_audio_remix_chain(self, job: CineProductionJob):
        ...
        audio_result = await self._execute_workflow(
            "ace_step",
            self.WORKFLOW_PATHS["ace_step"],
            {"audio_input": audio_context, "image": ref_image},
            comfyui_url=job.comfyui_url,
        )
        job.results.append({"step": "audio", "output": audio_result})
    ...
    async def _execute_workflow(
        ...
        result = await comfyui_executor.execute_workflow(
            workflow=workflow_api, comfyui_url=comfyui_url or self.comfyui_url
        )
        ...
        return {
            "outputs": {
                out["node_id"]: {
                    "images": [{"filename": out["filename"], "type": "output"}]
                }
    ...
    def _convert_ui_to_api(self, workflow_ui: Dict[str, Any]) -> Dict[str, Any]:
        ...
        links = {
            link[0]: (str(link[1]), link[2]) for link in workflow_ui.get("links", [])
        }
        ...
        for node in nodes:
            ...
            workflow_api[node_id] = {"class_type": node["type"], "inputs": inputs}

        return workflow_api

    async def _update_shot_metadata(
        self, job: CineProductionJob, results: List[Dict[str, Any]]
    ):
        ...
        try:
            ...
            for res in results:
                ...
                if step == "storyboard":
                    ...
                    if not shot.get("result_url") or "final_" not in shot.get(
                        "result_url"
                    ):
    ...
    def _apply_overrides(
        self, workflow_name: str, workflow: Dict[str, Any], overrides: Dict[str, Any]
    ):
    ...
    async def _free_memory(
        self, session: aiohttp.ClientSession, comfyui_url: str = None
    ):
        """Calls ComfyUI's /free endpoint to release GPU memory."""
        url = comfyui_url or self.comfyui_url
        try:
            async with session.post(
                f"{url}/free", json={"unload_models": True, "free_memory": True}
            ) as resp:
                if resp.status == 200:
                ...
                if resp.status == 200:
                    ...
                    if prompt_id in history:
                        ...
                        if all_files:
                            ...
                            main_result = next(
                                (f for f in all_files if f["type"] == "video"),
                                next(
                                    (f for f in all_files if f["type"] == "image"),
                                    all_files[0],
                                ),
                            )
    ...
    async def _download_from_comfyui(
        self, filename: str, file_type: str = "output", comfyui_url: str = None
    ):
            ...
            try:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        ...
                        logger.error(
                            f"Failed to download {filename}: HTTP {resp.status}"
                        )
                        return None
            except Exception as e:
    ...
    async def _mux_results(
        ...
        video_local = await self._download_from_comfyui(
            video_filename, comfyui_url=job.comfyui_url
        )
        audio_local = await self._download_from_comfyui(
            audio_filename, comfyui_url=job.comfyui_url
        )
        ...
        success, error = self.ffmpeg.merge_audio_video(
            video_local, audio_local, output_path, replace_audio=True
        )
    ...
    async def _run_addon_generation(self, job: CineProductionJob):
        ...
        if not addon_info or addon_info.state != AddonState.ENABLED:
            logger.warning(
                f"Addon engine {engine_name} not found or not enabled. Falling back."
            )
            return
        ...
        try:
            ...
            if hasattr(addon_info.module, "addon") and addon_info.module.addon:
                ...
                result = await addon_instance.generate(
                    scene_data, config_overrides=overrides
                )

                if result and result.get("status") == "success":
                    # Handle video or image results
                    res_path = result.get("video") or (
                    ...
                    if res_path:
                        ...
                        if os.path.abspath(res_path) != os.path.abspath(
                            target_path
                        ...
                        job.progress = 100.0
                        logger.info(
                            f"Addon {engine_name} successfully generated {res_type}"
                        )
                    else:
                        logger.error(
                            f"Addon {engine_name} success but no file path returned"
                        )
                else:
                    ...
                    job.results.append(
                        {
                            "step": "error",
                            "output": f"Addon {engine_name} error: {error_msg}",
                        }
                    )
            else:
                logger.error(
                    f"Addon {engine_name} module does not have 'addon' instance"
                )

        except Exception as e:
            logger.error(f"Exception in addon {engine_name} generation: {e}")
            job.results.append(
                {"step": "error", "output": f"Addon exception: {str(e)}"}
            )
